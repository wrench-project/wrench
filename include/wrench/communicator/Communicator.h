...
public:
  ...
  void barrier();
  void sendAndReceive(const std::map<unsigned long, double> &sends,
                      int num_receives);
  void sendReceiveAndCompute(const std::map<unsigned long, double> &sends,
                             int num_receives, double flops);
...
private:
  ...
  void performSMPIOperation(const std::string &op_name,
                            ...
                            simgrid::s4u::Host *root_hosts, int bytes,
