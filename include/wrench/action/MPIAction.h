...
protected:
  ...
  MPIAction(const std::string &name, unsigned long num_processes,
            unsigned long num_cores_per_process,
            std::function<
                void(const std::shared_ptr<ExecutionController> &controller)>
                lambda_mpi);
...
private:
  ...
  public:
    MPIPrivateExecutionController(const std::string &hostname,
                                  const std::string &suffix)
        : ExecutionController(hostname, suffix) {}
  ...
  class MPIProcess {
  public:
    MPIProcess(std::function<
                   void(const std::shared_ptr<ActionExecutor> &action_executor)>
                   lambda_mpi,
               simgrid::s4u::BarrierPtr barrier,
               std::shared_ptr<ActionExecutor> action_executor)
        : lambda_mpi(std::move(lambda_mpi)), barrier(barrier),
  ...
  private:
    std::function<void(const std::shared_ptr<ActionExecutor> &action_executor)>
        lambda_mpi;
    simgrid::s4u::BarrierPtr barrier;
    std::shared_ptr<ActionExecutor> action_executor;
  };
