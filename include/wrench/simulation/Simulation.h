...
/**
 * @brief A class that provides basic simulation methods.  Once the simulation
 * object has been explicitly or implicitly destroyed, then any call to the
 * WRENCH APIs has undefied behavior (due to memory_manager_service being
 * de-allocated).
 */
class Simulation {
public:
  ...
  static bool hostHasMountPoint(const std::string &hostname,
   ...
   */
  template <class T> std::shared_ptr<T> add(T *t) {
   ...
   * @return a shared_ptr to the service object
   */
  template <class T> std::shared_ptr<T> add(std::shared_ptr<T> t) {
    this->addService(t);
    return t;
  }
  ...
  getEnergyConsumed(const std::vector<std::string> &hostnames);
  //        double getEnergyTimestamp(const std::string &hostname, bool
  //        can_record = false);
  ...
  /**
   * @brief Creates a file copy on a storage service before the simulation
   * begins
   * @param file: a file
   * @param storage_service: a storage service
   */
  void stageFile(const std::shared_ptr<DataFile> file,
  ...
  void stageFile(const std::shared_ptr<DataFile> file,
    ...
    this->stageFile(
        wrench::FileLocation::LOCATION(storage_service, path, file));
  }
  ...
  static std::vector<std::string> getRoute(std::string &src_host,
                                           std::string &dst_host);
  ...
  static void createNewDisk(const std::string &hostname,
  ...
  startNewService(NetworkProximityService *service);
  std::shared_ptr<FileRegistryService>
  startNewService(FileRegistryService *service);
#ifdef PAGE_CACHE_SIMULATION
  ...
  /** \cond INTERNAL     */
  /***********************/
  void readFromDisk(double num_bytes, const std::string &hostname,
                    const std::string &mount_point, simgrid::s4u::Disk *disk);
  void readFromDiskAndWriteToDiskConcurrently(
  ...
  void writeToDisk(double num_bytes, const std::string &hostname,
                   const std::string &mount_point, simgrid::s4u::Disk *disk);

#ifdef PAGE_CACHE_SIMULATION
  void readWithMemoryCache(const std::shared_ptr<DataFile> &file,
                           double n_bytes,
                           const std::shared_ptr<FileLocation> &location);
  void writebackWithMemoryCache(const std::shared_ptr<DataFile> &file,
                                double n_bytes,
                                const std::shared_ptr<FileLocation> &location,
                                bool is_dirty);
  void
  writeThroughWithMemoryCache(const std::shared_ptr<DataFile> &file,
                              double n_bytes,
                              const std::shared_ptr<FileLocation> &location);
