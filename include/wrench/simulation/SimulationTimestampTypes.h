...
typedef std::tuple<std::shared_ptr<wrench::DataFile>,
...
namespace std {
template <> struct hash<FileCopy> {
public:
  size_t operator()(const FileCopy &file) const {
    return std::hash<void *>()(std::get<0>(file).get()) ^
           std::hash<void *>()(std::get<1>(file).get()) ^
           std::hash<void *>()(std::get<2>(file).get());
    //            return std::hash<void *>()(file.file.get()) ^ std::hash<void
    //            *>()(file.src.get()) ^ std::hash<void *>()(file.dst.get());
  }
};

template <> struct hash<FileReadWrite> {
public:
...
template <> struct hash<DiskAccess> {
public:
  size_t operator()(const DiskAccess &diskAccess) const {
    return std::hash<std::string>()(std::get<0>(diskAccess)) ^
           std::hash<std::string>()(std::get<1>(diskAccess)) ^
           std::hash<int>()(std::get<2>(diskAccess));
  }
};
}; // namespace std
...
/**
 * @brief File, Source, Whoami used to be hashed as key for unordered multimap
 * for ongoing file operations.
 */
/// typedef std::tuple<std::shared_ptr<DataFile>, std::shared_ptr<FileLocation>
/// , std::shared_ptr<StorageService> > File;
...
protected:
  static std::map<std::string, SimulationTimestampTask *>
      pending_task_timestamps;
  void setEndpoints();
  SimulationTimestampTask(double date,
...
private:
  friend class SimulationOutput;
  SimulationTimestampTaskStart(double date,
                               const std::shared_ptr<WorkflowTask> &task);
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampTaskFailure(double date,
                                 const std::shared_ptr<WorkflowTask> &task);
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampTaskCompletion(double date,
                                    const std::shared_ptr<WorkflowTask> &task);
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampTaskTermination(double date,
                                     const std::shared_ptr<WorkflowTask> &);
};
...
protected:
   ...
   */
  /// static std::unordered_multimap<File, std::pair<SimulationTimestampFileRead
  /// *, double>, decltype(&file_hash)> pending_file_reads;
  static std::unordered_multimap<
      FileReadWrite,
      std::pair<SimulationTimestampFileRead *, std::shared_ptr<WorkflowTask>>>
      pending_file_reads;

  void setEndpoints();
  friend class SimulationOutput;
  SimulationTimestampFileRead(double date, std::shared_ptr<DataFile> file,
 ...
 */
class SimulationTimestampFileReadCompletion
    : public SimulationTimestampFileRead {
private:
  friend class SimulationOutput;
  SimulationTimestampFileReadCompletion(
      double date, const std::shared_ptr<DataFile> &file,
      const std::shared_ptr<FileLocation> &src,
      const std::shared_ptr<StorageService> &service,
      std::shared_ptr<WorkflowTask> task = nullptr);
...
protected:
   ...
   */
  /// static std::unordered_multimap<File,
  /// std::pair<SimulationTimestampFileWrite *, double>, decltype(&file_hash)>
  /// pending_file_writes;
  static std::unordered_multimap<
      FileReadWrite,
      std::pair<SimulationTimestampFileWrite *, std::shared_ptr<WorkflowTask>>>
      pending_file_writes;

  void setEndpoints();

  friend class SimulationOutput;
  SimulationTimestampFileWrite(double date, std::shared_ptr<DataFile> file,
 ...
 */
class SimulationTimestampFileWriteFailure
    : public SimulationTimestampFileWrite {
private:
  friend class SimulationOutput;
...
protected:
  SimulationTimestampFileCopy(double date, std::shared_ptr<DataFile> file,
                              std::shared_ptr<FileLocation> src,
                              std::shared_ptr<FileLocation> dst);
   ...
   * @brief the data structure that holds the ongoing file writes.
   */
  static std::unordered_multimap<FileCopy, SimulationTimestampFileCopy *>
      pending_file_copies;
...
private:
  ...
  SimulationTimestampFileCopyStart(double date, std::shared_ptr<DataFile> file,
                                   std::shared_ptr<FileLocation> src,
                                   std::shared_ptr<FileLocation> dst);
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampFileCopyFailure(double date,
                                     std::shared_ptr<DataFile> file,
                                     std::shared_ptr<FileLocation> src,
                                     std::shared_ptr<FileLocation> dst);
};
 ...
 */
class SimulationTimestampFileCopyCompletion
    : public SimulationTimestampFileCopy {
private:
  friend class SimulationOutput;
  SimulationTimestampFileCopyCompletion(double date,
                                        std::shared_ptr<DataFile> file,
                                        std::shared_ptr<FileLocation> src,
                                        std::shared_ptr<FileLocation> dst);
};
...
protected:
   ...
   */
  static std::unordered_multimap<DiskAccess, SimulationTimestampDiskRead *>
      pending_disk_reads;

  void setEndpoints();
  friend class SimulationOutput;
  SimulationTimestampDiskRead(double date, std::string hostname,
                              std::string mount, double bytes, int counter);
};
...
public:
  friend class SimulationOutput;
  SimulationTimestampDiskReadStart(double date, std::string hostname,
                                   std::string mount, double bytes,
                                   int counter);
...
private:
  friend class SimulationOutput;
  SimulationTimestampDiskReadFailure(double date, const std::string &hostname,
                                     const std::string &mount, double bytes,
                                     int counter);
};
 ...
 */
class SimulationTimestampDiskReadCompletion
    : public SimulationTimestampDiskRead {
private:
  friend class SimulationOutput;
  SimulationTimestampDiskReadCompletion(double date,
...
protected:
   ...
   */
  static std::unordered_multimap<DiskAccess, SimulationTimestampDiskWrite *>
      pending_disk_writes;

  void setEndpoints();
  friend class SimulationOutput;
  SimulationTimestampDiskWrite(double date, std::string hostname,
                               std::string mount, double bytes, int counter);
};
...
public:
  friend class SimulationOutput;
  SimulationTimestampDiskWriteStart(double date, std::string hostname,
                                    std::string mount, double bytes,
                                    int counter);
 ...
 */
class SimulationTimestampDiskWriteFailure
    : public SimulationTimestampDiskWrite {
private:
  friend class SimulationOutput;
  SimulationTimestampDiskWriteFailure(double date, const std::string &hostname,
                                      const std::string &mount, double bytes,
                                      int counter);
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampDiskWriteCompletion(double date,
                                         const std::string &hostname,
                                         const std::string &mount, double bytes,
                                         int counter);
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampPstateSet(double date, const std::string &hostname,
                               int pstate);
  std::string hostname;
  int pstate;
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampEnergyConsumption(double date, const std::string &hostname,
                                       double joules);
  std::string hostname;
  double joules;
};
...
private:
  friend class SimulationOutput;
  SimulationTimestampLinkUsage(double date, const std::string &linkname,
