...
public:
   ...
   */
  std::vector<SimulationTimestamp<T> *> getTrace() { return this->trace; }
...
/**
 * @brief A specialized class to represent a trace of
 * SimulationTimestampPstateSet timestamps
 */
template <>
class SimulationTrace<SimulationTimestampPstateSet>
    : public GenericSimulationTrace {
public:
  /**
   ...
   */
  void addTimestamp(
      SimulationTimestamp<SimulationTimestampPstateSet> *new_timestamp) {
    ...
    if (hostname_search == latest_timestamps_by_host.end()) {
      ...
      SimulationTimestamp<SimulationTimestampPstateSet> *&latest_timestamp =
          this->trace[this->latest_timestamps_by_host[hostname]];

      // if the new_timestamp has the same date as the latest_timestamp, then
      // the new_timestamp replaces the latest_timestamp in the trace, else the
      // new time_stamp is added to the trace and the map of latest timestamps
      // is updated to reflect this change
      if (std::fabs(new_timestamp->getDate() - latest_timestamp->getDate()) <
          DBL_EPSILON) {
        ...
        if (new_timestamp->getDate() > latest_timestamp->getDate()) {
          ...
          throw std::runtime_error(
              "SimulationTrace<SimulationTimestampPstateSet>::addTimestamp() "
  ...
  /**
   * @brief Retrieve the trace as a vector of
   * SimulationTimestamp<SimulationTimestampPstateSet> timestamps
   * @return a vector of pointers to
   * SimulationTimestamp<SimulationTimestampPstateSet> objects
   */
