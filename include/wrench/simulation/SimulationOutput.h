...
public:
  ...
  void dumpWorkflowExecutionJSON(const std::shared_ptr<Workflow> &workflow,
                                 const std::string &file_path,
                                 bool generate_host_utilization_layout = false,
                                 bool writing_file = true);

  void dumpWorkflowGraphJSON(const std::shared_ptr<Workflow> &workflow,
                             const std::string &file_path,
                             bool writing_file = true);

  void dumpHostEnergyConsumptionJSON(const std::string &file_path,
                                     bool writing_file = true);

  void dumpPlatformGraphJSON(const std::string &file_path,
                             bool writing_file = true);

  void dumpDiskOperationsJSON(const std::string &file_path,
                              bool writing_file = true);

  void dumpLinkUsageJSON(const std::string &file_path,
                         bool writing_file = true);

  void dumpUnifiedJSON(const std::shared_ptr<Workflow> &workflow,
                       std::string file_path, bool include_platform = false,
  ...
  void addTimestampTaskStart(double date,
                             const std::shared_ptr<WorkflowTask> &task);

  void addTimestampTaskFailure(double date,
                               const std::shared_ptr<WorkflowTask> &task);

  void addTimestampTaskCompletion(double date,
                                  const std::shared_ptr<WorkflowTask> &task);

  void addTimestampTaskTermination(double date,
                                   const std::shared_ptr<WorkflowTask> &task);

  void addTimestampFileReadStart(double date,
  ...
  void
  addTimestampFileReadFailure(double date,
                              const std::shared_ptr<DataFile> &file,
                              const std::shared_ptr<FileLocation> &src,
                              const std::shared_ptr<StorageService> &service,
                              std::shared_ptr<WorkflowTask> task = nullptr);
  ...
  void
  addTimestampFileReadCompletion(double date,
                                 const std::shared_ptr<DataFile> &file,
                                 const std::shared_ptr<FileLocation> &src,
                                 const std::shared_ptr<StorageService> &service,
                                 std::shared_ptr<WorkflowTask> task = nullptr);
  ...
  void
  addTimestampFileWriteStart(double date, const std::shared_ptr<DataFile> &file,
                             const std::shared_ptr<FileLocation> &src,
                             const std::shared_ptr<StorageService> &service,
                             std::shared_ptr<WorkflowTask> task = nullptr);

  void
  addTimestampFileWriteFailure(double date,
                               const std::shared_ptr<DataFile> &file,
                               const std::shared_ptr<FileLocation> &src,
                               const std::shared_ptr<StorageService> &service,
                               std::shared_ptr<WorkflowTask> task = nullptr);

  void addTimestampFileWriteCompletion(
      double date, const std::shared_ptr<DataFile> &file,
      const std::shared_ptr<FileLocation> &src,
      const std::shared_ptr<StorageService> &service,
      std::shared_ptr<WorkflowTask> task = nullptr);

  void addTimestampFileCopyStart(double date,
                                 const std::shared_ptr<DataFile> &file,
                                 const std::shared_ptr<FileLocation> &src,
                                 const std::shared_ptr<FileLocation> &dst);

  void addTimestampFileCopyFailure(double date,
                                   const std::shared_ptr<DataFile> &file,
                                   const std::shared_ptr<FileLocation> &src,
                                   const std::shared_ptr<FileLocation> &dst);

  void addTimestampFileCopyCompletion(double date,
                                      std::shared_ptr<DataFile> file,
                                      std::shared_ptr<FileLocation> src,
                                      std::shared_ptr<FileLocation> dst);

  void addTimestampDiskReadStart(double date, std::string hostname,
                                 std::string mount, double bytes,
                                 int unique_sequence_number);

  void addTimestampDiskReadFailure(double date, const std::string &hostname,
                                   const std::string &mount, double bytes,
                                   int unique_sequence_number);

  void addTimestampDiskReadCompletion(double date, const std::string &hostname,
                                      const std::string &mount, double bytes,
                                      int unique_sequence_number);

  void addTimestampDiskWriteStart(double date, std::string hostname,
                                  std::string mount, double bytes,
                                  int unique_sequence_number);

  void addTimestampDiskWriteFailure(double date, const std::string &hostname,
                                    const std::string &mount, double bytes,
                                    int unique_sequence_number);

  void addTimestampDiskWriteCompletion(double date, const std::string &hostname,
                                       const std::string &mount, double bytes,
                                       int unique_sequence_number);

  void addTimestampPstateSet(double date, const std::string &hostname,
                             int pstate);

  void addTimestampEnergyConsumption(double date, const std::string &hostname,
                                     double joules);

  void addTimestampLinkUsage(double date, const std::string &linkname,
  ...
  template <class T> void addTimestamp(T *timestamp) {
    ...
    ((SimulationTrace<T> *)(this->traces[type_index]))
        ->addTimestamp(new SimulationTimestamp<T>(timestamp));
  }
...
private:
   ...
   * @tparam a particular SimulationTimestampXXXX class (defined in
   * SimulationTimestampTypes.h)
   * @param enabled true is the time stamp type should be enabled, false
   * otherwise
   */
