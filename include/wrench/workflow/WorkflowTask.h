...
public:
  ...
  struct WorkflowTaskExecution {
     ...
     */
    explicit WorkflowTaskExecution(double task_start)
...
private:
  ...
  std::string execution_host;     // Host on which the task executed ("" if not
                                  // executed successfully - yet)
  State visible_state;            // To be exposed to developer level
  State upcoming_visible_state; // A visible state that will become active once
                                // a WMS has process a previously sent workflow
                                // execution event
  ...
  WorkflowTask(std::string id, double flops, unsigned long min_num_cores,
