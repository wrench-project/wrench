...
public:
  ...
  enum State {
    ...
    SUBMITTED,
    /** @brief Job has finished executing and all actions were successfully
       completed **/
    COMPLETED,
    /** @brief Job has finished executing but not all actions were successfully
     * completed. Actions may have failed, the job may have been
     * terminated/killed, or parent jobs may have been discontinued.
     */
    DISCONTINUED
  };
  ...
  std::shared_ptr<SleepAction> addSleepAction(const std::string &name,
                                              double sleep_time);

  std::shared_ptr<FileReadAction>
  addFileReadAction(const std::string &name,
  ...
  std::shared_ptr<FileReadAction> addFileReadAction(
  ...
  std::shared_ptr<FileWriteAction>
  addFileWriteAction(const std::string &name,
                     const std::shared_ptr<DataFile> &file,
                     const std::shared_ptr<StorageService> &storageService);

  std::shared_ptr<FileCopyAction>
  addFileCopyAction(const std::string &name,
                    const std::shared_ptr<DataFile> &file,
  ...
  addFileDeleteAction(const std::string &name,
                      const std::shared_ptr<DataFile> &file,
                      const std::shared_ptr<StorageService> &storageService);
  ...
  std::shared_ptr<FileRegistryAddEntryAction> addFileRegistryAddEntryAction(
      const std::string &name,
  ...
  std::shared_ptr<ComputeAction>
  addComputeAction(const std::string &name, double flops, double ram,
                   unsigned long min_num_cores, unsigned long max_num_cores,
                   const std::shared_ptr<ParallelModel> &parallel_model);

  std::shared_ptr<CustomAction> addCustomAction(
      const std::string &name, double ram, unsigned long num_cores,
      const std::function<void(std::shared_ptr<ActionExecutor> action_executor)>
          &lambda_execute,
      const std::function<void(std::shared_ptr<ActionExecutor> action_executor)>
          &lambda_terminate);

  std::shared_ptr<CustomAction>
  addCustomAction(std::shared_ptr<CustomAction> custom_action);

  std::shared_ptr<MPIAction> addMPIAction(
      const std::string &name,
      const std::function<void(
          const std::shared_ptr<ExecutionController> &controller)> &mpi_code,
  ...
  void addActionDependency(const std::shared_ptr<Action> &parent,
  ...
  //        void setServiceSpecificArgs(std::map<std::string, std::string>
  //        service_specific_args); const std::map<std::string, std::string> &
  //        getServiceSpecificArgs();
...
protected:
  ...
  void updateStateActionMap(const std::shared_ptr<Action> &action,
                            Action::State old_state, Action::State new_state);
...
private:
  ...
  bool pathExists(const std::shared_ptr<Action> &a,
                  const std::shared_ptr<Action> &b);
  static bool pathExists(const std::shared_ptr<CompoundJob> &a,
  ...
  std::unordered_map<Action::State, std::set<std::shared_ptr<Action>>>
      state_task_map;
