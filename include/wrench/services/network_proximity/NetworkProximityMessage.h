...
/**
 * @brief A message sent to a NetworkProximityService to request a network
 * proximity lookup
 */
...
/**
 * @brief A message sent by a NetworkProximityService in answer to a network
 * proximity lookup request
 */
class NetworkProximityLookupAnswerMessage : public NetworkProximityMessage {
public:
  NetworkProximityLookupAnswerMessage(std::pair<std::string, std::string> hosts,
  ...
  /** @brief The timestamp of the oldest measurement data used to calculate the
   * proximity value */
  double timestamp;
};

/**
 * @brief A message received by a NetworkProximityService that updates its
 * database of proximity values
 */
...
/**
 * @brief A message sent between NetworkProximitySenderDaemon processes to
...
/**
 * @brief A message sent to a NetworkProximityService by a
 * NetworkProximitySenderDaemon to ask which other NetworkProximityDaemons it
 * should do measurements with next
 */
...
/**
 * @brief A message sent by a NetworkProximityService to a
 * NetworkProximitySenderDaemon to tell it which other NetworkProximityDaemons
 * it should do measurements with next
 */
...
public:
  ...
  /** @brief The next NetworkProximitySenderDaemon for the
   * NetworkProximitySenderDaemon to contact */
  std::shared_ptr<NetworkProximityReceiverDaemon> next_daemon_to_send;
...
/**
 * @brief A message sent to a NetworkProximityService to request a coordinate
 * lookup
 */
class CoordinateLookupRequestMessage : public NetworkProximityMessage {
public:
  CoordinateLookupRequestMessage(S4U_CommPort *answer_commport,
...
/**
 * @brief A message sent by a NetworkProximityService in answer to a coordinate
 * lookup request
 */
class CoordinateLookupAnswerMessage : public NetworkProximityMessage {
public:
  CoordinateLookupAnswerMessage(std::string requested_host, bool success,
