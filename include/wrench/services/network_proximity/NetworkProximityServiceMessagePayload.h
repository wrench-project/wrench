...
public:
  /** @brief The number of bytes in the message sent to the service to request a
   * proximity value lookup **/
  DECLARE_MESSAGEPAYLOAD_NAME(NETWORK_DB_LOOKUP_REQUEST_MESSAGE_PAYLOAD);

  /** @brief The number of bytes in the message sent by the service in answer to
   * a request for a proximity value lookup **/
  DECLARE_MESSAGEPAYLOAD_NAME(NETWORK_DB_LOOKUP_ANSWER_MESSAGE_PAYLOAD);

  /** @brief The number of bytes in the message sent by a network proximity
   * daemon to the network proximity service to request which other network
   * proximity daemon it should run network proximity experiments with **/
  DECLARE_MESSAGEPAYLOAD_NAME(NETWORK_DAEMON_CONTACT_REQUEST_PAYLOAD);

  /** @brief The number of bytes in the message sent by the service to a network
   * proximity daemon in answer to a request for which other network proximity
   * daemon to run network proximity experiments with **/
  DECLARE_MESSAGEPAYLOAD_NAME(NETWORK_DAEMON_CONTACT_ANSWER_PAYLOAD);

  /** @brief The number of bytes in the message sent by a network proximity
   * daemon to the network proximity service to report on an  RTT measurement
   * experiment  **/
