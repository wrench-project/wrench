...
public:
  /** @brief The number of bytes in a request control message sent to the daemon
   * to request a list of file locations **/
  DECLARE_MESSAGEPAYLOAD_NAME(FILE_LOOKUP_REQUEST_MESSAGE_PAYLOAD);
  /** @brief The number of bytes per file location returned in an answer sent by
   * the daemon to answer a file location request **/
