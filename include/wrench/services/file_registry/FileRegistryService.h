 ...
 *        the database holds a set of <file, storage service> entries. A WMS can
 * add, lookup, and remove entries at will from this database.
 */
...
private:
  ...
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      {FileRegistryServiceMessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD,
...
public:
  ...
  lookupEntry(const std::shared_ptr<DataFile> &file);

  std::map<double, std::shared_ptr<FileLocation>>
  lookupEntry(const std::shared_ptr<DataFile> &file,
              const std::string &reference_host,
              const std::shared_ptr<NetworkProximityService>
