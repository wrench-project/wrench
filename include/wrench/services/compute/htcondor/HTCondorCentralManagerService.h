...
private:
  ...
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      {HTCondorCentralManagerServiceMessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD,
...
public:
  ...
  void submitCompoundJob(std::shared_ptr<CompoundJob> job,
                         const std::map<std::string, std::string>
                             &service_specific_arguments) override;

  ~HTCondorCentralManagerService() override;

  void terminateCompoundJob(std::shared_ptr<CompoundJob> job) override{};

  //        bool jobKindIsSupported(const std::shared_ptr<Job> &job,
  //        std::map<std::string, std::string> service_specific_arguments);

  std::shared_ptr<FailureCause> jobCanRunSomewhere(
...
private:
  ...
  void
  processNegotiatorCompletion(std::set<std::shared_ptr<Job>> &pending_jobs);
