 ...
 *        and with how many cores. If no host is specified, the service will
 * pick the least loaded host. If no number of cores is specified, the service
 *        will use as many cores as possible.  The service
 *        will make sure that the RAM capacity of a host is not exceeded by
 * possibly delaying job executions until enough RAM is available. Note that if
 * the submitted jobs require a total number of cores larger than available,
 * say, on a particular host, then these jobs will simply time-share the cores.
 * In other words, this service does not provide space-sharing of hosts/cores
...
private:
  ...
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      {BareMetalComputeServiceMessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD,
      ...
      {BareMetalComputeServiceMessagePayload::
       ...
       S4U_CommPort::default_control_message_size},
      {BareMetalComputeServiceMessagePayload::PILOT_JOB_STARTED_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BareMetalComputeServiceMessagePayload::PILOT_JOB_EXPIRED_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BareMetalComputeServiceMessagePayload::PILOT_JOB_FAILED_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BareMetalComputeServiceMessagePayload::
...
public:
  ...
  bool isThereAtLeastOneHostWithIdleResourcesInstant(unsigned long num_cores,
                                                     double ram);
...
protected:
  ...
  //                                messagepayload_list,
  //                                std::shared_ptr<PilotJob> pj, const
  //                                std::string &suffix,
  //                                std::shared_ptr<StorageService>
  //                                scratch_space);
  ...
  void
  initiateInstance(const std::string &hostname,
                   std::map<std::string, std::tuple<unsigned long, double>>
                       compute_resources,
                   WRENCH_PROPERTY_COLLECTION_TYPE property_list,
                   WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list,
...
protected:
  ...
  //        std::shared_ptr<PilotJob> containing_pilot_job;// In case this
  //        service is in fact a pilot job
   ...
   */
  std::unordered_map<std::shared_ptr<CompoundJob>, int>
      num_dispatched_actions_for_cjob;

  // Add the scratch files of one standardjob to the list of all the scratch
  // files of all the standard jobs inside the pilot job
  //        void storeFilesStoredInScratch(std::set<std::shared_ptr<DataFile>>
  //        scratch_files);
  ...
  void terminateCurrentCompoundJob(
      const std::shared_ptr<CompoundJob> &job,
      ComputeService::TerminationCause termination_cause);
  ...
  static std::tuple<std::string, unsigned long>
  ...
  /**
   * @brief The HostStateChangeDetector that is (optionally) started by this
   * service
   */
