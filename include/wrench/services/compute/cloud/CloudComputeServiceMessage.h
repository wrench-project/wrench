...
/**
 * @brief A message sent to a CloudComputeService to request the list of its
 * execution hosts
 */
...
/**
 * @brief A message sent by a CloudComputeService in answer to a list of
 * execution hosts request
 */
...
public:
  CloudComputeServiceCreateVMRequestMessage(
      S4U_CommPort *answer_commport, unsigned long num_cores, double ram_memory,
...
public:
  ...
  S4U_CommPort *answer_commport;
  /** @brief The number of cores the service can use (0 means "use as many as
   * there are cores on the host") */
  unsigned long num_cores;
  /** @brief The VM RAM memory_manager_service capacity (0 means "use all
   * memory_manager_service available on the host", this can be lead to out of
   * memory_manager_service issue) */
  double ram_memory;
  /** @brief The physical host on which the VM should be created ("" means "the
   * service picks") */
  std::string physical_host;
  /** @brief A property list for the bare_metal_standard_jobs that will run on
   * the VM ({} means "use all defaults") */
  WRENCH_PROPERTY_COLLECTION_TYPE property_list;
  /** @brief A message payload list for the bare_metal_standard_jobs that will
   * run on the VM ({} means "use all defaults") */
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list;
};

/**
 * @brief A message sent by a CloudComputeService in answer to a VM creation
 * request
 */
...
/**
 * @brief A message sent by a CloudComputeService in answer to a VM shutdown
 * request
 */
...
/**
 * @brief A message sent by a CloudComputeService in answer to a VM start
 * request
 */
...
/**
 * @brief A message sent by a CloudComputeService in answer to a VM suspend
 * request
 */
...
/**
 * @brief A message sent by a CloudComputeService in answer to a VM resume
 * request
 */
...
/**
 * @brief A message sent by a CloudComputeService in answer to a VM destroy
 * request
 */
