...
/**
 * @brief A batch-scheduled compute service that manages a set of compute hosts
 * and controls access to their resource via a batch queue.
 *
 *        In the current implementation of
 *        this service, like for many of its real-world counterparts,
 * memory_manager_service partitioning among jobs onq the same host is not
 * handled.  When multiple jobs share hosts, which can happen when jobs require
 * only a few cores per host and can thus be co-located on the same hosts in a
 * non-exclusive fashion, each job simply runs as if it had access to the full
 * RAM of each compute host it is scheduled on. The simulation of these
 *        memory_manager_service contended scenarios is thus, for now, not
 * realistic as there is no simulation of the effects of memory_manager_service
 * sharing (e.g., swapping).
 */
...
private:
  ...
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      {BatchComputeServiceMessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD,
      ...
      {BatchComputeServiceMessagePayload::
       ...
       S4U_CommPort::default_control_message_size},
      {BatchComputeServiceMessagePayload::
           TERMINATE_STANDARD_JOB_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BatchComputeServiceMessagePayload::
           TERMINATE_STANDARD_JOB_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BatchComputeServiceMessagePayload::
      ...
      {BatchComputeServiceMessagePayload::PILOT_JOB_EXPIRED_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BatchComputeServiceMessagePayload::
           TERMINATE_PILOT_JOB_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BatchComputeServiceMessagePayload::
           TERMINATE_PILOT_JOB_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {BatchComputeServiceMessagePayload::
...
public:
  ...
  // helper function
  static unsigned long parseUnsignedLongServiceSpecificArgument(
      const std::string &key, const std::map<std::string, std::string> &args);

  void validateServiceSpecificArguments(
...
private:
  ...
  void submitCompoundJob(
      std::shared_ptr<CompoundJob> job,
      const std::map<std::string, std::string> &batch_job_args) override;
  ...
  std::map<std::shared_ptr<CompoundJob>, std::shared_ptr<Alarm>>
      compound_job_alarms;
  ...
  std::map<std::shared_ptr<CompoundJob>,
           std::shared_ptr<BareMetalComputeServiceOneShot>>
      running_bare_metal_one_shot_compute_services;

  // Master List of batch jobs
  std::map<std::shared_ptr<CompoundJob>, std::shared_ptr<BatchJob>> all_jobs;

  // A set of running batch jobs
  std::map<std::shared_ptr<CompoundJob>, std::shared_ptr<BatchJob>>
      running_jobs;
  ...
  std::set<std::string> scheduling_algorithms = {
      "conservative_bf",
      "crasher",
      "easy_bf",
      "easy_bf_fast",
  ...
  std::set<std::string> queue_ordering_options = {
      "fcfs",     "lcfs",      "desc_bounded_slowdown", "desc_slowdown",
  ...
  void processGetResourceInformation(S4U_CommPort *answer_commport,
                                     const std::string &key);

  void processCompoundJobCompletion(
      const std::shared_ptr<BareMetalComputeServiceOneShot> &executor,
      const std::shared_ptr<CompoundJob> &job);

  void processCompoundJobFailure(
      const std::shared_ptr<BareMetalComputeServiceOneShot> &executor,
      const std::shared_ptr<CompoundJob> &job,
      const std::shared_ptr<FailureCause> &cause);

  void terminateRunningCompoundJob(
      const std::shared_ptr<CompoundJob> &job,
      ComputeService::TerminationCause termination_cause);

  // Terminate the batch service (this is usually for pilot jobs when they act
  // as a batch service)
  ...
  void
  processCompoundJobTerminationRequest(const std::shared_ptr<CompoundJob> &job,
  ...
  void sendCompoundJobFailureNotification(
      const std::shared_ptr<CompoundJob> &job, const std::string &job_id,
      const std::shared_ptr<FailureCause> &cause);

  // process a job submission
  void processJobSubmission(const std::shared_ptr<BatchJob> &job,
                            S4U_CommPort *answer_commport);

  // start a job
  void startJob(
      const std::map<simgrid::s4u::Host *, std::tuple<unsigned long, double>> &,
      const std::shared_ptr<CompoundJob> &, const std::shared_ptr<BatchJob> &,
