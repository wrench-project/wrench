...
public:
   ...
   */
  explicit BatschedBatchScheduler(BatchComputeService *cs)
      : BatchScheduler(cs) {}
  ...
  /**
   * @brief Method to process a job termination when the job is already
   * forgotten (useful for batsched)
   ...
   * be estimated. Each job configuration is :(i) a name; (ii) a number of
   * nodes; (iii) a number of cores per node; and (iv) an execution duration in
   * seconds.
   *
   * @return a map of estimated start dates (in seconds), indexed by job
   * configuration name
   */
...
private:
  void notifyJobEventsToBatSched(std::string job_id, std::string status,
                                 std::string job_state, std::string kill_reason,
