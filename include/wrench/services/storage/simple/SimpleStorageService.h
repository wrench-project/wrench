...
/**
 * @brief A storage service that provides direct access to some storage
 * resources (e.g., one or more disks). An important (configurable) property of
 * the storage service is SimpleStorageServiceProperty::BUFFER_SIZE (see
 * documentation thereof), which defines the buffer size that the storage
 * service uses. Specifically, when the storage service receives/sends data
 * from/to the network, it does so in a loop over data "chunks", with pipelined
 * network and disk I/O operations. The smaller the buffer size the more "fluid"
 *        the model, but the more time-consuming the simulation. A large buffer
 * size, however, may lead to less realistic simulations. At the extreme, an
 * infinite buffer size would correspond to fully sequential executions (first a
 * network receive/send, and then a disk write/read). Setting the buffer size to
 * "0" corresponds to a fully fluid model in which individual data chunk
 * operations are not simulated, thus achieving both accuracy (unless one
 * specifically wishes to study the effects of buffering) and quick simulation
 * times. For now, setting the buffer size to "0" is not implemented. The
 * default buffer size is 10 MiB (note that the user can always declare a disk
 * with arbitrary bandwidth in the platform description XML).
 */
...
protected:
  ...
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      {SimpleStorageServiceMessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::DAEMON_STOPPED_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FREE_SPACE_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FREE_SPACE_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_DELETE_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_DELETE_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_LOOKUP_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_LOOKUP_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_COPY_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_COPY_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_WRITE_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_WRITE_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {SimpleStorageServiceMessagePayload::FILE_READ_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
...
public:
  ...
  static SimpleStorageService *createSimpleStorageService(
  ...
  virtual bool reserveSpace(std::shared_ptr<FileLocation> &location) override {
    ...
    return this->file_systems[mount_point]->reserveSpace(location->getFile(),
                                                         path_at_mount_point);
  }
   ...
   */
  virtual void
  unreserveSpace(std::shared_ptr<FileLocation> &location) override {
    std::string mount_point;
    std::string path_at_mount_point;
    this->splitPath(location->getPath(), mount_point, path_at_mount_point);
   ...
   */
  virtual bool isBufferized() const override { return this->is_bufferized; }

  /**
   * @brief Determine the storage service's buffer size
   * @return a size in bytes
   */
  virtual double getBufferSize() const override { return this->buffer_size; }
...
protected:
  ...
  bool splitPath(const std::string &path, std::string &mount_point,
                 std::string &path_at_mount_point);
