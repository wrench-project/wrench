 ...
 * @param resources Map of storage node hostnames and associated StorageServices
 * (one per disk)
 * @param mapping Mapping of current allocations (files and the file parts
 * locations)
 * @param previous_allocations Previous allocations of parts of the same file as
 * the current file part (when the allocator doesn't do striping internally)
 * @return Vector of FileLocation (potentially with one element only) which
 * should be used for the given file.
 */
using StorageSelectionStrategyCallback =
    std::function<std::vector<std::shared_ptr<FileLocation>>(
        const std::shared_ptr<DataFile> &file,
        const std::map<std::string,
                       std::vector<std::shared_ptr<StorageService>>> &resources,
...
/**
 * @brief An abstract storage service which holds a collection of concrete
 * storage services (eg. SimpleStorageServices). It does not provide direct
 * access to any storage resource. It is meant to be used as a way to postpone
 * the selection of a storage service for a file action (read, write, copy, etc)
 * until a later time in the simulation, rather than during job definition. A
 * typical use for the CompoundStorageService is to select a definitive
 *        SimpleStorageService for each action of a job during its scheduling in
 * a BatchScheduler class. This should never receive messages for I/O
 * operations, as any standard storage service (File
 * Read/Write/Delete/Copy/Lookup requests), instead, it overides the main
 * functions of StorageService (readFile / writeFile /...) and will craft
 * messages intended for one or many of its underlying storage services.
 */
...
public:
  ...
  double
  getFileLastWriteDate(const std::shared_ptr<FileLocation> &location) override;
   ...
   */
  bool isBufferized() const override { return false; }

  /**
   * @brief Determine the storage service's buffer size
   * @return a size in bytes
   */
  double getBufferSize() const override { return 0; }
  ...
  bool reserveSpace(std::shared_ptr<FileLocation> &location) override {
    throw std::runtime_error(
        "CompoundStorageService::reserveSpace(): not implemented");
  }

  /**
   * @brief Remove a directory and all its content at the storage service (in
   * zero simulated time)
   * @param path: a path
   */
  void removeDirectory(const std::string &path) override {
    throw std::runtime_error(
        "CompoundStorageService::removeDirectory(): not implemented");
  }
  ...
  void unreserveSpace(std::shared_ptr<FileLocation> &location) override {
    throw std::runtime_error(
        "CompoundStorageService::unreserveSpace(): not implemented");
  }
  ...
  void createFile(const std::shared_ptr<FileLocation> &location) override {
    throw std::runtime_error(
        "CompoundStorageService::createFile(): not implemented");
  }
  ...
  void removeFile(const std::shared_ptr<FileLocation> &location) override {
    throw std::runtime_error(
        "CompoundStorageService::removeFile(): not implemented");
  }
  ...
  lookupFileLocation(const std::shared_ptr<FileLocation> &location);

  std::vector<std::shared_ptr<FileLocation>>
  lookupFileLocation(const std::shared_ptr<DataFile> &file,
                     S4U_CommPort *answer_commport);

  std::vector<std::shared_ptr<FileLocation>>
  lookupOrDesignateStorageService(const std::shared_ptr<FileLocation> location);
  ...
  void writeFile(S4U_CommPort *answer_commport,
                 ...
                 double num_bytes_to_write, bool wait_for_answer) override;
  ...
  void
  copyFileIamDestination(const std::shared_ptr<FileLocation> &src_location,
                         const std::shared_ptr<FileLocation> &dst_location);
...
private:
   ...
   *         Some values are set to zero because in the current implementation,
   * it is expected that the CompoundStorageService will always immediately
   * refuse / reject such requests, with minimum cost to the user.
   */
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      {CompoundStorageServiceMessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {CompoundStorageServiceMessagePayload::DAEMON_STOPPED_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {CompoundStorageServiceMessagePayload::FREE_SPACE_REQUEST_MESSAGE_PAYLOAD,
       0},
      ...
      {CompoundStorageServiceMessagePayload::FILE_LOOKUP_ANSWER_MESSAGE_PAYLOAD,
       0},
      {CompoundStorageServiceMessagePayload::FILE_COPY_REQUEST_MESSAGE_PAYLOAD,
       0},
...
private:
  ...
  std::vector<std::shared_ptr<FileLocation>> lookupOrDesignateStorageService(
      const std::shared_ptr<DataFile> concrete_file_location,
      unsigned int stripe_count, S4U_CommPort *answer_commport);

  bool processStorageSelectionMessage(
      const CompoundStorageAllocationRequestMessage *msg);

  bool
  processStorageLookupMessage(const CompoundStorageLookupRequestMessage *msg);

  bool processNextMessage(SimulationMessage *message);

  /* Key : hostname of storage server, value : list of storage services (one per
   * disk / raid / ...) on this storage server */
  std::map<std::string, std::vector<std::shared_ptr<StorageService>>>
      storage_services = {};
  ...
  /**
   *  @brief  Whether to strip a file in the CSS or in the external allocation
   * function. Internal flag set from CSS property.
   */
