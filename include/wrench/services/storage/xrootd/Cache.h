...
class Cache {
private:
  /** @brief The internal cache data structure, currently just a map of data
   * files pointers to a set of file locations, each with corresponding last
   * update time */
  std::unordered_map<std::shared_ptr<DataFile>,
                     std::map<std::shared_ptr<FileLocation>, double>>
      cache; // possibly change time to last access time
public:
  ...
  void add(const std::shared_ptr<DataFile> &file,
           const std::shared_ptr<FileLocation> &location);
  void add(const std::shared_ptr<DataFile> &file,
           const std::set<std::shared_ptr<FileLocation>> &locations);
  std::set<std::shared_ptr<FileLocation>>
  get(const std::shared_ptr<DataFile> &file);

  std::set<std::shared_ptr<FileLocation>>
  operator[](const std::shared_ptr<DataFile> &file);
