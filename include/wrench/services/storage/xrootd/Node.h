 ...
 * All nodes are classified as storage services even though not all have
 * physical storage Unless a node is also has an internal storage service, some
 * normal storage service messages will error out. Only File Read, locate, and
 * delete are supported at this time, anything else requires talking directly to
 * a specific file server with physical storage. Nodes not directly be created,
 * instead an XRootD Metavisor should create them
 */
class Node : public StorageService {
public:
  std::shared_ptr<Node> addChildSupervisor(const std::string &hostname);
  std::shared_ptr<Node> addChildStorageServer(
...
private:
  ...
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      {MessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::DAEMON_STOPPED_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::FILE_LOOKUP_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::FILE_LOOKUP_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::UPDATE_CACHE,
       S4U_CommPort::default_control_message_size},
      ...
      {MessagePayload::FILE_DELETE_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::FILE_DELETE_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::FILE_WRITE_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::FILE_WRITE_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::FILE_COPY_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {MessagePayload::FILE_COPY_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size}};
...
public:
  ...
  Node(Deployment *deployment, const std::string &hostname,
       WRENCH_PROPERTY_COLLECTION_TYPE storage_property_list,
       WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE storage_messagepayload_list);
...
private:
  ...
  splitStack(vector<stack<Node *>> search_stack);
  virtual std::shared_ptr<FileLocation>
  selectBest(std::set<std::shared_ptr<FileLocation>> locations);
  vector<stack<Node *>>
  constructFileSearchTree(const vector<shared_ptr<Node>> &targets);
  stack<Node *> constructSearchStack(Node *target);
  // std::shared_ptr<FileLocation> hasFile(shared_ptr<DataFile> file);

  bool makeSupervisor();
  bool
  makeFileServer(std::set<std::string> path,
  ...
  Cache cache;
  /** @brief The supervisor of this node.  All continue search and ripple delete
   * messages should come from this, and all update cache messages go to this
   * server */
  Node *supervisor = nullptr;
  /** @brief The Meta supervisor for this entire XRootD data federation */
  Deployment *metavisor = nullptr;
  /** @brief Whether this node is running a reduced simulation.  Initilized from
   * the properties in main */
  bool reduced;
  friend Deployment;
  friend SearchStack;
  /***********************/
