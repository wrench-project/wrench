...
public:
  ...
  public:
     ...
     * @param num_current_transactions Number of current transactions using this
     * file on disk
     */
    FileOnDiskLRUCaching(double last_write_date,
                         unsigned int lru_sequence_number,
                         unsigned short num_current_transactions)
        : FileOnDisk(last_write_date), lru_sequence_number(lru_sequence_number),
    ...
    unsigned int lru_sequence_number;
    /**
     * @brief The number of transactions that involve this file, meaning that
     * it's not evictable is > 0
     */
    unsigned short num_current_transactions;
  };
...
private:
  ...
  std::map<unsigned int, std::tuple<std::string, std::shared_ptr<DataFile>>>
      lru_list;

  void print_lru_list() {
    std::cerr << "LRU LIST:\n";
    for (auto const &lru : this->lru_list) {
      std::cerr << "[" << lru.first << "] " << std::get<0>(lru.second) << ":"
                << std::get<1>(lru.second)->getID() << "\n";
    }
  }
