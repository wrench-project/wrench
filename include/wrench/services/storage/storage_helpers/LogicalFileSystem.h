...
public:
  ...
  public:
     ...
     */
    explicit FileOnDisk(double last_write_date)
  ...
  /**
   * @brief A constant that signifies /dev/null, when the actual
   * location/path/mountpoint/etc. is unknown
   */
  const static std::string DEV_NULL;

  static std::unique_ptr<LogicalFileSystem>
  createLogicalFileSystem(const std::string &hostname,
  ...
  void unreserveSpace(const std::shared_ptr<DataFile> &file,
                      const std::string &absolute_path);
  ...
  bool isFileInDirectory(const std::shared_ptr<DataFile> &file,
                         const std::string &absolute_path);
  std::set<std::shared_ptr<DataFile>>
  listFilesInDirectory(const std::string &absolute_path);
   ...
   */
  virtual void storeFileInDirectory(const std::shared_ptr<DataFile> &file,
                                    const std::string &absolute_path) = 0;
  /**
   ...
   * @throw std::invalid_argument
   */
  virtual void removeFileFromDirectory(const std::shared_ptr<DataFile> &file,
                                       const std::string &absolute_path) = 0;
  /**
   ...
   */
  virtual void updateReadDate(const std::shared_ptr<DataFile> &file,
                              const std::string &absolute_path) = 0;
  /**
   ...
   */
  virtual void incrementNumRunningTransactionsForFileInDirectory(
   ...
   * @param absolute_path: the file path
   */
  virtual void decrementNumRunningTransactionsForFileInDirectory(
      const std::shared_ptr<DataFile> &file,
      const std::string &absolute_path) = 0;
...
protected:
  ...
  LogicalFileSystem(const std::string &hostname,
                    StorageService *storage_service,
  ...
  std::unordered_map<std::string,
                     std::map<std::shared_ptr<DataFile>,
                              std::shared_ptr<LogicalFileSystem::FileOnDisk>>>
      content;
  ...
  void assertDirectoryExist(const std::string &absolute_path) {
    if (not this->doesDirectoryExist(absolute_path)) {
      throw std::invalid_argument(
          "LogicalFileSystem::assertDirectoryExists(): directory " +
  ...
  void assertDirectoryIsEmpty(const std::string &absolute_path) {
    ...
    if (not this->isDirectoryEmpty(absolute_path)) {
      throw std::invalid_argument(
          "LogicalFileSystem::assertDirectoryIsEmpty(): directory " +
          absolute_path + "is not empty");
    }
  }
   ...
   * @param absolute_path: the path
   */
  void assertFileIsInDirectory(const std::shared_ptr<DataFile> &file,
                               const std::string &absolute_path) {
    assertDirectoryExist(absolute_path);
    if (this->content[absolute_path].find(file) ==
        this->content[absolute_path].end()) {
      throw std::invalid_argument(
          "LogicalFileSystem::assertFileIsInDirectory(): File " +
          file->getID() + " is not in directory " + absolute_path);
    }
  }
};
