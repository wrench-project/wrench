...
/**
 * @brief A message sent to by a FileTransferThread to report on success/failure
 * of the transfer
 */
...
public:
   ...
   * @param src_commport: the source commport_name of the transfer (or "" if
   * source wasn't a commport_name)
   * @param src_location: the source location of the transfer (or nullptr if
   * source wasn't a location)
   * @param dst_commport: the destination commport_name of the transfer (or ""
   * if source wasn't a commport_name)
   * @param dst_location: the destination location of the transfer (or nullptr
   * if source wasn't a location)
   * @param answer_commport_if_read: the commport_name that a "read is done" may
   * be sent to if necessary
   * @param answer_commport_if_write: the commport_name that a "write is done"
   * may be sent to if necessary
   * @param answer_commport_if_copy: the commport_name that a "copy is
   * done/failed" may be sent if necessary
  ...
  FileTransferThreadNotificationMessage(
      ...
      std::shared_ptr<DataFile> file, S4U_CommPort *src_commport,
      ...
      S4U_CommPort *answer_commport_if_copy, bool success,
      std::shared_ptr<FailureCause> failure_cause)
      : FileTransferThreadMessage(0),
        file_transfer_thread(file_transfer_thread), file(file),
        ...
        answer_commport_if_copy(answer_commport_if_copy), success(success),
  ...
  /** @brief Destination commport_name (or "" if destination wasn't a
   * commport_name) */
  S4U_CommPort *dst_commport;
  /** @brief Destination location (or nullptr if source wasn't a location) */
  std::shared_ptr<FileLocation> dst_location;

  /** @brief If this was a file read, the commport_name to which an answer
   * should be send */
  S4U_CommPort *answer_commport_if_read;
  /** @brief If this was a file write, the commport_name to which an answer
   * should be send */
  S4U_CommPort *answer_commport_if_write;
  /** @brief If this was a file copy, the commport_name to which an answer
   * should be send */
