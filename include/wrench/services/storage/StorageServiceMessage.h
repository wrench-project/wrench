...
public:
  StorageServiceFreeSpaceRequestMessage(S4U_CommPort *answer_commport,
...
public:
  StorageServiceFileLookupAnswerMessage(std::shared_ptr<DataFile> file,
                                        bool file_is_available, double payload);
...
/**
 * @brief A message sent  by a StorageService in answer to a file deletion
 * request
 */
...
/**
 * @brief A message sent to a StorageService to copy a file from another
 * StorageService
 */
...
public:
  ...
  std::shared_ptr<FailureCause> failure_cause;
  /** @brief The set of destination commports and the number of bytes to send to
   * each */
  std::map<S4U_CommPort *, double> data_write_commport_and_bytes;
...
public:
  ...
  std::shared_ptr<FailureCause> failure_cause;
  /** @brief The commport_name to which the file content should be sent (or
   * nullptr) */
...
/**
 * @brief A message sent/received by a StorageService that has a file size as a
 * payload
 */
