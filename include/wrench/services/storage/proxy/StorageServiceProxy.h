...
public:
   ...
   */
  double getFileLastWriteDate(
      const std::shared_ptr<FileLocation> &location) override; // forward

  virtual void deleteFile(const std::shared_ptr<StorageService> &targetServer,
                          const std::shared_ptr<DataFile> &file);

  virtual bool lookupFile(const std::shared_ptr<StorageService> &targetServer,
                          const std::shared_ptr<DataFile> &file);

  // void readFile(const std::shared_ptr<FileLocation> &location, double
  // num_bytes) override;
  virtual void readFile(const std::shared_ptr<StorageService> &targetServer,
                        const std::shared_ptr<DataFile> &file);
  virtual void readFile(const std::shared_ptr<StorageService> &targetServer,
                        const std::shared_ptr<DataFile> &file,
                        double num_bytes);
  ...
  virtual void writeFile(const std::shared_ptr<StorageService> &targetServer,
                         const std::shared_ptr<DataFile> &file,
                         const std::string &path);
  virtual void writeFile(const std::shared_ptr<StorageService> &targetServer,
                         const std::shared_ptr<DataFile> &file);
  ...
  /**
   * @brief Factory to create a StorageServiceProxy that can a default
   * destination to forward requests too, and will cache requests as they are
   * made
  ...
  static std::shared_ptr<StorageServiceProxy> createRedirectProxy(
      ...
      const std::shared_ptr<StorageService> &remote = nullptr,
      WRENCH_PROPERTY_COLLECTION_TYPE properties = {},
      WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE message_payloads = {}) {
    return std::make_shared<StorageServiceProxy>(hostname, cache, remote,
                                                 properties, message_payloads);
  }
  ...
  bool reserveSpace(std::shared_ptr<FileLocation> &location) override {
    throw std::runtime_error(
        "StorageServiceProxy::reserveSpace(): should not be called");
  };
  ...
  void unreserveSpace(std::shared_ptr<FileLocation> &location) override {
    throw std::runtime_error(
        "StorageServiceProxy::unreserveSpace(): should not be called");
  };

  /**
   * @brief Factory to create a StorageServiceProxy that does not cache reads,
   * and does not have a default destination to forward too
   * @param hostname: hostname
   * @param properties: Properties for the fileServiceProxy
   * @param message_payloads: Message Payloads for the fileServiceProxy
   * @return the StorageServiceProxy created
   */
  static std::shared_ptr<StorageServiceProxy> createCachelessRedirectProxy(
      const std::string &hostname,
      WRENCH_PROPERTY_COLLECTION_TYPE properties = {},
      WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE message_payloads = {}) {

    throw std::runtime_error("Cacheless proxies are not currently supported");
    return std::make_shared<StorageServiceProxy>(hostname, nullptr, nullptr,
                                                 properties, message_payloads);
  }

  /**
   * @brief Factory to create a StorageServiceProxy that does not cache reads,
   * and only forwards requests to another service
   * @param hostname: hostname
   ...
   * @return the StorageServiceProxy created
   */
  static std::shared_ptr<StorageServiceProxy> createCachelessProxy(
      const std::string &hostname,
    ...
    throw std::runtime_error("Cacheless proxies are not currently supported");
    return std::make_shared<StorageServiceProxy>(
        hostname, nullptr, default_remote, properties, message_payloads);
  }

protected:
  /** @brief Pending operations **/
  std::map<std::shared_ptr<DataFile>, std::vector<unique_ptr<ServiceMessage>>>
      pending;
  ...
  bool commonReadFile(StorageServiceFileReadRequestMessage *msg,
...
private:
  ...
  WRENCH_PROPERTY_COLLECTION_TYPE default_property_values = {
      //                {StorageServiceProperty::BUFFER_SIZE, "10000000"},// 10
      //                MEGA BYTE
  ...
  WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE default_messagepayload_values = {
      ...
      {StorageServiceMessagePayload::FREE_SPACE_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {StorageServiceMessagePayload::FREE_SPACE_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {StorageServiceMessagePayload::FILE_DELETE_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {StorageServiceMessagePayload::FILE_DELETE_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {StorageServiceMessagePayload::FILE_LOOKUP_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {StorageServiceMessagePayload::FILE_LOOKUP_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {StorageServiceMessagePayload::FILE_COPY_REQUEST_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
      {StorageServiceMessagePayload::FILE_COPY_ANSWER_MESSAGE_PAYLOAD,
       S4U_CommPort::default_control_message_size},
...
public:
   ...
   * @param target: a (remote) storage service to access, which overrides the
   * default remote service (if any) of the proxy
   * @param other: a file location whose storage service should be the proxy
   * @return a proxy location
   */
  static std::shared_ptr<ProxyLocation>
  LOCATION(const std::shared_ptr<StorageService> &target,
  ...
  LOCATION(const std::shared_ptr<StorageService> &target,
           ...
           const std::shared_ptr<DataFile> &file) {
    return std::shared_ptr<ProxyLocation>(
        new ProxyLocation(target, FileLocation::LOCATION(ss, file)));
  }
  ...
  LOCATION(const std::shared_ptr<StorageService> &target,
           ...
           const std::shared_ptr<DataFile> &file) {
    return std::shared_ptr<ProxyLocation>(
        new ProxyLocation(target, FileLocation::LOCATION(ss, server_ss, file)));
  }
#endif
  ...
  static std::shared_ptr<ProxyLocation>
  LOCATION(const std::shared_ptr<StorageService> &target,
           const std::shared_ptr<StorageService> &ss, const std::string &path,
           const std::shared_ptr<DataFile> &file) {
    return std::shared_ptr<ProxyLocation>(
        new ProxyLocation(target, FileLocation::LOCATION(ss, path, file)));
  }
...
private:
   ...
   */
  ProxyLocation(const std::shared_ptr<StorageService> &target,
                const std::shared_ptr<FileLocation> &other)
      : FileLocation(*other), target(target) {
    if (target == nullptr) {
      throw std::invalid_argument(
          "ProxyLocation::LOCATION(): Cannot pass nullptr target");
    }
  }
  /***********************/
  /** \endcond           */
