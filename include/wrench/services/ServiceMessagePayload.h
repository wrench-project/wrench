...
#define DECLARE_MESSAGEPAYLOAD_NAME(name)                                      \
  static const wrench::WRENCH_MESSAGEPAYLOAD_TYPE name

#define SET_MESSAGEPAYLOAD_NAME(classname, name)                               \
  const wrench::WRENCH_MESSAGEPAYLOAD_TYPE classname::name =                   \
      classname::addMessagePayload(#classname, #name)
//#name
...
public:
  static WRENCH_MESSAGEPAYLOAD_TYPE
  addMessagePayload(std::string classname, std::string messagePayload);
  static WRENCH_MESSAGEPAYLOAD_TYPE translateString(std::string messagePayload);
  static std::string
  translatePayloadType(WRENCH_MESSAGEPAYLOAD_TYPE messagePayload);
