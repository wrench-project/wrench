...
/**
 * @brief A helper daemon (co-located with an execution controler) that handles
...
public:
  ...
  void initiateAsynchronousFileCopy(const std::shared_ptr<FileLocation> &src,
                                    ...
                                    const std::shared_ptr<FileRegistryService>
                                        &file_registry_service = nullptr);

  void
  initiateAsynchronousFileRead(const std::shared_ptr<FileLocation> &location,
                               const double num_bytes);

  void
  initiateAsynchronousFileRead(const std::shared_ptr<FileLocation> &location);

  void initiateAsynchronousFileWrite(
...
protected:
  ...
  explicit DataMovementManager(std::string hostname,
...
private:
  ...
  struct CopyRequestSpecs {
    ...
    bool operator==(const CopyRequestSpecs &rhs) const {
      return (src->equal(rhs.src) && dst->equal(rhs.dst));
    }
  };
  ...
  struct ReadRequestSpecs {
    ...
    ReadRequestSpecs(std::shared_ptr<FileLocation> location, double num_bytes)
