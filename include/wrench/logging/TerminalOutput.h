...
#define TRACK_OBJECT(name)                                                     \
  {                                                                            \
    ...
    std::cerr << "#" << (name) << "++: " << object_tracker->tracker[name]      \
              << std::endl;                                                    \
  }
...
// The whole point is for the map to not be a static object, but instead be
// inside a memory-leaked object so that it will not risk being de-allocated
// before static objects are deallocated and trigger destructor calls that refer
// to the map. static ObjectTracker *object_tracker = new ObjectTracker();
#else
#define TRACK_OBJECT(name)                                                     \
  {}
#define UNTRACK_OBJECT(name)                                                   \
...
/* Wrappers around XBT_* macros, using a bit of those macro's internal magic as
 * well to avoid generating useless (but space consuming) color ASCII codes
 */

#define WRENCH_LOG_CATEGORY(cname, desc)                                       \
  XBT_LOG_NEW_DEFAULT_CATEGORY(cname, desc)

#define WRENCH_INFO(...)                                                       \
  if (_XBT_LOG_ISENABLEDV((*_misuse_of_XBT_LOG_macros_detected__default),      \
...
/**
 * @brief Color-enabling wrappers around Simgrid's XBT_INFO, XBT_DEBUG, XBT_WARN
 * logging macros
 */
