...
public:
  /** @brief The ram capacity of a physical host whenever not specified in the
   * platform description file */
  static constexpr double DEFAULT_RAM =
      (1024.0 * 1024.0 * 1024.0 * 1024.0 * 1024.0); // 1 PiB
...
public:
  ...
  static void writeToDisk(double num_bytes, const std::string &hostname,
                          std::string mount_point, simgrid::s4u::Disk *disk);
  static void readFromDisk(double num_bytes, const std::string &hostname,
                           std::string mount_point, simgrid::s4u::Disk *disk);
  static void readFromDiskAndWriteToDiskConcurrently(
  ...
  static double getDiskCapacity(const std::string &hostname,
                                std::string mount_point);
  static std::vector<std::string> getDisks(const std::string &hostname);
  static simgrid::s4u::Disk *hostHasMountPoint(const std::string &hostname,
                                               const std::string &mount_point);

  void checkLinkBandwidths();

  static void yield();
  static std::string getHostProperty(const std::string &hostname,
                                     const std::string &property_name);
  static void setHostProperty(const std::string &hostname,
                              const std::string &property_name,
                              const std::string &property_value);
  static std::string getClusterProperty(const std::string &cluster_id,
                                        const std::string &property_name);

  // start energy related calls
  static double getEnergyConsumedByHost(const std::string &hostname);
  //		static double getTotalEnergyConsumed(const std::vector<std::string>
  //&hostnames);
  ...
  static std::map<std::string, std::vector<std::string>>
  getAllHostnamesByZone();
  static std::map<std::string, std::vector<std::string>>
  getAllClusterIDsByZone();
  static std::map<std::string, std::vector<std::string>>
  getAllSubZoneIDsByZone();
  ...
  get_host_or_vm_by_name_or_null(const std::string &name);
...
private:
  static void traverseAllNetZonesRecursive(
      simgrid::s4u::NetZone *nz,
      std::map<std::string, std::vector<std::string>> &result,
      bool get_subzones, bool get_clusters, bool get_hosts_from_zones,
