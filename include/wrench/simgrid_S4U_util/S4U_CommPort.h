...
public:
   ...
   */
  template <class TMessageType> std::string get_type_name() {
   ...
   * @return the message, in a unique_ptr of the type specified.  Otherwise
   * throws a runtime_error
   *
   * @throw std::shared_ptr<NetworkError>
   */
  template <class TMessageType>
  std::unique_ptr<TMessageType>
  getMessage(const std::string &error_prefix = "") {
    ...
    if (auto msg = dynamic_cast<TMessageType *>(message.get())) {
      ...
      throw std::runtime_error(
          error_prefix + " Unexpected [" +
          ((message.get() != nullptr) ? message->getName() : "null-message") +
          "] message while waiting for " + get_type_name<TMessageType>() +
  ...
  template <class TMessageType>
  std::unique_ptr<TMessageType>
  getMessage(double timeout, const std::string &error_prefix = "") {
    auto id = ++messageCounter;
#ifndef NDEBUG
    ...
    if (auto msg = dynamic_cast<TMessageType *>(message.get())) {
      ...
      throw std::runtime_error(
          error_prefix + " Unexpected [" + message->getName() +
          "] message while waiting for " + get_type_name<TMessageType>() +
          ". Request ID: " + std::to_string(id));
    }
  }
   ...
   * @throw std::shared_ptr<NetworkError>
   */
  std::unique_ptr<SimulationMessage> getMessage() { return getMessage(true); }
  ...
  /**
   * @brief The "not a commport_name" commport_name, to avoid getting answers
   * back when asked to prove an "answer commport_name"
   */
   ...
   * @return the commport's name
   */
  [[nodiscard]] const char *get_cname() const { return this->name.c_str(); }
