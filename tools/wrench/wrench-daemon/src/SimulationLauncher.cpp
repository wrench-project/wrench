...
/**
 * @brief Method to create the simulation, which is called in a thread inside a
 * child process. To be able to notify the parent process of the nature of an
 * error, if any, this method simple sets an error message variable and returns
 *
 * @param full_log: whether to show all simulation log
 * @param platform_xml: XML platform description (an XML string - not a file
 * path)
 * @param controller_host: hostname of the host that will run the
 * execution_controller
 * @param sleep_us: number of microseconds to sleep at each iteration of the
 * main loop
 */
void SimulationLauncher::createSimulation(bool full_log,
  ...
  try {
    ...
    std::string platform_file_path =
        "/tmp/wrench_daemon_platform_file_" + std::to_string(getpid()) + ".xml";
    ...
    if (not wrench::Simulation::doesHostExist(controller_host)) {
      throw std::runtime_error("The platform does not contain a "
                               "(execution_controller) host with name " +
                               controller_host);
    }
 ...
 */
void SimulationLauncher::launchSimulation() { this->simulation->launch(); }
