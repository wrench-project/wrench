...
WRENCH_LOG_CATEGORY(simulation_controller,
                    "Log category for SimulationController");
...
#define PARSE_MESSAGE_PAYLOAD_LIST()                                           \
  ...
  {                                                                            \
    ...
    for (auto it = jsonData.cbegin(); it != jsonData.cend(); ++it) {           \
      auto message_payload_key =                                               \
          ServiceMessagePayload::translateString(it.key());                    \
 ...
 *
 * @param hostname string containing the name of the host on which this service
 * runs
 */
SimulationController::SimulationController(const std::string &hostname,
                                           int sleep_us)
    : ExecutionController(hostname, "SimulationController"),
      sleep_us(sleep_us) {}

template <class T> json SimulationController::startService(T *s) {
  ...
  this->things_to_do.push([this, s, &s_created]() {
    try {
      ...
      if (auto cs = std::dynamic_pointer_cast<wrench::ComputeService>(
              new_service_shared_ptr)) {
        WRENCH_INFO("Started a new compute service");
        this->compute_service_registry.insert(new_service_shared_ptr->getName(),
                                              cs);
      } else if (auto ss = std::dynamic_pointer_cast<wrench::StorageService>(
                     new_service_shared_ptr)) {
        WRENCH_INFO("Started a new storage service");
        this->storage_service_registry.insert(new_service_shared_ptr->getName(),
                                              ss);
      } else if (auto fs =
                     std::dynamic_pointer_cast<wrench::FileRegistryService>(
                         new_service_shared_ptr)) {
        WRENCH_INFO("Started a new storage service");
        this->file_registry_service_registry.insert(
            new_service_shared_ptr->getName(), fs);
      } else {
        throw std::runtime_error(
            "SimulationController::startNewService(): Unknown service type");
      }
...
int SimulationController::main() {
  ...
  S4U_Daemon::map_actor_to_recv_commport[simgrid::s4u::this_actor::get_pid()] =
      this->recv_commport;
  ...
  while (keep_going) {
    ...
    if (time_horizon_to_reach < 0) {
      ...
      this->event_queue.push(
          std::make_pair(Simulation::getCurrentSimulatedDate(), event));
    }
    ...
    double time_to_sleep =
        std::max<double>(0, time_horizon_to_reach -
    ...
    if (time_to_sleep > 0.0) {
      ...
      while (auto event = this->waitForNextEvent(
                 10 * JOB_MANAGER_COMMUNICATION_TIMEOUT_VALUE)) {
    ...
    // Sleep since no matter what we're in locked step with client time and
    // don't want to burn CPU cycles like crazy. Could probably sleep 1s...
    usleep(sleep_us);
  }
  return 0;
}
 ...
 */
void SimulationController::stopSimulation() { keep_going = false; }
...
json SimulationController::advanceTime(json data) {
  ...
  this->time_horizon_to_reach =
      Simulation::getCurrentSimulatedDate() + increment_in_seconds;
  return {};
}
 ...
 */
json SimulationController::eventToJSON(
    double date, const std::shared_ptr<wrench::ExecutionEvent> &event) {
  ...
  } else if (auto complete_sj =
                 std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(
                     event)) {
    event_desc["event_type"] = "standard_job_completion";
    job = complete_sj->standard_job;
  } else if (auto failed_cj =
                 std::dynamic_pointer_cast<wrench::CompoundJobFailedEvent>(
                     event)) {
    event_desc["event_type"] = "compound_job_failure";
    event_desc["failure_cause"] = failed_cj->failure_cause->toString();
    job = failed_cj->job;
  } else if (auto completed_cj =
                 std::dynamic_pointer_cast<wrench::CompoundJobCompletedEvent>(
                     event)) {
    event_desc["event_type"] = "compound_job_completion";
    job = completed_cj->job;
  }

  event_desc["compute_service_name"] =
      job->getParentComputeService()->getName();
...
json SimulationController::waitForNextSimulationEvent(const json &data) {
  // Set the time horizon to -1, to signify the "wait for next event" to the
...
json SimulationController::addBareMetalComputeService(json data) {
  ...
  for (auto it = jsonData.cbegin(); it != jsonData.cend(); ++it) {
    auto spec = it.value();
    if (spec[0] < 0)
      spec[0] = ComputeService::ALL_CORES;
    if (spec[1] < 0)
      spec[1] = ComputeService::ALL_RAM;
    resources.emplace(it.key(), spec);
  }

  // Create the new service
  auto new_service = new BareMetalComputeService(
      head_host, resources, scratch_space, service_property_list,
...
json SimulationController::createVM(json data) {
  ...
  this->things_to_do.push([num_cores, ram_memory, service_property_list,
                           service_message_payload_list, cs, &vm_created]() {
    auto cloud_cs = std::dynamic_pointer_cast<CloudComputeService>(cs);
    std::string vm_name;
    try {
      vm_name = cloud_cs->createVM(num_cores, ram_memory, service_property_list,
                                   service_message_payload_list);
    ...
    try {
      if (not cloud_cs->isVMRunning(vm_name)) {
        throw std::invalid_argument(
            "Cannot shutdown VM because it's not running");
      }
...
json SimulationController::addSimpleStorageService(json data) {
  ...
  // Create the new service
  auto new_service = SimpleStorageService::createSimpleStorageService(
      head_host, mount_points, {}, {});
...
json SimulationController::fileRegistryServiceAddEntry(json data) {
  ...
  if (not this->file_registry_service_registry.lookup(
          file_registry_service_name, frs)) {
    throw std::runtime_error("Unknown file registry service " +
...
json SimulationController::createStandardJob(json data) {
  ...
  std::map<std::shared_ptr<DataFile>, std::shared_ptr<FileLocation>>
      file_locations;
  for (auto it = data["file_locations"].begin();
       it != data["file_locations"].end(); ++it) {
...
json SimulationController::submitStandardJob(json data) {
  ...
  this->things_to_do.push(
      [this, job, cs, service_specific_args, &job_submitted]() {
        try {
...
json SimulationController::addComputeAction(json data) {
  ...
  auto action = compound_job->addComputeAction(
      compute_action_name, flops, ram, min_num_cores, max_num_cores, model);
...
json SimulationController::addFileCopyAction(json data) {
  std::string compound_job_name =
      data["compound_job_name"]; // lookup compound job in registry
  std::shared_ptr<CompoundJob> compound_job;
  if (not this->compound_job_registry.lookup(compound_job_name, compound_job)) {
    throw std::runtime_error("Unknown compound job " + compound_job_name);
  }
  ...
  auto action = compound_job->addFileCopyAction(file_copy_action_name, file,
                                                src_ss, dest_ss);
...
json SimulationController::addFileDeleteAction(json data) {
  ...
  auto action =
      compound_job->addFileDeleteAction(file_delete_action_name, file, ss);
...
json SimulationController::addFileWriteAction(json data) {
  ...
  auto action =
      compound_job->addFileWriteAction(file_write_action_name, file, ss);
...
json SimulationController::addFileReadAction(json data) {
  ...
  // Question: Does the following function "getNumBytesToRead()" return total
  // number of bytes in file if the
...
json SimulationController::addActionDependency(json data) {
  ...
  try {
    ...
    throw std::runtime_error("Unknown action " + parent_action_name +
                             " in job " + compound_job_name);
  }
  ...
  try {
    ...
    throw std::runtime_error("Unknown action " + child_action_name +
                             " in job " + compound_job_name);
  }
  ...
  try {
    ...
    throw std::runtime_error("Cannot add action dependency: " +
                             std::string(e.what()));
  }
...
json SimulationController::addParentJob(json data) {
  ...
  if (not this->compound_job_registry.lookup(parent_compound_job_name,
                                             parent_compound_job)) {
    throw std::runtime_error("Unknown compound job " +
                             parent_compound_job_name);
  }
  if (not this->compound_job_registry.lookup(child_compound_job_name,
                                             child_compound_job)) {
...
json SimulationController::getActionStartDate(json data) {
  ...
  try {
    ...
    throw std::runtime_error("Unknown action " + action_name + " in job " +
                             compound_job_name);
  }
  json answer;
...
json SimulationController::createTask(json data) {
  ...
  auto t = workflow->addTask(data["name"], data["flops"], data["min_num_cores"],
                             data["max_num_cores"], data["memory"]);
  return {};
}
...
json SimulationController::getTaskMinNumCores(json data) {
  ...
  answer["min_num_cores"] =
      workflow->getTaskByID(data["task_name"])->getMinNumCores();
  return answer;
}
...
json SimulationController::getTaskMaxNumCores(json data) {
  ...
  answer["max_num_cores"] =
      workflow->getTaskByID(data["task_name"])->getMaxNumCores();
  return answer;
}
...
json SimulationController::getTaskMemory(json data) {
  ...
  answer["memory"] =
      workflow->getTaskByID(data["task_name"])->getMemoryRequirement();
  return answer;
}
...
json SimulationController::getTaskNumberOfChildren(json data) {
  ...
  answer["number_of_children"] =
      workflow->getTaskByID(data["task_name"])->getNumberOfChildren();
  return answer;
}
...
json SimulationController::getTaskBottomLevel(json data) {
  ...
  answer["bottom_level"] =
      workflow->getTaskByID(data["task_name"])->getBottomLevel();
  return answer;
}
...
json SimulationController::getVMComputeService(json data) {
  ...
  auto cloud_cs = std::dynamic_pointer_cast<CloudComputeService>(cs);
  json answer;
  answer["vm_compute_service"] =
      cloud_cs->getVMComputeService(vm_name)->getName();
  return answer;
}
...
json SimulationController::createWorkflowFromJSON(json data) {
  ...
  bool ignore_cycle_creating_dependencies =
      data["ignore_cycle_creating_dependencies"];
  ...
  json answer;
  try {
    auto wf = WfCommonsWorkflowParser::createWorkflowFromJSONString(
        json_string, reference_flop_rate, ignore_machine_specs,
    ...
    throw std::runtime_error("Error while importing workflow from JSON: " +
                             std::string(e.what()));
  }
}
