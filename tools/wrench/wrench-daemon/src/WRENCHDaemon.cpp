 ...
 * @param sleep_us number of micro-seconds or real time that the simulation
 * daemon's simulation execution_controller thread should sleep at each
 * iteration
 */
WRENCHDaemon::WRENCHDaemon(bool simulation_logging, bool daemon_logging,
                           int port_number, const std::string &allowed_origin,
                           int sleep_us)
    : simulation_logging(simulation_logging), daemon_logging(daemon_logging),
      port_number(port_number), sleep_us(sleep_us) {
  WRENCHDaemon::allowed_origins.push_back(allowed_origin);
}

/**
 * @brief Helper method to check whether a port is available for
 * binding/listening
...
bool WRENCHDaemon::isPortTaken(int port) {
  ...
  if (ret_value == 0) {
    ...
    throw std::runtime_error(
        "isPortTaken(): port should be either taken or not taken");
  }
}
 ...
 * @param port_number the port_number on which simulation client will need to
 * connect
 */
 ...
 */
void setSimulationStartFailureAnswer(Response &res,
 ...
 */
void writeStringToSharedMemorySegment(int shm_segment_id,
...
void WRENCHDaemon::startSimulation(const Request &req, Response &res) {
  ...
  try {
    ...
    setSimulationStartFailureAnswer(
        res, "Internal error: malformed json in request");
    return;
  }

  // Find an available port number on which the simulation daemon will be able
  // to run
  int simulation_port_number;
  while (isPortTaken(simulation_port_number =
                         PORT_MIN + rand() % (PORT_MAX - PORT_MIN)))
    ;

  // Create a shared memory segment, to which an error message will be written
  // by the child process (the simulation daemon) in case it fails on startup
  // due to a simulation creation failure
  auto shm_segment_id = shmget(IPC_PRIVATE, 4096, IPC_CREAT | SHM_R | SHM_W);
  if (shm_segment_id == -1) {
    perror("shmget()");
    setSimulationStartFailureAnswer(res,
                                    "Internal wrench-daemon error: shmget(): " +
  ...
  if (child_pid == -1) {
    perror("fork()");
    setSimulationStartFailureAnswer(res,
                                    "Internal wrench-daemon error: fork(): " +
                                        std::string(strerror(errno)));
    return;
  }
  ...
  if (!child_pid) { // The child process
    ...
    if (pipe(fd) == -1) {
      perror("pipe()");
      writeStringToSharedMemorySegment(
          shm_segment_id, "Internal wrench-daemon error: pipe(): " +
                              std::string(strerror(errno)));
      exit(1);
    }
    ...
    // trick). This trick is a life-saver here, since setting up a SIGCHLD
    // handler to reap children would get in the way of what we need to do in
    // the code hereafter.
    auto grand_child_pid = fork();
    if (grand_child_pid == -1) {
      perror("fork()");
      writeStringToSharedMemorySegment(
          shm_segment_id, "Internal wrench-daemon error: fork(): " +
                              std::string(strerror(errno)));
      exit(1);
    }
    ...
    if (!grand_child_pid) { // the grand-child
      ...
      // mutex/condvar for synchronization with the simulation thread I am about
      // to create
      std::mutex guard;
      std::condition_variable signal;

      // Create AND launch the simulation in a separate thread (it is tempting
      // to do the creation here and the launch in a thread, but it seems that
      // SimGrid does not like that - likely due to the maestro business)
      auto simulation_thread =
          std::thread([simulation_launcher, this, body, &guard, &signal]() {
            // Create simulation
            simulation_launcher->createSimulation(
                this->simulation_logging, body["platform_xml"],
                body["controller_hostname"], this->sleep_us);
            // Signal the parent thread that simulation creation has been done,
            // successfully or not
            {
      ...
      // Waiting for the simulation thread to have created the simulation,
      // successfully or not
      {
        std::unique_lock<std::mutex> lock(guard);
        signal.wait(lock);
      ...
      // If there was a simulation launch error, then put the error message in
      // the shared memory segment, communicate the failure to the parent
      // process via a pipe,  and exit
      if (simulation_launcher->launchError()) {
        simulation_thread.join(); // THIS IS NECESSARY, otherwise the exit
                                  // silently segfaults!
        // Put the error message in shared memory segment
        writeStringToSharedMemorySegment(
            shm_segment_id, simulation_launcher->launchErrorMessage());

        // Write success status to the parent
        bool success = false;
        if (write(fd[1], &success, sizeof(bool)) == -1) {
          perror("WARNING: write()"); // just a warning, since we're already in
                                      // error mode anyway
        }
        // Close the write-end of the pipe
        close(fd[1]);
        // Terminate with a non-zero error code, just for kicks (nobody's
        // calling waitpid)
        exit(1);
        ...
        if (write(fd[1], &success, sizeof(bool)) == -1) {
          perror("write()");
          writeStringToSharedMemorySegment(
              shm_segment_id, "Internal wrench-daemon error: write(): " +
                                  std::string(strerror(errno)));
          exit(1);
        }
    ...
    if (waitpid(child_pid, &stat_loc, 0) == -1) {
      perror("waitpid()");
      setSimulationStartFailureAnswer(
          res, "Internal wrench-daemon error: waitpid(): " +
                   std::string(strerror(errno)));
      return;
    }

    // Create json answer that will inform the client of success or failure,
    // based on child's exit code (which was relayed to this process from the
    // grand-child)
    if (WEXITSTATUS(stat_loc) == 0) {
      setSimulationStartSuccessAnswer(res, simulation_port_number);
    } else {
      // Grab the error message from the shared memory segment and set up the
      // failure answer
      setSimulationStartFailureAnswer(
          res, readStringFromSharedMemorySegment(shm_segment_id));
    }

    // Destroy the shared memory segment (important, since there is a limited
    // number of them we can create, and besides we should clean up after
    // ourselves)
...
void WRENCHDaemon::run() {
  ...
  server.Post("/api/startSimulation",
  ...
  server.set_error_handler([](const Request &req, Response &res) {
    WRENCHDaemon::error_handling(req, res);
  });
  ...
  while (true) {
    // This is in a while loop because, on Linux, the main process seems to
    // return from the listen() call below, not sure why... perhaps this while
    // loop could be removed, but it likely doesn't hurt
