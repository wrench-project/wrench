...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help", "Show this help message")(
      ...
      "port",
      po::value<int>()->default_value(8101)->notifier(in(1024, 49151, "port")),
      "port number, between 1024 and 4951, on which this daemon will listen")(
      "allow-origin", po::value<std::string>()->default_value(""),
      "allow origin for http connections to avoid CORS errors if needed (e.g., "
      "--allow-origin http://localhost:8000)")(
      "sleep-us",
      po::value<int>()->default_value(200)->notifier(
  ...
  WRENCHDaemon daemon(vm["simulation-logging"].as<bool>(),
                      vm["daemon-logging"].as<bool>(), vm["port"].as<int>(),
