...
/**
 * @brief A class that implements the Simulation Controller as a WMS. This is
 * the key WRENCH service that controls the simulation, handling all interaction
 * with a Simulation Daemon (which itself was spawned by wrench-daemon, and
 * which handles all interaction with the simulation client).
 */
...
private:
  template <class T> json startService(T *s);
  ...
  KeyValueStore<std::shared_ptr<FileRegistryService>>
      file_registry_service_registry;

  // Thread-safe queues for the server thread and the simulation thread to
  // communicate
  BlockingQueue<std::pair<double, std::shared_ptr<wrench::ExecutionEvent>>>
      event_queue;

  //        BlockingQueue<std::tuple<std::shared_ptr<StandardJob>,
  //        std::shared_ptr<ComputeService>, std::map<std::string,
  //        std::string>>> submissions_to_do;
  ...
  static json eventToJSON(double date,
                          const std::shared_ptr<wrench::ExecutionEvent> &event);
};
} // namespace wrench
