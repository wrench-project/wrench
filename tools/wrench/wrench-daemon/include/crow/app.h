...
public:
  ...
  /// Process only the method and URL of a request and provide a route (or an
  /// error response)
  ...
  /// Process a fully parsed request from start to finish (primarily used for
  /// debugging)
  ...
  void validate() {
    if (!validated_) {
      ...
      // stat on windows doesn't care whether '/' or '\' is being used. on Linux
      // however, using '\' doesn't work. therefore every instance of '\' gets
      // replaced with '/' then a check is done to make sure the directory ends
      // with '/'.
...
#endif
      {
        for (Blueprint *bp : router_.blueprints()) {
          if (!bp->static_dir().empty()) {
            // stat on windows doesn't care whether '/' or '\' is being used. on
            // Linux however, using '\' doesn't work. therefore every instance
            // of '\' gets replaced with '/' then a check is done to make sure
            // the directory ends with '/'.
            ...
            bp->new_rule_tagged<crow::black_magic::get_parameter_tag(
                ...
                [static_dir_](crow::response &res,
                              std::string file_path_partial) {
                  utility::sanitize_filename(file_path_partial);
                  res.set_static_file_info_unsafe(static_dir_ +
                                                  file_path_partial);
                  res.end();
                });
          }
