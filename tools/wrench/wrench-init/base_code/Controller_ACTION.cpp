 ...
 * @brief Constructor
 *
 * @param bare_metal_compute_service: a set of compute services available to run
 * actions
...
int Controller::main() {
  ...
  auto some_other_file =
      wrench::Simulation::addFile("some_other_file", 2 * GBYTE);
  wrench::StorageService::createFileAtLocation(
      wrench::FileLocation::LOCATION(this->storage_service, some_file));

  /* Create a job manager so that we can create/submit jobs */
  auto job_manager = this->createJobManager();

  WRENCH_INFO("Creating a compound job with a file read action followed by a "
              "compute action");
  auto job1 = job_manager->createCompoundJob("job1");
  auto fileread = job1->addFileReadAction(
      "fileread",
      wrench::FileLocation::LOCATION(this->storage_service, some_file));
  auto compute = job1->addComputeAction("compute", 100 * GFLOP, 50 * MBYTE, 1,
                                        3, wrench::ParallelModel::AMDAHL(0.8));
  job1->addActionDependency(fileread, compute);

  WRENCH_INFO("Creating a compound job with a file write action and a "
              "(simultaneous) sleep action");
  auto job2 = job_manager->createCompoundJob("job2");
  auto filewrite = job2->addFileWriteAction(
      "filewrite",
      wrench::FileLocation::LOCATION(this->storage_service, some_other_file));
  ...
  std::vector<std::shared_ptr<wrench::Action>> actions = {fileread, compute,
                                                          filewrite, sleep};
