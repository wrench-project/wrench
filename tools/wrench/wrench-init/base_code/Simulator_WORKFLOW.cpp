...
int main(int argc, char **argv) {
  ...
  auto wms = simulation->add(new wrench::Controller(
      workflow, baremetal_service, storage_service, "UserHost"));
  ...
  std::vector<std::shared_ptr<wrench::WorkflowTask>> tasks = {task1, task2,
                                                              task3, task4};
  for (auto const &t : tasks) {
    printf("Task %s: %.2fs - %.2fs\n", t->getID().c_str(), t->getStartDate(),
           t->getEndDate());
  }
