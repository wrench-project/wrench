...
WRENCH_LOG_CATEGORY(wfcommons_workflow_parser,
                    "Log category for WfCommonsWorkflowParser");
 ...
 */
std::shared_ptr<Workflow> WfCommonsWorkflowParser::createWorkflowFromJSON(
    const std::string &filename, const std::string &reference_flop_rate,
    bool ignore_machine_specs, bool redundant_dependencies,
    bool ignore_cycle_creating_dependencies, unsigned long min_cores_per_task,
    unsigned long max_cores_per_task, bool enforce_num_cores,
    bool ignore_avg_cpu, bool show_warnings) {
  std::ifstream file;
  ...
  try {
    ...
    throw std::invalid_argument(
        "WfCommonsWorkflowParser::createWorkflowFromJson(): Invalid JSON file "
        "(" +
        std::string(e.what()) + ")");
  }
}
 ...
 * Documentation in .h file
 */
std::shared_ptr<Workflow> WfCommonsWorkflowParser::createWorkflowFromJSONString(
  ...
  try {
    ...
    throw std::invalid_argument(
        "WfCommonsWorkflowParser::createWorkflowFromJson(): Could not find a "
        "'schema_version' key");
  }
  if (schema_version != "1.5") {
    throw std::invalid_argument(
        "WfCommonsWorkflowParser::createWorkflowFromJson(): Only handles "
        "WfFormat schema version 1.5 "
        "(use the script at "
        "https://github.com/wfcommons/WfFormat/tree/main/tools/ to update your "
        "workflow instances).");
  }
  ...
  if (not workflow_spec.contains("execution")) {
    throw std::invalid_argument(
        "WfCommonsWorkflowParser::createWorkflowFromJson(): The WfInstance "
        "doesn't contain a 'workflow/execution' key. "
        "Although this key isn't required in the WfInstances format, WRENCH "
        "requires it to determine task "
  ...
  if (workflow_spec.at("execution").contains("machines")) {
    ...
    for (auto const &machine_spec : machine_specs) {
      ...
      try {
        ...
        if (show_warnings)
          std::cerr << "[WARNING]: Machine " + name +
                           " does not define a speed\n";
        mhz = -1.0; // unknown
      }
  ...
  for (const auto &task_exec : task_execs) {
    ...
    if (not ignore_avg_cpu) {
      if ((num_cores < 0) and (avg_cpu < 0)) {
        ...
        if (show_warnings)
          std::cerr << "[WARNING]: Task " << task->getID() << " has avgCPU "
                    << avg_cpu << "% but does not specify the number of cores:"
                    << "Assuming " << std::ceil(avg_cpu / 100.0) << " cores\n";
        num_cores = std::ceil(avg_cpu / 100.0);
      } else if (avg_cpu < 0) {
        if (show_warnings)
          std::cerr << "[WARNING]: Task " + task->getID() +
                           " does not specify avgCPU: "
                           "Assuming 100%.\n";
        avg_cpu = 100.0 * num_cores;
      } else if (avg_cpu > 100 * num_cores) {
        if (show_warnings) {
          std::cerr << "[WARNING]: Task " << task->getID() << " specifies "
                    << (unsigned long)num_cores << " cores and avgCPU "
                    << avg_cpu << "%, "
                    << "which is impossible: Assuming avgCPU "
                    << 100.0 * num_cores << " instead.\n";
        }
        avg_cpu = 100.0 * num_cores;
      }
    } else {
    ...
    // Overwrite the default is we don't enforce the default values AND the JSON
    // specifies core numbers
    ...
    if (task_exec.contains("machines") and machines.size() > 0) {
      ...
      if (machines.size() > 1) {
        throw std::invalid_argument(
            "WfCommonsWorkflowParser::createWorkflowFromJSON(): Task " +
            task->getID() +
            " was executed on multiple machines, which WRENCH currently does "
            "not support");
      }
    ...
    if (ignore_machine_specs or execution_machine.empty()) {
      ...
      if (machines.find(execution_machine) == machines.end()) {
        throw std::invalid_argument(
            "WfCommonsWorkflowParser::createWorkflowFromJSON(): Task " +
            task->getID() + " is said to have been executed on machine " +
            execution_machine +
      ...
      if (machines[execution_machine].second >= 0) {
        ...
        double actual_flop_rate =
            total_compute_power_used * 1000.0 * 1000.0 * 1000.0;
        flop_amount = runtimeInSeconds * actual_flop_rate;
      } else {
        flop_amount = (double)min_num_cores * runtimeInSeconds *
                      flop_rate; // Assume a min-core execution
      }
    }
  ...
  for (auto const &task_spec : task_specs) {
    ...
    for (auto const &parent : task_spec.at("parents")) {
      try {
        ...
        workflow->addControlDependency(parent_task, task,
    ...
    for (auto const &child : task_spec.at("children")) {
      try {
        ...
        workflow->addControlDependency(task, child_task,
                                       redundant_dependencies);
      } catch (std::invalid_argument &e) {
        // do nothing
      } catch (std::runtime_error &e) {
