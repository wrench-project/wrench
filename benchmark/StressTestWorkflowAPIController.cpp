...
XBT_LOG_NEW_DEFAULT_CATEGORY(
    stress_test_workflow_controller,
    "Log category for Stress Test Workflow Controller");
...
int StressTestWorkflowAPIController::main() {
  ...
  for (unsigned int i = 0; i < this->num_jobs; i++) {
    shared_ptr<WorkflowTask> task =
        workflow->addTask("task_" + std::to_string(i), 1000.0, 1, 1, 1.0);
    task->addOutputFile(
        wrench::Simulation::addFile("outfile_" + std::to_string(i), 100000000));
    task->addInputFile(
        wrench::Simulation::addFile("infile_" + std::to_string(i), 100000000));
  }
  ...
  // REMOVE//std::set<std::shared_ptr<ComputeService>> compute_services =
  // this->getAvailableComputeServices<ComputeService>();
  // REMOVE//std::set<std::shared_ptr<StorageService>> storage_services =
  ...
  while ((not tasks_to_do.empty()) or (not tasks_pending.empty())) {
    ...
    while ((!tasks_to_do.empty()) and
           (tasks_pending.size() < max_num_pending_tasks)) {
      ...
      StorageService::createFileAtLocation(
          wrench::FileLocation::LOCATION(target_ss, input_file));
      auto job = job_manager->createStandardJob(
          to_submit,
          {{input_file, wrench::FileLocation::LOCATION(target_ss, input_file)},
           {output_file,
            wrench::FileLocation::LOCATION(target_ss, output_file)}});
    ...
    if (auto real_event =
            dynamic_cast<wrench::StandardJobCompletedEvent *>(event.get())) {
      shared_ptr<WorkflowTask> completed_task =
