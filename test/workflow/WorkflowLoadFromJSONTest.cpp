...
protected:
  ...
  std::string json_file_path =
      "../test/wfcommons_instances/1000genome-chameleon-2ch-100k-001.json";
};
...
TEST_F(WorkflowLoadFromJSONTest, LoadValidJSON) {
  ...
  ASSERT_THROW(workflow =
                   wrench::WfCommonsWorkflowParser::createWorkflowFromJSON(
                       "bogus", "1f", false),
               std::invalid_argument);
  ASSERT_NO_THROW(workflow =
                      wrench::WfCommonsWorkflowParser::createWorkflowFromJSON(
                          this->json_file_path, "1f", false));
  ...
  unsigned long num_input_files =
      workflow->getTaskByID("individuals_ID0000001")->getInputFiles().size();
  unsigned long num_output_files =
      workflow->getTaskByID("individuals_ID0000001")->getOutputFiles().size();

  ASSERT_EQ(num_input_files, 2);
  ASSERT_EQ(num_output_files, 1);

  //    ASSERT_NEAR(workflow->getTaskByID("individuals_ID0000001")->getFlops(),
  //    588.914, 0.001);
  ASSERT_EQ(workflow->getTaskByID("individuals_ID0000001")->getMinNumCores(),
            1);
  ASSERT_EQ(workflow->getTaskByID("individuals_ID0000001")->getMaxNumCores(),
            1);
