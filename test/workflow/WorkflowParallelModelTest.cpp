...
WRENCH_LOG_CATEGORY(parallel_model_test,
                    "Log category for Parallel Model Test");
...
protected:
  ParallelModelTest() {
    ...
    std::string xml =
        ...
        "       <route src=\"WMSHost\" dst=\"WMSHost\"> <link_ctn id=\"1\"/> "
        "</route>"
        "   </zone> "
        "</platform>";
...
public:
  ParallelModelTestWMS(ParallelModelTest *test,
...
private:
  ...
  int main() override {
    ...
    auto job =
        job_manager->createStandardJob(this->workflow->getTaskByID("task1"));
...
void ParallelModelTest::do_AdmdahlParallelModelTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new ParallelModelTestWMS(this, workflow, wms_host)));
  double work = 100.0;
  double alpha = 0.3;
  ...
  auto real_parallel_model =
      std::dynamic_pointer_cast<wrench::AmdahlParallelModel>(parallel_model);
  ...
  double makespan =
      task->getComputationEndDate() - task->getComputationStartDate();
  double expected_makespan = (alpha * work) / 4 + (1 - alpha) * work;

  if (std::abs(makespan - expected_makespan) > 0.001) {
    throw std::runtime_error(
        "Unexpected task1 makespan: " + std::to_string(makespan) +
...
void ParallelModelTest::do_CustomParallelModelTest_test() {
  ...
  task->setParallelModel(wrench::ParallelModel::CUSTOM(
      [](double work, unsigned long num_threads) { return 0.1 * work; },
  ...
  auto parallel_model = task->getParallelModel();
  auto real_parallel_model =
      std::dynamic_pointer_cast<wrench::CustomParallelModel>(parallel_model);
  ASSERT_NE(real_parallel_model, nullptr);
