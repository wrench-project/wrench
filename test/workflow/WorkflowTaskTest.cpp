...
public:
  ...
  std::shared_ptr<wrench::StorageService> storage_service = nullptr,
                                          backup_storage_service = nullptr;
  std::shared_ptr<wrench::FileRegistryService> file_registry_service = nullptr;

  std::shared_ptr<wrench::Workflow> workflow;
  std::shared_ptr<wrench::WorkflowTask> t1, t2, t4, t5, t6;
  std::shared_ptr<wrench::DataFile> large_input_file, small_input_file,
      t4_output_file;
...
TEST_F(WorkflowTaskTest, TaskStructure) {
  ...
  ASSERT_EQ(
      t2->getState(),
      wrench::WorkflowTask::State::NOT_READY); // due to control dependency
...
TEST_F(WorkflowTaskTest, GetSet) {
  ...
  ASSERT_EQ(t1->getInternalState(),
            wrench::WorkflowTask::InternalState::TASK_NOT_READY);
  ...
  // getting the execution_host before a task has run at least once should
  // return an empty string
  ASSERT_TRUE(t1->getExecutionHost().empty());
  ASSERT_TRUE(t1->getPhysicalExecutionHost().empty());

  ASSERT_NO_THROW(t1->setStartDate(
      1.0)); // need to set start date first before anything else
...
TEST_F(WorkflowTaskTest, InputOutputFile) {
  ...
  std::shared_ptr<wrench::DataFile> f1 =
      wrench::Simulation::addFile("file-01", 10);
  std::shared_ptr<wrench::DataFile> f2 =
      wrench::Simulation::addFile("file-02", 100);
  ...
  std::shared_ptr<wrench::WorkflowTask> t3 =
      workflow->addTask("task1-03", 50, 2, 4, 0);
  t3->addInputFile(f2);
...
TEST_F(WorkflowTaskTest, StateToString) {
  ...
  ASSERT_EQ(wrench::WorkflowTask::stateToString(
                wrench::WorkflowTask::State::NOT_READY),
            "NOT READY");
  ASSERT_EQ(
      wrench::WorkflowTask::stateToString(wrench::WorkflowTask::State::READY),
      "READY");
  ASSERT_EQ(
      wrench::WorkflowTask::stateToString(wrench::WorkflowTask::State::PENDING),
      "PENDING");
  ASSERT_EQ(wrench::WorkflowTask::stateToString(
                wrench::WorkflowTask::State::COMPLETED),
            "COMPLETED");
  ASSERT_EQ(
      wrench::WorkflowTask::stateToString(wrench::WorkflowTask::State::UNKNOWN),
      "UNKNOWN");
  ASSERT_EQ(
      wrench::WorkflowTask::stateToString((wrench::WorkflowTask::State)100),
      "INVALID");
  ...
  ASSERT_EQ(wrench::WorkflowTask::stateToString(
            ...
            "COMPLETED");
  ASSERT_EQ(wrench::WorkflowTask::stateToString(
                wrench::WorkflowTask::InternalState::TASK_FAILED),
            "FAILED");
  ASSERT_EQ(wrench::WorkflowTask::stateToString(
                (wrench::WorkflowTask::InternalState)100),
            "UNKNOWN STATE");
}
...
private:
  ...
  int main() override {
    ...
    auto job_that_will_fail = job_manager->createStandardJob(
        this->test->t4,
    ...
    // while large_input_file is being read, we delete small_input_file so that
    // the one task job will fail
    wrench::StorageService::deleteFileAtLocation(wrench::FileLocation::LOCATION(
        this->test->storage_service,
        wrench::Simulation::getFileByID("zz_small_input_file")));
    ...
    auto job_that_will_complete = job_manager->createStandardJob(
        this->test->t4,
        {{this->test->small_input_file,
    ...
    auto job_that_will_be_terminated =
        job_manager->createStandardJob(this->test->t5);
...
void WorkflowTaskTest::do_WorkflowTaskExecutionHistory_test(
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new WorkflowTaskExecutionHistoryTestWMS(this, wms_host)));
  ...
  // t4's second execution was successful, so all values in its history for the
  // second execution should be set
  wrench::WorkflowTask::WorkflowTaskExecution t4_successful_execution =
      t4_history.top();
  ...
  for (size_t i = 1; i < t4_successful_execution_history_values.size(); ++i) {
    ASSERT_GE(t4_successful_execution_history_values.at(i),
              t4_successful_execution_history_values.at(i - 1));
  }
  ...
  // t4's first execution was unsuccessful, only task_start, read_input_start,
  // execution_host should be set, everything else should be -1
  wrench::WorkflowTask::WorkflowTaskExecution t4_unsuccessful_execution =
      t4_history.top();
  ...
  ASSERT_STREQ(t4_unsuccessful_execution.execution_host.c_str(),
               "ExecutionHost");

  // the rest of the values should be set to -1 since the task failed while
  // reading input
  ...
  // t5 should have ran then been cleanly_terminated right after computation
  // started
  ...
  // t6 should have run then failed right after computation started because the
  // compute service was stopped
