  ...
  WorkflowTest() {
    ...
    try {
      ...
      throw std::runtime_error(
          "Creating a dependency cycle in workflow should throw");
    ...
    if ((tasks.find(t2) == tasks.end()) or (tasks.find(t3) == tasks.end())) {
      throw std::runtime_error(
          "getTasksThatInput() doesn't generate the same output");
    }
    t1->getPriority();
    try {
      t1->updateStartDate(666.6);
      throw std::runtime_error(
          "Should not be able to call WorkflowTask::updateStartDate()");
    } catch (std::runtime_error &ignore) {
    }
    try {
      t1->setTerminationDate(666.6);
      throw std::runtime_error(
          "Should not be able to call WorkflowTask::setTerminationDate()");
    } catch (std::runtime_error &ignore) {
    }
  }
...
TEST_F(WorkflowTest, WorkflowStructure) {
  ...
  ASSERT_THROW(workflow->getTaskNumberOfChildren(nullptr),
               std::invalid_argument);
  ASSERT_THROW(workflow->getTaskNumberOfParents(nullptr),
               std::invalid_argument);
  ...
  ASSERT_EQ(std::find(top_level_equal_to_1_or_2.begin(),
                      top_level_equal_to_1_or_2.end(), t1),
            top_level_equal_to_1_or_2.end());
  ASSERT_NE(std::find(top_level_equal_to_1_or_2.begin(),
                      top_level_equal_to_1_or_2.end(), t2),
            top_level_equal_to_1_or_2.end());
  ASSERT_NE(std::find(top_level_equal_to_1_or_2.begin(),
                      top_level_equal_to_1_or_2.end(), t3),
            top_level_equal_to_1_or_2.end());
  ASSERT_NE(std::find(top_level_equal_to_1_or_2.begin(),
                      top_level_equal_to_1_or_2.end(), t4),
            top_level_equal_to_1_or_2.end());
...
TEST_F(WorkflowTest, ControlDependency) {
  ...
  ASSERT_THROW(workflow->addControlDependency(nullptr, nullptr),
               std::invalid_argument);
  ASSERT_THROW(workflow->addControlDependency(t1, nullptr),
               std::invalid_argument);
  ASSERT_THROW(workflow->addControlDependency(nullptr, t1),
               std::invalid_argument);
  ...
  ASSERT_THROW(workflow->removeControlDependency(nullptr, t4),
               std::invalid_argument);
  ASSERT_THROW(workflow->removeControlDependency(t1, nullptr),
               std::invalid_argument);
...
void doTopBottomLevelsTest(bool dynamic_updates) {
  ...
  ASSERT_TRUE(std::find(tl_range_1.begin(), tl_range_1.end(), t2) !=
              tl_range_1.end());
  ASSERT_TRUE(std::find(tl_range_1.begin(), tl_range_1.end(), t3) !=
              tl_range_1.end());
  auto bl_range_1 = wf->getTasksInBottomLevelRange(1, 1);
  ASSERT_TRUE(bl_range_1.size() == 1);
  ASSERT_TRUE(std::find(bl_range_1.begin(), bl_range_1.end(), t3) !=
              bl_range_1.end());
...
TEST_F(WorkflowTest, WorkflowTaskThrow) {
  ...
  ASSERT_THROW(workflow->addTask("task1-error", -100, 1, 1, 0),
               std::invalid_argument);
  ASSERT_THROW(workflow->addTask("task1-error", 100, 2, 1, 0),
               std::invalid_argument);
  ASSERT_THROW(workflow->addTask("task1-error", 100, 1, 1, -1.0),
               std::invalid_argument);
  ...
  ASSERT_THROW(workflow->addTask("task1-test-01", 1, 1, 1, 0),
               std::invalid_argument);
  ...
  std::shared_ptr<wrench::WorkflowTask> bogus =
      bogus_workflow->addTask("bogus", 100.0, 1, 1, 0.0);
  ASSERT_THROW(bogus->setParallelModel(wrench::ParallelModel::AMDAHL(-2.0)),
               std::invalid_argument);
  ASSERT_THROW(bogus->setParallelModel(wrench::ParallelModel::AMDAHL(2.0)),
               std::invalid_argument);
  ASSERT_THROW(
      bogus->setParallelModel(wrench::ParallelModel::CONSTANTEFFICIENCY(-2.0)),
      std::invalid_argument);
  ASSERT_THROW(
      bogus->setParallelModel(wrench::ParallelModel::CONSTANTEFFICIENCY(2.0)),
      std::invalid_argument);
  ...
  //  ASSERT_THROW(workflow->updateTaskState(nullptr,
  //  wrench::WorkflowTask::State::FAILED), std::invalid_argument);
}

TEST_F(WorkflowTest, DataFile) {
  ASSERT_THROW(wrench::Simulation::addFile("file-error-00", -1),
               std::invalid_argument);
  ASSERT_THROW(wrench::Simulation::addFile("file-01", 10),
               std::invalid_argument);

  ASSERT_THROW(wrench::Simulation::getFileByID("file-nonexist"),
               std::invalid_argument);
...
TEST_F(WorkflowTest, SumFlops) {
  ...
  ASSERT_NO_THROW(sum_flops =
                      wrench::Workflow::getSumFlops(workflow->getTasks()));
...
TEST_F(WorkflowTest, LowLevelDagOfTasksTest) {
  ...
  ASSERT_THROW(dag.removeVertex((wrench::WorkflowTask *)1),
               std::invalid_argument);
  ASSERT_NO_THROW(dag.addVertex((wrench::WorkflowTask *)1));
  ASSERT_NO_THROW(dag.addVertex((wrench::WorkflowTask *)2));

  ASSERT_NO_THROW(
      dag.addEdge((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)2));
  ASSERT_NO_THROW(
      dag.removeEdge((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)2));
  ASSERT_THROW(
      dag.removeEdge((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)3),
      std::invalid_argument);
  ASSERT_THROW(
      dag.removeEdge((wrench::WorkflowTask *)3, (wrench::WorkflowTask *)1),
      std::invalid_argument);
  ASSERT_THROW(
      dag.addEdge((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)3),
      std::invalid_argument);
  ASSERT_THROW(
      dag.addEdge((wrench::WorkflowTask *)3, (wrench::WorkflowTask *)1),
      std::invalid_argument);

  ASSERT_NO_THROW(
      dag.doesPathExist((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)2));
  ASSERT_THROW(
      dag.doesPathExist((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)3),
      std::invalid_argument);
  ASSERT_THROW(
      dag.doesPathExist((wrench::WorkflowTask *)3, (wrench::WorkflowTask *)1),
      std::invalid_argument);

  ASSERT_NO_THROW(
      dag.doesEdgeExist((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)2));
  ASSERT_THROW(
      dag.doesEdgeExist((wrench::WorkflowTask *)1, (wrench::WorkflowTask *)3),
      std::invalid_argument);
  ASSERT_THROW(
      dag.doesEdgeExist((wrench::WorkflowTask *)3, (wrench::WorkflowTask *)1),
      std::invalid_argument);

  ASSERT_NO_THROW(dag.getNumberOfChildren((wrench::WorkflowTask *)1));
  ASSERT_THROW(dag.getNumberOfChildren((wrench::WorkflowTask *)3),
               std::invalid_argument);

  ASSERT_NO_THROW(dag.getChildren((wrench::WorkflowTask *)1));
  ASSERT_THROW(dag.getChildren((wrench::WorkflowTask *)3),
               std::invalid_argument);

  ASSERT_NO_THROW(dag.getNumberOfParents((wrench::WorkflowTask *)1));
  ASSERT_THROW(dag.getNumberOfParents((wrench::WorkflowTask *)3),
               std::invalid_argument);

  ASSERT_NO_THROW(dag.getParents((wrench::WorkflowTask *)1));
  ASSERT_THROW(dag.getParents((wrench::WorkflowTask *)3),
               std::invalid_argument);
}
