...
WRENCH_LOG_CATEGORY(energy_consumption_test,
                    "Log category for EnergyConsumptionTest");
...
protected:
  ...
  EnergyConsumptionTest() {
    ...
    std::string xml =
        ...
        "value=\"100.000000:200.000000:200.000000,93.000000:170.000000:170."
        "000000,90.000000:150.000000:150.000000\" />"
...
private:
  ...
  int main() override {
...

    {
      ...
      std::vector<std::string> simulation_hosts =
          wrench::Simulation::getHostnameList();

      // Now based on this default speed, (100MF), execute a job requiring 10^10
      // flops and check the time
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task1", 10000000000, 1, 1, 1.0);

      // Create a StandardJob
      auto job = job_manager->createStandardJob(task);
      // sleep for 10 seconds
      wrench::S4U_Simulation::sleep(10);
      // let's execute the job, this should take ~100 sec based on the 100MF
      // speed
      ...
      try {
        std::map<std::string, double> value =
            this->simulation->getEnergyConsumed(
                std::vector<std::string>({"dummy_unavailable_host"}));
        throw std::runtime_error(
            "Should not have been able to read the energy for dummy hosts");
      } catch (std::invalid_argument &e) {
      }
      ...
      try {
        double value =
            this->simulation->getCurrentPstate("dummy_unavailable_host");
        throw std::runtime_error(
            "Should not have been able to read the energy for dummy hosts");
      } catch (std::invalid_argument &e) {
      }
      ...
      try {
        double value =
            this->simulation->getMaxPowerConsumption("dummy_unavailable_host");
        throw std::runtime_error(
            "Should not have been able to read the energy for dummy hosts");
      } catch (std::invalid_argument &e) {
      }
...
void EnergyConsumptionTest::do_AccessEnergyApiExceptionTests_test() {
  ...
  EXPECT_NO_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          hostname,
          {std::make_pair(compute_hostname,
  ...
  EXPECT_NO_THROW(wms = simulation->add(
                      new EnergyApiAccessExceptionsTestWMS(this, hostname)));
...
class EnergyApiAccessExceptionsPluginNotActiveTestWMS
  ...
  int main() override {
...

    {
      ...
      try {
        ...
        throw std::runtime_error("Should not have been able to read the energy "
                                 "without activating energy plugin");
      ...
      try {
        ...
        throw std::runtime_error("Should not have been able to set the pstate "
                                 "without activating energy plugin");
      } catch (std::runtime_error &e) {
      }
    }
...
void EnergyConsumptionTest::
    do_AccessEnergyApiExceptionPluginNotActiveTests_test() {
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(
          new EnergyApiAccessExceptionsPluginNotActiveTestWMS(this, hostname)));
...
public:
  EnergyConsumptionTestWMS(EnergyConsumptionTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
  ...
  int main() override {
...

    {
      ...
      if (std::abs(observed_duration - expected_duration) > EPSILON) {
        throw std::runtime_error(
            "EnergyConsumptionTest::SimpleEnergyConsumptionTest(): Took more "
            "time to compute than expected with the current speed of the host");
      }
    }
...
void EnergyConsumptionTest::do_EnergyConsumption_test() {
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(new EnergyConsumptionTestWMS(this, hostname)));
...
public:
  EnergyAPICheckTestWMS(EnergyConsumptionTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
  ...
  int main() override {
    ...
    auto job_manager = this->createJobManager();
    {
      ...
      try {
        ...
        throw std::runtime_error(
            "Shouldn't be able to get list of pstate for a bogus host");
      ...
      std::vector<int> list_of_pstates =
          wrench::Simulation::getListOfPstates(simulation_hosts[1]);
      ...
      if (cur_pstate ==
          this->simulation->getCurrentPstate(simulation_hosts[1])) {
        throw std::runtime_error(
            "The pstate should have changed but it did not change");
      }

      // check if the max power possible/min power available in this pstate is
      // different than the maximum power possible/min power available in the
      // previous state
      for (auto host : simulation_hosts) {
        ...
        for (auto state : states) {
          ...
          if (prev_max_power ==
              this->simulation->getMaxPowerConsumption(host)) {
            throw std::runtime_error(
          ...
          this->simulation->setPstate(host, state);
          if (prev_min_power ==
              this->simulation->getMinPowerConsumption(host)) {
            throw std::runtime_error(
      ...
      // lets check if the energy consumed by host1 is less than the energy
      // consumed by host1 + host2
      double energy_consumed_1 =
          this->simulation->getEnergyConsumed(simulation_hosts[1]);
      std::map<std::string, double> energy_consumed_2_map =
          this->simulation->getEnergyConsumed(std::vector<std::string>(
              {simulation_hosts[1], simulation_hosts[2]}));
      ...
      if (energy_consumed_1 > energy_consumed_2) {
        throw std::runtime_error(
            "Energy consumed by host X is greater than the combined energy "
            "consumed by host X and host Y");
      }
    }
...
void EnergyConsumptionTest::do_SimpleApiChecksEnergy_test() {
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(new EnergyAPICheckTestWMS(this, hostname)));
...
class EnergyConsumptionPStateChangeTestWMS
    : public wrench::ExecutionController {
...
private:
  ...
  int main() override {
    ...
    // The tests is just to switch pstate and check if energy consumed is +ve
    // and we don't have any segfaults or something
    //  Create a job manager
    auto job_manager = this->createJobManager();

    {
      ...
      std::shared_ptr<wrench::WorkflowTask> task2 =
          this->test->workflow->addTask("task2", 10000000000, 1, 1, 1.0);

      // Create a StandardJob
      auto job2 = job_manager->createStandardJob(task2);

      // First energy consumption test
      double before_current_energy_consumed_by_host1 =
          this->simulation->getEnergyConsumed(simulation_hosts[1]);
      // run a new job
      // let's execute the job, this should take ~100 sec based on the 100MF
      // speed
      ...
      double after_current_energy_consumed_by_host1 =
          this->simulation->getEnergyConsumed(simulation_hosts[1]);
      double energy_consumed_while_running_with_higher_speed =
          after_current_energy_consumed_by_host1 -
          before_current_energy_consumed_by_host1;
      double higher_speed_compuation_time = wrench::S4U_Simulation::getClock();

      if (energy_consumed_while_running_with_higher_speed <= 0) {
        throw std::runtime_error("Unexpectedly the energy consumed is less "
                                 "than 0 for the max speed??");
      }

      // switch pstate
      int max_pstate_possible =
          this->simulation->getNumberofPstates(simulation_hosts[1]);
      // let's directly switch to pstate 2
      int pstate = 2;
      this->simulation->setPstate(simulation_hosts[1], pstate);

      // Second energy consumption test
      double before_current_energy_consumed_by_host2 =
          this->simulation->getEnergyConsumed(simulation_hosts[1]);
      // run a new job
      // let's execute the job, this should take ~100 sec based on the 100MF
      // speed
      ...
      double after_current_energy_consumed_by_host2 =
          this->simulation->getEnergyConsumed(simulation_hosts[1]);
      double energy_consumed_while_running_with_lower_speed =
          after_current_energy_consumed_by_host2 -
          before_current_energy_consumed_by_host2;
      double lower_speed_computation_time =
          wrench::S4U_Simulation::getClock() - higher_speed_compuation_time;

      if (energy_consumed_while_running_with_lower_speed <= 0) {
        throw std::runtime_error("Unexpectedly the energy consumed is less "
                                 "than 0 for a lower speed ??");
      }

      // check if the power states and times map with energy consumed
      // 100.0:200.0, 93.0:170.0, 90.0:150.0
      // in pstate 0, the min_power, according to xml is 100.0 and the max power
      // is 200.0 in pstate 2, the min_power, according to xml is 90.0 and the
      // max power is 150.0 so, energy_consumed/time_taken might give us an
      // approximate wattage power which should be in between these ranges in
      // fact, we are using these hosts to the full power, so the power wattage
      // should be near the max values

      double exact_max_wattage_power_1 =
          wrench::Simulation::getMaxPowerConsumption(simulation_hosts[1]);
      double exact_max_wattage_power_2 =
          wrench::Simulation::getMaxPowerConsumption(simulation_hosts[1]);
      double EPSILON = 1.0;
      double computed_wattage_power_1 =
          energy_consumed_while_running_with_higher_speed /
          higher_speed_compuation_time;
      double computed_wattage_power_2 =
          energy_consumed_while_running_with_lower_speed /
          lower_speed_computation_time;

      if (std::abs(exact_max_wattage_power_1 - computed_wattage_power_1) >
              EPSILON &&
          std::abs(exact_max_wattage_power_2 - computed_wattage_power_2) >
              EPSILON) {
        throw std::runtime_error("Something wrong with the computed energy and "
...
void EnergyConsumptionTest::do_EnergyConsumptionPStateChange_test() {
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(
          new EnergyConsumptionPStateChangeTestWMS(this, hostname)));
...
public:
  PluginNotActivatedTestWMS(EnergyConsumptionTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
void EnergyConsumptionTest::do_PluginNotActive_test() {
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(new PluginNotActivatedTestWMS(this, hostname)));
