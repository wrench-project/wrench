...
private:
    ...
    int main() override {
        ...
        // Starting a victim on the FailedHost (that will reply with a bogus TTL Expiration message)
        victim = std::shared_ptr<wrench::Sleeper>(new wrench::Sleeper("FailedHostTrace", 100, new wrench::ServiceTTLExpiredMessage(1), this->mailbox_name));
        victim->simulation = this->simulation;
        victim->start(victim, true, false); // Daemonized, no auto-restart
        ...
        // Starting a victim (that will reply with a bogus TTL Expiration message)
        victim = std::shared_ptr<wrench::Sleeper>(new wrench::Sleeper("FailedHostTrace", 50, new wrench::ServiceTTLExpiredMessage(1), this->mailbox_name));
        victim->simulation = this->simulation;
        victim->start(victim, true, false); // Daemonized, no auto-restart
