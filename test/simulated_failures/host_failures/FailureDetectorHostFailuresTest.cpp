...
WRENCH_LOG_CATEGORY(failure_detector_host_failure_test,
                    "Log category for FailureDetectorHostFailuresTest");
...
private:
  ...
  int main() override {
    ...
    auto victim1 =
        std::shared_ptr<wrench::SleeperVictim>(new wrench::SleeperVictim(
            "FailedHost", 200, new wrench::ServiceDaemonStoppedMessage(1),
            this->commport));
    ...
    auto failure_detector1 =
        std::shared_ptr<wrench::ServiceTerminationDetector>(
            new wrench::ServiceTerminationDetector(
                "StableHost", victim1, this->commport, true, false));
    failure_detector1->setSimulation(this->simulation);
    failure_detector1->start(failure_detector1, true,
                             false); // Daemonized, no auto-restart

    // Starting a victim on the FailedHostTrace, which should fail at time 100
    auto victim2 =
        std::shared_ptr<wrench::SleeperVictim>(new wrench::SleeperVictim(
            "FailedHostTrace", 200, new wrench::ServiceDaemonStoppedMessage(1),
            this->commport));
    victim2->setSimulation(this->simulation);
    victim2->start(victim2, true, false); // Daemonized, no auto-restart

    // Starting the failure detector!
    auto failure_detector2 =
        std::shared_ptr<wrench::ServiceTerminationDetector>(
            new wrench::ServiceTerminationDetector(
                "StableHost", victim2, this->commport, true, false));
    failure_detector2->setSimulation(this->simulation);
    failure_detector2->start(failure_detector2, true,
                             false); // Daemonized, no auto-restart
    ...
    try {
      ...
      throw std::runtime_error("Network error while getting a message!" +
                               cause->toString());
    }

    auto real_msg =
        std::dynamic_pointer_cast<wrench::ServiceHasCrashedMessage>(message);
    if (not real_msg) {
      throw std::runtime_error("Unexpected " + message->getName() + " message");
    } else {
      if (real_msg->service != victim1) {
        throw std::runtime_error(
            "Got a failure notification, but not for the right service!");
      }
    }
...
private:
  ...
  int main() override {
    ...
    // Starting a victim on the FailedHost, which should fail at time 50
    auto victim1 =
        std::shared_ptr<wrench::ComputerVictim>(new wrench::ComputerVictim(
            "FailedHost", 200, new wrench::ServiceDaemonStoppedMessage(1),
            this->commport));
    victim1->setSimulation(this->simulation);
    victim1->start(victim1, true, false); // Daemonized, no auto-restart
