...
WRENCH_LOG_CATEGORY(service_start_restart_test,
                    "Log category for ServiceStartRestartTest");
  ...
  int main() override {
    ...
    auto sleeper =
        std::shared_ptr<wrench::SleeperVictim>(new wrench::SleeperVictim(
            "FailedHost", 100, new wrench::ServiceDaemonStoppedMessage(1),
            this->commport));
...
void ServiceReStartHostFailuresTest::do_StartServiceOnDownHostTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new StartServiceOnDownHostTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    if (not std::dynamic_pointer_cast<wrench::ServiceDaemonStoppedMessage>(
            message)) {
...
void ServiceReStartHostFailuresTest::do_ServiceRestartTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new ServiceRestartTestWMS(this, hostname)));
