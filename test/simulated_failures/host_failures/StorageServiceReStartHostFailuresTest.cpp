...
WRENCH_LOG_CATEGORY(storage_service_start_restart_host_failures_test,
                    "Log category for StorageServiceReStartHostFailuresTest");
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not have been able to read the file (first attempt)");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should have been able to read the file (second attempt)");
    }

    // Starting a FailedHost murderer!!
    murderer = std::make_shared<wrench::ResourceSwitcher>(
        "StableHost", 100, "FailedHost",
        wrench::ResourceSwitcher::Action::TURN_OFF,
        wrench::ResourceSwitcher::ResourceType::HOST);
    murderer->setSimulation(this->simulation);
    ...
    // Starting a FailedHost resurector!!
    resurector = std::make_shared<wrench::ResourceSwitcher>(
        "StableHost", 500, "FailedHost",
        wrench::ResourceSwitcher::Action::TURN_ON,
        wrench::ResourceSwitcher::ResourceType::HOST);
    resurector->setSimulation(this->simulation);
...
TEST_F(StorageServiceReStartHostFailuresTest,
       DISABLED_StorageServiceReStartTest) {
  DO_TEST_WITH_FORK(do_StorageServiceRestartTest_test);
}

void StorageServiceReStartHostFailuresTest::
    do_StorageServiceRestartTest_test() {
