...
WRENCH_LOG_CATEGORY(
    comprehensive_integration_host_failure_test,
    "Log category for ComprehensiveIntegrationHostFailuresTest");

#define NUM_TASKS 100
#define MAX_TASK_DURATION_WITH_ON_CORE 3600
#define CHAOS_MONKEY_MIN_SLEEP_BEFORE_OFF 100
#define CHAOS_MONKEY_MAX_SLEEP_BEFORE_OFF                                      \
  3000 // The bigger this number the less flaky the platform
#define CHAOS_MONKEY_MIN_SLEEP_BEFORE_ON 100
#define CHAOS_MONKEY_MAX_SLEEP_BEFORE_ON                                       \
  2000 // The bigger this number the more flaky the platform
...
protected:
  std::string createRoute(const std::string &src, const std::string &dst,
  ...
  ComprehensiveIntegrationHostFailuresTest() {
    ...
    std::vector<std::string> hostnames = {
        "WMSHost",        "StorageHost1",  "StorageHost2", "CloudHead",
        "CloudHost1",     "CloudHost2",    "CloudHost3",   "BareMetalHead",
        "BareMetalHost1", "BareMetalHost2"};
    ...
    xml +=
        "<link id=\"linkcloudinternal\" bandwidth=\"1kBps\" latency=\"0\"/>\n";
    xml += "<link id=\"linkbaremetal\" bandwidth=\"1kBps\" latency=\"0\"/>\n";
    xml += "<link id=\"linkbaremetalinternal\" bandwidth=\"1kBps\" "
           "latency=\"0\"/>\n";
    ...
    xml +=
        createRoute("CloudHead", "CloudHost1", {"link", "linkcloudinternal"});
    xml +=
        createRoute("CloudHead", "CloudHost2", {"link", "linkcloudinternal"});
    xml +=
        createRoute("CloudHead", "CloudHost3", {"link", "linkcloudinternal"});

    xml += createRoute("BareMetalHead", "BareMetalHost1",
                       {"link", "linkbaremetalinternal"});
    xml += createRoute("BareMetalHead", "BareMetalHost2",
                       {"link", "linkbaremetalinternal"});

    xml += createRoute("StorageHost1", "CloudHost1",
                       {"linkstorage1", "linkcloud", "linkcloudinternal"});
    xml += createRoute("StorageHost1", "CloudHost2",
                       {"linkstorage1", "linkcloud", "linkcloudinternal"});
    xml += createRoute("StorageHost1", "CloudHost3",
                       {"linkstorage1", "linkcloud", "linkcloudinternal"});
    xml += createRoute("StorageHost2", "CloudHost1",
                       {"linkstorage2", "linkcloud", "linkcloudinternal"});
    xml += createRoute("StorageHost2", "CloudHost2",
                       {"linkstorage2", "linkcloud", "linkcloudinternal"});
    xml += createRoute("StorageHost2", "CloudHost3",
                       {"linkstorage2", "linkcloud", "linkcloudinternal"});

    xml +=
        createRoute("StorageHost1", "BareMetalHost1",
                    {"linkstorage1", "linkbaremetal", "linkbaremetalinternal"});
    xml +=
        createRoute("StorageHost1", "BareMetalHost2",
                    {"linkstorage1", "linkbaremetal", "linkbaremetalinternal"});
    xml +=
        createRoute("StorageHost2", "BareMetalHost1",
                    {"linkstorage2", "linkbaremetal", "linkbaremetalinternal"});
    xml +=
        createRoute("StorageHost2", "BareMetalHost2",
                    {"linkstorage2", "linkbaremetal", "linkbaremetalinternal"});
...
private:
  ...
  void createMonkey(const std::string &victimhost, double alpha = 1.0) {
    ...
    auto switcher = std::make_shared<wrench::ResourceRandomRepeatSwitcher>(
        "WMSHost", seed, CHAOS_MONKEY_MIN_SLEEP_BEFORE_OFF,
        CHAOS_MONKEY_MAX_SLEEP_BEFORE_OFF, CHAOS_MONKEY_MIN_SLEEP_BEFORE_ON,
        CHAOS_MONKEY_MAX_SLEEP_BEFORE_ON, victimhost,
        wrench::ResourceRandomRepeatSwitcher::ResourceType::HOST);
  ...
  int main() override {
    ...
    while (not this->test->workflow->isDone()) {
      ...
      if (wrench::Simulation::getCurrentSimulatedDate() >
          17508.33982809935332625173) {
        break;
      }
      ...
      for (auto const &vm : this->vms) {
        if (this->test->cloud_service->isVMDown(vm)) {
          try {
            ...
            if (std::dynamic_pointer_cast<wrench::NotEnoughResources>(
                    e.getCause())) {
              //  WRENCH_INFO("Cannot start VM");
            } else if (std::dynamic_pointer_cast<wrench::HostError>(
                           e.getCause())) {
  ...
  bool scheduleAReadyTask() {
    ...
    } else if (this->test->storage_service2 and
               this->test->storage_service2->isUp()) {
    ...
    auto job = this->job_manager->createStandardJob(
        task, {
                  {task->getInputFiles().at(0),
                   wrench::FileLocation::LOCATION(target_storage_service,
                                                  task->getInputFiles().at(0))},
                  {task->getOutputFiles().at(0),
                   wrench::FileLocation::LOCATION(
                       target_storage_service, task->getOutputFiles().at(0))},
              });
    this->job_manager->submitJob(job, target_cs);

    //        WRENCH_INFO("Submitted task1 '%s' to '%s' with files to read from
    //        '%s",
    //                    task1->getID().c_str(),
  ...
  void processEventStandardJobCompletion(
      std::shared_ptr<wrench::StandardJobCompletedEvent> event) override {
    static int count = 0;
  ...
  void processEventStandardJobFailure(
      std::shared_ptr<wrench::StandardJobFailedEvent> event) override {
    auto task = *(event->standard_job->getTasks().begin());

    WRENCH_INFO("Task '%s' has failed: %s", task->getID().c_str(),
...
TEST_F(ComprehensiveIntegrationHostFailuresTest,
       OneNonFaultyStorageOneFaultyBareMetal) {
...
TEST_F(ComprehensiveIntegrationHostFailuresTest,
       OneFaultyStorageOneNonFaultyBareMetal) {
  std::map<std::string, bool> args;
  args["storage1"] = true;
...
TEST_F(ComprehensiveIntegrationHostFailuresTest,
       OneFaultyStorageOneFaultyBareMetal) {
  std::map<std::string, bool> args;
  args["storage1"] = true;
  args["baremetal"] = true;
  DO_TEST_WITH_FORK_ONE_ARG(do_IntegrationFailureTest_test, args);
...
TEST_F(ComprehensiveIntegrationHostFailuresTest,
       TwoFaultyStorageOneFaultyBareMetal) {
  std::map<std::string, bool> args;
  args["storage1"] = true;
  args["storage2"] = true;
  args["baremetal"] = true;
...
TEST_F(ComprehensiveIntegrationHostFailuresTest,
       OneNonFaultyStorageOneFaultyCloud) {
  std::map<std::string, bool> args;
  args["storage1"] = false;
  args["cloud"] = true;
  DO_TEST_WITH_FORK_ONE_ARG(do_IntegrationFailureTest_test, args);
...
TEST_F(ComprehensiveIntegrationHostFailuresTest,
       OneFaultyStorageOneFaultyCloud) {
  std::map<std::string, bool> args;
  args["storage1"] = true;
  args["cloud"] = true;
  DO_TEST_WITH_FORK_ONE_ARG(do_IntegrationFailureTest_test, args);
...
TEST_F(ComprehensiveIntegrationHostFailuresTest,
       TwoFaultyStorageOneFaultyCloud) {
  std::map<std::string, bool> args;
  args["storage1"] = true;
  args["storage2"] = true;
  args["cloud"] = true;
...
void ComprehensiveIntegrationHostFailuresTest::do_IntegrationFailureTest_test(
    std::map<std::string, bool> args) {
  ...
  if (args.find("storage1") != args.end()) {
    this->storage_service1 = simulation->add(
  ...
  if (args.find("storage2") != args.end()) {
    this->storage_service2 = simulation->add(
        wrench::SimpleStorageService::createSimpleStorageService(
            "StorageHost2", {"/"},
            {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE,
              std::to_string(1000000)}},
  ...
  ASSERT_TRUE((this->storage_service1 != nullptr) or
              (this->storage_service2 != nullptr));

  // Create BareMetal Service
  if (args.find("baremetal") != args.end()) {
    this->baremetal_service =
        std::dynamic_pointer_cast<wrench::BareMetalComputeService>(
  ...
  if (args.find("cloud") != args.end()) {
    ...
    this->cloud_service =
        std::dynamic_pointer_cast<wrench::CloudComputeService>(
            simulation->add(new wrench::CloudComputeService(
                cloudhead, cloudhosts, {"/scratch"}, {}, {})));
  }
  ...
  for (int i = 0; i < NUM_TASKS; i++) {
    //        auto task1 = workflow->addTask("task_" + std::to_string(i), 1 +
    //        rand() % MAX_TASK_DURATION_WITH_ON_CORE, 1, 3, 1.0, 0);
    auto task = workflow->addTask("task_" + std::to_string(i),
                                  MAX_TASK_DURATION_WITH_ON_CORE, 1, 3, 40);
    auto input_file =
        wrench::Simulation::addFile(task->getID() + ".input", 1 + rand() % 100);
    auto output_file = wrench::Simulation::addFile(task->getID() + ".output",
                                                   1 + rand() % 100);
  ...
  auto wms = simulation->add(new IntegrationFailureTestTestWMS(this, wms_host));
