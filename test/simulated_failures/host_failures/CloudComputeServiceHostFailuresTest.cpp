...
WRENCH_LOG_CATEGORY(cloud_compute_service_host_failures_test,
                    "Log category for CloudServiceHostFailuresTest");
...
private:
  ...
  int main() override {
    ...
    auto vm_name =
        cloud_service->createVM(1, this->test->task->getMemoryRequirement());
    ...
    auto cause =
        std::dynamic_pointer_cast<wrench::HostError>(real_event->failure_cause);
    if (not cause) {
      throw std::runtime_error("Invalid failure cause type: " +
    ...
    // Restart the VM (which shouldn't work because the Cloud has no more
    // resources right now)
    try {
      cloud_service->startVM(vm_name);
      throw std::runtime_error("Should not be able to restart VM since the "
                               "CloudComputeService no longer has resources!");
...
void CloudServiceHostFailuresTest::
    do_CloudServiceFailureOfAVMWithRunningJob_test() {
  ...
  compute_service = simulation->add(new wrench::CloudComputeService(
      stable_host, compute_hosts, "/scratch", {}));
...
class CloudServiceFailureOfAVMAndRestartTestWMS
  ...
  int main() override {
    ...
    if (not cause) {
      throw std::runtime_error(
          "Invalid failure cause: " + real_event->failure_cause->toString() +
          " (expected: HostError)");
    }
...
TEST_F(CloudServiceHostFailuresTest,
       FailureOfAVMAndRestartWithRunningJobAndRestart) {
  DO_TEST_WITH_FORK(
      do_CloudServiceFailureOfAVMWithRunningJobFollowedByRestart_test);
}
...
void CloudServiceHostFailuresTest::
  ...
  simulation->add(
      new CloudServiceFailureOfAVMAndRestartTestWMS(this, stable_host));
  ...
  int main() override {
    ...
    for (unsigned long trial = 0; trial < NUM_TRIALS; trial++) {
      ...
      auto switch2 = std::make_shared<wrench::ResourceRandomRepeatSwitcher>(
          "StableHost", seed1, 10, 100, 10, 100, "FailedHost2",
          wrench::ResourceRandomRepeatSwitcher::ResourceType::HOST);
      ...
      auto output_file = wrench::Simulation::addFile(
          "output_file_" + std::to_string(trial), 2000);
      ...
      do {
        ...
        try {
          ...
          vm_name = cloud_service->createVM(task->getMinNumCores(),
        ...
        //                                WRENCH_INFO("*** WAS ABLE TO START THE
        //                                VM AFTER %lu attempts",
        //                                num_vm_start_attempts);
        num_vm_start_attempts = 0;

        // Submit the standard job to the compute service, making it sure it
        // runs on FailedHost1
        job_manager->submitJob(job, vm_cs);
      ...
      WRENCH_INFO("*** WAS ABLE TO RUN THE JOB AFTER %lu attempts (%lu VM "
                  "start attempts)",
                  num_job_submission_attempts, total_num_vm_start_attempts);
      switch1->kill();
      switch2->kill();
...
void CloudServiceHostFailuresTest::do_CloudServiceRandomFailures_test() {
  ...
  wms =
      simulation->add(new CloudServiceRandomFailuresTestWMS(this, stable_host));
