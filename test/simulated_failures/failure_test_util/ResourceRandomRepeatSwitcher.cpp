...
WRENCH_LOG_CATEGORY(resource_random_repeat_switcher,
                    "Log category for ResourceRandomRepeatSwitcher");

wrench::ResourceRandomRepeatSwitcher::ResourceRandomRepeatSwitcher(
...
void wrench::ResourceRandomRepeatSwitcher::kill() { this->killActor(); }

int wrench::ResourceRandomRepeatSwitcher::main() {

  // Type of random number distribution
  std::uniform_real_distribution<double> dist_off(
      this->min_sleep_before_off_time, this->max_sleep_before_off_time);
  std::uniform_real_distribution<double> dist_on(
      this->min_sleep_before_on_time, this->max_sleep_before_on_time);
  ...
  while (true) {
    ...
    WRENCH_INFO("CHAOS MONKEY: Sleeping for %.3lf seconds...",
                sleep_before_off_time);
    wrench::Simulation::sleep(sleep_before_off_time);
    switch (this->resource_type) {
    case ResourceType::HOST: {
      WRENCH_INFO("CHAOS MONKEY: Turning OFF host %s",
                  this->resource_to_switch.c_str());
      wrench::Simulation::turnOffHost(this->resource_to_switch);
      break;
    }
    case ResourceType::LINK: {
      WRENCH_INFO("CHAOS MONKEY: Turning OFF link %s",
                  this->resource_to_switch.c_str());
    ...
    WRENCH_INFO("CHAOS MONKEY: Sleeping for %.3lf seconds...",
                sleep_before_on_time);
    wrench::Simulation::sleep(sleep_before_on_time);
    switch (this->resource_type) {
    case ResourceType::HOST: {
      WRENCH_INFO("CHAOS MONKEY: Turning ON host %s",
                  this->resource_to_switch.c_str());
      wrench::Simulation::turnOnHost(this->resource_to_switch);
      break;
    }
    case ResourceType::LINK: {
      WRENCH_INFO("CHAOS MONKEY: Turning ON link %s",
                  this->resource_to_switch.c_str());
