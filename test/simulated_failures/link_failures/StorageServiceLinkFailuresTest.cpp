...
WRENCH_LOG_CATEGORY(storage_service_link_failures_test,
                    "Log category for nStorageServiceLinkFailuresTest");
...
protected:
  StorageServiceLinkFailuresTest() {
    ...
    for (int host = 0; host < 4; host++) {
      xml += "       <host id=\"Host" + std::to_string(host) +
             "\" speed=\"1f\" core=\"10\"> ";
      for (int i = 0; i < NUM_STORAGE_SERVICES; i++) {
        xml += "          <disk id=\"large_disk" + std::to_string(i) +
      ...
      xml += "          <disk id=\"scratch\" read_bw=\"100MBps\" "
             "write_bw=\"100MBps\">"
    ...
    xml += "       <link id=\"link1\" bandwidth=\"1Bps\" latency=\"0us\"/>"
...
private:
  ...
  findRandomFileOnStorageService(std::shared_ptr<wrench::StorageService> ss) {
    std::uniform_int_distribution<unsigned long> dist_files(
        0, this->test->files.size() - 1);
    ...
    for (int trial = 0; trial < NUM_FILES; trial++) {
      ...
      if (wrench::StorageService::lookupFileAtLocation(
              wrench::FileLocation::LOCATION(ss, potential_file))) {
  ...
  void doRandomSynchronousFileCopy() {
    ...
    if ((file != nullptr) &&
        (not wrench::StorageService::lookupFileAtLocation(
  ...
  void doRandomFileWrite() {
    ...
    auto file = wrench::Simulation::addFile(
        "written_file_" + std::to_string(count++), FILE_SIZE);
    wrench::StorageService::writeFileAtLocation(
        wrench::FileLocation::LOCATION(dest, file));
    wrench::StorageService::deleteFileAtLocation(
        wrench::FileLocation::LOCATION(dest, file));
  }
  ...
  int main() override {
    ...
    auto switcher1 = std::make_shared<wrench::ResourceRandomRepeatSwitcher>(
        "Host1", 123, 1, 1045, 1, 5, "link1",
        wrench::ResourceRandomRepeatSwitcher::ResourceType::LINK);
    ...
    //        unsigned long NUM_TRIALS = 5000;  to high and it breaks on some CI
    //        containers
...
void StorageServiceLinkFailuresTest::
    do_StorageServiceLinkFailureSimpleRandom_Test() {
  ...
  wrench::WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE payloads = {
      ...
      {wrench::StorageServiceMessagePayload::FILE_READ_ANSWER_MESSAGE_PAYLOAD,
       message_payload},
      {wrench::StorageServiceMessagePayload::FILE_NOT_FOUND_MESSAGE_PAYLOAD,
       message_payload},
      {wrench::StorageServiceMessagePayload::
           NOT_ENOUGH_STORAGE_SPACE_MESSAGE_PAYLOAD,
       message_payload},
  };
  ...
  for (unsigned int i = 0; i < NUM_STORAGE_SERVICES; i++) {
    ...
    storage_services.push_back(simulation->add(
        wrench::SimpleStorageService::createSimpleStorageService(
            hostname, {"/disk" + std::to_string(i)},
            {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "10MB"}},
            payloads)));
  }

  file_registry_service = simulation->add(new wrench::FileRegistryService(
      "Host1",
  ...
  for (unsigned int i = 0; i < NUM_FILES; i++) {
    files.push_back(
        wrench::Simulation::addFile("file_" + std::to_string(i), FILE_SIZE));
  }
  ...
  wms = simulation->add(new StorageServiceLinkFailuresTestWMS(this, "Host1"));
