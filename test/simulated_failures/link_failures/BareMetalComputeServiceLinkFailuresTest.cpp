...
WRENCH_LOG_CATEGORY(bare_metal_compute_service_link_failures_test,
                    "Log category for BareMetalComputeServiceLinkFailuresTest");
...
class BareMetalComputeServiceResourceInformationTestWMS
    : public wrench::ExecutionController {

public:
  BareMetalComputeServiceResourceInformationTestWMS(
...
void BareMetalComputeServiceLinkFailuresTest::
    do_ResourceInformationLinkFailure_test() {
  ...
  this->cs = simulation->add(new wrench::BareMetalComputeService(
      ...
      "/scratch",
      {{wrench::BareMetalComputeServiceProperty::SCRATCH_SPACE_BUFFER_SIZE,
        "10MB"}},
      {
...
public:
  MultiActionJobLinkFailureTestWMS(
      BareMetalComputeServiceLinkFailuresTest *test,
      std::shared_ptr<wrench::Workflow> workflow,
      std::shared_ptr<wrench::BareMetalComputeService> compute_service,
      std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test),
        workflow(std::move(workflow)), compute_service(compute_service) {}
...
private:
  ...
  int main() override {
    ...
    for (unsigned long i = 0; i < first_chain_length; i++) {
      first_chain_tasks.push_back(
          job->addSleepAction("chain1_sleep_" + std::to_string(i), 10));
      if (i > 0) {
    ...
    for (const auto &a : job->getActions()) {
      if (a->getState() != wrench::Action::FAILED) {
        throw std::runtime_error("Invalid action state " +
                                 a->getStateAsString() + " (expecting FAILED)");
      }
    }
...
void BareMetalComputeServiceLinkFailuresTest::
  ...
  ASSERT_NO_THROW(wms = simulation->add(new MultiActionJobLinkFailureTestWMS(
