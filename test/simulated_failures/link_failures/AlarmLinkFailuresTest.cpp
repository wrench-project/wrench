...
WRENCH_LOG_CATEGORY(alarm_link_failures_test,
                    "Log category for AlarmLinkFailuresTest");
...
public:
  AlarmLinkFailuresTestWMS(AlarmLinkFailuresTest *test, std::string hostname)
...
private:
  ...
  int main() override {
    ...
    wrench::Alarm::createAndStartAlarm(
        this->simulation, 10, "Host2", commport,
        new wrench::ExecutionControllerAlarmTimerMessage("hello", 10000),
        "wms_timer");
    ...
    auto switcher =
        std::shared_ptr<wrench::ResourceSwitcher>(new wrench::ResourceSwitcher(
            "Host1", 20, "link1", wrench::ResourceSwitcher::Action::TURN_OFF,
            wrench::ResourceSwitcher::ResourceType::LINK));
...
void AlarmLinkFailuresTest::do_AlarmLinkFailure_Test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new AlarmLinkFailuresTestWMS(this, "Host1")));
