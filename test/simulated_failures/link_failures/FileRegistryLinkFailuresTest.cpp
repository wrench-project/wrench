...
WRENCH_LOG_CATEGORY(file_registry_link_failures_test,
                    "Log category for FileRegistryLinkFailuresTest");
...
protected:
  ...
  FileRegistryLinkFailuresTest() {
    ...
    for (int i = 0; i < NUM_STORAGE_SERVICES; i++) {
      xml += "          <disk id=\"large_disk_" + std::to_string(i) +
             "\" read_bw=\"100MBps\" write_bw=\"100MBps\">"
             "             <prop id=\"size\" value=\"100B\"/>"
             "             <prop id=\"mount\" value=\"/disk_" +
             std::to_string(i) +
             "\"/>"
             "          </disk>";
    }
...
private:
  ...
  int main() override {
    ...
    for (unsigned int i = 0; i < NUM_FILES; i++) {
      files.push_back(
          wrench::Simulation::addFile("file_" + std::to_string(i), 100.0));
    }
    ...
    std::mt19937 rng(666);
    std::uniform_int_distribution<unsigned long> dist_files(0,
                                                            files.size() - 1);
      ...
      try {
        ...
        this->test->file_registry_service->addEntry(
            wrench::FileLocation::LOCATION(
                this->test->storage_services.at(dist_storage(rng)), file));
        ...
        // Do a random lookup
        wrench::Simulation::sleep(1.0);
        this->test->file_registry_service->lookupEntry(
            files.at(dist_files(rng)));
...
void FileRegistryLinkFailuresTest::
    do_FileRegistryLinkFailureSimpleRandom_Test() {
  ...
  file_registry_service = simulation->add(
      new wrench::FileRegistryService("Host2", props, payloads));

  // Create a WMS
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;

  ASSERT_NO_THROW(wms = simulation->add(
                      new FileRegistryLinkFailuresTestWMS(this, "Host1")));
