...
WRENCH_LOG_CATEGORY(data_movement_manager_test,
                    "Log category for DataMovementManagerTest");
...
public:
  DataMovementManagerAsyncCopyTestWMS(
      DataMovementManagerTest *test,
      std::shared_ptr<wrench::FileRegistryService> file_registry_service,
      std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test),
        file_registry_service(std::move(std::move(file_registry_service))) {}
...
private:
  ...
  int main() override {
    ...
    auto src_file_1_locations =
        file_registry_service->lookupEntry(this->test->src_file_1);
    bool found = false;
    for (auto const &l : src_file_1_locations) {
      if (wrench::FileLocation::equal(
              l, wrench::FileLocation::LOCATION(this->test->dst_storage_service,
                                                l->getFile()))) {
        found = true;
      }
    }
    ...
    if (not found) {
      throw std::runtime_error("Synchronous file copy succeeded but file was "
                               "not registered at DstHost");
    }
    ...
    wrench::StorageService::deleteFileAtLocation(wrench::FileLocation::LOCATION(
        this->test->dst_storage_service, this->test->src_file_1));
    ...
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          wrench::FileLocation::LOCATION(this->test->src2_storage_service,
                                         this->test->src2_file_1),
          wrench::FileLocation::LOCATION(this->test->dst_storage_service,
                                         this->test->src2_file_1),
          file_registry_service);
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error(
          "Got an exception while trying to instantiate a file copy: " +
          std::string(e.what()));
    }
    ...
    if (std::dynamic_pointer_cast<wrench::FileCopyFailedEvent>(
            async_copy_event)) {
      throw std::runtime_error("Asynchronous file copy failed.");
    }

    auto src2_file_1_locations =
        file_registry_service->lookupEntry(this->test->src2_file_1);
    ...
    if (not found) {
      throw std::runtime_error("Asynchronous file copy succeeded but file was "
                               "not registered at DstHost.");
    }
    ...
    auto src_file_2_locations =
        file_registry_service->lookupEntry(this->test->src_file_2);
    ...
    if (!double_copy_failed) {
      throw std::runtime_error(
          "The second asynchronous file copy should have failed.");
    }

    if (not std::dynamic_pointer_cast<wrench::FileCopyCompletedEvent>(
            async_copy_event)) {
      throw std::runtime_error(
          "Asynchronous copy event should be FileCompletedEvent. Instead: " +
          async_copy_event->toString());
    }
    ...
    try {
      ...
      if (std::dynamic_pointer_cast<wrench::FileAlreadyBeingCopied>(
              e.getCause())) {
        double_copy_failed = true;
      }
    }
    ...
    auto async_dual_copy_event2_real =
        std::dynamic_pointer_cast<wrench::FileCopyCompletedEvent>(
            async_dual_copy_event2);

    if (not async_dual_copy_event2_real) {
      throw std::runtime_error(
          std::string("Unexpected workflow execution event: " +
                      async_dual_copy_event2->toString()));
    }
    ...
    auto src_file_3_locations =
        file_registry_service->lookupEntry(this->test->src_file_3);
    ...
    if (not found) {
      throw std::runtime_error(
          "File was not registered after Asynchronous copy completed.");
    }

    // try 1 asynchronous copy and then kill the file registry service right
    // after the copy is instantiated
    std::shared_ptr<wrench::ExecutionEvent> async_copy_event2;

    data_movement_manager->initiateAsynchronousFileCopy(
        wrench::FileLocation::LOCATION(this->test->src2_storage_service,
                                       this->test->src2_file_2),
        wrench::FileLocation::LOCATION(this->test->dst_storage_service,
                                       this->test->src2_file_2),
        file_registry_service);
    ...
    if (not wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->dst_storage_service,
                                           this->test->src2_file_2))) {
      throw std::runtime_error("Asynchronous file copy should have completed "
                               "even though the FileRegistryService was down.");
    }
...
void DataMovementManagerTest::do_AsyncCopy_test() {
  ...
  ASSERT_NO_THROW(file_registry_service = simulation->add(
...
class DataMovementManagerAsyncWriteTestWMS
    : public wrench::ExecutionController {

public:
  DataMovementManagerAsyncWriteTestWMS(
      DataMovementManagerTest *test,
      std::shared_ptr<wrench::FileRegistryService> file_registry_service,
      std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test),
  ...
  int main() override {
...

    {
      ...
      try {
        ...
        throw std::runtime_error(
            "Should not be able to do a file write with a nullptr location");
      ...
      auto write_location = wrench::FileLocation::LOCATION(
          this->test->dst_storage_service, this->test->src2_file_1);

      try {
        data_movement_manager->initiateAsynchronousFileWrite(
            write_location, file_registry_service);
      } catch (wrench::ExecutionException &e) {
        throw std::runtime_error(
            "Got an exception while trying to instantiate a file write: " +
            e.getCause()->toString());
      }
      ...
      try {
        data_movement_manager->initiateAsynchronousFileWrite(
            write_location, file_registry_service);
        throw std::runtime_error(
            "Should not be able to concurrently write to the same location");
      } catch (wrench::ExecutionException &e) {
        auto real_cause =
            std::dynamic_pointer_cast<wrench::FileAlreadyBeingWritten>(
                e.getCause());
        if (not real_cause) {
      ...
      if (std::dynamic_pointer_cast<wrench::FileWriteFailedEvent>(
              async_write_event)) {
        throw std::runtime_error(
            "Asynchronous file write failed (" +
            std::dynamic_pointer_cast<wrench::FileWriteFailedEvent>(
                async_write_event)
                ->toString() +
            ")");
      }
      auto real_event =
          std::dynamic_pointer_cast<wrench::FileWriteCompletedEvent>(
              async_write_event);
      if (not real_event) {
        throw std::runtime_error("Received an unexpected event: " +
                                 async_write_event->toString());
      }
      ...
      if (lookup.size() > 2) {
        throw std::runtime_error(
            "File registry service has been updated too much???");
      }
      bool found_it = false;
      ...
      if (not found_it) {
        throw std::runtime_error(
            "Something's wrong in the file registry entry");
      }
    }

    {
      ...
      auto write_location = wrench::FileLocation::LOCATION(
          this->test->dst_storage_service, too_big);
      ...
      if (std::dynamic_pointer_cast<wrench::FileWriteCompletedEvent>(
              async_write_event)) {
        throw std::runtime_error(
            "Asynchronous file write should have failed (" +
            std::dynamic_pointer_cast<wrench::FileWriteFailedEvent>(
                async_write_event)
                ->toString() +
            ")");
      }
      auto real_event = std::dynamic_pointer_cast<wrench::FileWriteFailedEvent>(
          async_write_event);
      if (not real_event) {
        throw std::runtime_error("Received an unexpected event: " +
                                 async_write_event->toString());
      ...
      if (not std::dynamic_pointer_cast<wrench::StorageServiceNotEnoughSpace>(
              real_event->failure_cause)) {
        throw std::runtime_error("wrong failure cause: " +
...
void DataMovementManagerTest::do_AsyncWrite_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(wms =
                      simulation->add(new DataMovementManagerAsyncWriteTestWMS(
                          this, file_registry_service, "WMSHost")));
  ...
  int main() override {
...

    {
      ...
      try {
        ...
        throw std::runtime_error(
            "Should not be able to do a file read with a nullptr location");
      } catch (std::invalid_argument &ignore) {
      }
      ...
      try {
        data_movement_manager->initiateAsynchronousFileRead(
            wrench::FileLocation::LOCATION(this->test->src_storage_service,
                                           this->test->src_file_1),
            this->test->src_file_1->getSize());
      } catch (wrench::ExecutionException &e) {
        throw std::runtime_error(
            "Got an exception while trying to instantiate a file read: " +
            std::string(e.what()));
      }
      ...
      try {
        data_movement_manager->initiateAsynchronousFileRead(
            wrench::FileLocation::LOCATION(this->test->src_storage_service,
                                           this->test->src_file_1));
        throw std::runtime_error(
            "Should not be able to concurrently read from the same location");
      } catch (wrench::ExecutionException &e) {
        auto real_cause =
            std::dynamic_pointer_cast<wrench::FileAlreadyBeingRead>(
                e.getCause());
        if (not real_cause) {
          throw std::runtime_error("Unexpected failure cause: " +
                                   real_cause->toString());
      ...
      if (std::dynamic_pointer_cast<wrench::FileReadFailedEvent>(
              async_read_event)) {
        throw std::runtime_error(
            "Asynchronous file read failed (" +
            std::dynamic_pointer_cast<wrench::FileReadFailedEvent>(
                async_read_event)
                ->toString() +
            ")");
      }
      auto real_event =
          std::dynamic_pointer_cast<wrench::FileReadCompletedEvent>(
              async_read_event);
      if (not real_event) {
        throw std::runtime_error("Received an unexpected event: " +
                                 async_read_event->toString());
      }
      real_event->toString(); // coverage
    }

    {
      ...
      if (std::dynamic_pointer_cast<wrench::FileReadCompletedEvent>(
              async_read_event)) {
        throw std::runtime_error(
            "Asynchronous file read should have failed (" +
            std::dynamic_pointer_cast<wrench::FileReadCompletedEvent>(
                async_read_event)
                ->toString() +
            ")");
      }
      auto real_event = std::dynamic_pointer_cast<wrench::FileReadFailedEvent>(
          async_read_event);
      if (not real_event) {
        throw std::runtime_error("Received an unexpected event: " +
                                 async_read_event->toString());
      ...
      real_event->toString(); // coverage
      if (not std::dynamic_pointer_cast<wrench::FileNotFound>(
              real_event->failure_cause)) {
        throw std::runtime_error(
            "Got the expected event, but and unexpected failure cause: " +
            real_event->failure_cause->toString());
      }
    }
