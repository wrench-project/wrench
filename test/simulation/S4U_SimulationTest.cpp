...
protected:
  S4U_SimulationTest() {
    ...
    std::string xml =
        ...
        "          <disk id=\"large_disk0\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        ...
        "             <prop id=\"mount\" value=\"/\"/>"
        "          </disk>"
        "          <disk id=\"no_capacity\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
...
private:
  ...
  int main() override {
    ...
    if (std::abs(f - 1.0) > 0.001) {
      throw std::runtime_error("Got wrong flop rate for current host (" +
                               std::to_string(f) + " instead of 1)");
    }
    f = wrench::S4U_Simulation::getHostFlopRate("Host1");
    if (std::abs(f - 1.0) > 0.001) {
      throw std::runtime_error("Got wrong flop rate for Host1 (" +
                               std::to_string(f) + " instead of 1)");
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't not be able to get flop rate for a bogus host");
    ...
    if (c != 10) {
      throw std::runtime_error("Got wrong num cores for currenthost (" +
                               std::to_string(c) + " instead of 10)");
    }
    c = wrench::S4U_Simulation::getHostNumCores("Host1");
    if (c != 10) {
      throw std::runtime_error("Got wrong num cores for Host1 (" +
                               std::to_string(c) + " instead of 10)");
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't not be able to get num core for a bogus host");
    } catch (std::invalid_argument &e) {
    }
    ...
    if (std::abs(m - 1024) > 0.001) {
      throw std::runtime_error(
          "Got wrong memory_manager_service capacity for curent host (" +
          std::to_string(m) + " instead of 1024)");
    }
    m = wrench::S4U_Simulation::getHostMemoryCapacity("Host1");
    if (std::abs(m - 1024) > 0.001) {
      throw std::runtime_error(
          "Got wrong memory_manager_service capacity for Host1 (" +
          std::to_string(m) + " instead of 1024)");
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't not be able to get memory_manager_service capacity for a "
          "bogus host");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't not be able to get on/off status for a bogus host");
    } catch (std::invalid_argument &e) {
    }
    ...
    if (p != "bar") {
      throw std::runtime_error("Got wrong memory_manager_service property "
                               "value for current host ('" +
                               p + "' instead of 'bar')");
    }
    try {
      wrench::S4U_Simulation::getHostProperty("Host1", "stuff");
      throw std::runtime_error(
          "Shouldn't not be able to get bogus property from host");
    } catch (std::invalid_argument &e) {
    }
    try {
      wrench::S4U_Simulation::getHostProperty("Bogus", "stuff");
      throw std::runtime_error(
          "Shouldn't not be able to get property from bogus host");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Getting disks for a bogus host should have thrown");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Getting disks for a non-bogus host should not have thrown");
    }

    try {
      wrench::S4U_Simulation::hostHasMountPoint("bogus", "/");
      throw std::runtime_error(
          "Checking mountpoint existence for a bogus host should have thrown");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error("Checking mountpoint existence for a non-bogus "
                               "host should not have thrown");
    }

    try {
      wrench::S4U_Simulation::getDiskCapacity("bogus", "/");
      throw std::runtime_error(
          "Getting disk capacity for a bogus host should have thrown");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Getting disk capacity for a non-bogus host should not have thrown");
    }

    try {
      wrench::S4U_Simulation::getDiskCapacity("Host1", "/no_capacity");
    } catch (std::invalid_argument &e) {
      throw std::runtime_error("Getting disk capacity for a disk with no "
                               "capacity capacity should not have thrown");
    }

    try {
      wrench::S4U_Simulation::getDiskCapacity("Host1", "/bogus");
      throw std::runtime_error(
          "Getting disk capacity for bogus mountpoint should have thrown");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      wrench::S4U_Simulation::getHostProperty("Bogus", "stuff");
      throw std::runtime_error(
          "Getting property for bogus host should have thrown");
    } catch (std::invalid_argument &e) {
    }
    try {
      wrench::S4U_Simulation::getHostProperty("Host1", "custom_property");
      throw std::runtime_error(
          "Getting property for bogus property key should have thrown");
    } catch (std::invalid_argument &e) {
    }
    try {
      wrench::S4U_Simulation::setHostProperty("Bogus", "custom_property",
                                              "whatever");
      throw std::runtime_error(
          "Setting property for bogus host should have thrown");
    } catch (std::invalid_argument &e) {
    }
    try {
      wrench::S4U_Simulation::setHostProperty("Host1", "custom_property",
                                              "whatever");
    } catch (std::invalid_argument &e) {
      throw std::runtime_error(
          "Setting property for valid host should have worked");
    }
    try {
      auto value =
          wrench::S4U_Simulation::getHostProperty("Host1", "custom_property");
      ...
      throw std::runtime_error(
          "Getting property for valid host should have worked");
    }
    if (wrench::S4U_Simulation::getHostProperty("Host1", "custom_property") !=
        "whatever") {
      throw std::runtime_error(
          "Getting property for valid host should have returned the set value");
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to set bandwidth of a non-existing link");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to write disk for non-existing host");
    } catch (std::invalid_argument &ignore) {
    }
    try {
      wrench::S4U_Simulation::writeToDisk(1000, "Host1", "/bogus", nullptr);
      throw std::runtime_error(
          "Should not be able to write disk for non-existing mount point");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    wrench::S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(
        1000, 1000, "Host1", "/tmp", "/", nullptr, nullptr);
    try {
      wrench::S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(
          1000, 1000, "Bogus", "/tmp", "/", nullptr, nullptr);
      throw std::runtime_error(
          "Should not be able to read/write to disk on non-existing host");
    } catch (std::invalid_argument &ignore) {
    }
    try {
      wrench::S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(
          1000, 1000, "Host1", "/bogus", "/", nullptr, nullptr);
      throw std::runtime_error("Should not be able to read/write to disk on "
                               "non-existing src mountpoint");
    } catch (std::invalid_argument &ignore) {
    }
    try {
      wrench::S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(
          1000, 1000, "Host1", "/tmp", "/bogus", nullptr, nullptr);
      throw std::runtime_error("Should not be able to read/write to disk on "
                               "non-existing dst mountpoint");
    } catch (std::invalid_argument &ignore) {
    }
...
TEST_F(S4U_SimulationTest, BasicAPI) { DO_TEST_WITH_FORK(do_basicAPI_Test); }
...
void S4U_SimulationTest::do_basicAPI_Test() {
  ...
  // Create a WMS
  ASSERT_NO_THROW(
      simulation->add(new S4U_SimulationAPITestWMS(this, hostname)));
...
public:
  S4U_DeadlockTestWMS(S4U_SimulationTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
TEST_F(S4U_SimulationTest, Deadlock) { DO_TEST_WITH_FORK(do_deadlock_Test); }
...
void S4U_SimulationTest::do_deadlock_Test() {
  ...
  // Create a WMS
  ASSERT_NO_THROW(simulation->add(new S4U_DeadlockTestWMS(this, hostname)));
  ...
  try {
    simulation->launch();
    throw std::runtime_error(
        "launch() should have thrown a runtime_error due to a deadlock");
