...
public:
  S4U_VirtualMachineTestWMS(S4U_VirtualMachineTest *test, std::string hostname)
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to start a VM on a bogus host");
    ...
    try {
      vm->suspend();
      throw std::runtime_error(
          "Should not be able to suspend a VM that's not running");
    ...
    try {
      vm->shutdown();
      throw std::runtime_error(
          "Should not be able to shutdown a VM that's not running");
    } catch (std::runtime_error &e) {
    }
    ...
    try {
      vm->start(host);
      throw std::runtime_error(
          "Should not be able to start a VM that's already running");
    } catch (std::runtime_error &e) {
    }
    ...
    try {
      vm->resume();
      throw std::runtime_error(
          "Should not be able to resume a VM that's not running");
    } catch (std::runtime_error &e) {
    }
...
TEST_F(S4U_VirtualMachineTest, Basic) { DO_TEST_WITH_FORK(do_basic_Test); }
...
void S4U_VirtualMachineTest::do_basic_Test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new S4U_VirtualMachineTestWMS(this, hostname)));
