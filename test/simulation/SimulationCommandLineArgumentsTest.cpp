...
public:
  ...
  void do_FullLogArgument_test(const std::string &arg,
                               int expected_num_log_lines);
...
void SimulationCommandLineArgumentsTest::do_versionArgument_test() {
  ...
  if (pid == 0) { // Child
    close(1);
    FILE *stdout_file =
        fopen((UNIQUE_TMP_PATH_PREFIX + "unit_tests.stdout").c_str(), "w");
  ...
  // Check file content
  FILE *stdout_file =
      fopen((UNIQUE_TMP_PATH_PREFIX + "unit_tests.stdout").c_str(), "r");
  int linecount = 0;
  char *line = nullptr;
  ssize_t read;
  size_t linecapp;
  while (getline(&line, &linecapp, stdout_file) != -1) {
    ...
    if (linecount == 1) {
      ASSERT_EQ(
          strncmp(line, "This program was linked against WRENCH version ",
                  strlen("This program was linked against WRENCH version ")),
          0);
    }
    if (linecount == 17) {
      ASSERT_EQ(
          strncmp(line, "This program was linked against SimGrid version ",
...
void SimulationCommandLineArgumentsTest::do_NoColorArgument_test() {
  ...
  close(2);
  FILE *stderr_file =
      fopen((UNIQUE_TMP_PATH_PREFIX + "unit_tests.stderr").c_str(), "w");
  ...
  stderr_file =
      fopen((UNIQUE_TMP_PATH_PREFIX + "unit_tests.stderr").c_str(), "r");
  ssize_t read;
  char *line = nullptr;
  size_t linecapp;
  ...
  while (getline(&line, &linecapp, stderr_file) != -1) {
    if (linecapp >= 3) {
      for (size_t i = 0; i < strlen(line) - 2; i++) {
        if ((line[i] == '\033') and (line[i + 1] == '[') and
            (line[i + 2] == '1')) {
          found_color = true;
          break;
        }
...
void SimulationCommandLineArgumentsTest::do_FullLogArgument_test(
    const std::string &arg, int expected_num_log_lines) {
