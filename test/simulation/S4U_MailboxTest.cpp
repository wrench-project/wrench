...
private:
    ...
    int main() override {
        ...
        if (this == this->test->wms1.get()) {
            ...
            // One send
            auto pending_send = this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100));
            pending_send->wait();

            // Another send
            auto another_pending_send = this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100));
            ...
            sends.push_back(this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100)));
            sends.push_back(this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100)));
            ...
            sends_timeout.push_back(this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100)));
            sends_timeout.push_back(this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100)));
            ...
            pending_send = this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100));
            ...
            sends_failure.push_back(this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100)));
            sends_failure.push_back(this->test->wms2->mailbox->iputMessage(new wrench::SimulationMessage(100)));
            wrench::Simulation::sleep(10);
            ...
            try {
                this->test->wms2->mailbox->putMessage(new wrench::SimulationMessage(100));
                throw std::runtime_error("Should have gotten a NetworkError");
            } catch (wrench::ExecutionException &e) {
                auto cause = std::dynamic_pointer_cast<wrench::NetworkError>(e.getCause());
                cause->toString();
            ...
            // One recv
            auto pending_recv = this->test->wms2->mailbox->igetMessage();
            pending_recv->wait();

            // Another recv
            auto another_pending_recv = this->test->wms2->mailbox->igetMessage();
            ...
            recvs.push_back(this->test->wms2->mailbox->igetMessage());
            recvs.push_back(this->test->wms2->mailbox->igetMessage());
            ...
            this->test->wms2->mailbox->getMessage();
            this->test->wms2->mailbox->getMessage();
            ...
            pending_recv = this->test->wms2->mailbox->igetMessage();
            wrench::Simulation::sleep(10);
            try {
                pending_recv->wait();
                throw std::runtime_error("Should have gotten a NetworkError");
...
private:
    ...
    int main() override {
        ...
        try {
            this->mailbox->getMessage<wrench::SimulationMessage>(10);
        ...
        try {
            auto pending = this->mailbox->igetMessage();
            pending->wait(10);
            throw std::runtime_error("Should have gotten an exception");
        } catch (wrench::ExecutionException &e) {
            auto real_error = std::dynamic_pointer_cast<wrench::NetworkError>(e.getCause());
...
private:
    ...
    int main() override {
        ...
        // Coverage
        wrench::S4U_Mailbox::NULL_MAILBOX->putMessage(nullptr);
        wrench::S4U_Mailbox::NULL_MAILBOX->iputMessage(nullptr);
        try {
            wrench::S4U_Mailbox::NULL_MAILBOX->getMessage();
            throw std::runtime_error("Shouldn't be able to get message from NULL_MAILBOX");
        } catch (std::invalid_argument &ignore) {}
        try {
            wrench::S4U_Mailbox::NULL_MAILBOX->igetMessage();
            throw std::runtime_error("Shouldn't be able to get message from NULL_MAILBOX");
        } catch (std::invalid_argument &ignore) {}
