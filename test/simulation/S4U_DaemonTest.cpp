...
public:
  explicit Sleep100Daemon(std::string hostname)
...
public:
  S4U_DaemonTestWMS(S4U_DaemonTest *test, const std::string &hostname)
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to start a lifesaver-less daemon");
    ...
    try {
      daemon->createLifeSaver(std::shared_ptr<Sleep100Daemon>(daemon));
      throw std::runtime_error(
          "Should not be able to create a second life saver");
    } catch (std::runtime_error &e) {
    }
    ...
    try {
      daemon->startDaemon(false, false);
      throw std::runtime_error(
          "Should not be able to start a simulation-less daemon");
    } catch (std::runtime_error &e) {
    }
    ...
    if (std::abs(now - 120) > 1) {
      throw std::runtime_error("Joining at time " + std::to_string(now) +
                               " instead of expected 120");
    }
...
TEST_F(S4U_DaemonTest, Basic) { DO_TEST_WITH_FORK(do_basic_Test) }
...
void S4U_DaemonTest::do_basic_Test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new S4U_DaemonTestWMS(this, hostname)));
...
public:
  S4U_DaemonNoCleanupTestWMS(S4U_DaemonTest *test, std::string hostname)
...
void S4U_DaemonTest::do_noCleanup_Test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new S4U_DaemonNoCleanupTestWMS(this, hostname)));
