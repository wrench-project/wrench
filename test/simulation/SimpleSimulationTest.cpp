...
private:
  ...
  int main() override {
    ...
    std::vector<std::shared_ptr<wrench::WorkflowTask>> tasks =
        this->test->workflow->getReadyTasks();
    if (tasks.size() != 5) {
      throw std::runtime_error(
          "Should have five tasks ready to run, due to dependencies");
    }

    std::map<std::string, std::vector<std::shared_ptr<wrench::WorkflowTask>>>
        clustered_tasks = this->test->workflow->getReadyClusters();
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to create a job with an empty task1 list");
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to create a VM that exceeds "
                               "the capacity of all hosts on the service");
    } catch (wrench::ExecutionException &e) {
      auto cause =
          std::dynamic_pointer_cast<wrench::NotEnoughResources>(e.getCause());
      if (not cause) {
        throw std::runtime_error(
            "Unexpected failure cause: " + e.getCause()->toString() +
    ...
    try {
      cs->startVM(vm_name);
      throw std::runtime_error(
          "Should not be able to start an already-started VM");
    } catch (wrench::ExecutionException &e) {
    ...
    for (const auto &task : tasks) {
      try {
        ...
        if (one_task_jobs[job_index]->getNumTasks() != 1) {
          throw std::runtime_error(
              "A one-task1 job should say it has one task1");
        }
        if (one_task_jobs[job_index]->getNumCompletedTasks() != 0) {
          throw std::runtime_error("A one-task1 job that hasn't even started "
                                   "should not say it has a completed task1");
        }
      ...
      if (wrench::Simulation::getCurrentSimulatedDate() - job_submit_date >
          1.0) {
    ...
    // Try to create and submit a job with tasks that are pending, which should
    // fail
    auto bogus_job = job_manager->createStandardJob(*(tasks.begin()));
    try {
      job_manager->submitJob(bogus_job, vm_cs);
      throw std::runtime_error(
          "Should not be able to create a job with PENDING tasks");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to get link usage for an empty-name link");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to get information about bogus link");
    } catch (std::invalid_argument &ignore) {
    }
    try {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to get route with bogus src endpoint");
    } catch (std::invalid_argument &ignore) {
    }
    try {
      wrench::Simulation::getRoute(src_host, bogus);
      throw std::runtime_error(
          "Shouldn't be able to get route with bogus dst endpoint");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    for (auto &one_task_job : one_task_jobs) {
      if (one_task_job->getNumCompletedTasks() != 1) {
        throw std::runtime_error(
            "A job with one completed task1 should say it has one completed "
            "task1 (instead of " +
    ...
    {
      // Try to create and submit a job with tasks that are completed, which
      // should fail
    ...
    try {
      cs->resume();
      throw std::runtime_error(
          "Should not be able to resume a service that's down");
    } catch (wrench::ExecutionException &e) {
    }
...
void SimpleSimulationTest::do_getReadyTasksTest_test(double buffer_size) {
  ...
  ASSERT_THROW(simulation->add(new wrench::CloudComputeService(
                   "DualCoreHost", hosts, "/scratch")),
               std::runtime_error);
  ASSERT_THROW(
  ...
  ASSERT_THROW(simulation->add(
               ...
               std::runtime_error);
  ASSERT_THROW(simulation->add(new wrench::FileRegistryService("DualCoreHost")),
               std::runtime_error);
  ...
  storage_service =
      simulation->add(wrench::SimpleStorageService::createSimpleStorageService(
          ...
          {{wrench::SimpleStorageServiceProperty::
            ...
            "567"},
           {wrench::ServiceProperty::translateString(
                "StorageServiceProperty::BUFFER_SIZE"),
            "678MiB"}},
          {{wrench::SimpleStorageServiceMessagePayload::
                FILE_COPY_ANSWER_MESSAGE_PAYLOAD,
            123},
           {wrench::ServiceMessagePayload::translateString(
                "StorageServiceMessagePayload::FILE_LOOKUP_ANSWER_MESSAGE_"
                "PAYLOAD"),
            234}}));

  ASSERT_DOUBLE_EQ(678 * 1024 * 1024,
                   storage_service->getPropertyValueAsSizeInByte(
                       wrench::SimpleStorageServiceProperty::BUFFER_SIZE));

  // Coverage
  ASSERT_EQ(wrench::ServiceProperty::translateString(
                wrench::ServiceProperty::translatePropertyType(
                    wrench::SimpleStorageServiceProperty::
                        MAX_NUM_CONCURRENT_DATA_CONNECTIONS)),
            wrench::SimpleStorageServiceProperty::
                MAX_NUM_CONCURRENT_DATA_CONNECTIONS);
  ...
  ASSERT_EQ(storage_service->getPropertyValueAsUnsignedLong(
                wrench::SimpleStorageServiceProperty::
                    MAX_NUM_CONCURRENT_DATA_CONNECTIONS),
            567);
  ASSERT_EQ(storage_service->getPropertyValueAsUnsignedLong(
                wrench::ServiceProperty::translateString(
                    "SimpleStorageServiceProperty::MAX_NUM_CONCURRENT_DATA_"
                    "CONNECTIONS")),
            567);
  ASSERT_EQ(storage_service->getPropertyValueAsUnsignedLong(
                wrench::SimpleStorageServiceProperty::BUFFER_SIZE),
  ...
  ASSERT_THROW(storage_service->getMessagePayloadValue(-1),
               std::invalid_argument);
  ...
  ASSERT_EQ(123, storage_service->getMessagePayloadValue(
                     wrench::ServiceMessagePayload::translateString(
                         "StorageServiceMessagePayload::FILE_COPY_ANSWER_"
                         "MESSAGE_PAYLOAD")));
  ...
  ASSERT_EQ(234, storage_service->getMessagePayloadValue(
                     wrench::ServiceMessagePayload::translateString(
                         "StorageServiceMessagePayload::FILE_LOOKUP_ANSWER_"
                         "MESSAGE_PAYLOAD")));
  ...
  ASSERT_NO_THROW(compute_service =
                      simulation->add(new wrench::CloudComputeService(
                          hostname, {{"QuadCoreHost"}}, "/scratch",
                          property_list, messagepayload_list)));

  // Check on properties and payload
  ASSERT_DOUBLE_EQ(compute_service->getPropertyValueAsTimeInSecond(
                       wrench::CloudComputeServiceProperty::VM_BOOT_OVERHEAD),
                   0.1);
  ASSERT_EQ(compute_service->getPropertyValueAsString(
                wrench::CloudComputeServiceProperty::
                    VM_RESOURCE_ALLOCATION_ALGORITHM),
            "best-fit-ram-first");
  ASSERT_THROW(compute_service->getPropertyValueAsDouble(
                   wrench::CloudComputeServiceProperty::
                       VM_RESOURCE_ALLOCATION_ALGORITHM),
               std::invalid_argument);
  ASSERT_THROW(compute_service->getPropertyValueAsUnsignedLong(
                   wrench::CloudComputeServiceProperty::
                       VM_RESOURCE_ALLOCATION_ALGORITHM),
               std::invalid_argument);
  ASSERT_THROW(compute_service->getPropertyValueAsBoolean(
                   wrench::CloudComputeServiceProperty::
                       VM_RESOURCE_ALLOCATION_ALGORITHM),
               std::invalid_argument);
  ASSERT_EQ(compute_service->getMessagePayloadValue(
                wrench::CloudComputeServiceMessagePayload::
                    DESTROY_VM_ANSWER_MESSAGE_PAYLOAD),
            2);
  ASSERT_EQ(compute_service->getMessagePayloadValue(
  ...
  ASSERT_EQ(
      compute_service->getPropertyValueAsString(
          wrench::ServiceProperty::translateString(
              "CloudComputeServiceProperty::VM_RESOURCE_ALLOCATION_ALGORITHM")),
      "best-fit-ram-first");
  ASSERT_EQ(compute_service->getPropertyValueAsTimeInSecond(
                wrench::ServiceProperty::translateString(
                    "CloudComputeServiceProperty::VM_BOOT_OVERHEAD")),
            0.1);
  ASSERT_EQ(compute_service->getMessagePayloadValue(
                wrench::ServiceMessagePayload::translateString(
                    "CloudComputeServiceMessagePayload::DESTROY_VM_ANSWER_"
                    "MESSAGE_PAYLOAD")),
            2);
  ASSERT_EQ(compute_service->getMessagePayloadValue(
                wrench::ServiceMessagePayload::translateString(
  ...
  ASSERT_THROW(compute_service->getMessagePayloadValue(
                   wrench::ServiceMessagePayload::translateString(
                       "CloudComputeServiceMessagePayload::STOP_DAEMON_MESSAGE_"
                       "PAYLOAD")),
               std::exception);

  // Try to get the property in bogus ways, for coverage
  ASSERT_THROW(
      compute_service->getPropertyValueAsBoolean(
  ...
  ASSERT_THROW(compute_service->getPropertyValueAsUnsignedLong(
               ...
               std::invalid_argument);
  ASSERT_THROW(compute_service->getPropertyValueAsTimeInSecond(
                   wrench::BareMetalComputeServiceProperty::
                       TERMINATE_WHENEVER_ALL_RESOURCES_ARE_DOWN),
               std::invalid_argument);
  ASSERT_THROW(compute_service->getPropertyValueAsBandwidthInBytePerSecond(
                   wrench::BareMetalComputeServiceProperty::
                       TERMINATE_WHENEVER_ALL_RESOURCES_ARE_DOWN),
               std::invalid_argument);
  ASSERT_THROW(compute_service->getPropertyValueAsSizeInByte(
                   wrench::BareMetalComputeServiceProperty::
                       TERMINATE_WHENEVER_ALL_RESOURCES_ARE_DOWN),
               std::invalid_argument);

  // Try to get a message payload value, just for kicks
  ASSERT_NO_THROW(compute_service->getMessagePayloadValue(
      wrench::ServiceMessagePayload::STOP_DAEMON_MESSAGE_PAYLOAD));

  // Create a WMS
  ASSERT_NO_THROW(
      simulation->add(new SimpleSimulationReadyTasksTestWMS(this, hostname)));

  // BOGUS ADDS
  ASSERT_THROW(simulation->add((wrench::ExecutionController *)nullptr),
               std::invalid_argument);
  ASSERT_THROW(simulation->add((wrench::StorageService *)nullptr),
               std::invalid_argument);
  ASSERT_THROW(simulation->add((wrench::ComputeService *)nullptr),
               std::invalid_argument);
  ASSERT_THROW(simulation->add((wrench::NetworkProximityService *)nullptr),
               std::invalid_argument);
  ASSERT_THROW(simulation->add((wrench::FileRegistryService *)nullptr),
               std::invalid_argument);

  //    // Try to stage a file without a file registry
  //    ASSERT_THROW(simulation->stageFile(input_file, storage_service),
  //    std::runtime_error);
