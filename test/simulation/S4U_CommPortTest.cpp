    ...
    if (this == this->test->wms1.get()) {
      ...
      try {
        wrench::S4U_PendingCommunication::waitForSomethingToHappen(
            empty_pending_comms, -1);
        throw std::runtime_error(
            "Was expecting a std::invalid_argument exception");
      } catch (std::invalid_argument &) {
      }

      // One send
      pending_send = this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100));
      pending_send->wait();

      // Another send
      auto another_pending_send = this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100));
      another_pending_send->wait(200);

      // Two sends, no timeout
      std::vector<std::shared_ptr<wrench::S4U_PendingCommunication>> sends;
      sends.push_back(this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100)));
      sends.push_back(this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100)));
      index =
          wrench::S4U_PendingCommunication::waitForSomethingToHappen(sends, -1);
      sends.at(index)->wait();
      index =
          wrench::S4U_PendingCommunication::waitForSomethingToHappen(sends, -1);
      sends.at(index)->wait();
      ...
      // Two sends, timeout
      std::vector<std::shared_ptr<wrench::S4U_PendingCommunication>>
          sends_timeout;
      sends_timeout.push_back(this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100)));
      sends_timeout.push_back(this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100)));
      double now = wrench::Simulation::getCurrentSimulatedDate();
      index = wrench::S4U_PendingCommunication::waitForSomethingToHappen(
          sends_timeout, 10);
      if (index != ULONG_MAX) {
        throw std::runtime_error(
            "Was expecting a timeout on the waitForSomethingToHappen");
      }
      if (fabs(10 - (wrench::Simulation::getCurrentSimulatedDate() - now)) >
          0.5) {
        throw std::runtime_error("Seems like we didn't wait for the timeout!");
      }
      //            index =
      //            wrench::S4U_PendingCommunication::waitForSomethingToHappen(sends_timeout,
      //            1000); if (index != ULONG_MAX) {
      //                throw std::runtime_error("Should have gotten
      //                ULONG_MAX");
      //            }
      //            index =
      //            wrench::S4U_PendingCommunication::waitForSomethingToHappen(sends_timeout,
      ...
      std::vector<std::shared_ptr<wrench::S4U_PendingCommunication>>
          sends_failure;
      sends_failure.push_back(this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100)));
      sends_failure.push_back(this->test->wms2->commport->iputMessage(
          new wrench::SimulationMessage(100)));
      wrench::Simulation::sleep(10);
      ...
      try {
        this->test->wms2->commport->putMessage(
            new wrench::SimulationMessage(100));
        throw std::runtime_error("Should have gotten a NetworkError");
      } catch (wrench::ExecutionException &e) {
        auto cause =
      ...
      recvs.push_back(this->test->wms2->commport->igetMessage());
      index =
          wrench::S4U_PendingCommunication::waitForSomethingToHappen(recvs, -1);
      recvs.at(index)->wait();
      index =
          wrench::S4U_PendingCommunication::waitForSomethingToHappen(recvs, -1);
      recvs.at(index)->wait();
...
void S4U_CommPortTest::do_AsynchronousCommunication_test() {
  ...
  this->wms1 =
      simulation->add(new AsynchronousCommunicationTestWMS(this, "Host1"));
  this->wms2 =
      simulation->add(new AsynchronousCommunicationTestWMS(this, "Host2"));
...
public:
  NetworkTimeoutTestWMS(S4U_CommPortTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
  ...
  int main() override {
    ...
    try {
      ...
      if (!real_error->isTimeout()) {
        throw std::runtime_error(
            "Network error failure cause should be a time out");
      }
    ...
    {
      auto pending = this->commport->igetMessage();
      std::vector<wrench::S4U_PendingCommunication *> pending_comms = {
          pending.get()};
      auto index = wrench::S4U_PendingCommunication::waitForSomethingToHappen(
          pending_comms, 10);
      if (index != ULONG_MAX) {
...
public:
  NullCommPortTestWMS(S4U_CommPortTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to get message from NULL_COMMPORT");
