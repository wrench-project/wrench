...
WRENCH_LOG_CATEGORY(multi_controller_test,
                    "Log category for MultiControllerSimulationTest");
...
public:
  TestController(std::string controller_name, double sleep_time,
                 double *finish_time, const std::string &hostname)
...
void MultiControllerSimulationTest::do_multiControllerTest_test() {
  ...
  double ft1, ft2, ft3;
  auto c1 =
      simulation->add(new TestController("Controller#1", 10, &ft1, "Host1"));
  auto c2 =
      simulation->add(new TestController("Controller#2", 100, &ft2, "Host1"));
  auto c3 =
      simulation->add(new TestController("Controller#3", 30, &ft3, "Host1"));
