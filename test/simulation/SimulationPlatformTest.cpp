...
public:
  SimulationPlatformTestWMS(SimulationPlatformTest *test, std::string &hostname)
  ...
  int main() override {
    ...
    if (subnetzones["world"].size() != 2) {
      throw std::runtime_error(
          "Invalid number of netzones found under the 'world' netzone");
    }
    if (std::find(subnetzones["world"].begin(), subnetzones["world"].end(),
                  "subzone") == subnetzones["world"].end()) {
      throw std::runtime_error(
          "Should find netzone 'subzone' in netzone 'world'");
    }
    if (std::find(subnetzones["world"].begin(), subnetzones["world"].end(),
                  "host_zone") == subnetzones["world"].end()) {
      throw std::runtime_error(
          "Should find netzone 'host_zone' in netzone 'world'");
    }
    ...
    if (clusters["world"].size() != 2) {
      throw std::runtime_error(
          "Invalid number of clusters found under the 'world' netzone");
    }
    if (std::find(clusters["world"].begin(), clusters["world"].end(),
                  "simple") == clusters["world"].end()) {
      throw std::runtime_error(
          "Should find cluster 'simple' in netzone 'world'");
    }
    if (std::find(clusters["world"].begin(), clusters["world"].end(),
                  "backboned") == clusters["world"].end()) {
      throw std::runtime_error(
          "Should find cluster 'backboned' in netzone 'world'");
    }
    if (clusters["subzone"].size() != 1) {
      throw std::runtime_error(
          "Invalid number of cluster found under the 'subzone' netzone");
    }
    if (std::find(clusters["subzone"].begin(), clusters["subzone"].end(),
                  "halfduplex") == clusters["subzone"].end()) {
      throw std::runtime_error(
          "Should find cluster 'halfduplex' in netzone 'subzone'");
    }
    ...
    if (hosts["host_zone"].size() != 1) {
      throw std::runtime_error(
          "Invalid number of hosts found under zone 'host_zone' found");
    }
    ...
    if (hosts["simple"].size() != 8) {
      throw std::runtime_error(
          "Invalid number of hosts found under cluster 'simple' found");
    }
    if (hosts["backboned"].size() != 8) {
      throw std::runtime_error(
          "Invalid number of hosts found under cluster 'backboned' found");
    }
    if (hosts["halfduplex"].size() != 4) {
      throw std::runtime_error(
          "Invalid number of hosts found under cluster 'halfduplex' found");
    }

    if (wrench::S4U_Simulation::getClusterProperty("simple", "wattage_off") !=
        "0.0") {
...
void SimulationPlatformTest::do_SimulationPlatformTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new SimulationPlatformTestWMS(this, hostname)));
...
public:
  CreateNewDiskTestWMS(SimulationPlatformTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    wrench::S4U_Simulation::createNewDisk("subzonehost", "new_disk", 10.0, 10.0,
                                          100.0, "/foo");

    // Start a storage service that uses this disk
    auto ss = this->simulation->startNewService(
    ...
    try {
      wrench::StorageService::createFileAtLocation(
          wrench::FileLocation::LOCATION(ss, too_big));
      throw std::runtime_error("Should not be able to create a file that big "
                               "on the newly created storage service");
    } catch (wrench::ExecutionException &ignore) {
    }
    auto not_too_big = wrench::Simulation::addFile("not_too_big", 20.0);
    wrench::StorageService::createFileAtLocation(
        wrench::FileLocation::LOCATION(ss, not_too_big));
    wrench::StorageService::removeFileAtLocation(
        wrench::FileLocation::LOCATION(ss, not_too_big));
    wrench::StorageService::createFileAtLocation(
        wrench::FileLocation::LOCATION(ss, not_too_big));
...
void SimulationPlatformTest::do_CreateNewDiskTest_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(
      wms = simulation->add(new CreateNewDiskTestWMS(this, hostname)));
...
void SimulationPlatformTest::do_ProgrammaticPlatformTest_test() {
  ...
  ASSERT_THROW(simulation->instantiatePlatform(platform_creator),
               std::runtime_error);
