...
class DynamicServiceCreationReadyTasksTestWMS
...
private:
  ...
  int main() override {
    ...
    auto dynamically_created_file_registry_service =
        simulation->startNewService(new wrench::FileRegistryService(hostname));

    // Dynamically create a Network Proximity Service on this host
    auto dynamically_created_network_proximity_service =
        simulation->startNewService(new wrench::NetworkProximityService(
            hostname, {"DualCoreHost", "QuadCoreHost"}));
    ...
    auto dynamically_created_compute_service =
        std::dynamic_pointer_cast<wrench::CloudComputeService>(
            simulation->startNewService(new wrench::CloudComputeService(
        ...
        one_task_jobs[job_index] = job_manager->createStandardJob(
            ...
            {std::make_tuple(
                ...
                wrench::FileLocation::LOCATION(
                    dynamically_created_storage_service,
                    this->test->input_file))},
            {});
    ...
    for (auto &j : one_task_jobs) {
      if (j->getNumCompletedTasks() != 1) {
        throw std::runtime_error("A job with one completed task1 should say it "
                                 "has one completed task1");
      }
    }
...
void DynamicServiceCreationTest::do_getReadyTasksTest_test() {
  ...
  ASSERT_THROW(simulation->startNewService((wrench::ComputeService *)nullptr),
               std::invalid_argument);
  ASSERT_THROW(simulation->startNewService((wrench::StorageService *)nullptr),
               std::invalid_argument);
  ASSERT_THROW(
      simulation->startNewService((wrench::NetworkProximityService *)nullptr),
      std::invalid_argument);
  ASSERT_THROW(
      simulation->startNewService((wrench::FileRegistryService *)nullptr),
      std::invalid_argument);
  ASSERT_THROW(simulation->startNewService((wrench::ComputeService *)666),
               std::runtime_error);
  ASSERT_THROW(simulation->startNewService((wrench::StorageService *)666),
               std::runtime_error);
  ASSERT_THROW(
      simulation->startNewService((wrench::NetworkProximityService *)666),
      std::runtime_error);
  ASSERT_THROW(simulation->startNewService((wrench::FileRegistryService *)666),
               std::runtime_error);
