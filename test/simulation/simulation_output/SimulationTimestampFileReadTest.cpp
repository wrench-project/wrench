...
WRENCH_LOG_CATEGORY(simulation_timestamp_file_read_test,
                    "Log category for SimulationTimestampFileReadTest");
 ...
 * This test ensures that SimulationTimestampFileReadStart,
 * SimulationTimestampFileReadFailure, and SimulationTimestampFileReadCompletion
 * objects are added to their respective simulation traces at the appropriate
 * times.
 */
class SimulationTimestampFileReadBasicTestWMS
...
void SimulationTimestampFileReadTest::
    do_SimulationTimestampFileReadBasic_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimulationTimestampFileReadBasicTestWMS(this, host1)));
  ...
  auto start_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampFileReadStart>();
  auto failure_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampFileReadFailure>();
  auto completion_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampFileReadCompletion>();
  ...
  wrench::SimulationTimestampFileRead *file_1_start =
      start_timestamps[0]->getContent();
  wrench::SimulationTimestampFileRead *file_1_end =
      completion_timestamps[0]->getContent();

  wrench::SimulationTimestampFileRead *file_2_start =
      start_timestamps[1]->getContent();
  wrench::SimulationTimestampFileRead *file_2_end =
      completion_timestamps[1]->getContent();

  wrench::SimulationTimestampFileRead *file_3_1_start =
      start_timestamps[2]->getContent();
  wrench::SimulationTimestampFileRead *file_3_1_end =
      completion_timestamps[2]->getContent();

  wrench::SimulationTimestampFileRead *xl_file_start =
      start_timestamps[3]->getContent();
  wrench::SimulationTimestampFileRead *xl_file_end =
      completion_timestamps[3]->getContent();

  // list of expected matching start and end timestamps
  std::vector<std::pair<wrench::SimulationTimestampFileRead *,
  ...
  ASSERT_THROW(simulation->getOutput().addTimestampFileReadStart(
                   0.0, file_1, nullptr, service, task1),
               std::invalid_argument);
  ...
  ASSERT_THROW(simulation->getOutput().addTimestampFileReadCompletion(
                   0.0, nullptr, nullptr, service, task1),
               std::invalid_argument);
