...
WRENCH_LOG_CATEGORY(simulation_timestamp_disk_read_write_test,
                    "Log category for SimulationTimestampDiskReadWriteTest");
 ...
 */
class SimulationTimestampDiskReadWriteBasicTestWMS
    : public wrench::ExecutionController {
public:
  SimulationTimestampDiskReadWriteBasicTestWMS(
      SimulationTimestampDiskReadWriteTest *test, std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    wrench::StorageService::writeFileAtLocation(wrench::FileLocation::LOCATION(
        this->test->storage_service_2, this->test->file_1));

    return 0;
  }
};

TEST_F(SimulationTimestampDiskReadWriteTest,
       SimulationTimestampDiskReadWriteBasicTest) {
  DO_TEST_WITH_FORK(do_SimulationTimestampDiskReadWriteBasic_test);
}

void SimulationTimestampDiskReadWriteTest::
    do_SimulationTimestampDiskReadWriteBasic_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimulationTimestampDiskReadWriteBasicTestWMS(this, host1)));
  ...
  auto diskread_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampDiskReadStart>();
  auto diskwrite_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampDiskWriteStart>();
