...
protected:
  ...
  SimulationOutputTest() {
    ...
    //      task5 = workflow->addTask("task_5_30s_1_to_3_cores", 30.0, 1, 3,
    //      0.0); task6 = workflow->addTask("task_6_10s_1_to_2_cores", 12.0, 1,
    //      2, 0.0); task1->setClusterID("ID1"); task2->setClusterID("ID1");
...
public:
  EmptySimulationOutputWMS(SimulationOutputTest *test, std::string &hostname)
...
void SimulationOutputTest::do_emptyTrace_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new EmptySimulationOutputWMS(this, hostname)));
