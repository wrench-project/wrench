...
WRENCH_LOG_CATEGORY(simulation_timestamp_file_write_test,
                    "Log category for SimulationTimestampFileWriteTest");
 ...
 * This test ensures that SimulationTimestampFileWriteStart,
 * SimulationTimestampFileWriteFailure, and
 * SimulationTimestampFileWriteCompletion objects are added to their respective
 * simulation traces at the appropriate times.
 */
class SimulationTimestampFileWriteBasicTestWMS
    : public wrench::ExecutionController {
public:
  SimulationTimestampFileWriteBasicTestWMS(
      SimulationTimestampFileWriteTest *test, std::string &hostname)
...
TEST_F(SimulationTimestampFileWriteTest,
       SimulationTimestampFileWriteBasicTest) {
  DO_TEST_WITH_FORK(do_SimulationTimestampFileWriteBasic_test);
}

void SimulationTimestampFileWriteTest::
    do_SimulationTimestampFileWriteBasic_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimulationTimestampFileWriteBasicTestWMS(this, host1)));

  // stage files
  std::set<std::shared_ptr<wrench::DataFile>> files_to_stage = {
      file_1, file_2, file_3, xl_file};
  ...
  auto start_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampFileWriteStart>();
  auto failure_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampFileWriteFailure>();
  auto completion_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampFileWriteCompletion>();
  ...
  wrench::SimulationTimestampFileWrite *file_1_start =
      start_timestamps[0]->getContent();
  wrench::SimulationTimestampFileWrite *file_1_end =
      completion_timestamps[0]->getContent();

  wrench::SimulationTimestampFileWrite *file_2_start =
      start_timestamps[1]->getContent();
  wrench::SimulationTimestampFileWrite *file_2_end =
      completion_timestamps[1]->getContent();

  wrench::SimulationTimestampFileWrite *file_3_1_start =
      start_timestamps[2]->getContent();
  wrench::SimulationTimestampFileWrite *file_3_1_end =
      completion_timestamps[2]->getContent();

  wrench::SimulationTimestampFileWrite *xl_file_start =
      start_timestamps[3]->getContent();
  wrench::SimulationTimestampFileWrite *xl_file_end =
      completion_timestamps[3]->getContent();

  // list of expected matching start and end timestamps
  std::vector<std::pair<wrench::SimulationTimestampFileWrite *,
                        wrench::SimulationTimestampFileWrite *>>
      file_write_timestamps = {
  ...
  for (auto &fc : file_write_timestamps) {
    ...
    ASSERT_EQ(this->storage_service,
  ...
  ASSERT_THROW(simulation->getOutput().addTimestampFileWriteStart(
                   0.0, file_1, nullptr, service, task),
               std::invalid_argument);
