...
protected:
  ...
  SimulationTimestampTaskTest() {
    ...
    std::string xml =
        ...
        "             <prop id=\"mount\" value=\"/\"/>"
        "          </disk>"
        "          <disk id=\"large_disk_backup\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"100000000000000B\"/>"
        ...
        "       <route src=\"ExecutionHost\" dst=\"WMSHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "       <route src=\"ComputeHost\" dst=\"WMSHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "       <route src=\"ComputeHost\" dst=\"ExecutionHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "   </zone> "
        "</platform>";
 ...
 * This test ensures that SimulationTimestampTaskStart,
 * SimulationTimestampTaskFailure, and SimulationTimestampTaskCompletion objects
 * are added to their respective simulation traces at the appropriate times.
 */
...
private:
  ...
  int main() override {
    ...
    this->test->failed_task =
        this->test->workflow->addTask("failed_task", 100000.0, 1, 1, 0);
...
void SimulationTimestampTaskTest::do_SimulationTimestampTaskBasic_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new SimulationTimestampTaskBasicTestWMS(this, wms_host)));
   ...
   * WorkflowTask member variables start_date and end_date should equal
   * SimulationTimestamp<SimulationTimestampTaskStart>::getContent()->getDate()
   * and
   * SimulationTimestamp<SimulationTimestampTaskCompletion>::getContent()->getDate()
   * respectively
   */

  auto timestamp_start_trace =
      simulation->getOutput().getTrace<wrench::SimulationTimestampTaskStart>();
  auto timestamp_completion_trace =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampTaskCompletion>();
  double task1_start_date = this->task1->getStartDate();
  double task1_start_timestamp =
      timestamp_start_trace[0]->getContent()->getDate();

  ASSERT_DOUBLE_EQ(task1_start_date, task1_start_timestamp);

  double task1_completion_date = this->task1->getEndDate();
  double task1_completion_timestamp =
      timestamp_completion_trace[0]->getContent()->getDate();

  ASSERT_LT(std::abs(task1_completion_date - task1_completion_timestamp),
            0.001);

  double task2_start_timestamp =
      timestamp_start_trace[1]->getContent()->getDate();
  double task2_completion_timestamp =
      timestamp_completion_trace[1]->getContent()->getDate();
  ...
  auto timestamp_failure_trace =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampTaskFailure>();
  //    for (auto const &ts : timestamp_start_trace) {
  //        std::cerr << "  - " << ts->getContent()->getTask()->getID() << ": "
  //        << ts->getDate() << "\n";
  //    }
  double failed_task_start_timestamp =
      timestamp_start_trace[2]->getContent()->getDate();
  double failed_task_end_date = this->failed_task->getEndDate();
  double failed_task_failure_timestamp =
      timestamp_failure_trace[0]->getContent()->getDate();
  ...
  for (auto &ts : timestamp_completion_trace) {
    if (ts->getContent()->getTask()->getID() == "failed_task") {
      throw std::runtime_error(
          "timestamp_completion_trace should not have the 'failed_task'");
    }
  }

  // check that endpoints match up correctly
  wrench::SimulationTimestampTask *task_1_start =
      timestamp_start_trace[0]->getContent();
  wrench::SimulationTimestampTask *task_1_end =
      timestamp_completion_trace[0]->getContent();
  ...
  wrench::SimulationTimestampTask *task_2_start =
      timestamp_start_trace[1]->getContent();
  wrench::SimulationTimestampTask *task_2_end =
      timestamp_completion_trace[1]->getContent();
  ...
  wrench::SimulationTimestampTask *failed_task_start =
      timestamp_start_trace[2]->getContent();
  wrench::SimulationTimestampTask *failed_task_failed =
      timestamp_failure_trace[0]->getContent();
  ...
  ASSERT_THROW(simulation->getOutput().addTimestampTaskStart(0.0, nullptr),
               std::invalid_argument);
  ASSERT_THROW(simulation->getOutput().addTimestampTaskFailure(0.0, nullptr),
               std::invalid_argument);
  ASSERT_THROW(simulation->getOutput().addTimestampTaskCompletion(0.0, nullptr),
               std::invalid_argument);
  ASSERT_THROW(
      simulation->getOutput().addTimestampTaskTermination(0.0, nullptr),
      std::invalid_argument);
#endif
...
class SimulationTimestampTaskMultipleTestWMS
...
private:
  ...
  int main() override {
    ...
    for (int i = 0; i < num_task1_runs; ++i) {
      this->test->task1->setInternalState(
          wrench::WorkflowTask::InternalState::TASK_READY);
...
#endif
    this->test->failed_task =
        this->test->workflow->addTask("failed_task", 10, 1, 1, 0);
    this->test->failed_task->addInputFile(this->test->large_input_file);
    this->test->failed_task->addInputFile(this->test->small_input_file);
...
void SimulationTimestampTaskTest::do_SimulationTimestampTaskMultiple_test() {
  ...
  ASSERT_NO_THROW(storage_service = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          wms_host, {"/"})));
  ASSERT_NO_THROW(backup_storage_service = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          wms_host, {"/backup"})));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimulationTimestampTaskMultipleTestWMS(this, wms_host)));
  ...
  ASSERT_NO_THROW(simulation->stageFile(small_input_file, storage_service));
  ASSERT_NO_THROW(
      simulation->stageFile(small_input_file, backup_storage_service));
  ...
  auto starts_trace =
      simulation->getOutput().getTrace<wrench::SimulationTimestampTaskStart>();
  auto completions_trace =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampTaskCompletion>();
  auto failures_trace = simulation->getOutput()
                            .getTrace<wrench::SimulationTimestampTaskFailure>();
  ...
  int num_task1_starts = std::count_if(
      starts_trace.begin(), starts_trace.end(),
      [&task1_id](
          wrench::SimulationTimestamp<wrench::SimulationTimestampTaskStart>
              *ts) {
        return ((*ts).getContent()->getTask()->getID() == task1_id);
      });

  int num_failed_task_starts = std::count_if(
      starts_trace.begin(), starts_trace.end(),
      [&failed_task_id](
          wrench::SimulationTimestamp<wrench::SimulationTimestampTaskStart>
              *ts) {
  ...
  int num_task1_completions = std::count_if(
      completions_trace.begin(), completions_trace.end(),
      [&task1_id](
  ...
  int num_failed_task_failures = std::count_if(
      failures_trace.begin(), failures_trace.end(),
      [&failed_task_id](
          wrench::SimulationTimestamp<wrench::SimulationTimestampTaskFailure>
              *ts) {
        return ((*ts).getContent()->getTask()->getID() == failed_task_id);
      });

  int num_failed_task_completions = std::count_if(
      completions_trace.begin(), completions_trace.end(),
      [&failed_task_id](
          wrench::SimulationTimestamp<wrench::SimulationTimestampTaskCompletion>
              *ts) {
  ...
  //    ASSERT_DOUBLE_EQ(std::floor(task1_completion_date_1 -
  //    task1_start_date_1), 10.0);
  ASSERT_TRUE(std::abs<double>((task1_completion_date_1 - task1_start_date_1) -
                               10.0) < EPSILON);
  ...
  //    "\n"; ASSERT_DOUBLE_EQ(std::floor(task1_completion_date_2 -
  //    task1_start_date_2), 10.0);
  ASSERT_TRUE(std::abs<double>((task1_completion_date_2 - task1_start_date_2) -
                               10.0) < EPSILON);
...
/*
 * Testing that SimulationTimestampTaskFailure timestamps get created when tasks
 * fail due to a compute service being shut down while containing standard jobs
 * with running tasks.
 *
 * Also testing that SimulationTimestampTaskTerminated timestamps get created
 * when tasks are deliberately cleanly_terminated by the WMS.
 */

class SimulationTimestampTaskTerminateAndFailTestWMS
    : public wrench::ExecutionController {
public:
  SimulationTimestampTaskTerminateAndFailTestWMS(
      SimulationTimestampTaskTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    this->test->task1 =
        this->test->workflow->addTask("terminated_task", 1000.0, 1, 1, 0);
    auto job_that_will_be_terminated =
        job_manager->createStandardJob(this->test->task1);
    job_manager->submitJob(job_that_will_be_terminated,
                           this->test->compute_service);
    wrench::S4U_Simulation::sleep(10.0);
    job_manager->terminateJob(job_that_will_be_terminated);
    // should a StandardJobTerminated event be sent? (if terminateJob is called
    // then waitForAndProcessNextEvent() we get stuck)

    this->test->task2 =
        this->test->workflow->addTask("failed_task", 1000.0, 1, 1, 0);
...
TEST_F(SimulationTimestampTaskTest,
       SimulationTimestampTaskTerminateAndFailTest) {
  DO_TEST_WITH_FORK(do_SimulationTimestampTaskTerminateAndFail_test);
}

void SimulationTimestampTaskTest::
    do_SimulationTimestampTaskTerminateAndFail_test() {
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 2;
  auto argv = (char **)calloc(argc, sizeof(char *));
  argv[0] = strdup("unit_test");
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimulationTimestampTaskTerminateAndFailTestWMS(this, wms_host)));
   ...
   *  - terminated_task is terminated (SimulationTimestampTaskTerminated should
   * be generated)
   *  - failed_task start (SimulatonTimestampTaskStart should be generated)
   *  - some time passes and compute services gets turned off while task1 is
   * running
   *  - failed_task fails (SimulationTimestampTaskFailure should be generated)
   */
  auto start_timestamps =
      simulation->getOutput().getTrace<wrench::SimulationTimestampTaskStart>();
  auto terminated_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampTaskTermination>();
  auto failure_timestamps =
      simulation->getOutput()
          .getTrace<wrench::SimulationTimestampTaskFailure>();
  ...
  wrench::SimulationTimestampTask *terminated_task_start =
      start_timestamps[0]->getContent();
  wrench::SimulationTimestampTask *terminated_task_termination =
      terminated_timestamps[0]->getContent();
  ...
  wrench::SimulationTimestampTask *failed_task_start =
      start_timestamps[1]->getContent();
  wrench::SimulationTimestampTask *failed_task_failure =
      failure_timestamps[0]->getContent();
