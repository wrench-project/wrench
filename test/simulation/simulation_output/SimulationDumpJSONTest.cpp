...
protected:
  ...
  SimulationDumpJSONTest() {
    ...
    std::string xml2 =
        ...
        "<host id=\"host1\" speed=\"100.0Mf,50.0Mf,20.0Mf\" pstate=\"1\" "
        "core=\"1\" >"
        "<prop id=\"wattage_per_state\" value=\"100.0:200.0, 93.0:170.0, "
        "90.0:150.0\" />"
        "<prop id=\"wattage_off\" value=\"10\" />"
        "</host>"

        "<host id=\"host2\" speed=\"100.0Mf,50.0Mf,20.0Mf\" pstate=\"0\" "
        "core=\"1\" >"
        "<prop id=\"wattage_per_state\" value=\"100.0:200.0, 93.0:170.0, "
        "90.0:150.0\" />"
        "<prop id=\"wattage_off\" value=\"10\" />"
    ...
    std::string xml3 =
        ...
        "<link_ctn id=\"1\"/> <link_ctn id=\"2\"/> </route>"
    ...
    std::string xml3_broken =
        ...
        "   </zone> "
        "</platform>";
    FILE *platform_file3_broken =
        fopen(platform_file_path3_broken.c_str(), "w");
    ...
    std::string xml4 =
        ...
        "     <cluster id=\"hpc.edu\" prefix=\"hpc.edu/node_\" suffix=\"\" "
        "radical=\"0-3"
        "\" core=\"1\" speed=\"1000Gf\" bw=\"1288.6597MBps\" lat=\"10us\" "
        "router_id=\"hpc_gateway\">"
        ...
        "          <host id=\"my_lab_computer.edu\" speed=\"1000Gf\" "
        "core=\"1\"/>"
        "      </zone>"
        "      <!-- effective bandwidth = 125 MBps -->"
        "      <link id=\"link1\" bandwidth=\"128.8659MBps\" "
        "latency=\"100us\"/>"
        "      <zoneRoute src=\"AS2\" dst=\"hpc.edu\" "
        "gw_src=\"storage_db.edu\" gw_dst=\"hpc_gateway\">"
        "        <link_ctn id=\"link1\"/>"
        "      </zoneRoute>"
        "      <zoneRoute src=\"AS3\" dst=\"hpc.edu\" "
        "gw_src=\"my_lab_computer.edu\" gw_dst=\"hpc_gateway\">"
        "        <link_ctn id=\"link1\"/>"
        "      </zoneRoute>"
        "      <zoneRoute src=\"AS3\" dst=\"AS2\" "
        "gw_src=\"my_lab_computer.edu\" gw_dst=\"storage_db.edu\">"
        "        <link_ctn id=\"link1\"/>"
        "      </zoneRoute>"
        "   </zone>"
        "</platform>";
    ...
    std::string xml5 =
        ...
        "       <host id=\"host1\" speed=\"1f\" core=\"10\"> "
        "         <prop id=\"ram\" value=\"10B\"/>"
        "         <disk id=\"large_disk\" read_bw=\"100000TBps\" "
        "write_bw=\"100000TBps\">"
        ...
        "       <host id=\"host2\" speed=\"1f\" core=\"20\"> "
        "          <prop id=\"ram\" value=\"20B\"/> "
        "          <disk id=\"large_disk1\" read_bw=\"100000TBps\" "
        "write_bw=\"100000TBps\">"
        "                            <prop id=\"size\" value=\"5000GiB\"/>"
        "                            <prop id=\"mount\" value=\"/\"/>"
        "       </disk>"
    ...
    std::string xml6 =
        ...
        "          <disk id=\"large_disk1\" read_bw=\"2MBps\" "
        "write_bw=\"2MBps\">"
        "                            <prop id=\"size\" value=\"5000GiB\"/>"
        "                            <prop id=\"mount\" value=\"/\"/>"
        "       </disk>"
        ...
        "       <route src=\"host1\" dst=\"host2\"> <link_ctn id=\"1\"/> "
        "</route>"
        "       <route src=\"host1\" dst=\"host1\"> <link_ctn id=\"1\"/> "
        "</route>"
        "       <route src=\"host2\" dst=\"host2\"> <link_ctn id=\"1\"/> "
        "</route>"
        "   </zone> "
        "</platform>";
  ...
  std::string platform_file_path3_broken =
      UNIQUE_TMP_PATH_PREFIX + "platform3_broken.xml";
  ...
  std::string execution_data_json_file_path =
      UNIQUE_TMP_PATH_PREFIX + "workflow_data.json";
  std::string workflow_graph_json_file_path =
      UNIQUE_TMP_PATH_PREFIX + "workflow_graph_data.json";
  std::string energy_consumption_data_file_path =
      UNIQUE_TMP_PATH_PREFIX + "energy_consumption.json";
  std::string platform_graph_json_file_path =
      UNIQUE_TMP_PATH_PREFIX + "platform_graph.json";
  std::string link_usage_json_file_path =
      UNIQUE_TMP_PATH_PREFIX + "link_usage.json";
  std::string disk_operations_json_file_path =
      UNIQUE_TMP_PATH_PREFIX + "disk_operations.json";
  std::string unified_json_file_path =
      UNIQUE_TMP_PATH_PREFIX + "unified_output.json";
};

// some comparison functions to be used when sorting lists of JSON objects so
// that the tests are deterministic
...
void SimulationDumpJSONTest::do_SimulationDumpWorkflowExecutionJSON_test() {
  ...
  EXPECT_THROW(simulation->getOutput().dumpWorkflowExecutionJSON(
                   nullptr, execution_data_json_file_path),
               std::invalid_argument);
  EXPECT_THROW(simulation->getOutput().dumpWorkflowExecutionJSON(workflow, ""),
               std::invalid_argument);

  EXPECT_NO_THROW(simulation->getOutput().dumpWorkflowExecutionJSON(
      workflow, execution_data_json_file_path, false));
  ...
  /*
   * nlohmann::json doesn't maintain order when you push_back json objects into
   * a vector so, before comparing results with expected values, we sort the
   * json lists so the test is deterministic.
   */

  std::sort(result_json["workflow_execution"]["tasks"].begin(),
            result_json["workflow_execution"]["tasks"].end(),
            compareStartTimes);
  std::sort(expected_json["workflow_execution"]["tasks"].begin(),
            expected_json["workflow_execution"]["tasks"].end(),
            compareStartTimes);
...
/**********************************************************************/
void SimulationDumpJSONTest::
    do_SimulationSearchForHostUtilizationGraphLayout_test() {
  int argc = 1;
  auto argv = (char **)calloc(argc, sizeof(char *));
  argv[0] = strdup("unit_test");
  ...
  EXPECT_NO_THROW(simulation->getOutput().dumpWorkflowExecutionJSON(
      workflow, execution_data_json_file_path, true));
  ...
  /*
   * Two tasks run, one after the other on host1. task2 starts at the same time
   * that task1 ends. Both tasks should have a vertical position of 0.
   */
  t1->setStartDate(1.0);
  t1->setEndDate(2.0);
  t1->setExecutionHost("host1");
  ...
  /*
   * Two hosts run two tasks each. We expect the following vertical positions to
   * be set:
   *  - task1: 0
   *  - task2: 5
   *  - task3: 0
  ...
  /*
   * An execution that has no layout because we were possibly oversubscribed.
   * std::runtime_error should be thrown.
   */
  t1->setStartDate(1.0);
  t1->setEndDate(2.0);
  t1->setExecutionHost("host1");
  ...
  // EXPECT_THROW(simulation->getOutput().dumpWorkflowExecutionJSON(workflow,
  // execution_data_json_file_path, true), std::runtime_error);
...
TEST_F(SimulationDumpJSONTest,
       SimulationSearchForHostUtilizationGraphLayoutTest) {
...
void SimulationDumpJSONTest::do_SimulationDumpWorkflowGraphJSON_test() {
  ...
  EXPECT_THROW(simulation->getOutput().dumpWorkflowGraphJSON(
                   nullptr, UNIQUE_TMP_PATH_PREFIX + "file.json"),
               std::invalid_argument);

  std::ifstream graph_json_file;

  // Generate a workflow with two independent tasks. Both tasks each have one
  ...
  EXPECT_NO_THROW(simulation->getOutput().dumpWorkflowGraphJSON(
      independent_tasks_workflow, workflow_graph_json_file_path));
   ...
   * json lists so the test is deterministic.
   */
  //    std::sort(result_json1["workflow_graph"]["edges"].begin(),
  //    result_json1["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(result_json1["workflow_graph"]["vertices"].begin(),
  //    result_json1["workflow_graph"]["vertices"].end(), compareNodes);
  //
  //    std::sort(expected_json1["workflow_graph"]["edges"].begin(),
  //    expected_json1["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(expected_json1["workflow_graph"]["vertices"].begin(),
  //    expected_json1["workflow_graph"]["vertices"].end(), compareNodes);
  ...
  // Generate a workflow with two tasks, two input files, and four output files.
  // Both tasks use both input files and produce two output files each.
  ...
  EXPECT_NO_THROW(simulation->getOutput().dumpWorkflowGraphJSON(
      two_tasks_use_all_files_workflow, workflow_graph_json_file_path));
...
{"name":"WRENCH-generated workflow","schemaVersion":"1.3","workflow":{"machines":[],"makespan":-1.0,"tasks":[{"children":[],"cores":1,"files":[{"link":"input","name":"input_file1","size":1.0},{"link":"input","name":"input_file2","size":2.0},{"link":"output","name":"output_file1","size":1.0},{"link":"output","name":"output_file2","size":2.0}],"machine":"","memory":0.0,"name":"task1","parents":[],"runtime":-1.0,"type":"compute"},{"children":[],"cores":1,"files":[{"link":"input","name":"input_file1","size":1.0},{"link":"input","name":"input_file2","size":2.0},{"link":"output","name":"output_file3","size":1.0},{"link":"output","name":"output_file4","size":1.0}],"machine":"","memory":0.0,"name":"task2","parents":[],"runtime":-1.0,"type":"compute"}]}}
)"_json;
   ...
   * json lists so the test is deterministic.
   */
  //    std::sort(result_json2["workflow_graph"]["edges"].begin(),
  //    result_json2["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(result_json2["workflow_graph"]["vertices"].begin(),
  //    result_json2["workflow_graph"]["vertices"].end(), compareNodes);
  //
  //    std::sort(expected_json2["workflow_graph"]["edges"].begin(),
  //    expected_json2["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(expected_json2["workflow_graph"]["vertices"].begin(),
  //    expected_json2["workflow_graph"]["vertices"].end(), compareNodes);
  ...
  // Generate a workflow where one task1 forks into two tasks, then those two
  // tasks join into one.
  ...
  t4->addInputFile(wrench::Simulation::getFileByID("task3_output1"));
  ...
  EXPECT_NO_THROW(simulation->getOutput().dumpWorkflowGraphJSON(
      fork_join_workflow, workflow_graph_json_file_path));
...
{"name":"WRENCH-generated workflow","schemaVersion":"1.3","workflow":{"machines":[],"makespan":-1.0,"tasks":[{"children":["task2","task3"],"cores":1,"files":[{"link":"input","name":"task1_input","size":1.0},{"link":"output","name":"task1_output1","size":1.0},{"link":"output","name":"task1_output2","size":1.0}],"machine":"","memory":0.0,"name":"task1","parents":[],"runtime":-1.0,"type":"compute"},{"children":["task4"],"cores":1,"files":[{"link":"input","name":"task1_output1","size":1.0},{"link":"output","name":"task2_output1","size":1.0}],"machine":"","memory":0.0,"name":"task2","parents":["task1"],"runtime":-1.0,"type":"compute"},{"children":["task4"],"cores":1,"files":[{"link":"input","name":"task1_output2","size":1.0},{"link":"output","name":"task3_output1","size":1.0}],"machine":"","memory":0.0,"name":"task3","parents":["task1"],"runtime":-1.0,"type":"compute"},{"children":[],"cores":1,"files":[{"link":"input","name":"task2_output1","size":1.0},{"link":"input","name":"task3_output1","size":1.0},{"link":"output","name":"task4_output1","size":1.0}],"machine":"","memory":0.0,"name":"task4","parents":["task2","task3"],"runtime":-1.0,"type":"compute"}]}}
   ...
   * json lists so the test is deterministic.
   */
  //    std::sort(result_json3["workflow_graph"]["edges"].begin(),
  //    result_json3["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(result_json3["workflow_graph"]["vertices"].begin(),
  //    result_json3["workflow_graph"]["vertices"].end(), compareNodes);
  //
  //    std::sort(expected_json3["workflow_graph"]["edges"].begin(),
  //    expected_json3["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(expected_json3["workflow_graph"]["vertices"].begin(),
  //    expected_json3["workflow_graph"]["vertices"].end(), compareNodes);
...
/**********************************************************************/
class SimulationOutputDumpEnergyConsumptionTestWMS
...
private:
  ...
  int main() override {
    ...
    // then again at time 0.0, it is set to pstate 0 and only one timestamp
    // should be generated for this host at time 0.0
    ...
    wrench::S4U_Simulation::compute(6.1 * 100.0 *
                                    MEGAFLOP); // compute for 6 seconds
...
void SimulationDumpJSONTest::do_SimulationDumpHostEnergyConsumptionJSON_test() {
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(
          new SimulationOutputDumpEnergyConsumptionTestWMS(this, host)));

  simulation->getOutput().enableEnergyTimestamps(true);

  EXPECT_NO_THROW(simulation->launch());

  EXPECT_THROW(simulation->getOutput().dumpHostEnergyConsumptionJSON(""),
               std::invalid_argument);

  //    ASSERT_NO_THROW(simulation->getOutput().dumpHostEnergyConsumptionJSON(this->energy_consumption_data_file_path));

  simulation->getOutput().dumpUnifiedJSON(
      workflow, this->energy_consumption_data_file_path, false, false, false,
  ...
  std::sort(expected_json["energy_consumption"].begin(),
            expected_json["energy_consumption"].end(), compareHostname);
  for (size_t i = 0; i < expected_json["energy_consumption"].size(); ++i) {
    std::sort(
        expected_json["energy_consumption"][i]["consumed_energy_trace"].begin(),
        expected_json["energy_consumption"][i]["consumed_energy_trace"].end(),
        compareTime);
    std::sort(expected_json["energy_consumption"][i]["pstates"].begin(),
              expected_json["energy_consumption"][i]["pstates"].end(),
              comparePstate);
    std::sort(expected_json["energy_consumption"][i]["pstate_trace"].begin(),
              expected_json["energy_consumption"][i]["pstate_trace"].end(),
              comparePstate);
  }
  ...
  std::sort(result_json["energy_consumption"].begin(),
            result_json["energy_consumption"].end(), compareHostname);
  for (size_t i = 0; i < expected_json["energy_consumption"].size(); ++i) {
    std::sort(
        result_json["energy_consumption"][i]["consumed_energy_trace"].begin(),
        result_json["energy_consumption"][i]["consumed_energy_trace"].end(),
        compareTime);
    std::sort(result_json["energy_consumption"][i]["pstates"].begin(),
              result_json["energy_consumption"][i]["pstates"].end(),
              comparePstate);
    std::sort(result_json["energy_consumption"][i]["pstate_trace"].begin(),
              result_json["energy_consumption"][i]["pstate_trace"].end(),
              comparePstate);
  }
...
bool compareEdges(const nlohmann::json &lhs, const nlohmann::json &rhs) {
  return (lhs["source"]["type"].get<std::string>() + ":" +
          lhs["source"]["id"].get<std::string>() + "-" +
          lhs["target"]["type"].get<std::string>() + ":" +
          lhs["target"]["id"].get<std::string>()) <
         (rhs["source"]["type"].get<std::string>() + ":" +
          rhs["source"]["id"].get<std::string>() + "-" +
          rhs["target"]["type"].get<std::string>() + ":" +
          rhs["target"]["id"].get<std::string>());
}

bool compareRoutes(const nlohmann::json &lhs, const nlohmann::json &rhs) {
  return (lhs["source"].get<std::string>() + "-" +
          lhs["target"].get<std::string>()) <
         (rhs["source"].get<std::string>() + "-" +
          rhs["target"].get<std::string>());
}

/**********************************************************************/
/**               SimulationDumpLinkUsageJSONTest                    **/
/**********************************************************************/
class SimulationOutputDumpLinkUsageTestWMS
    : public wrench::ExecutionController {
public:
...
void SimulationDumpJSONTest::do_SimulationDumpLinkUsageJSON_test() {
  ...
  // std::shared_ptr<wrench::DataFile> file = new wrench::DataFile("test_file",
  // 10*GB);
  ...
  EXPECT_NO_THROW(wms = simulation->add(
                      new SimulationOutputDumpLinkUsageTestWMS(this, host)));
  ...
  EXPECT_THROW(simulation->getOutput().dumpLinkUsageJSON(""),
               std::invalid_argument);

  EXPECT_NO_THROW(simulation->getOutput().dumpLinkUsageJSON(
      this->link_usage_json_file_path));
  simulation->getOutput().dumpUnifiedJSON(
      link_usage_workflow, "/tmp/linkusage_unified.json", false, true, true,
      false, false, false, true);

  /* The 125000000.00000001 below is confusing, because it doesn't account for
   * the .97 factor that limits bandwidth, but that's something that was changed
   * in SimGrid3.28 (simulation times are not affected, just number of bytes) */
  nlohmann::json expected_json_link_usage_version1 = R"(
    {
        "link_usage": {
            "links": [
                {
  ...
  /* Depending on the SimGrid version there is a  .00000001 difference in value,
   * so let's not care and down the line remove one of these two versions */
  nlohmann::json expected_json_link_usage_version2 = R"(
    {
        "link_usage": {
            "links": [
                {
  ...
  EXPECT_TRUE((expected_json_link_usage_version1 == result_json) or
              (expected_json_link_usage_version2 == result_json));
...
/*
 * Testing the basic functionality of the SimulationTimestampDiskReadWrite
 * class. This test ensures that SimulationTimestampDiskReadWriteStart,
 * SimulationTimestampDiskReadWriteFailure, and
 * SimulationTimestampDiskReadWriteCompletion objects are added to their
 * respective simulation traces at the appropriate times.
 */
...
private:
  ...
  int main() override {
    ...
    wrench::StorageService::writeFileAtLocation(
        wrench::FileLocation::LOCATION(this->test->ss1, file_1));
    wrench::StorageService::writeFileAtLocation(
        wrench::FileLocation::LOCATION(this->test->ss2, file_1));
...
void SimulationDumpJSONTest::do_SimulationDumpDiskOperationsJSON_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new SimulationDumpDiskOperationsTestWMS(this, host1)));

  simulation->getOutput().enableDiskTimestamps(true);

  ASSERT_NO_THROW(simulation->launch());

  EXPECT_THROW(simulation->getOutput().dumpDiskOperationsJSON(""),
               std::invalid_argument);

  EXPECT_NO_THROW(simulation->getOutput().dumpDiskOperationsJSON(
      this->disk_operations_json_file_path));
  simulation->getOutput().dumpUnifiedJSON(workflow, "/tmp/disk_unified.json",
  ...
  for (auto const &operation : (std::vector<std::string>){"reads"}) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      double duration =
          (double)result_json["host1"]["/"][operation][i]["end"] -
...
void SimulationDumpJSONTest::do_SimulationDumpPlatformGraphJSON_test() {
  ...
  EXPECT_THROW(simulation->getOutput().dumpPlatformGraphJSON(""),
               std::invalid_argument);
  EXPECT_NO_THROW(simulation->getOutput().dumpPlatformGraphJSON(
      this->platform_graph_json_file_path));
  ...
  std::sort(result_json["platform"]["edges"].begin(),
            result_json["platform"]["edges"].end(), compareEdges);
  std::sort(expected_json["platform"]["edges"].begin(),
            expected_json["platform"]["edges"].end(), compareEdges);

  // sort the nodes
  std::sort(result_json["platform"]["vertices"].begin(),
            result_json["platform"]["vertices"].end(), compareNodes);
  std::sort(expected_json["platform"]["vertices"].begin(),
            expected_json["platform"]["vertices"].end(), compareNodes);

  // sort the routes
  std::sort(result_json["platform"]["routes"].begin(),
            result_json["platform"]["routes"].end(), compareRoutes);
  std::sort(expected_json["platform"]["routes"].begin(),
            expected_json["platform"]["routes"].end(), compareRoutes);
...
TEST_F(SimulationDumpJSONTest,
       SimulationDumpPlatformGraphJSONWithBrokenRoutingTest) {
  DO_TEST_WITH_FORK(do_SimulationDumpPlatformGraphJSONBrokenRouting_test);
}

void SimulationDumpJSONTest::
    do_SimulationDumpPlatformGraphJSONBrokenRouting_test() {
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  auto argv = (char **)calloc(argc, sizeof(char *));
  argv[0] = strdup("unit_test");
...
{"platform":{"edges":[{"source":{"id":"host1","type":"host"},"target":{"id":"1","type":"link"}},{"source":{"id":"1","type":"link"},"target":{"id":"host2","type":"host"}}],"routes":[{"latency":1e-06,"route":["1"],"source":"host1","target":"host2"}],"vertices":[{"bandwidth":125000000.0,"id":"1","latency":1e-06,"type":"link"},{"cluster_id":"host1","cores":10,"flop_rate":1.0,"id":"host1","memory_manager_service":10.0,"type":"host"},{"cluster_id":"host2","cores":20,"flop_rate":1.0,"id":"host2","memory_manager_service":20.0,"type":"host"}]},"workflow_execution":{"tasks":[{"color":"","compute":{"end":-1.0,"start":-1.0},"execution_host":{"cores":10,"flop_rate":1.0,"hostname":"host1","memory_manager_service":10.0},"failed":-1.0,"num_cores_allocated":8,"read":null,"task_id":"task1","terminated":-1.0,"whole_task":{"end":2.0,"start":1.0},"write":null},{"color":"","compute":{"end":-1.0,"start":-1.0},"execution_host":{"cores":20,"flop_rate":1.0,"hostname":"host2","memory_manager_service":20.0},"failed":-1.0,"num_cores_allocated":10,"read":null,"task_id":"task1","terminated":-1.0,"whole_task":{"end":3.0,"start":2.0},"write":null},{"color":"","compute":{"end":-1.0,"start":-1.0},"execution_host":{"cores":20,"flop_rate":1.0,"hostname":"host2","memory_manager_service":20.0},"failed":-1.0,"num_cores_allocated":20,"read":null,"task_id":"task2","terminated":-1.0,"whole_task":{"end":4.0,"start":3.0},"write":null}]},"workflow_graph":{"machines":[],"makespan":-1.0,"tasks":[{"children":[],"cores":1,"files":[],"machine":"","memory":0.0,"name":"task1","parents":[],"runtime":-1.0,"type":"compute"},{"children":[],"cores":1,"files":[],"machine":"","memory":0.0,"name":"task2","parents":[],"runtime":-1.0,"type":"compute"}]}}
  ...
  std::sort(result_json["platform"]["edges"].begin(),
            result_json["platform"]["edges"].end(), compareEdges);
  std::sort(expected_json5["platform"]["edges"].begin(),
            expected_json5["platform"]["edges"].end(), compareEdges);

  std::sort(result_json["platform"]["vertices"].begin(),
            result_json["platform"]["vertices"].end(), compareNodes);
  std::sort(expected_json5["platform"]["vertices"].begin(),
            expected_json5["platform"]["vertices"].end(), compareNodes);

  std::sort(result_json["platform"]["routes"].begin(),
            result_json["platform"]["routes"].end(), compareRoutes);
  std::sort(expected_json5["platform"]["routes"].begin(),
            expected_json5["platform"]["routes"].end(), compareRoutes);

  std::sort(result_json["workflow_execution"]["tasks"].begin(),
            result_json["workflow_execution"]["tasks"].end(),
            compareStartTimes);
  std::sort(expected_json5["workflow_execution"]["tasks"].begin(),
            expected_json5["workflow_execution"]["tasks"].end(),
            compareStartTimes);
  //
  //    std::sort(result_json["workflow_graph"]["edges"].begin(),
  //    result_json["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(result_json["workflow_graph"]["vertices"].begin(),
  //    result_json["workflow_graph"]["vertices"].end(), compareNodes);
  //
  //    std::sort(expected_json5["workflow_graph"]["edges"].begin(),
  //    expected_json5["workflow_graph"]["edges"].end(), compareLinks);
  //    std::sort(expected_json5["workflow_graph"]["vertices"].begin(),
  //    expected_json5["workflow_graph"]["vertices"].end(), compareNodes);
