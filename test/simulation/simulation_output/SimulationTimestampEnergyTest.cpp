...
WRENCH_LOG_CATEGORY(test_energy,
                    "Log category for SimulationTimestampEnergyTest");
...
protected:
  ...
  SimulationTimestampEnergyTest()
    ...
    std::string xml =
        ...
        "<host id=\"host1\" speed=\"100.0Mf,50.0Mf,20.0Mf\" pstate=\"0\" "
        "core=\"1\" >"
...
private:
  ...
  int main() override {
    ...
    // then the second call to setPstate(hostname, 1) should record a timestamp
    // that replaces the one previously added so that we don't end up with
    // timestamps that show two different pstates at the same point in time
...
void SimulationTimestampEnergyTest::do_SimulationTimestampPstateSet_test() {
  ...
  EXPECT_NO_THROW(wms = simulation->add(
                      new SimulationTimestampPstateSetTestWMS(this, host)));
  ...
  EXPECT_THROW(simulation->getOutput().addTimestampPstateSet(
                   wrench::Simulation::getCurrentSimulatedDate(), "", 0),
               std::invalid_argument);
#endif

  // Check that the expected SimulationTimestampPstateSet timestamps have been
  // added to simulation output
  auto set_pstate_timestamps =
      simulation->getOutput().getTrace<wrench::SimulationTimestampPstateSet>();
...
/*
 * Testing the basic functionality of the SimulationTimestampEnergyConsumption
 * class. This test assures that the timestamps are added to simulation output
 * when getEnergyTimestamp("hostname", true) is called.
 */
class SimulationTimestampEnergyConsumptionTestWMS
    : public wrench::ExecutionController {
public:
  SimulationTimestampEnergyConsumptionTestWMS(
      SimulationTimestampEnergyTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
TEST_F(SimulationTimestampEnergyTest,
       SimulationTimestampEnergyConsumptionTest) {
  DO_TEST_WITH_FORK(do_SimulationTimestampEnergyConsumption_test);
}

void SimulationTimestampEnergyTest::
    do_SimulationTimestampEnergyConsumption_test() {
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 2;
  auto argv = (char **)calloc(argc, sizeof(char *));
  argv[0] = strdup("unit_test");
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(
          new SimulationTimestampEnergyConsumptionTestWMS(this, host)));
  ...
  EXPECT_THROW(
      simulation->getOutput().addTimestampEnergyConsumption(0.0, "", 1.0),
      std::invalid_argument);
#endif

  // Check that the expected SimulationTimestampEnergyConsumption timestamps
  // have been added to simulation output
  auto energy_consumption_timestamps =
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error("createEnergyMeter should have thrown invalid "
                               "argument if given an empty hostname list");
    ...
    try {
      ...
      throw std::runtime_error(
          "createEnergyMeter should have thrown invalid argument if given a "
          "measurement period less than 1.0");
    ...
    try {
      ...
      throw std::runtime_error("createEnergyMeter requires a non-empty map of "
                               "(hostname, measurement period) pairs");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error("createEnergyMeter requires that measurement "
                               "periods be 1.0 or greater");
    } catch (std::invalid_argument &e) {
    }
    ...
    const double MEGAFLOP = 1000.0 * 1000.0;
    wrench::S4U_Simulation::compute(100.0 * 100.0 *
                                    MEGAFLOP); // compute for 100 seconds
...
void SimulationTimestampEnergyTest::
    do_EnergyMeterSingleMeasurementPeriod_test() {
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 2;
  auto argv = (char **)calloc(argc, sizeof(char *));
  argv[0] = strdup("unit_test");
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(new EnergyMeterTestWMS(this, workflow, host)));
  ...
  std::vector<wrench::SimulationTimestamp<
      wrench::SimulationTimestampEnergyConsumption> *>
      host1_timestamps;
  std::copy_if(
      energy_consumption_timestamps.begin(),
      energy_consumption_timestamps.end(), std::back_inserter(host1_timestamps),
      [](wrench::SimulationTimestamp<
          wrench::SimulationTimestampEnergyConsumption> *ts) -> bool {
        return (ts->getContent()->getHostname() == "host1" ? true : false);
      });

  // host1 computes for 100 seconds, and records timestamps in 10 second
  // intervals, so we should end up with 11 timestamps (starting with time 0)
  ...
  std::vector<std::pair<double, double>> host1_expected_timestamps = {
      {0, 0},      {10, 2000},  {20, 4000},  {30, 6000},
      {40, 8000},  {50, 10000}, {60, 12000}, {70, 14000},
      {80, 16000}, {90, 18000}, {100, 20000}};

  for (size_t i = 0; i < host1_timestamps.size(); ++i) {
    ASSERT_DOUBLE_EQ(host1_expected_timestamps[i].first,
                     host1_timestamps[i]->getDate());
    ASSERT_DOUBLE_EQ(host1_expected_timestamps[i].second,
                     host1_timestamps[i]->getContent()->getConsumption());
  }
  ...
  std::copy_if(
      energy_consumption_timestamps.begin(),
      energy_consumption_timestamps.end(), std::back_inserter(host2_timestamps),
      [](wrench::SimulationTimestamp<
          wrench::SimulationTimestampEnergyConsumption> *ts) -> bool {
        return (ts->getContent()->getHostname() == "host2" ? true : false);
      });

  // host2 is idle for 100 seconds, and records timestamps in 10 second
  // intervals, so we should end up with 10 timestamps (starting with time 0)
  ...
  for (size_t i = 0; i < host2_timestamps.size(); ++i) {
    ASSERT_DOUBLE_EQ(host2_expected_timestamps[i].first,
                     host2_timestamps[i]->getDate());
    ASSERT_DOUBLE_EQ(host2_expected_timestamps[i].second,
                     host2_timestamps[i]->getContent()->getConsumption());
  }
...
class EnergyMeterMultipleMeasurementPeriodTestWMS
    : public wrench::ExecutionController {
public:
  EnergyMeterMultipleMeasurementPeriodTestWMS(
      SimulationTimestampEnergyTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    const std::map<std::string, double> measurement_periods = {{"host1", 2.0},
                                                               {"host2", 3.0}};
...
void SimulationTimestampEnergyTest::
    do_EnergyMeterMultipleMeasurementPeriod_test() {
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 2;
  auto argv = (char **)calloc(argc, sizeof(char *));
  argv[0] = strdup("unit_test");
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(
          new EnergyMeterMultipleMeasurementPeriodTestWMS(this, host)));
  ...
  // expected values (timestamp, consumption)
  std::vector<std::pair<double, double>> host1_expected_timestamps = {
      {0.0, 0.0}, {2.0, 400.0}, {4.0, 800.0}, {6.0, 1200.0}};
