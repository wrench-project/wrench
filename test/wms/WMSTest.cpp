...
public:
  TestDefaultHandlerWMS(WMSTest *test, double sleep_time, std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    job_manager->submitJob(job2, batch,
                           {{"-N", "1"}, {"-t", "50"}, {"-c", "4"}});
    this->waitForAndProcessNextEvent();

    // Submit another pilot job, which won't be running for a while
    auto job2_1 = job_manager->createPilotJob();
    job_manager->submitJob(job2_1, batch,
                           {{"-N", "1"}, {"-t", "50"}, {"-c", "4"}});
    // Get the list of pending pilot jobs

    // Get a "STANDARD JOB FAILED" and "PILOT JOB EXPIRED" event (default
    // handler)
    std::shared_ptr<wrench::WorkflowTask> task2 =
        this->test->workflow->addTask("task2", 100.0, 1, 1, 0);
    ...
    if (std::abs(wrench::Simulation::getCurrentSimulatedDate() -
      ...
      throw std::runtime_error(
          "Did not get the timer event at the right date (" +
          std::to_string(wrench::Simulation::getCurrentSimulatedDate()) +
          " instead of " + std::to_string(timer_off_date) + ")");
    }
    return 0;
  }
};
...
void WMSTest::do_DefaultHandlerWMS_test() {
  ...
  ASSERT_NO_THROW(cs_cloud = simulation->add(new wrench::CloudComputeService(
                      hostname1, cloud_hosts, "/scratch", {}, {})));

  // Create a Batch Service
  std::vector<std::string> batch_hosts;
  batch_hosts.push_back(hostname2);
  ASSERT_NO_THROW(cs_batch = simulation->add(new wrench::BatchComputeService(
                      hostname2, batch_hosts, "/scratch", {},
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new TestDefaultHandlerWMS(this, 100, hostname1)));

  // Create a file registry
  ASSERT_NO_THROW(simulation->add(new wrench::FileRegistryService(hostname1)));
...
public:
  TestCustomHandlerWMS(WMSTest *test, double sleep_time, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test),
        sleep_time(sleep_time) {}
...
private:
  ...
  int main() override {
    ...
    std::shared_ptr<wrench::WorkflowTask> task1 =
        this->test->workflow->addTask("task1", 10.0, 1, 1, 0);
    auto job1 = job_manager->createStandardJob(task1);
    job_manager->submitJob(job1, vm_cs);
    this->waitForAndProcessNextEvent();
    if (this->counter != 1) {
      throw std::runtime_error(
          "Did not get expected StandardJobCompletionEvent");
    }
    ...
    // handler)
    std::shared_ptr<wrench::WorkflowTask> task2 =
        this->test->workflow->addTask("task2", 200.0, 1, 1, 0);
    auto job3 = job_manager->createStandardJob(task2);
    job_manager->submitJob(job3, job2->getComputeService());
    this->waitForAndProcessNextEvent();
    if (this->counter != 3) {
    ...
    if (this->counter != 8) {
      throw std::runtime_error(
          "Did not get expected CompoundJobCompleted event");
    }
  ...
  void processEventPilotJobStart(
      std::shared_ptr<wrench::PilotJobStartedEvent> event) override {
    this->counter = 2;
  }
  ...
  void processEventPilotJobExpiration(
      std::shared_ptr<wrench::PilotJobExpiredEvent> event) override {
    this->counter = 4;
  }

  void processEventFileCopyCompletion(
      std::shared_ptr<wrench::FileCopyCompletedEvent> event) override {
    this->counter = 5;
  }

  void processEventFileCopyFailure(
      std::shared_ptr<wrench::FileCopyFailedEvent> event) override {
    this->counter = 6;
  }
  ...
  void processEventCompoundJobFailure(
      std::shared_ptr<wrench::CompoundJobFailedEvent> event) override {
    this->counter = 9;
  }
};
...
void WMSTest::do_CustomHandlerWMS_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new TestCustomHandlerWMS(this, 100, hostname1)));
