...
public:
  DeferredWMSStartTestWMS(MultipleWMSTest *test,
                          std::shared_ptr<wrench::Workflow> workflow,
                          double sleep_time, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test),
        workflow(workflow), sleep_time(sleep_time) {}
      ...
      std::tuple<std::shared_ptr<wrench::FileLocation>,
          ...
          each_copy =
              std::make_tuple(wrench::FileLocation::LOCATION(
                                  this->test->storage_service, it.second),
...
void MultipleWMSTest::do_deferredWMSStartOneWMS_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new DeferredWMSStartTestWMS(
                      this, workflow, 100, hostname)));
...
void MultipleWMSTest::do_deferredWMSStartTwoWMS_test() {
  ...
  ASSERT_NO_THROW(wms1 = simulation->add(new DeferredWMSStartTestWMS(
                      this, workflow, 100, hostname)));

  // Create a second WMS
  auto workflow2 = this->createWorkflow("wf_2");
  std::shared_ptr<wrench::ExecutionController> wms2 = nullptr;
  ASSERT_NO_THROW(wms2 = simulation->add(new DeferredWMSStartTestWMS(
                      this, workflow2, 10000, hostname)));
