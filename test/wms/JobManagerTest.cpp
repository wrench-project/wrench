...
void JobManagerTest::do_JobManagerConstructorTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new JobManagerConstructorTestWMS(
...
public:
  JobManagerCreateJobTestWMS(JobManagerTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to create a standard job "
                               "with a nullptr task1 in it");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to create a standard job with nothing in it");
    } catch (std::invalid_argument &e) {
    }

    std::shared_ptr<wrench::WorkflowTask> t1 =
        this->test->workflow->addTask("t1", 1.0, 1, 1, 0.0);
    std::shared_ptr<wrench::WorkflowTask> t2 =
        this->test->workflow->addTask("t2", 1.0, 1, 1, 0.0);
    ...
    try {
      std::vector<std::tuple<std::shared_ptr<wrench::FileLocation>,
          ...
          pre_file_copies;
      pre_file_copies.push_back(
          std::make_tuple(wrench::FileLocation::SCRATCH(nullptr),
                          wrench::FileLocation::SCRATCH(nullptr)));
      job_manager->createStandardJob(
          {},
    ...
    try {
      std::vector<std::tuple<std::shared_ptr<wrench::FileLocation>,
          ...
          pre_file_copies;
      pre_file_copies.push_back(
          std::make_tuple(nullptr, wrench::FileLocation::SCRATCH(f)));
      job_manager->createStandardJob(
          {},
          (std::map<std::shared_ptr<wrench::DataFile>,
                    std::shared_ptr<wrench::FileLocation>>){},
    ...
    try {
      std::vector<std::tuple<std::shared_ptr<wrench::FileLocation>,
          ...
          pre_file_copies;
      pre_file_copies.push_back(
          std::make_tuple(wrench::FileLocation::SCRATCH(f), nullptr));
      job_manager->createStandardJob(
          {},
          (std::map<std::shared_ptr<wrench::DataFile>,
                    std::shared_ptr<wrench::FileLocation>>){},
    ...
    try {
      ...
      pre_file_copies.push_back(std::make_tuple(
          wrench::FileLocation::SCRATCH(f), wrench::FileLocation::SCRATCH(f)));
      job_manager->createStandardJob(
          {},
          (std::map<std::shared_ptr<wrench::DataFile>,
                    std::shared_ptr<wrench::FileLocation>>){},
          pre_file_copies, {}, {});
      throw std::runtime_error(
          "Should not be able to create a standard job with a pre file copy "
          "that has SCRATCH has both dst and src");
    } catch (std::invalid_argument &e) {
    }
    // Create a job with SCRATCH as both src and dst
    try {
      std::vector<std::tuple<std::shared_ptr<wrench::FileLocation>,
                             std::shared_ptr<wrench::FileLocation>>>
          post_file_copies;
      post_file_copies.push_back(std::make_tuple(
          wrench::FileLocation::SCRATCH(f), wrench::FileLocation::SCRATCH(f)));
      job_manager->createStandardJob(
          {},
          (std::map<std::shared_ptr<wrench::DataFile>,
          ...
          {}, post_file_copies, {});
      throw std::runtime_error(
          "Should not be able to create a standard job with a post file copy "
          "that has SCRATCH has both dst and src");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job_manager->createStandardJob(
          (std::vector<std::shared_ptr<wrench::WorkflowTask>>){t2});
      throw std::runtime_error("Should not be able to create a standard job "
                               "with a not-self-contained task1");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should be able to create a standard job with two dependent tasks");
    }
...
void JobManagerTest::do_JobManagerCreateJobTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new JobManagerCreateJobTestWMS(this, hostname)));
...
public:
  JobManagerSubmitJobTestWMS(JobManagerTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    std::shared_ptr<wrench::BareMetalComputeService> cs = simulation->add(
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to submit a job with a nullptr job");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to submit a job with a nullptr compute service");
    } catch (std::invalid_argument &e) {
    }
...
void JobManagerTest::do_JobManagerSubmitJobTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new JobManagerSubmitJobTestWMS(this, hostname)));
...
public:
  JobManagerResubmitJobTestWMS(JobManagerTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to submit a standard job to "
                               "a service that does not support them");
    } catch (std::invalid_argument &ignore) {
    }

    // Check task1 states
    std::shared_ptr<wrench::WorkflowTask> task1 =
        this->test->workflow->getTaskByID("task1");
    std::shared_ptr<wrench::WorkflowTask> task2 =
        this->test->workflow->getTaskByID("task2");
    if (task1->getState() != wrench::WorkflowTask::State::READY) {
      throw std::runtime_error(
          "Unexpected task1 state (should be READY but is " +
          wrench::WorkflowTask::stateToString(task1->getState()));
    }
    if (task2->getState() != wrench::WorkflowTask::State::NOT_READY) {
      throw std::runtime_error(
          "Unexpected task2 state (should be READY but is " +
    ...
    try {
      ...
      throw std::runtime_error("Should be able to subkmit a standard job to a "
...
void JobManagerTest::do_JobManagerResubmitJobTest_test() {
  ...
  ASSERT_NO_THROW(
      cs1 = simulation->add(new wrench::BareMetalComputeService(
          "Host2",
          {std::make_pair("Host2",
  ...
  ASSERT_NO_THROW(
      cs2 = simulation->add(new wrench::BareMetalComputeService(
          "Host3",
          {std::make_pair("Host3",
                          std::make_tuple(wrench::ComputeService::ALL_CORES,
                                          wrench::ComputeService::ALL_RAM))},
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new JobManagerResubmitJobTestWMS(this, "Host1")));

  // Add tasks to the workflow
  std::shared_ptr<wrench::WorkflowTask> task1 =
      workflow->addTask("task1", 10.0, 10, 10, 0.0);
  std::shared_ptr<wrench::WorkflowTask> task2 =
      workflow->addTask("task2", 10.0, 10, 10, 0.0);
...
public:
  JobManagerTerminateJobTestWMS(JobManagerTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    // Add tasks to the workflow
    std::shared_ptr<wrench::WorkflowTask> t1 =
        this->test->workflow->addTask("task1", 600, 10, 10, 80);
    std::shared_ptr<wrench::WorkflowTask> t2 =
        this->test->workflow->addTask("task2", 600, 9, 10, 80);
    std::shared_ptr<wrench::WorkflowTask> t3 =
        this->test->workflow->addTask("task3", 600, 8, 10, 0);
    std::shared_ptr<wrench::WorkflowTask> t4 =
    ...
    try {
      ...
      throw std::runtime_error("Should be able to submit the job");
    }
    ...
    try {
      ...
      throw std::runtime_error("Should be able to terminate job");
    }

    // Check task1 states
    bool t1_and_t2_valid_states;

    t1_and_t2_valid_states =
        ((t1->getState() == wrench::WorkflowTask::State::COMPLETED) and
    ...
    if (not t1_and_t2_valid_states) {
      throw std::runtime_error(
          "Unexpected task1 and task1 2 states (" +
    ...
    if (t3->getState() != wrench::WorkflowTask::State::NOT_READY) {
      throw std::runtime_error(
          "Unexpected task1 state (should be NOT_READY but is " +
...
void JobManagerTest::do_JobManagerTerminateJobTest_test() {
  ...
  ASSERT_NO_THROW(
      cs = simulation->add(new wrench::BareMetalComputeService(
          "Host3",
          {std::make_pair("Host3",
                          std::make_tuple(wrench::ComputeService::ALL_CORES,
                                          wrench::ComputeService::ALL_RAM))},
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new JobManagerTerminateJobTestWMS(this, "Host1")));
