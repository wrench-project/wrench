...
WRENCH_LOG_CATEGORY(batch_compute_service_mpi_action_test,
                    "Log category for BatchComputeServiceMPIAction test");
...
private:
  ...
  int main() override {
...

    {
      ...
      auto mpi_code_noop =
          [](const std::shared_ptr<wrench::ExecutionController> &controller) {};

      /* Add an action with 100 MPI processes, each of which has 10 cores */
      auto action_too_big = job_to_big->addMPIAction("my_mpi_action_too_big",
                                                     mpi_code_noop, 100, 10);

      /* Submit the job to the batch compute service */
      WRENCH_INFO("Submitting job %s to the batch service",
                  job_to_big->getName().c_str());
      ...
      try {
        job_manager->submitJob(job_to_big, batch_compute_service,
                               service_specific_args);
        throw std::runtime_error(
            "Should not be able to submit a job that's too big");
...
void BatchComputeServiceMPIActionTest::do_MPIAction_test() {
  ...
  ASSERT_NO_THROW(
      compute_service = simulation->add(new wrench::BatchComputeService(
          "Host1", {"Host1", "Host2", "Host3", "Host4"}, "", {})));

  // Create a WMS
  std::string hostname = "Host1";
  ASSERT_NO_THROW(simulation->add(
      new BatchMPIActionTestWMS(this, compute_service, hostname)));
