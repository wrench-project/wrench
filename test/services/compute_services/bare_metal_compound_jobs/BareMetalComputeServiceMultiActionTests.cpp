...
WRENCH_LOG_CATEGORY(bare_metal_compute_service_multi_action_test,
                    "Log category for BareMetalComputeServiceMultiAction test");
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to add nullptr Action dependencies");
    ...
    for (unsigned long i = 0; i < first_chain_length; i++) {
      first_chain_tasks.push_back(job->addSleepAction(
          "chain1_sleep_" + std::to_string(i), dist_sleep(rng)));
      if (i > 0) {
        job->addActionDependency(first_chain_tasks[i - 1],
                                 first_chain_tasks[i]);
      }
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to add a task with same name as an existing task");
    ...
    for (unsigned long i = 0; i < fork_width; i++) {
      fork_tasks.push_back(job->addSleepAction(
          "fork_sleep_" + std::to_string(i), dist_sleep(rng)));
      job->addActionDependency(first_chain_tasks[first_chain_tasks.size() - 1],
                               fork_tasks[i]);
    }

    std::vector<std::shared_ptr<wrench::SleepAction>> second_chain_tasks;
    for (unsigned long i = 0; i < second_chain_length; i++) {
      second_chain_tasks.push_back(job->addSleepAction(
          "chain2_sleep_" + std::to_string(i), dist_sleep(rng)));
      if (i > 0) {
        job->addActionDependency(second_chain_tasks[i - 1],
                                 second_chain_tasks[i]);
      } else if (i == 0) {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to add action to job that has been submitted");
    } catch (std::runtime_error &ignore) {
    }
    ...
    // Check makespan
    if (std::abs<double>(wrench::Simulation::getCurrentSimulatedDate() -
                         expected_makespan) > 0.0001) {
...
void BareMetalComputeServiceMultiActionTest::do_DAGOfSleeps_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new DAGOfSleepsTestWMS(this, workflow, hostname)));
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to add a dependency between the same action");
    } catch (std::invalid_argument &ignore) {
    }
...
void BareMetalComputeServiceMultiActionTest::do_NonDAG_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  std::string hostname = "Host1";
  ASSERT_NO_THROW(wms = simulation->add(new NonDAGsTestWMS(this, hostname)));
...
public:
  RAMConstraintsAndPrioritiesTestWMS(
      BareMetalComputeServiceMultiActionTest *test, std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    auto compute1 = job->addComputeAction("compute_1", 100.0, 200.0, 1, 2,
                                          wrench::ParallelModel::AMDAHL(1.0));
    auto compute2 = job->addComputeAction("compute_2", 100.0, 200.0, 1, 2,
                                          wrench::ParallelModel::AMDAHL(1.0));
    auto sleep2 = job->addSleepAction("sleep_2", 10.0);
    auto compute3 = job->addComputeAction("compute_3", 100.0, 600.0, 1, 1,
                                          wrench::ParallelModel::AMDAHL(1.0));
    compute3->setPriority(10.0);
    auto compute4 = job->addComputeAction("compute_4", 100.0, 600.0, 1, 2,
                                          wrench::ParallelModel::AMDAHL(1.0));
    ...
    // Check timing
    if (std::abs<double>(compute1->getEndDate() - compute2->getEndDate()) >
        0.0001) {
      throw std::runtime_error("Compute1 and Compute2 actions should have "
                               "completed at the same time");
    }

    if (std::abs<double>(compute3->getEndDate() - compute4->getStartDate()) >
        0.0001) {
      throw std::runtime_error("Compute4 action should start only after "
                               "Compute3 action has completed");
    }
...
void BareMetalComputeServiceMultiActionTest::
    do_RAMConstraintsAndPriorities_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new RAMConstraintsAndPrioritiesTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    auto file_read = job->addFileReadAction(
        "file_read", wrench::FileLocation::LOCATION(
                         this->test->storage_service1, this->test->input_file));
    auto sleep_after_file_read =
        job->addSleepAction("sleep_after_file_read", 10.0);
    auto compute = job->addComputeAction("compute", 10000.0, 100.0, 1, 1,
                                         wrench::ParallelModel::AMDAHL(1.0));
    ...
    auto real_cause = std::dynamic_pointer_cast<wrench::SomeActionsHaveFailed>(
        real_event->failure_cause);
    ...
    if (sleep1->getState() != wrench::Action::State::COMPLETED) {
      throw std::runtime_error("Unexpected state of the sleep1 action: " +
                               sleep1->getStateAsString());
    }
    if (file_read->getState() != wrench::Action::State::FAILED) {
      throw std::runtime_error("Unexpected state of the file_read action: " +
                               file_read->getStateAsString());
    }
    if (sleep_after_file_read->getState() != wrench::Action::State::NOT_READY) {
      throw std::runtime_error(
          "Unexpected state of the sleep_after_file_read action: " +
          sleep_after_file_read->getStateAsString());
    }
    if (compute->getState() != wrench::Action::State::KILLED) {
      throw std::runtime_error("Unexpected state of the compute action: " +
                               compute->getStateAsString());
    }
    if (sleep_after_compute->getState() != wrench::Action::State::NOT_READY) {
      throw std::runtime_error(
          "Unexpected state of the sleep_after_compute action: " +
          sleep_after_compute->getStateAsString());
    }
...
void BareMetalComputeServiceMultiActionTest::do_PartialFailure_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(
      wms = simulation->add(new PartialFailureTestWMS(
...
void BareMetalComputeServiceMultiActionTest::do_PartialTermination_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(
      wms = simulation->add(new PartialTerminationTestWMS(
          this, workflow, {compute_service}, {storage_service1}, hostname)));
