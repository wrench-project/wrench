...
WRENCH_LOG_CATEGORY(
    bare_metal_compute_service_actions_that_communicate_test,
    "Log category for BareMetalComputeServiceActionsThatCommunicateTest test");

class BareMetalComputeServiceActionsThatCommunicateTest
    : public ::testing::Test {
public:
...
class BareMetalTwoCommunicatingActionsTestExecutionController
    : public wrench::ExecutionController {
public:
  BareMetalTwoCommunicatingActionsTestExecutionController(
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to create a zero-sized communicator");
    ...
    auto lambda_execute =
        [communicator](
          ...
          WRENCH_INFO("I am in a communicator with rank %lu/%lu", my_rank,
                      num_procs);
    ...
    auto action1 =
        job->addCustomAction("action1", 0, 0, lambda_execute, lambda_terminate);
    auto action2 =
        job->addCustomAction("action2", 0, 0, lambda_execute, lambda_terminate);
    ...
    if (not std::dynamic_pointer_cast<wrench::CompoundJobCompletedEvent>(
            event)) {
      throw std::runtime_error("Did not receive the expected "
                               "CompoundJobCompletedEvent (instead got " +
                               event->toString() + ")");
    }

    // Inspect actions
    if (action1->getState() != wrench::Action::COMPLETED or
        action1->getFailureCause() != nullptr) {
      throw std::runtime_error("Action 1 should have completed");
    }
    if (action2->getState() != wrench::Action::COMPLETED or
        action1->getFailureCause() != nullptr) {
...
TEST_F(BareMetalComputeServiceActionsThatCommunicateTest,
       TwoCommunicatingTasks) {
  DO_TEST_WITH_FORK(do_TwoCommunicatingActions_test);
}

void BareMetalComputeServiceActionsThatCommunicateTest::
    do_TwoCommunicatingActions_test() {
  ...
  auto controller = simulation->add(
      new BareMetalTwoCommunicatingActionsTestExecutionController(
          this, "Host1", compute_service));
...
class BareMetalMPIAllToAllTestExecutionController
    : public wrench::ExecutionController {
public:
  BareMetalMPIAllToAllTestExecutionController(
      BareMetalComputeServiceActionsThatCommunicateTest *test,
      std::string hostname,
      const std::shared_ptr<wrench::BareMetalComputeService> &compute_service)
      : wrench::ExecutionController(hostname, "test") {
...
private:
  ...
  int main() override {
    ...
    auto action1 =
        job->addCustomAction("action1", 0, 1, lambda_execute, lambda_terminate);
    auto action2 =
        job->addCustomAction("action2", 0, 1, lambda_execute, lambda_terminate);
...
void BareMetalComputeServiceActionsThatCommunicateTest::
  ...
  // Create an Execution Controller
  auto controller =
      simulation->add(new BareMetalMPIAllToAllTestExecutionController(
          this, "Host1", compute_service));
