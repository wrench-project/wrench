...
WRENCH_LOG_CATEGORY(bare_metal_compute_service_multi_job_test,
                    "Log category for BareMetalComputeServiceMultiJob test");
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to add cross-job dependencies");
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to add a cycle when adding a bogus parent!");
    } catch (std::invalid_argument &e) {
    }
    try {
      job4->addChildJob(job1);
      throw std::runtime_error(
          "Shouldn't be able to add a cycle when adding a bogus child!");
    } catch (std::invalid_argument &e) {
    }
...
void BareMetalComputeServiceActionMultiJobTest::do_DAGOfJobs_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new DAGOfJobsTestWMS(this, hostname)));
