...
WRENCH_LOG_CATEGORY(bare_metal_compute_service_one_action_test,
                    "Log category for BareMetalComputeServiceOneAction test");
...
class BareMetalBadSetupTestExecutionController
    : public wrench::ExecutionController {
public:
  BareMetalBadSetupTestExecutionController(
      BareMetalComputeServiceOneActionTest *test, std::string &hostname)
...
void BareMetalComputeServiceOneActionTest::do_BadSetup_test() {
  ...
  ASSERT_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          "Host1", (std::map<std::string, std::tuple<unsigned long, double>>){},
          {})),
  ...
  ASSERT_NO_THROW(
      controller = simulation->add(
          new BareMetalBadSetupTestExecutionController(this, hostname)));
...
void BareMetalComputeServiceOneActionTest::do_Noop_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(
      wms = simulation->add(new BareMetalNoopTestWMS(this, hostname)));
...
public:
  BareMetalOneSleepActionTestWMS(
      BareMetalComputeServiceOneActionTest *test,
      std::shared_ptr<wrench::Workflow> workflow,
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to submit a job with no actions in it");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to lookup by name an action that doesn't exist");
    } catch (std::invalid_argument &ignore) {
    }
...
void BareMetalComputeServiceOneActionTest::do_OneSleepAction_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(
      wms = simulation->add(new BareMetalOneSleepActionTestWMS(
          this, workflow, {compute_service}, {storage_service1}, hostname)));
...
class BareMetalOneComputeActionNotEnoughResourcesTestWMS
    : public wrench::ExecutionController {
public:
  BareMetalOneComputeActionNotEnoughResourcesTestWMS(
      BareMetalComputeServiceOneActionTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    auto action1 =
        job1->addComputeAction("my_computation", 1000.0, 0.0, 100, 100,
    ...
    // Create a compute action that asks for too many cores
    auto action2 =
        job2->addComputeAction("my_computation", 1000.0, 1000.0, 1, 1,
                               wrench::ParallelModel::AMDAHL(1.0));
    ...
    for (auto const &job : jobs) {
      ...
      try {
        job_manager->submitJob(job, this->test->compute_service, {});
        throw std::runtime_error("Shouldn't be able to submit a job that asks "
...
TEST_F(BareMetalComputeServiceOneActionTest,
       OneComputeActionNotEnoughResources) {
  DO_TEST_WITH_FORK(do_OneComputeActionNotEnoughResources_test);
}

void BareMetalComputeServiceOneActionTest::
  ...
  ASSERT_NO_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          ...
          {std::make_pair(
              "Host4",
              std::make_tuple(wrench::ComputeService::ALL_CORES, 100.0))},
          {"/scratch"}, {})));
...
class BareMetalOneComputeActionBogusServiceSpecificArgsTestWMS
    : public wrench::ExecutionController {
public:
  BareMetalOneComputeActionBogusServiceSpecificArgsTestWMS(
      BareMetalComputeServiceOneActionTest *test,
      std::shared_ptr<wrench::Workflow> workflow,
      const std::set<std::shared_ptr<wrench::ComputeService>> &compute_services,
      const std::set<std::shared_ptr<wrench::StorageService>> &storage_services,
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new BareMetalOneComputeActionBogusServiceSpecificArgsTestWMS(
              this, workflow, {compute_service}, {storage_service1},
              hostname)));
...
private:
  ...
  int main() override {
    ...
    if ((action->getStartDate() > 0.0001) or
        (std::abs<double>(action->getEndDate() - 1.0) > 0)) {
...
void BareMetalComputeServiceOneActionTest::
    do_OneSleepActionServiceCrashed_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(wms = simulation->add(
                      new BareMetalServiceCrashedTestWMS(this, hostname)));
...
void BareMetalComputeServiceOneActionTest::do_OneSleepJobTermination_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(wms = simulation->add(
                      new BareMetalJobTerminationTestWMS(this, hostname)));
...
class BareMetalServiceCrashedRestartedTestWMS
    : public wrench::ExecutionController {
public:
  BareMetalServiceCrashedRestartedTestWMS(
      BareMetalComputeServiceOneActionTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    if ((std::abs<double>(action->getStartDate() - 2.0) > 0.0001) or
        (std::abs<double>(action->getEndDate() - 12.0) > 0.0001)) {
      throw std::runtime_error("Unexpected action start/end dates");
    }
    ...
    if ((top.failure_cause != nullptr) or
        ...
        (top.execution_host != "Host4") or (top.num_cores_allocated != 0) or
    ...
    top = history.top();
    if ((top.failure_cause == nullptr) or
        (std::dynamic_pointer_cast<wrench::HostError>(top.failure_cause) ==
         nullptr) or
...
void BareMetalComputeServiceOneActionTest::
    do_OneSleepActionServiceCrashedRestarted_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new BareMetalServiceCrashedRestartedTestWMS(this, hostname)));
...
public:
  BareMetalServiceFileNotThereTestWMS(
      BareMetalComputeServiceOneActionTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
void BareMetalComputeServiceOneActionTest::
    do_OneFileReadActionFileNotThere_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(wms = simulation->add(
                      new BareMetalServiceFileNotThereTestWMS(this, hostname)));
...
void BareMetalComputeServiceOneActionTest::do_OneSleepActionServiceDown_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(wms = simulation->add(
                      new BareMetalServiceServiceDownTestWMS(this, hostname)));
...
class BareMetalServiceServiceSuspendedTestWMS
    : public wrench::ExecutionController {
public:
  BareMetalServiceServiceSuspendedTestWMS(
      BareMetalComputeServiceOneActionTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
void BareMetalComputeServiceOneActionTest::
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new BareMetalServiceServiceSuspendedTestWMS(this, hostname)));
...
void BareMetalComputeServiceOneActionTest::do_OneSleepActionBadScratch_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(wms = simulation->add(
                      new BareMetalServiceBadScratchTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
...

    {
      // Create a compound job
      auto job = job_manager->createCompoundJob("my_job");
      auto action = job->addFileRegistryAddEntryAction(
          "add_entry", this->test->file_registry_service,
          wrench::FileLocation::LOCATION(this->test->storage_service1,
                                         this->test->input_file));
...

    {
      // Create a compound job
      auto job = job_manager->createCompoundJob("my_job");
      auto action = job->addFileRegistryDeleteEntryAction(
          "delete_entry", this->test->file_registry_service,
          wrench::FileLocation::LOCATION(this->test->storage_service1,
                                         this->test->input_file));
...
void BareMetalComputeServiceOneActionTest::do_FileRegistryActions_test() {
  ...
  std::string hostname = "Host1";
  ASSERT_NO_THROW(
      wms = simulation->add(new FileRegistryActionTestWMS(this, hostname)));
