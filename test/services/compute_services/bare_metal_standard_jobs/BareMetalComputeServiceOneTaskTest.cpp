...
WRENCH_LOG_CATEGORY(bare_metal_compute_service_test,
                    "Log category for BareMetalComputeServiceOneTask test");
...
public:
  ...
  void do_ExecutionButDontWait_test();
...
protected:
  ...
  BareMetalComputeServiceOneTaskTest() {
    ...
    std::string xml =
        ...
        "          </disk>"
        "          <disk id=\"large_disk3\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"30000B\"/>"
        "             <prop id=\"mount\" value=\"/disk3\"/>"
        "          </disk>"
        "          <disk id=\"scratch_disk\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        ...
        "       <route src=\"TwoCoreHost\" dst=\"OneCoreHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "       <route src=\"RAMHost\" dst=\"OneCoreHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "   </zone> "
        "</platform>";
...
void BareMetalComputeServiceOneTaskTest::do_BadSetup_test() {
  ...
  ASSERT_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          "bogus", (std::map<std::string, std::tuple<unsigned long, double>>){},
          {})),
  ...
  ASSERT_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          hostname,
          {std::make_pair(
              hostname, std::make_tuple(100, wrench::ComputeService::ALL_RAM))},
          {})),
      std::invalid_argument);
  ...
  ASSERT_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          ...
          {std::make_pair(
              "RAMHost",
              std::make_tuple(wrench::ComputeService::ALL_CORES, -1.0))},
          {})),
      std::invalid_argument);
  ...
  ASSERT_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          ...
          {std::make_pair(
              "RAMHost",
              std::make_tuple(wrench::ComputeService::ALL_CORES, 100000.0))},
          {})),
      std::invalid_argument);
...
public:
  NoopTestWMS(BareMetalComputeServiceOneTaskTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    wrench::TerminalOutput::disableColor(); // just for increasing stupid
                                            // coverage
...
void BareMetalComputeServiceOneTaskTest::do_Noop_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new NoopTestWMS(this, hostname)));
...
public:
  StandardJobConstructorTestWMS(BareMetalComputeServiceOneTaskTest *test,
                                std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to create a job with an empty task1");
          ...
          {{test->input_file, wrench::FileLocation::LOCATION(
           ...
           {test->output_file,
            wrench::FileLocation::LOCATION(test->storage_service1,
                                           test->output_file)}});
      throw std::runtime_error(
          "Should not be able to create a job with an empty task1 vector");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          (std::vector<std::shared_ptr<wrench::WorkflowTask>>){nullptr,
                                                               nullptr},
          {{test->input_file, wrench::FileLocation::LOCATION(
                                  test->storage_service1, test->input_file)},
           {test->output_file,
            wrench::FileLocation::LOCATION(test->storage_service1,
                                           test->output_file)}});
      throw std::runtime_error(
          "Should not be able to create a job with a vector of empty tasks");
    } catch (std::invalid_argument &e) {
    }

    // Create another task1
    std::shared_ptr<wrench::WorkflowTask> task_big =
        this->test->workflow->addTask("task2", 3600, 2, 2, 2048);
    ...
    try {
      ...
      job = job_manager->createStandardJob(
          ...
          {{nullptr, wrench::FileLocation::LOCATION(test->storage_service1,
                                                    test->output_file)}});
      throw std::runtime_error("Should not be able to create a job with an "
                               "nullptr file in file locations");
    } catch (std::invalid_argument &e) {
    }
          ...
          {{test->input_file, nullptr},
           {test->output_file,
            wrench::FileLocation::LOCATION(test->storage_service1,
                                           test->output_file)}});
      throw std::runtime_error("Should not be able to create a job with an "
                               "nullptr storage service in file locations");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          ...
          {std::make_tuple(
              wrench::FileLocation::LOCATION(test->storage_service1, nullptr),
              wrench::FileLocation::LOCATION(test->storage_service2, nullptr))},
          {}, {});
      throw std::runtime_error("Should not be able to create a job with a "
                               "nullptr file in pre file copies");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          ...
          {std::make_tuple(nullptr, wrench::FileLocation::LOCATION(
                                        test->storage_service2, nullptr))},
          {}, {});
      throw std::runtime_error(
          "Should not be able to create a job with a nullptr src storage "
          "service in pre file copies");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          ...
          {{test->input_file, wrench::FileLocation::LOCATION(
           ...
           {test->output_file, wrench::FileLocation::LOCATION(
                                   test->storage_service1, test->output_file)}},
          {std::make_tuple(wrench::FileLocation::LOCATION(
                               test->storage_service1, test->output_file),
                           nullptr)},
          {}, {});
      throw std::runtime_error(
          "Should not be able to create a job with a nullptr dst storage "
          "service in pre file copies");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          ...
          {});
      throw std::runtime_error("Should not be able to create a job with a "
                               "nullptr file in post file copies");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          ...
          {},
          {std::make_tuple(
              nullptr, wrench::FileLocation::LOCATION(test->storage_service2,
                                                      test->output_file))},
          {});
      throw std::runtime_error(
          "Should not be able to create a job with a nullptr src storage "
          "service in post file copies");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          ...
          {}, {},
          {wrench::FileLocation::LOCATION(test->storage_service1, nullptr)});
      throw std::invalid_argument("Should not be able to create a job with a "
                                  "nullptr file in file deletions");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      job = job_manager->createStandardJob(
          ...
          {}, {}, {nullptr});
      throw std::invalid_argument("Should not be able to create a job with a "
                                  "nullptr storage service in file deletions");
    } catch (std::invalid_argument &e) {
    }
...
void BareMetalComputeServiceOneTaskTest::do_StandardJobConstructor_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms =
      simulation->add(new StandardJobConstructorTestWMS(this, hostname1));
...
private:
  ...
  int main() override {
    ...
    if (ram_capacity != wrench::S4U_Simulation::DEFAULT_RAM) {
      throw std::runtime_error(
          "RAM Capacity of TwoCoreHost should be the default");
    }
    ...
    if (ram_capacity == wrench::S4U_Simulation::DEFAULT_RAM) {
      throw std::runtime_error(
          "RAM Capacity of RAMHost should not be the default");
    }
...
void BareMetalComputeServiceOneTaskTest::do_HostMemory_test() {
  ...
  // Create a WMS
  auto wms = simulation->add(new HostMemoryTestWMS(this, "TwoCoreHost"));
...
private:
  ...
  int main() override {
    ...
    auto job = job_manager->createStandardJob(
        test->task, {{test->input_file,
                      wrench::FileLocation::LOCATION(
                          test->storage_service1, "/disk1", test->input_file)},
                     {test->output_file, wrench::FileLocation::LOCATION(
                                             test->storage_service1, "/disk1",
                                             test->output_file)}});

    //        std::cerr << "TOTAL SPACE = " <<
    //        this->test->storage_service1->getTotalSpace() << "\n"; std::cerr
    //        << "FREE SPACE = " << this->test->storage_service1->getFreeSpace()
    //        << "\n";
    // Submit the job
    ...
    try {
      wrench::StorageService::lookupFileAtLocation(
          wrench::FileLocation::LOCATION(this->test->storage_service1,
                                         nullptr));
      throw std::runtime_error(
          "Should not have been able to lookup a nullptr file");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      wrench::StorageService::lookupFileAtLocation(
          wrench::FileLocation::LOCATION(nullptr, this->test->output_file));
      throw std::runtime_error(
          "Should not have been able to lookup a nullptr storage service");
    } catch (std::invalid_argument &e) {
    }
...
void BareMetalComputeServiceOneTaskTest::do_ExecutionWithLocationMap_test() {
  ...
  ASSERT_GE(task->getStartDate(), 0.0);
  ...
  std::vector<
      wrench::SimulationTimestamp<wrench::SimulationTimestampTaskCompletion> *>
      task_completion_trace =
          simulation->getOutput()
              .getTrace<wrench::SimulationTimestampTaskCompletion>();
  ASSERT_EQ(simulation->getOutput()
                .getTrace<wrench::SimulationTimestampTaskCompletion>()
                .size(),
            1);
  ASSERT_LT(
      std::abs(simulation->getOutput()
                   .getTrace<wrench::SimulationTimestampTaskCompletion>()[0]
                   ->getContent()
                   ->getDate() -
               task->getEndDate()),
      0.01);
...
class ExecutionWithLocationMapMultipleTestWMS
    : public wrench::ExecutionController {
public:
  ExecutionWithLocationMapMultipleTestWMS(
      BareMetalComputeServiceOneTaskTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    file_locations[test->input_file].push_back(wrench::FileLocation::LOCATION(
        test->storage_service2, "/disk2", test->input_file));
    file_locations[test->input_file].push_back(wrench::FileLocation::LOCATION(
        test->storage_service1, "/disk1", test->input_file));
    file_locations[test->output_file] = {};
    file_locations[test->output_file].push_back(wrench::FileLocation::LOCATION(
        test->storage_service1, "/disk1", test->output_file));

    try {
      auto job = job_manager->createStandardJob(nullptr, file_locations);
      throw std::runtime_error(
          "Shouldn't be able to create a job with a null task");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not have been able to lookup a nullptr location");
    } catch (std::invalid_argument &e) {
    }
...
void BareMetalComputeServiceOneTaskTest::
    do_ExecutionWithLocationMapMultiple_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  auto **argv = (char **)calloc(argc, sizeof(char *));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new ExecutionWithLocationMapMultipleTestWMS(this, hostname)));
...
class ExecutionWithDefaultStorageServiceTestWMS
    : public wrench::ExecutionController {
public:
  ExecutionWithDefaultStorageServiceTestWMS(
      BareMetalComputeServiceOneTaskTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
TEST_F(BareMetalComputeServiceOneTaskTest,
       DISABLED_ExecutionWithDefaultStorageService) {
  DO_TEST_WITH_FORK(do_ExecutionWithDefaultStorageService_test);
}

void BareMetalComputeServiceOneTaskTest::
    do_ExecutionWithDefaultStorageService_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
...
class ExecutionWithPrePostCopiesAndCleanupTestWMS
    : public wrench::ExecutionController {
public:
  ...
  int main() override {
    ...
    auto job = job_manager->createStandardJob(
        ...
        {{test->input_file, wrench::FileLocation::LOCATION(
                                test->storage_service1, test->input_file)},
         {test->output_file, wrench::FileLocation::LOCATION(
                                 test->storage_service2, test->output_file)}},
        {std::make_tuple(wrench::FileLocation::LOCATION(test->storage_service1,
                                                        test->input_file),
                         wrench::FileLocation::LOCATION(test->storage_service2,
                                                        test->input_file))},
        {std::make_tuple(wrench::FileLocation::LOCATION(test->storage_service2,
                                                        test->output_file),
                         wrench::FileLocation::LOCATION(test->storage_service1,
                                                        test->output_file))},
        {wrench::FileLocation::LOCATION(test->storage_service2,
    ...
    if (std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(event)) {
      // do nothing
    } else if (auto real_event =
                   std::dynamic_pointer_cast<wrench::StandardJobFailedEvent>(
                       event)) {
    ...
    if (wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service2,
                                           this->test->input_file))) {
      throw std::runtime_error(
          "Input file should not be on Storage Service #2");
    }
    if (wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service2,
                                           this->test->output_file))) {
      throw std::runtime_error(
          "Output file should not be on Storage Service #2");
    }
...
TEST_F(BareMetalComputeServiceOneTaskTest,
       ExecutionWithPrePostCopiesTaskCleanup) {
  DO_TEST_WITH_FORK(do_ExecutionWithPrePostCopiesTaskCleanup_test)
}

void BareMetalComputeServiceOneTaskTest::
    do_ExecutionWithPrePostCopiesTaskCleanup_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  char **argv = (char **)calloc(argc, sizeof(char *));
...
class ExecutionWithPrePostCopiesNoTaskNoCleanupTestWMS
    : public wrench::ExecutionController {
public:
...
TEST_F(BareMetalComputeServiceOneTaskTest,
       ExecutionWithPrePostCopiesNoTaskNoCleanup) {
  DO_TEST_WITH_FORK(do_ExecutionWithPrePostCopiesNoTaskNoCleanup_test)
}

void BareMetalComputeServiceOneTaskTest::
    do_ExecutionWithPrePostCopiesNoTaskNoCleanup_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  char **argv = (char **)calloc(argc, sizeof(char *));
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new ExecutionWithPrePostCopiesNoTaskNoCleanupTestWMS(
                          this, hostname)));
...
class ExecutionWithPreNoPostCopiesNoTaskCleanupTestWMS
    : public wrench::ExecutionController {
public:
  ExecutionWithPreNoPostCopiesNoTaskCleanupTestWMS(
      BareMetalComputeServiceOneTaskTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
TEST_F(BareMetalComputeServiceOneTaskTest,
       ExecutionWithPreNoPostCopiesNoTaskCleanup) {
  DO_TEST_WITH_FORK(do_ExecutionWithPreNoPostCopiesNoTaskCleanup_test)
}

void BareMetalComputeServiceOneTaskTest::
    do_ExecutionWithPreNoPostCopiesNoTaskCleanup_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  char **argv = (char **)calloc(argc, sizeof(char *));
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new ExecutionWithPreNoPostCopiesNoTaskCleanupTestWMS(
                          this, hostname)));
...
private:
  ...
  int main() override {
    ...
    std::map<std::shared_ptr<wrench::DataFile>,
             std::shared_ptr<wrench::FileLocation>>
        file_locations;
    file_locations[test->input_file] = wrench::FileLocation::LOCATION(
        test->storage_service2, test->input_file);
    file_locations[test->output_file] = wrench::FileLocation::LOCATION(
        test->storage_service2, test->output_file);
    auto job = job_manager->createStandardJob(test->task, file_locations);
    job_manager->submitJob(job, test->compute_service);
    ...
    if (real_event) {
      ...
      if (not cause) {
        throw std::runtime_error("Got a Standard Job Failure as expected, but "
      ...
      if (cause->getFile() != test->input_file) {
        throw std::runtime_error("Got the expected failure, but the failure "
...
void BareMetalComputeServiceOneTaskTest::do_ExecutionWithMissingFile_test() {
  ...
  ASSERT_DOUBLE_EQ(this->task->getFailureDate(), 1.0);
...
private:
  ...
  int main() override {
    ...
    try {
      job_manager->submitJob(job, test->compute_service,
                             {{"task2", "OneCoreHost:2"}});
      throw std::runtime_error("Should not be able to submit a job to a "
                               "service without enough cores");
    } catch (wrench::ExecutionException &e) {
      auto cause =
          std::dynamic_pointer_cast<wrench::NotEnoughResources>(e.getCause());
      if (not cause) {
        throw std::runtime_error(
            "Received the expected exception, but unexpected failure cause: " +
            e.getCause()->toString() + " (expected: NotEnoughResources)");
      }
      std::string error_msg = cause->toString();
      if (cause->getJob() != job) {
        throw std::runtime_error("Got the expected failure, but the failure "
                                 "cause does not point to the right job");
      }
      if (cause->getService() != test->compute_service) {
        throw std::runtime_error(
            "Got the expected failure, but the failure cause does not point to "
...
private:
  ...
  int main() override {
    ...
    try {
      job_manager->submitJob(job, test->compute_service);
      throw std::runtime_error(
          "Should not be able to submit a job to a service without enough RAM");
    } catch (wrench::ExecutionException &e) {
      auto cause =
          std::dynamic_pointer_cast<wrench::NotEnoughResources>(e.getCause());
      if (not cause) {
...
private:
  ...
  int main() override {
    ...
    try {
      job_manager->submitJob(job, test->compute_service);
      throw std::runtime_error(
          "Should not be able to submit a job to a service that is down");
    } catch (wrench::ExecutionException &e) {
      auto cause =
          std::dynamic_pointer_cast<wrench::ServiceIsDown>(e.getCause());
      if (not cause) {
        throw std::runtime_error(
            "Got the expected exception, but an expected failure cause: " +
...
void BareMetalComputeServiceOneTaskTest::do_ExecutionWithDownService_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new ExecutionWithDownServiceTestWMS(this, hostname)));
...
class ExecutionWithSuspendedServiceTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
    ...
    // Suspend the service
    wrench::Simulation::sleep(1);
    WRENCH_INFO("SUSPENDING THE SERVICE");
    ...
    try {
      job_manager->submitJob(job, test->compute_service);
      throw std::runtime_error(
          "Should not be able to submit a job to a service that is suspended");
    } catch (wrench::ExecutionException &e) {
      auto cause =
    ...
    // Resume the service
    WRENCH_INFO("RESUMING THE SERVICE");
    test->compute_service->resume();
    WRENCH_INFO("RESUMED THE SERVICE");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should  be able to submit a job to a service that has been resumed");
    }
...
void BareMetalComputeServiceOneTaskTest::
    do_ExecutionWithSuspendedService_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  auto **argv = (char **)calloc(argc, sizeof(char *));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new ExecutionWithSuspendedServiceTestWMS(this, hostname)));
...
TEST_F(BareMetalComputeServiceOneTaskTest, ExecutionButDontWait) {
  DO_TEST_WITH_FORK(do_ExecutionButDontWait_test);
}

void BareMetalComputeServiceOneTaskTest::do_ExecutionButDontWait_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  auto **argv = (char **)calloc(argc, sizeof(char *));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new ExecutionButDontWaitTestWMS(this, hostname)));
