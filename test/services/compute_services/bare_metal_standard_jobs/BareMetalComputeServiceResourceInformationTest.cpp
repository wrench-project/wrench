...
protected:
  ...
  BareMetalComputeServiceTestResourceInformation() {
    ...
    std::string xml =
        ...
        "</route>"
        "       <route src=\"Host1\" dst=\"Host4\"> <link_ctn id=\"1\"/> "
        "</route>"
        "   </zone> "
        "</platform>";
  ...
  int main() override {
    ...
    if (num_hosts != 2) {
      throw std::runtime_error(
          "getNumHosts() should return 2 for compute service #1");
    }

    num_hosts = this->test->compute_service2->getNumHosts();
    if (num_hosts != 2) {
      throw std::runtime_error(
          "getNumHosts() should return 2 for compute service #2");
    }
    ...
    if (host_list.size() != 2) {
      throw std::runtime_error("getHosts() should return a list with 2 items "
                               "for compute service #1");
    }
    if (std::find(host_list.begin(), host_list.end(), "Host1") ==
        host_list.end()) {
      throw std::runtime_error("getHosts() should return a list that contains "
                               "'Host1' for compute service #1");
    }
    if (std::find(host_list.begin(), host_list.end(), "Host2") ==
        host_list.end()) {
      throw std::runtime_error("getHosts() should return a list that contains "
                               "'Host2' for compute service #1");
    }
    ...
    if ((num_cores.size() != 2) or ((*(num_cores.begin())).second != 4) or
        ((*(++num_cores.begin())).second != 4)) {
      throw std::runtime_error(
          "getHostNumCores() should return {4,4} for compute service #1");
    }

    num_cores = this->test->compute_service2->getPerHostNumCores();
    if ((num_cores.size() != 2) or ((*(num_cores.begin())).second != 8) or
        ((*(++num_cores.begin())).second != 8)) {
      throw std::runtime_error(
          "getHostNumCores() should return {8,8} for compute service #1");
    }

    if (this->test->compute_service1->getTotalNumCores() != 8) {
      throw std::runtime_error(
          "getHostTotalNumCores() should return 8 for compute service #1");
    }

    if (this->test->compute_service2->getTotalNumCores() != 16) {
      throw std::runtime_error(
          "getHostTotalNumCores() should return 16 for compute service #2");
    }
    ...
    if ((sorted_ram_capacities.size() != 2) or
      ...
      throw std::runtime_error(
          "getHostMemoryCapacity() should return {1024,2048} or {2048,1024} "
          "for compute service #1");
    }

    // Get Core flop rates
    std::map<std::string, double> core_flop_rates =
        this->test->compute_service1->getCoreFlopRate();
    ...
    if ((sorted_core_flop_rates.size() != 2) or
      ...
      throw std::runtime_error("getCoreFlopRate() should return {1,10} or "
                               "{10,1} for compute service #1");
    }

    // Create a job that will use cores on compute service #1
    std::shared_ptr<wrench::WorkflowTask> t1 =
        this->test->workflow->addTask("task1", 60.0000, 3, 3, 0);
    std::shared_ptr<wrench::WorkflowTask> t2 =
        this->test->workflow->addTask("task2", 60000000000.0001, 2, 2, 0);
    ...
    job_manager->submitJob(job, this->test->compute_service1,
                           {{"task1", "Host1:3"}, {"task2", "Host2:2"}});

    wrench::Simulation::sleep(1.0);

    // Get number of idle cores
    std::map<std::string, unsigned long> num_idle_cores =
        this->test->compute_service1->getPerHostNumIdleCores();
    ...
    if ((idle_core_counts.size() != 2) or (idle_core_counts[0] != 1) or
        (idle_core_counts[1] != 2)) {
      throw std::runtime_error("getPerHostNumIdleCores() should return {1,2} "
                               "or {2,1} for compute service #1");
    }

    if (this->test->compute_service1->getTotalNumIdleCores() != 3) {
      throw std::runtime_error(
          "getTotalNumIdleCores() should return 3 for compute service #1");
    }

    this->test->compute_service1
        ->getPerHostAvailableMemoryCapacity(); // coverage
...
void BareMetalComputeServiceTestResourceInformation::
    do_ResourceInformation_test() {
  ...
  ASSERT_NO_THROW(
      compute_service1 = simulation->add(new wrench::BareMetalComputeService(
          "Host1",
          {{std::make_pair(
               "Host1", std::make_tuple(4, wrench::ComputeService::ALL_RAM))},
           {std::make_pair(
               "Host2", std::make_tuple(4, wrench::ComputeService::ALL_RAM))}},
          "", {})));
  ...
  ASSERT_NO_THROW(
      compute_service2 = simulation->add(new wrench::BareMetalComputeService(
          "Host1",
          {{std::make_pair(
               "Host3", std::make_tuple(8, wrench::ComputeService::ALL_RAM))},
           {std::make_pair(
               "Host4", std::make_tuple(8, wrench::ComputeService::ALL_RAM))}},
          "", {})));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new ResourceInformationTestWMS(this, "Host1")));
