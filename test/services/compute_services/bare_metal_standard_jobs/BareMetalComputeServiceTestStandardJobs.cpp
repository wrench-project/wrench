...
protected:
  ...
  BareMetalComputeServiceTestStandardJobs() {
    ...
    std::string xml =
        ...
        "       <route src=\"DualCoreHost\" dst=\"QuadCoreHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "   </zone> "
        "</platform>";
...
class MulticoreComputeServiceUnsupportedJobTypeTestWMS
    : public wrench::ExecutionController {

public:
  MulticoreComputeServiceUnsupportedJobTypeTestWMS(
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to submit a pilot job to a "
                               "compute service that does not support them");
...
void BareMetalComputeServiceTestStandardJobs::
    do_UnsupportedStandardJobs_test() {
...
class MulticoreComputeServiceBogusNumCoresTestWMS
    : public wrench::ExecutionController {

public:
  MulticoreComputeServiceBogusNumCoresTestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    std::map<std::shared_ptr<wrench::DataFile>,
        ...
        file_locations;
    file_locations[this->test->input_file] =
        wrench::FileLocation::SCRATCH(this->test->input_file);
    file_locations[this->test->output_file3] =
        wrench::FileLocation::SCRATCH(this->test->output_file3);
    auto two_core_task_job = job_manager->createStandardJob(
        {this->test->task3}, file_locations,
        {std::make_tuple(
    ...
    try {
      job_manager->submitJob(two_core_task_job, this->test->compute_service,
                             {{"task_3_10s_2cores", "1"}});
      throw std::runtime_error("Should not be able to submit a job asking for "
                               "1 core for a 2-core tasks");
...
void BareMetalComputeServiceTestStandardJobs::do_BogusNumCores_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new MulticoreComputeServiceBogusNumCoresTestWMS(this, hostname)));
...
class MulticoreComputeServiceTwoSingleCoreTasksTestWMS
    : public wrench::ExecutionController {

public:
  MulticoreComputeServiceTwoSingleCoreTasksTestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    file_locations[this->test->input_file] =
        wrench::FileLocation::SCRATCH(this->test->input_file);
    file_locations[this->test->output_file1] =
        wrench::FileLocation::SCRATCH(this->test->output_file1);
    file_locations[this->test->output_file2] =
        wrench::FileLocation::SCRATCH(this->test->output_file2);

    auto two_task_job = job_manager->createStandardJob(
        {this->test->task1, this->test->task2}, file_locations,
        {std::make_tuple(
            wrench::FileLocation::LOCATION(this->test->storage_service,
                                           this->test->input_file),
            wrench::FileLocation::SCRATCH(this->test->input_file))},
    ...
    if (two_task_job->getMinimumRequiredMemory() !=
        std::max<double>(this->test->task1->getMemoryRequirement(),
                         this->test->task2->getMemoryRequirement())) {
    ...
    if (delta > 0.1) {
      throw std::runtime_error("Task completion times should be about 0.0 "
                               "seconds apart but they are " +
    ...
    double work_time =
        this->test->task1->getEndDate() - this->test->task1->getStartDate();
    if ((now - job_start) - work_time - 2 * post_pre_overhead > 0.1) {
      throw std::runtime_error(
          "Something doesn't add up given the pre- and post-job overhead");
    }
...
class MulticoreComputeServiceTwoDualCoreTasksCase1TestWMS
    : public wrench::ExecutionController {

public:
  MulticoreComputeServiceTwoDualCoreTasksCase1TestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    file_locations[this->test->output_file3] =
        wrench::FileLocation::SCRATCH(this->test->output_file3);
    file_locations[this->test->output_file4] =
        wrench::FileLocation::SCRATCH(this->test->output_file4);
    auto two_task_job = job_manager->createStandardJob(
        {this->test->task3, this->test->task4}, file_locations,
        {std::make_tuple(
            wrench::FileLocation::LOCATION(this->test->storage_service,
                                           this->test->input_file),
            wrench::FileLocation::SCRATCH(this->test->input_file))},
    ...
    // Check completion states and times
    RUNTIME_EQ(this->test->task3->getState(), wrench::WorkflowTask::COMPLETED,
               "task 3 state");
    RUNTIME_EQ(this->test->task4->getState(), wrench::WorkflowTask::COMPLETED,
               "task 4 state");

    RUNTIME_DBL_EQ(std::min<double>(this->test->task3->getEndDate(),
                                    this->test->task4->getEndDate()),
                   5.0, "earliest task completion date", EPSILON);
    RUNTIME_DBL_EQ(std::max<double>(this->test->task3->getEndDate(),
                                    this->test->task4->getEndDate()),
                   10.0, "latest task completion date", EPSILON);
...
class MulticoreComputeServiceTwoDualCoreTasksCase2TestWMS
    : public wrench::ExecutionController {

public:
  MulticoreComputeServiceTwoDualCoreTasksCase2TestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    // Create a 2-task1 job
    auto two_task_job = job_manager->createStandardJob(
        {this->test->task5, this->test->task6}, file_locations,
        {std::make_tuple(
            wrench::FileLocation::LOCATION(this->test->storage_service,
                                           this->test->input_file),
            wrench::FileLocation::SCRATCH(this->test->input_file))},
    ...
    //      task5 = workflow->addTask("task_5_30s_1_to_3_cores", 30.0, 1,
    //      3, 1.0, 0); task6 =
    //      workflow->addTask("task_6_10s_1_to_2_cores", 12.0, 1, 2, 1.0, 0);
    //
    if (delta_task5 > EPSILON) {
      throw std::runtime_error("Unexpected task5 end date " +
                               std::to_string(task5_end_date) +
                               " (should be 10.0)");
    }

    if (delta_task6 > EPSILON) {
      throw std::runtime_error("Unexpected task6 end date " +
                               std::to_string(task6_end_date) +
                               " (should be 12.0)");
    }
...
class BareMetalComputeServiceTwoDualCoreTasksCase3TestWMS
    : public wrench::ExecutionController {

public:
  BareMetalComputeServiceTwoDualCoreTasksCase3TestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    // Create a 2-task1 job
    auto two_task_job_1 = job_manager->createStandardJob(
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to use wrongly formatted "
    ...
    try {
      job_manager->submitJob(
          two_task_job_1, this->test->compute_service,
    ...
    // Submit the 2-task1 job for execution
    // service-specific args format testing: "hostname:num_cores", and
    // "num_cores" both tasks should run in parallel, using 2 of the 4 cores
    // each
    job_manager->submitJob(two_task_job_1, this->test->compute_service,
                           {{"task_6_10s_1_to_2_cores", "QuadCoreHost:2"},
                            {"task_5_30s_1_to_3_cores", "2"}});
    ...
    if (this->test->task5->getNumCoresAllocated() != 2) {
      throw std::runtime_error(
          "It looks like task5 didn't run with 2 cores according to in-task1 "
          "info (" +
          std::to_string(this->test->task5->getNumCoresAllocated()) + ")");
    }
    if (this->test->task6->getNumCoresAllocated() != 2) {
      throw std::runtime_error("It looks like task6 didn't run with 2 cores "
                               "according to in-task1 info");
    }
    ...
    // service-specific args format testing: "hostname", "" <- that's an empty
    // string both tasks should run in parallel, use 4 cores each, thus
    // oversubscribing
    auto two_task_job_2 = job_manager->createStandardJob(
        {this->test->task7, this->test->task8},
        (std::map<std::shared_ptr<wrench::DataFile>,
                  std::shared_ptr<wrench::FileLocation>>){},
        {std::make_tuple(
            wrench::FileLocation::LOCATION(this->test->storage_service,
    ...
    job_manager->submitJob(two_task_job_2, this->test->compute_service,
                           {{"task_7_8s_1_to_4_cores", "QuadCoreHost"},
                            {"task_8_8s_2_to_4_cores", ""}});
    ...
    if (std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(event)) {
      two_task_job_2_completion_date =
    ...
    if (this->test->task7->getNumCoresAllocated() != 4) {
      throw std::runtime_error("It looks like task7 didn't run with 4 cores "
                               "according to in-task1 info");
    }
    if (this->test->task8->getNumCoresAllocated() != 4) {
      throw std::runtime_error("It looks like task8 didn't run with 4 cores "
                               "according to in-task1 info");
    }

    // the standard job is expected take about 4 seconds, since each task1 would
    // run for 2 seconds if the compute host wasn't oversubscribed
    double two_task_job_2_duration =
        two_task_job_2_completion_date - two_task_job_2->getSubmitDate();
    if (two_task_job_2_duration < (4.0 - EPSILON) ||
        two_task_job_2_duration > (4.0 + EPSILON)) {
      throw std::runtime_error(
          "two_task_job_2 should have taken about 4 seconds, but did not (" +
          std::to_string(two_task_job_2_duration) + ")");
    }
...
class BareMetalComputeServiceJobImmediateTerminationTestWMS
    : public wrench::ExecutionController {

public:
  BareMetalComputeServiceJobImmediateTerminationTestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error("Unexpected exception while terminating job: " +
                               std::string(e.what()));
    }
    ...
    if (two_task_job->getState() != wrench::StandardJob::TERMINATED) {
      throw std::runtime_error(
          "Terminated Standard Job is not in TERMINATED state");
    }
    ...
    if ((this->test->task1->getState() != wrench::WorkflowTask::READY) ||
      ...
      throw std::runtime_error(
          "Tasks in a TERMINATED job should be in the READY state but instead "
          "(" +
...
void BareMetalComputeServiceTestStandardJobs::
    do_JobImmediateTermination_test() {
...
class BareMetalComputeServiceJobTerminationTestWMS
    : public wrench::ExecutionController {

public:
  BareMetalComputeServiceJobTerminationTestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
void BareMetalComputeServiceTestStandardJobs::do_JobTermination_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new BareMetalComputeServiceJobTerminationTestWMS(this, hostname)));
...
class BareMetalComputeServiceNonSubmittedJobTerminationTestWMS
    : public wrench::ExecutionController {

public:
  BareMetalComputeServiceNonSubmittedJobTerminationTestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    // Create a 2-task1 job
    auto two_task_job =
        job_manager->createStandardJob({this->test->task1, this->test->task2});

    // Try to terminate it now (which is stupid)
    try {
      job_manager->terminateJob(two_task_job);
      throw std::runtime_error("Trying to terminate a non-submitted job should "
      ...
      if (cause->getService() != nullptr) {
        throw std::runtime_error(
            "Got the expected exception and failure cause, but the failure "
            "cause does not point to the right service (should be nullptr)");
      }
      cause->toString();
    }
...
void BareMetalComputeServiceTestStandardJobs::
    do_NonSubmittedJobTermination_test() {
...
class BareMetalComputeServiceCompletedJobTerminationTestWMS
    : public wrench::ExecutionController {

public:
  BareMetalComputeServiceCompletedJobTerminationTestWMS(
      BareMetalComputeServiceTestStandardJobs *test,
      const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
void BareMetalComputeServiceTestStandardJobs::
    do_CompletedJobTermination_test() {
...
class BareMetalComputeServiceShutdownComputeServiceWhileJobIsRunningTestWMS
    : public wrench::ExecutionController {
...
private:
  ...
  int main() override {
    ...
    this->test->compute_service->stop(
        true, wrench::ComputeService::TerminationCause::
    ...
    if (real_event) {
      auto cause = std::dynamic_pointer_cast<wrench::ServiceIsDown>(
          real_event->failure_cause);
      if (not cause) {
        throw std::runtime_error(
            "Got a job failure event, but an unexpected failure cause: " +
            real_event->failure_cause->toString() +
            " (expected: ServiceIsDown)");
      }
      if (cause->getService() != this->test->compute_service) {
        throw std::runtime_error(
            "Got the correct failure even, a correct cause type, but the cause "
...
TEST_F(BareMetalComputeServiceTestStandardJobs,
       ShutdownComputeServiceWhileJobIsRunning) {
  DO_TEST_WITH_FORK(do_ShutdownComputeServiceWhileJobIsRunning_test);
}

void BareMetalComputeServiceTestStandardJobs::
    do_ShutdownComputeServiceWhileJobIsRunning_test() {
...
class BareMetalComputeServiceShutdownStorageServiceBeforeJobIsSubmittedTestWMS
    : public wrench::ExecutionController {
...
private:
  ...
  int main() override {
    ...
    auto real_event =
        std::dynamic_pointer_cast<wrench::StandardJobFailedEvent>(event);
    if (real_event) {
      auto cause = std::dynamic_pointer_cast<wrench::InvalidDirectoryPath>(
          real_event->failure_cause);
      if (not cause) {
        throw std::runtime_error(
            "Got the expected job failure but unexpected failure cause: " +
            real_event->failure_cause->toString() +
            " (expected: InvalidDirectoryPath)");
      }
      if (cause->getLocation()->getStorageService() !=
          this->test->compute_service->getScratch()) {
        throw std::runtime_error(
            "Got the correct failure even, a correct cause type, but the cause "
            "points to the wrong storage service");
      }
...
TEST_F(BareMetalComputeServiceTestStandardJobs,
       ShutdownStorageServiceBeforeJobIsSubmitted) {
  //    DO_TEST_WITH_FORK_ONE_ARG(do_ShutdownStorageServiceBeforeJobIsSubmitted_test,
  //    100000);
  DO_TEST_WITH_FORK_ONE_ARG(do_ShutdownStorageServiceBeforeJobIsSubmitted_test,
                            0);
}
...
void BareMetalComputeServiceTestStandardJobs::
    do_ShutdownStorageServiceBeforeJobIsSubmitted_test(double buffer_size) {
