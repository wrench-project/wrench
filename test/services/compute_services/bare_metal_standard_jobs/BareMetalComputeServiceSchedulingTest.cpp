...
WRENCH_LOG_CATEGORY(test_bare_metalk_compute_service_scheduling,
                    "Log category for BareMetalComputeServiceTestScheduling");
...
public:
  ...
  // Old Default
  std::shared_ptr<wrench::ComputeService>
      cs_fcfs_aggressive_maximum_maximum_flops_best_fit = nullptr;
  // "minimum" core allocation
  std::shared_ptr<wrench::ComputeService>
      cs_fcfs_aggressive_minimum_maximum_flops_best_fit = nullptr;
  // "maximum_minimum_cores" task1 selection
  std::shared_ptr<wrench::ComputeService>
      cs_fcfs_aggressive_maximum_maximum_minimum_cores_best_fit = nullptr;
...
protected:
  ...
  BareMetalComputeServiceTestScheduling() {
    ...
    std::string xml =
        ...
        "</route>"
        "       <route src=\"Host1\" dst=\"Host3\"> <link_ctn id=\"1\"/> "
        "</route>"
        "       <route src=\"Host2\" dst=\"Host3\"> <link_ctn id=\"1\"/> "
        "</route>"
        "   </zone> "
        "</platform>";
...
private:
  ...
  int main() override {
    ...
    auto ram_availabilities =
        this->test->cs->getPerHostAvailableMemoryCapacity();
    if ((not BareMetalComputeServiceTestScheduling::isAboutTheSame(
            ram_availabilities["Host1"], 500)) ||
        (not BareMetalComputeServiceTestScheduling::isAboutTheSame(
            ram_availabilities["Host2"], 1000))) {
      throw std::runtime_error(
          "Unexpected memory_manager_service availabilities");
    }

    // Wait for completions
    std::map<std::shared_ptr<wrench::WorkflowTask>, std::tuple<double, double>>
        times;
    for (int i = 0; i < 4; i++) {
      ...
      if (not std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(
              event)) {
        throw std::runtime_error("Unexpected execution event: " +
                                 event->toString());
      }
    ...
    if (std::get<0>(times[tasks.at(0)]) > 1.0) {
      throw std::runtime_error("Unexpected start time for task1: " +
                               std::to_string(std::get<0>(times[tasks.at(0)])));
    }
    if ((std::get<1>(times[tasks.at(0)]) < 60.0) ||
        (std::get<1>(times[tasks.at(0)]) > 61.0)) {
      throw std::runtime_error("Unexpected end time for task1: " +
                               std::to_string(std::get<1>(times[tasks.at(0)])));
    }
    // TASK #4
    if (std::get<0>(times[tasks.at(3)]) > 1.0) {
      throw std::runtime_error("Unexpected start time for task4: " +
                               std::to_string(std::get<0>(times[tasks.at(3)])));
    }
    if ((std::get<1>(times[tasks.at(3)]) < 60.0) ||
        (std::get<1>(times[tasks.at(3)]) > 61.0)) {
      throw std::runtime_error("Unexpected end time for task4: " +
                               std::to_string(std::get<1>(times[tasks.at(3)])));
    }
    // TASK #2
    if ((std::get<0>(times[tasks.at(1)]) < 60.0) ||
        (std::get<0>(times[tasks.at(1)]) > 61.0)) {
      throw std::runtime_error("Unexpected start time for task2: " +
                               std::to_string(std::get<0>(times[tasks.at(1)])));
    }
    if ((std::get<1>(times[tasks.at(1)]) < 120.0) ||
        (std::get<1>(times[tasks.at(1)]) > 121.0)) {
      throw std::runtime_error("Unexpected end time for task2: " +
                               std::to_string(std::get<1>(times[tasks.at(1)])));
    }
    // TASK #3
    if ((std::get<0>(times[tasks.at(2)]) < 120.0) ||
        (std::get<0>(times[tasks.at(2)]) > 121.0)) {
      throw std::runtime_error("Unexpected start time for task3: " +
                               std::to_string(std::get<0>(times[tasks.at(2)])));
    }
    if ((std::get<1>(times[tasks.at(2)]) < 180.0) ||
        (std::get<1>(times[tasks.at(2)]) > 181.0)) {
      throw std::runtime_error("Unexpected end time for task3: " +
                               std::to_string(std::get<1>(times[tasks.at(2)])));
    }
...
void BareMetalComputeServiceTestScheduling::do_RAMPressure_test() {
  ...
  ASSERT_NO_THROW(
      cs = simulation->add(new wrench::BareMetalComputeService(
          "Host1", (std::vector<std::string>){"Host1", "Host2"}, "", {}, {})));
  std::set<std::shared_ptr<wrench::ComputeService>> compute_services;
  compute_services.insert(cs);

  // Create a WMS
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(wms = simulation->add(new RAMPressureTestWMS(this, "Host1")));
...
void BareMetalComputeServiceTestScheduling::do_LoadBalancing1_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(
      wms = simulation->add(new LoadBalancing1TestWMS(this, "Host1")));
...
void BareMetalComputeServiceTestScheduling::do_LoadBalancing2_test() {
  ...
  ASSERT_NO_THROW(
      cs = simulation->add(new wrench::BareMetalComputeService(
          "Host1", (std::vector<std::string>){"Host1", "Host3"}, "", {}, {})));
  std::set<std::shared_ptr<wrench::ComputeService>> compute_services;
  compute_services.insert(cs);
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new LoadBalancing2TestWMS(this, "Host1")));
