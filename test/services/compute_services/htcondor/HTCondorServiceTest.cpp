...
WRENCH_LOG_CATEGORY(htcondor_service_test,
                    "Log category for HTCondorServiceTest");
...
protected:
  ...
  HTCondorServiceTest() {
    ...
    std::string xml1 =
        ...
        "             <prop id=\"mount\" value=\"/scratch\"/>"
        "          </disk>"
        "          <disk id=\"other_scratch\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"1000000GB\"/>"
        "             <prop id=\"mount\" value=\"/scratch2\"/>"
        "          </disk>"
        ...
        "       <route src=\"DualCoreHost\" dst=\"QuadCoreHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "       <route src=\"BatchHost1\" dst=\"BatchHost2\"> <link_ctn "
        "id=\"2\"/> </route>"
        "       <route src=\"DualCoreHost\" dst=\"BatchHost1\"> <link_ctn "
        "id=\"3\"/> </route>"
        "       <route src=\"DualCoreHost\" dst=\"BatchHost2\"> <link_ctn "
        "id=\"4\"/> </route>"
        "       <route src=\"QuadCoreHost\" dst=\"BatchHost1\"> <link_ctn "
        "id=\"5\"/> </route>"
        "       <route src=\"QuadCoreHost\" dst=\"BatchHost2\"> <link_ctn "
        "id=\"6\"/> </route>"
        "   </zone> "
        "</platform>";
...
public:
  HTCondorStandardJobTestWMS(HTCondorServiceTest *test, std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    auto two_task_job = job_manager->createStandardJob(
        {this->test->task1}, file_locations, {}, {}, {});

    // Submit the job for execution with service-specific args, which is not
    // allowed
    try {
      ...
      throw std::runtime_error("Should not have been able to submit a job with "
...
void HTCondorServiceTest::do_StandardJobTaskTest_test() {
  ...
  ASSERT_NO_THROW(htcondor_service =
                      simulation->add(new wrench::HTCondorComputeService(
                          hostname, std::move(compute_services),
                          {{wrench::HTCondorComputeServiceProperty::
                                INSTANT_RESOURCE_AVAILABILITIES,
                            "false"}})));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new HTCondorStandardJobTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    this->test->storage_service->deleteFile(wrench::FileLocation::LOCATION(
        this->test->storage_service, this->test->input_file));
...
class HTCondorStandardJobAddComputeServiceTestWMS
...
public:
  HTCondorPilotJobTestWMS(HTCondorServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    job_manager->submitJob(pilot_job, this->test->htcondor_service,
...

    {
      ...
      auto real_event =
          std::dynamic_pointer_cast<wrench::PilotJobStartedEvent>(event);
      if (not real_event) {
        throw std::runtime_error(
            "Did not get the expected PilotJobStartEvent (got " +
            event->toString() + ")");
      }
    }
...
void HTCondorServiceTest::do_PilotJobTaskTest_test() {
  ...
  auto batch_compute_service = simulation->add(new wrench::BatchComputeService(
      execution_host, {execution_host}, "/scratch2"));
  ...
  ASSERT_NO_THROW(
      htcondor_service = simulation->add(
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new HTCondorPilotJobTestWMS(this, hostname)));
...
public:
  HTCondorSimpleServiceTestWMS(HTCondorServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
    ...
    auto two_task_job = job_manager->createStandardJob(
        ...
        {std::make_tuple(
            wrench::FileLocation::LOCATION(
                htcondor_cs->getLocalStorageService(), this->test->input_file),
            wrench::FileLocation::SCRATCH(this->test->input_file))},
        {}, {});
...
void HTCondorServiceTest::do_SimpleServiceTest_test() {
...

  {
    ...
    ASSERT_THROW(htcondor_service =
                     simulation->add(new wrench::HTCondorComputeService(
                         hostname, std::move(invalid_compute_services), {})),
...
public:
  HTCondorGridUniverseTestWMS(HTCondorServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    std::map<std::shared_ptr<wrench::DataFile>,
             std::shared_ptr<wrench::FileLocation>>
        file_locations;
    file_locations[this->test->input_file2] = wrench::FileLocation::LOCATION(
        htcondor_cs->getLocalStorageService(), this->test->input_file2);
    std::shared_ptr<wrench::StandardJob> grid_job =
        job_manager->createStandardJob(
            {this->test->task7}, file_locations,
            {std::make_tuple(
                wrench::FileLocation::LOCATION(this->test->storage_service,
                                               this->test->input_file2),
                wrench::FileLocation::LOCATION(
    ...
    // Submit the job for execution
    try {
      job_manager->submitJob(grid_job, this->test->htcondor_service,
                             service_specific_arguments);
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error(e.what());
    }
...
void HTCondorServiceTest::do_GridUniverseTest_test() {
  ...
  auto batch_service = simulation->add(new wrench::BatchComputeService(
      "BatchHost1", {"BatchHost1", "BatchHost2"}, "/scratch", {}));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new HTCondorGridUniverseTestWMS(this, hostname)));
  ...
  // simulation->getOutput().dumpUnifiedJSON(grid_workflow,
  // "/tmp/workflow_data.json", false, true, false, false, false, false, false);
...
/************************************************************************************************/
/**          GRID UNIVERSE JOB SUBMISSION TEST WHERE GRID UNIVERSE JOBS NOT
 * SUPPORTED          **/
/************************************************************************************************/
...
void HTCondorServiceTest::do_NoGridUniverseSupportTest_test() {
  ...
  // Fails because it will try to run a grid job on a system that does not
  // support standard jobs, WMS is set up to
...
/************************************************************************************************/
/**          GRID UNIVERSE JOB SUBMISSION TEST WHERE NON-GRID UNIVERSE JOBS NOT
 * SUPPORTED          **/
/************************************************************************************************/
...
private:
  ...
  int main() override {
    ...
    // Submit the  job for execution
    try {
      job_manager->submitJob(non_grid_job, this->test->htcondor_service,
                             test_service_specs);
      //            throw std::runtime_error("Shouldn't have been able to submit
      //            a grid job"); return 1;
    } catch (std::invalid_argument &ignore) {
      //            return 0;
    }
    ...
    if (not std::dynamic_pointer_cast<wrench::NotAllowed>(
...
public:
  HTCondorNoGridJobTestWMS(HTCondorServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
  ...
  int main() override {
    ...
    try {
      job_manager->submitJob(grid_job, this->test->htcondor_service,
                             test_service_specs);
      throw std::runtime_error(
          "Should not have been able to submit job successfully");
    } catch (std::invalid_argument &ignore) {
      return 0;
    }
  }
...
void HTCondorServiceTest::do_NoGridJobSupportTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new HTCondorNoGridJobTestWMS(this, hostname)));
  ...
  int main() override {
...

    {
      ...
      try {
        ...
        if (not real_cause) {
          throw std::runtime_error(
              "Should have gotten a NotEnoughResources failure cause");
        }
      }
    }
...
public:
  HTCondorScratchTestWMS(HTCondorServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test") {
    this->test = test;
  }
...
private:
  ...
  int main() override {
    ...
    file_locations[this->test->input_file] =
        wrench::FileLocation::SCRATCH(this->test->input_file);
    std::vector<std::tuple<std::shared_ptr<wrench::FileLocation>,
                           std::shared_ptr<wrench::FileLocation>>>
        pre_file_copies;
...
void HTCondorServiceTest::do_ScratchTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new HTCondorScratchTestWMS(this, hostname)));
