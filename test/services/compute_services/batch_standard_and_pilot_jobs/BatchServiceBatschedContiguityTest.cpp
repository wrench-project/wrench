...
WRENCH_LOG_CATEGORY(batch_service_contiguity_test,
...
public:
  ...
  std::shared_ptr<wrench::ComputeService>
      batch_service_conservative_bf_contiguous = nullptr;
  std::shared_ptr<wrench::ComputeService>
      batch_service_conservative_bf_non_contiguous = nullptr;
  std::shared_ptr<wrench::ComputeService> batch_service_easy_bf_contiguous =
      nullptr;
  std::shared_ptr<wrench::ComputeService> batch_service_easy_bf_non_contiguous =
      nullptr;
...
private:
  ...
  int main() override {
    ...
    compute_services.push_back(
        this->test->batch_service_conservative_bf_contiguous);
    compute_services.push_back(
        this->test->batch_service_conservative_bf_non_contiguous);
    compute_services.push_back(this->test->batch_service_easy_bf_contiguous);
    compute_services.push_back(
        this->test->batch_service_easy_bf_non_contiguous);

    for (auto const &cs : compute_services) {

      std::shared_ptr<wrench::WorkflowTask> task1 =
          this->test->workflow->addTask(cs->getName() + "task1", 59, 1, 1, 0);
      std::shared_ptr<wrench::WorkflowTask> task2 =
          this->test->workflow->addTask(cs->getName() + "task2", 118, 1, 1, 0);
      std::shared_ptr<wrench::WorkflowTask> task3 =
          this->test->workflow->addTask(cs->getName() + "task3", 59, 1, 1, 0);
      std::shared_ptr<wrench::WorkflowTask> task4 =
          this->test->workflow->addTask(cs->getName() + "task4", 59, 1, 1, 0);
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (real_event) {
          std::shared_ptr<wrench::WorkflowTask> task =
              *(real_event->standard_job->getTasks().begin());
          if (task == task4) {
      ...
      if ((cs == this->test->batch_service_conservative_bf_contiguous) and
          (not contiguous)) {
        throw std::runtime_error(
            "conservative_bf/contiguous is not contiguous!");
      }
      if ((cs == this->test->batch_service_conservative_bf_non_contiguous) and
          (contiguous)) {
        throw std::runtime_error(
            "conservative_bf/non-contiguous is contiguous!");
      }
      if ((cs == this->test->batch_service_easy_bf_contiguous) and
          (not contiguous)) {
        throw std::runtime_error("easy_bf/contiguous is not contiguous!");
      }
      if ((cs == this->test->batch_service_easy_bf_non_contiguous) and
          (contiguous)) {
...
#else
TEST_F(BatchServiceBatschedContiguityTest,
       DISABLED_BatchJobContiguousAllocationTest)
#endif
{
  DO_TEST_WITH_FORK(do_BatchJobContiguousAllocationTest_test);
}

void BatchServiceBatschedContiguityTest::
    do_BatchJobContiguousAllocationTest_test() {
  ...
  batch_service_conservative_bf_contiguous =
  ...
  batch_service_conservative_bf_non_contiguous =
      simulation->add(new wrench::BatchComputeService(
          hostname, {"Host1", "Host2", "Host3", "Host4"}, "",
          {{wrench::BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM,
            "conservative_bf"},
  ...
  batch_service_easy_bf_contiguous =
      simulation->add(new wrench::BatchComputeService(
          hostname, {"Host1", "Host2", "Host3", "Host4"}, "",
          {{wrench::BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM,
            "easy_bf"},
  ...
  batch_service_easy_bf_non_contiguous =
      simulation->add(new wrench::BatchComputeService(
          hostname, {"Host1", "Host2", "Host3", "Host4"}, "",
          {{wrench::BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM,
            "easy_bf"},
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new BatchJobContiguousAllocationTestWMS(this, hostname)));
