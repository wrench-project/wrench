...
WRENCH_LOG_CATEGORY(batch_service_fcfs_test,
                    "Log category for BatchServiceFCFSTest");
...
public:
  SimpleFCFSTestWMS(BatchServiceFCFSTest *test, std::string hostname)
...
private:
  ...
  int main() override {
    ...
    for (int i = 0; i < 8; i++) {
      tasks[i] = this->test->workflow->addTask("task" + std::to_string(i), 60,
                                               1, 1, 0);
      jobs[i] = job_manager->createStandardJob(tasks[i]);
    }

    std::map<std::string, std::string> two_hosts_ten_cores,
        two_hosts_five_cores, one_hosts_five_cores, four_hosts_five_cores;
    ...
    std::map<std::string, std::string> job_args[8] = {
        two_hosts_ten_cores,  four_hosts_five_cores, two_hosts_ten_cores,
        two_hosts_ten_cores,  four_hosts_five_cores, two_hosts_five_cores,
        one_hosts_five_cores, four_hosts_five_cores};

    double expected_completion_times[8] = {60,  120, 180, 180,
                                           240, 240, 240, 300};
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (delta > EPSILON) {
        throw std::runtime_error(
            "Unexpected job completion time for the job containing task " +
            tasks[i]->getID() + ": " +
            std::to_string(actual_completion_times[i]) + " (expected: " +
...
void BatchServiceFCFSTest::do_SimpleFCFS_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new SimpleFCFSTestWMS(this, hostname)));
...
class SimpleFCFSQueueWaitTimePredictionWMS
...
private:
  ...
  int main() override {
    ...
    std::map<std::string, std::string> two_hosts_ten_cores,
        two_hosts_five_cores, one_hosts_five_cores, three_hosts_five_cores,
    ...
    std::map<std::string, std::string> job_args[9] = {
        two_hosts_ten_cores,      three_hosts_five_cores,
        two_hosts_ten_cores,      two_hosts_ten_cores,
        three_hosts_five_cores,   two_hosts_five_cores,
        one_hosts_five_cores,     two_hosts_five_cores,
        one_host_four_cores_short};
  ...
  int main() override {
    ...
    try {
      std::map<std::string, double> jobs_estimated_start_times =
          this->test->compute_service->getStartTimeEstimates(set_of_jobs);
      throw std::runtime_error(
          "Should not have been able to get prediction for BESTFIT algorithm");
    } catch (wrench::ExecutionException &e) {
      auto cause = std::dynamic_pointer_cast<wrench::FunctionalityNotAvailable>(
          e.getCause());
      if (not cause) {
        throw std::runtime_error(
      ...
      if (cause->getService() != this->test->compute_service) {
        throw std::runtime_error("Got expected exception and cause type, but "
                                 "compute service is wrong");
      }
      if (cause->getFunctionalityName() != "start time estimates") {
        throw std::runtime_error("Got expected exception and cause type, but "
                                 "functionality name is wrong (" +
...
void BatchServiceFCFSTest::do_BrokenQueueWaitTimePrediction_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new BrokenQueueWaitTimePredictionWMS(this, hostname)));
