...
protected:
  ...
  BatchServiceTest() {
    ...
    std::string xml =
        ...
        "       <route src=\"Host3\" dst=\"Host1\"> <link_ctn id=\"1\"/> "
        "</route>"
        "       <route src=\"Host3\" dst=\"Host4\"> <link_ctn id=\"1\"/> "
        "</route>"
        "       <route src=\"Host4\" dst=\"Host1\"> <link_ctn id=\"1\"/> "
        "</route>"
...
public:
  BogusSetupTestWMS(BatchServiceTest *test, const std::string &hostname)
...
class TerminateOneStandardJobSubmissionTestWMS
...
private:
  ...
  int main() override {
    ...
    // Submit job1 that should start right away
    {
      ...
      // Create a StandardJob with some pre-copies and post-deletions (not
      // useful, but this is testing after all)

      job1 = job_manager->createStandardJob(
          {task1},
          {{task1->getInputFiles().at(0),
          ...
          {std::tuple<std::shared_ptr<wrench::FileLocation>,
              ...
              wrench::FileLocation::LOCATION(
                  ...
                  wrench::Simulation::getFileByID("input_file")),
              wrench::FileLocation::LOCATION(
                  this->test->storage_service2,
                  wrench::Simulation::getFileByID("input_file")))},
          {},
          {std::shared_ptr<wrench::FileLocation>(wrench::FileLocation::LOCATION(
              this->test->storage_service2,
              wrench::Simulation::getFileByID("input_file")))});
    ...
    try {
      ...
      throw std::runtime_error("Error while getting and execution event: " +
                               e.getCause()->toString());
    }
    if (not std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(
            event)) {
...
void BatchServiceTest::do_TerminateStandardJobsTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new TerminateOneStandardJobSubmissionTestWMS(this, hostname)));

  // Create two files
  std::shared_ptr<wrench::DataFile> input_file =
      wrench::Simulation::addFile("input_file", 10000.0);
...
class TerminateOnePilotJobSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
    ...
    batch_job_args["-c"] = "10";  // number of cores per node
    try {
      job_manager->submitJob(pjob1, this->test->compute_service,
                             batch_job_args);
      job_manager->submitJob(pjob2, this->test->compute_service,
                             batch_job_args);
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error("Exception: " + std::string(e.what()));
    }
    ...
    if (pjob1->getState() != wrench::PilotJob::State::TERMINATED) {
      throw std::runtime_error(
          "Pilot job #1's state should be TERMINATED (instead: " +
          std::to_string(pjob1->getState()) + ")");
    }
    if (pjob2->getState() != wrench::PilotJob::State::TERMINATED) {
      throw std::runtime_error(
          "Pilot job #2's state should be TERMINATED (instead: " +
          std::to_string(pjob2->getState()) + ")");
    }
...
class OneStandardJobSubmissionTestWMS : public wrench::ExecutionController {
public:
  ...
  int main() override {
...

    {
      ...
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task1", 60, 2, 2, 0);
      ...
      auto job = job_manager->createStandardJob(
          {task},
          {{task->getInputFiles().at(0),
            wrench::FileLocation::LOCATION(this->test->storage_service1,
                                           task->getInputFiles().at(0))},
           {task->getOutputFiles().at(0),
            wrench::FileLocation::LOCATION(this->test->storage_service1,
                                           task->getOutputFiles().at(0))}},
          {std::tuple<std::shared_ptr<wrench::FileLocation>,
                      std::shared_ptr<wrench::FileLocation>>(
      ...
      try {
        ...
        if (not std::dynamic_pointer_cast<wrench::NotAllowed>(e.getCause())) {
          throw std::runtime_error("Got an expected exception, but the failure "
                                   "cause is not NotAllowed");
        }
      }
      ...
      // Shutdown the compute service, for testing purposes, which should do
      // nothing
      this->test->compute_service->stop();
    }
...
void BatchServiceTest::do_OneStandardJobTaskTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new OneStandardJobSubmissionTestWMS(this, hostname)));
...
public:
  StandardJobFailureTestWMS(BatchServiceTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
...

    {
      ...
      // Create a StandardJob with a bogus pre file copy (source and destination
      // are swapped!)
      auto job = job_manager->createStandardJob(
          {task},
          {{task->getInputFiles().at(0),
      ...
      if (not std::dynamic_pointer_cast<wrench::StandardJobFailedEvent>(
              event)) {
        throw std::runtime_error("Unexpected workflow execution event: " +
                                 event->toString());
      }
...
void BatchServiceTest::do_StandardJobFailureTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new StandardJobFailureTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    for (size_t i = 0; i < 2; i++) {
      ...
      double ram = 0.0;
      tasks.push_back(
          this->test->workflow->addTask("test_job_1_task_" + std::to_string(i),
                                        task_flops, num_cores, num_cores, ram));
    }
    ...
    batch_job_args["-t"] =
        std::to_string(18000); // Time in seconds (at least 1 minute)
    ...
    for (size_t i = 0; i < 2; i++) {
      ...
      double ram = 0.0;
      tasks.push_back(
          this->test->workflow->addTask("test_job_2_task_" + std::to_string(i),
                                        task_flops, num_cores, num_cores, ram));
    }
    ...
    // Submit this job to the BatchComputeService service, after sleeping some
    // epsilon to ensure some sequence of the two jobs and not simultaneous
    // events
    ...
    for (auto const &job : {standard_job_1, standard_job_2}) {
      ...
      try {
        event = this->waitForNextEvent();
        auto real_event =
            std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(event);
        if (real_event) {
          if (real_event->standard_job != job) {
            throw std::runtime_error("Wrong job completion order: got " +
                                     real_event->standard_job->getName() +
                                     " but expected " + job->getName());
          }
        } else {
          throw std::runtime_error("Unexpected workflow execution event: " +
                                   event->toString());
      ...
      RUNTIME_DBL_EQ(completion_time, expected_completion_time,
                     "Unexpected job completion time for job " +
                         job->getName() + ": " +
...
private:
  ...
  int main() override {
    ...
    {
      // Create a pilot job that needs 1 host, 1 code, 0 bytes of RAM and 30
      // seconds
      ...
      try {
        ...
        throw std::runtime_error("Should not be able to submit a pilot job to "
                                 "a null compute service");
      } catch (std::invalid_argument &ignore) {
      }
      ...
      try {
        ...
        throw std::runtime_error(
            "Should not be able to submit a pilot job with bogus arguments");
      } catch (std::invalid_argument &ignore) {
      }
...
void BatchServiceTest::do_PilotJobTaskTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new OnePilotJobSubmissionTestWMS(this, hostname)));
...
class StandardPlusPilotJobSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
...

    {
      // Create a sequential task1 that lasts one min and requires 2 cores
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task1", 50, 2, 2, 0);
      task->addInputFile(wrench::Simulation::getFileByID("input_file"));
      task->addOutputFile(wrench::Simulation::getFileByID("output_file"));
...
void BatchServiceTest::do_StandardPlusPilotJobTaskTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new StandardPlusPilotJobSubmissionTestWMS(this, hostname)));
...
class InsufficientCoresJobSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
    ...
    // Standard Job
    {
      // Create a sequential task1 that lasts one min and requires 2 cores
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task1", 50, 2, 12, 0);
      ...
      batch_job_args["-t"] = "120"; // time in seconds
      batch_job_args["-c"] =
          "12"; // number of cores per node, which is too many!

      try {
        job_manager->submitJob(job, this->test->compute_service,
                               batch_job_args);
        throw std::runtime_error(
            "Job Submission should have generated an exception");
      } catch (wrench::ExecutionException &e) {
        auto cause =
    ...
    // Pilot Job
    {
      // Create a PilotJob with some pre-copies and post-deletions (not useful,
      // but this is testing after all)
...
void BatchServiceTest::do_InsufficientCoresTaskTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new InsufficientCoresJobSubmissionTestWMS(this, hostname)));
  ...
  int main() override {
...

    {
      ...
      try {
        job_manager->submitJob(job, this->test->compute_service,
                               batch_job_args);
        throw std::runtime_error("Should not have been able to submit jobs "
                                 "without service-specific args");
...
void BatchServiceTest::do_noArgumentsJobSubmissionTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new NoArgumentsJobSubmissionTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
...

    {
      ...
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task1", 65, 1, 1, 0);
      task->addInputFile(wrench::Simulation::getFileByID("input_file"));
      task->addOutputFile(wrench::Simulation::getFileByID("output_file"));
      ...
      auto real_event =
          std::dynamic_pointer_cast<wrench::StandardJobFailedEvent>(event);
      if (real_event) {
        auto cause = std::dynamic_pointer_cast<wrench::JobTimeout>(
            real_event->failure_cause);
        if (not cause) {
          throw std::runtime_error(
              "Expected event, but unexpected failure cause: " +
              real_event->failure_cause->toString() +
              " (expected: JobTimeout)");
        }
        if (cause->getJob() != job) {
          throw std::runtime_error("Expected JobTimeout failure cause does not "
                                   "point to expected job");
        }
...
void BatchServiceTest::do_StandardJobTimeOutTaskTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new StandardJobTimeoutSubmissionTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    {
      // Create a pilot job that needs 1 host, 1 core, 0 bytes of RAM, and 90
      // seconds
      auto pilot_job = job_manager->createPilotJob();
...
void BatchServiceTest::do_PilotJobTimeOutTaskTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new PilotJobTimeoutSubmissionTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
...

    {
      ...
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task", 50, 8, 8, 0);
      task->addInputFile(wrench::Simulation::getFileByID("input_file"));
      task->addOutputFile(wrench::Simulation::getFileByID("output_file"));

      // Create another sequential task1 that lasts one min and requires 9 cores
      std::shared_ptr<wrench::WorkflowTask> task1 =
          this->test->workflow->addTask("task1", 50, 9, 9, 0);
      ...
      std::shared_ptr<wrench::WorkflowTask> task2 =
          this->test->workflow->addTask("task2", 50, 1, 1, 0);
      ...
      auto job1 = job_manager->createStandardJob(
          ...
          {std::tuple<std::shared_ptr<wrench::FileLocation>,
              ...
              wrench::FileLocation::LOCATION(
                  this->test->storage_service1,
                  wrench::Simulation::getFileByID("input_file_1")),
              wrench::FileLocation::LOCATION(
                  this->test->storage_service2,
                  wrench::Simulation::getFileByID("input_file_1")))},
          {},
      ...
      auto job2 = job_manager->createStandardJob(
          ...
          {std::tuple<std::shared_ptr<wrench::FileLocation>,
              ...
              wrench::FileLocation::LOCATION(
                  this->test->storage_service1,
                  wrench::Simulation::getFileByID("input_file_2")),
              wrench::FileLocation::LOCATION(
                  this->test->storage_service2,
                  wrench::Simulation::getFileByID("input_file_2")))},
          {},
      ...
      if (task1->getExecutionHost() != task2->getExecutionHost()) {
        throw std::runtime_error(
            "BatchServiceTest::BestFitStandardJobSubmissionTest():: BestFit "
            "did not pick the right hosts");
      }
...
void BatchServiceTest::do_BestFitTaskTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new BestFitStandardJobSubmissionTestWMS(this, hostname)));
  ...
  std::shared_ptr<wrench::DataFile> output_file =
      wrench::Simulation::addFile("output_file", 20000.0);
  std::shared_ptr<wrench::DataFile> input_file_1 =
      wrench::Simulation::addFile("input_file_1", 10000.0);
  std::shared_ptr<wrench::DataFile> output_file_1 =
      wrench::Simulation::addFile("output_file_1", 20000.0);
  std::shared_ptr<wrench::DataFile> input_file_2 =
      wrench::Simulation::addFile("input_file_2", 10000.0);
  std::shared_ptr<wrench::DataFile> output_file_2 =
      wrench::Simulation::addFile("output_file_2", 20000.0);
...
class FirstFitStandardJobSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
    ...
    {
      // In this test, we create 40 tasks, each task1 uses one core. There are 4
      // hosts with 10 cores. So the first 10 tasks should run on first host,
      // next 10 tasks should run on second host, next 10 tasks should run on
      // third and last 10 tasks should run on fourth host using first fit host
      ...
      unsigned long repetition =
          num_tasks / (num_cores_in_each_task * num_hosts_in_platform);
      std::vector<std::shared_ptr<wrench::WorkflowTask>> tasks = {};
      std::vector<std::shared_ptr<wrench::StandardJob>> jobs = {};
      for (int i = 0; i < num_tasks; i++) {
        tasks.push_back(this->test->workflow->addTask(
            "task1" + std::to_string(i), 59, num_cores_in_each_task,
      ...
      for (unsigned long i = 0; i < num_hosts_in_platform; i++) {
        for (unsigned long j = i * repetition; j < (((i + 1) * repetition) - 1);
             j++) {
          if (tasks[j]->getExecutionHost() !=
              tasks[j + 1]->getExecutionHost()) {
            throw std::runtime_error(
                "BatchServiceTest::FirstFitStandardJobSubmissionTest():: The "
                "tasks did not execute on the right hosts");
          }
        }
      }
...
void BatchServiceTest::do_FirstFitTaskTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new FirstFitStandardJobSubmissionTestWMS(this, hostname)));
...
class RoundRobinStandardJobSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
    ...
    {
      // Create a sequential task1 that lasts one min and requires 2 cores
      std::shared_ptr<wrench::WorkflowTask> task1_1 =
          this->test->workflow->addTask("task1_1", 60, 2, 2, 0);
      std::shared_ptr<wrench::WorkflowTask> task1_2 =
          this->test->workflow->addTask("task1_2", 60, 2, 2, 0);

      // Create another sequential task1 that lasts one min and requires 9 cores
      std::shared_ptr<wrench::WorkflowTask> task2 =
          this->test->workflow->addTask("task2", 360, 9, 9, 0);

      // Create another sequential task1 that lasts one min and requires 1 core
      std::shared_ptr<wrench::WorkflowTask> task3 =
          this->test->workflow->addTask("task3", 59, 1, 1, 0);

      // Create another sequential task1 that lasts one min and requires 10
      // cores
      std::shared_ptr<wrench::WorkflowTask> task4 =
          this->test->workflow->addTask("task4", 600, 10, 10, 0);
      ...
      WRENCH_INFO("Task1_1 completed on host %s",
                  task1_1->getExecutionHost().c_str());
      WRENCH_INFO("Task1_2 completed on host %s",
                  task1_2->getExecutionHost().c_str());
      WRENCH_INFO("Task2 completed on host %s",
                  task2->getExecutionHost().c_str());
      WRENCH_INFO("Task3 completed on host %s",
                  task3->getExecutionHost().c_str());
      WRENCH_INFO("Task4 completed on host %s",
                  task4->getExecutionHost().c_str());

      double EPSILON = 1.0;
      double not_expected_date =
          60; // FIRSTFIT and BESTFIT would complete in ~60 seconds but
              // ROUNDROBIN would finish, in this case, in ~90 seconds
      if (std::abs(wrench::Simulation::getCurrentSimulatedDate() -
                   not_expected_date) <= EPSILON) {
        throw std::runtime_error(
            "BatchServiceTest::ROUNDROBINTEST():: The tasks did not finish on "
            "time: Simulation Date > Expected Date");
      } else {
        // congrats, round robin works
        // however let's check further if the task1 hostname is equal to the
        // task4 hostname
        if ((task1_1->getExecutionHost() != task4->getExecutionHost()) and
            (task1_2->getExecutionHost() != task4->getExecutionHost())) {
          throw std::runtime_error(
              "BatchServiceTest::ROUNDROBINTEST():: The tasks did not execute "
              "on the right hosts: " +
              task1_1->getExecutionHost() + "-" + task1_2->getExecutionHost() +
              "-" + task2->getExecutionHost() + "-" +
              task3->getExecutionHost() + "-" + task4->getExecutionHost());
        }
      }
...

    {
      ...
      for (int i = 0; i < num_tasks; i++) {
        tasks.push_back(this->test->workflow->addTask(
            "task1" + std::to_string(i), 59, 1, 1, 0));
        jobs.push_back(job_manager->createStandardJob(tasks[i]));
        std::map<std::string, std::string> args;
        args["-N"] = "1";
        args["-t"] = "60";
      ...
      for (int i = 0; i < num_tasks; i++) {
        if (tasks[i]->getExecutionHost() !=
            tasks[(i + num_hosts) % num_hosts]->getExecutionHost()) {
          throw std::runtime_error(
              "BatchServiceTest::ROUNDROBINTEST():: The tasks in the second "
              "test did not execute on the right hosts");
        }
      }
...
void BatchServiceTest::do_RoundRobinTask_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new RoundRobinStandardJobSubmissionTestWMS(this, hostname)));
...
class StandardJobInsidePilotJobTimeoutSubmissionTestWMS
    : public wrench::ExecutionController {
public:
  ...
  int main() override {
...

    {
      ...
      std::shared_ptr<wrench::DataFile> file1 =
          wrench::Simulation::getFileByID("input_file");
      std::shared_ptr<wrench::DataFile> file2 =
          wrench::Simulation::getFileByID("output_file");
      ...
      try {
        ...
        throw std::runtime_error("Got some exception " + std::string(e.what()));
      }
      ...
      auto job = job_manager->createStandardJob(
          {task},
          {{file1, wrench::FileLocation::LOCATION(this->test->storage_service1,
                                                  file1)}},
          {}, {}, {});
      ...
      try {
        ...
        throw std::runtime_error(
            "Unexpected exception while terminating pilot job: " +
            std::string(e.what()));
      }
      ...
      if (real_event) {
        ...
        if (cause->getJob() != job) {
          throw std::runtime_error(
              "Got the correct failure even, a correct cause type, but the "
              "cause points to the wrong job");
        }
      } else {
        throw std::runtime_error("Unexpected workflow execution event: " +
                                 event->toString());
...
void BatchServiceTest::do_StandardJobInsidePilotJobTimeOutTaskTest_test() {
  ...
  ASSERT_NO_THROW(
      compute_service = simulation->add(new wrench::BatchComputeService(
          hostname, {"Host1", "Host2", "Host3", "Host4"}, "/scratch")));
...
class StandardJobInsidePilotJobSucessSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
class InsufficientCoresInsidePilotJobSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
...

    {
      ...
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task1", 60, 5, 5, 0);
      task->addInputFile(wrench::Simulation::getFileByID("input_file"));
      task->addOutputFile(wrench::Simulation::getFileByID("output_file"));
...
class MultipleStandardJobSubmissionTestWMS
    : public wrench::ExecutionController {
public:
  ...
  int main() override {
...

    {
      ...
      for (int i = 0; i < num_standard_jobs; i++) {
        // Create a sequential task1 that lasts for random seconds and requires
        // 2 cores
        std::shared_ptr<wrench::WorkflowTask> task =
            this->test->workflow->addTask("task1" + std::to_string(i),
                                          each_task_time, 2, 2, 0);
        auto job = job_manager->createStandardJob(task);
      ...
      batch_job_args["-N"] = "1";
      batch_job_args["-t"] =
          std::to_string((each_task_time)*num_standard_jobs); // time in seconds
      batch_job_args["-c"] = "2"; // number of cores per node
      for (const auto &standard_jobs : jobs) {
        try {
          job_manager->submitJob(standard_jobs, this->test->compute_service,
                                 batch_job_args);
        } catch (wrench::ExecutionException &e) {
          throw std::runtime_error("Exception: " + std::string(e.what()));
        }
...
void BatchServiceTest::do_MultipleStandardTaskTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new MultipleStandardJobSubmissionTestWMS(this, hostname)));
...
class DifferentBatchAlgorithmsSubmissionTestWMS
    : public wrench::ExecutionController {
public:
...
void BatchServiceTest::do_DifferentBatchAlgorithmsSubmissionTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new DifferentBatchAlgorithmsSubmissionTestWMS(this, hostname)));
...
class ShutdownWithPendingRunningJobsTestWMS
    : public wrench::ExecutionController {
public:
...
private:
  ...
  int main() override {
    ...
    for (int i = 0; i < 3; i++) {
      tasks[i] = this->test->workflow->addTask("task1" + std::to_string(i), 600,
                                               10, 10, 0);
    }
    ...
    for (int i = 0; i < 3; i++) {
      ...
      batch_job_args["-c"] = "10";  // number of cores per node
      try {
        job_manager->submitJob(jobs[i], this->test->compute_service,
                               batch_job_args);
      } catch (wrench::ExecutionException &e) {
        throw std::runtime_error("Exception: " + std::string(e.what()));
      }
    ...
    this->test->compute_service->stop(
        true, wrench::ComputeService::TerminationCause::TERMINATION_JOB_KILLED);
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (not std::dynamic_pointer_cast<wrench::StandardJobFailedEvent>(
              event)) {
        throw std::runtime_error(
            "Should have received a STANDARD_JOB_FAILURE event (received " +
            event->toString());
      }

      auto real_event =
          std::dynamic_pointer_cast<wrench::StandardJobFailedEvent>(event);
      auto cause = std::dynamic_pointer_cast<wrench::JobKilled>(
          real_event->failure_cause);
      if (not cause) {
        throw std::runtime_error(
            "Expected event, but unexpected failure cause: " +
      ...
      if ((cause->getJob() != jobs[0]) and (cause->getJob() != jobs[1]) and
          (cause->getJob() != jobs[2])) {
        throw std::runtime_error(
            "Expected JobKilled failure cause does not point to expected job");
      }
      cause->toString(); // for coverage
    }
...
void BatchServiceTest::do_ShutdownWithPendingRunningJobsTest_test() {
  ...
  ASSERT_NO_THROW(
      compute_service = simulation->add(
          new wrench::BatchComputeService(hostname, {"Host1"}, "", {})));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new ShutdownWithPendingRunningJobsTestWMS(this, hostname)));
