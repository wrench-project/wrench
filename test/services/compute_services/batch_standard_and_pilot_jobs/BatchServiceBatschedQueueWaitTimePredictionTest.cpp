...
WRENCH_LOG_CATEGORY(
    batch_service_queue_wait_time_prediction_test,
    "Log category for BatchServiceBatschedQueueWaitTimePredictionTest");
...
protected:
  ...
  BatchServiceBatschedQueueWaitTimePredictionTest() {
    ...
    std::string xml =
        ...
        "       <route src=\"Host3\" dst=\"Host1\"> <link_ctn id=\"2\"/> "
        "</route>"
        "       <route src=\"Host4\" dst=\"Host1\"> <link_ctn id=\"2\"/> "
        "</route>"
...
class BatchJobBrokenEstimateWaitingTimeTestWMS
    : public wrench::ExecutionController {

public:
  BatchJobBrokenEstimateWaitingTimeTestWMS(
  ...
  int main() override {
...

    {
      ...
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task1", 299, 1, 1, 0);
      ...
      // std::tuple<std::string,unsigned int,double> my_job =
      // {job_id,nodes,walltime_seconds};
      std::tuple<std::string, unsigned long, unsigned long, double> my_job =
          std::make_tuple(job_id, nodes, 1, walltime_seconds);
      ...
      try {
        jobs_estimated_start_times =
            batch_service->getStartTimeEstimates(set_of_jobs);
        throw std::runtime_error(
            "Should not be able to get a queue waiting time estimate");
      } catch (wrench::ExecutionException &e) {
        auto cause =
            std::dynamic_pointer_cast<wrench::FunctionalityNotAvailable>(
                e.getCause());
        if (not cause) {
          throw std::runtime_error(
              "Got an expected exception but unexpected failure cause : " +
              e.getCause()->toString() +
              " (expected: FunctionalityNotAvailable)");
        }
        if (cause->getService() != batch_service) {
          throw std::runtime_error(
              "Got the expected exception, but the failure cause does not "
...
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       BatchJobBrokenEstimateWaitingTimeTest)
#else
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       DISABLED_BatchJobBrokenEstimateWaitingTimeTest)
#endif
{
  DO_TEST_WITH_FORK(do_BatchJobBrokenEstimateWaitingTimeTest_test);
}

void BatchServiceBatschedQueueWaitTimePredictionTest::
    do_BatchJobBrokenEstimateWaitingTimeTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new BatchJobBrokenEstimateWaitingTimeTestWMS(this, hostname)));
...
class BatchJobBasicEstimateWaitingTimeTestWMS
    : public wrench::ExecutionController {

public:
  BatchJobBasicEstimateWaitingTimeTestWMS(
      BatchServiceBatschedQueueWaitTimePredictionTest *test,
      std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
...

    {
      ...
      unsigned int nodes = 2;
      double walltime_seconds = 1000;
      // std::tuple<std::string,unsigned long,double> my_job =
      // {job_id,nodes,walltime_seconds};
      std::tuple<std::string, unsigned long, unsigned long, double> my_job =
          std::make_tuple(job_id, nodes, 1, walltime_seconds);
      std::set<std::tuple<std::string, unsigned long, unsigned long, double>>
      ...
      try {
        ...
        throw std::runtime_error(
            "Exception while getting queue waiting time estimate: " +
            std::string(e.what()));
      }
      double expected_wait_time = 300 - first_job_running;
      double tolerance = 1; // in seconds
      double delta = std::abs(expected_wait_time -
                              (jobs_estimated_start_times[job_id] - tolerance));
      if (delta > 1) {
        throw std::runtime_error(
            "Estimated start time incorrect (expected: " +
            std::to_string(expected_wait_time) + ", got: " +
            std::to_string(jobs_estimated_start_times[job_id]) + ")");
      }
...
#ifdef ENABLE_BATSCHED
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       BatchJobBasicEstimateWaitingTimeTest)
#else
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       DISABLED_BatchJobBasicEstimateWaitingTimeTest)
#endif
{
...
void BatchServiceBatschedQueueWaitTimePredictionTest::
    do_BatchJobBasicEstimateWaitingTimeTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new BatchJobBasicEstimateWaitingTimeTestWMS(this, hostname)));
...
public:
  BatchJobEstimateWaitingTimeTestWMS(
      BatchServiceBatschedQueueWaitTimePredictionTest *test,
      std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {
    this->test = test;
...
private:
  ...
  int main() override {
...

    {
      ...
      // Create a sequential task1 that lasts one min and requires 2 cores
      std::shared_ptr<wrench::WorkflowTask> task =
          this->test->workflow->addTask("task2", 60, 2, 2, 0);
      task->addInputFile(wrench::Simulation::getFileByID("input_file"));
      task->addOutputFile(wrench::Simulation::getFileByID("output_file"));
...
#ifdef ENABLE_BATSCHED
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       BatchJobEstimateWaitingTimeTest)
#else
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       DISABLED_BatchJobEstimateWaitingTimeTest)
#endif
{
...
void BatchServiceBatschedQueueWaitTimePredictionTest::
    do_BatchJobEstimateWaitingTimeTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new BatchJobEstimateWaitingTimeTestWMS(this, hostname)));
...
class BatchJobLittleComplexEstimateWaitingTimeTestWMS
    : public wrench::ExecutionController {

public:
  BatchJobLittleComplexEstimateWaitingTimeTestWMS(
      BatchServiceBatschedQueueWaitTimePredictionTest *test,
      std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
  ...
  int main() override {
...

    {
      ...
      std::shared_ptr<wrench::WorkflowTask> task1 =
          this->test->workflow->addTask("task2", 299, 1, 1, 0);
      ...
      std::shared_ptr<wrench::WorkflowTask> task2 =
          this->test->workflow->addTask("task3", 299, 1, 1, 0);
       ...
       *  (the BATSCHED_JOB_EXTRA_TIME constant in
       * BatchComputeService::sendAllQueuedJobsToBatsched()
       */
      std::string job_id = "my_job1";
      ...
      if ((jobs_estimated_start_times[job_id] - 903) > 1) {
        throw std::runtime_error(
            "A) Estimated queue start time incorrect (expected: " +
            std::to_string(903) + ", got: " +
            std::to_string(jobs_estimated_start_times[job_id]) + ")");
      }
      ...
      if (std::abs(jobs_estimated_start_times[job_id] - 301) > 1) {
        throw std::runtime_error(
            "B) Estimated start time incorrect (expected: " +
            std::to_string(301) + ", got: " +
            std::to_string(jobs_estimated_start_times[job_id]) + ")");
      }
      ...
      if (std::abs(jobs_estimated_start_times[job_id] - 301) > 1) {
        throw std::runtime_error(
            "C) Estimated start time incorrect (expected: " +
            std::to_string(301) + ", got: " +
            std::to_string(jobs_estimated_start_times[job_id]) + ")");
      }
      ...
      if (std::abs(jobs_estimated_start_times[job_id] - 903) > 1) {
        throw std::runtime_error(
            "D) Estimated start time incorrect (expected: " +
            std::to_string(903) + ", got: " +
            std::to_string(jobs_estimated_start_times[job_id]) + ")");
      }
...
#ifdef ENABLE_BATSCHED
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       BatchJobLittleComplexEstimateWaitingTimeTest)
#else
TEST_F(BatchServiceBatschedQueueWaitTimePredictionTest,
       DISABLED_BatchJobLittleComplexEstimateWaitingTimeTest)
#endif
{
...
void BatchServiceBatschedQueueWaitTimePredictionTest::
    do_BatchJobLittleComplexEstimateWaitingTimeTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new BatchJobLittleComplexEstimateWaitingTimeTestWMS(this, hostname)));
