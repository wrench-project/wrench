...
WRENCH_LOG_CATEGORY(batch_service_queue_wait_time_prediction_test,
...
public:
    ...
    void do_BatchJobBasicEstimateWaitingTimeTest_test(const std::string &algo);
...
protected:
    ~BatchServiceBatschedQueueWaitTimePredictionTest() override {
        workflow->clear();
...
private:
    ...
    int main() override {
...

        {
                ...
                {
                    {
                        ...
                        wrench::FileLocation::LOCATION(this->test->storage_service1, task->getInputFiles().at(0))
                    },
                    {
                        task->getOutputFiles().at(0),
                        wrench::FileLocation::LOCATION(this->test->storage_service1, task->getOutputFiles().at(0))
                    }
                },
                {
            ...
            std::tuple<std::string, unsigned long, unsigned long, sg_size_t> my_job = std::make_tuple(
                job_id, nodes, 1, walltime_seconds);
...
void BatchServiceBatschedQueueWaitTimePredictionTest::do_BatchJobBrokenEstimateWaitingTimeTest_test() {
    ...
    ASSERT_THROW(compute_service = simulation->add(
                     new wrench::BatchComputeService(hostname,
                         {"Host1", "Host2", "Host3", "Host4"}, "", {{wrench::BatchComputeServiceProperty::
                         BATCH_SCHEDULING_ALGORITHM, "BOGUS"}})),
                 std::invalid_argument);
    ASSERT_NO_THROW(compute_service = simulation->add(
        new wrench::BatchComputeService(hostname,
            {"Host1", "Host2", "Host3", "Host4"}, "", {{wrench::BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM,
            "easy_bf"}})));
...
private:
    ...
    int main() override {
        ...
        std::tuple<std::string, unsigned long, unsigned long, sg_size_t> my_job = std::make_tuple(
            job_id, nodes, 10, walltime_seconds);
        std::set<std::tuple<std::string, unsigned long, unsigned long, sg_size_t>> set_of_jobs = {my_job};
        ...
        if (delta > 1) {
            throw std::runtime_error(
                "Estimated start time incorrect (expected: " + std::to_string(expected_wait_time) + ", got: " +
...
{
#ifdef ENABLE_BATSCHED
    DO_TEST_WITH_FORK_ONE_ARG(do_BatchJobBasicEstimateWaitingTimeTest_test, "conservative_bf");
#else
    DO_TEST_WITH_FORK_ONE_ARG(do_BatchJobBasicEstimateWaitingTimeTest_test, "fcfs");
    DO_TEST_WITH_FORK_ONE_ARG(do_BatchJobBasicEstimateWaitingTimeTest_test, "conservative_bf");
    DO_TEST_WITH_FORK_ONE_ARG(do_BatchJobBasicEstimateWaitingTimeTest_test, "conservative_bf_core_level");
#endif
}


void BatchServiceBatschedQueueWaitTimePredictionTest::do_BatchJobBasicEstimateWaitingTimeTest_test(const std::string &algo) {
    // Create and initialize a simulation
    auto simulation = wrench::Simulation::createSimulation();
    int argc = 1;
    auto argv = (char**)calloc(argc, sizeof(char*));
    ...
    ASSERT_NO_THROW(compute_service = simulation->add(
        new wrench::BatchComputeService(hostname,
            {"Host1", "Host2", "Host3", "Host4"}, "", {{wrench::BatchComputeServiceProperty::BATSCHED_LOGGING_MUTED,
            "false"}, {wrench::BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM, algo}, {wrench::
            BatchComputeServiceProperty::BATCH_RJMS_PADDING_DELAY, "0"}})));
...
void BatchServiceBatschedQueueWaitTimePredictionTest::do_BatchJobEstimateWaitingTimeTest_test() {
    ...
    ASSERT_NO_THROW(compute_service = simulation->add(
        new wrench::BatchComputeService(hostname,
            {"Host1", "Host2", "Host3", "Host4"}, "", {{wrench::BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM,
            "conservative_bf"}, {wrench::BatchComputeServiceProperty::BATCH_RJMS_PADDING_DELAY, "0"}})));
    ...
    int main() override {
...

        {
                ...
                {
                    {
                        ...
                        wrench::FileLocation::LOCATION(this->test->storage_service1, task1->getInputFiles().at(0))
                    },
                    {
                        task1->getOutputFiles().at(0),
                        wrench::FileLocation::LOCATION(this->test->storage_service1, task1->getOutputFiles().at(0))
                    }
                },
                {
                    {
            ...
            auto job2 = job_manager->createStandardJob(
                {task2},
                {
                    ...
                    },
                    {
                        ...
                        wrench::FileLocation::LOCATION(this->test->storage_service1, task2->getOutputFiles().at(0))
                    }
                },
                {
                    std::tuple<std::shared_ptr<wrench::FileLocation>, std::shared_ptr<wrench::FileLocation>>(
            ...
            std::tuple<std::string, unsigned long, unsigned long, sg_size_t> my_job = std::make_tuple(
                job_id, nodes, 1, walltime_seconds);
            std::set<std::tuple<std::string, unsigned long, unsigned long, sg_size_t>> set_of_jobs = {my_job};
            std::map<std::string, double> jobs_estimated_start_times = batch_service->
                getStartTimeEstimates(set_of_jobs);

            if ((jobs_estimated_start_times[job_id] - 903) > 1) {
                throw std::runtime_error(
                    "A) Estimated queue start time incorrect (expected: " + std::to_string(903) + ", got: " +
                    std::to_string(jobs_estimated_start_times[job_id]) + ")");
            }
            ...
            if (std::abs(jobs_estimated_start_times[job_id] - 301) > 1) {
                throw std::runtime_error(
                    "B) Estimated start time incorrect (expected: " + std::to_string(301) + ", got: " + std::to_string(
                        jobs_estimated_start_times[job_id]) + ")");
            }
            ...
            if (std::abs(jobs_estimated_start_times[job_id] - 301) > 1) {
                throw std::runtime_error(
                    "C) Estimated start time incorrect (expected: " + std::to_string(301) + ", got: " + std::to_string(
                        jobs_estimated_start_times[job_id]) + ")");
            }
            ...
            if (std::abs(jobs_estimated_start_times[job_id] - 903) > 1) {
                throw std::runtime_error(
                    "D) Estimated start time incorrect (expected: " + std::to_string(903) + ", got: " + std::to_string(
                        jobs_estimated_start_times[job_id]) + ")");
            }
