...
WRENCH_LOG_CATEGORY(batch_service_conservative_bf_test,
                    "Log category for BatchServiceCONSERVATIVEBFTest");
...
private:
  ...
  int main() override {
    ...
    for (int i = 0; i < 4; i++) {
      tasks[i] = this->test->workflow->addTask("task1" + std::to_string(i), 60,
                                               1, 1, 0);
      jobs[i] = job_manager->createStandardJob(tasks[i]);
    }

    std::map<std::string, std::string> job1, job2, job3, job4;
    ...
    for (int i = 0; i < 4; i++) {
      double delta =
          std::abs(actual_completion_times[i] - expected_completion_times[i]);
      if (delta > EPSILON) {
        throw std::runtime_error(
            "Unexpected job completion time for the job containing task1 " +
            tasks[i]->getID() + ": " +
            std::to_string(actual_completion_times[i]) +
            "(expected: " + std::to_string(expected_completion_times[i]) + ")");
      }
    }
...
void BatchServiceCONSERVATIVE_BFTest::do_SimpleCONSERVATIVE_BF_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new SimpleCONSERVATIVE_BFTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    for (int i = 0; i < NUM_JOBS; i++) {
      ...
      tasks[i] = this->test->workflow->addTask(
          "task1" + std::to_string(i), 60 + 60 * (random % 30), 1, 1, 0);
      jobs[i] = job_manager->createStandardJob(tasks[i]);
    }
    ...
    std::map<std::string,
             std::pair<std::shared_ptr<wrench::StandardJob>, double>>
    ...
    for (int i = 0; i < NUM_JOBS; i++) {
      ...
      if (auto real_event =
        ...
        actual_completion_times[real_event->standard_job->getName()] =
            std::make_pair(real_event->standard_job,
                           wrench::Simulation::getCurrentSimulatedDate());
      ...
      std::set<std::tuple<std::string, unsigned long, unsigned long, double>>
          set_of_jobs = {
              (std::tuple<std::string, unsigned long, unsigned long, double>){
                  "testing_job1_" + std::to_string(i), 1, 10, 60},
              (std::tuple<std::string, unsigned long, unsigned long, double>){
                  "testing_job2_" + std::to_string(i), 1, 10, 10000},
              (std::tuple<std::string, unsigned long, unsigned long, double>){
                  "testing_job3_" + std::to_string(i), 2, 10, 10},
              (std::tuple<std::string, unsigned long, unsigned long, double>){
                  "testing_job4_" + std::to_string(i), 2, 10, 80},
              (std::tuple<std::string, unsigned long, unsigned long, double>){
                  "testing_job5_" + std::to_string(i), 5, 10, 400},
              (std::tuple<std::string, unsigned long, unsigned long, double>){
                  "testing_job6_" + std::to_string(i), 3, 10, 400},
              (std::tuple<std::string, unsigned long, unsigned long, double>){
                  "testing_job7_" + std::to_string(i), 4, 10, 400},
          };
...
void BatchServiceCONSERVATIVE_BFTest::do_LargeCONSERVATIVE_BF_test(int seed) {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new LargeCONSERVATIVE_BFTestWMS(
...
class SimpleCONSERVATIVE_BFQueueWaitTimePredictionWMS
    : public wrench::ExecutionController {

public:
  SimpleCONSERVATIVE_BFQueueWaitTimePredictionWMS(
      BatchServiceCONSERVATIVE_BFTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    std::map<std::string, std::string> one_hosts_ten_cores_1min,
        two_hosts_ten_cores_1min, two_hosts_ten_cores_2min,
    ...
    for (auto job : set_of_jobs) {
      ...
      if (std::abs(estimated - expected) > 1.0) {
        throw std::runtime_error("invalid prediction for job '" + id +
                                 "': got " + std::to_string(estimated) +
                                 " but expected is " +
...
#ifdef ENABLE_BATSCHED
TEST_F(BatchServiceCONSERVATIVE_BFTest,
       DISABLED_SimpleCONSERVATIVE_BFQueueWaitTimePrediction)
#else
TEST_F(BatchServiceCONSERVATIVE_BFTest,
       SimpleCONSERVATIVE_BFQueueWaitTimePrediction)
#endif
{
  DO_TEST_WITH_FORK(do_SimpleCONSERVATIVE_BFQueueWaitTimePrediction_test);
}

void BatchServiceCONSERVATIVE_BFTest::
    do_SimpleCONSERVATIVE_BFQueueWaitTimePrediction_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimpleCONSERVATIVE_BFQueueWaitTimePredictionWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    std::map<std::string, std::string> job1, job2, job3, job4, job5;
    ...
    for (auto const &i : actual_completion_times) {
      ...
      double completion_time = i.second.second;
      WRENCH_INFO(
          "COMPLETION TIME %s (%s nodes, %s seconds): %lf", i.first.c_str(),
          job->getServiceSpecificArguments()["-N"].c_str(),
          job->getServiceSpecificArguments()["-t"].c_str(), completion_time);
    }

    //        // Check
    //        for (int i=0; i < 5; i++) {
    //            double delta = std::abs(actual_completion_times[i] -
    //            expected_completion_times[i]); if (delta > EPSILON) {
    //                throw std::runtime_error("Unexpected job completion time
    //                for the job containing task1 " +
    //                                         tasks[i]->getID() +
    //                                         ": " +
    //                                         std::to_string(actual_completion_times[i])
    //                                         +
...
void BatchServiceCONSERVATIVE_BFTest::do_BatschedBroken_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new BatschedBrokenTestWMS(this, hostname)));
...
class SimpleCONSERVATIVE_BF_CORE_LEVELTestWMS
    : public wrench::ExecutionController {
  ...
  int main() override {
    ...
    for (int i = 0; i < num_jobs_to_submit; i++) {
      ...
      if (std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(event)) {
        ...
        actual_completion_times[job_index] =
            wrench::Simulation::getCurrentSimulatedDate();
      } else {
        throw std::runtime_error("Unexpected workflow execution event: " +
                                 event->toString());
...
#ifdef ENABLE_BATSCHED
TEST_F(BatchServiceCONSERVATIVE_BFTest,
       DISABLED_SimpleCONSERVATIVE_BF_CORE_LEVELTest)
#else
...
void BatchServiceCONSERVATIVE_BFTest::
    do_SimpleCONSERVATIVE_BF_CORE_LEVEL_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimpleCONSERVATIVE_BF_CORE_LEVELTestWMS(this, hostname)));
...
class LargeCONSERVATIVE_BF_CORE_LEVELTestWMS
    : public wrench::ExecutionController {
...
#ifdef ENABLE_BATSCHED
TEST_F(BatchServiceCONSERVATIVE_BFTest,
       DISABLED_LargeCONSERVATIVE_BFCORE_LEVELTest)
#else
...
void BatchServiceCONSERVATIVE_BFTest::do_LargeCONSERVATIVE_BF_CORE_LEVEL_test(
    int seed) {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new LargeCONSERVATIVE_BF_CORE_LEVELTestWMS(this, hostname)));
