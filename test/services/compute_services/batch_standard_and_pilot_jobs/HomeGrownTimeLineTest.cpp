...
WRENCH_LOG_CATEGORY(availability_timeline_test,
                    "Log category for AvailabilityTimeLineTest");
...
public:
  NodeAvailabilityTimelineTestWMS(BatchServiceAvailabilityTimeLineTest *test,
...
private:
  ...
  int main() override {
    ...
    // Note that there is check on the validity of the interval in terms of the
    // number of nodes
    //        auto wj2 =
    //        std::shared_ptr<wrench::CompoundJob>((wrench::CompoundJob
    //        *)(1234), [](void *ptr){});
    std::shared_ptr<wrench::CompoundJob> wj2 =
        job_manager->createCompoundJob("wj2");
    auto bj2 =
        std::make_shared<wrench::BatchJob>(wj2, 2, 20, 20, 1, "who", 0, 0);
    ...
    //        std::shared_ptr<wrench::CompoundJob>((wrench::CompoundJob
    //        *)(1234), [](void *ptr){});
    std::shared_ptr<wrench::CompoundJob> wj3 =
        job_manager->createCompoundJob("wj3");
    auto bj3 =
        std::make_shared<wrench::BatchJob>(wj2, 3, 20, 20, 1, "who", 0, 0);
...
void BatchServiceAvailabilityTimeLineTest::
    do_NodeAvailabilityTimeLineTest_test() {
  ...
  // Create a WMS
  ASSERT_NO_THROW(
      simulation->add(new NodeAvailabilityTimelineTestWMS(this, hostname)));
...
public:
  explicit CoreAvailabilityTimelineTestWMS(const std::string &hostname)
...
void BatchServiceAvailabilityTimeLineTest::
    do_CoreAvailabilityTimeLineTest_test() {
  ...
  // Create a WMS
  ASSERT_NO_THROW(
      simulation->add(new CoreAvailabilityTimelineTestWMS(hostname)));
