...
WRENCH_LOG_CATEGORY(
    virtualized_cluster_service_resource_allocation_test,
    "Log category for VirtualizedClusterServiceResourceAllocationTest");
...
public:
  ...
  std::shared_ptr<wrench::CloudComputeService> cloud_service_first_fit =
      nullptr;
  std::shared_ptr<wrench::CloudComputeService>
      cloud_service_best_fit_ram_first = nullptr;
  std::shared_ptr<wrench::CloudComputeService>
      cloud_service_best_fit_cores_first = nullptr;
...
protected:
  ...
  VirtualizedClusterServiceResourceAllocationTest() {
    ...
    std::string xml =
        ...
        "             <prop id=\"mount\" value=\"/\"/>"
        "          </disk>"
        "          <disk id=\"scratch_disk1\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"100B\"/>"
        "             <prop id=\"mount\" value=\"/scratch1\"/>"
        "          </disk>"
        "          <disk id=\"scratch_disk2\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"100B\"/>"
        "             <prop id=\"mount\" value=\"/scratch2\"/>"
        "          </disk>"
        "          <disk id=\"scratch_disk3\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"100B\"/>"
        ...
        "       <route src=\"Gateway\" dst=\"4Cores10RAM\"> <link_ctn "
        "id=\"1\"/> </route>"
        "       <route src=\"Gateway\" dst=\"2Cores20RAM\"> <link_ctn "
        "id=\"1\"/> </route>"
        "   </zone> "
        "</platform>";
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error("FirstFit Fail Case #1: Starting the 2nd VM "
                               "should have caused a NotEnoughResources error");
    ...
    try {
      ...
      throw std::runtime_error("FirstFit Fail Case #2: Starting the 3rd VM "
                               "should have caused a NotEnoughResources error");
    } catch (wrench::ExecutionException &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "BestFitCoresFirst Fail Case #1: Starting the 2nd VM should have "
          "caused a NotEnoughResources error");
    } catch (wrench::ExecutionException &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "BestFitRAMFirst Fail Case #1: Starting the 2nd VM should have "
          "caused a NotEnoughResources error");
    } catch (wrench::ExecutionException &e) {
    }
...
TEST_F(VirtualizedClusterServiceResourceAllocationTest,
       VMResourceAllocationAlgorithm) {
  DO_TEST_WITH_FORK(do_VMResourceAllocationAlgorithm_test);
}

void VirtualizedClusterServiceResourceAllocationTest::
    do_VMResourceAllocationAlgorithm_test() {
  ...
  cloud_service_first_fit = simulation->add(new wrench::CloudComputeService(
      hostname, compute_hosts, {"/scratch1"},
      {{wrench::CloudComputeServiceProperty::VM_RESOURCE_ALLOCATION_ALGORITHM,
        "first-fit"}}));

  cloud_service_best_fit_ram_first = simulation->add(
      new wrench::CloudComputeService(hostname, compute_hosts, {"/scratch2"},
                                      {{wrench::CloudComputeServiceProperty::
                                            VM_RESOURCE_ALLOCATION_ALGORITHM,
                                        "best-fit-ram-first"}}));

  cloud_service_best_fit_cores_first = simulation->add(
      new wrench::CloudComputeService(hostname, compute_hosts, {"/scratch3"},
                                      {{wrench::CloudComputeServiceProperty::
                                            VM_RESOURCE_ALLOCATION_ALGORITHM,
                                        "best-fit-cores-first"}}));
