...
WRENCH_LOG_CATEGORY(virtualized_cluster_service_test,
                    "Log category for VirtualizedClusterServiceTest");
...
public:
  ...
  std::shared_ptr<wrench::VirtualizedClusterComputeService> compute_service =
      nullptr;
...
protected:
  ...
  VirtualizedClusterServiceTest() {
    ...
    std::string xml =
        ...
        "id=\"1\"/> </route>"
        "       <route src=\"TinyHost\" dst=\"QuadCoreHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "       <route src=\"DualCoreHost\" dst=\"TinyHost\"> <link_ctn "
        "id=\"1\"/> </route>"
        "   </zone> "
        "</platform>";
...
void VirtualizedClusterServiceTest::do_ConstructorTest_test() {
  ...
  ASSERT_THROW(cloud_compute_service =
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to interact with a non-existent VM");
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to submit a standard job "
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to submit a pilot job "
                               "directly to the Cloud service");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to pass ALL_CORES to createVM()");
    } catch (std::invalid_argument &ignore) {
    }
    try {
      auto vm_name = cs->createVM(2, wrench::ComputeService::ALL_RAM);
      throw std::runtime_error(
          "Should not be able to pass ALL_RAM to createVM()");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to shutdown a non-running VM");
    } catch (wrench::ExecutionException &e) {
      auto cause = std::dynamic_pointer_cast<wrench::NotAllowed>(e.getCause());
      if (not cause) {
        throw std::runtime_error(
            "Invalid failure cause: " + e.getCause()->toString() +
            " (expected: NotAllowed)");
      }
      auto error_msg = cause->toString(); // coverage
      if (cause->getService() != cs) {
        throw std::runtime_error(
            "Failure cause does not point to the (correct) service");
      }
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to get a Compute Service for a bogus VM");
    } catch (std::invalid_argument &ignore) {
    }
    try {
      cs->getVMPhysicalHostname("bogus");
      throw std::runtime_error(
          "Should not be able to get a Physical Hostname for a bogus VM");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    if (cs->getVMComputeService(vm_name) != nullptr) {
      throw std::runtime_error(
          "A non-started VM should have a nullptr compute service");
    }
    ...
    if (vm_cs != vm_cs_should_be_same) {
      throw std::runtime_error(
          "It should be possible to get the computer service of a started VM");
    }
    ...
    try {
      cs->shutdownVM("bogus");
      throw std::runtime_error(
          "Should not be able to shutdown a non-existing VM");
    } catch (std::invalid_argument &e) {
    }
...
void VirtualizedClusterServiceTest::do_StandardJobTaskTest_test() {
  ...
  std::vector<std::string> execution_hosts = {
      wrench::Simulation::getHostnameList()[1]};
  ASSERT_NO_THROW(cloud_compute_service =
                      simulation->add(new wrench::CloudComputeService(
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new CloudStandardJobTestWMS(this, hostname)));
...
class VirtualizedClusterVMMigrationTestWMS
  ...
  int main() override {
    ...
    try {
      ...
      try {
        ...
        throw std::runtime_error(
            "Shouldn't be able to create a VM on a bogus host");
      } catch (std::invalid_argument &e) {
      }
      ...
      try {
        ...
        throw std::runtime_error(
            "Shouldn't be able to pass ALL_CORES to createVM()");
      } catch (std::invalid_argument &ignore) {
      }
      ...
      try {
        ...
        throw std::runtime_error(
            "Shouldn't be able to pass ALL_RAM to createVM()");
      } catch (std::invalid_argument &ignore) {
      }
      ...
      try {
        cs->startVM(vm_name);
        throw std::runtime_error(
            "Shouldn't be able to start a VM that is not DOWN");
      } catch (wrench::ExecutionException &e) {
      }
      ...
      // migrating the VM
      wrench::Simulation::sleep(0.01); // TODO: Without this sleep, the test
                                       // hangs! This is being investigated...

      try { // try a bogus one for coverage
        cs->migrateVM("NON-EXISTENT", "DualCoreHost");
        throw std::runtime_error(
            "Should not be able to migrate a non-existent VM");
      } catch (std::invalid_argument &e) {
      }

      try { // try a bogus one for coverage
        cs->migrateVM(vm_name, "TinyHost");
        throw std::runtime_error("Should not be able to migrate a VM to a host "
      ...
      if (hostname_pre != src_host) {
        throw std::runtime_error("VM should be running on physical host " +
                                 src_host);
      }
      ...
      if (hostname_post != dst_host) {
        throw std::runtime_error(
            "VM should, after migration, be running on physical host " +
            dst_host);
      }
...
void VirtualizedClusterServiceTest::do_VMMigrationTest_test() {
  ...
  ASSERT_THROW(compute_service =
                   simulation->add(new wrench::VirtualizedClusterComputeService(
                       hostname, nothing, "/scratch", {})),
               std::invalid_argument);
...
private:
  ...
  int main() override {
    try {
      // no VMs
      unsigned long sum_num_cores =
          this->test->cloud_compute_service->getTotalNumCores();

      unsigned long sum_num_idle_cores =
          this->test->cloud_compute_service->getTotalNumIdleCores();

      if (sum_num_cores != 6 || sum_num_idle_cores != 6) {
        throw std::runtime_error(
            "getTotalNumCores() and getTotalNumIdleCores() should be 6 (they "
            "report " +
            std::to_string(sum_num_cores) + " and " +
      ...
      if (sum_num_idle_cores != 4) {
        throw std::runtime_error(
            "getTotalNumIdleCores() should be 4 (it is reported as " +
            std::to_string(sum_num_idle_cores) + ")");
      }
      ...
      if (sum_num_idle_cores != 2) {
        throw std::runtime_error(
            "getTotalNumCores() and getTotalNumIdleCores() should be 2 (it is "
            "reported as " +
            std::to_string(sum_num_idle_cores) + ")");
      }
...
void VirtualizedClusterServiceTest::do_NumCoresTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new CloudNumCoresTestWMS(this, hostname)));
...
public:
  ...
  StopAllVMsTestWMS(VirtualizedClusterServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {
...
void VirtualizedClusterServiceTest::do_StopAllVMsTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new StopAllVMsTestWMS(this, hostname)));
...
public:
  ...
  ShutdownVMTestWMS(VirtualizedClusterServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {
...
private:
  ...
  int main() override {
    ...
    file_locations[this->test->output_file1] = wrench::FileLocation::LOCATION(
        this->test->storage_service, this->test->output_file1);
    auto job =
        job_manager->createStandardJob(this->test->task1, file_locations);

    // Submit a job
    try {
      job_manager->submitJob(job, cs->getVMComputeService(*(vm_list.begin())));
      throw std::runtime_error("should have thrown an exception since there "
                               "are no resources available");
    } catch (wrench::ExecutionException &e) {
      // do nothing, should have thrown an exception since there are no
      // resources available
    }
    ...
    try {
      ...
      if (not cs->isVMSuspended(vm_list[3])) {
        throw std::runtime_error(
            "A just suspended VM should be in the suspended state");
      }
    ...
    try {
      cs->suspendVM(vm_list[3]);
      throw std::runtime_error(
          "Should not be able to suspend a non-running VM");
    } catch (wrench::ExecutionException &e) {
      auto cause = std::dynamic_pointer_cast<wrench::NotAllowed>(e.getCause());
      if (not cause) {
    ...
    try {
      ...
      if (not cs->isVMRunning(vm_list[3])) {
        throw std::runtime_error(
            "A just resumed VM should be in the running state");
      }
    } catch (std::runtime_error &e) {
      throw std::runtime_error(e.what());
    }
    ...
    try {
      cs->resumeVM(vm_list[3]);
      throw std::runtime_error(
          "VM was already resumed, so an exception was expected!");
    } catch (wrench::ExecutionException &e) {
      // Expected
    }
    ...
    file_locations[this->test->input_file] = wrench::FileLocation::LOCATION(
        this->test->storage_service, this->test->input_file);
    file_locations[this->test->output_file2] = wrench::FileLocation::LOCATION(
        this->test->storage_service, this->test->output_file2);
    auto other_job =
        job_manager->createStandardJob(this->test->task2, file_locations);
    ...
    try {
      WRENCH_INFO(
          "Suspending the one running VM (which is thus running the job)");
      cs->suspendVM(vm_list[3]);
    } catch (wrench::ExecutionException &e) {
    ...
    double job_turnaround_time =
        wrench::Simulation::getCurrentSimulatedDate() - job_start_date;
    if (std::abs(job_turnaround_time - 110) > EPSILON) {
      throw std::runtime_error("Unexpected job turnaround time " +
                               std::to_string(job_turnaround_time));
    }
...
void VirtualizedClusterServiceTest::do_ShutdownVMTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new ShutdownVMTestWMS(this, hostname)));
...
class ShutdownVMAndThenShutdownServiceTestWMS
    : public wrench::ExecutionController {
...
private:
  ...
  int main() override {
    ...
    std::vector<std::tuple<std::string,
                           std::shared_ptr<wrench::BareMetalComputeService>>>
        vm_list;
...
public:
  ...
  SubmitToVMTestWMS(VirtualizedClusterServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {
    this->setMessagePayloads(this->default_messagepayload_values, {});
  }
...
private:
  ...
  int main() override {
    ...
    //        std::vector<std::tuple<std::string,
    //        std::shared_ptr<wrench::BareMetalComputeService>>> vm_list;
    std::vector<std::string> vm_list;
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to run job since VMs are stopped");
    } catch (wrench::ExecutionException &e) {
      auto cause =
    ...
    try {
      ...
      throw std::runtime_error("Couldn't start VM: " +
                               e.getCause()->toString());
    }
...
void VirtualizedClusterServiceTest::do_SubmitToVMTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new SubmitToVMTestWMS(this, hostname)));
...
class CloudServiceVMStartShutdownStartShutdownTestWMS
    : public wrench::ExecutionController {

public:
  CloudServiceVMStartShutdownStartShutdownTestWMS(
      VirtualizedClusterServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    try {
      cloud_service->destroyVM(vm_name);
      throw std::runtime_error(
          "Shouldn't be able to destroy an already destroyed VM");
    } catch (std::invalid_argument &e) {
    }
...
void VirtualizedClusterServiceTest::do_VMStartShutdownStartShutdown_test() {
  ...
  cloud_compute_service = simulation->add(
      new wrench::CloudComputeService(hostname, compute_hosts, "/scratch", {}));
...
class CloudServiceVMShutdownWhileJobIsRunningTestWMS
    : public wrench::ExecutionController {

public:
  CloudServiceVMShutdownWhileJobIsRunningTestWMS(
      VirtualizedClusterServiceTest *test, std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    auto cause =
        std::dynamic_pointer_cast<wrench::JobKilled>(real_event->failure_cause);
    if (not cause) {
      throw std::runtime_error(
          "Unexpected failure cause: " + real_event->failure_cause->toString() +
          " (expected: JobKilled)");
    }
    if (cause->getJob() != job) {
      throw std::runtime_error(
          "Failure cause does not point to the correct job");
    }
...
class CloudServiceVMComputeServiceStopWhileJobIsRunningTestWMS
    : public wrench::ExecutionController {

public:
  CloudServiceVMComputeServiceStopWhileJobIsRunningTestWMS(
      VirtualizedClusterServiceTest *test, std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    vm_cs->stop(true, wrench::ComputeService::TerminationCause::
    ...
    if (not real_event) {
      throw std::runtime_error(
          "Unexpected workflow execution event: " + event->toString() +
          " (should be STANDARD_JOB_FAILURE)");
    }
    ...
    wrench::Simulation::sleep(
        0.0001); // To let the cloud CS to update the VM state

    // Check that the VM is down, as it should
    if (not cloud_service->isVMDown(vm_name)) {
      throw std::runtime_error("VM should be down after its "
                               "bare_metal_standard_jobs has been stopped");
    }
...
void VirtualizedClusterServiceTest::
    do_VMComputeServiceStopWhileJobIsRunning_test() {
  // Create and initialize a simulation
  auto simulation = wrench::Simulation::createSimulation();
  int argc = 1;
  auto argv = (char **)calloc(argc, sizeof(char *));
  ...
  wms = simulation->add(
      new CloudServiceVMComputeServiceStopWhileJobIsRunningTestWMS(this,
                                                                   hostname));
