...
WRENCH_LOG_CATEGORY(file_registry_service_test,
                    "Log category for File Registry Service test");
...
protected:
  ...
  FileRegistryTest() {
    ...
    std::string xml =
        ...
        "          </disk>"
        "          <disk id=\"scratch_large_disk\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
...
public:
  FileRegistryTestWMS(FileRegistryTest *test, std::string hostname)
...
private:
  ...
  int main() override {
    ...
    try {
      frs->addEntry(wrench::FileLocation::LOCATION(this->test->storage_service1,
                                                   nullptr));
      throw std::runtime_error(
          "Should not be able to add an entry with a nullptr file");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to add an entry with a nullptr storage service");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to lookup an entry with a nullptr file");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not have gotten an exception when looking up entry");
    }
    ...
    if ((*locations.begin())->getStorageService() !=
    ...
    try {
      frs->removeEntry(wrench::FileLocation::LOCATION(
          this->test->storage_service1, nullptr));
      throw std::runtime_error(
          "Should not be able to remove an entry with a nullptr file");
    } catch (std::invalid_argument &e) {
    }

    try {
      frs->removeEntry(wrench::FileLocation::LOCATION(nullptr, file1));
      throw std::runtime_error("Should not be able to remove an entry with a "
                               "nullptr storage service");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      frs->removeEntry(
          wrench::FileLocation::LOCATION(this->test->storage_service1, file1));
      throw std::runtime_error(
          "Should not be able to remove an entry from a service that is down");
...
void FileRegistryTest::do_FileRegistry_Test() {
  ...
  ASSERT_NO_THROW(wms =
                      simulation->add(new FileRegistryTestWMS(this, hostname)));
...
public:
  FileRegistryLookupEntryTestWMS(FileRegistryTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    std::shared_ptr<wrench::DataFile> file1 =
        wrench::Simulation::addFile("file1", 100.0);
    ...
    frs->addEntry(
        wrench::FileLocation::LOCATION(this->test->storage_service2, file1));
    frs->addEntry(
        wrench::FileLocation::LOCATION(this->test->storage_service3, file1));

    wrench::S4U_Simulation::sleep(600.0);

    std::vector<std::string> file1_expected_locations = {"Host4", "Host1",
                                                         "Host2"};
    std::vector<std::string> file1_locations(3);
    std::map<double, std::shared_ptr<wrench::FileLocation>>
        file1_locations_by_proximity;
    ...
    for (auto &storage_service : file1_locations_by_proximity) {
      file1_locations[count++] =
          storage_service.second->getStorageService()->getHostname();
    }

    bool is_equal =
        std::equal(file1_expected_locations.begin(),
                   file1_expected_locations.end(), file1_locations.begin());

    if (!is_equal) {
      throw std::runtime_error(
          "lookupEntry using NetworkProximityService did not return Storage "
          "Services in ascending order of Network Proximity");
    }

    auto last_location = file1_locations_by_proximity.rbegin();
    if (last_location->second->getStorageService()->getHostname() != "Host2") {
      throw std::runtime_error(
          "lookupEntry using NetworkProximityService did not return the "
          "correct unmonitored Storage Service");
    } else if (last_location->first != DBL_MAX) {
      throw std::runtime_error("lookupEntry using NetworkProximityService did "
                               "not include the unmonitored Storage Service");
    }

    // Use a bogus host
    try {
      frs->lookupEntry(file1, "BogusHost", nps);
      throw std::runtime_error("Should not be able to lookup a file by "
                               "proximity with a bogus reference host");
    } catch (std::invalid_argument &e) {
    }
...
void FileRegistryTest::do_lookupEntry_Test() {
  ...
  network_proximity_service = simulation->add(
      new wrench::NetworkProximityService(host1, {host1, host3, host4}));
  ...
  wms = simulation->add(new FileRegistryLookupEntryTestWMS(this, host1));
