...
WRENCH_LOG_CATEGORY(xrootd_storage_service_basic_functional_test,
                    "Log category for XRootDServiceBasicFunctionalTest");
...
protected:
  ...
  std::string platform =
      ...
      "       <route src=\"Host1\" dst=\"Host2\"> <link_ctn id=\"link12\"/> "
      "</route>"
      "       <route src=\"Host1\" dst=\"Host3\"> <link_ctn id=\"link13\"/> "
      "</route>"
      "       <route src=\"Host2\" dst=\"Host3\"> <link_ctn id=\"link23\"/> "
      "</route>"
      "   </zone> "
      "</platform>";
};
...
class XRootDServiceBasicFunctionalityTestExecutionController
    : public wrench::ExecutionController {

public:
  XRootDServiceBasicFunctionalityTestExecutionController(
      XRootDServiceBasicFunctionalTest *test, std::string hostname)
...
private:
  ...
  int main() override {
    ...
    this->test->root_supervisor->getChild(0)->createFile(
        wrench::FileLocation::LOCATION(
            this->test->root_supervisor->getChild(0)->getStorageServer(),
            "/disk100", file3));
    ...
    try {
      ...
      throw std::runtime_error(
          "Non extant files should throw exceptions when not found");
    } catch (wrench::ExecutionException &ignore) {
      if (not std::dynamic_pointer_cast<wrench::FileNotFound>(
              ignore.getCause())) {
        throw std::runtime_error(
            "Should have received a FileNotFound execution when reading file2 "
            "from supervisor");
      }
    }
    ...
    try {
      ...
      if (not std::dynamic_pointer_cast<wrench::FileNotFound>(
              ignore.getCause())) {
        throw std::runtime_error("Should have received a FileNotFound "
                                 "execution when reading file2 from child");
      }
    }

    if (!this->test->root_supervisor->lookupFile(file1))
      throw std::runtime_error("File that exists not located - cached");
    if (!this->test->root_supervisor->getChild(0)->lookupFile(file3))
      throw std::runtime_error("File that exists not located - direct");
    if (!this->test->root_supervisor->lookupFile(file3))
      throw std::runtime_error("File that exists not located - uncached");
    if (this->test->root_supervisor->lookupFile(file2))
      throw std::runtime_error("File that does not exist located - indirect");
    if (this->test->root_supervisor->getChild(0)->lookupFile(file2))
      throw std::runtime_error("File that does not exist located - direct");
    ...
    try {
      ...
      if (not std::dynamic_pointer_cast<wrench::FileNotFound>(
              ignore.getCause())) {
        throw std::runtime_error(
            "Should have received a FileNotFound execution when reading file1");
      }
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to write a file directly on a supervisor node");
    } catch (wrench::ExecutionException &ignore) {
    ...
    if (!this->test->root_supervisor->getChild(1)->lookupFile(file4)) {
      throw std::runtime_error(
          "It seems that file4 was never copied to child 1 from child 0");
    }
    ...
    if (!this->test->standalone_ss->lookupFile(file5)) {
      throw std::runtime_error(
          "It seems that file5 was never copied to standalone ss from child 0");
    }
    ...
    if (!this->test->root_supervisor->getChild(1)->lookupFile(file6)) {
      throw std::runtime_error(
          "It seems that file6 was never copied from standalone ss to child 1");
    }

    // Copy a file from Supervisor to stand-alone storage service (which is not
    // alloweD)
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be allowed to copy from supervisor to storage service");
    } catch (wrench::ExecutionException &e) {
      if (not std::dynamic_pointer_cast<wrench::NotAllowed>(e.getCause())) {
        throw std::runtime_error(
            "Got expected exception, but wrong failure cause");
      }
    }
    ...
    if (this->test->root_supervisor->getChild(0)->getParent() !=
        this->test->root_supervisor.get()) {
    ...
    // TODO not really sure a good way to test if load is correct but this
    // covers, and root should be 0
...
void XRootDServiceBasicFunctionalTest::do_BasicFunctionality_test(
    std::string arg) {
  ...
  wrench::XRootD::Deployment xrootd_deployment(
      ...
      {{wrench::XRootD::Property::CACHE_MAX_LIFETIME, "28800"},
       {wrench::XRootD::Property::REDUCED_SIMULATION, arg},
       {wrench::XRootD::Property::FILE_NOT_FOUND_TIMEOUT, "10"}},
  ...
  ASSERT_THROW(this->root_supervisor =
                   xrootd_deployment.createRootSupervisor("Host1"),
               std::runtime_error);

  auto ss2 = this->root_supervisor->addChildStorageServer(
      "Host2", "/disk100",
      {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "10MB"}}, {});
  auto ss3 = this->root_supervisor->addChildStorageServer(
      "Host3", "/disk100",
      {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "10MB"}}, {});
  ...
  auto controller = simulation->add(
      new XRootDServiceBasicFunctionalityTestExecutionController(this,
                                                                 "Host1"));
