...
WRENCH_LOG_CATEGORY(xrootd_storage_service_full_functional_test,
                    "Log category for XRootDServiceFullFunctionalTest");
...
std::string formatDate(double time) {
  ...
  return std::to_string(days) + "-" + padLong(hours) + ':' + padLong(minutes) +
         ':' + padDouble((double)seconds + ms);
}
...
class XRootDServiceFullFunctionalityTestExecutionController
    : public wrench::ExecutionController {

public:
  XRootDServiceFullFunctionalityTestExecutionController(
...
private:
  ...
  int main() override {
    ...
    auto location =
        wrench::FileLocation::LOCATION(root->getChild(0), files[0]); // coverage
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to reserve space on a non-storage node");
    } catch (std::runtime_error &ignore) {
    }
    try {
      root->unreserveSpace(location);
      throw std::runtime_error(
          "Shouldn't be able to unreserve space on a non-storage node");
    } catch (std::runtime_error &ignore) {
    }
    try {
      root->removeFile(files[0]);
      throw std::runtime_error(
          "Shouldn't be able to call removeFile() on a non-storage node");
    } catch (std::runtime_error &ignore) {
    }
    ...
    root->getChild(2)->getChild(2)->getChild(2)->getChild(0)->createFile(
        files[6]); // leaf 7
    root->getChild(2)->getChild(2)->getChild(2)->getChild(1)->createFile(
        files[7]); // leaf 8
    root->getChild(2)
        ->getChild(2)
        ->getChild(2)
        ->getChild(2)
        ->getChild(0)
        ->createFile(files[8]); // leaf 9
    root->getChild(2)
        ->getChild(2)
        ->getChild(2)
        ->getChild(2)
        ->getChild(1)
        ->createFile(files[9]); // leaf 10
    root->getChild(2)
        ->getChild(2)
        ->getChild(2)
        ->getChild(2)
        ->getChild(2)
        ->createFile(files[10]); // leaf 11
    ...
    root->getChild(2)
        ...
        ->getChild(2)
        ->getChild(2)
        ->createFile(files[12]); // File for supervisor tests
    ...
    root->getChild(2)
        ...
        ->getChild(2)
        ->getChild(2)
        ->createFile(files[13]); // File for direct tests

    try {
      root->createFile(files[0]);
      throw std::runtime_error(
          "Should not be able to create a file on a non-storage node");
    } catch (std::runtime_error &ignore) {
    }
    ...
    auto fileread1 =
        job1->addFileReadAction("fileread1", files[0], root); // should be fast
    auto fileread2 = job1->addFileReadAction("fileread2", files[1],
                                             root); // should be equally fast
    auto fileread3 =
        job1->addFileReadAction("fileread3", files[2], root); // depth 1 search
    auto fileread4 =
        job1->addFileReadAction("fileread4", files[3], root); // depth 1 search
    auto fileread5 =
        job1->addFileReadAction("fileread5", files[4], root); // depth 2 search
    auto fileread6 =
        job1->addFileReadAction("fileread6", files[5], root); // depth 2 search
    auto fileread7 =
        job1->addFileReadAction("fileread7", files[6], root); // depth 3 search
    auto fileread8 =
        job1->addFileReadAction("fileread8", files[7], root); // depth 3 search
    auto fileread9 =
        job1->addFileReadAction("fileread9", files[8], root); // depth 4 search
    auto fileread10 =
        job1->addFileReadAction("fileread10", files[9], root); // depth 4 search
    auto fileread11 = job1->addFileReadAction("fileread11", files[10],
                                              root); // depth 4 search
    auto fileread12 = job1->addFileReadAction("fileread12", files[11],
                                              root); // this file does not exist
    auto fileread13 = job1->addFileReadAction(
        "fileread13", files[10], root); // depth 4 search, but cached

    auto fileread14 = job1->addFileReadAction("fileread14", files[11],
                                              root); // this file does not exist

    auto fileread15 = job1->addFileReadAction(
        "fileread15", files[10], root); // depth 4 search, but no longer cached
    auto fileread16 = job1->addFileReadAction(
        "fileread16", files[12],
        root->getChild(2)->getChild(2)->getChild(2)->getChild(
            2)); // check superviosor
    auto fileread17 = job1->addFileReadAction(
        "fileread17", files[12],
        root->getChild(2)->getChild(2)->getChild(2)->getChild(
            2)); // check superviosor cached
    auto fileread18 = job1->addFileReadAction(
        "fileread18", files[13],
        root->getChild(2)->getChild(2)->getChild(2)->getChild(2)->getChild(
            2)); // direct
    auto fileread19 = job1->addFileReadAction(
        "fileread19", files[13],
        root->getChild(2)->getChild(2)->getChild(2)->getChild(2)->getChild(
            2)); // direct cached
    ...
    auto compute = job1->addComputeAction(
        "compute", 500 * GFLOP, 50 * MBYTE, 1, 3,
        wrench::ParallelModel::AMDAHL(0.8)); // should invalidate cache
    ...
    job1->addActionDependency(fileread12,
                              fileread14); // this task should never start
    ...
    std::vector<std::shared_ptr<wrench::Action>> actions = {
        fileread1,  fileread2,  fileread3,  fileread4,  fileread5,
        fileread6,  fileread7,  fileread8,  fileread9,  fileread10,
        fileread11, fileread13, compute,    fileread12, fileread15,
        fileread16, fileread17, fileread18, fileread19, fileread14};
    std::vector<std::string> comments = {
        ...
        "depth 4 search",
        "Depth 4, BUT cached",
        "This long compute should invalidate the caches",
        "This file Does not exist",
        "depth 4, file should no longer be cached",
        "depth 4, but directly from supervisor, should be depth 1",
        "repeat but cached",
        "direct leaf access",
        "direct leaf access but cached",
  ...
  void processEventCompoundJobCompletion(
      std::shared_ptr<wrench::CompoundJobCompletedEvent> event) override {
    /* Retrieve the job that this event is for */
    auto job = event->job;
    /* Print info about all actions in the job */
    WRENCH_INFO("Notified that compound job %s has completed:",
                job->getName().c_str());
  }
};
...
void XRootDServiceFullFunctionalTest::do_FullFunctionality_test() {
  ...
  auto baremetal_service = simulation->add(
      new wrench::BareMetalComputeService("user", {"user"}, "", {}, {}));
  simulation->add(baremetal_service);

  /* Create an XRootD manager object, with a couple of configuration properties.
   * The REDUCED_SIMULATION property, when set to "true" ("false" is the
   * default), abstracts away more simulation details, which speeds up the
   * simulation but may make it less realistic. See the documentation for more
   * details.
   */
  wrench::XRootD::Deployment xrootd_deployment(
      simulation,
...
*/
  ...
  auto controller =
      simulation->add(new XRootDServiceFullFunctionalityTestExecutionController(
          this, &xrootd_deployment, baremetal_service, "root"));
