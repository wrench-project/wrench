...
WRENCH_LOG_CATEGORY(Compound_storage_service_functional_test,
                    "Log category for CompoundStorageServiceFunctionalTest");
...
public:
  ...
  // External storage
  std::shared_ptr<wrench::SimpleStorageService>
      simple_storage_service_external = nullptr;

  // Services for disks of node SimpleStorageHost0
  std::shared_ptr<wrench::SimpleStorageService> simple_storage_service_100_0 =
      nullptr;
  std::shared_ptr<wrench::SimpleStorageService> simple_storage_service_510_0 =
      nullptr;
  std::shared_ptr<wrench::SimpleStorageService> simple_storage_service_1000_0 =
      nullptr;
  // Services for disks of node SimpleStorageHost1
  std::shared_ptr<wrench::SimpleStorageService> simple_storage_service_100_1 =
      nullptr;
  std::shared_ptr<wrench::SimpleStorageService> simple_storage_service_510_1 =
      nullptr;
  std::shared_ptr<wrench::SimpleStorageService> simple_storage_service_1000_1 =
      nullptr;
...
protected:
  ...
  CompoundStorageServiceFunctionalTest() {
    ...
    // Create a three-hosts platform file (2 for simple storage, one for
    // Compound Storage)
    std::string xml =
        "<?xml version='1.0'?>"
        ...
        "          </disk>"
        "          <disk id=\"other_other_large_disk\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        ...
        "       <route src=\"CompoundStorageHost\" "
        "dst=\"SimpleStorageHost0\"><link_ctn id=\"Link1\"/></route> "
        "       <route src=\"CompoundStorageHost\" "
        "dst=\"SimpleStorageHost1\"><link_ctn id=\"Link1\"/></route> "
        "       <route src=\"CompoundStorageHost\" "
        "dst=\"ComputeHost\"><link_ctn id=\"Link1\"/></route> "
        "       <route src=\"SimpleStorageHost0\" "
        "dst=\"ComputeHost\"><link_ctn id=\"Link1\"/></route> "
        "       <route src=\"SimpleStorageHost1\" "
        "dst=\"ComputeHost\"><link_ctn id=\"Link1\"/></route> "
        "       <route src=\"SimpleStorageHost0\" "
        "dst=\"SimpleStorageHost1\"><link_ctn id=\"Link1\"/></route> "
        "   </zone> "
...
public:
  ...
  std::vector<std::shared_ptr<wrench::FileLocation>> operator()(
    ...
    // std::cout << "# Call count 1: "<< std::to_string(call_count) <<
    // std::endl;
    auto capacity_req = file->getSize();
    std::shared_ptr<wrench::FileLocation> designated_location = nullptr;
    // std::cout << "Calling on the rrStorageSelectionStrategy for file " <<
    // file->getID() << " (" << std::to_string(file->getSize()) << "B)" <<
    // std::endl;
    auto current = resources.find(last_selected_server);
    unsigned int current_disk_selection = internal_disk_selection;
    // std::cout << "Last selected server " << last_selected_server <<
    // std::endl; std::cout << "Starting from server " << current->first <<
    // std::endl; std::cout << "Internal disk selection " <<
    // std::to_string(internal_disk_selection) << std::endl;

    auto continue_disk_loop = true;

    do {

      // std::cout << "Considering disk index " <<
      // std::to_string(current_disk_selection) << std::endl;
      auto nb_of_local_disks = current->second.size();
      auto storage_service =
          current->second[current_disk_selection % nb_of_local_disks];
      // std::cout << "- Looking at storage service " <<
      // storage_service->getName() << std::endl;

      auto free_space = storage_service->getTotalFreeSpace();
      // std::cout << "- It has " << free_space << "B of free space" <<
      // std::endl;
      ...
      if (free_space >= capacity_req) {
        ...
        // std::cout << "Chose server " << current->first <<
        // storage_service->getBaseRootPath() << std::endl; Update for next
        // function call
        ...
        // std::cout << "Next first server will be " << last_selected_server <<
        // std::endl;
        break;
      }
    ...
    // call_count++;
    // std::cout << "# Call count 2: "<< std::to_string(call_count) <<
    // std::endl;

    // std::cout << "smartStorageSelectionStrategy has done its work." <<
    // std::endl;
...
private:
  ...
  int main() override {
    ...
    wrench::StorageService::createFileAtLocation(wrench::FileLocation::LOCATION(
        test->simple_storage_service_external, "/disk1000", test->file_100));
    // Copy to CSS
    auto fileCopyAction = job->addFileCopyAction(
        "stagingCopy_1",
        wrench::FileLocation::LOCATION(test->simple_storage_service_external,
                                       "/disk1000", test->file_100),
        wrench::FileLocation::LOCATION(test->compound_storage_service,
                                       test->file_100));

    // Read from CSS
    auto fileReadAction = job->addFileReadAction(
        "fRead_1", wrench::FileLocation::LOCATION(
                       test->compound_storage_service, test->file_100));
    job->addActionDependency(fileCopyAction, fileReadAction);

    // ## Stage file on external storage (WITH STRIPPING)
    wrench::StorageService::createFileAtLocation(wrench::FileLocation::LOCATION(
        test->simple_storage_service_external, "/disk1000", test->file_500));
    // Copy to CSS
    auto fileCopyAction_2 = job->addFileCopyAction(
        "stagingCopy_2",
        wrench::FileLocation::LOCATION(test->simple_storage_service_external,
                                       "/disk1000", test->file_500),
        wrench::FileLocation::LOCATION(test->compound_storage_service,
                                       test->file_500));
    job->addActionDependency(fileReadAction, fileCopyAction_2);

    // Read from CSS
    auto fileReadAction_2 = job->addFileReadAction(
        "fRead_2", wrench::FileLocation::LOCATION(
                       test->compound_storage_service, test->file_500));
    ...
    auto read_file_copy_1 = test->compound_storage_service->lookupFileLocation(
        test->file_100, tmp_commport);
    if (read_file_copy_1.size() != 1) {
      throw std::runtime_error(
          "Lookup returned an incorrect number of parts for file_100 on CSS");
    }
    if (read_file_copy_1[0]->getStorageService()->getBaseRootPath() !=
        "/disk510/") {
      throw std::runtime_error("file_100 should be on /disk510/");
    }

    auto read_file_copy_2 = test->compound_storage_service->lookupFileLocation(
        test->file_500, tmp_commport);
    if (read_file_copy_2.size() != 2) {
      throw std::runtime_error(
          "Lookup returned an incorrect number of parts for file_500 on CSS");
    }
    for (auto part : read_file_copy_2) {
      auto file_location = wrench::FileLocation::LOCATION(
          test->simple_storage_service_external, part->getFile());
      if (test->simple_storage_service_external->hasFile(file_location)) {
        throw std::runtime_error(
            "Src file part from stripping shouldn't be on source anymore");
      }
    }
    if (read_file_copy_2[0]->getStorageService()->getBaseRootPath() !=
        "/disk1000/") {
      throw std::runtime_error("file_500_part_0 should be on /disk1000/");
    }
    if (read_file_copy_2[1]->getStorageService()->getBaseRootPath() !=
        "/disk510/") {
      throw std::runtime_error("file_500_part_1 should be on /disk510/");
    }

    auto external_free_space =
        test->simple_storage_service_external->getTotalFreeSpace();
    if (external_free_space != 400) {
      throw std::runtime_error("Residual data on external free space not "
                               "cleaned up after stropped copy");
    }
    wrench::S4U_CommPort::retireTemporaryCommPort(tmp_commport);
...
void CompoundStorageServiceFunctionalTest::do_CopyToCSS_test() {
  ...
  ASSERT_NO_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          "ComputeHost",
          {std::make_pair("ComputeHost",
  ...
  ASSERT_NO_THROW(simple_storage_service_510_0 = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          "SimpleStorageHost0", {"/disk510"}, {}, {})));
  ASSERT_NO_THROW(simple_storage_service_1000_0 = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          "SimpleStorageHost0", {"/disk1000"}, {}, {})));

  ASSERT_NO_THROW(simple_storage_service_external = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          "SimpleStorageHost1", {"/disk1000"}, {}, {})));

  // Create a valid Compound Storage Service, without user provided callback (no
  // intercept capabilities) std::shared_ptr<wrench::StorageAllocator> allocator
...
private:
  ...
  int main() override {
    ...
    // Write to CSS
    auto fileWriteAction_1 = job->addFileWriteAction(
        "fWrite_1", wrench::FileLocation::LOCATION(
                        test->compound_storage_service, test->file_100));

    // Write to CSS (with stripping)
    auto fileWriteAction_2 = job->addFileWriteAction(
        "fWrite_2", wrench::FileLocation::LOCATION(
                        test->compound_storage_service, test->file_500));
    ...
    auto tmp_commport = wrench::S4U_CommPort::getTemporaryCommPort();
    // Check that all file copies worked as intended
    auto write_file_lookup_1 =
        test->compound_storage_service->lookupFileLocation(test->file_100,
                                                           tmp_commport);
    if (write_file_lookup_1.size() != 1) {
      throw std::runtime_error(
          "Lookup returned an incorrect number of parts for file_500 on CSS");
    }
    if (write_file_lookup_1[0]->getStorageService()->getBaseRootPath() !=
        "/disk510/") {
      throw std::runtime_error("file_100 should be on /disk510/");
    }

    auto write_file_lookup_2 =
        test->compound_storage_service->lookupFileLocation(test->file_500,
                                                           tmp_commport);
    if (write_file_lookup_2.size() != 2) {
      throw std::runtime_error(
          "Lookup returned an incorrect number of parts for file_500 on CSS");
    }
    if (write_file_lookup_2[0]->getStorageService()->getBaseRootPath() !=
        "/disk1000/") {
      throw std::runtime_error("file_500_part_0 should be on /disk1000/");
    }
    if (write_file_lookup_2[1]->getStorageService()->getBaseRootPath() !=
        "/disk510/") {
      throw std::runtime_error("file_500_part_1 should be on /disk510/");
    }
...
private:
  ...
  int main() override {
    ...
    auto fileCopyAction_1 = job->addFileCopyAction(
        "fCopy_1",
        wrench::FileLocation::LOCATION(test->compound_storage_service,
                                       test->file_100),
        wrench::FileLocation::LOCATION(test->simple_storage_service_external,
    ...
    if (!test->simple_storage_service_external->hasFile(
            wrench::FileLocation::LOCATION(
                test->simple_storage_service_external, "/disk1000",
                test->file_500))) {
    ...
    if (test->simple_storage_service_external->getTotalFreeSpace() != 400) {
      throw std::runtime_error("External storage doesn't have the expected "
                               "free space (should be 400)");
    }
...
private:
  ...
  int main() override {
    ...
    wrench::StorageService::createFileAtLocation(wrench::FileLocation::LOCATION(
        test->simple_storage_service_external, "/disk1000", test->file_1000));
    ...
    auto fileReadAction = job->addFileReadAction(
        "fRead_1000", wrench::FileLocation::LOCATION(
                          test->compound_storage_service, test->file_1000));
    actions.push_back(fileReadAction);
    job->addActionDependency(stagingAction, fileReadAction);
    // Compute would go here

    auto fileWriteAction = job->addFileWriteAction(
        "fWrite_500", wrench::FileLocation::LOCATION(
                          test->compound_storage_service, test->file_500));
    ...
    auto deleteExternalFileAction = job->addFileDeleteAction(
        "delExt_1000",
        wrench::FileLocation::LOCATION(test->simple_storage_service_external,
                                       test->file_1000));
    ...
    auto deleteCSS1000Action = job->addFileDeleteAction(
        "delRF_1000", wrench::FileLocation::LOCATION(
                          test->compound_storage_service, test->file_1000));
    actions.push_back(deleteCSS1000Action);
    job->addActionDependency(archiveAction, deleteCSS1000Action);

    auto deleteCSS500Action = job->addFileDeleteAction(
        "delRF_500", wrench::FileLocation::LOCATION(
                         test->compound_storage_service, test->file_500));
    ...
    if (external_free_space != 500) {
      auto exc_msg = "External free space has " +
                     std::to_string(external_free_space) +
                     "B of free space."
                     "It should have 500B";
      throw std::runtime_error(exc_msg);
    }
    ...
    if (!css_File_500.empty()) {
      throw std::runtime_error(
          "file_500 is still present on CSS, it shouldn't");
    }
...
void CompoundStorageServiceFunctionalTest::do_fullJob_test() {
  ...
  ASSERT_NO_THROW(
      compound_storage_service =
          simulation->add(new wrench::CompoundStorageService(
              "CompoundStorageHost",
              {simple_storage_service_510_0, simple_storage_service_100_1,
               simple_storage_service_1000_0},
              allocatorCallback,
              {{wrench::CompoundStorageServiceProperty::
                    MAX_ALLOCATION_CHUNK_SIZE,
                "100"}},
...
class CompoundStorageServiceBasicFunctionalityTestCtrl
    : public wrench::ExecutionController {

public:
  CompoundStorageServiceBasicFunctionalityTestCtrl(
      CompoundStorageServiceFunctionalTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    auto simple_storage_services =
        test->compound_storage_service->getAllServices();
    ...
    for (auto const &svc : simple_storage_services) {
      if (expected_services.find(svc.first) == expected_services.end()) {
        throw std::runtime_error(
            "Can't find one of the hosts in the services list");
      }
      if (expected_services[svc.first] != svc.second) {
        throw std::runtime_error(
            "One of the Storage hosts has a different list of associated disk "
            "services than expected");
      }
    }

    // Verify synchronous request for current free space (currently same as
    // capacity, as no file has been placed on internal services)
    ...
    if (free_space != expected_capacity) {
      throw std::runtime_error(
          "'Free Space' available to CompoundStorageService is incorrect");
    }
    // Verify that total space is correct
    auto capacity = test->compound_storage_service->getTotalSpace();
    if (capacity != expected_capacity) {
      throw std::runtime_error(
          "'Total Space' available to CompoundStorageService is incorrect");
    }

    // Verify that compound storage service unique mount point is DEV_NULL
    //        auto mount_point =
    //        test->compound_storage_service->getMountPoint(); if (mount_point
    //        != wrench::LogicalFileSystem::DEV_NULL + "/") {
    //            throw std::runtime_error("CompoundStorageService should have
    //            only one 'LogicalFileSystem::DEV_NULL' filesystem");
    //        }

    // We don't support getLoad or getFileLastWriteDate on
    // CompoundStorageService yet (and won't ?)
    try {
      test->compound_storage_service->getLoad();
      throw std::runtime_error(
          "CompoundStorageService doesn't have a getLoad() implemented");
    } catch (std::logic_error &e) {
    }

    {
      auto file_1_loc = wrench::FileLocation::LOCATION(
          test->compound_storage_service, test->file_1);
      try {
        test->compound_storage_service->getFileLastWriteDate(file_1_loc);
        throw std::runtime_error(
            "We shouldn't be able to get a FileLastWriteDate on a file that "
            "wasn't written to the CSS first");
      } catch (std::invalid_argument &e) {
      }
    }

    // CompoundStorageServer should never be a scratch space (at init or set as
    // later)
    if (test->compound_storage_service->isScratch() == true) {
      throw std::runtime_error(
          "CompoundStorageService should never have isScratch == true");
    }

    try {
      test->compound_storage_service->setIsScratch(true);
      throw std::runtime_error(
          "CompoundStorageService can't be setup as a scratch space");
    } catch (std::logic_error &e) {
    }

    if (test->compound_storage_service->isBufferized()) {
      throw std::runtime_error(
          "CompoundStorageService shouldn't be bufferized");
    }
    // ## Test multiple messages that should answer with a failure cause, and in
    // turn generate an ExecutionException on caller's side

    // File copy, CSS as src, file is not known by CSS:
    {
      auto file_1_loc_ss = wrench::FileLocation::LOCATION(
          test->simple_storage_service_100_0, test->file_1);
      wrench::StorageService::createFileAtLocation(file_1_loc_ss);
      auto file_1_loc_css = wrench::FileLocation::LOCATION(
          test->compound_storage_service, test->file_1);
      try {
        wrench::StorageService::copyFile(file_1_loc_css, file_1_loc_ss);
        throw std::runtime_error("File doesn't exist on CSS, copy with CSS as "
                                 "src should be impossible");
      } catch (wrench::ExecutionException &e) {
        if (!std::dynamic_pointer_cast<wrench::FileNotFound>(e.getCause())) {
          throw std::runtime_error(
              "Exception cause should have been 'FileNotFound'");
        }
      }
      test->simple_storage_service_100_0->deleteFile(file_1_loc_ss);
    }

    // File copy, CSS as dst, file can't be allocated (no callback provided, and
    // the default one doesn't return anything) - src file exists:
    {
      auto file_1_loc_ss = wrench::FileLocation::LOCATION(
          test->simple_storage_service_100_0, test->file_1);
      wrench::StorageService::createFileAtLocation(file_1_loc_ss);
      ...
      try {
        ...
        throw std::runtime_error(
            "File doesn't exist and can't be allocated on CSS, copy with CSS "
            "as dst should be impossible");
      } catch (wrench::ExecutionException &e) {
        if (!std::dynamic_pointer_cast<wrench::StorageServiceNotEnoughSpace>(
                e.getCause())) {
          throw std::runtime_error(
              "Exception cause should have been 'StorageServiceNotEnoughSpace' "
              "(which is slightly incorrect in this case)");
        }
      }
      test->simple_storage_service_100_0->deleteFile(file_1_loc_ss);
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to delete file from a CompoundStorageService if "
          "it has not first been written / copied to it");
    } catch (wrench::ExecutionException &e) {
      if (!std::dynamic_pointer_cast<wrench::FileNotFound>(e.getCause())) {
        throw std::runtime_error(
            "Exception cause should have been 'FileNotFound'");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to read file from a CompoundStorageService if it "
          "has not first been written / copied to it");
    } catch (wrench::ExecutionException &e) {
      if (!std::dynamic_pointer_cast<wrench::FileNotFound>(e.getCause())) {
        throw std::runtime_error(
            "Exception cause should have been 'FileNotFound'");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to write file on a CompoundStorageService "
          "because no selection callback was provided");
    } catch (wrench::ExecutionException &e) {
      if (!std::dynamic_pointer_cast<wrench::StorageServiceNotEnoughSpace>(
              e.getCause())) {
        throw std::runtime_error(
    ...
    if (wrench::StorageService::lookupFileAtLocation(file_1_loc_css))
      throw std::runtime_error(
          "Should not be able to lookup file from a CompoundStorageService if "
          "it has not been written/copied to it first");
...
void CompoundStorageServiceFunctionalTest::do_BasicFunctionality_test() {
  ...
  ASSERT_NO_THROW(simple_storage_service_100_0 = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          "SimpleStorageHost0", {"/disk100"}, {}, {})));
  // Bufferized
  ASSERT_NO_THROW(
      simple_storage_service_510_1 = simulation->add(
          wrench::SimpleStorageService::createSimpleStorageService(
              "SimpleStorageHost1", {"/disk510"},
  ...
  ASSERT_THROW(
      compound_storage_service = simulation->add(
          new wrench::CompoundStorageService("CompoundStorageHost", {})),
      std::invalid_argument);

  // Fail to create a Compound Storage Service (one of the storage service is
  // just a nullptr)
  ASSERT_THROW(compound_storage_service =
                   simulation->add(new wrench::CompoundStorageService(
                       "CompoundStorageHost", {simple_storage_service_1000_0,
                                               simple_storage_service_100_0})),
               std::invalid_argument);
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(wms = simulation->add(
                      new CompoundStorageServiceBasicFunctionalityTestCtrl(
                          this, "CompoundStorageHost")));

  // A bogus staging (can't use CompoundStorageService for staging)
  ASSERT_THROW(simulation->stageFile(file_10, compound_storage_service),
               std::invalid_argument);
...
public:
  CompoundStorageServiceErrorTestCtrl(
      CompoundStorageServiceFunctionalTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    // 1 - Copy from CSS to SS, using a file that was not created on CSS
    // beforehand
    ...
    auto fileCopyActionCSS_SS = jobCopyError->addFileCopyAction(
        ...
        wrench::FileLocation::LOCATION(test->compound_storage_service,
                                       test->file_500),
        wrench::FileLocation::LOCATION(test->simple_storage_service_1000_0,
                                       test->file_500));

    job_manager->submitJob(jobCopyError, test->compute_service, {});

    // Dirty solution to make all tests run : normally we would stop simulation
    // after the first error, and wouldn't care about what happens to following
    // jobs, but here we need to give some time to each job to "properly" fail
    ...
    wrench::StorageService::createFileAtLocation(wrench::FileLocation::LOCATION(
        test->simple_storage_service_1000_0, test->file_1000));
    ...
    // 3 - Read from CSS, using a file that was not written/copied to it
    // beforehand
    ...
    if (!jobCopyError->hasFailed())
      throw std::runtime_error("1-Unexpected job state: " +
                               jobCopyError->getStateAsString());
    if (!std::dynamic_pointer_cast<wrench::FileNotFound>(
            fileCopyActionCSS_SS->getFailureCause()))
      throw std::runtime_error(
          "1-Did not receive a 'NotAllowed' failure cause as expected");

    // 2
    this->waitForNextEvent();
    if (!jobCopySizeError->hasFailed())
      throw std::runtime_error("2-Unexpected job state: " +
                               jobCopySizeError->getStateAsString());
    if (!std::dynamic_pointer_cast<wrench::StorageServiceNotEnoughSpace>(
            fileCopyActionSS_CSS->getFailureCause()))
      throw std::runtime_error(
          "2-Did not receive a 'StorageServiceNotEnoughSpace' failure cause as "
          "expected");

    // 3
    this->waitForNextEvent();
    if (!jobReadError->hasFailed())
      throw std::runtime_error("3-Unexpected job state: " +
                               jobReadError->getStateAsString());
    if (!std::dynamic_pointer_cast<wrench::FileNotFound>(
            fileReadActionCSS->getFailureCause()))
      throw std::runtime_error(
          "3-Did not receive a 'FileNotFound' failure cause as expected");

    // 4
    this->waitForNextEvent();
    if (!jobWriteError->hasFailed())
      throw std::runtime_error("4-Unexpected job state: " +
                               jobWriteError->getStateAsString());
    if (!std::dynamic_pointer_cast<wrench::FileNotFound>(
            fileWriteActionCSS->getFailureCause()))
      throw std::runtime_error(
          "4-Did not receive a 'FileNotFound' failure cause as expected");

    // 5
    this->waitForNextEvent();
    if (!jobDeleteError->hasFailed())
      throw std::runtime_error("5-Unexpected job state: " +
                               jobDeleteError->getStateAsString());
    if (!std::dynamic_pointer_cast<wrench::FileNotFound>(
            fileDeleteActionCSS->getFailureCause()))
      throw std::runtime_error(
          "5-Did not receive a 'FileNotFound' failure cause as expected");
...
void CompoundStorageServiceFunctionalTest::do_BasicError_test() {
  ...
  // Bufferized
  ASSERT_NO_THROW(
      simple_storage_service_1000_0 = simulation->add(
          wrench::SimpleStorageService::createSimpleStorageService(
              "SimpleStorageHost0", {"/disk1000"},
              {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "1000000"}},
              {})));
  ...
  // Create a valid Compound Storage Service (using a non-bufferized storage
  // service in this case) with a user-provided callback CAREFUL -> REUSING
  // CALLBACK FROM PREVIOUS TEST
  TestAllocator allocator;
  wrench::StorageSelectionStrategyCallback allocatorCallback =
      [&allocator](
          const std::shared_ptr<wrench::DataFile> &file,
  ...
  ASSERT_NO_THROW(compound_storage_service =
                      simulation->add(new wrench::CompoundStorageService(
                          "CompoundStorageHost", {simple_storage_service_510_1},
                          allocatorCallback,
                          {{wrench::CompoundStorageServiceProperty::
                                MAX_ALLOCATION_CHUNK_SIZE,
                            "100"}})));
  ...
  // Create a Controller
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(wms = simulation->add(new CompoundStorageServiceErrorTestCtrl(
                      this, "CompoundStorageHost")));
