...
WRENCH_LOG_CATEGORY(logical_file_system_test,
                    "Log category for LogicalFileSystemTest");
...
protected:
  LogicalFileSystemTest() {
    ...
    std::string xml =
        ...
        "             <prop id=\"mount\" value=\"/\"/>"
        "          </disk>"
        "          <disk id=\"100bytedisk\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
...
TEST_F(LogicalFileSystemTest, BasicTests) { DO_TEST_WITH_FORK(do_BasicTests); }
...
void LogicalFileSystemTest::do_BasicTests() {
  ...
  std::shared_ptr<wrench::SimpleStorageService> storage_service1,
      storage_service2;
  ASSERT_NO_THROW(storage_service1 = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          "Host", {"/"})));

  ASSERT_THROW(wrench::LogicalFileSystem::createLogicalFileSystem(
                   "Host", nullptr, "/tmp", "NONE"),
               std::invalid_argument);
  ASSERT_THROW(wrench::LogicalFileSystem::createLogicalFileSystem(
                   "Host", storage_service1.get(), "/bogus"),
               std::invalid_argument);

  // Create a Logical File System
  try {
    wrench::LogicalFileSystem::createLogicalFileSystem(
        "Host", storage_service1.get(), "/tmp", "BOGUS");
    throw std::runtime_error("Should not be able to create a logical file "
                             "system with a bogus caching policy");
  } catch (std::invalid_argument &ignore) {
  }

  auto fs1 = wrench::LogicalFileSystem::createLogicalFileSystem(
      "Host", storage_service1.get(), "/tmp", "NONE");

  // Attempt to create a redundant Logical File System
  try {
    wrench::LogicalFileSystem::createLogicalFileSystem(
        "Host", storage_service1.get(), "/tmp");
    throw std::runtime_error(
        "Initializing a redundant file system should have thrown");
  } catch (std::invalid_argument &ignore) {
  }
  ...
  fs1->incrementNumRunningTransactionsForFileInDirectory(file_80,
                                                         "/files"); // coverage
  fs1->decrementNumRunningTransactionsForFileInDirectory(file_80,
                                                         "/files"); // coverage
...
void LogicalFileSystemTest::do_DevNullTests() {
  ...
  // Create a Logical File System
  auto fs1 = wrench::LogicalFileSystem::createLogicalFileSystem(
      "Host", storage_service.get(), "/dev/null");
  ...
  // Create a Logical File System
  auto fs2 = wrench::LogicalFileSystem::createLogicalFileSystem(
      "Host", storage_service.get(), "/dev/null", "LRU");
...
TEST_F(LogicalFileSystemTest, LRUTests) { DO_TEST_WITH_FORK(do_LRUTests); }
...
void LogicalFileSystemTest::do_LRUTests() {
  ...
  auto fs1 = wrench::LogicalFileSystem::createLogicalFileSystem(
      "Host", storage_service.get(), "/tmp", "LRU");
  ...
  //  If I store another file that requires 50 bytes, but make file_10
  //  unevictable, file_50 should be evicted
