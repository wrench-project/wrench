...
WRENCH_LOG_CATEGORY(storage_service_proxy_simultaneous_test,
                    "Log category for StorageServiceProxySimultaneousTest");
...
class StorageServiceProxySimultaneousTestExecutionController
    : public wrench::ExecutionController {

public:
  StorageServiceProxySimultaneousTestExecutionController(
      StorageServiceProxySimultaneousTest *test, std::string hostname,
      const std::shared_ptr<wrench::DataFile> &file1, bool testWithDefault)
      : wrench::ExecutionController(hostname, "test"), file1(file1), test(test),
        order(testWithDefault) {}
...
TEST_F(StorageServiceProxySimultaneousTest,
       SimultaneousUncachedReadTest_CopyThenRead) {
  DO_TEST_WITH_FORK_ONE_ARG(do_Simultaneous_test, "CopyThenRead");
}
TEST_F(StorageServiceProxySimultaneousTest,
       SimultaneousUncachedReadTest_MagicRead) {
  DO_TEST_WITH_FORK_ONE_ARG(do_Simultaneous_test, "MagicRead");
}
TEST_F(StorageServiceProxySimultaneousTest,
       SimultaneousUncachedReadTest_ReadThrough) {
  DO_TEST_WITH_FORK_ONE_ARG(do_Simultaneous_test, "ReadThrough");
}

void StorageServiceProxySimultaneousTest::do_Simultaneous_test(
    std::string mode) {
  ...
  this->remote =
      simulation->add(wrench::SimpleStorageService::createSimpleStorageService(
          "Remote", {"/disk100"},
          {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "10TB"}}, {}));
  auto cache =
      simulation->add(wrench::SimpleStorageService::createSimpleStorageService(
          "Proxy", {"/disk100"},
          {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "10TB"}}, {}));
  ...
  this->proxy =
      simulation->add(wrench::StorageServiceProxy::createRedirectProxy(
          "Proxy", cache, remote,
          {{wrench::StorageServiceProxyProperty::UNCACHED_READ_METHOD, mode}}));
  ...
  auto controller1 = simulation->add(
      new StorageServiceProxySimultaneousTestExecutionController(this, "Client",
                                                                 file1, true));
  auto controller2 = simulation->add(
      new StorageServiceProxySimultaneousTestExecutionController(this, "Client",
                                                                 file1, false));
