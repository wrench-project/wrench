...
WRENCH_LOG_CATEGORY(storage_service_proxy_basic_functional_test,
                    "Log category for StorageServiceProxyBasicTest");
...
protected:
  StorageServiceProxyBasicTest() {
    ...
    std::string xml =
        ...
        "      <link id=\"backdoor\" bandwidth=\"0.0001bps\" "
        "latency=\"100us\"/>"
        "       <route src=\"Client\" dst=\"Proxy\"> <link_ctn id=\"link12\"/> "
        "</route>"
        "       <route src=\"Proxy\" dst=\"Remote\"> <link_ctn id=\"link13\"/> "
        "</route>"
        "       <route src=\"Proxy\" dst=\"Target\"> <link_ctn id=\"link23\"/> "
        "</route>"
        //"      <route src=\"Client\" dst=\"Remote\"> <link_ctn
        //id=\"backdoor\"/> </route>" "       <route src=\"Client\"
        //dst=\"Target\"> <link_ctn id=\"backdoor\"/> </route>"
        "   </zone> "
        "</platform>";
...
class StorageServiceProxyBasicTestExecutionController
    : public wrench::ExecutionController {

public:
  StorageServiceProxyBasicTestExecutionController(
      StorageServiceProxyBasicTest *test, std::string hostname,
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to create a file on a proxy directly");
    ...
    //       if(testWithDefault)
    //       ((shared_ptr<StorageService>)proxy)->lookupFile(file1);
    ...
    try {
      ...
      if (not std::dynamic_pointer_cast<wrench::FileNotFound>(
        ...
        throw std::runtime_error("Should have received a FileNotFound "
                                 "execution when reading file2 from remote");
      }
    }
    ...
    if (firstFile - start < (secondFile - firstFile) * 1.2) {
      throw std::runtime_error("caching was not significantly faster than not "
                               "caching for file read");
    }
...
void StorageServiceProxyBasicTest::do_BasicFunctionality_test(
  ...
  if (mode == "ReadThrough") {

    {
      ...
      simgrid::s4u::Engine::get_instance()
          ->netzone_by_name_or_null("AS0")
          ->add_route(
              simgrid::s4u::Host::by_name("Remote"),
              simgrid::s4u::Host::by_name("Client"),
              {simgrid::s4u::LinkInRoute(simgrid::s4u::Link::by_name("link12")),
               simgrid::s4u::LinkInRoute(
                   simgrid::s4u::Link::by_name("link13"))});
    }
    {
      ...
      simgrid::s4u::Engine::get_instance()
          ->netzone_by_name_or_null("AS0")
          ->add_route(
              simgrid::s4u::Host::by_name("Target"),
              simgrid::s4u::Host::by_name("Client"),
              {simgrid::s4u::LinkInRoute(simgrid::s4u::Link::by_name("link12")),
               simgrid::s4u::LinkInRoute(
                   simgrid::s4u::Link::by_name("link23"))});
    }
  } else {
    {
      ...
      simgrid::s4u::Engine::get_instance()
          ->netzone_by_name_or_null("AS0")
          ->add_route(simgrid::s4u::Host::by_name("Target"),
                      simgrid::s4u::Host::by_name("Client"),
                      {simgrid::s4u::LinkInRoute(
                          simgrid::s4u::Link::by_name("backdoor"))});
    }
    {

      simgrid::s4u::Engine::get_instance()
          ->netzone_by_name_or_null("AS0")
          ->add_route(simgrid::s4u::Host::by_name("Remote"),
                      simgrid::s4u::Host::by_name("Client"),
                      {simgrid::s4u::LinkInRoute(
                          simgrid::s4u::Link::by_name("backdoor"))});
    }
  ...
  if (arg) {
    this->proxy =
        simulation->add(wrench::StorageServiceProxy::createRedirectProxy(
            "Proxy", cache, remote,
  ...
  auto controller = simulation->add(
      new StorageServiceProxyBasicTestExecutionController(this, "Client", arg));
