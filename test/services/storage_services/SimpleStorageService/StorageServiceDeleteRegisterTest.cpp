...
class SimpleStorageServiceDeleteRegisterTestWMS
    : public wrench::ExecutionController {
public:
  SimpleStorageServiceDeleteRegisterTestWMS(
...
private:
  ...
  int main() override {
    ...
    file_registry_service->addEntry(
        wrench::FileLocation::LOCATION(storage_service, file_1));
    file_registry_service->addEntry(
        wrench::FileLocation::LOCATION(storage_service, file_2));
    ...
    if (file_registry_service->lookupEntry(file_1).size() != 1) {
      throw std::runtime_error(
          "The file_1 should still be registered with the FileRegistryService");
    }
    ...
    file_registry_service->removeEntry(
        wrench::FileLocation::LOCATION(storage_service, file_2));

    if (!file_registry_service->lookupEntry(file_2).empty()) {
      throw std::runtime_error(
          "The file_2 should not be registered with the FileRegistryService");
    }
...
void SimpleStorageServiceDeleteRegisterTest::do_DeleteRegisterTest(
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new SimpleStorageServiceDeleteRegisterTestWMS(
