...
WRENCH_LOG_CATEGORY(simple_storage_service_performance_test,
                    "Log category for SimpleStorageServicePerformanceTest");
...
public:
  ...
  static double computeExpectedTwoStagePipelineTime(double bw_stage1,
                                                    double bw_stage2,
                                                    double total_size,
                                                    double buffer_size) {
  ...
  static double computeExpectedThreeStagePipelineTime(double bw_stage1,
                                                      double bw_stage2,
                                                      double bw_stage3,
                                                      double total_size,
                                                      double buffer_size) {
    ...
    if (buffer_size >= total_size) {
      return (total_size / bw_stage1) + (total_size / bw_stage2) +
             (total_size / bw_stage3);
    } else {
      double bottleneck_bandwidth =
          std::min<double>(std::min<double>(bw_stage1, bw_stage2), bw_stage3);
      double num_full_buffers = std::floor(total_size / buffer_size);
      double last_nonfull_buffer_size =
          total_size - num_full_buffers * buffer_size;
      ...
      return buffer_size / bw_stage1 +
             ...
             std::max<double>(last_nonfull_buffer_size / bw_stage1,
                              buffer_size /
                                  std::min<double>(bw_stage2, bw_stage3)) +
             std::max<double>(last_nonfull_buffer_size / bw_stage2,
                              buffer_size / bw_stage3) +
...
protected:
  ...
  SimpleStorageServicePerformanceTest() {
    ...
    std::string xml =
        ...
        std::to_string(MBPS_BANDWIDTH) + "MBps\" write_bw=\"" +
...
class SimpleStorageServiceConcurrentFileCopiesTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceConcurrentFileCopiesTestWMS(
      SimpleStorageServicePerformanceTest *test, std::string hostname,
      double buffer_size)
  ...
  int main() override {
    ...
    if (std::abs(copy2_start - copy3_start) > 0.1) {
      throw std::runtime_error(
          "Time between two asynchronous operations is too big");
    }

    if ((std::abs(transfer_time_2 / transfer_time_1 - 2.0) > 0.001) ||
        (std::abs(transfer_time_3 / transfer_time_1 - 2.0) > 0.001)) {
      throw std::runtime_error(
          "Concurrent transfers should be roughly twice slower");
    }

    if (std::abs(event2_arrival - event3_arrival) > 0.1) {
      throw std::runtime_error(
          "Time between two asynchronous operation completions is too big");
    }

    // Do absolute checksThree
    double expected_transfer_time_1 = SimpleStorageServicePerformanceTest::
        computeExpectedThreeStagePipelineTime(
            MBPS_BANDWIDTH * 1000 * 1000, 0.92 * MBPS_BANDWIDTH * 1000 * 1000,
            MBPS_BANDWIDTH * 1000 * 1000, FILE_SIZE, buffer_size);

    if (std::abs(transfer_time_1 - expected_transfer_time_1) > 1.0) {
      throw std::runtime_error("Unexpected transfer time #1 " +
                               std::to_string(transfer_time_1) +
                               " (should be around " +
                               std::to_string(expected_transfer_time_1) + ")");
    }

    double expected_transfer_time_2 = SimpleStorageServicePerformanceTest::
        computeExpectedThreeStagePipelineTime(
            MBPS_BANDWIDTH * 1000 * 1000 / 2,
            0.92 * MBPS_BANDWIDTH * 1000 * 1000 / 2,
            MBPS_BANDWIDTH * 1000 * 1000 / 2, FILE_SIZE, buffer_size);

    if (std::abs(transfer_time_2 - expected_transfer_time_2) > 1.0) {
      throw std::runtime_error("Unexpected transfer time #2 " +
                               std::to_string(transfer_time_2) +
                               " (should be around " +
                               std::to_string(expected_transfer_time_2) + ")");
    }
...
TEST_F(SimpleStorageServicePerformanceTest, ConcurrentFileCopies) {
  ...
  DO_TEST_WITH_FORK_ONE_ARG(do_ConcurrentFileCopies_test,
                            FILE_SIZE / 10 + FILE_SIZE / 3);
...
void SimpleStorageServicePerformanceTest::do_ConcurrentFileCopies_test(
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new SimpleStorageServiceConcurrentFileCopiesTestWMS(
...
public:
  SimpleStorageServiceFileReadTestWMS(
      SimpleStorageServicePerformanceTest *test,
      std::shared_ptr<wrench::FileRegistryService> file_registry_service,
      std::string hostname, double buffer_size)
      : wrench::ExecutionController(hostname, "test"), test(test),
        file_registry_service(file_registry_service), buffer_size(buffer_size) {
  }
  ...
  int main() override {
    ...
    try {
      wrench::StorageService::readFileAtLocation(wrench::FileLocation::LOCATION(
          this->test->storage_service_1, this->test->file_1));
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error(e.what());
    }

    double elapsed =
        wrench::Simulation::getCurrentSimulatedDate() - before_read;

    double effective_network_bandwidth = (MBPS_BANDWIDTH * 1000 * 1000) * 0.92;
    double effective_disk_bandwidth = (MBPS_BANDWIDTH * 1000 * 1000);

    double expected_elapsed = SimpleStorageServicePerformanceTest::
        computeExpectedTwoStagePipelineTime(effective_disk_bandwidth,
                                            effective_network_bandwidth,
                                            FILE_SIZE, buffer_size);

    if (std::abs(elapsed - expected_elapsed) > 1.0) {
      throw std::runtime_error(
          "Incorrect file read time " + std::to_string(elapsed) +
...
void SimpleStorageServicePerformanceTest::do_FileRead_test(double buffer_size) {
  ...
  // Create a WMS
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(wms = simulation->add(new SimpleStorageServiceFileReadTestWMS(
                      this, file_registry_service, "WMSHost", buffer_size)));
