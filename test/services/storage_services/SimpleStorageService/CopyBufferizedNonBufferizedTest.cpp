...
public:
  ...
  std::shared_ptr<wrench::StorageService> storage_service_bufferized_1 =
      nullptr;
  std::shared_ptr<wrench::StorageService> storage_service_bufferized_2 =
      nullptr;
  std::shared_ptr<wrench::StorageService> storage_service_non_bufferized_1 =
      nullptr;
  std::shared_ptr<wrench::StorageService> storage_service_non_bufferized_2 =
      nullptr;
...
protected:
  ...
  CopyBufferizedNonBufferizedTest() {
    ...
    // create the files
    file_1_size_100 =
        wrench::Simulation::addFile("file_1_size_100", 100 * 1000 * 1000);
    file_2_size_200 =
        wrench::Simulation::addFile("file_2_size_200", 200 * 1000 * 1000);
...
private:
  ...
  int main() override {
    ...
    // Create the files
    this->test->storage_service_bufferized_1->createFile(
        this->test->file_1_size_100);
    this->test->storage_service_non_bufferized_1->createFile(
        this->test->file_2_size_200);
    ...
    if (not this->test->storage_service_bufferized_2->lookupFile(
            this->test->file_1_size_100)) {
      throw std::runtime_error(
          "Cannot find copied file after bufferized to bufferized");
    }
    ...
    data_movement_manager->doSynchronousFileCopy(
        ...
        wrench::FileLocation::LOCATION(
            this->test->storage_service_non_bufferized_2,
            this->test->file_1_size_100));
    if (not this->test->storage_service_non_bufferized_2->lookupFile(
            this->test->file_1_size_100)) {
      throw std::runtime_error(
          "Cannot find copied file after bufferized to non-bufferized");
    }

    //        std::cerr << "DOING NON-BUFFERIZED TO BUFFERIZED\n";
    data_movement_manager->doSynchronousFileCopy(
        wrench::FileLocation::LOCATION(
            this->test->storage_service_non_bufferized_1,
            this->test->file_2_size_200),
        wrench::FileLocation::LOCATION(this->test->storage_service_bufferized_2,
                                       this->test->file_2_size_200));
    if (not this->test->storage_service_bufferized_2->lookupFile(
            this->test->file_2_size_200)) {
      throw std::runtime_error(
          "Cannot find copied file after non-bufferized to bufferized");
    }
    ...
    if (not this->test->storage_service_non_bufferized_2->lookupFile(
            this->test->file_2_size_200)) {
      throw std::runtime_error(
          "Cannot find copied file after non-bufferized to non-bufferized");
    }
...
void CopyBufferizedNonBufferizedTest::
    do_CopyBufferizedNonBufferizedTest_test() {
  ...
  ASSERT_NO_THROW(
      storage_service_bufferized_1 = simulation->add(
          wrench::SimpleStorageService::createSimpleStorageService(
              ...
              {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "1MB"},
               {wrench::SimpleStorageServiceProperty::
                ...
                "10"}})));
  ASSERT_NO_THROW(
      storage_service_non_bufferized_1 = simulation->add(
          wrench::SimpleStorageService::createSimpleStorageService(
              "StorageHost1", {"/disk2"},
              {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "0"},
               {wrench::SimpleStorageServiceProperty::
                ...
                "10"}})));
  ASSERT_NO_THROW(
      storage_service_bufferized_2 = simulation->add(
          wrench::SimpleStorageService::createSimpleStorageService(
              "StorageHost2", {"/disk1"},
              {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "2MB"},
               {wrench::SimpleStorageServiceProperty::
                ...
                "10"}})));
  ASSERT_NO_THROW(
      storage_service_non_bufferized_2 = simulation->add(
          wrench::SimpleStorageService::createSimpleStorageService(
              "StorageHost2", {"/disk2"},
              {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "0"},
               {wrench::SimpleStorageServiceProperty::
  ...
  // Create a WMS
  ASSERT_NO_THROW(
      simulation->add(new CopyBufferizedNonBufferizedTestWMS(this, "WMSHost")));
...
class SelfCopyBufferizedNonBufferizedTestWMS
...
private:
  ...
  int main() override {
    ...
    // Create the files
    this->test->storage_service_bufferized_1->createFile(
        this->test->file_1_size_100, "/disk1");
    this->test->storage_service_non_bufferized_1->createFile(
        this->test->file_2_size_200, "/disk1");

    auto data_movement_manager = this->createDataMovementManager();

    data_movement_manager->doSynchronousFileCopy(
        wrench::FileLocation::LOCATION(this->test->storage_service_bufferized_1,
                                       "/disk1", this->test->file_1_size_100),
        wrench::FileLocation::LOCATION(this->test->storage_service_bufferized_1,
                                       "/disk2", this->test->file_1_size_100));

    if (not this->test->storage_service_bufferized_1->lookupFile(
            this->test->file_1_size_100, "/disk2")) {
      throw std::runtime_error(
          "Cannot find copied file after self copy bufferized");
    }

    data_movement_manager->doSynchronousFileCopy(
        wrench::FileLocation::LOCATION(
            this->test->storage_service_non_bufferized_1, "/disk1",
            this->test->file_2_size_200),
        wrench::FileLocation::LOCATION(
            this->test->storage_service_non_bufferized_1, "/disk2",
            this->test->file_2_size_200));

    if (not this->test->storage_service_non_bufferized_1->lookupFile(
            this->test->file_2_size_200, "/disk2")) {
      throw std::runtime_error(
          "Cannot find copied file after self copy non-bufferized");
    }
...
void CopyBufferizedNonBufferizedTest::
    do_SelfCopyBufferizedNonBufferizedTest_test() {
  ...
  ASSERT_NO_THROW(simulation->add(
      new SelfCopyBufferizedNonBufferizedTestWMS(this, "WMSHost")));
