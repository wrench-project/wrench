...
WRENCH_LOG_CATEGORY(
    simple_storage_service_limited_connection_test,
    "Log category for SimpleStorageServiceLimitedConnectionTest");

#define NUM_PARALLEL_TRANSFERS                                                 \
  7 // Changing this number  to a non n*3+ 1 number will ikely
...
public:
  ...
  std::shared_ptr<wrench::StorageService> storage_service_wms_unlimited =
      nullptr;
...
protected:
  ...
  SimpleStorageServiceLimitedConnectionsTest() {
    ...
    std::string xml =
        ...
        "          <disk id=\"large_disk\" read_bw=\"50MBps\" "
        "write_bw=\"50MBps\">"
        "             <prop id=\"size\" value=\"" +
        ...
        "          <disk id=\"large_disk1\" read_bw=\"50MBps\" "
        "write_bw=\"50MBps\">"
        "             <prop id=\"size\" value=\"" +
        std::to_string(STORAGE_SIZE) +
        "B\"/>"
        ...
        "          </disk>"
        "          <disk id=\"large_disk2\" read_bw=\"50MBps\" "
        "write_bw=\"50MBps\">"
        "             <prop id=\"size\" value=\"" +
        std::to_string(STORAGE_SIZE) +
        "B\"/>"
...
class SimpleStorageServiceConcurrencyFileCopiesLimitedConnectionsTestWMS
...
private:
  ...
  int main() override {
    ...
    // Reading
    for (auto dst_storage_service : {this->test->storage_service_unlimited,
                                     this->test->storage_service_limited}) {
      ...
      for (int i = 0; i < NUM_PARALLEL_TRANSFERS; i++) {
        std::shared_ptr<wrench::ExecutionEvent> event1 =
      ...
      if (dst_storage_service == this->test->storage_service_unlimited) {
        ...
        for (int i = 1; i < NUM_PARALLEL_TRANSFERS; i++) {
          if (std::abs(baseline_elapsed - elapsed[i]) > 1) {
            throw std::runtime_error("Incoherent transfer elapsed times for "
                                     "the unlimited storage service");
          }
        }
      }
      ...
      if (dst_storage_service == this->test->storage_service_limited) {
        ...
        if (std::abs<double>(elapsed[NUM_PARALLEL_TRANSFERS - 1] -
                             elapsed[NUM_PARALLEL_TRANSFERS - 2] -
                             (elapsed[0] / 3.0)) > 1) {
          success = false;
        }

        if (not success) {
          throw std::runtime_error("Incoherent transfer elapsed times for the "
...
void SimpleStorageServiceLimitedConnectionsTest::do_ConcurrencyFileCopies_test(
  ...
  for (int i = 0; i < NUM_PARALLEL_TRANSFERS; i++) {
    ASSERT_NO_THROW(
        simulation->stageFile(files[i], storage_service_wms_unlimited));
    ASSERT_NO_THROW(
        simulation->stageFile(files[i], storage_service_wms_limited));
  }
