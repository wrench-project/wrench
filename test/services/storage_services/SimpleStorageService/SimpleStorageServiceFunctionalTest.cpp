...
WRENCH_LOG_CATEGORY(simple_storage_service_functional_test,
                    "Log category for SimpleStorageServiceFunctionalTest");
...
protected:
  ...
  SimpleStorageServiceFunctionalTest() {
    ...
    std::string xml =
        ...
        "          </disk>"
        "          <disk id=\"somewhere1\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"100B\"/>"
        "             <prop id=\"mount\" value=\"/somewhere1/\"/>"
        "          </disk>"
        "          <disk id=\"somewhere2\" read_bw=\"100MBps\" "
        "write_bw=\"100MBps\">"
        "             <prop id=\"size\" value=\"100B\"/>"
...
class SimpleStorageServiceBasicFunctionalityTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceBasicFunctionalityTestWMS(
      SimpleStorageServiceFunctionalTest *test, std::string hostname)
...
private:
  ...
  int main() override {
    ...
    try {
      this->simulation->stageFile(this->test->file_1,
                                  this->test->storage_service_100);
      throw std::runtime_error("Should not be possible to call stageFile() "
                               "from a non-maestro process");
    ...
    for (const auto &f : {this->test->file_1, this->test->file_10,
                          this->test->file_100, this->test->file_500}) {
      std::set<std::shared_ptr<wrench::FileLocation>> result =
          this->test->file_registry_service->lookupEntry(f);

      if ((result.size() != 1) || ((*(result.begin()))->getStorageService() !=
                                   this->test->storage_service_1000)) {
        throw std::runtime_error(
            "File registry service should know that file " + f->getID() +
            " is (only) on storage service " +
    ...
    {
      auto sss = std::dynamic_pointer_cast<wrench::SimpleStorageService>(
          this->test->storage_service_multimp);
      sss->getMountPoints();
      ...
      try {
        sss->getBaseRootPath();
        throw std::runtime_error("Shouldn't be able to get base root path from "
                                 "a multi-mount-point simple storage service");
      ...
      if ((not wrench::StorageService::lookupFileAtLocation(
          ...
          (wrench::StorageService::lookupFileAtLocation(
              wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                             f)))) {
        throw std::runtime_error("Some storage services do/don't have the "
                                 "files that they shouldn't/should have");
      }
    }
    ...
    try {
      wrench::StorageService::copyFile(
          ...
          nullptr);
      throw std::runtime_error(
          "Should not be able to copy a file to a nullptr location!");
    } catch (std::invalid_argument &) {
    }
    ...
    try {
      wrench::StorageService::copyFile(
          nullptr, wrench::FileLocation::LOCATION(
                       this->test->storage_service_100, this->test->file_500));
      throw std::runtime_error(
          "Should not be able to copy a file from a nullptr location!");
    } catch (std::invalid_argument &) {
    }
    ...
    try {
      wrench::StorageService::copyFile(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         this->test->file_500));
      throw std::runtime_error(
          "Should not be able to store a file to a storage service that "
    ...
    if (wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                           this->test->file_500))) {
      throw std::runtime_error("File copy to a storage service without enough "
                               "space shouldn't have succeeded");
    }
    ...
    if (this->test->storage_service_1000->getFileLastWriteDate(
            wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                           this->test->file_10)) > 0) {
      throw std::runtime_error(
          "Last file write date of staged file should be 0");
    }
    ...
    try {
      ...
      throw std::runtime_error("Should be able to store a file to a storage "
                               "service that has enough capacity");
    }
    double after_copy = wrench::Simulation::getCurrentSimulatedDate();

    double last_file_write_date =
        this->test->storage_service_100->getFileLastWriteDate(
            wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                           this->test->file_10));
    if ((last_file_write_date < before_copy) or
        (last_file_write_date > after_copy)) {
      throw std::runtime_error("Last file write date is incoherent");
    }

    try {
      this->test->storage_service_100->getFileLastWriteDate(
          (std::shared_ptr<wrench::FileLocation>)nullptr);
      throw std::runtime_error("Should not be able to pass a nullptr location "
                               "to getFileLastWriteDate()");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should be able to get a storage's service free space");
    }
    if (free_space != 90.0) {
      throw std::runtime_error("Free space on storage service is wrong (" +
    ...
    try {
      free_space =
          this->test->storage_service_100->getTotalFreeSpaceAtPath("/disk100");
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error(
          "Should be able to get a storage's service free space at a path");
    }
    if (free_space != 90.0) {
      throw std::runtime_error("Free space on storage service is wrong (" +
                               std::to_string(free_space) +
    ...
    try {
      free_space =
          this->test->storage_service_100->getTotalFreeSpaceAtPath("bogus");
    } catch (wrench::ExecutionException &ignore) {
      throw std::runtime_error("Should be able to get a storage's service free "
                               "space, even at a bogus path");
    }
    if (free_space != 0.0) {
      throw std::runtime_error(
    ...
    // Bogus read
    try {
      wrench::StorageService::readFileAtLocation(wrench::FileLocation::LOCATION(
          this->test->storage_service_100, nullptr));
    ...
    try {
      wrench::StorageService::readFileAtLocation(wrench::FileLocation::LOCATION(
          this->test->storage_service_100, this->test->file_10));
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error(
          "Should be able to read a file available on a storage service");
    }

    // Read a file on a storage service that doesn't have that file
    try {
      wrench::StorageService::readFileAtLocation(wrench::FileLocation::LOCATION(
          this->test->storage_service_100, this->test->file_100));
      throw std::runtime_error(
          "Should not be able to read a file unavailable a storage service");
    } catch (wrench::ExecutionException &e) {
    }
    ...
    { // Test using readFiles()
      ...
      try {
        std::map<std::shared_ptr<wrench::DataFile>,
            ...
            locations;
        locations[nullptr] = wrench::FileLocation::LOCATION(
            this->test->storage_service_100, nullptr);
      ...
      try {
        std::map<std::shared_ptr<wrench::DataFile>,
                 std::shared_ptr<wrench::FileLocation>>
            locations;
        locations[this->test->file_10] = wrench::FileLocation::LOCATION(
            this->test->storage_service_100, this->test->file_10);
        wrench::StorageService::readFiles(locations);
      } catch (wrench::ExecutionException &e) {
        throw std::runtime_error(
      ...
      try {
        std::map<std::shared_ptr<wrench::DataFile>,
                 std::shared_ptr<wrench::FileLocation>>
            locations;
        locations[this->test->file_100] = wrench::FileLocation::LOCATION(
            this->test->storage_service_100, this->test->file_100);
        wrench::StorageService::readFiles(locations);
        throw std::runtime_error(
            "Should not be able to read a file unavailable a storage service");
    ...
    try {
      wrench::StorageService::deleteFileAtLocation(
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         this->test->file_100));
      throw std::runtime_error(
          "Should not be able to delete a file unavailable a storage service");
    } catch (wrench::ExecutionException &e) {
      auto cause =
          std::dynamic_pointer_cast<wrench::FileNotFound>(e.getCause());
      if (not cause) {
        throw std::runtime_error(
            "Got an expected exception, but unexpected failure cause: " +
            e.getCause()->toString() + " (expected: FileNotFound)");
      }
      if (cause->getLocation()->getStorageService() !=
          this->test->storage_service_100) {
        throw std::runtime_error(
            "Got the expected 'file not found' exception, but the failure "
            "cause does not point to the correct storage service");
      }
      if (cause->getFile() != this->test->file_100) {
        throw std::runtime_error(
            "Got the expected 'file not found' exception, but the failure "
            "cause does not point to the correct file");
      }
    }
    ...
    try {
      ...
      auto cause =
          std::dynamic_pointer_cast<wrench::FileNotFound>(e.getCause());
      if (not cause) {
        throw std::runtime_error("Got an expected 'file not found' exception, "
                                 "but unexpected failure cause: " +
                                 e.getCause()->toString() +
    ...
    try {
      ...
      throw std::runtime_error(
          "Should  be able to delete a file available a storage service");
    }
    ...
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         nullptr));
      throw std::runtime_error(
          "Shouldn't be able to do an initiateAsynchronousFileCopy with a "
          "nullptr file");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          nullptr, wrench::FileLocation::LOCATION(
                       this->test->storage_service_100, this->test->file_1));
      throw std::runtime_error(
          "Shouldn't be able to do an initiateAsynchronousFileCopy with a "
          "nullptr src");
    } catch (std::invalid_argument &e) {
    ...
    try {
      ...
      throw std::runtime_error("Error while getting an execution event: " +
                               e.getCause()->toString());
    }
    ...
    if (!wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                           this->test->file_1))) {
      throw std::runtime_error(
          "Asynchronous file copy operation didn't copy the file");
    }
    ...
    if (auto real_event =
            std::dynamic_pointer_cast<wrench::FileCopyFailedEvent>(event)) {
      auto cause =
          std::dynamic_pointer_cast<wrench::StorageServiceNotEnoughSpace>(
              real_event->failure_cause);
      if (not cause) {
        throw std::runtime_error(
            "Got expected event but unexpected failure cause: " +
            real_event->failure_cause->toString() +
            " (expected: FileCopyFailedEvent)");
      }
    } else {
      throw std::runtime_error("Unexpected workflow execution event: " +
                               event->toString());
    ...
    auto real_event =
        std::dynamic_pointer_cast<wrench::FileCopyFailedEvent>(event);
    if (real_event) {
      auto cause = std::dynamic_pointer_cast<wrench::FileNotFound>(
          real_event->failure_cause);
      if (not cause) {
        throw std::runtime_error(
            "Got expected event but unexpected failure cause: " +
    ...
    try {
      wrench::StorageService::deleteFileAtLocation(
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         nullptr));
      throw std::runtime_error(
          "Should not be able to delete a nullptr file from a location");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      wrench::StorageService::lookupFileAtLocation(
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         this->test->file_1));
      throw std::runtime_error(
          "Should not be able to lookup a file from a DOWN service");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::ServiceIsDown>(e.getCause());
      if (not cause) {
        throw std::runtime_error("Got an exception, as expected, but of the "
      ...
      if (cause->getService() != this->test->storage_service_100) {
        throw std::runtime_error(
            "Got the expected 'service is down' exception, but the failure "
            "cause does not point to the correct storage service");
      }
    }
    ...
    try {
      ...
      if (not cause) {
        throw std::runtime_error(
            "Got an exception, as expected, but an unexpected failure cause: " +
    ...
    try {
      wrench::StorageService::readFileAtLocation(wrench::FileLocation::LOCATION(
          this->test->storage_service_100, this->test->file_1));
      throw std::runtime_error(
          "Should not be able to read a file from a down service");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::ServiceIsDown>(e.getCause());
    ...
    try {
      wrench::StorageService::writeFileAtLocation(
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         this->test->file_1));
      throw std::runtime_error(
          "Should not be able to write a file from a DOWN service");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::ServiceIsDown>(e.getCause());
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to get free space info from a DOWN service");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::ServiceIsDown>(e.getCause());
      ...
      // Check Exception details
      wrench::ServiceIsDown *real_cause =
          (wrench::ServiceIsDown *)e.getCause().get();
...
void SimpleStorageServiceFunctionalTest::do_BasicFunctionality_test(
  ...
  ASSERT_NO_THROW(storage_service_multimp = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          hostname, {"/somewhere1", "/somewhere2"},
                          {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE,
                            std::to_string(buffer_size)}},
                          {})));
  ...
  // A bogus staging
  ASSERT_THROW(simulation->stageFile(nullptr, storage_service_100),
               std::invalid_argument);

  // Another bogus staging
  ASSERT_THROW(simulation->stageFile(file_500, storage_service_100),
               std::invalid_argument);
...
class SimpleStorageServiceSynchronousFileCopyTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceSynchronousFileCopyTestWMS(
      SimpleStorageServiceFunctionalTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
  ...
  int main() override {
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         nullptr));
      throw std::runtime_error("Shouldn't be able to do a synchronous file "
                               "copy with a nullptr file");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          nullptr, wrench::FileLocation::LOCATION(
                       this->test->storage_service_510, this->test->file_500));
      throw std::runtime_error(
          "Shouldn't be able to do a synchronous file copy with a nullptr src");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                         this->test->file_1));
      throw std::runtime_error("Shouldn't be able to do a synchronous file "
                               "copy where locations don't have the same file");
    } catch (std::invalid_argument &e) {
    }
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         this->test->file_500));
      throw std::runtime_error(
          "Should not be able to do a file copy with a bogus path");
    } catch (wrench::ExecutionException &e) {
      auto cause = e.getCause();
      if (auto real_cause =
              std::dynamic_pointer_cast<wrench::InvalidDirectoryPath>(
                  e.getCause())) {
        real_cause->toString();    // Coverage
        real_cause->getLocation(); // Coverage
      } else if (auto real_cause =
                     std::dynamic_pointer_cast<wrench::FileNotFound>(
                         e.getCause())) {
        real_cause->toString();    // Coverage
        ...
        throw std::runtime_error(
            "Got the expected exception, but the failure cause is not "
            "InvalidDirectoryPath or FileNotFound (it's " +
            cause->toString() + ")");
      }
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Got an exception while doing a synchronous file copy: " +
          std::string(e.what()));
    }
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         ...
                                         this->test->file_500));
      throw std::runtime_error(
          "Should not be able to write a file beyond the storage capacity");
    } catch (wrench::ExecutionException &e) {
    }
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                         "/disk1000", this->test->file_1),
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         "/disk510", this->test->file_1));
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error(
          "Got an exception while doing a synchronous file copy: " +
          std::string(e.what()));
...
void SimpleStorageServiceFunctionalTest::do_SynchronousFileCopy_test(
  ...
  ASSERT_NO_THROW(
      compute_service = simulation->add(new wrench::BareMetalComputeService(
          hostname, {std::make_pair(hostname, std::make_tuple(1, 0))}, "",
          {})));
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimpleStorageServiceSynchronousFileCopyTestWMS(this, hostname)));
...
class SimpleStorageServiceAsynchronousFileCopyTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceAsynchronousFileCopyTestWMS(
      SimpleStorageServiceFunctionalTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
  ...
  int main() override {
    ...
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         this->test->file_10));
      throw std::runtime_error("Should not be able to do an asynchronous file "
                               "copy for different files");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Got an exception while trying to initiate a file copy: " +
          std::string(e.what()));
    }
    ...
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         this->test->file_500));
      throw std::runtime_error(
          "A duplicate asynchronous file copy should fail!");
    } catch (wrench::ExecutionException &e) {
      auto cause = std::dynamic_pointer_cast<wrench::FileAlreadyBeingCopied>(
          e.getCause());
      if (not cause) {
        throw std::runtime_error(
            "Got expected exception, but unexpected failure cause: " +
            e.getCause()->toString() + " (expected: FileAlreadyBeingCopied)");
      }
      if (cause->getSourceLocation()->getFile() != this->test->file_500) {
        throw std::runtime_error("Got expected failure cause, but failure "
                                 "cause does not point to the right file");
      }
      if (not cause->getDestinationLocation()->equal(
              wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                             this->test->file_500))) {
        throw std::runtime_error("Got expected failure cause, but failure "
                                 "cause does not point to the right location");
      }
      //            if (cause->getDestinationLocation()->getPath() !=
      //            cause->getDestinationLocation()->getStorageService()->getBaseRootPath())
      //            {
      //                std::cerr <<
      //                "cause->getDestinationLocation()->getPath(): " <<
      //                cause->getDestinationLocation()->getPath() << "\n";
      //                std::cerr <<
      //                "cause->getDestinationLocation()->getStorageService()->getBaseRootPath():
      //                " <<
      //                cause->getDestinationLocation()->getStorageService()->getBaseRootPath()
      //                << "\n"; throw std::runtime_error("Got expected failure
      //                cause, but failure cause does not point to the right
      //                path");
      //            }
...
void SimpleStorageServiceFunctionalTest::do_AsynchronousFileCopy_test(
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new SimpleStorageServiceAsynchronousFileCopyTestWMS(this, hostname)));
...
class SimpleStorageServiceSynchronousFileCopyFailuresTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceSynchronousFileCopyFailuresTestWMS(
      SimpleStorageServiceFunctionalTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         this->test->file_500));
      throw std::runtime_error(
          "Should have gotten a 'not enough space' exception");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::StorageServiceNotEnoughSpace>(
      ...
      if (not cause) {
        throw std::runtime_error(
            ...
            e.getCause()->toString() +
            " (expected: StorageServiceNotEnoughSpace)");
      }
      // Check Exception details
      std::string error_msg = cause->toString();
      if (cause->getFile() != this->test->file_500) {
        throw std::runtime_error(
            "Got the expected 'not enough space' exception, but the failure "
            "cause does not point to the correct file");
      }
    ...
    if (wrench::StorageService::hasFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                           this->test->file_500))) {
    ...
    try {
      ...
      throw std::runtime_error("Copying a file onto itself shouldn't lead to "
                               "an exception (just a printed warning)");
    }

    // Do the file copy for a file that's not there

    // First delete the file on the 1000 storage service
    wrench::StorageService::deleteFileAtLocation(wrench::FileLocation::LOCATION(
        this->test->storage_service_1000, this->test->file_500));
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         this->test->file_500));
      throw std::runtime_error(
          "Should have gotten a 'file not found' exception");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::FileNotFound>(e.getCause());
    ...
    try {
      data_movement_manager->doSynchronousFileCopy(
          ...
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         this->test->file_500));
      throw std::runtime_error(
          "Should have gotten a 'service is down' exception");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::ServiceIsDown>(e.getCause());
...
void SimpleStorageServiceFunctionalTest::do_SynchronousFileCopyFailures_test(
  ...
  ASSERT_NO_THROW(storage_service_100->getPropertyValueAsDouble(
      wrench::SimpleStorageServiceProperty::
          MAX_NUM_CONCURRENT_DATA_CONNECTIONS));
...
class SimpleStorageServiceAsynchronousFileCopyFailuresTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceAsynchronousFileCopyFailuresTestWMS(
      SimpleStorageServiceFunctionalTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    if (real_event) {
      ...
      if (cause->getFile() != this->test->file_500) {
        throw std::runtime_error(
            "Got the expected exception and failure type, but the failure "
            "cause doesn't point to the right file");
      }
      if (cause->getStorageService() != this->test->storage_service_100) {
        throw std::runtime_error(
            "Got the expected exception and failure type, but the failure "
            "cause doesn't point to the right storage service");
      }
    ...
    if (real_event2) {
      auto cause = std::dynamic_pointer_cast<wrench::FileNotFound>(
          real_event2->failure_cause);
      if (not cause) {
        throw std::runtime_error(
            "Got an expected exception, but an unexpected failure cause: " +
            real_event2->failure_cause->toString() +
            " (expected: FileNotFound)");
      }
      if (cause->getFile() != this->test->file_100) {
        throw std::runtime_error(
...
private:
  ...
  int main() override {
    ...
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                         "/disk1000", this->test->file_10),
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         "/disk510/foo/", this->test->file_10));
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error("Got an unexpected exception");
    }
    ...
    try {
      ...
      throw std::runtime_error(
          "Should be able to delete file storage_service_510:/:file_10");
    }

    // Copy storage_service_510:/:file_10 to storage_service_1000:foo:file_10:
    // SHOULD NOT WORK
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                         this->test->file_10),
          wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                         "/disk1000/foo", this->test->file_10));
    } catch (wrench::ExecutionException &e) {
      throw std::runtime_error("Got an unexpected exception");
    }
    ...
    // Copy storage_service_510:foo:file_10 to storage_service_510:foo    SHOULD
    // NOT WORK
    try {
      data_movement_manager->initiateAsynchronousFileCopy(
          wrench::FileLocation::LOCATION(this->test->storage_service_510,
      ...
      throw std::runtime_error(
          "Should be able to copy a file onto itself (no-op)");
    }
    ...
    if (not wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                           this->test->file_10))) {
      throw std::runtime_error(
          "File should be in storage_service_1000 at the mount point root");
    }
    if (not wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                           "/disk1000", this->test->file_10))) {
    ...
    if (not wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service_510,
                                           ...
                                           this->test->file_10))) {
      throw std::runtime_error(
          "File should be in storage_service_510 at path /large_disk/foo/");
    }
    if (not wrench::StorageService::lookupFileAtLocation(
            wrench::FileLocation::LOCATION(this->test->storage_service_1000,
                                           "/disk1000/foo",
                                           this->test->file_10))) {
      throw std::runtime_error(
          "File should be in storage_service_1000 at path /large_disk/foo");
    }
...
void SimpleStorageServiceFunctionalTest::do_Partitions_test(
  ...
  ASSERT_NO_THROW(wms = simulation->add(new PartitionsTestWMS(this, hostname)));
...
private:
  ...
  int main() override {
    ...
    try {
      wrench::StorageService::writeFileAtLocation(
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         "/disk100", nullptr));
      throw std::runtime_error(
          "Should not be able to write a nullptr file to a service");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    try {
      wrench::StorageService::writeFileAtLocation(
          wrench::FileLocation::LOCATION(this->test->storage_service_100,
                                         "/disk100", this->test->file_500));
      throw std::runtime_error("Should not be able to write to a storage "
                               "service with not enough space");
    } catch (wrench::ExecutionException &e) {
      auto cause =
          std::dynamic_pointer_cast<wrench::StorageServiceNotEnoughSpace>(
              e.getCause());
      if (not cause) {
        throw std::runtime_error(
            "Got an expected exception but unexpected cause type: " +
            e.getCause()->toString() +
            " (expected: StorageServiceNotEnoughSpace");
      }
      if (cause->getStorageService() != this->test->storage_service_100) {
...
void SimpleStorageServiceFunctionalTest::do_FileWrite_test(double buffer_size) {
  ...
  ASSERT_NO_THROW(storage_service_100 = simulation->add(
                      wrench::SimpleStorageService::createSimpleStorageService(
                          hostname, {"/disk100", "/disk1000"},
                          {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE,
                            std::to_string(buffer_size)}},
                          {})));

  // FileLocation Testing
  ASSERT_THROW(wrench::FileLocation::LOCATION(nullptr, file_1),
               std::invalid_argument);
  ASSERT_THROW(wrench::FileLocation::LOCATION(storage_service_100, nullptr),
               std::invalid_argument);
  ASSERT_THROW(wrench::FileLocation::LOCATION(nullptr, "/disk100", file_1),
               std::invalid_argument);
  ASSERT_THROW(wrench::FileLocation::LOCATION(storage_service_100, "", file_1),
               std::invalid_argument);
  //    ASSERT_THROW(wrench::FileLocation::LOCATION(storage_service_100,
  //    "/bogus", file_1), std::invalid_argument);
  ASSERT_THROW(wrench::FileLocation::SCRATCH(file_1)->getStorageService(),
               std::invalid_argument);
  ASSERT_THROW(wrench::FileLocation::SCRATCH(file_1)->getPath(),
               std::invalid_argument);
  ASSERT_THROW(wrench::FileLocation::sanitizePath(""), std::invalid_argument);
  //    ASSERT_THROW(wrench::FileLocation::sanitizePath(" "),
  //    std::invalid_argument);
  //    ASSERT_THROW(wrench::FileLocation::sanitizePath("../.."),
  //    std::invalid_argument);
  ...
  ASSERT_NO_THROW(wms = simulation->add(new FileWriteTestWMS(this, hostname)));
