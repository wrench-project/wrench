...
WRENCH_LOG_CATEGORY(simple_storage_service_lru_caching_test,
                    "Log category for SimpleStorageServiceLRUCachingTest");
...
class SimpleStorageServiceLRUCachingTestWMS
...
void SimpleStorageServiceCachingTest::do_SimpleLRUCaching_test(
    double buffer_size) {
  ...
  ASSERT_NO_THROW(
      this->storage_service_1 = simulation->add(
...
class SimpleStorageServiceLRUCachingCopyTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceLRUCachingCopyTestWMS(
      SimpleStorageServiceCachingTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    wrench::StorageService::copyFile(
        wrench::FileLocation::LOCATION(ss1, file_50),
        wrench::FileLocation::LOCATION(ss2, file_50));
    ...
    wrench::StorageService::copyFile(
        wrench::FileLocation::LOCATION(ss1, file_20),
        wrench::FileLocation::LOCATION(ss2, file_20));
    // The LRU list:  SS1: file_10, file_20, file_50
    //                SS2: file_40, file_20
...
void SimpleStorageServiceCachingTest::do_SimpleLRUCachingCopy_test(
    double buffer_size) {
  ...
  ASSERT_NO_THROW(
      this->storage_service_2 = simulation->add(
          wrench::SimpleStorageService::createSimpleStorageService(
              hostname, {"/disk2"},
              {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE,
                std::to_string(buffer_size)},
...
class SimpleStorageServiceLRUCachingUnevictableTestWMS
    : public wrench::ExecutionController {

public:
  SimpleStorageServiceLRUCachingUnevictableTestWMS(
      SimpleStorageServiceCachingTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    data_manager->initiateAsynchronousFileCopy(
        wrench::FileLocation::LOCATION(ss1, file_50),
        wrench::FileLocation::LOCATION(ss2, file_50));
    ...
    // Writing a file to ss1, which "shouldn't" evict file_50 even though it's
    // the LRU Instead it should evict file_10 and file_20
    //        std::cerr << "WRITING file_40 to SS1, which should evict file_10
    //        and file_20\n";
...
void SimpleStorageServiceCachingTest::do_SimpleLRUCachingUnevictable_test(
    double buffer_size) {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new SimpleStorageServiceLRUCachingUnevictableTestWMS(
                          this, hostname)));
