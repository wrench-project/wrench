...
WRENCH_LOG_CATEGORY(action_scheduler_test,
                    "Log category for ActionExecutionServiceTest");
...
class ActionExecutionServiceOneActionSuccessTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceOneActionSuccessTestWMS(
      ActionExecutionServiceTest *test, const std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    std::map<simgrid::s4u::Host *, std::tuple<unsigned long, double>>
        compute_resources;
    compute_resources[wrench::S4U_Simulation::get_host_or_vm_by_name("Host3")] =
        std::make_tuple(3, 100.0);
    auto action_execution_service =
        std::shared_ptr<wrench::ActionExecutionService>(
    ...
    try {
      ...
      auto cause =
          std::dynamic_pointer_cast<wrench::NetworkError>(e.getCause());
      throw std::runtime_error(
          "Network error while getting reply from Executor!" +
          cause->toString());
    }

    // Did we get the expected message?
    auto msg = std::dynamic_pointer_cast<
        wrench::ActionExecutionServiceActionDoneMessage>(message);
    if (!msg) {
      throw std::runtime_error("Unexpected '" + message->getName() +
                               "' message");
    }
    ...
    RUNTIME_EQ(action->getState(), wrench::Action::State::COMPLETED,
               "action state");
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceOneActionSuccessTest_test() {
...
class ActionExecutionServiceOneActionBogusSpecTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceOneActionBogusSpecTestWMS(
      ActionExecutionServiceTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    // Add a sleep action to it
    auto action =
        job->addComputeAction("compute", 100.0, 0.0, 100, 100,
    ...
    try {
      action_execution_service->submitAction(action);
      throw std::runtime_error(
          "Should not be able to submit action with bogus number of cores");
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceOneActionBogusSpecTest_test() {
...
class ActionExecutionServiceNonReadyActionTestWMS
    : public wrench::ExecutionController {
...
private:
  ...
  int main() override {
    ...
    auto action1 =
        job->addComputeAction("compute1", 100.0, 0.0, 1, 2,
                              wrench::ParallelModel::CONSTANTEFFICIENCY(1.0));
    auto action2 =
        job->addComputeAction("compute2", 100.0, 0.0, 1, 2,
                              wrench::ParallelModel::CONSTANTEFFICIENCY(1.0));
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to submit a non-READY action");
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceNonReadyActionTest_test() {
...
class ActionExecutionServiceOneActionTerminateTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceOneActionTerminateTestWMS(
      ActionExecutionServiceTest *test, std::string hostname)
...
private:
  ...
  int main() override {
    ...
    try {
      action_execution_service->submitAction(action);
      throw std::runtime_error("Should not be able to submit a non-ready "
                               "action to the action_execution_service");
    } catch (std::runtime_error &e) {
      // expected
    }

    // Terminate the action
    action_execution_service->terminateAction(
        action,
        wrench::ComputeService::TerminationCause::TERMINATION_JOB_KILLED);

    // Is the start-date sensible?
    RUNTIME_DBL_EQ(action->getStartDate(), 0, "action start date", EPSILON);
    //        if (action->getStartDate() < 0.0 or action->getStartDate() >
    //        EPSILON) {
    //            throw std::runtime_error("Unexpected action start date: " +
    //            std::to_string(action->getStartDate()));
    //        }

    // Is the end-date sensible?
    RUNTIME_DBL_EQ(action->getEndDate(), 5.0, "action end date", EPSILON);
    //        if (action->getEndDate() + EPSILON < 5.0 or action->getEndDate()
    //        > 5.0 + EPSILON) {
    //            throw std::runtime_error("Unexpected action end date: " +
    //            std::to_string(action->getEndDate()) + " (expected: ~5.0)");
    //        }

    // Is the state sensible?
    RUNTIME_EQ(action->getState(), wrench::Action::State::KILLED,
               "action state");
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceOneActionTerminateTest_test() {
...
class ActionExecutionServiceOneActionCrashRestartTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceOneActionCrashRestartTestWMS(
      ActionExecutionServiceTest *test,
      std::shared_ptr<wrench::Workflow> workflow, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    // Add a sleep action to it
    auto action = job->addComputeAction("my_compute", 100.0, 80.0, 2, 3,
    ...
    // Is the end-date sensible?
    RUNTIME_DBL_EQ(action->getEndDate(), 44.333333333333335701810,
                   "action end date", EPSILON);
    //        double expected = 44.333333333333335701810;
    //        if (std::abs<double>(action->getEndDate() - expected) > EPSILON) {
    //            throw std::runtime_error("Unexpected action end date: " +
    //            std::to_string(action->getEndDate()) +
    //                                     " (expected: " +
    ...
    if (history.top().ram_allocated != 80.0)
      throw std::runtime_error("Action history most recent: unexpected ram " +
                               std::to_string(history.top().ram_allocated));
    if (history.top().num_cores_allocated != 3)
      throw std::runtime_error(
          "Action history most recent: unexpected num cores " +
          std::to_string(history.top().num_cores_allocated));
    if (history.top().execution_host != "Host3")
      throw std::runtime_error(
          "Action history most recent: unexpected execution host " +
          history.top().execution_host);
    if (history.top().physical_execution_host != "Host3")
      throw std::runtime_error(
          "Action history most recent: unexpected physical execution host " +
          history.top().physical_execution_host);
    if (history.top().failure_cause != nullptr)
      throw std::runtime_error(
          "Action history most recent: unexpected failure cause: " +
          history.top().failure_cause->toString());
    if (history.top().start_date < 11.0 - EPSILON or
        history.top().start_date > 11.0 + EPSILON)
      throw std::runtime_error(
          "Action history most recent: unexpected start date: " +
          std::to_string(history.top().start_date));
    if (history.top().end_date + EPSILON < 44.33333333333333570181 or
        history.top().end_date > 44.33333333333333570181 + EPSILON)
      throw std::runtime_error(
          "Action history most recent: unexpected end date: " +
          std::to_string(history.top().end_date));
    // Older
    history.pop();
    if (history.top().ram_allocated != 80.0)
      throw std::runtime_error("Action history older: unexpected ram " +
                               std::to_string(history.top().ram_allocated));
    if (history.top().num_cores_allocated != 3)
      throw std::runtime_error(
          "Action history older: unexpected num cores " +
          std::to_string(history.top().num_cores_allocated));
    if (history.top().execution_host != "Host3")
      throw std::runtime_error(
          "Action history older: unexpected execution host " +
          history.top().execution_host);
    if (history.top().physical_execution_host != "Host3")
      throw std::runtime_error(
          "Action history older: unexpected physical execution host " +
          history.top().physical_execution_host);
    if (history.top().failure_cause == nullptr)
      throw std::runtime_error(
          "Action history older: There should be a failure cause: " +
          history.top().failure_cause->toString());
    if (history.top().start_date > EPSILON)
      throw std::runtime_error("Action history older: unexpected start date: " +
                               std::to_string(history.top().start_date));
    if (history.top().end_date + EPSILON < 1.0 or
        history.top().end_date > 1.0 + EPSILON)
      throw std::runtime_error("Action history older: unexpected end date: " +
                               std::to_string(history.top().end_date));
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceOneActionCrashRestartTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new ActionExecutionServiceOneActionCrashRestartTestWMS(
                          this, this->workflow, "Host1")));
...
class ActionExecutionServiceOneActionCrashNoRestartTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceOneActionCrashNoRestartTestWMS(
      ActionExecutionServiceTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    auto action_execution_service =
        std::shared_ptr<wrench::ActionExecutionService>(
            new wrench::ActionExecutionService(
                "Host2", compute_resources, this->getSharedPtr<Service>(),
                {{wrench::ActionExecutionServiceProperty::
                      FAIL_ACTION_AFTER_ACTION_EXECUTOR_CRASH,
                  "true"}},
                {}));
    ...
    //        if (action->getEndDate() + EPSILON < 1.0 or action->getEndDate()
    //        > 1.0 + EPSILON) {
    //            throw std::runtime_error("Unexpected action end date: " +
    //            std::to_string(action->getEndDate()) + " (expected: ~1.0)");
    //        }

    // Is the state sensible?
    RUNTIME_EQ(action->getState(), wrench::Action::State::FAILED,
               "action state");
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceOneActionCrashNoRestartTest_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(wms = simulation->add(
                      new ActionExecutionServiceOneActionCrashNoRestartTestWMS(
                          this, "Host1")));
...
class ActionExecutionServiceOneActionFailureTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceOneActionFailureTestWMS(
      ActionExecutionServiceTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    // Is the end-date sensible?
    if (action->getEndDate() + EPSILON < action->getStartDate() + 1.0 or
        action->getEndDate() > action->getStartDate() + 1.0 + EPSILON) {
      throw std::runtime_error("Unexpected action end date: " +
                               std::to_string(action->getEndDate()) +
                               " (expected: ~1.0)");
    }
    ...
    if (not(std::dynamic_pointer_cast<wrench::NetworkError>(
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceOneActionFailureTest_test() {
...
class ActionExecutionServiceOneActionNotEnoughResourcesTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceOneActionNotEnoughResourcesTestWMS(
      ActionExecutionServiceTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    auto action1 = job->addComputeAction("my_compute_1", 100.0, 200.0, 1, 1,
                                         wrench::ParallelModel::AMDAHL(1.0));
    auto action2 = job->addComputeAction("my_compute_2", 100.0, 80.0, 5, 5,
                                         wrench::ParallelModel::AMDAHL(1.0));
    ...
    try {
      ...
      if (not(std::dynamic_pointer_cast<wrench::NotEnoughResources>(
              e.getCause()))) {
        throw std::runtime_error("Unexpected failure cause");
      }
    }
...
TEST_F(ActionExecutionServiceTest, OneActionNotEnoughResources) {
  DO_TEST_WITH_FORK(
      do_ActionExecutionServiceOneActionNotEnoughResourcesTest_test);
}
...
void ActionExecutionServiceTest::
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(
          new ActionExecutionServiceOneActionNotEnoughResourcesTestWMS(
              this, "Host1")));
...
class ActionExecutionServiceThreeActionsInSequenceTestWMS
    : public wrench::ExecutionController {

public:
  ActionExecutionServiceThreeActionsInSequenceTestWMS(
      ActionExecutionServiceTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    auto action1 = job->addComputeAction("compute_action_1", 20.0, 80.0, 2, 2,
                                         wrench::ParallelModel::AMDAHL(1.0));

    // Add 2-core compute action action that will start 2nd
    auto action2 = job->addComputeAction("compute_action_2", 90.0, 90.0, 2, 2,
                                         wrench::ParallelModel::AMDAHL(1.0));

    // Add 1-core compute action action that will start 3rd
    auto action3 = job->addComputeAction("compute_action_3", 10.0, 20.0, 2, 2,
                                         wrench::ParallelModel::AMDAHL(1.0));
    ...
    // Did we get the expected message?
    auto msg1 = std::dynamic_pointer_cast<
        wrench::ActionExecutionServiceActionDoneMessage>(message);
    if (!msg1) {
      throw std::runtime_error("Unexpected '" + message->getName() +
                               "' message");
    ...
    if (msg1->action != action1) {
      throw std::runtime_error("Unexpected " + msg1->action->getName() +
                               " action completion");
    }
    ...
    // Did we get the expected message?
    auto msg2 = std::dynamic_pointer_cast<
        wrench::ActionExecutionServiceActionDoneMessage>(message);
    if (!msg2) {
      throw std::runtime_error("Unexpected '" + message->getName() +
                               "' message");
    ...
    if (msg2->action != action2) {
      throw std::runtime_error("Unexpected " + msg2->action->getName() +
                               " action completion");
    }
    ...
    // Did we get the expected message?
    auto msg3 = std::dynamic_pointer_cast<
        wrench::ActionExecutionServiceActionDoneMessage>(message);
    if (!msg3) {
      throw std::runtime_error("Unexpected '" + message->getName() +
                               "' message");
    ...
    if (msg3->action != action3) {
      throw std::runtime_error("Unexpected " + msg3->action->getName() +
                               " action completion");
    }

    // Are the dates sensible?
    if (action1->getStartDate() > EPSILON)
      throw std::runtime_error("Unexpected action1 start date " +
                               std::to_string(action1->getStartDate()));
    if (std::abs<double>(action1->getEndDate() - 10.0) > EPSILON)
      throw std::runtime_error("Unexpected action1 end date " +
                               std::to_string(action1->getEndDate()));
    if (std::abs<double>(action2->getStartDate() - 10.0) > EPSILON)
      throw std::runtime_error("Unexpected action2 start date " +
                               std::to_string(action2->getStartDate()));
    if (std::abs<double>(action2->getEndDate() - 55.0) > EPSILON)
      throw std::runtime_error("Unexpected action2 end date " +
                               std::to_string(action2->getEndDate()));
    if (std::abs<double>(action3->getStartDate() - 55.0) > EPSILON)
      throw std::runtime_error("Unexpected action3 start date " +
                               std::to_string(action3->getStartDate()));
    if (std::abs<double>(action3->getEndDate() - 60.0) > EPSILON)
      throw std::runtime_error("Unexpected action3 end date " +
                               std::to_string(action3->getEndDate()));
...
void ActionExecutionServiceTest::
    do_ActionExecutionServiceThreeActionsInSequenceTest_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  ASSERT_NO_THROW(wms = simulation->add(
                      new ActionExecutionServiceThreeActionsInSequenceTestWMS(
                          this, "Host1")));
