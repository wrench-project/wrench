...
WRENCH_LOG_CATEGORY(host_state_change_detector_service_test,
                    "Log category for HostStateChangeDetectorService test");
...
protected:
  HostStateChangeDetectorServiceTest() {
    ...
    std::string xml =
        ...
        "       <host id=\"Host2\" speed=\"1.0f,2.0f\" pstate=\"0\" "
        "core=\"1\"/> "
...
public:
  HostStateChangeDetectorTestWMS(HostStateChangeDetectorServiceTest *test,
      ...
      : wrench::ExecutionController(hostname, "test"), test(test),
        notify_when_speed_change(notify_when_speed_change) {}
...
private:
  ...
  int main() override {
    ...
    auto ssd = std::shared_ptr<wrench::HostStateChangeDetector>(
        new wrench::HostStateChangeDetector(
            this->hostname, hosts, true, true, this->notify_when_speed_change,
    ...
    if (not std::dynamic_pointer_cast<wrench::HostHasTurnedOffMessage>(
            message)) {
      throw std::runtime_error(
          "Did not get the expected 'host has turned off' message");
    }
    ...
    if (not std::dynamic_pointer_cast<wrench::HostHasTurnedOnMessage>(
            message)) {
      throw std::runtime_error(
          "Did not get the expected 'host has turned on' message");
    }
    ...
    if (this->notify_when_speed_change) {
      ...
      if (not std::dynamic_pointer_cast<wrench::HostHasChangedSpeedMessage>(
              message)) {
        throw std::runtime_error(
            "Did not get the expected 'host has changed speed' message");
      }
    }
...
void HostStateChangeDetectorServiceTest::do_StateChangeDetection_test(
  ...
  // Create the WMS
  auto wms = simulation->add(new HostStateChangeDetectorTestWMS(
      this, "Host1", notify_when_speed_change));
