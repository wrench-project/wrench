...
public:
  AlarmDownHostTestWMS(AlarmTest *test, std::string hostname)
  ...
  int main() override {
    ...
    try {
      wrench::Alarm::createAndStartAlarm(
          this->simulation, 10.0, "Host2", commport,
          new wrench::SimulationMessage(1), "bogus");
      throw std::runtime_error(
          "Should not be able to create an alarm on a down host");
...
TEST_F(AlarmTest, DownHost) { DO_TEST_WITH_FORK(do_downHost_Test); }
...
void AlarmTest::do_downHost_Test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new AlarmDownHostTestWMS(this, hostname)));
