...
WRENCH_LOG_CATEGORY(file_write_action_executor_test,
                    "Log category for FileWriteActionExecutorTest");
...
class FileWriteActionExecutorSuccessTestWMS
...
private:
  ...
  int main() override {
    ...
    auto file_write_action =
        job->addFileWriteAction("", this->test->file, this->test->ss1);
    ...
    RUNTIME_DBL_EQ(file_write_action->getStartDate(), 0.0, "action start date",
                   EPSILON);

    // Is the end-date sensible?
    RUNTIME_DBL_EQ(file_write_action->getEndDate(), 10.8349, "action end date",
                   EPSILON);

    // Is the state sensible?
    RUNTIME_EQ(file_write_action->getState(), wrench::Action::State::COMPLETED,
               "action state");
...
void FileWriteActionExecutorTest::do_FileWriteActionExecutorSuccessTest_test() {
  ...
  wms =
      simulation->add(new FileWriteActionExecutorSuccessTestWMS(this, "Host1"));
