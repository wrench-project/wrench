...
WRENCH_LOG_CATEGORY(file_read_action_executor_test,
                    "Log category for FileReadActionExecutorTest");
...
public:
  ...
  void
  do_FileReadActionExecutorKillingStorageServiceTest_test(double buffer_size);
...
class FileReadActionExecutorSuccessTestWMS
    : public wrench::ExecutionController {

public:
  FileReadActionExecutorSuccessTestWMS(
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to read more bytes than the file contains");
    ...
    auto file_read_action_executor = std::shared_ptr<wrench::ActionExecutor>(
        new wrench::ActionExecutor("Host2", 0, 0.0, 0, false, this->commport,
                                   file_read_action, nullptr));
    ...
    auto msg =
        std::dynamic_pointer_cast<wrench::ActionExecutorDoneMessage>(message);
    if (!msg) {
    ...
    RUNTIME_DBL_EQ(file_read_action->getStartDate(), 0.0, "action start date",
                   EPSILON);

    // Is the end-date sensible?
    RUNTIME_DBL_EQ(file_read_action->getEndDate(), 10.8349, "action end date",
                   EPSILON);

    // Is the state sensible?
    RUNTIME_EQ(file_read_action->getState(), wrench::Action::State::COMPLETED,
               "action state");
...
void FileReadActionExecutorTest::do_FileReadActionExecutorSuccessTest_test() {
  ...
  wms = simulation->add(
      new FileReadActionExecutorSuccessTestWMS(this, workflow, "Host1"));
...
class FileReadActionExecutorMultipleAttemptsSuccessTestWMS
    : public wrench::ExecutionController {

public:
  FileReadActionExecutorMultipleAttemptsSuccessTestWMS(
      FileReadActionExecutorTest *test,
      std::shared_ptr<wrench::Workflow> workflow, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test),
        workflow(workflow) {}
...
private:
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Shouldn't be able to add a file-read action with a vector of "
          "locations that are not for the same file");
    } catch (std::invalid_argument &ignore) {
    }
    ...
    RUNTIME_EQ(file_read_action->getState(), wrench::Action::State::COMPLETED,
               "action state");
    //        if (file_read_action->getState() !=
    //        wrench::Action::State::COMPLETED) {
    //            throw std::runtime_error("Unexpected action state: " +
    //            file_read_action->getStateAsString());
    //        }
...
void FileReadActionExecutorTest::
    do_FileReadActionExecutorMultipleAttemptsSuccessTest_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  wms =
      simulation->add(new FileReadActionExecutorMultipleAttemptsSuccessTestWMS(
          this, workflow, "Host1"));
...
class FileReadActionExecutorMissingFileTestWMS
    : public wrench::ExecutionController {
...
private:
  ...
  int main() override {
    ...
    if (not std::dynamic_pointer_cast<wrench::FileNotFound>(
            file_read_action->getFailureCause())) {
      throw std::runtime_error("Unexpected failure cause: " +
                               file_read_action->getFailureCause()->toString());
    }
...
void FileReadActionExecutorTest::
    do_FileReadActionExecutorMissingFileTest_test() {
...
class FileReadActionExecutorKillingStorageServiceTestWMS
    : public wrench::ExecutionController {

public:
  FileReadActionExecutorKillingStorageServiceTestWMS(
      FileReadActionExecutorTest *test, std::string hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
TEST_F(FileReadActionExecutorTest, KillingStorageServiceTest) {
  DO_TEST_WITH_FORK_ONE_ARG(
      do_FileReadActionExecutorKillingStorageServiceTest_test, 1000000);
  //    DO_TEST_WITH_FORK_ONE_ARG(do_FileReadActionExecutorKillingStorageServiceTest_test,
  //    0);
}

void FileReadActionExecutorTest::
    do_FileReadActionExecutorKillingStorageServiceTest_test(
        double buffer_size) {
  ...
  ASSERT_NO_THROW(wms = simulation->add(
                      new FileReadActionExecutorKillingStorageServiceTestWMS(
                          this, "Host1")));
