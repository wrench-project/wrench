...
WRENCH_LOG_CATEGORY(file_delete_action_executor_test,
                    "Log category for FileDeleteActionExecutorTest");
...
class FileDeleteActionExecutorSuccessTestWMS
...
private:
  ...
  int main() override {
    ...
    auto file_delete_action =
        job->addFileDeleteAction("", this->test->file, this->test->ss);
    // coverage
    ...
    file_delete_action_executor->start(file_delete_action_executor, true,
                                       false);
    ...
    RUNTIME_DBL_EQ(file_delete_action->getStartDate(), 0.0, "action start date",
                   EPSILON);
    RUNTIME_DBL_EQ(file_delete_action->getEndDate(), 0.216755,
                   "action end date", EPSILON);

    // Is the state sensible
    RUNTIME_EQ(file_delete_action->getState(), wrench::Action::State::COMPLETED,
               "action state");
...
void FileDeleteActionExecutorTest::
