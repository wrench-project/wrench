...
WRENCH_LOG_CATEGORY(compute_action_executor_test,
                    "Log category for ComputeActionExecutorTest");
...
public:
  ComputeActionExecutorTestWMS(ComputeActionExecutorTest *test,
      ...
      : wrench::ExecutionController(hostname, "test"), test(test),
        simulation_computation_as_sleep(simulation_computation_as_sleep) {}
...
private:
  ...
  int main() override {
    ...
    auto action_executor =
        std::shared_ptr<wrench::ActionExecutor>(new wrench::ActionExecutor(
            "Host2", num_cores, ram, 0.1, simulation_computation_as_sleep,
            this->commport, action, nullptr));
    ...
    if (action->getEndDate() + EPSILON < 10.2 or
        action->getEndDate() > 10.2 + EPSILON) {
...
void ComputeActionExecutorTest::do_ComputeActionExecutorSuccessTest_test(
  ...
  ASSERT_NO_THROW(wms = simulation->add(new ComputeActionExecutorTestWMS(
                      this, "Host1", simulation_computation_as_sleep)));
...
private:
  ...
  int main() override {
    ...
    // Coverage
    try {
      job->addComputeAction("", -1.0, -1.0, 0, 1,
                            wrench::ParallelModel::AMDAHL(1.0));
      throw std::runtime_error(
          "Shouldn't be able to create a compute action with bogus arguments");
    } catch (std::invalid_argument &ignore) {
    }

    // Too much RAM
    action = std::dynamic_pointer_cast<wrench::Action>(job->addComputeAction(
        "", 20.0, 300.0, 1, 4, wrench::ParallelModel::AMDAHL(1.0)));

    auto action_executor = std::shared_ptr<wrench::ActionExecutor>(
        new wrench::ActionExecutor("Host2", num_cores, ram, 0.1, false,
                                   this->commport, action, nullptr));
    ...
    if (not std::dynamic_pointer_cast<wrench::FatalFailure>(
            action->getFailureCause())) {
      throw std::runtime_error(
          "Action's failure cause should be NotEnoughResources");
    }
