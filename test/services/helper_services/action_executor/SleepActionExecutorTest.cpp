...
WRENCH_LOG_CATEGORY(sleep_action_executor_test,
                    "Log category for SleepActionExecutorTest");
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to create sleep action with negative sleep time");
    ...
    auto sleep_action_executor =
        std::shared_ptr<wrench::ActionExecutor>(new wrench::ActionExecutor(
            "Host2", 0, 0.0, 0, false, this->commport, sleep_action, nullptr));
    // Start it
    ...
    if (sleep_action->getStartDate() < 0.0 or
        sleep_action->getStartDate() > EPSILON) {
      throw std::runtime_error("Unexpected action start date: " +
                               std::to_string(sleep_action->getStartDate()));
    }

    // Is the end-date sensible?
    if (sleep_action->getEndDate() + EPSILON < 10.0 or
        sleep_action->getEndDate() > 10.0 + EPSILON) {
      throw std::runtime_error("Unexpected action end date: " +
                               std::to_string(sleep_action->getEndDate()));
    }
...
void SleepActionExecutorTest::do_SleepActionExecutorSuccessTest_test() {
  ...
  ASSERT_NO_THROW(wms = simulation->add(new SleepActionExecutorSuccessTestWMS(
                      this, wrench::Workflow::createWorkflow(), "Host1")));
