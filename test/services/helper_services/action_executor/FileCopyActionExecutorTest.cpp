...
WRENCH_LOG_CATEGORY(file_copy_action_executor_test,
                    "Log category for FileCopyActionExecutorTest");
...
class FileCopyActionExecutorSuccessTestWMS
...
private:
  ...
  int main() override {
    ...
    auto file_copy_action = job->addFileCopyAction(
        "", this->test->file, this->test->ss1, this->test->ss2);
    ...
    auto file_copy_action_executor = std::make_shared<wrench::ActionExecutor>(
        "Host2", 0, 0.0, 0, false, this->commport, file_copy_action, nullptr);
    // Start it
    ...
    // Did the copy happen?
    if (not wrench::StorageService::hasFileAtLocation(
    ...
    if (file_copy_action->getStartDate() < 0.0 or
        file_copy_action->getStartDate() > EPSILON) {
      throw std::runtime_error(
          "Unexpected action start date: " +
          std::to_string(file_copy_action->getStartDate()));
    }

    // Is the end-date sensible?
    if (file_copy_action->getEndDate() < 10.8 or
        file_copy_action->getEndDate() > 10.9) {
      throw std::runtime_error("Unexpected action end date: " +
                               std::to_string(file_copy_action->getEndDate()) +
                               " (expected: ~10.9)");
    }
    ...
    if (file_copy_action->getState() != wrench::Action::State::COMPLETED) {
      throw std::runtime_error("Unexpected action state: " +
                               file_copy_action->getStateAsString());
    }
...
void FileCopyActionExecutorTest::do_FileCopyActionExecutorSuccessTest_test() {
  ...
  wms =
      simulation->add(new FileCopyActionExecutorSuccessTestWMS(this, "Host1"));
...
class FileCopyActionExecutorSuccessSameHostTestWMS
    : public wrench::ExecutionController {
...
void FileCopyActionExecutorTest::
    do_FileCopyActionExecutorSuccessSameHostTest_test() {
  ...
  std::shared_ptr<wrench::ExecutionController> wms = nullptr;
  wms = simulation->add(
      new FileCopyActionExecutorSuccessSameHostTestWMS(this, "Host1"));
