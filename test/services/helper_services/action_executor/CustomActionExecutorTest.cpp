...
WRENCH_LOG_CATEGORY(custom_action_executor_test,
                    "Log category for CustomActionExecutorTest");
...
private:
  ...
  int main() override {
    ...
    auto lambda_execute =
        [storage_service,
         file](const std::shared_ptr<wrench::ActionExecutor> &action_executor) {
    ...
    auto action_executor = std::make_shared<wrench::ActionExecutor>(
        "Host2", num_cores, ram, 0, false, this->commport, action, nullptr);
    ...
    //            std::to_string(action->getEndDate()) + " (expected:
    //            ~20.847443)");
    //        }
...
void CustomActionExecutorTest::do_CustomActionExecutorSuccessTest_test() {
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new CustomActionExecutorTestWMS(this, "Host1")));
