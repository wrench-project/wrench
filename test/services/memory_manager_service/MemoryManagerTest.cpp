...
protected:
  ...
  MemoryManagerTest() {
    ...
    std::string xml =
        ...
        "             <prop id=\"mount\" value=\"/\"/>"
        "          </disk>"
        "          <disk id=\"memory\" read_bw=\"1000MBps\" "
        "write_bw=\"1000MBps\">"
        "             <prop id=\"size\" value=\"" +
        std::to_string(PAGE_CACHE_RAM_SIZE) +
        "GB\"/>"
  ...
  std::string bad_platform_file_path =
      UNIQUE_TMP_PATH_PREFIX + "bad_platform.xml";
...
void MemoryManagerTest::do_MemoryManagerBadSetupTest_test() {
  ...
  ASSERT_THROW(simulation->instantiatePlatform(bad_platform_file_path),
...
public:
  MemoryManagerChainOfTasksTestWMS(MemoryManagerTest *test,
                                   std::shared_ptr<wrench::Workflow> workflow,
                                   std::string &hostname)
...
private:
  ...
  int main() override {
          ...
          task, {{task->getInputFiles().at(0),
                 ...
                 {task->getOutputFiles().at(0),
                  wrench::FileLocation::LOCATION(
                      test->storage_service1, task->getOutputFiles().at(0))}});
      // Submit the job
    ...
    // Coverage
    auto memory_manager =
        this->simulation->getMemoryManagerByHost(this->getHostname());
...
void MemoryManagerTest::do_MemoryManagerChainOfTasksTest_test() {
  ...
  auto previous_output_file =
      wrench::Simulation::addFile("task0_input", FILE_SIZE * GB);
  int num_tasks = 10;
  for (int i = 0; i < num_tasks; i++) {
    auto task =
        workflow->addTask("task1" + std::to_string(i), 100.0, 1, 1, 0.0);
    auto output_file = wrench::Simulation::addFile(
        "task1" + std::to_string(i) + "_output", FILE_SIZE * GB);
  ...
  ASSERT_NO_THROW(wms = simulation->add(new MemoryManagerChainOfTasksTestWMS(
                      this, workflow, hostname)));
  // Create a File Registry Service
  ASSERT_NO_THROW(simulation->add(new wrench::FileRegistryService(hostname)));

  // Staging the input_file on storage service #1
  ASSERT_NO_THROW(simulation->stageFile(
      wrench::Simulation::getFileByID("task0_input"), storage_service1));
