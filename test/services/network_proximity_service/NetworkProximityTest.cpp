...
WRENCH_LOG_CATEGORY(network_proximity_service_test,
                    "Log category for NetworkProximityTest");
...
public:
  ProxTestWMS(NetworkProximityTest *test, const std::string &hostname)
...
private:
  ...
  int main() override {
    ...
    hosts_to_compute_proximity =
        std::make_pair(wrench::Simulation::getHostnameList()[2],
    ...
    double proximity = network_proximity_service
                           ->getHostPairDistance(hosts_to_compute_proximity)
                           .first;

    while (proximity == DBL_MAX && count < max_count) {
      count++;
      wrench::S4U_Simulation::sleep(20.0);
      proximity = network_proximity_service
                      ->getHostPairDistance(hosts_to_compute_proximity)
                      .first;
    ...
    this->test->network_proximity_service->stop();

    try {
      this->test->network_proximity_service->getHostPairDistance(
          hosts_to_compute_proximity);
      throw std::runtime_error(
          "Should not be able to query a service that is down");
      ...
      if (cause->getService() != (this->test->network_proximity_service)) {
        throw std::runtime_error(
            "Got the expected 'service is down' exception, but the failure "
            "cause does not point to the correct service");
      }
    }
...
void NetworkProximityTest::do_NetworkProximity_Test() {
  ...
  std::string network_proximity_db_hostname =
      wrench::Simulation::getHostnameList()[1];
  ...
  std::vector<std::string> hosts_in_network = {
      network_daemon1, network_daemon2, network_daemon3, network_daemon4};

  // A few bogus constructor calls
  ASSERT_THROW(new wrench::NetworkProximityService(
                   network_proximity_db_hostname, hosts_in_network,
  ...
  ASSERT_THROW(new wrench::NetworkProximityService(
                   network_proximity_db_hostname, too_few_hosts, {}),
               std::invalid_argument);
  ...
  ASSERT_NO_THROW(wms = simulation->add(new ProxTestWMS(this, hostname)));
...
public:
  CompareProxTestWMS(NetworkProximityTest *test, const std::string &hostname)
      : wrench::ExecutionController(hostname, "test"), test(test) {}
...
private:
  ...
  int main() override {
    ...
    double first_pair_proximity =
        this->test->network_proximity_service
            ->getHostPairDistance(first_pair_to_compute_proximity)
            .first;

    while (first_pair_proximity == DBL_MAX && count < max_count) {
      count++;
      wrench::Simulation::sleep(10.0);
      first_pair_proximity =
          this->test->network_proximity_service
              ->getHostPairDistance(first_pair_to_compute_proximity)
              .first;
    }
    ...
    double second_pair_proximity =
        this->test->network_proximity_service
            ->getHostPairDistance(second_pair_to_compute_proximity)
            .first;

    while (second_pair_proximity == DBL_MAX && count < max_count) {
      count++;
      wrench::Simulation::sleep(10.0);
      second_pair_proximity =
          this->test->network_proximity_service
              ->getHostPairDistance(second_pair_to_compute_proximity)
              .first;
    }
    ...
    if (first_pair_proximity > second_pair_proximity) {
      throw std::runtime_error("Expected proximity between a pair to be less "
                               "than the other pair of hosts");
    }
    return 0;
  }
};
...
void NetworkProximityTest::do_CompareNetworkProximity_Test() {
  ...
  ASSERT_NO_THROW(wms =
                      simulation->add(new CompareProxTestWMS(this, hostname)));
...
public:
  VivaldiConvergeWMS(NetworkProximityTest *test,
                     std::set<std::shared_ptr<wrench::NetworkProximityService>>
...
private:
  ...
  int main() override {
    ...
    for (auto &nps : this->network_proximity_services) {
    ...
    double alltoall_proximity =
        alltoall_service->getHostPairDistance(hosts_to_compute_proximity).first;
    double vivaldi_proximity =
        vivaldi_service->getHostPairDistance(hosts_to_compute_proximity).first;
    ...
    std::pair<double, double> coordinates =
        vivaldi_service->getHostCoordinate(target_host).first;

    if (coordinates.first == 0 && coordinates.second == 0) {
      throw std::runtime_error(
          "Vivaldi algorithm did not update the coordinates of host:" +
          target_host);
    }
    ...
    try {
      ...
      throw std::runtime_error("Should not be able to get coordinates from an "
                               "all-to-all proximity service");
    ...
    try {
      ...
      throw std::runtime_error(
          "Should not be able to get coordinates from a service that is down");
    } catch (wrench::ExecutionException &e) {
      // Check Exception
      auto cause =
          std::dynamic_pointer_cast<wrench::ServiceIsDown>(e.getCause());
...
void NetworkProximityTest::do_VivaldiConverge_Test() {
  ...
  std::shared_ptr<wrench::NetworkProximityService> alltoall_network_service =
      nullptr;
  std::shared_ptr<wrench::NetworkProximityService> vivaldi_network_service =
      nullptr;
  ...
  ASSERT_NO_THROW(wms = simulation->add(new VivaldiConvergeWMS(
                      this, {alltoall_network_service, vivaldi_network_service},
                      hostname)));
...
public:
  ValidatePropertiesWMS(NetworkProximityTest *test, std::string hostname)
...
void NetworkProximityTest::do_ValidateProperties_Test() {
  ...
  ASSERT_THROW(network_proximity_service =
                   simulation->add(new wrench::NetworkProximityService(
                       network_proximity_db_hostname, hosts_in_network,
                       {{wrench::NetworkProximityServiceProperty::
                             NETWORK_PROXIMITY_SERVICE_TYPE,
  ...
  ASSERT_NO_THROW(
      wms = simulation->add(new ValidatePropertiesWMS(this, hostname)));
