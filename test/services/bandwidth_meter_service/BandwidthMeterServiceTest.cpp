...
WRENCH_LOG_CATEGORY(bandwidth_meter_service_test,
                    "Log category for BandwidthMeterServiceTest");
  ...
  int main() override {
    ...
    try {
      ...
      throw std::runtime_error(
          "Bad invocation of simple constructor should have thrown");
    ...
    try {
      ...
      this->createBandwidthMeter(measurement_periods);
      throw std::runtime_error(
          "Bad invocation of complex constructor should have thrown");
    } catch (std::invalid_argument &e) {
      // expected
    }
...
void BandwidthMeterServiceTest::do_BandwidthMeterCreationDestruction_test() {
  ...
  EXPECT_NO_THROW(
      wms = simulation->add(new BasicCreationDestructionTestWMS(this, host)));
