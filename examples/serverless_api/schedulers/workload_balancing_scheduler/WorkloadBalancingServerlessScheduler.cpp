 ...
 */
 ...
 std::vector<std::pair<std::shared_ptr<Invocation>, std::string>> WorkloadBalancingServerlessScheduler::scheduleFunctions(
     const std::vector<std::shared_ptr<Invocation>>& schedulableInvocations,
     std::shared_ptr<ServerlessStateOfTheSystem> state
 ) {
     ...
     for (const auto& [node, function_allocation] : allocation_plan) {
         for (const auto& [function_name, cores_allocated] : function_allocation) {
             if (cores_allocated == 0 || invocations_by_function.find(function_name) == invocations_by_function.end()) {
                 continue;
             }
             ...
             while (scheduled < cores_allocated && !invocations.empty() && availableCores[node] > 0) {
                 ...
                 if (state->isImageOnNode(node, image)) {
                     schedulingDecisions.push_back({inv, node});
                     availableCores[node]--;
                     scheduled++;
                 }
             }
 ...
 void WorkloadBalancingServerlessScheduler::calculateFunctionWorkloads(const std::vector<std::shared_ptr<Invocation>>& invocations) {
     // Clear existing data
     ...
     for (const auto& inv : invocations) {
         ...
         double time_limit = inv->getRegisteredFunction()->getTimeLimit();
 ...
 void WorkloadBalancingServerlessScheduler::createAllocationPlan(const std::shared_ptr<ServerlessStateOfTheSystem>& state) {
     // Clear existing plan
     ...
     for (const auto& [function_name, workload] : function_workloads) {
         double proportion = workload / total_workload;
         unsigned cores_for_function = static_cast<unsigned>(std::ceil(proportion * total_cores));
         ...
         if (cores_for_function > 0) {
             function_core_allocation.push_back({function_name, cores_for_function});
         }
     }
     ...
     for (const auto& [function_name, cores_needed] : function_core_allocation) {
         ...
         while (cores_remaining > 0) {
             ...
             // Allocate cores
             unsigned to_allocate = std::min(cores_remaining, best_available);
