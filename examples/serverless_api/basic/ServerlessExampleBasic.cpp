...
/**
 ** This simulator simulates the execution of a bunch of function invocations placed
 ...
 ** frontend to the serverless compute service (ServerlessHeadNode), which can use some number of compute nodes
 ** to run functions (ServerlessComputeNode1, ServerlessComputeNode2, ...). The number of compute nodes
 ** is set via a command-line arguments, as is the number of function invocations placed by the controller.
 **
...
/**
 * Helper functor class to create the hardware platform (programatically, instead of via an XML file).
 **/
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " <# function invocation> <# compute nodes> [--log=custom_controller.threshold=info]" << std::endl;
        exit(1);
    }
    unsigned int num_invocations = atoi(argv[1]);
    unsigned int num_compute_nodes = atoi(argv[2]);

    /* Using the basic First Come First Serve scheduler */
    std::string scheduler_type = "fcfs";

    auto scheduler = std::make_shared<wrench::FCFSServerlessScheduler>();

    /* Create the simulated platform programatically */
