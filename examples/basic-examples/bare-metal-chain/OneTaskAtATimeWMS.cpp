...
namespace wrench {
    ...
    int OneTaskAtATimeWMS::main() {
        ...
        while (not this->getWorkflow()->isDone()) {
            ...
            /* Create a standard job for the task, first creating a map that defines from
             * which locations the task's input/output files should be read */
            ...
            /* Submit the job to the compute service, specifying that 5 cores should be used */
            std::map<std::string, std::string> service_specific_arguments;
            service_specific_arguments[ready_task->getID()] = "5";
            job_manager->submitJob(standard_job, compute_service, service_specific_arguments);
