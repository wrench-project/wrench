...
Controller::Controller(
    ...
    const shared_ptr<StorageService> &target, const std::string &hostname)
    : ExecutionController(hostname, "controller"),
      bare_metal_compute_service(bare_metal_compute_service), proxy(proxy),
...
int Controller::main() {
  ...
  // proxy->createFile(cachedFile); // What this line should do is ambiguous and
  // not supported.
  ...
  /* Another way to do the same */
  StorageService::readFileAtLocation(
      ProxyLocation::LOCATION(target, proxy, targetFile));
  ...
  /* Another way to do the same */
  StorageService::writeFileAtLocation(
      FileLocation::LOCATION(proxy, remoteFile));
  ...
  WRENCH_INFO("Submitting a job that should successfully read the file from "
              "the Proxy using the default remote");
  ...
  WRENCH_INFO("Submitting a job that should successfully read the file from "
              "Proxy using the non-default target");
  auto job2 = job_manager->createCompoundJob("job2");

  /* read a file found on the non default target remote via the proxy */
  auto fileread2 = job2->addFileReadAction(
      "fileread2", ProxyLocation::LOCATION(target, proxy, remoteFile));
 ...
 */
void Controller::processEventCompoundJobCompletion(
    std::shared_ptr<CompoundJobCompletedEvent> event) {
  /* Retrieve the job that this event is for */
  auto job = event->job;
  WRENCH_INFO("Notified that compound job %s has successfully completed",
              job->getName().c_str());
}
 ...
 * @param event: the event
 */
void Controller::processEventCompoundJobFailure(
    std::shared_ptr<CompoundJobFailedEvent> event) {
  /* Retrieve the job that this event is for */
  auto job = event->job;
  auto action = *(job->getActions().begin());

  WRENCH_INFO("Notified that compound job %s has failed (%s)",
              job->getName().c_str(),
