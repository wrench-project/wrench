...
Controller::Controller(
    ...
    XRootD::Deployment *xrootd_deployment, const std::string &hostname)
...
int Controller::main() {
  ...
  /* This is the XRootD tree that was constructed before the simulation was
launched.
   * We will create replicas of the "some_file" file at nodes leaf4, leaf8,
leaf9, which
...
*/
  ...
  WRENCH_INFO(
      "Submitting a job that should successfully read the file from XRootD");
  ...
  /* Delete the file replica at leaf4 and submit a similar job, which now will
   * fail
  ...
  /* Sleep 2 hours, which is long enough for the XRootD cache to be cleared
   * (based on the {wrench::XRootD::Property::CACHE_MAX_LIFETIME,"3600"}
   * property passed to the Deployment constructor in Simulator.cpp
   */
