...
BatchMPIActionController::BatchMPIActionController(
    ...
    std::shared_ptr<StorageService> ss, const std::string &hostname)
    : ExecutionController(hostname, "mamj"), batch_cs(std::move(batch_cs)),
      ss(std::move(ss)) {}
...
int BatchMPIActionController::main() {
  ...
  /* MPI code to execute as part of the action. Note that there are extra
   * barriers so that the log output is not weirdly interleaved */
  auto mpi_code = [storage_service, file](
                      const std::shared_ptr<ExecutionController> &controller) {
    int num_procs;
    int my_rank;
    ...
    for (unsigned long iter = 0; iter < 10; iter++) {
      ...
      wrench::Simulation::computeMultiThreaded(
          num_threads, thread_creation_overhead, sequential_work,
          parallel_per_thread_work);

      // Launch an asynchronous IO read to the storage service
      double num_io_bytes = 100 * MB;
      data_manager->initiateAsynchronousFileRead(
          FileLocation::LOCATION(storage_service, file), num_io_bytes);

      // Participate in an all-to-all communication
      MPI_Alltoall(data, num_comm_bytes, MPI_CHAR, data, num_comm_bytes,
                   MPI_CHAR, MPI_COMM_WORLD);

      // Wait for the asynchronous IO read to complete
      auto event = controller->waitForNextEvent();
      auto io_event =
          std::dynamic_pointer_cast<wrench::FileReadCompletedEvent>(event);
      if (not io_event) {
        throw std::runtime_error("Custom action: unexpected IO event: " +
                                 io_event->toString());
      }
    }
  ...
  job->addMPIAction("my_mpi_action", mpi_code, num_mpi_processes,
                    num_cores_per_process);
  ...
  std::map<std::string, std::string> service_specific_args = {
      ...
      {"-c", std::to_string(
                 num_cores_per_process)},  // Number of cores per compute node
