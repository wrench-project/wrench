...
/**
 ** This simulator simulates the execution of a MPI action
 **
 ** Example invocation of the simulator with no logging:
 **    ./wrench-example-mpi-action ./dragonfly_cluster.xml
 **
 ...
 **    ./wrench-example-wrench-example-mpi-action ./dragonfly_cluster.xml
 *--wrench-full-log
 **/
...
#include "BatchMPIActionController.h" // Controller implementation
...
int main(int argc, char **argv) {
  ...
  auto batch_service = simulation->add(
      new wrench::BatchComputeService(head_node, host_list, "", {}, {}));

  /* Create a new disk on the platform */
  wrench::Simulation::createNewDisk(head_node, "disk0", 100 * MBps, 50 * MBps,
                                    50 * TB, "/");
  ...
  auto wms = simulation->add(new wrench::BatchMPIActionController(
      batch_service, storage_service, host_list.at(0)));
