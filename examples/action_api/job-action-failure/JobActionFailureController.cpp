...
WRENCH_LOG_CATEGORY(custom_controller,
                    "Log category for JobActionFailureController");
...
JobActionFailureController::JobActionFailureController(
    ...
    : ExecutionController(hostname, "mamj"), bm_cs(std::move(bm_cs)),
      ss_1(std::move(ss_1)), ss_2(std::move(ss_2)) {}
...
int JobActionFailureController::main() {
   ...
   * simulation, but it is too big for the storage service where we will write
   * it! */
  ...
  auto compute_action = job->addComputeAction(
      "compute", 200 * GFLOP, 100, 3, 4, wrench::ParallelModel::AMDAHL(0.95));

  /* Add a file-write action that will fail */
  auto file_write_action = job->addFileWriteAction(
      "file_write", wrench::FileLocation::LOCATION(ss_2, output_file));

  /* Add a long compute action that will fail when we kill the compute service!
   */
  auto compute_long =
      job->addComputeAction("compute_long", 10000 * GFLOP, 100, 1, 20,
                            wrench::ParallelModel::AMDAHL(0.57));
  ...
  this->bm_cs->stop(true, wrench::ComputeService::TerminationCause::
  ...
  if (auto job_completion_event =
    ...
    WRENCH_INFO("Job failure cause: %s",
                job_completion_event->failure_cause->toString().c_str());
    WRENCH_INFO("It had %zu actions:", completed_job->getActions().size());
    for (auto const &action : completed_job->getActions()) {
      WRENCH_INFO("  * Action %s: in state %s", action->getName().c_str(),
