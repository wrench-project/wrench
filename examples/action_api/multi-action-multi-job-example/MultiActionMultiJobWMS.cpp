...
#include "MultiActionMultiJobWMS.h"

WRENCH_LOG_CATEGORY(custom_wms, "Log category for MultiActionMultiJobWMS");
...
namespace wrench {
     ...
     */
    MultiActionMultiJobWMS::MultiActionMultiJobWMS(const std::set<std::shared_ptr<ComputeService>> &compute_services,
    ...
    /**
     * @brief main method of the MultiActionMultiJobWMS daemon
     *
     * @return 0 on completion
     *
     * @throw std::runtime_error
     */
    int MultiActionMultiJobWMS::main() {
     ...
     */
    void MultiActionMultiJobWMS::processEventStandardJobCompletion(std::shared_ptr<StandardJobCompletedEvent> event) {
     ...
     * @param event: the event
     */
    void MultiActionMultiJobWMS::processEventStandardJobFailure(std::shared_ptr<StandardJobFailedEvent> event) {
        /* Retrieve the job that this event is for */
        auto job = event->standard_job;
        /* Retrieve the job's first (and in our case only) task */
        auto task = job->getTasks().at(0);
