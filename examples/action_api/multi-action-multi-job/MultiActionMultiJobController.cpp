...
WRENCH_LOG_CATEGORY(custom_controller,
                    "Log category for MultiActionMultiJobController");
...
MultiActionMultiJobController::MultiActionMultiJobController(
    ...
    std::shared_ptr<StorageService> ss_1, std::shared_ptr<StorageService> ss_2,
...
int MultiActionMultiJobController::main() {
  ...
  /* Create a two-action job where the first action computes something, and the
   * second action write data to data_file somewhere */
  auto job2 = job_manager->createCompoundJob("job2");
  auto job2_compute_action =
      job2->addComputeAction("compute", 1000 * GFLOP, 5 * MB, 2, 8,
                             wrench::ParallelModel::AMDAHL(0.8));
  // If no action name is given, a unique name will be picked
  auto job2_file_write_action = job2->addFileWriteAction(
      "", wrench::FileLocation::LOCATION(ss_1, output_file));
  ...
  // A small data structure that, just for fun, specifies a target storage
  // service to used based on the custom action is running
  std::map<std::string, std::shared_ptr<StorageService>> ss_to_use = {
      {"ComputeHost1", ss_1}, {"ComputeHost2", ss_2}};

  // Create the custom action with two lambdas
  job3->addCustomAction(
      "file_copy", 0, 0,
      [ss_to_use,
       input_file](const std::shared_ptr<ActionExecutor> &action_executor) {
        ...
        WRENCH_INFO(
            "Custom action about to read file from storage service on host %s",
            target_ss->getHostname().c_str());
        // Read an input_file from the target storage service (which takes some
        // time)
        target_ss->readFile(
            wrench::FileLocation::LOCATION(target_ss, "/data/", input_file));
        // Sleep for 5 seconds
        WRENCH_INFO(
            "Custom action got the file and now is sleeping for 5 seconds");
        Simulation::sleep(5.0);
        WRENCH_INFO("Custom action deleting the file!");
        // Deleted the input file from the target storage service!
        target_ss->deleteFile(
            wrench::FileLocation::LOCATION(target_ss, "/data/", input_file));
      },
  ...
  WRENCH_INFO("Submitting job %s to the bare-metal compute service",
              job2->getName().c_str());
  ...
  /* Wait for and react to execution events. We should be getting 3 "job
   * completed" events */
  int num_events = 0;
  while (num_events < 3) {
    ...
    if (auto job_completion_event =
      ...
      for (auto const &action : completed_job->getActions()) {
        ...
        WRENCH_INFO(
            "     - it started at time %.2lf and finished at time %.2lf",
