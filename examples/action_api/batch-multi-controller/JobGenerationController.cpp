...
namespace wrench {
    ...
    JobGenerationController::JobGenerationController(const std::string& hostname,
                                                     ...
                                                     batch_service_controllers) : ExecutionController(hostname, "me"),
        _num_jobs(num_jobs), _batch_service_controllers(batch_service_controllers) {
    }
    ...
    int JobGenerationController::main() {
        ...
        for (int i = 0; i < _num_jobs; i++) {
            ...
            int arrival_time = now + dist(rng) % 100;
            now += arrival_time;
        ...
        while (num_completed_jobs < _num_jobs) {
            ...
            if (std::dynamic_pointer_cast<TimerEvent>(event)) {
                ...
                WRENCH_INFO("Sending a job request for job %s to batch service controller #%d",
                ...
                if (auto job_notification_message = std::dynamic_pointer_cast<JobNotificationMessage>(
                    ...
                    WRENCH_INFO("Notified that job %s has completed!", job_notification_message->_name.c_str());
                    num_completed_jobs++;
                }
            }
        }
