...
WRENCH_LOG_CATEGORY(custom_controller,
                    "Log category for SuperCustomActionController");

namespace wrench {

/**
 * @brief A class that showcases how one can extend the wrench::CustomAction
 * class. In this case, this is a simple extension that adds a single integer
 *        instance variable.
 */
...
public:
  ...
  DerivedCustomAction(
      ...
      std::function<void(std::shared_ptr<ActionExecutor> action_executor)>
          lambda_terminate,
      int value)
      : CustomAction(name, ram, num_cores, std::move(lambda_execute),
...
SuperCustomActionController::SuperCustomActionController(
    ...
    std::shared_ptr<StorageService> ss_1, const std::string &hostname)
    : ExecutionController(hostname, "mamj"), bm_cs(std::move(bm_cs)),
      cloud_cs(std::move(cloud_cs)), ss_1(std::move(ss_1)) {}
...
int SuperCustomActionController::main() {
  ...
  WRENCH_INFO("Controller starting on host %s",
              Simulation::getHostName().c_str());

  /* Create a job manager so that we can create/submit jobs */
  auto job_manager = this->createJobManager();

  /* Specify a 1000-byte file that will be written to at some point in the
   * simulation */
  auto output_file = Simulation::addFile("output_file", 100 * MB);

  /* Specify a 10,000-nyte file that will be read from at some point in the
   * simulation */
  auto input_file = Simulation::addFile("input_file", 200 * MB);

  /* Creates an instance of input_file on both storage services. This takes zero
   * simulation time. After
   * all, that file needs to be there somewhere initially if it's indeed some
   * input file */
  wrench::StorageService::createFileAtLocation(
      wrench::FileLocation::LOCATION(ss_1, "/data/", input_file));

  /* Create a one-action job with a custom action */
  auto job = job_manager->createCompoundJob("job");

  std::map<std::string, unsigned long> num_cores_to_use_for_vm = {
      {"ComputeHost1", 2}, {"ComputeHost2", 4}};
  auto cloud_service = this->cloud_cs;
  job->addCustomAction(
      "powerful", 0, 0,
      [num_cores_to_use_for_vm,
       cloud_service](const std::shared_ptr<ActionExecutor> &action_executor) {
        TerminalOutput::setThisProcessLoggingColor(TerminalOutput::COLOR_RED);

        WRENCH_INFO("Custom action executing on host %s",
                    action_executor->getHostname().c_str());
  ...
  /* Submit the job to the bare-metal service compute service, and forcing the
   * action "powerful" to run on ComputeHost2 by passing (optional)
   * service-specific arguments */
  WRENCH_INFO("Submitting the job %s to the bare-metal service",
              job->getName().c_str());
  job_manager->submitJob(job, bm_cs, {{"powerful", "ComputeHost2"}});

  /* Wait for an execution event */
  auto event = this->waitForNextEvent();
  if (auto job_completion_event =
          std::dynamic_pointer_cast<wrench::CompoundJobCompletedEvent>(event)) {
  ...
  auto even_more_custom_job =
      job_manager->createCompoundJob("even_more_custom_job");
  auto my_derived_custom_action = std::make_shared<DerivedCustomAction>(
      "powerful", 0, 0,
      [num_cores_to_use_for_vm,
       cloud_service](std::shared_ptr<ActionExecutor> action_executor) {
        TerminalOutput::setThisProcessLoggingColor(TerminalOutput::COLOR_RED);

        auto the_action = std::dynamic_pointer_cast<DerivedCustomAction>(
            action_executor->getAction());
        WRENCH_INFO("Derived custom action executing on host %s",
                    action_executor->getHostname().c_str());
        WRENCH_INFO("The additional instance variable's value was %d",
  ...
  /* Submit the job to the bare-metal service compute service, letting the
   * service pick where to run the job's actions */
  WRENCH_INFO("Submitting the job %s to the bare-metal service",
              even_more_custom_job->getName().c_str());
  ...
  if (auto job_completion_event =
    ...
    WRENCH_INFO("Job %s has completed!", completed_job->getName().c_str());
    WRENCH_INFO("It had %zu actions:", completed_job->getActions().size());
    auto derived_custom_action = std::dynamic_pointer_cast<DerivedCustomAction>(
        *(completed_job->getActions().begin()));
    WRENCH_INFO(
        "The additional instance variable's value in the action is now %d",
        derived_custom_action->additional_variable);
  } else {
    throw std::runtime_error("Unexpected event: " + event->toString());
  }
