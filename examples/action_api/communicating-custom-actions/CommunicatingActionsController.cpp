...
WRENCH_LOG_CATEGORY(custom_controller,
                    "Log category for CommunicatingActionsController");
...
int CommunicatingActionsController::main() {
  ...
  /* Create actions that will participate in a parallel computation that mimics
   * what could
   ...
   */
  auto communicator =
      wrench::Communicator::createCommunicator(COMMUNICATOR_SIZE);

  /* Now let's create all actions */
  WRENCH_INFO(
      "Adding %lu actions (that will communicate with each other) to the job",
      COMMUNICATOR_SIZE);
  for (unsigned long i = 0; i < COMMUNICATOR_SIZE; i++) {

    auto lambda_execute =
        [communicator, storage_service,
         file](const std::shared_ptr<wrench::ActionExecutor> &action_executor) {
          ...
          WRENCH_INFO("I am an action with rank %lu in my communicator",
                      my_rank);
          communicator->barrier();
    ...
    job->addCustomAction("action_" + std::to_string(i), 0, 1, lambda_execute,
                         lambda_terminate);
  }
