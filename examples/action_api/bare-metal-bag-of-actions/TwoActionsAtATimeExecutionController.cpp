...
/**
 ** An execution controller implementation that repeatedly submits two compute
 *actions at a time
...
WRENCH_LOG_CATEGORY(custom_controller,
                    "Log category for TwoActionsAtATimeExecutionController");
 ...
 */
TwoActionsAtATimeExecutionController::TwoActionsAtATimeExecutionController(
    int num_actions, std::shared_ptr<BareMetalComputeService> compute_service,
...
int TwoActionsAtATimeExecutionController::main() {
  ...
  WRENCH_INFO("About to execute a workload with %d compute actions",
              this->num_actions);
  ...
  for (int i = 0; i < num_actions; i += 2) {
    ...
    auto file_read_1 = job->addFileReadAction(
        ...
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<1>(actions.at(0))));
    auto compute_1 = job->addComputeAction(
        "computation_" + std::to_string(i), std::get<0>(actions.at(0)), 0.0, 2,
        2, wrench::ParallelModel::AMDAHL(0.9));
    auto file_write_1 = job->addFileWriteAction(
        "file_write_" + std::to_string(i),
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<2>(actions.at(0))));
    ...
    auto file_read_2 = job->addFileReadAction(
        ...
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<1>(actions.at(0))));
    auto compute_2 = job->addComputeAction(
        "computation_" + std::to_string(i + 1), std::get<0>(actions.at(0)), 0.0,
        4, 4, wrench::ParallelModel::AMDAHL(0.9));
    std::shared_ptr<Action> file_write_2 = job->addFileWriteAction(
     ...
     * the event will be a CompoundJobCompletionEvent, which is processed by the
     * method processEventCompoundJobCompletion() that this class overrides. */
 ...
 */
void TwoActionsAtATimeExecutionController::processEventCompoundJobCompletion(
  ...
  std::vector<std::shared_ptr<Action>> sorted_actions(job_actions.begin(),
                                                      job_actions.end());
  std::sort(sorted_actions.begin(), sorted_actions.end(),
            [](const std::shared_ptr<Action> &a1,
               const std::shared_ptr<Action> &a2) -> bool {
              if (a1->getExecutionHistory().top().start_date ==
                  a2->getExecutionHistory().top().start_date) {
                return ((uintptr_t)a1.get() > (uintptr_t)a2.get());
              } else {
                return (a1->getExecutionHistory().top().start_date <
                        a2->getExecutionHistory().top().start_date);
              }
            });
  ...
  for (auto const &action : sorted_actions) {
    WRENCH_INFO(
        "  - Action %s ran with %lu cores from time %.2lf until time %.2lf",
 ...
 * @param event: the event
 */
void TwoActionsAtATimeExecutionController::processEventCompoundJobFailure(
