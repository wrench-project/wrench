...
/**
 ** An execution controller implementation that greedily submits actions to a
 *batch
...
WRENCH_LOG_CATEGORY(custom_controller,
                    "Log category for GreedyExecutionController");
 ...
 * @param storage_services: a set of storage services available to store data
 * files
 * @param hostname: the name of the host on which to start the WMS
 */
GreedyExecutionController::GreedyExecutionController(
    int num_actions, const std::shared_ptr<BatchComputeService> compute_service,
    const std::shared_ptr<SimpleStorageService> storage_service,
    const std::string &hostname)
    : ExecutionController(hostname, "me"), compute_service(compute_service),
      storage_service(storage_service), num_actions(num_actions) {}
...
int GreedyExecutionController::main() {
  ...
  std::uniform_real_distribution<double> gflop_dist(10000 * GFLOP,
                                                    100000 * GFLOP);
  ...
  WRENCH_INFO("About to execute a workload with %d compute action_specs",
              this->num_actions);

  /* Create random amounts of work in GFlop, input sizes in bytes, and output
   * sizes in bytes */
  std::vector<std::tuple<double, std::shared_ptr<wrench::DataFile>,
                         std::shared_ptr<wrench::DataFile>>>
      action_specs;
  ...
  for (int i = 0; i < num_actions; i++) {
    ...
    // Create input file
    auto input_file = wrench::Simulation::addFile(
        "input_file_" + std::to_string(i), mb_dist(rng));
    // Create a copy of the input file on the storage service
    wrench::StorageService::createFileAtLocation(
        wrench::FileLocation::LOCATION(this->storage_service, input_file));
    auto output_file = wrench::Simulation::addFile(
        "output_file_" + std::to_string(i), mb_dist(rng));
  ...
  for (int i = 0; i < this->num_actions; i += 2) {
    ...
    auto file_read_1 = job->addFileReadAction(
        "file_read_" + std::to_string(i),
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<1>(action_specs.at(i + 1))));
    auto compute_1 = job->addComputeAction(
        "computation_" + std::to_string(i), std::get<0>(action_specs.at(i + 1)),
        0.0, 1, 10, wrench::ParallelModel::AMDAHL(0.9));
    auto file_write_1 = job->addFileWriteAction(
        "file_write_" + std::to_string(i),
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<2>(action_specs.at(i + 1))));
    ...
    auto file_read_2 = job->addFileReadAction(
        "file_read_" + std::to_string(i + 1),
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<1>(action_specs.at(i + 1))));
    auto compute_2 =
        job->addComputeAction("computation_" + std::to_string(i + 1),
                              std::get<0>(action_specs.at(i + 1)), 0.0, 1, 10,
                              wrench::ParallelModel::AMDAHL(0.9));
    auto file_write_2 = job->addFileWriteAction(
        "file_write_" + std::to_string(i + 1),
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<2>(action_specs.at(i + 1))));
    ...
    // Submit the job!
    WRENCH_INFO("Submitting job %s (%d nodes, %d cores per node, %d seconds) "
                "for executing actions %s and %s",
                job->getName().c_str(), num_nodes, num_cores_per_nodes,
                num_seconds, compute_1->getName().c_str(),
                compute_2->getName().c_str());
  ...
  /* Wait for execution events, one per job. Some job will likely fail! The code
   * below doe "manual" processing of the event, rather than using registered
   * callback functions */
  for (int i = 0; i < this->num_actions / 2; i++) {
    WRENCH_INFO("Number of queued jobs at the batch service: %zu",
                this->compute_service->getQueue().size());
  ...
  for (auto const &action : compute_actions) {
    ...
    WRENCH_INFO("Action %s: %s", action->getName().c_str(),
                action->getStateAsString().c_str());
    WRENCH_INFO("  - ran as part of job %s, which was submitted at time %.2lf "
                "and finished at time %.2lf",
                action->getJob()->getName().c_str(),
                action->getJob()->getSubmitDate(),
                action->getJob()->getEndDate());
    if (action->getState() == Action::State::KILLED) {
      WRENCH_INFO("  - action failure cause: %s",
