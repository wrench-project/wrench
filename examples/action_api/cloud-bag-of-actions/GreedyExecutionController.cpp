...
GreedyExecutionController::GreedyExecutionController(
    int num_actions, std::shared_ptr<CloudComputeService> compute_service,
    ...
    : ExecutionController(hostname, "me"),
      ...
      storage_service(std::move(storage_service)), num_actions(num_actions) {}
...
int GreedyExecutionController::main() {
  ...
  for (int i = 0; i < num_actions; i++) {
    ...
    // Create a copy of the input file on the storage service (takes zero
    // simulation time)
  ...
  for (int i = 0; i < this->num_actions; i += 2) {
    ...
    WRENCH_INFO("Submitting job %s for executing action %s on VM %s",
                job1->getName().c_str(), compute_1->getName().c_str(),
                vm1_name.c_str());
    ...
    auto file_read_2 = job2->addFileReadAction(
        "file_read_" + std::to_string(i + 1),
        wrench::FileLocation::LOCATION(this->storage_service,
                                       std::get<1>(action_specs.at(i + 1))));
    auto compute_2 =
        job2->addComputeAction("computation_" + std::to_string(i + 1),
                               std::get<0>(action_specs.at(i + 1)), 0.0, 1, 10,
                               wrench::ParallelModel::AMDAHL(0.9));
    auto file_write_2 = job2->addFileWriteAction(
    ...
    WRENCH_INFO("Submitting job %s for executing action %s on VM %s",
                job2->getName().c_str(), compute_2->getName().c_str(),
                vm2_name.c_str());
 ...
 */
void GreedyExecutionController::processEventCompoundJobFailure(
 ...
 * @param event: the event
 */
void GreedyExecutionController::processEventCompoundJobCompletion(
