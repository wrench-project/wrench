...
/**
 ** This WMS submits n tasks for execution to HTCondor. The first ~n/2 are
 *submitted together as a single grid-universe job. The remaining tasks are
 *submitted as individual non-grid-universe jobs.
 **/
...
CondorWMS::CondorWMS(
    ...
    std::string hostname)
    : ExecutionController(hostname, "condor-grid"), workflow(workflow),
...
int CondorWMS::main() {
  ...
  auto my_vm =
      cloud_compute_service->createVM(5, 32.0 * 1000 * 1000 * 1000, {}, {});
  WRENCH_INFO("Starting the VM instance, which exposes a usable bare-metal "
              "compute service");
  ...
  WRENCH_INFO(
      "At this point, HTCondor has access to one batch_standard_and_pilot_jobs "
      "compute service and one bare-metal service (which runs on a VM)");
  ...
  WRENCH_INFO("Creating a standard job with the first %zu tasks",
              first_tasks.size());
  auto grid_universe_job =
      job_manager->createStandardJob(first_tasks, file_locations);
  WRENCH_INFO("Submitting the job as a grid-universe job to HTCondor, asking "
              "for 3 compute nodes");
  ...
  // This argument below is not required, as there is a single
  // batch_standard_and_pilot_jobs service in this example
  htcondor_service_specific_arguments["-service"] =
      batch_compute_service->getName();
  job_manager->submitJob(grid_universe_job, htcondor_compute_service,
                         htcondor_service_specific_arguments);
  ...
  for (auto const &task : last_tasks) {
    WRENCH_INFO("Creating and submitting a single-task job (for task %s) as a "
                "non-grid-universe job to HTCondor (will run on the VM)",
                task->getID().c_str());
    auto job = job_manager->createStandardJob(task, file_locations);
    job_manager->submitJob(job, htcondor_compute_service);
  }

  WRENCH_INFO("Waiting for Workflow Execution Events until the workflow "
 ...
 */
void CondorWMS::processEventStandardJobCompletion(
    std::shared_ptr<StandardJobCompletedEvent> event) {
  /* Retrieve the job that this event is for */
  auto job = event->standard_job;
  WRENCH_INFO("Notified that a standard job has completed: ");
  for (auto const &task : job->getTasks()) {
    WRENCH_INFO("    - Task %s ran on host %s (started at time %.2lf and "
                "finished at time %.2lf)",
                task->getID().c_str(), task->getPhysicalExecutionHost().c_str(),
