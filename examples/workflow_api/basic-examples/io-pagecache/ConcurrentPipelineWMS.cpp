 ...
 * @param bare_metal_compute_services: a bare-metal compute service available to
 * run tasks
 * @param storage_service: a storage service available to store files
 * @param hostname: the name of the host on which to start the WMS
 */
ConcurrentPipelineWMS::ConcurrentPipelineWMS(
...
int ConcurrentPipelineWMS::main() {
  ...
  while (not this->workflow->isDone()) {
    ...
    for (auto ready_task : ready_tasks) {
      ...
      /* Create the job  */
      auto standard_job =
          job_manager->createStandardJob(ready_task, file_locations);
 ...
 */
void ConcurrentPipelineWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void ConcurrentPipelineWMS::processEventStandardJobFailure(
