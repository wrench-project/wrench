 ...
 **    - Submit it to the first available bare-metal compute service as a job in
 *a way that
 **       - Uses 10 cores
...
NFSPipelineWMS::NFSPipelineWMS(
    ...
    : ExecutionController(hostname, "nfs-pipeline-wms"), workflow(workflow),
      bare_metal_compute_service(bare_metal_compute_service),
      client_storage_service(client_storage_service),
      server_storage_service(server_storage_service) {}
...
int NFSPipelineWMS::main() {
  ...
  while (not this->workflow->isDone()) {
    std::vector<std::shared_ptr<wrench::WorkflowTask>> ready_tasks =
    ...
    for (const auto &ready_task : ready_tasks) {
        ...
        if (Simulation::isPageCachingEnabled()) {
          file_locations[input_file] =
              FileLocation::LOCATION(this->client_storage_service,
                                     this->server_storage_service, input_file);
        } else {
#endif
          file_locations[input_file] =
              FileLocation::LOCATION(this->server_storage_service, input_file);
#ifdef PAGE_CACHE_SIMULATION
        }
#endif
      }
        ...
        if (Simulation::isPageCachingEnabled()) {
          file_locations[output_file] =
              FileLocation::LOCATION(this->server_storage_service,
                                     this->server_storage_service, output_file);
        } else {
#endif
          file_locations[output_file] =
              FileLocation::LOCATION(this->server_storage_service, output_file);
#ifdef PAGE_CACHE_SIMULATION
        }
#endif
      }
      ...
      auto standard_job = job_manager->createStandardJob(
          {ready_task}, file_locations, pre_file_copies, post_file_copies, {});
 ...
 */
void NFSPipelineWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void NFSPipelineWMS::processEventStandardJobFailure(
