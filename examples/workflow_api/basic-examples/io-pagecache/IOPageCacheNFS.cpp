...
workflow_multithread(int num_pipes, int num_tasks, int core_per_task,
...
int main(int argc, char **argv) {
  ...
  if (argc < 5) {
    std::cerr
        << "Usage: " << argv[0]
        << " <num_tasks> <file_size_gb> <cpu_time_sec> <xml_platform_file> "
           "[--wrench-pagecache-simulation] [--log=custom_wms.threshold=info]"
        << std::endl;
    exit(1);
  }
  ...
  /* Declare a workflow */
  auto workflow =
      workflow_multithread(no_pipelines, num_task, 1, cpu_time_sec * 1000000000,
                           file_size_gb * 1000000000, mem_req_gb * 1000000000);

  std::cerr << "Instantiating a SimpleStorageService on storage_host..."
            << std::endl;
  auto server_storage_service =
  ...
  auto client_storage_service =
      simulation->add(wrench::SimpleStorageService::createSimpleStorageService(
          "compute_host", {"/"},
          {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "100MB"}}, {}));

  std::cerr << "Instantiating a bare-metal compute service on compute_host..."
            << std::endl;
  auto baremetal_service = simulation->add(new wrench::BareMetalComputeService(
      "compute_host", {"compute_host"}, "", {}, {}));

  auto wms = simulation->add(new wrench::NFSPipelineWMS(
      workflow, {baremetal_service}, client_storage_service,
      server_storage_service, "compute_host"));

  std::cerr << "Instantiating a FileRegistryService on compute_host ..."
            << std::endl;
