...
std::shared_ptr<wrench::Workflow>
generate_workflow(int num_pipes, int num_tasks, int core_per_task, long flops,
                  long file_size, long mem_required) {
  ...
  for (int i = 0; i < num_pipes; i++) {
    ...
    for (int j = 0; j < num_tasks; j++) {
      ...
      auto task = workflow->addTask("task_" + std::to_string(i) + "_" +
                                        std::to_string(j),
    ...
    for (int j = 0; j < num_tasks + 1; j++) {
      wrench::Simulation::addFile(
          "file_" + std::to_string(i) + "_" + std::to_string(j), file_size);
    }

    /* Create tasks and set input/output files for each task */
    for (int j = 0; j < num_tasks; j++) {

      auto task = workflow->getTaskByID("task_" + std::to_string(i) + "_" +
                                        std::to_string(j));

      task->addInputFile(wrench::Simulation::getFileByID(
          "file_" + std::to_string(i) + "_" + std::to_string(j)));
      task->addOutputFile(wrench::Simulation::getFileByID(
          "file_" + std::to_string(i) + "_" + std::to_string(j + 1)));
    }
  }
...
void export_output_single(wrench::SimulationOutput output, int num_tasks,
                          std::string filename) {
  auto read_start = output.getTrace<wrench::SimulationTimestampFileReadStart>();
  auto read_end =
      output.getTrace<wrench::SimulationTimestampFileReadCompletion>();
  auto write_start =
      output.getTrace<wrench::SimulationTimestampFileWriteStart>();
  auto write_end =
      output.getTrace<wrench::SimulationTimestampFileWriteCompletion>();
  ...
  for (int i = 0; i < num_tasks; i++) {
    std::cerr << "Task " << read_end[i]->getContent()->getTask()->getID()
              << " completed at " << task_end[i]->getDate() << " in "
              << task_end[i]->getDate() - task_start[i]->getDate() << std::endl;

    fprintf(log_file, "read, %lf, %lf\n", read_start[i]->getDate(),
            read_end[i]->getDate());
    fprintf(log_file, "write, %lf, %lf\n", write_start[i]->getDate(),
            write_end[i]->getDate());
  }
...
void export_output_multi(wrench::SimulationOutput &output, int num_tasks,
                         std::string filename) {
  auto read_start = output.getTrace<wrench::SimulationTimestampFileReadStart>();
  auto read_end =
      output.getTrace<wrench::SimulationTimestampFileReadCompletion>();
  ...
  FILE *log_file = fopen(filename.c_str(), "w");
  fprintf(log_file,
          "read_start, read_end, cpu_start, cpu_end, write_start, write_end\n");

  for (int i = 0; i < num_tasks; i++) {
    std::cerr << read_end[i]->getContent()->getTask()->getID() << " started at "
              << task_start[i]->getDate() << ", ended at "
              << task_end[i]->getDate() << ", completed in "
              << task_end[i]->getDate() - task_start[i]->getDate() << std::endl;
...
int main(int argc, char **argv) {
  ...
  if (argc < 5) {
    std::cerr << "Usage: " << argv[0]
              << " <num_tasks> <file_size_gb> <cpu_time_sec> "
                 "<xml_platform_file> [--log=custom_wms.threshold=info]"
              << std::endl;
    exit(1);
  }
  ...
  /* Declare a workflow */
  auto workflow =
      generate_workflow(no_pipelines, num_task, 1, cpu_time_sec * 1000000000,
  ...
  auto storage_service =
      simulation->add(wrench::SimpleStorageService::createSimpleStorageService(
          "host01", {"/"},
          {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, "100MB"}}, {}));

  std::cerr << "Instantiating a bare-metal compute service on ComputeHost..."
            << std::endl;
  auto baremetal_service = simulation->add(
      new wrench::BareMetalComputeService("host01", {"host01"}, "", {}, {}));

  auto wms = simulation->add(new wrench::ConcurrentPipelineWMS(
      workflow, {baremetal_service}, {storage_service}, "host01"));
  ...
  {
    std::string filename =
        "multi_" + sub_dir + "dump_" + to_string(no_pipelines) + ".json";
...

  {
    ...
    export_output_multi(simulation->getOutput(), workflow->getNumberOfTasks(),
                        filename);
    std::cerr << "Written output to file " + filename << "\n";
    //        simulation->getMemoryManagerByHost("host01")->export_log("multi/"
    //        + sub_dir + to_string(no_pipelines) + "_sim_mem.csv");
  }
