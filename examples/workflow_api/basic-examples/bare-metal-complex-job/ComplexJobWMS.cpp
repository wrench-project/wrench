...
/**
 ** A Workflow Management System (WMS) implementation that operates on a
 *workflow
 ** with a single task that has two input files and two output files as follows:
 **  - Run the workflow has a single job that:
 **    - Copies the first input file from the first storage service to the
 *second one
 **    - Runs the task so that it produces its output files on the second
 *storage service
 ...
 * @param bare_metal_compute_service: a bare-metal compute services available to
 * run tasks
...
ComplexJobWMS::ComplexJobWMS(
    ...
    : ExecutionController(hostname, "complex-job"), workflow(workflow),
...
int ComplexJobWMS::main() {
  ...
  file_locations[infile_1] = FileLocation::LOCATION(
      storage_service2, infile_1); // read from storage service #2
  file_locations[infile_2] = FileLocation::LOCATION(
      storage_service1, infile_2); // read from storage service #1
  file_locations[outfile_1] = FileLocation::LOCATION(
      storage_service2, outfile_1); // written to storage service #2
  file_locations[outfile_2] = FileLocation::LOCATION(
      storage_service2, outfile_2); // written to storage service #2
  ...
  pre_file_copies.emplace_back(
      FileLocation::LOCATION(storage_service1, infile_1),
  ...
  post_file_copies.emplace_back(
      FileLocation::LOCATION(storage_service2, outfile_1),
  ...
  cleanup_file_deletions.emplace_back(
      FileLocation::LOCATION(storage_service2, outfile_2));

  /* Create the standard job */
  WRENCH_INFO("Creating a complex job with pre file copies, post file copies, "
              "and post file deletions to execute task  %s",
              task->getID().c_str());
  auto job =
      job_manager->createStandardJob({task}, file_locations, pre_file_copies,
  ...
  /* Wait for a workflow execution event and process it. In this case we know
   * that the event will be a StandardJobCompletionEvent, which is processed by
   * the method processEventStandardJobCompletion() that this class overrides.
   */
 ...
 */
void ComplexJobWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void ComplexJobWMS::processEventStandardJobFailure(
