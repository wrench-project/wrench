 ...
 **    - Submit both of them as a single batch_standard_and_pilot_jobs job to
 *the compute service
 **       - the job asks for two whole 10-core compute nodes to run two
 **         tasks at once (unless a single task is left), but requesting,
 **         with high probability, an amount of time only sufficient to run a
 *single task.
 **         So one task succeeds, the other one failed, but  will be resubmitted
 *later.
 **/
 ...
 * @param batch_compute_service: a batch compute service available to run tasks
 * @param storage_service: a storage service available to store files
 * @param hostname: the name of the host on which to start the WMS
 */
TwoTasksAtATimeBatchWMS::TwoTasksAtATimeBatchWMS(
    ...
    : ExecutionController(hostname,
      ...
      workflow(workflow), batch_compute_service(batch_compute_service),
...
int TwoTasksAtATimeBatchWMS::main() {
  ...
  double core_flop_rate =
      (*(batch_compute_service->getCoreFlopRate().begin())).second;
  ...
  for (auto const &t : this->workflow->getTasks()) {
    double parallel_efficiency =
        std::dynamic_pointer_cast<wrench::ConstantEfficiencyParallelModel>(
            t->getParallelModel())
            ->getEfficiency();
    double in_seconds =
        (t->getFlops() / core_flop_rate) / (10 * parallel_efficiency);
  ...
  while (not this->workflow->isDone()) {
    ...
    auto ready_task2 = (ready_tasks.size() > 1
                            ? ready_tasks.at((index + 1) % ready_tasks.size())
                            : nullptr);

    /* Swap the tasks if ready_task2 is cheaper than ready_task 1, for the  sake
     * of the example */
    ...
    if (ready_task2) {
      WRENCH_INFO(
          "Creating a job to execute tasks %s (%.1lf Gflops) and task %s "
          "(%.1lf Gflops)",
      ...
      WRENCH_INFO("Creating a job to execute task %s (%.1lf Gflops)",
                  ready_task->getID().c_str(),
                  ready_task->getFlops() / 1000000000.0);
    }
    ...
    file_locations[input_file] =
        FileLocation::LOCATION(storage_service, input_file);
    file_locations[output_file] =
        FileLocation::LOCATION(storage_service, output_file);
    if (ready_task2) {
      input_file = ready_task2->getInputFiles().at(0);
      output_file = ready_task2->getOutputFiles().at(0);
      file_locations[input_file] =
          FileLocation::LOCATION(storage_service, input_file);
      file_locations[output_file] =
          FileLocation::LOCATION(storage_service, output_file);
    ...
    if (ready_task2) {
      standard_job = job_manager->createStandardJob({ready_task, ready_task2},
                                                    file_locations);
    } else {
    ...
    WRENCH_INFO("Task %s should run in under %ld seconds",
                ready_task->getID().c_str(),
                execution_times_in_seconds[ready_task]);
    if (ready_task2) {
      WRENCH_INFO("Task %s should run in under %ld seconds",
                  ready_task2->getID().c_str(),
                  execution_times_in_seconds[ready_task2]);
    }

    // But let's submit the job so that it requests time sufficient only
    // for the cheaper task, which will lead to the
    // expensive task, if any, to be terminated prematurely (i.e., it will fail
    // and thus still be ready).

    service_specific_arguments["-t"] =
        std::to_string(execution_times_in_seconds[ready_task]);

    WRENCH_INFO(
        "Submitting the job, asking for %s %s-core nodes for %s seconds",
    ...
    job_manager->submitJob(standard_job, batch_compute_service,
     ...
     * method, but instead calls the lower-level waitForNextExecutionEvent()
     * method and then process the event manually */
    WRENCH_INFO("Waiting for the next event");

    try {
      auto event = this->waitForNextEvent();
      // Check that it is the expected event, just in  case
      if (auto job_failed_event =
      ...
      } else if (auto job_completed_event = std::dynamic_pointer_cast<
                     wrench::StandardJobCompletedEvent>(event)) {
    ...
    /* At this point, the cheap task should have completed, and the expensive
     * one,
     * if any, should have failed. Let's check and print some logging info */
    if (ready_task->getState() != WorkflowTask::COMPLETED) {
      throw std::runtime_error("Task " + ready_task->getID() +
                               "should have completed successfully!");
    } else {
      WRENCH_INFO("Task %s has completed successfully :)",
                  ready_task->getID().c_str());
    }
    if (ready_task2) {
      if (ready_task2->getState() != WorkflowTask::READY) {
        throw std::runtime_error("Task " + ready_task2->getID() +
                                 "should have failed");
      } else {
