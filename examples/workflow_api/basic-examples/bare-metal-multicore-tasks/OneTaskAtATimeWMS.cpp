 ...
 **    - Pick one ready task
 **    - Submit it as part of a single job to the one available bare-metal
 *compute service so that:
...
int OneTaskAtATimeWMS::main() {
  ...
  while (not this->workflow->isDone()) {
    ...
    /* No need to use service specific arguments to specify a number of cores
     * for the task. By default, the compute service will run the task with the
     * largest possible number of cores. */
