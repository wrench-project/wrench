 ...
 */
WorkflowAsAsingleJobWMS::WorkflowAsAsingleJobWMS(
    ...
    : ExecutionController(hostname, "one-task-at-a-time"), workflow(workflow),
      bare_metal_compute_service(compute_service),
...
int WorkflowAsAsingleJobWMS::main() {
  ...
  // For the workflow input files, in fact, set the location to the storage
  // service
  for (auto const &f : this->workflow->getInputFiles()) {
    file_locations[f] = FileLocation::LOCATION(storage_service, f);
  }
  // For the workflow output files, in fact, set the location to the storage
  // service
 ...
 */
void WorkflowAsAsingleJobWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void WorkflowAsAsingleJobWMS::processEventStandardJobFailure(
