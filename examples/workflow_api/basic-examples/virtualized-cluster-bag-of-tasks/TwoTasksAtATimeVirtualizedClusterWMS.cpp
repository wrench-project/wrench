...
WRENCH_LOG_CATEGORY(custom_wms,
                    "Log category for TwoTasksAtATimeVirtualizedClusterWMS");
 ...
 * @param virtualized_cluster_compute_service: a virtualized cluster compute
 * service available to run tasks
...
int TwoTasksAtATimeVirtualizedClusterWMS::main() {
  ...
  WRENCH_INFO("Creating a 'large' VM with 5 cores  and a 'small' VM with 2 "
              "cores, both of them with 5GB RAM");
  auto large_vm = virtualized_cluster_compute_service->createVM(
      5, 5 * GB, "VirtualizedClusterHost1");
  auto small_vm = virtualized_cluster_compute_service->createVM(
      2, 5 * GB, "VirtualizedClusterHost2");

  /* Start the VMs */
  WRENCH_INFO("Start the large VM on host VirtualizedClusterHost1");
  auto large_vm_compute_service =
      virtualized_cluster_compute_service->startVM(large_vm);
  WRENCH_INFO("Start the small VM on host VirtualizedClusterHost2");
  auto small_vm_compute_service =
      virtualized_cluster_compute_service->startVM(small_vm);
  ...
  while (not this->workflow->isDone()) {
    ...
    file_locations1[input_file] =
        FileLocation::LOCATION(storage_service, input_file);
    file_locations1[output_file] =
    ...
    WRENCH_INFO("Creating a job to run task %s (%.2lf)",
                ...
                cheap_ready_task->getFlops());

    auto standard_job1 =
        job_manager->createStandardJob(cheap_ready_task, file_locations1);
    ...
    file_locations2[input_file] =
        FileLocation::LOCATION(storage_service, input_file);
    file_locations2[output_file] =
        FileLocation::LOCATION(storage_service, output_file);

    /* Create the job  */
    WRENCH_INFO("Creating a job to run task %s (%.2lf)",
                expensive_ready_task->getID().c_str(),
                expensive_ready_task->getFlops());

    auto standard_job2 =
        job_manager->createStandardJob(expensive_ready_task, file_locations2);
    ...
    /* Migrate the VM */
    WRENCH_INFO("Migrating the small VM from VirtualizedClusterHost1 to "
                "VirtualizedClusterHost2");
    virtualized_cluster_compute_service->migrateVM(small_vm,
                                                   "VirtualizedClusterHost2");
    WRENCH_INFO("VM Migrated!");

    /* Wait for  workflow execution event and process it. In this case we know
     * that the event will be a StandardJobCompletionEvent, which is processed
     * by the method processEventStandardJobCompletion() that this class
     * overrides. */
 ...
 */
void TwoTasksAtATimeVirtualizedClusterWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void TwoTasksAtATimeVirtualizedClusterWMS::processEventStandardJobFailure(
  ...
  WRENCH_INFO(
      "Notified that a standard job has failed for task %s with error %s",
      task->getID().c_str(), event->failure_cause->toString().c_str());
