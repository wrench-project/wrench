 ...
 **    - Submit both of them as part of a single job to the first available
 *bare-metal compute service so that:
 ...
 * @param bare_metal_compute_service: a bare-metal compute service for running
 * tasks
 * @param storage_service: a storage service for storing files
 * @param hostname: the name of the host on which to start the WMS
 */
TwoTasksAtATimeWMS::TwoTasksAtATimeWMS(
    ...
    : ExecutionController(hostname, "two-tasks-at-a-time"), workflow(workflow),
...
int TwoTasksAtATimeWMS::main() {
  ...
  while (not this->workflow->isDone()) {
    ...
    /* Then, we create the "service-specific arguments" that make it possible to
     * configure
     * the way in which tasks in a job can run on the compute service */
    std::map<std::string, std::string> service_specific_args;
    service_specific_args[cheap_ready_task->getID()] =
        "4"; // 4 cores, run on any host
    service_specific_args[expensive_ready_task->getID()] =
        "6"; // 6 cores, run on any host
    ...
    job_manager->submitJob(standard_job, this->bare_metal_compute_service,
