 ...
 * @param cloud_compute_service: a cloud compute service available to run tasks
 * @param storage_service: a storage service available to store files
 * @param hostname: the name of the host on which to start the WMS
 */
TwoTasksAtATimeCloudWMS::TwoTasksAtATimeCloudWMS(
    ...
    : ExecutionController(hostname, "two-tasks-at-a-time-cloud"),
      workflow(workflow), cloud_compute_service(cloud_compute_service),
...
int TwoTasksAtATimeCloudWMS::main() {
  ...
  WRENCH_INFO(
      "Creating a 'large' VM with 5 cores  and a 'small' VM with 2 cores");
  ...
  while (not this->workflow->isDone()) {
    ...
    WRENCH_INFO("Changing the pstate of the large VM to the lower pstate to "
                "save on energy temporarily");
    ...
    WRENCH_INFO(
        "Changing the pstate of the large VM back to the higher pstate");
 ...
 */
void TwoTasksAtATimeCloudWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void TwoTasksAtATimeCloudWMS::processEventStandardJobFailure(
