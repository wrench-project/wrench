 ...
 **
 **  - Copy the first input file from the first storage service to the second
 *one
 **  - Runs the task so that it produces its output files on the second storage
 *service
 ...
 * @param bare_metal_compute_service: a bare-metal compute service available to
 * run tasks
 ...
 */
DataMovementWMS::DataMovementWMS(
    ...
    : ExecutionController(hostname, "data-movement"), workflow(workflow),
...
int DataMovementWMS::main() {
  ...
  WRENCH_INFO("Synchronously copying file infile_1 from storage_service1 to "
              "storage_service2");
  ...
  file_locations[outfile_1] =
      FileLocation::LOCATION(storage_service2, outfile_1);
  file_locations[outfile_2] =
  ...
  WRENCH_INFO("Asynchronously copying outfile_1 from storage_service2 to "
              "storage_service1");
  ...
  try {
    ...
    if (auto file_copy_completion_event =
            std::dynamic_pointer_cast<wrench::FileCopyCompletedEvent>(event)) {
      WRENCH_INFO(
          "Notified of the file copy completion for file %s, as expected",
 ...
 */
void DataMovementWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void DataMovementWMS::processEventStandardJobFailure(
