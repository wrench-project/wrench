 ...
 **  - Intermediate file is kept in the batch_standard_and_pilot_jobs compute
...
int PilotJobWMS::main() {
  ...
  for (auto const &t : this->workflow->getTasks()) {
    ...
    WRENCH_INFO("Task %s should run in under %ld seconds", t->getID().c_str(),
                execution_times_in_seconds[t]);
  }
  ...
  service_specific_arguments["-t"] =
      std::to_string((execution_times_in_seconds[task_0] +
                      execution_times_in_seconds[task_1]) /
                     2);

  WRENCH_INFO(
      "Submitting a pilot job that  requests %s %s-core nodes for %s seconds",
      service_specific_arguments["-N"].c_str(),
      service_specific_arguments["-c"].c_str(),
      service_specific_arguments["-t"].c_str());

  job_manager->submitJob(pilot_job, batch_compute_service,
  ...
  auto standard_job =
      job_manager->createStandardJob({task_0, task_1}, file_locations);
 ...
 */
void PilotJobWMS::processEventStandardJobCompletion(
 ...
 * @param event: the event
 */
void PilotJobWMS::processEventStandardJobFailure(
 ...
 * @param event: the event
 */
void PilotJobWMS::processEventPilotJobExpiration(
    std::shared_ptr<PilotJobExpiredEvent>) {
 ...
 * @param event: the event
 */
void PilotJobWMS::processEventPilotJobStart(
    std::shared_ptr<PilotJobStartedEvent>) {
