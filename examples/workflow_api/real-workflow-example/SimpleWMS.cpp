...
SimpleWMS::SimpleWMS(
    ...
    : ExecutionController(hostname, "simple"), workflow(workflow),
...
int SimpleWMS::main() {
  ...
  // Create and start two VMs on the cloud service to use for the whole
  // execution
  auto vm1 = this->cloud_compute_service->createVM(
      2, 0.0); // 2 cores, 0 RAM (RAM isn't used in this simulation)
  auto vm1_cs = this->cloud_compute_service->startVM(vm1);
  this->core_utilization_map[vm1_cs] = 2;

  auto vm2 = this->cloud_compute_service->createVM(
      4, 0.0); // 4 cores, 0 RAM (RAM isn't used in this simulation)
  auto vm2_cs = this->cloud_compute_service->startVM(vm2);
  this->core_utilization_map[vm2_cs] = 4;

  while (true) {
    // If a pilot job is not running on the batch service, let's submit one that
    // asks for 3 cores on 2 compute nodes for 1 hour
    if (not pilot_job) {
    ...
    // Construct the list of currently available bare-metal services (on VMs and
    // perhaps within pilot job as well)
    std::set<std::shared_ptr<BareMetalComputeService>>
        available_compute_service = {vm1_cs, vm2_cs};
    if (this->pilot_job_is_running) {
      available_compute_service.insert(pilot_job->getComputeService());
    }

    scheduleReadyTasks(workflow->getReadyTasks(), job_manager,
                       available_compute_service);
 ...
 */
void SimpleWMS::processEventStandardJobFailure(
    std::shared_ptr<StandardJobFailedEvent> event) {
  auto job = event->standard_job;
  TerminalOutput::setThisProcessLoggingColor(TerminalOutput::COLOR_RED);
  WRENCH_INFO("Task %s has failed",
 ...
 * @param event: a workflow execution event
 */
void SimpleWMS::processEventStandardJobCompletion(
 ...
 * @param event: a workflow execution event
 */
void SimpleWMS::processEventPilotJobStart(
    std::shared_ptr<PilotJobStartedEvent> event) {
  TerminalOutput::setThisProcessLoggingColor(TerminalOutput::COLOR_BLUE);
  WRENCH_INFO(
      "The pilot job has started (it exposes bare-metal compute service %s)",
      event->pilot_job->getComputeService()->getName().c_str());
  TerminalOutput::setThisProcessLoggingColor(TerminalOutput::COLOR_GREEN);
  this->pilot_job_is_running = true;
  this->core_utilization_map[this->pilot_job->getComputeService()] =
      event->pilot_job->getComputeService()->getTotalNumIdleCores();
}
 ...
 * @param event: a workflow execution event
 */
void SimpleWMS::processEventPilotJobExpiration(
    std::shared_ptr<PilotJobExpiredEvent> event) {
  TerminalOutput::setThisProcessLoggingColor(TerminalOutput::COLOR_RED);
  WRENCH_INFO("The pilot job has expired (it was exposing bare-metal compute "
              "service %s)",
              event->pilot_job->getComputeService()->getName().c_str());
  TerminalOutput::setThisProcessLoggingColor(TerminalOutput::COLOR_GREEN);
...
/**
 * @brief Helper method to schedule a task one available compute services. This
 * is a very, very simple/naive scheduling approach, that greedily runs tasks on
 * idle cores of whatever compute services are available right now, using 1 core
 * per task. Obviously, much more sophisticated approaches/algorithms are
 * possible. But this is sufficient for the sake of an example.
 *
 ...
 * @return
 */
void SimpleWMS::scheduleReadyTasks(
    std::vector<std::shared_ptr<WorkflowTask>> ready_tasks,
    ...
    for (auto const &cs : compute_services) {
      if (this->core_utilization_map[cs] > 0) {
        ...
        for (auto const &f : task->getInputFiles()) {
          file_locations[f] =
              wrench::FileLocation::LOCATION(this->storage_service, f);
        }
        ...
        try {
          ...
          WRENCH_INFO("Submitting task %s to compute service %s",
                      task->getID().c_str(), cs->getName().c_str());
          job_manager->submitJob(job, cs);
          this->core_utilization_map[cs]--;
          num_tasks_scheduled++;
          scheduled = true;
        } catch (ExecutionException &e) {
          WRENCH_INFO("WARNING: Was not able to submit task %s, likely due to "
                      "the pilot job having expired "
  ...
  WRENCH_INFO("Was able to schedule %lu out of %zu ready tasks",
              num_tasks_scheduled, ready_tasks.size());
}
