 ...
 * use SET_PROPERTY_NAME and DECLARE_PROPERTY_NAME
...
ServiceProperty::addServiceProperty(std::string classname,
                                    std::string serviceProperty) {
  ++WRENCH_PROPERTY_COUNT;
  wrapper_stringToPropertyMap()[classname + "::" + serviceProperty] =
      WRENCH_PROPERTY_COUNT;
  wrapper_propertyToStringMap()[WRENCH_PROPERTY_COUNT] =
      classname + "::" + serviceProperty;
  return WRENCH_PROPERTY_COUNT;
}
/**
 * @brief translate a string key to a property ID
 * @param serviceProperty: the name of the service Property to get in
 * classname::serviceProperty form (Note: the classname must be the parent class
 * that defines the property)
 * @return a property type
 */
WRENCH_PROPERTY_TYPE
ServiceProperty::translateString(std::string serviceProperty) {
 ...
 */
std::string
ServiceProperty::translatePropertyType(WRENCH_PROPERTY_TYPE serviceProperty) {
