...
WRENCH_LOG_CATEGORY(wrench_core_bandwidth_meter,
                    "Log category for Link Bandwidth Meter");
 ...
 */
BandwidthMeterService::BandwidthMeterService(
  ...
  if (measurement_periods.empty()) {
    throw std::invalid_argument(
        "BandwidthMeter::BandwidthMeter(): no host to meter!");
  }

  for (auto &l : measurement_periods) {
    if (not S4U_Simulation::linkExists(l.first)) {
      throw std::invalid_argument(
          "BandwidthMeter::BandwidthMeter(): unknown link " + l.first);
    }
    if (l.second < 0.01) {
      throw std::invalid_argument(
          "BandwidthMeter::BandwidthMeter(): measurement period must be at "
          "least 0.01 second (link " +
          l.first + ")");
    }
    this->measurement_periods[l.first] = l.second;
    this->time_to_next_measurement[l.first] =
        0.0; // We begin by taking a measurement
  }
}
 ...
 */
BandwidthMeterService::BandwidthMeterService(
    const std::string &hostname, const std::vector<std::string> &linknames,
  ...
  if (measurement_period < 0.01) {
    throw std::invalid_argument("BandwidthMeter::BandwidthMeter(): measurement "
                                "period must be at least 0.01 second");
  }

  for (auto const &l : linknames) {
    if (not S4U_Simulation::linkExists(l)) {
      throw std::invalid_argument(
          "BandwidthMeter::BandwidthMeter(): unknown link " + l);
    }
 ...
 */
void BandwidthMeterService::kill() { this->killActor(); }
...
int BandwidthMeterService::main() {
  ...
  WRENCH_INFO("New Bandwidth Meter Manager starting (%s) and monitoring links",
  ...
  while (true) {
    ...
    for (auto &l : this->time_to_next_measurement) {
      l.second = std::max<double>(
          0, l.second - (Simulation::getCurrentSimulatedDate() - before));
    }
    ...
    for (auto &l : this->time_to_next_measurement) {
      if (l.second < EPSILON) {
        ...
        this->time_to_next_measurement[l.first] =
            this->measurement_periods[l.first];
      }
    }
  }
...
bool BandwidthMeterService::processNextMessage(double timeout) {
  ...
  try {
    auto msg = this->commport->getMessage<ServiceStopDaemonMessage>(
        timeout, "BandwidthMeter::waitForNextMessage(): Received an");
    return false;
