...
EnergyMeterService::EnergyMeterService(
  ...
  if (measurement_periods.empty()) {
    throw std::invalid_argument(
        "EnergyMeter::EnergyMeter(): no host to meter!");
  }

  for (auto &h : measurement_periods) {
    if (not S4U_Simulation::hostExists(h.first)) {
      throw std::invalid_argument("EnergyMeter::EnergyMeter(): unknown host " +
                                  h.first);
    }
    if (h.second < 1.0) {
      throw std::invalid_argument("EnergyMeter::EnergyMeter(): measurement "
                                  "period must be at least 1 second (host " +
                                  h.first + ")");
    }
    this->measurement_periods[h.first] = h.second;
    this->time_to_next_measurement[h.first] =
 ...
 */
EnergyMeterService::EnergyMeterService(
    const std::string hostname, const std::vector<std::string> &hostnames,
 ...
 */
void EnergyMeterService::kill() { this->killActor(); }
...
int EnergyMeterService::main() {
  ...
  WRENCH_INFO("New Energy Meter Manager starting (%s)",
  ...
  while (true) {
    ...
    for (auto &h : this->time_to_next_measurement) {
      h.second = std::max<double>(
          0, h.second - (Simulation::getCurrentSimulatedDate() - before));
    }
    ...
    for (auto &h : this->time_to_next_measurement) {
      if (h.second < EPSILON) {
        ...
        this->time_to_next_measurement[h.first] =
            this->measurement_periods[h.first];
      }
    }
  }
...
bool EnergyMeterService::processNextMessage(double timeout) {
  ...
  try {
    auto msg = this->commport->getMessage<ServiceStopDaemonMessage>(
        timeout, "EnergyMeter::waitForNextMessage(): Received an");
