...
namespace wrench {
    namespace XRootD {
      ...
      */
        ...
        bool Node::processNextMessage() {
            ...
            if (reduced) {
                //handle the 2 Advanced Messages if reduced simulation is enabled for this node
                if (auto msg = dynamic_cast<AdvancedContinueSearchMessage*>(message.get())) {
                    ...
                    if (cached(msg->file)) {
                        //caching is even more efficient than advanced search
                        ...
                        if (!children.empty()) {
                            ...
                            for (auto const& entry : splitStacks) {
                                if (entry.first == this) {
                                    //this node was the target
                                    if (internalStorage &&
                                        //check the storage, it SHOULD be there, but we should check still
            ...
            if (auto msg = dynamic_cast<ServiceStopDaemonMessage*>(message.get())) {
                //handle all the rest of the messages
                try {
                ...
                if (cached(file)) {
                    ...
                    if (internalStorage && internalStorage->hasFile(file)) {
                        ...
                        //File not in internal storage or cache
                        //S4U_Simulation::compute(
                        //        this->getPropertyValueAsDouble(Property::SEARCH_BROADCAST_OVERHEAD));
                        //extra compute call
                        ...
                        if (!children.empty()) {
                            ...
                            // you asked a leaf directly, and it didn't have the file
                    ...
                    if (!*msg->answered) {
                        *msg->answered = true;
                        ...
                        if (msg->original) {
                            ...
                            //this was a file lookup
                            msg->answer_commport->dputMessage(
                                new StorageServiceFileLookupAnswerMessage(
                                    msg->file,
                                    true,
                                    //xrootd silently fails if the file doesn't exist, so if we have gotten here, the file does for sure exist
                                    getMessagePayloadValue(
                                        MessagePayload::FILE_SEARCH_ANSWER_MESSAGE_PAYLOAD)));
                        }
                    }
                ...
                S4U_Simulation::compute(this->getPropertyValueAsDouble(Property::SEARCH_BROADCAST_OVERHEAD));
                if (!children.empty() && msg->timeToLive > 0) {
                    //shotgun remove search message to all children
                    shared_ptr<bool> answered = make_shared<bool>(false);
                ...
                //we got a message targeted at a normal storage server
                if (not internalStorage) {
                    ...
                    throw std::runtime_error(
                        "Non-Storage XRooD node received a message it cannot process - internal error");
                }
                else {
        ...
        */
        bool Node::makeSupervisor() {
            //this function does nothing anymore?
            return true;
        }
        ...
        */
        Node::Node(Deployment* deployment, const std::string& hostname,
                   ...
                   const WRENCH_MESSAGE_PAYLOAD_COLLECTION_TYPE& messagepayload_list) : StorageService(
            hostname, "XRootDNode") {
     ...
     */
        ...
        */
        stack<Node*> Node::constructSearchStack(Node* target) const {
            stack<Node*> ret;
            Node* next = target;
            ...
            if (next != this) {
                //failed to find this in parent tree
                ret = stack<Node*>();
            }
            else {
        ...
        */
        void Node::removeFile(const std::shared_ptr<FileLocation>& location) {
            ...
            auto it = std::find(metavisor->files[location->getFile()].begin(),
                                metavisor->files[location->getFile()].end(), this->getSharedPtr<Node>());
        ...
        */
        void Node::writeFile(S4U_CommPort* answer_commport,
            ...
            auto new_location = FileLocation::LOCATION(internalStorage, location->getDirectoryPath(),
                                                       location->getFile());
        ...
        std::shared_ptr<Node> Node::addChildStorageServer(const std::string& hostname, const std::string& mount_point,
            ...
            if (storage_property_list.find(wrench::SimpleStorageServiceProperty::BUFFER_SIZE) != storage_property_list.
                end()) {
                if (UnitParser::parse_size(storage_property_list[wrench::SimpleStorageServiceProperty::BUFFER_SIZE]) <
                    1) {
                    throw std::invalid_argument(
                        "Node::addChildStorageServer(): XRootD current does not support 0 buffer_size");
                }
            }
            return this->addChild(this->deployment->createStorageServer(hostname, mount_point,
                                                                        storage_property_list,
                                                                        std::move(storage_messagepayload_list),
                                                                        std::move(node_property_list),
                                                                        std::move(node_messagepayload_list)));
        }
        ...
        bool Node::hasFile(const shared_ptr<FileLocation>& location) {
            ...
            //alternatively
            return !constructFileSearchTree(metavisor->getFileNodes(location->getFile())).empty();
            //meta-search the subtree for the file.  If it's in the subtree we can find a route to it, so we have it
        }
