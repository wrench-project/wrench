...
WRENCH_LOG_CATEGORY(wrench_core_xrootd_data_server, "Log category for XRootD");
namespace wrench {
namespace XRootD {
/**
...
int Node::main() {
  ...
  std::string message = "XRootD Node " + this->getName() +
...
std::shared_ptr<Node> Node::addChild(std::shared_ptr<Node> child) {
  ...
  throw std::runtime_error("Supervisor running on " + hostname +
                           " already has 64 children");
}
//            /**
//             * @brief Get mount point of underlying storage service
//             * @return the (sole) mount point of the service if there is one,
//             and null otherwise
//             */
...
//            }
//            /**
//             * @brief Get the set of mount points of the underlying storage
//             service
//             * @return the set of mount points of the service if there is one,
...
//            }
//            /**
//             * @brief Check whether the underlying storage service has
//             multiple mount points
//             * @return whether the service has multiple mount points, if there
//             is no storage service, returns false
//             */
...
//            }
//            /**
//            * @brief Check whether the underlying storage service has a
//            particular mount point
//            * @param mp: a mount point
//            *
//            * @return whether the underlying service has that mount point, if
//            there is no storage service, returns false
//            */
...
bool Node::processNextMessage() {
  ...
  S4U_Simulation::compute(
      this->getPropertyValueAsDouble(Property::MESSAGE_OVERHEAD));
  try {
  ...
  if (reduced) { // handle the 2 Advanced Messages if reduced simulation is
                 // enabled for this node
    if (auto msg =
            dynamic_cast<AdvancedContinueSearchMessage *>(message.get())) {
      WRENCH_DEBUG("Advanced Continue Search for %s",
                   msg->file->getID().c_str());

      S4U_Simulation::compute(
          this->getPropertyValueAsDouble(Property::CACHE_LOOKUP_OVERHEAD));
      if (cached(msg->file)) { // caching is even more efficient than advanced
                               // search
        ...
        supervisor->commport->dputMessage(new UpdateCacheMessage(
            msg->answer_commport, msg->original, msg->node, msg->file, cached,
        ...
        if (!children.empty()) {
          ...
          map<Node *, vector<stack<Node *>>> splitStacks =
              splitStack(msg->search_stack);
          S4U_Simulation::compute(this->getPropertyValueAsDouble(
              Property::SEARCH_BROADCAST_OVERHEAD));
          ...
          for (auto const &entry : splitStacks) {
            if (entry.first == this) { // this node was the target
              if (internalStorage &&   // check the storage, it SHOULD be there,
                                     // but we should check still
                  internalStorage->hasFile(msg->file)) {
                // File in internal storage
                cache.add(msg->file,
                          FileLocation::LOCATION(internalStorage, msg->file));
                supervisor->commport->dputMessage(new UpdateCacheMessage(
                    msg->answer_commport, msg->original, msg->node, msg->file,
              ...
              entry.first->commport->dputMessage(
                  new AdvancedContinueSearchMessage(msg, entry.second));
            }
          }
        }

        if (internalStorage && // check the storage, it SHOULD be there, but we
                               // should check still
            internalStorage->hasFile(msg->file)) {
          // File in internal storage
          cache.add(msg->file,
      ...
      S4U_Simulation::compute(
          this->getPropertyValueAsDouble(Property::UPDATE_CACHE_OVERHEAD));
      ...
      if (!children.empty()) {
        ...
        for (auto const &entry : splitStacks) {
          if (entry.first != this) {
            entry.first->commport->dputMessage(
                new AdvancedRippleDelete(msg, entry.second));
          }
        }
      }
      return true;
    } // both of these must return something, or we will break later in this
      // function
  }
  if (auto msg = dynamic_cast<ServiceStopDaemonMessage *>(
    ...
    // WRENCH_INFO("Got message %p %d
    // %d",msg,*msg->answered,msg->fileReadRequest);
    if (!*msg->answered) {
      *msg->answered = true;
      ...
      if (msg->fileReadRequest) {
        ...
        msg->answer_commport->dputMessage(
            new StorageServiceFileReadAnswerMessage(
                FileLocation::LOCATION(getSharedPtr<Node>(), msg->file), false,
                std::shared_ptr<FailureCause>(new FileNotFound(
    ...
    if (cached(file)) { // File Cached
      ...
      if (internalStorage && internalStorage->hasFile(file)) {
        ...
        if (children.size() > 0) { // recursive search
          ...
          Alarm::createAndStartAlarm(this->simulation,
                                     wrench::S4U_Simulation::getClock() +
                                         this->getPropertyValueAsTimeInSecond(
                                             Property::FILE_NOT_FOUND_TIMEOUT),
                                     this->hostname, this->commport,
                                     new FileNotFoundAlarm(msg->answer_commport,
                                                           file, false,
                                                           answered),
                                     "XROOTD_FileNotFoundAlarm");
          if (reduced) {
            WRENCH_DEBUG("Starting advanced lookup for %s",
                         file->getID().c_str());

            auto targets = metavisor->getFileNodes(file);
            auto search_stack = constructFileSearchTree(targets);
            map<Node *, vector<stack<Node *>>> splitStacks =
                splitStack(search_stack);
            WRENCH_DEBUG("Searching %zu subtrees for %s", search_stack.size(),
                         file->getID().c_str());
            S4U_Simulation::compute(this->getPropertyValueAsDouble(
                Property::SEARCH_BROADCAST_OVERHEAD));
            for (auto entry : splitStacks) {
              if (entry.first == this) { // this node was the target
                // we shouldn't have to worry about this, it should have been
                // handled earlier.
                //  But just in case, I don't want a rogue search going who
                //  knows where
              } else {
                entry.first->commport->dputMessage(
                    new AdvancedContinueSearchMessage(
                        msg->answer_commport, nullptr, file, this,
                        getMessagePayloadValue(MessagePayload::CONTINUE_SEARCH),
                        answered, metavisor->defaultTimeToLive, entry.second));
              }
            }
            ...
            for (auto child : children) {
              child->commport->dputMessage(new ContinueSearchMessage(
                  msg->answer_commport, nullptr, file, this,
                  getMessagePayloadValue(MessagePayload::CONTINUE_SEARCH),
                  answered, metavisor->defaultTimeToLive));
            }
          }
        } else {
          msg->answer_commport->dputMessage(
  ...
  } else if (auto msg = dynamic_cast<StorageServiceFileReadRequestMessage *>(
                 message.get())) {
    ...
    if (cached(file)) { // File Cached
      ...
      best->getStorageService()->commport->dputMessage(
          new StorageServiceFileReadRequestMessage(
              msg->answer_commport, simgrid::s4u::this_actor::get_host(), best,
      ...
      if (internalStorage && internalStorage->hasFile(file)) {
        ...
        WRENCH_DEBUG("File %s found in internal Storage",
                     file->getID().c_str());
        // File in internal storage
        cache.add(file, FileLocation::LOCATION(internalStorage, file));
        internalStorage->commport->dputMessage(
            new StorageServiceFileReadRequestMessage(
                msg->answer_commport, simgrid::s4u::this_actor::get_host(),
        ...
        if (!children.empty()) { // recursive search
          shared_ptr<bool> answered = make_shared<bool>(false);
          Alarm::createAndStartAlarm(
              this->simulation,
              wrench::S4U_Simulation::getClock() +
                  this->getPropertyValueAsTimeInSecond(
                      Property::FILE_NOT_FOUND_TIMEOUT),
              this->hostname, this->commport,
              new FileNotFoundAlarm(msg->answer_commport, file, true, answered),
              "XROOTD_FileNotFoundAlarm");
          if (reduced) {
            WRENCH_DEBUG("Starting advanced search for %s",
                         file->getID().c_str());
            ...
            for (auto child : children) {
              child->commport->dputMessage(new ContinueSearchMessage(
                  msg->answer_commport,
                  make_shared<StorageServiceFileReadRequestMessage>(msg), file,
                  this, getMessagePayloadValue(MessagePayload::CONTINUE_SEARCH),
                  answered, metavisor->defaultTimeToLive));
            }
          }
          ...
          msg->answer_commport->dputMessage(
              new StorageServiceFileReadAnswerMessage(
                  FileLocation::LOCATION(internalStorage, file), false,
                  std::shared_ptr<FailureCause>(new FileNotFound(
                      FileLocation::LOCATION(internalStorage, file))),
                  nullptr, 0, 1,
                  getMessagePayloadValue(
    ...
    if (cached(msg->file)) { // File Cached
      ...
      if (internalStorage && internalStorage->hasFile(msg->file)) {
        WRENCH_DEBUG("Found %s in internal storage",
                     msg->file->getID().c_str());
        // File in internal storage
        cache.add(msg->file,
                  FileLocation::LOCATION(internalStorage, msg->file));
        ...
        if (children.size() > 0 &&
            msg->timeToLive >
                0) { // shotgun continued search message to all children
          WRENCH_DEBUG(" Basic Search broadcast for %s",
                       msg->file->getID().c_str());

          S4U_Simulation::compute(this->getPropertyValueAsDouble(
              Property::SEARCH_BROADCAST_OVERHEAD));
          for (auto child : children) {
            child->commport->dputMessage(new ContinueSearchMessage(msg));
          }
        } else {
          // this is a leaf that just didn't have the file.  XRootD protocol is
          // to silently fail in this case.  Do not respond
        }
      }
    }
    return true;
    ...
    if (this != msg->node && supervisor) {
      ...
      if (!*msg->answered) {
        *msg->answered = true;
        // WRENCH_INFO("%p %p",msg,msg->answered.get());
        // WRENCH_DEBUG("Sending File %s found to request",
        // msg->file->getID().c_str());
        ...
        if (msg->original) { // this was a file read
          ...
          best->getStorageService()->commport->dputMessage(
              new StorageServiceFileReadRequestMessage(
                  msg->answer_commport, simgrid::s4u::this_actor::get_host(),
                  best, msg->original->num_bytes_to_read,
                  getMessagePayloadValue(
          ...
          msg->answer_commport->dputMessage(
              new StorageServiceFileLookupAnswerMessage(
                  msg->file,
                  true, // xrootd silently fails if the file doesn't exist, so
                        // if we have gotten here, the file does for sure exist
                  getMessagePayloadValue(
                      MessagePayload::FILE_SEARCH_ANSWER_MESSAGE_PAYLOAD)));
        }
      }
    ...
    if (reduced) { // TODO use optimised search
      ...
      for (auto entry : splitStacks) {
        if (entry.first == this) { // this node was the target
          // we shouldn't have to worry about this, it should have been handled
          // earlier.
          //  But just in case, I don't want a rogue search going who knows
          //  where
        } else {
          entry.first->commport->dputMessage(new AdvancedRippleDelete(
              msg, metavisor->defaultTimeToLive, entry.second));
        }
      }
      msg->answer_commport->dputMessage(
          new StorageServiceFileDeleteAnswerMessage(
              msg->location->getFile(), getSharedPtr<Node>(), true, nullptr,
              getMessagePayloadValue(StorageServiceMessagePayload::
                                         FILE_DELETE_ANSWER_MESSAGE_PAYLOAD))

      );
    } else {
      this->commport->dputMessage(
          new RippleDelete(msg, metavisor->defaultTimeToLive));

      msg->answer_commport->dputMessage(
          new StorageServiceFileDeleteAnswerMessage(
              msg->location->getFile(), this->getSharedPtr<Node>(), true,
              nullptr,
              getMessagePayloadValue(
    ...
    S4U_Simulation::compute(
        this->getPropertyValueAsDouble(Property::SEARCH_BROADCAST_OVERHEAD));
    if (children.size() > 0 &&
        msg->timeToLive > 0) { // shotgun remove search message to all children
      shared_ptr<bool> answered = make_shared<bool>(false);
    ...
    if (not internalStorage) {
      ...
      std::string error_message =
          "Cannot write file at non-storage XRootD node";
      ...
      msg->answer_commport->dputMessage(
          new StorageServiceFileWriteAnswerMessage(
              ...
              {}, 0,
              getMessagePayloadValue(
                  MessagePayload::FILE_WRITE_ANSWER_MESSAGE_PAYLOAD)));

    } else {
      // Forward the message
      msg->location = FileLocation::LOCATION(internalStorage, file);
      //                    msg->buffer_size =
      //                    internalStorage->getPropertyValueAsSizeInByte(
    ...
    if (not internalStorage) {
      ...
      if (msg->answer_commport) {
        std::string error_message =
            "Cannot copy file to/from non-storage XRooD node";
        auto location = FileLocation::LOCATION(getSharedPtr<Node>(), file);
        msg->answer_commport->dputMessage(
            new StorageServiceFileCopyAnswerMessage(
                msg->src, msg->dst, false,
                std::shared_ptr<FailureCause>(
                    new NotAllowed(getSharedPtr<Node>(), error_message)),
                getMessagePayloadValue(
                    MessagePayload::FILE_COPY_ANSWER_MESSAGE_PAYLOAD)));
      }
      ...
      internalStorage->commport->dputMessage(message.release());
                 ...
                 message.get())) { // we got a message targeted at a normal
                                   // storage server
    if (not internalStorage) {
      // TODO: deal with the Copy message, which then needs a reply with a
      // NotAllowed failure cause,
      // TODO: just like for FileWriteRequest above
      throw std::runtime_error("Non-Storage XRooD node received a message it "
                               "cannot process - internal error");
    } else {
      // Forwarding the message as-is to the internal Storage
      internalStorage->commport->dputMessage(message.release());
    }
  } else {
    throw std::runtime_error("Node:processNextMessage(): Unexpected [" +
...
Node::selectBest(std::set<std::shared_ptr<FileLocation>> locations) {
 ...
 */
Node *Node::getParent() { return supervisor; }

//        /**
//        * @brief A meta operation to determine if a file exists on this node
//        * @param file: the file to check for
//        * @return a shared pointer the file location if the file was found.
//        Nullptr if the file was not found or if this is not a storage server
//        */
//        std::shared_ptr<FileLocation> Node::hasFile(shared_ptr<DataFile>
//        file){
 ...
 */
bool Node::cached(shared_ptr<DataFile> file) { return cache.isCached(file); }
/**
...
Node::getCached(shared_ptr<DataFile> file) {
  return cache[file];
}

/**
 * @brief Makes this node a supervisor.  Since there is nothing special about a
 * supervisor, this cant fail and does nothing
 * @return true
 */
 ...
 * @param hostname: the name of the host on which the service and its storage
 * service should run
 * @param property_list: A property list
 * @param messagepayload_list: A Message Payload list
 *
 */
Node::Node(Deployment *deployment, const std::string &hostname,
           WRENCH_PROPERTY_COLLECTION_TYPE property_list,
           WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list)
    : StorageService(hostname, "XRootDNode") {

  // Create /dev/null mountpoint so that Locations can be created
  this->file_systems[LogicalFileSystem::DEV_NULL] =
      LogicalFileSystem::createLogicalFileSystem(
          hostname, this, LogicalFileSystem::DEV_NULL, "NONE");

  this->setProperties(this->default_property_values, property_list);
  setMessagePayloads(default_messagepayload_values, messagepayload_list);
  cache.maxCacheTime =
      getPropertyValueAsTimeInSecond(Property::CACHE_MAX_LIFETIME);
  this->deployment = deployment;
  //            this->buffer_size = DBL_MAX;// Not used, but letting it be zero
  //            will raise unwanted exception since
  //            // clients "think" that they're talking to a real storage
  //            service
}
 ...
 * @param property_list: The property list to use for the underlying storage
 * server
 * @param messagepayload_list: The message payload list to use for the
 * underlying storage server
 *
 * @return true if the Node was made a storage server.  false if it was already
 * a storage server
 */
bool Node::makeFileServer(
    std::set<std::string> path, WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list) {
  if (internalStorage != nullptr) {
    return false;
  }
  //            internalStorage = make_shared<SimpleStorageService>(hostname,
  //            path, property_list, messagepayload_list);
  internalStorage = std::shared_ptr<SimpleStorageService>(
      SimpleStorageService::createSimpleStorageService(
          hostname, path, property_list, messagepayload_list));
  return true;
}
 ...
 * @return A map where each key is the next node to broadcast too, and the value
 * is the search stack to send
 */
map<Node *, vector<stack<Node *>>>
Node::splitStack(vector<stack<Node *>> search_stack) {
  map<Node *, vector<stack<Node *>>> splitStacks;
  for (stack<Node *> aStack : search_stack) {
    if (!aStack.empty()) {
      ...
      splitStacks[this].push_back(
          stack<Node *>()); // indicates THIS node was an endpoint
    }
  }
 ...
 */
double
Node::getFileLastWriteDate(const std::shared_ptr<FileLocation> &location) {
  if (location == nullptr) {
    throw std::invalid_argument(
        "Node::getFileLastWriteDate(): Invalid nullptr argument");
  }
  if (internalStorage) {
    return internalStorage->getFileLastWriteDate(location);
  } else {
    throw std::runtime_error(
        "Node::getFileLastWriteDate() called on non storage Node " + hostname);
  }
}
...
/**
 * @brief Remove a directory and all its content at the Node (in zero simulated
 * time)
 * @param path: a path
 */
void Node::removeDirectory(const std::string &path) {
  throw std::runtime_error(
      "Node::removeDirectory(): Not implemented yet (ever?)");
}
 ...
 */
vector<stack<Node *>>
Node::constructFileSearchTree(const vector<shared_ptr<Node>> &targets) {
  vector<stack<Node *>> ret;
  for (auto target : targets) {
    if (target.get() == this) {
      ret.push_back(stack<Node *>()); // I don't think this should ever happen,
                                      // but it might
    } else {
 ...
 *
 * @returns the path to the target if it is in the subtree, empty stack
 * otherwise.
 */
stack<Node *> Node::constructSearchStack(Node *target) {
  ...
  if (next != this) { // failed to find this in parent tree
    ...
    WRENCH_DEBUG("Found file server %s in subtree at depth %zu",
                 target->getName().c_str(), ret.size());
  }
  return ret;
}

/**
 * @brief create a new file in the federation on this node.  Use instead of
 * wrench::Simulation::createFile when adding files to XRootD
 * @param location: a file location, must be the same object as the function is
 * invoked on
 *
 * @throw std::invalid_argument
 */
void Node::createFile(const std::shared_ptr<FileLocation> &location) {
  if (internalStorage == nullptr) {
    throw std::runtime_error("Node::createFile() called on non storage Node " +
                             hostname);
  }
...
void Node::removeFile(const std::shared_ptr<FileLocation> &location) {
  if (internalStorage == nullptr) {
    throw std::runtime_error("Node::removeFile() called on non storage Node " +
                             hostname);
  }

  internalStorage->removeFile(location);
  auto it = std::find(metavisor->files[location->getFile()].begin(),
                      metavisor->files[location->getFile()].end(),
 ...
 * @param wait_for_answer: true if this method should wait for the answer, false
 * otherwise
 *
 * @throw std::invalid_argument
 */
void Node::writeFile(S4U_CommPort *answer_commport,
  ...
  if (internalStorage == nullptr) {
    ...
    throw ExecutionException(std::shared_ptr<FailureCause>(
        new NotAllowed(getSharedPtr<Node>(), error_message)));
  }
  // Replace the location with
  // TODO: THIS LOCATION REWRITE WAS DONE TO FIX SOMETHING BUT HENRI HAS NO
  // TODO: IDEA HOW COME IT'S EVER WORKED BEFORE SINCE THE FTT INSIDE THE
  // INTERNALSTORAGE
  // TODO: WILL SAY "THIS IS NOT ME, MY PARENT IS THE INTERNALSTORAGE, NOT THE
  // NODE"
  auto new_location = FileLocation::LOCATION(
      internalStorage, location->getPath(), location->getFile());
  internalStorage->writeFile(answer_commport, new_location, num_bytes_to_write,
                             wait_for_answer);
  //            internalStorage->writeFile(answer_commport, location,
  //            wait_for_answer);
  metavisor->files[location->getFile()].push_back(this->getSharedPtr<Node>());
}
 ...
 * @return The child
 */
std::shared_ptr<Node> Node::addChildStorageServer(
    const std::string &hostname, const std::string &mount_point,
  ...
  if (storage_property_list.find(
          wrench::SimpleStorageServiceProperty::BUFFER_SIZE) !=
      storage_property_list.end()) {
    if (UnitParser::parse_size(
            storage_property_list
                [wrench::SimpleStorageServiceProperty::BUFFER_SIZE]) < 1) {
      throw std::invalid_argument("Node::addChildStorageServer(): XRootD "
                                  "current does not support 0 buffer_size");
    }
  }
  return this->addChild(this->deployment->createStorageServer(
      hostname, mount_point, storage_property_list, storage_messagepayload_list,
      node_property_list, node_messagepayload_list));
}

/**
 * @brief Determines whether the storage service has the file. This doesn't
 * simulate anything and is merely a zero-simulated-time data structure lookup.
 * If you want to simulate the overhead of querying the StorageService, instead
 * use lookupFile().
 * @param location: a location
...
bool Node::hasFile(const shared_ptr<FileLocation> &location) {
  ...
  // return false;//no internal storage here, so I dont have any files.  But I
  // am pretending to have some, so its reasonable to ask. alternativly
  return !constructFileSearchTree(metavisor->getFileNodes(location->getFile()))
              .empty(); // meta search the subtree for the file.  If its in the
                        // subtree we can find a route to it, so we have it
}
...
bool Node::isBufferized() const {
  if (internalStorage) {
    ...
    throw std::runtime_error(
        "Node::isBufferized() called on non storage Node " + hostname);
  }
}
...
double Node::getBufferSize() const {
  if (internalStorage) {
    ...
    throw std::runtime_error(
        "Node::getBufferSize() called on non storage Node " + hostname);
  }
}
...
bool Node::reserveSpace(std::shared_ptr<FileLocation> &location) {
  if (internalStorage) {
    ...
    throw std::runtime_error(
        "Node::reserveSpace() called on non storage Node " + hostname);
  }
}
...
void Node::unreserveSpace(std::shared_ptr<FileLocation> &location) {
  if (internalStorage) {
    ...
    throw std::runtime_error(
        "Node::unreserveSpace() called on non storage Node " + hostname);
  }
}
