...
std::unordered_map<std::string, std::shared_ptr<FileLocation>>
    FileLocation::file_location_map;
...
FileLocation::createFileLocation(const std::shared_ptr<StorageService> &ss,
                                 const std::shared_ptr<DataFile> &file,
                                 const std::string &path, bool is_scratch) {
  // This looks inefficient (compare to using std::hash<void*> for instance),
  // but it has no collisions and benchmarking showed that (with compiler
  // optimizations), it makes very little difference
  std::string key = (ss ? ss->getName() : "") + "|" + path + "|" +
                    file->getID() + "|" + (is_scratch ? "1" : "0");
  if (FileLocation::file_location_map.find(key) !=
      FileLocation::file_location_map.end()) {
    return FileLocation::file_location_map[key];
  }
  auto new_location = std::shared_ptr<FileLocation>(
      new FileLocation(ss, file, path, is_scratch));

  if (FileLocation::file_location_map.size() -
          FileLocation::file_location_map_previous_size >
      RECLAIM_TRIGGER) {
    FileLocation::reclaimFileLocations();
    FileLocation::file_location_map_previous_size =
        FileLocation::file_location_map.size();
  }
...
void FileLocation::reclaimFileLocations() {
  for (auto it = FileLocation::file_location_map.cbegin();
       it != FileLocation::file_location_map.cend();) {
 ...
 */
std::shared_ptr<FileLocation>
FileLocation::SCRATCH(const std::shared_ptr<DataFile> &file) {
  if (file == nullptr) {
    throw std::invalid_argument(
        "FileLocation::SCRATCH(): Cannot pass nullptr file");
  }
...
/**
 * @brief File location specifier for a storage service's (single) mount point
 * root
 *
 ...
 */
std::shared_ptr<FileLocation>
FileLocation::LOCATION(const std::shared_ptr<StorageService> &ss,
                       const std::shared_ptr<DataFile> &file) {
  if (!ss or !file) {
    throw std::invalid_argument(
        "FileLocation::LOCATION(): invalid nullptr arguments");
  }
...
/**
 * @brief File location specifier for a storage service's (single) mount point
 * root Used in case of NFS with page cache.
 * @param ss: storage service on the client (whose the page cache that data is
 * written to)
 * @param server_ss: a server storage service in NFS that stores the file on
 * disk
 * @param file: a file
 * @return a file location specification
 *
...
FileLocation::LOCATION(const std::shared_ptr<StorageService> &ss,
  ...
  if (ss == nullptr) {
    throw std::invalid_argument(
        "FileLocation::LOCATION(): Cannot pass nullptr storage service");
  }
...
FileLocation::LOCATION(const std::shared_ptr<StorageService> &ss,
  ...
  if (file == nullptr) {
    throw std::invalid_argument(
        "FileLocation::LOCATION(): Cannot pass nullptr file");
  }
  if (path.empty()) {
    throw std::invalid_argument(
        "FileLocation::LOCATION(): must specify a non-empty path");
  }

  return FileLocation::createFileLocation(
      ss, file, FileLocation::sanitizePath(path + "/"), false);
}
...
std::string FileLocation::toString() {
  if (this->is_scratch) {
    ...
    return this->storage_service->getName() + ":" + sanitizePath(this->path) +
           ":" + this->file->getID();
  }
}
...
std::shared_ptr<StorageService> FileLocation::getStorageService() {
  if (this->is_scratch) {
    throw std::invalid_argument("FileLocation::getStorageService(): No storage "
                                "service for a SCRATCH location");
  }
 ...
 */
std::shared_ptr<StorageService> FileLocation::setStorageService(
 ...
 * @return a file
 */
std::shared_ptr<DataFile> FileLocation::getFile() { return this->file; }
...
std::shared_ptr<StorageService> FileLocation::getServerStorageService() {
  if (this->is_scratch) {
    throw std::invalid_argument(
        "FileLocation::getServerStorageService(): No server storage service "
        "for a SCRATCH location");
  }
...
std::string FileLocation::getPath() {
  if (this->is_scratch) {
    throw std::invalid_argument(
        "FileLocation::getPath(): No path for a SCRATCH location");
  }
  return this->path;
}
 ...
 */
bool FileLocation::isScratch() const { return this->is_scratch; }
...
std::string FileLocation::sanitizePath(const std::string &path) {
  if (path == "/")
    return "/"; // make the common case fast

  if (path.empty()) {
    throw std::invalid_argument(
        "FileLocation::sanitizePath(): path cannot be empty");
  }
  ...
  for (auto const &c : disallowed_characters) {
    if (path.find(c) != std::string::npos) {
      throw std::invalid_argument(
          "FileLocation::sanitizePath(): Disallowed character '" +
          std::string(1, c) + "' in path (" + path + ")");
    }
  }
  ...
  //                if (new_tokens.empty()) {
  //                    throw
  //                    std::invalid_argument("FileLocation::sanitizePath():
  //                    Invalid path (" + path + ")");
  //                } else {
  //                    new_tokens.pop_back();
  //                }
 ...
 */
bool FileLocation::properPathPrefix(const std::string &path1,
                                    const std::string &path2) {

  return ((path2.size() >= path1.size() &&
           path2.compare(0, path1.size(), path1) == 0) or
          (path2.size() < path1.size() &&
           path1.compare(0, path2.size(), path2) == 0));
}
...
simgrid::s4u::Disk *FileLocation::getDiskOrNull() {
  ...
  auto sss =
      std::dynamic_pointer_cast<SimpleStorageService>(this->storage_service);
  if (not sss) {
    return nullptr;
  }
