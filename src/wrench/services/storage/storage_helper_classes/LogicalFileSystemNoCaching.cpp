...
WRENCH_LOG_CATEGORY(wrench_core_logical_file_system_no_caching,
                    "Log category for Logical File System No caching");
 ...
 */
LogicalFileSystemNoCaching::LogicalFileSystemNoCaching(
 ...
 */
void LogicalFileSystemNoCaching::storeFileInDirectory(
  ...
  if (this->free_space < 0) {
    throw std::runtime_error("LogicalFileSystemNoCaching::storeFileInDirectory("
                             "): free space is <0, should never happen");
  }

  this->content[fixed_path][file] =
      std::make_shared<LogicalFileSystemNoCaching::FileOnDiskNoCaching>(
 ...
 * @throw std::invalid_argument
 */
void LogicalFileSystemNoCaching::removeFileFromDirectory(
    const std::shared_ptr<DataFile> &file, const std::string &absolute_path) {
  if (devnull) {
    return;
  }
 ...
 * @throw std::invalid_argument
 */
void LogicalFileSystemNoCaching::removeAllFilesInDirectory(
 ...
 */
void LogicalFileSystemNoCaching::updateReadDate(
    const std::shared_ptr<DataFile> &file, const std::string &absolute_path) {
  // Do nothing, as with no caching we don't care about the read date (for now
  // at least)
}
 ...
 */
void LogicalFileSystemNoCaching::
