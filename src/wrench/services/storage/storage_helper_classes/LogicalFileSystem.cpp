...
WRENCH_LOG_CATEGORY(wrench_core_logical_file_system,
                    "Log category for Logical File System");
 ...
 */
std::unique_ptr<LogicalFileSystem> LogicalFileSystem::createLogicalFileSystem(
    const std::string &hostname, StorageService *storage_service,
    const std::string &mount_point, const std::string &eviction_policy) {

  if (storage_service == nullptr) {
    throw std::invalid_argument("LogicalFileSystem::createLogicalFileSystem(): "
                                "nullptr storage_service argument");
  }
  ...
  if (mount_point != DEV_NULL) {
    ...
    if (lfs != LogicalFileSystem::mount_points.end()) {
      throw std::invalid_argument(
          "LogicalFileSystem::createLogicalFileSystem(): A logical file "
          "system, used by some storage service, "
          "already exist on host " +
          hostname + " at mount point " + mount_point);
    }
  }

  //        std::cerr << "CREATING LOGICAL FS " << hostname << ":" <<
  //        mount_point << "\n";
  std::unique_ptr<LogicalFileSystem> to_return;
  if (eviction_policy == "NONE") {
    to_return = std::unique_ptr<LogicalFileSystem>(
        new LogicalFileSystemNoCaching(hostname, storage_service, mount_point));
  } else if (eviction_policy == "LRU") {
    to_return =
        std::unique_ptr<LogicalFileSystem>(new LogicalFileSystemLRUCaching(
            hostname, storage_service, mount_point));
  } else {
    throw std::invalid_argument("LogicalFileSystem::createLogicalFileSystem(): "
 ...
 */
LogicalFileSystem::LogicalFileSystem(const std::string &hostname,
  ...
  if (mount_point == DEV_NULL) {
    ...
    if (not this->disk) {
      throw std::invalid_argument(
          "LogicalFileSystem::LogicalFileSystem(): Host " + hostname +
          " does not have a disk mounted at " + mount_point);
    }
    this->total_capacity =
        S4U_Simulation::getDiskCapacity(hostname, mount_point);
  }
 ...
 * @param absolute_path: the directory's absolute path
 *
 * @return true if the file is present, false if not (or if directory does not
 * exist)
 *
 * @throw std::invalid_argument
 */
bool LogicalFileSystem::isFileInDirectory(const std::shared_ptr<DataFile> &file,
  ...
  return (this->content[fixed_path].find(file) !=
          this->content[fixed_path].end());
}
...
LogicalFileSystem::listFilesInDirectory(const std::string &absolute_path) {
 ...
 */
double LogicalFileSystem::getFreeSpace() const { return this->free_space; }
...
bool LogicalFileSystem::reserveSpace(const std::shared_ptr<DataFile> &file,
  ...
  if (this->reserved_space.find(key) != this->reserved_space.end()) {
    WRENCH_WARN("LogicalFileSystem::reserveSpace(): Space was already being "
                "reserved for storing file %s at path %s:%s. "
                "This is likely a redundant copy, and nothing needs to be done",
                file->getID().c_str(), this->hostname.c_str(),
                fixed_path.c_str());
    return true;
  }
 ...
 * @throw std::invalid_argument
 */
void LogicalFileSystem::unreserveSpace(const std::shared_ptr<DataFile> &file,
                                       const std::string &absolute_path) {
 ...
 */
double LogicalFileSystem::getFileLastWriteDate(const shared_ptr<DataFile> &file,
 ...
 */
simgrid::s4u::Disk *LogicalFileSystem::getDisk() { return this->disk; }
 ...
 * @throw std::invalid_argument
 */
void LogicalFileSystem::stageFile(const std::shared_ptr<DataFile> &file,
  ...
  if (this->free_space < file->getSize()) {
    ...
    throw std::invalid_argument(
        "LogicalFileSystem::stageFile(): Insufficient space to store file " +
