...
WRENCH_LOG_CATEGORY(wrench_core_logical_file_system_lru_caching,
                    "Log category for Logical File System LRU Caching");
 ...
 */
LogicalFileSystemLRUCaching::LogicalFileSystemLRUCaching(
 ...
 */
void LogicalFileSystemLRUCaching::storeFileInDirectory(
    const std::shared_ptr<DataFile> &file, const std::string &absolute_path) {
  if (devnull) {
    return;
  }
  ...
  if (file_already_there) {
    unsigned old_seq = std::static_pointer_cast<FileOnDiskLRUCaching>(
                           this->content[fixed_path][file])
                           ->lru_sequence_number;
    this->lru_list.erase(old_seq);
  }

  this->content[fixed_path][file] = std::make_shared<FileOnDiskLRUCaching>(
      S4U_Simulation::getClock(), this->next_lru_sequence_number++, 0);
  this->lru_list[this->next_lru_sequence_number - 1] =
      std::make_tuple(fixed_path, file);
 ...
 * @throw std::invalid_argument
 */
void LogicalFileSystemLRUCaching::removeFileFromDirectory(
    const std::shared_ptr<DataFile> &file, const std::string &absolute_path) {
  if (devnull) {
    return;
  }
  ...
  auto seq = std::static_pointer_cast<FileOnDiskLRUCaching>(
                 this->content[fixed_path][file])
                 ->lru_sequence_number;
 ...
 * @throw std::invalid_argument
 */
void LogicalFileSystemLRUCaching::removeAllFilesInDirectory(
  ...
  for (auto const &c : this->content[fixed_path]) {
    this->lru_list.erase(
        std::static_pointer_cast<FileOnDiskLRUCaching>(c.second)
            ->lru_sequence_number);
  }
 ...
 */
void LogicalFileSystemLRUCaching::updateReadDate(
    const std::shared_ptr<DataFile> &file, const std::string &absolute_path) {
  if (devnull) {
    return;
  }
  ...
  if (this->content[fixed_path].find(file) != this->content[fixed_path].end()) {
    ...
    unsigned old_seq = std::static_pointer_cast<FileOnDiskLRUCaching>(
                           ...
                           ->lru_sequence_number;
    this->lru_list.erase(old_seq);
    std::static_pointer_cast<FileOnDiskLRUCaching>(
        this->content[fixed_path][file])
        ->lru_sequence_number = new_seq;
...
bool LogicalFileSystemLRUCaching::evictFiles(double needed_free_space) {
  ...
  for (auto const &lru : this->lru_list) {
    ...
    if (std::static_pointer_cast<FileOnDiskLRUCaching>(
            this->content[path][file])
            ->num_current_transactions > 0) {
      //                std::cerr << "PASSING OVER " << path << ":" <<
      //                file->getID() << "\n";
      continue;
    }
  ...
  for (const unsigned int &key : to_evict) {
    ...
    //            std::cerr << "Evicting file " <<  path.c_str() << ":" <<
    //            file->getID().c_str() << "\n";
...
/**
 * @brief Increment the number of running transactions that have to do with a
 * file
 * @param file: the file
 * @param absolute_path: the file path
 */
void LogicalFileSystemLRUCaching::
    incrementNumRunningTransactionsForFileInDirectory(
        const shared_ptr<DataFile> &file, const string &absolute_path) {

  auto fixed_path = FileLocation::sanitizePath(absolute_path + "/");

  if ((this->content.find(fixed_path) != this->content.end()) and
      (this->content[fixed_path].find(file) !=
       this->content[fixed_path].end())) {
    std::static_pointer_cast<FileOnDiskLRUCaching>(
        this->content[fixed_path][file])
        ->num_current_transactions++;
  }
}

/**
 * @brief Decrement the number of running transactions that have to do with a
 * file
 * @param file: the file
 * @param absolute_path: the file path
 */
void LogicalFileSystemLRUCaching::
    decrementNumRunningTransactionsForFileInDirectory(
        const shared_ptr<DataFile> &file, const string &absolute_path) {
