...
WRENCH_LOG_CATEGORY(wrench_core_file_transfer_thread,
                    "Log category for File Transfer Thread");
 ...
 * @param answer_commport_if_read: the commport to send an answer to in case
 * this was a file read ("" if none). This will simply be reported to the parent
 * service, who may use it as needed
 * @param answer_commport_if_write: the commport to send an answer to in case
 * this was a file write ("" if none). This will simply be reported to the
 * parent service, who may use it as needed
 * @param answer_commport_if_copy: the commport to send an answer to in case
 * this was a file copy ("" if none). This will simply be reported to the parent
 * service, who may use it as needed
 * @param buffer_size: the buffer size to use
 */
FileTransferThread::FileTransferThread(
    std::string hostname, std::shared_ptr<StorageService> parent,
    std::shared_ptr<DataFile> file, double num_bytes_to_transfer,
    S4U_CommPort *src_commport, std::shared_ptr<FileLocation> dst_location,
    ...
    : Service(std::move(hostname), "file_transfer_thread"),
      parent(std::move(parent)), file(std::move(file)),
...
FileTransferThread::FileTransferThread(
    std::string hostname, std::shared_ptr<StorageService> parent,
    std::shared_ptr<DataFile> file, double num_bytes_to_transfer,
    std::shared_ptr<FileLocation> src_location, S4U_CommPort *dst_commport,
    S4U_CommPort *answer_commport_if_read,
    S4U_CommPort *answer_commport_if_write,
    S4U_CommPort *answer_commport_if_copy, double buffer_size)
    : Service(std::move(hostname), "file_transfer_thread"),
 ...
 * @param answer_commport_if_read: the commport to send an answer to in case
 * this was a file read (nullptr if none). This will simply be reported to the
 * parent service, who may use it as needed
 * @param answer_commport_if_write: the commport to send an answer to in case
 * this was a file write (nullptr if none). This will simply be reported to the
 * parent service, who may use it as needed
 * @param answer_commport_if_copy: the commport to send an answer to in case
 * this was a file copy (nullptr if none). This will simply be reported to the
 * parent service, who may use it as needed
 * @param buffer_size: the buffer size to use
 */
FileTransferThread::FileTransferThread(
 ...
 */
void FileTransferThread::cleanup(bool has_returned_from_main,
                                 int return_value) {
...
int FileTransferThread::main() {
  ...
  WRENCH_INFO(
      "New FileTransferThread (file=%s, bytes_to_transfer=%.2lf, "
      "src_commport=%s; src_location=%s; dst_commport=%s; dst_location=%s; "
      "answer_commport_if_read=%s; answer_commport_if_write=%s; "
      "answer_commport_if_copy=%s; buffer size=%.2lf)",
      file->getID().c_str(), this->num_bytes_to_transfer,
      ...
      ((answer_commport_if_read == nullptr)
           ? "none"
           : answer_commport_if_read->get_cname()),
      ((answer_commport_if_write == nullptr)
           ? "none"
           : answer_commport_if_write->get_cname()),
      ((answer_commport_if_copy == nullptr)
           ? "none"
           : answer_commport_if_copy->get_cname()),
      this->buffer_size);

  // Create a message to send back (some field of which may be overwritten
  // below)
  msg_to_send_back = new FileTransferThreadNotificationMessage(
      this->getSharedPtr<FileTransferThread>(), this->file, this->src_commport,
      this->src_location, this->dst_commport, this->dst_location,
      this->answer_commport_if_read, this->answer_commport_if_write,
      this->answer_commport_if_copy, true, nullptr);

  if ((this->src_location) and
      (this->src_location->getStorageService() == this->parent) and
      (dst_commport)) {
    /** Sending a local file to the network **/
    try {
      sendLocalFileToNetwork(this->file, this->src_location,
                             num_bytes_to_transfer, this->dst_commport);
    } catch (ExecutionException &e) {
      WRENCH_INFO("FileTransferThread::main(): Network error (%s)",
    ...
    try {
      receiveFileFromNetwork(this->file, this->src_commport,
                             this->dst_location);
    } catch (ExecutionException &e) {
      WRENCH_INFO("FileTransferThread::main() Network error (%s)",
                  e.getCause()->toString().c_str());
  ...
  } else if ((this->src_location) and
             (this->src_location->getStorageService() == this->parent) and
             (this->dst_location) and
             (this->dst_location->getStorageService() == this->parent)) {
    ...
    try {
      downloadFileFromStorageService(this->file, this->src_location,
                                     this->dst_location);
    } catch (ExecutionException &e) {
      msg_to_send_back->success = false;
    ...
    throw std::runtime_error(
        "FileTransferThread::main(): Invalid src/dst combination (" +
  ...
  // Call retire on all commports passed, which is pretty brute force but should
  // work since I synchronized with the parent! BUT IT SHOULDN'T BE MY JOB!!!!
 ...
 */
void FileTransferThread::receiveFileFromNetwork(
    const std::shared_ptr<DataFile> &f, S4U_CommPort *commport,
    const std::shared_ptr<FileLocation> &location) {
  /** Ideal Fluid model buffer size */
  if (this->buffer_size < DBL_EPSILON) {
    throw std::runtime_error("FileTransferThread::receiveFileFromNetwork(): "
                             "Zero buffer size not implemented yet");
    ...
    try {
      ...
      if (Simulation::isPageCachingEnabled()) {
        simulation
            ->getMemoryManagerByHost(location->getStorageService()->hostname)
            ->log();
      }
#endif
      ...
      while (not done) {
        ...
        // In NFS, write to cache only if the current host not the server host
        // where the f is stored If the current host is f server, write to disk
        // directly
#ifdef PAGE_CACHE_SIMULATION
        if (Simulation::isPageCachingEnabled()) {
          ...
          if (write_locally) {
            simulation->writebackWithMemoryCache(f, msg->payload, location,
                                                 true);
          } else {
...
#endif
          // Write to disk
          auto dst_ss = std::dynamic_pointer_cast<SimpleStorageService>(
              dst_location->getStorageService());
          if (!dst_ss) {
            throw std::runtime_error(
                "FileTransferThread::receiveFileFromNetwork(): Storage Service "
                "should be a SimpleStorageService for disk write");
          }
          simulation->writeToDisk(
              msg->payload, location->getStorageService()->hostname,
        ...
        msg = req->wait();
        if (auto file_content_chunk_msg =
                dynamic_cast<StorageServiceFileContentChunkMessage *>(
                    msg.get())) {
          done = file_content_chunk_msg->last_chunk;
        } else {
          throw std::runtime_error("FileTransferThread::receiveFileFromNetwork("
      ...
      if (Simulation::isPageCachingEnabled()) {
        ...
        if (!ss) {
          throw std::runtime_error(
              "FileTransferThread::receiveFileFromNetwork(): Writing to disk "
              "can only be to a SimpleStorageService");
        }
        simulation->writeToDisk(msg->payload, ss->hostname,
                                ss->getPathMountPoint(location->getPath()),
                                location->getDiskOrNull());
#ifdef PAGE_CACHE_SIMULATION
      }
#endif
 ...
 * @throw shared_ptr<FailureCause>
 */
void FileTransferThread::sendLocalFileToNetwork(
    const std::shared_ptr<DataFile> &f,
    const std::shared_ptr<FileLocation> &location, double num_bytes,
    S4U_CommPort *commport) {
  /** Ideal Fluid model buffer size */
  if (this->buffer_size < DBL_EPSILON) {
    throw std::runtime_error("FileTransferThread::sendLocalFileToNetwork(): "
                             "Zero buffer size not implemented yet");
    ...
    try {
      ...
      while (remaining > DBL_EPSILON) {
...
#endif
          WRENCH_INFO("Reading %s bytes from disk",
        ...
        if (req) {
          req->wait();
          //                        WRENCH_INFO("Bytes sent over the network
          //                        were received");
        }
        //                    WRENCH_INFO("Asynchronously sending %s bytes over
        //                    the network", std::to_string(chunk_size).c_str());
        req = commport->iputMessage(new StorageServiceFileContentChunkMessage(
            this->file, chunk_size, (remaining <= 0)));
      }
#ifdef PAGE_CACHE_SIMULATION
      if (Simulation::isPageCachingEnabled()) {
        simulation
...
#endif
      req->wait();
      //                WRENCH_INFO("Bytes sent over the network were
      //                received");
 ...
 */
void FileTransferThread::copyFileLocally(
    const std::shared_ptr<DataFile> &f,
  ...
  if ((src_loc->getStorageService() == dst_loc->getStorageService()) and
    ...
    WRENCH_INFO("FileTransferThread::copyFileLocally(): Copying f %s onto "
                "itself at location %s... ignoring",
  ...
  /** Ideal Fluid model buffer size */
  if (this->buffer_size < DBL_EPSILON) {
    throw std::runtime_error("FileTransferThread::copyFileLocally(): Zero "
                             "buffer size not implemented yet");

  } else {
    auto src_ss = std::dynamic_pointer_cast<SimpleStorageService>(
        src_loc->getStorageService());
    if (!src_ss) {
      throw std::runtime_error(
          "FileTransferThread::receiveFileFromNetwork(): Disk operation can "
          "only be to a SimpleStorageService");
    }
    auto src_mount_point = src_ss->getPathMountPoint(src_loc->getPath());
    auto dst_ss = std::dynamic_pointer_cast<SimpleStorageService>(
        dst_loc->getStorageService());
    if (!dst_ss) {
      throw std::runtime_error(
          "FileTransferThread::receiveFileFromNetwork(): Disk operation can "
          "only be to a SimpleStorageService");
    ...
    while (remaining - this->buffer_size > DBL_EPSILON) {
      simulation->readFromDiskAndWriteToDiskConcurrently(
          ...
          src_loc->getStorageService()->hostname, src_mount_point,
          dst_mount_point, src_loc->getDiskOrNull(), dst_loc->getDiskOrNull());
 ...
 * @param dst_loc: the destination location
 */
void FileTransferThread::downloadFileFromStorageService(
    const std::shared_ptr<DataFile> &f,
    const std::shared_ptr<FileLocation> &src_loc,
    const std::shared_ptr<FileLocation> &dst_loc) {
  ...
  if (f == nullptr) {
    throw std::invalid_argument(
        "StorageService::downloadFile(): Invalid arguments");
  }
#endif

  WRENCH_INFO("Downloading file  %s from location %s", f->getID().c_str(),
              src_loc->toString().c_str());

  // Check that the buffer size is compatible
  if (((this->buffer_size < DBL_EPSILON) &&
       (src_loc->getStorageService()->getBufferSize() > DBL_EPSILON)) or
      ((this->buffer_size > DBL_EPSILON) &&
       (src_loc->getStorageService()->getBufferSize() < DBL_EPSILON))) {
    throw std::invalid_argument(
        "FileTransferThread::downloadFileFromStorageService(): "
        "Incompatible buffer size specs (both must be zero, or both must be "
        "non-zero");
  }
  ...
  src_loc->getStorageService()->commport->putMessage(
      new StorageServiceFileReadRequestMessage(
          request_answer_commport, simgrid::s4u::this_actor::get_host(),
          src_loc, f->getSize(),
  ...
  if (auto msg =
          dynamic_cast<StorageServiceFileReadAnswerMessage *>(message.get())) {
    ...
    WRENCH_INFO(
        "Download request accepted (will receive file content on commport %s)",
        commport_to_receive_the_file_content->get_cname());
  } else {
    throw std::runtime_error(
        "FileTransferThread::downloadFileFromStorageService(): Received an "
        "unexpected [" +
  ...
  try {
    ...
    if (!dst_ss) {
      throw std::runtime_error(
          "FileTransferThread::receiveFileFromNetwork(): Storage Service "
          "should be a SimpleStorageService for disk operations");
    }
    while (not done) {
      // Issue the receive
      auto req = commport_to_receive_the_file_content->igetMessage();
      //                    WRENCH_INFO("Downloaded of %f of f  %s from location
      //                    %s",
      //                                msg->payload, f->getID().c_str(),
      //                                src_loc->toString().c_str());
      // Do the I/O
#ifdef PAGE_CACHE_SIMULATION
      if (Simulation::isPageCachingEnabled()) {
...
#endif
        // Write to disk
        simulation->writeToDisk(msg->payload, dst_ss->getHostname(),
...
#endif
      // Wait for the comm to finish
      //                    WRENCH_INFO("Wrote of %f of f  %s", msg->payload,
      //                    f->getID().c_str());
      msg = req->wait();
      if (auto file_content_chunk_msg =
              dynamic_cast<StorageServiceFileContentChunkMessage *>(
    ...
    // Wait for final ack
    request_answer_commport->getMessage<StorageServiceAckMessage>(
        10, "StorageService::downloadFileFromStorageService(): Received an");
#endif
  } catch (ExecutionException &e) {
    throw;
  }
