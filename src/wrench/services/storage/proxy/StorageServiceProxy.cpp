...
namespace wrench {
    ...
    bool StorageServiceProxy::processNextMessage() {
        ...
        try {
            message = this->mailbox->getMessage<ServiceMessage>();
        ...
        if (auto msg = dynamic_cast<ServiceStopDaemonMessage *>(message.get())) {//handle all the rest of the messages
            ...
            if (cache->hasFile(msg->location)) {//forward request to cache
                ...
                msg->answer_mailbox->dputMessage(new StorageServiceFileLookupAnswerMessage(msg->location->getFile(), true, StorageServiceMessagePayload::FILE_LOOKUP_ANSWER_MESSAGE_PAYLOAD));

            } else if (target) {
                pending[msg->location->getFile()].push_back(std::move(message));
                //message=std::move(pending[msg->location->getFile()][0]);
                target->mailbox->dputMessage(
                ...
                msg->answer_mailbox->dputMessage(new StorageServiceFileLookupAnswerMessage(msg->location->getFile(), false, StorageServiceMessagePayload::FILE_LOOKUP_ANSWER_MESSAGE_PAYLOAD));
            }
            ...
            if (deleted) {
                msg->answer_mailbox->dputMessage(new StorageServiceFileDeleteAnswerMessage(msg->location->getFile(), target, true, nullptr, StorageServiceMessagePayload::FILE_DELETE_ANSWER_MESSAGE_PAYLOAD));
            } else {
                msg->answer_mailbox->dputMessage(new StorageServiceFileDeleteAnswerMessage(msg->location->getFile(), target, false, std::make_shared<FileNotFound>(msg->location), StorageServiceMessagePayload::FILE_DELETE_ANSWER_MESSAGE_PAYLOAD));
            }
            ...
            if (cache) {     //check cache
                ...
                cache->mailbox->putMessage(new StorageServiceFileWriteRequestMessage(mailbox, msg->requesting_host,
                                                                                                  FileLocation::LOCATION(cache, msg->location->getFile()), msg->location->getFile()->getSize(), 0));
            } else {
                msg->answer_mailbox->putMessage(new StorageServiceFileWriteAnswerMessage(msg->location, false, nullptr, {}, 0, StorageServiceMessagePayload::FILE_WRITE_ANSWER_MESSAGE_PAYLOAD));
            }
            ...
            for (unsigned int i = 0; i < messages.size(); i++) {
                if (auto tmpMsg = dynamic_cast<StorageServiceFileLookupRequestMessage *>(messages[i].get())) {
                    tmpMsg->answer_mailbox->dputMessage(new StorageServiceFileLookupAnswerMessage(*msg));
            ...
            for (unsigned int i = 0; i < messages.size(); i++) {
                if (auto tmpMsg = dynamic_cast<StorageServiceFileWriteRequestMessage *>(messages[i].get())) {
                    ...
                    tmpMsg->answer_mailbox->dputMessage(new StorageServiceFileWriteAnswerMessage(*msg));
            ...
            for (unsigned int i = 0; i < messages.size(); i++) {
                if (auto tmpMsg = dynamic_cast<StorageServiceFileWriteRequestMessage *>(messages[i].get())) {
                    ...
                    tmpMsg->answer_mailbox->dputMessage(new StorageServiceAckMessage(*msg));//forward ACK
                    auto target = remote;
    ...
    bool StorageServiceProxy::copyThenRead(unique_ptr<ServiceMessage> &message) {
        ...
        if (auto msg = dynamic_cast<StorageServiceFileReadRequestMessage *>(message.get())) {
            ...
            if (target) {
                ...
                msg->answer_mailbox->putMessage(new StorageServiceFileReadAnswerMessage(msg->location, false, std::make_shared<FileNotFound>(msg->location), nullptr, 0, 1, StorageServiceMessagePayload::FILE_READ_ANSWER_MESSAGE_PAYLOAD));
            }
            return true;
            ...
            for (unsigned int i = 0; i < messages.size(); i++) {
                ...
                if (auto tmpMsg = dynamic_cast<StorageServiceFileReadRequestMessage *>(messages[i].get())) {
                    ...
                    if (msg->success) {
                        ...
                        mailbox->dputMessage(tmpMsg);//now that the data is cached, resend the message
                        std::swap(messages[i], messages.back());
                        messages.back().release();
                        messages.pop_back();
                        i--;
                    } else {
                        tmpMsg->answer_mailbox->putMessage(new StorageServiceFileReadAnswerMessage(tmpMsg->location, false, msg->failure_cause, nullptr, 0, 1, StorageServiceMessagePayload::FILE_READ_ANSWER_MESSAGE_PAYLOAD));
                        std::swap(messages[i], messages.back());
    ...
    bool StorageServiceProxy::magicRead(unique_ptr<ServiceMessage> &message) {
        ...
        if (auto msg = dynamic_cast<StorageServiceFileReadRequestMessage *>(message.get())) {
            ...
            for (unsigned int i = 0; i < messages.size(); i++) {
                if (auto tmpMsg = dynamic_cast<StorageServiceFileReadRequestMessage *>(messages[i].get())) {
                    if (msg->success) {
                        tmpMsg->answer_mailbox->putMessage(new StorageServiceFileReadAnswerMessage(tmpMsg->location, true, nullptr, nullptr, 0, 1, StorageServiceMessagePayload::FILE_READ_ANSWER_MESSAGE_PAYLOAD));//magic read, send buffersize 0 and we are assumed to be nonbufferized
                        tmpMsg->answer_mailbox->putMessage(new StorageServiceAckMessage(tmpMsg->location));                                                                                                         //emediatly send the expected ack
                        std::swap(messages[i], messages.back());
                        messages.pop_back();
                        i--;
                    } else {
    ...
    bool StorageServiceProxy::readThrough(unique_ptr<ServiceMessage> &message) {
        ...
        if (auto msg = dynamic_cast<StorageServiceFileReadRequestMessage *>(message.get())) {
            ...
            if (target) {
                ...
                target->mailbox->dputMessage(forward);                                                    //send to target
            } else {
                msg->answer_mailbox->putMessage(new StorageServiceFileReadAnswerMessage(msg->location, false, std::make_shared<FileNotFound>(msg->location), nullptr, 0, 1, StorageServiceMessagePayload::FILE_READ_ANSWER_MESSAGE_PAYLOAD));
            }
            return true;
            ...
            for (unsigned int i = 0; i < messages.size(); i++) {
                if (auto tmpMsg = dynamic_cast<StorageServiceFileReadRequestMessage *>(messages[i].get())) {
                    if (msg->success) {
                        ...
                        tmpMsg->answer_mailbox->dputMessage(message.release());//forward success message to first waiting read host
                        return true;
                    } else {
            ...
            for (unsigned int i = 0; i < messages.size(); i++) {
                if (auto tmpMsg = dynamic_cast<StorageServiceFileReadRequestMessage *>(messages[i].get())) {
                    if (first) {//this is the fileread we have been faking
                        ...
                        tmpMsg->answer_mailbox->dputMessage(new StorageServiceAckMessage(*msg));
                        std::swap(messages[i], messages.back());
                        messages.back().release();
                        messages.pop_back();
                        i--;
                        ...
                        tmpMsg->payload = 0;                      //this message has already been sent, this is a fake resend
                        mailbox->dputMessage(tmpMsg);//these should now be cached, and should just drop down to the cache automatically
                        std::swap(messages[i], messages.back());
                        messages.back().release();
                        messages.pop_back();
                        i--;
