...
WRENCH_LOG_CATEGORY(wrench_core_storage_service,
                    "Log category for Storage Service");
...
/**
 * @brief Indicate whether this storage service is a scratch service of a
 * ComputeService
...
//    /**
//     * @brief Store a file at a particular location BEFORE the simulation is
//     launched
//     *
//     * @param location: a file location
//     *
//     */
//    void StorageService::stageFile(const std::shared_ptr<FileLocation>
//    &location) {
//        location->getStorageService()->stageFile(location->getFile(),
//        location->getMountPoint(),
//                                                 location->getAbsolutePathAtMountPoint());
//    }
//
//    /**
//     * @brief Store a file at a particular mount point and directory BEFORE
...
//     */
//    void StorageService::stageFile(const std::shared_ptr<DataFile> &file,
...
void StorageService::writeFile(S4U_CommPort *answer_commport,
  ...
  if (location == nullptr) {
    throw std::invalid_argument(
        "StorageService::writeFile(): Invalid arguments");
  }

  this->assertServiceIsUp();

  this->commport->putMessage(new StorageServiceFileWriteRequestMessage(
      answer_commport, simgrid::s4u::this_actor::get_host(), location,
      num_bytes_to_write,
  ...
  {
    auto msg =
        answer_commport->getMessage<StorageServiceFileWriteAnswerMessage>(
    ...
    if (buffer_size < 1) {
      ...
      answer_commport->getMessage<StorageServiceAckMessage>(
...
/**
 * @brief Synchronously asks the storage service for its total free space
 * capacity, that is, the total capacity at the "/" path. Note that this doesn't
 * mean that that free space could be used to store a single file, as the
 * storage service may have file systems at multiple mount points, may me a
 * front-end for a set of storage systems, etc.
...
/**
 * @brief Synchronously asks the storage service for its total free space
 * capacity at a particular path. Note that this doesn't mean that that free
 * space could be used to store a single file, as the storage service may have
 * file systems at multiple mount points, may be a front-end for a set of
 * storage systems, etc.
...
double StorageService::getTotalFreeSpaceAtPath(const std::string &path) {
  ...
  auto msg = answer_commport->getMessage<StorageServiceFreeSpaceAnswerMessage>(
      this->network_timeout,
      "StorageService::getTotalFreeSpaceAtPath() Received an");
...
bool StorageService::lookupFile(S4U_CommPort *answer_commport,
  ...
  if (!answer_commport or !location) {
    throw std::invalid_argument(
        "StorageService::lookupFile(): Invalid nullptr arguments");
  }

  assertServiceIsUp(this->getSharedPtr<Service>());

  // Send a message to the daemon
  this->commport->putMessage(new StorageServiceFileLookupRequestMessage(
...
void StorageService::readFile(S4U_CommPort *answer_commport,
  ...
  this->commport->putMessage(new StorageServiceFileReadRequestMessage(
      answer_commport, simgrid::s4u::this_actor::get_host(), location,
      num_bytes,
      this->getMessagePayloadValue(
...
/**
 * @brief Synchronously and sequentially read a set of files from storage
 * services
 *
...
/**
 * @brief Synchronously and sequentially upload a set of files from storage
 * services
 *
 * @param locations: a map of files to locations
 *
 ...
 * @throw ExecutionException
 */
void StorageService::writeFiles(
    std::map<std::shared_ptr<DataFile>, std::shared_ptr<FileLocation>>
        locations) {
...
/**
 * @brief Synchronously and sequentially write/read a set of files to/from
 * storage services
 *
 ...
 * @throw ExecutionException
 */
void StorageService::writeOrReadFiles(
  ...
  for (auto const &f : locations) {
    if ((f.first == nullptr) or (f.second == nullptr)) {
      throw std::invalid_argument(
          "StorageService::writeOrReadFiles(): invalid argument");
    }
  }

  // Create a temporary sorted list of files so that the order in which files
  // are read/written is deterministic!
  ...
  for (auto const &f : sorted_files) {
    ...
    if (action == READ) {
      WRENCH_INFO("Reading file %s from location %s", file->getID().c_str(),
      ...
      WRENCH_INFO("Writing file %s to location %s", file->getID().c_str(),
                  location->toString().c_str());
...
void StorageService::deleteFile(S4U_CommPort *answer_commport,
  ...
  if (!answer_commport or !location) {
    throw std::invalid_argument(
        "StorageService::deleteFile(): Invalid nullptr arguments");
  }

  if (location->isScratch()) {
    throw std::invalid_argument(
        "StorageService::deleteFile(): Cannot be called on a SCRATCH location");
  }

  assertServiceIsUp(this->getSharedPtr<Service>());

  // Send a message to the storage service's daemon
  this->commport->putMessage(new StorageServiceFileDeleteRequestMessage(
      answer_commport, location,
      this->getMessagePayloadValue(
...
/**
 * @brief Synchronously ask the storage service to read a file from another
 * storage service
 *
 ...
 * @throw std::invalid_argument
 */
void StorageService::copyFile(
    const std::shared_ptr<FileLocation> &src_location,
    const std::shared_ptr<FileLocation> &dst_location) {
  if ((src_location == nullptr) || (dst_location == nullptr)) {
    throw std::invalid_argument(
        "StorageService::copyFile(): Invalid nullptr arguments");
  }
  if (src_location->getFile() != dst_location->getFile()) {
    throw std::invalid_argument("StorageService::copyFile(): src and dst "
                                "locations should be for the same file");
  }

  if (std::dynamic_pointer_cast<CompoundStorageService>(
          src_location->getStorageService()) or
      std::dynamic_pointer_cast<CompoundStorageService>(
  ...
  //            throw std::runtime_error("Cannot copy a file from a
  //            non-bufferized storage service to a bufferized storage service
  //            (not implemented, yet)");
  //        }
  ...
  src_location->getStorageService()
      ...
      .addTimestampFileCopyStart(Simulation::getCurrentSimulatedDate(), file,
                                 src_location, dst_location);
  commport_to_contact->putMessage(new StorageServiceFileCopyRequestMessage(
      answer_commport, src_location, dst_location,
 ...
 *
 * @param answer_commport: the commport to which a notification message will be
 * sent
 ...
 *
 */
void StorageService::initiateFileCopy(
    S4U_CommPort *answer_commport,
    const std::shared_ptr<FileLocation> &src_location,
    const std::shared_ptr<FileLocation> &dst_location) {
  if ((src_location == nullptr) || (dst_location == nullptr)) {
    throw std::invalid_argument(
        "StorageService::initiateFileCopy(): Invalid nullptr arguments");
  }
  if (src_location->getFile() != dst_location->getFile()) {
    throw std::invalid_argument("StorageService::initiateFileCopy(): src and "
                                "dst locations should be for the same file");
  }
...
//    /**
//     * @brief Store a file at a particular mount point ex-nihilo. Doesn't
//     notify a file registry service and will do nothing (and won't complain)
//     if the file already exists
//     * at that location.
//     *
//     * @param location: a file location, must be the same object as the
//     function is invoked on
//     *
//     * @throw std::invalid_argument
//     */
//    void StorageService::createFile(const std::shared_ptr<FileLocation>
//    &location) {
//        if (location->getStorageService() !=
//        this->getSharedPtr<StorageService>()) {
//            throw
//            std::invalid_argument("StorageService::createFile(file,location)
//            must be called on the same StorageService that the location
//            uses");
//        }
...
//    /**
// * @brief Store a file at a particular mount point ex-nihilo. Doesn't notify a
// file registry service and will do nothing (and won't complain) if the file
// already exists
// * at that location.
//
//*
...
//
//        createFile(FileLocation::LOCATION(this->getSharedPtr<StorageService>(),
//        path, file));
//    }
...
//    /**
//     * @brief Determines whether the storage service has the file. This
//     doesn't simulate anything and is merely a zero-simulated-time data
//     structure lookup.
//     * If you want to simulate the overhead of querying the StorageService,
//     instead use lookupFile().
//     *
//     * @param file a file
//     *
//     * @return true if the file is present, false otherwise
