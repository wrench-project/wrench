 ...
 */
void SimpleStorageServiceNonBufferized::cleanup(bool has_returned_from_main,
                                                int return_value) {
...
SimpleStorageServiceNonBufferized::SimpleStorageServiceNonBufferized(
    ...
    : SimpleStorageService(
          hostname, std::move(mount_points), std::move(property_list),
 ...
 */
void SimpleStorageServiceNonBufferized::processTransactionCompletion(
    const std::shared_ptr<Transaction> &transaction) {

  if (transaction->src_location) {
    transaction->src_location->getStorageService()
        ->decrementNumRunningOperationsForLocation(transaction->src_location);
  }

  // If I was the source and the destination was bufferized, I am the one
  // creating the file there! (yes, this is ugly and lame, and one day we'll
  // clean the storage service implementation
  if (transaction->src_location != nullptr and
    ...
    transaction->dst_location->getStorageService()->createFile(
        transaction->dst_location);
  }

  // TODO: If below I do dputMessage instead of putMessage, the MessImpl objects
  // accumulate
  ...
  if (transaction->dst_location == nullptr) {
    ...
    transaction->commport->dputMessage(
        new StorageServiceAckMessage(transaction->src_location));
    ...
    WRENCH_INFO("File %s stored",
                transaction->dst_location->getFile()->getID().c_str());
    // Deal with time stamps, previously we could test whether a real timestamp
    // was passed, now this. Maybe no corresponding timestamp.
    //            WRENCH_INFO("Sending back an ack for a successful file read");
    transaction->commport->dputMessage(
        new StorageServiceAckMessage(transaction->dst_location));
  } else {
    if (transaction->dst_location->getStorageService() == shared_from_this()) {
      ...
      try {
        this->simulation->getOutput().addTimestampFileCopyCompletion(
            ...
            transaction->dst_location->getFile(), transaction->src_location,
            transaction->dst_location);
    ...
    transaction->commport->dputMessage(new StorageServiceFileCopyAnswerMessage(
        transaction->src_location, transaction->dst_location, true, nullptr,
        this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
                                         FILE_COPY_ANSWER_MESSAGE_PAYLOAD)));
  }
}
 ...
 * @param transaction: the transaction
 */
void SimpleStorageServiceNonBufferized::processTransactionFailure(
    const std::shared_ptr<Transaction> &transaction) {
  throw std::runtime_error("SimpleStorageServiceNonBufferized::"
                           "processTransactionFailure(): not implemented");
}
...
int SimpleStorageServiceNonBufferized::main() {
  ...
  std::string message = "Simple Storage Service (Non-Bufferized) " +
                        this->getName() + "  starting on host " +
                        this->getHostname();
  ...
  //        WRENCH_INFO("STREAMS PENDING: %zu",
  //        this->pending_transactions.size()); WRENCH_INFO("STREAMS RUNNING:
  //        %zu", this->pending_transactions.size());
  ...
  if (Simulation::isPageCachingEnabled()) {
    ...
    this->memory_manager = MemoryManager::initAndStart(
        this->simulation, memory_disk, 0.4, 5, 30, this->hostname);
  }
#endif
  ...
  while (true) {
    ...
    if (not comm_has_been_posted) {
      try {
        comm_ptr = this->commport->s4u_mb->get_async<void>(
    ...
    if (not mess_has_been_posted) {
      mess_ptr = this->commport->s4u_mq->get_async<void>(
          (void **)(&(simulation_message)));
    ...
    try {
      ...
      auto stream =
          boost::dynamic_pointer_cast<simgrid::s4u::Io>(finished_activity);
  ...
  WRENCH_INFO("Simple Storage Service (Non-Bufferized) %s on host %s cleanly "
              "terminating!",
...
bool SimpleStorageServiceNonBufferized::processNextMessage(
  ...
  if (auto msg = dynamic_cast<ServiceStopDaemonMessage *>(message)) {
  ...
  } else if (auto msg = dynamic_cast<StorageServiceFreeSpaceRequestMessage *>(
                 message)) {
    ...
    return processFileLookupRequest(msg->location, msg->answer_commport);
                 ...
                 message)) {
    return processFileWriteRequest(msg->location, msg->num_bytes_to_write,
                                   msg->answer_commport, msg->requesting_host);

  } else if (auto msg = dynamic_cast<StorageServiceFileReadRequestMessage *>(
                 message)) {
    return processFileReadRequest(msg->location, msg->num_bytes_to_read,
                                  msg->answer_commport, msg->requesting_host);
 ...
 */
bool SimpleStorageServiceNonBufferized::processFileWriteRequest(
    std::shared_ptr<FileLocation> &location, double num_bytes_to_write,
    S4U_CommPort *answer_commport, simgrid::s4u::Host *requesting_host) {

  if (buffer_size >= 1.0) {
    throw std::runtime_error(
        "SimpleStorageServiceNonBufferized::processFileWriteRequest(): Cannot "
        "process a write requests with a non-zero buffer size");
  }
  ...
  if (not this->splitPath(location->getPath(), mount_point,
                          path_at_mount_point)) {
    ...
    if (not file_already_there) {
      ...
      if (not success) {
        failure_cause =
            std::shared_ptr<FailureCause>(new StorageServiceNotEnoughSpace(
                file, this->getSharedPtr<SimpleStorageService>()));
      }
    }
  }
  ...
  if (failure_cause) {
    try {
      answer_commport->dputMessage(new StorageServiceFileWriteAnswerMessage(
          location, false,
          std::shared_ptr<FailureCause>(new StorageServiceNotEnoughSpace(
              file, this->getSharedPtr<SimpleStorageService>())),
          {}, 0,
          this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
                                           FILE_WRITE_ANSWER_MESSAGE_PAYLOAD)));
    } catch (wrench::ExecutionException &e) {
      return true;
    }
    return true;
  ...
  answer_commport->dputMessage(new StorageServiceFileWriteAnswerMessage(
      location, true, nullptr, {}, this->buffer_size,
      this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
                                       FILE_WRITE_ANSWER_MESSAGE_PAYLOAD)));
  ...
  auto transaction = std::make_shared<Transaction>(
      nullptr, requesting_host, nullptr, location, me_host, me_disk,
      answer_commport, num_bytes_to_write);
...
bool SimpleStorageServiceNonBufferized::processFileReadRequest(
    const std::shared_ptr<FileLocation> &location, double num_bytes_to_read,
    S4U_CommPort *answer_commport, simgrid::s4u::Host *requesting_host) {
  ...
  std::string path_at_mount_point;
  if ((not this->splitPath(location->getPath(), mount_point,
                           path_at_mount_point)) or
      (not this->file_systems[mount_point]->doesDirectoryExist(
          path_at_mount_point))) {
    failure_cause =
        std::shared_ptr<FailureCause>(new InvalidDirectoryPath(location));
  } else {
    fs = this->file_systems[mount_point].get();

    if (not fs->isFileInDirectory(file, path_at_mount_point)) {
      WRENCH_INFO("Received a read request for a file I don't have (%s)",
  ...
  try {
    answer_commport->putMessage(new StorageServiceFileReadAnswerMessage(
        location, success, failure_cause,
  ...
  if (success) {
    ...
    location->getStorageService()->incrementNumRunningOperationsForLocation(
        location);
    ...
    // Create a Transaction
    auto transaction = std::make_shared<Transaction>(
        location, me_host, me_disk, nullptr, requesting_host, nullptr,
        answer_commport, num_bytes_to_read);
...
bool SimpleStorageServiceNonBufferized::processFileCopyRequestIAmNotTheSource(
  ...
  WRENCH_INFO("FileCopyRequest: %s -> %s", src_location->toString().c_str(),
  ...
  if (src_disk == nullptr) {
    throw std::runtime_error("SimpleStorageServiceNonBufferized::"
                             "processFileCopyRequestIAmNotTheSource(): source "
                             "disk not found - internal error");
  }
  auto dst_disk = dst_location->getDiskOrNull();
  if (dst_disk == nullptr) {
    throw std::runtime_error("SimpleStorageServiceNonBufferized::"
                             "processFileCopyRequestIAmNotTheSource(): "
                             "destination disk not found - internal error");
  }
  ...
  if (not this->splitPath(dst_location->getPath(), dst_mount_point,
                          dst_path_at_mount_point)) {
  ...
  if (!failure_cause) {
    ...
    bool file_is_already_here =
        fs->isFileInDirectory(dst_location->getFile(), dst_path_at_mount_point);
    if ((not file_is_already_here) and
        (not fs->reserveSpace(dst_location->getFile(),
                              dst_path_at_mount_point))) {
  ...
  if (failure_cause) {
    this->simulation->getOutput().addTimestampFileCopyFailure(
        Simulation::getCurrentSimulatedDate(), file, src_location,
        dst_location);
    try {
      answer_commport->putMessage(new StorageServiceFileCopyAnswerMessage(
          src_location, dst_location, false,
          std::shared_ptr<FailureCause>(new StorageServiceNotEnoughSpace(
              file, this->getSharedPtr<SimpleStorageService>())),
          this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
                                           FILE_COPY_ANSWER_MESSAGE_PAYLOAD)));
  ...
  src_location->getStorageService()->incrementNumRunningOperationsForLocation(
      src_location);

  // At this point, there is enough space
  // Create a Transaction
  auto transaction = std::make_shared<Transaction>(
      src_location, src_host, src_disk, dst_location, dst_host, dst_disk,
      answer_commport, file->getSize());
...
bool SimpleStorageServiceNonBufferized::processFileCopyRequestIAmTheSource(
  ...
  // TODO: This code is duplicated with the IAmNotTheSource version of this
  // method
  auto src_host = src_location->getStorageService()->getHost();
  auto dst_host = dst_location->getStorageService()->getHost();
  ...
  if (src_disk == nullptr) {
    throw std::runtime_error(
        "SimpleStorageServiceNonBufferized::processFileCopyRequestIAmTheSource("
        "): source disk not found - internal error");
  }
  auto dst_disk = dst_location->getDiskOrNull();
  if (dst_disk == nullptr) {
    throw std::runtime_error(
  ...
  if (not this->splitPath(src_location->getPath(), src_mount_point,
                          src_path_at_mount_point)) {
    try {
      answer_commport->putMessage(new StorageServiceFileCopyAnswerMessage(
          src_location, dst_location, false,
  ...
  // Do I have the file
  if (not my_fs->isFileInDirectory(src_location->getFile(),
                                   src_path_at_mount_point)) {
    try {
      answer_commport->putMessage(new StorageServiceFileCopyAnswerMessage(
          src_location, dst_location, false,
          std::shared_ptr<FailureCause>(new FileNotFound(src_location)),
          this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
                                           FILE_COPY_ANSWER_MESSAGE_PAYLOAD)));
  ...
  //        auto dst_file_system =
  //        dst_location->getStorageService()->file_systems[dst_location->getMountPoint()].get();
  //        bool file_already_at_destination =
  //        dst_file_system->isFileInDirectory(dst_location->getFile(),
  //        dst_location->getAbsolutePathAtMountPoint());
  bool file_already_at_destination =
      StorageService::hasFileAtLocation(dst_location);

  // If not already at destination make space for it, and if not possible, then
  // return an error
    ...
    if (not dst_location->getStorageService()->reserveSpace(dst_location)) {
      try {
        answer_commport->putMessage(new StorageServiceFileCopyAnswerMessage(
            src_location, dst_location, false,
            std::shared_ptr<FailureCause>(new StorageServiceNotEnoughSpace(
                dst_location->getFile(), dst_location->getStorageService())),
            this->getMessagePayloadValue(
...
void SimpleStorageServiceNonBufferized::startPendingTransactions() {
  while (
      (not this->pending_transactions.empty()) and
    ...
    auto sg_iostream = simgrid::s4u::Io::streamto_init(
                           transaction->src_host, transaction->src_disk,
                           transaction->dst_host, transaction->dst_disk)
...
double SimpleStorageServiceNonBufferized::getLoad() {
  return (double)this->running_transactions.size() +
         (double)this->pending_transactions.size();
}
...
bool SimpleStorageServiceNonBufferized::processFileCopyRequest(
  ...
  if (not StorageService::hasFileAtLocation(src)) {
    answer_commport->dputMessage(new StorageServiceFileCopyAnswerMessage(
        src, dst, false, std::make_shared<FileNotFound>(src),
        this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
                                         FILE_COPY_ANSWER_MESSAGE_PAYLOAD)));
 ...
 */
int SimpleStorageServiceNonBufferized::getNumRunningTransactionsOnDisk(
