...
namespace wrench {
    ...
    void NetworkConnectionManager::addConnection(std::unique_ptr<NetworkConnection> connection) {
      if ((connection->type == NetworkConnection::INCOMING_DATA) or (connection->type == NetworkConnection::OUTGOING_DATA)) {
        ...
        if (this->running_control_connection == nullptr) {
         ...
         if (not this->running_control_connection->start()) {
           throw std::runtime_error("NetworkConnectionManager::addConnection(): Cannot start incoming control connection!");
         }
        }
      }
    }
