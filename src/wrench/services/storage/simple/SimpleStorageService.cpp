 ...
 */
SimpleStorageService *SimpleStorageService::createSimpleStorageService(
  ...
  if (property_list.find(wrench::SimpleStorageServiceProperty::BUFFER_SIZE) !=
      property_list.end()) {
    double buffer_size = UnitParser::parse_size(
    ...
    property_list[wrench::SimpleStorageServiceProperty::BUFFER_SIZE] =
        "0B"; // enforce a zero buffersize
  }

  if (Simulation::isLinkShutdownSimulationEnabled() and (not bufferized)) {
    throw std::runtime_error(
        "SimpleStorageService::createSimpleStorageService(): Cannot use "
        "non-bufferized (i.e., buffer size == 0) "
        "storage services and also simulate link shutdowns. This feature is "
        "not implemented yet.");
  }
 ...
 */
SimpleStorageService::SimpleStorageService(
    const std::string &hostname, const std::set<std::string> &mount_points,
    WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list,
    const std::string &suffix)
    : StorageService(hostname, "simple_storage" + suffix) {
  ...
  if (mount_points.empty()) {
    throw std::invalid_argument(
        "SimpleStorageService::SimpleStorageService(): A storage service must "
        "have at least one mount point");
  }
  for (const auto &mp : mount_points) {
    this->file_systems[mp] = LogicalFileSystem::createLogicalFileSystem(
        this->getHostname(), this, FileLocation::sanitizePath(mp),
...
bool SimpleStorageService::processFileDeleteRequest(
  ...
  if (not this->splitPath(location->getPath(), mount_point,
    ...
    if ((not fs->isFileInDirectory(file, path_at_mount_point))) {
      // If this is scratch, we don't care, perhaps it was taken care of
      // elsewhere...
      if (not this->isScratch()) {
        failure_cause =
            std::shared_ptr<FailureCause>(new FileNotFound(location));
      }
    } else {
  ...
  answer_commport->dputMessage(new StorageServiceFileDeleteAnswerMessage(
      location->getFile(), this->getSharedPtr<SimpleStorageService>(),
      (failure_cause == nullptr), failure_cause,
...
bool SimpleStorageService::processFileLookupRequest(
  ...
  answer_commport->dputMessage(new StorageServiceFileLookupAnswerMessage(
      location->getFile(), file_found,
      this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
...
/**
 * @brief Get the total static capacity of the storage service (in zero
 * simulation time)
...
/**
 * @brief Return current total free space from all mount point (in zero
 * simulation time) for IO tracing purpose.
...
std::string SimpleStorageService::getBaseRootPath() {
  if (this->file_systems.size() > 1) {
    throw std::runtime_error(
        "SimpleStorageService::getBaseRootPath(): storage service has mutiple "
        "mount points, and thus no single getRootPath");
  } else {
...
//            throw std::invalid_argument(
//                    "StorageService::getMountPoint(): The storage service has
//                    more than one mount point");
//        }
//        return
//        wrench::FileLocation::sanitizePath(this->file_systems.begin()->first);
//    }
 ...
 * @return false if the daemon should terminate
 */
bool SimpleStorageService::processFreeSpaceRequest(
    S4U_CommPort *answer_commport, const std::string &path) {
  double free_space = 0;

  auto sanitized_path = FileLocation::sanitizePath(path);
  for (auto const &mp : this->file_systems) {
    if ((sanitized_path == "/") or
        (sanitized_path == FileLocation::sanitizePath(mp.first))) {
      free_space += mp.second->getFreeSpace();
    }
  }

  answer_commport->dputMessage(new StorageServiceFreeSpaceAnswerMessage(
      free_space,
      this->getMessagePayloadValue(SimpleStorageServiceMessagePayload::
                                       FREE_SPACE_ANSWER_MESSAGE_PAYLOAD)));
  return true;
}
 ...
 * @return false if the daemon should terminate
 */
bool SimpleStorageService::processStopDaemonRequest(
 ...
 * @param path: the path
 *
 * @return the file's last write date, or -1 if the file is not found or if the
 * path is invalid
 *
 */
double SimpleStorageService::getFileLastWriteDate(
    const std::shared_ptr<DataFile> &file, const std::string &path) {
  if (!file) {
    throw std::invalid_argument("SimpleStorageService::getFileLastWriteDate(): "
 ...
 */
bool SimpleStorageService::hasFile(
    const std::shared_ptr<FileLocation> &location) {
  std::string mount_point;
  std::string path_at_mount_point;
  if (not this->splitPath(location->getPath(), mount_point,
...
void SimpleStorageService::removeDirectory(const std::string &path) {
  std::string mount_point;
  std::string path_at_mount_point;
  if (not this->splitPath(FileLocation::sanitizePath(path), mount_point,
                          path_at_mount_point)) {
    return;
  }
  auto fs = this->file_systems[mount_point].get();
 ...
 * @param location: a location
 */
void SimpleStorageService::removeFile(
    const std::shared_ptr<FileLocation> &location) {
  std::string mount_point;
  std::string path_at_mount_point;
  if (not this->splitPath(FileLocation::sanitizePath(location->getPath()),
                          mount_point, path_at_mount_point)) {
    return;
  }
  auto fs = this->file_systems[mount_point].get();
  if (fs->doesDirectoryExist(path_at_mount_point)) {
 ...
 */
bool SimpleStorageService::splitPath(const std::string &path,
                                     std::string &mount_point,
                                     std::string &path_at_mount_point) {
  auto sanitized_path = FileLocation::sanitizePath(path);
  for (auto const &fs : this->file_systems) {
 ...
 * @param location: a location
 */
void SimpleStorageService::decrementNumRunningOperationsForLocation(
    const std::shared_ptr<FileLocation> &location) {
  std::string mount_point;
  std::string path_at_mount_point;
  ...
  this->file_systems[mount_point]
      ->decrementNumRunningTransactionsForFileInDirectory(location->getFile(),
                                                          path_at_mount_point);
}
 ...
 * @param location: a location
 */
void SimpleStorageService::incrementNumRunningOperationsForLocation(
    const std::shared_ptr<FileLocation> &location) {
  std::string mount_point;
  std::string path_at_mount_point;
  ...
  this->splitPath(location->getPath(), mount_point, path_at_mount_point);
  this->file_systems[mount_point]
      ->incrementNumRunningTransactionsForFileInDirectory(location->getFile(),
                                                          path_at_mount_point);
}
 ...
 * @param location: a location
 */
void SimpleStorageService::createFile(
    const std::shared_ptr<FileLocation> &location) {
  ...
  this->splitPath(location->getPath(), mount_point, path_at_mount_point);
  this->file_systems[mount_point]->unreserveSpace(location->getFile(),
                                                  path_at_mount_point);
  bool enough_space = (this->file_systems[mount_point]->getFreeSpace() >=
  ...
  this->file_systems[mount_point]->storeFileInDirectory(location->getFile(),
                                                        path_at_mount_point);
}

/**
 * @brief Get a file's last write date at a the storage service (in zero
 * simulated time)
 * @param location:  a location
...
double SimpleStorageService::getFileLastWriteDate(
  ...
  return this->file_systems[mount_point]->getFileLastWriteDate(
 ...
 */
simgrid::s4u::Disk *
SimpleStorageService::getDiskForPathOrNull(const string &path) {
  std::string mount_point, path_at_mount_point;
