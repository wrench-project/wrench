 ...
 */
void Service::setProperty(WRENCH_PROPERTY_TYPE property,
 ...
 * @param messagepayload: the message payload (which must a a string
 * representation of a >=0 double)
 * @param value: the message payload value
 * @throw std::invalid_argument
 */
void Service::setMessagePayload(WRENCH_MESSAGEPAYLOAD_TYPE messagepayload,
                                double value) {
  // Check that the value is a >=0 double

  if (value < 0) {
    throw std::invalid_argument(
        "Service::setMessagePayload(): Invalid message payload value " +
        ServiceMessagePayload::translatePayloadType(messagepayload) + ": " +
        std::to_string(value));
  }
...
std::string Service::getPropertyValueAsString(WRENCH_PROPERTY_TYPE property) {
  if (this->property_list.find(property) == this->property_list.end()) {
    throw std::invalid_argument(
        "Service::getPropertyValueAsString(): Cannot find value for property " +
        ServiceProperty::translatePropertyType(property) +
...
double Service::getPropertyValueAsDouble(WRENCH_PROPERTY_TYPE property) {
  ...
  if (sscanf(string_value.c_str(), "%lf", &value) != 1) {
    throw std::invalid_argument(
        "Service::getPropertyValueAsDouble(): Invalid double property value " +
        ServiceProperty::translatePropertyType(property) + " " +
 ...
 */
double Service::getPropertyValueWithUnitsAsValue(
...
double Service::getPropertyValueAsTimeInSecond(WRENCH_PROPERTY_TYPE property) {
  return this->getPropertyValueWithUnitsAsValue(property,
                                                UnitParser::parse_time);
}

/**
 * @brief Method to parse a property value that is a date size with (optional)
 * units
 * @param property: the property
...
double Service::getPropertyValueAsSizeInByte(WRENCH_PROPERTY_TYPE property) {
  return this->getPropertyValueWithUnitsAsValue(property,
                                                UnitParser::parse_size);
}

/**
 * @brief Method to parse a property value that is a bandwidth with (optional)
 * units
 * @param property: the property
 * @return the bandwidth in byte/sec
 */
double Service::getPropertyValueAsBandwidthInBytePerSecond(
    WRENCH_PROPERTY_TYPE property) {
  return this->getPropertyValueWithUnitsAsValue(property,
 ...
 */
unsigned long
Service::getPropertyValueAsUnsignedLong(WRENCH_PROPERTY_TYPE property) {
  unsigned long value;
  std::string string_value;
  string_value = this->getPropertyValueAsString(property);
  if (string_value == "infinity") {
  ...
  if (sscanf(string_value.c_str(), "%lu", &value) != 1) {
    throw std::invalid_argument(
        "Service::getPropertyValueAsUnsignedLong(): Invalid unsigned long "
        "property value " +
        ServiceProperty::translatePropertyType(property) + " " +
        this->getPropertyValueAsString(property));
  }
 ...
 */
double
Service::getMessagePayloadValue(WRENCH_MESSAGEPAYLOAD_TYPE message_payload) {
  if (this->messagepayload_list.find(message_payload) ==
      this->messagepayload_list.end()) {
    try {
      throw std::invalid_argument(
          "Service::getMessagePayloadValue(): Cannot find value for "
          "message_payload " +
          ServiceMessagePayload::translatePayloadType(message_payload) +
          " (perhaps a derived service class does not provide a default "
          "value?)");
 ...
 */
const WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE &
Service::getMessagePayloadList() const {
...
bool Service::getPropertyValueAsBoolean(WRENCH_PROPERTY_TYPE property) {
  ...
  if (string_value == "true" or string_value == "True") {
    ...
    throw std::invalid_argument(
        "Service::getPropertyValueAsBoolean(): Invalid boolean property "
        "value " +
        ServiceProperty::translatePropertyType(property) + " " +
        this->getPropertyValueAsString(property));
  }
 ...
 * @param auto_restart: true if the daemon should restart automatically after a
 * reboot or not
 *
 * @throw std::runtime_error
 * @throw std::shared_ptr<HostError>
 */
void Service::start(const std::shared_ptr<Service> &this_service,
...
/**
 * @brief Synchronously stop the service (does nothing if the service is already
 * stopped)
 *
...
void Service::resume() {
  if (this->state != Service::SUSPENDED) {
    std::string what =
        "Service cannot be resumed because it is not in the suspended state";
    throw ExecutionException(
        std::make_shared<NotAllowed>(this->getSharedPtr<Service>(), what));
  }
  this->resumeActor();
  this->state = Service::UP;
}
 ...
 * @return the hostname
 */
std::string Service::getHostname() { return this->hostname; }

/**
 * @brief Get the the host on which the service is / will be running
 * @return the hostname
 */
simgrid::s4u::Host *Service::getHost() { return this->host; }

/**
 * @brief Get the physical name of the host on which the service is / will be
 * running
 * @return the physical hostname
 */
std::string Service::getPhysicalHostname() {
  if (S4U_VirtualMachine::vm_to_pm_map.find(this->hostname) !=
 ...
 */
bool Service::isUp() { return (this->state == Service::UP); }

/**
 * @brief Set the state of the service to DOWN
 */
void Service::setStateToDown() { this->state = Service::DOWN; }

/**
 * @brief Set default and user-defined properties
 * @param default_property_values: list of default properties
 * @param overridden_property_values: list of overridden properties (override
 * the default)
 */
void Service::setProperties(
    const WRENCH_PROPERTY_COLLECTION_TYPE &default_property_values,
 ...
 * @param overridden_messagepayload_values: list of overridden message payloads
 * (override the default)
 */
void Service::setMessagePayloads(
    const WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE &default_messagepayload_values,
    const WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE
        &overridden_messagepayload_values) {
 ...
 */
void Service::serviceSanityCheck() { assertServiceIsUp(); }

/**
 * @brief Returns the service's network timeout value
 * @return a duration in seconds
 */
double Service::getNetworkTimeoutValue() const { return this->network_timeout; }
...
void Service::assertServiceIsUp() {
  if (this->state == Service::DOWN) {
    throw ExecutionException(std::shared_ptr<FailureCause>(
        new ServiceIsDown(this->getSharedPtr<Service>())));
  }
  if (this->state == Service::SUSPENDED) {
    throw ExecutionException(std::shared_ptr<FailureCause>(
        new ServiceIsSuspended(this->getSharedPtr<Service>())));
  }
}
