...
Block::Block(std::string fid, std::shared_ptr<FileLocation> location, double sz,
    ...
    : file_id(fid), location(location), size(sz), last_access(last_access),
      dirty(is_dirty), dirty_time(dirty_time) {}
 ...
 * @return the file id
 */
std::string Block::getFileId() { return this->file_id; }

/**
 * @brief Set the block's file id
 * @param fid: a file id
 */
void Block::setFileId(std::string &fid) { this->file_id = fid; }
 ...
 */
double Block::getSize() const { return this->size; }

/**
 * @brief Set the block's size
 * @param size: a size in bytes
 */
void Block::setSize(double size) { this->size = size; }

/**
 * @brief Get the block's last access time
 * @return a date
 */
double Block::getLastAccess() const { return this->last_access; }
 ...
 */
bool Block::isDirty() const { return this->dirty; }

/**
 * @brief Set the block's dirty status
 * @param is_dirty: true or false
 */
void Block::setDirty(bool is_dirty) { this->dirty = is_dirty; }

/**
 * @brief Get the block's dirty time
 * @return a date
 */
double Block::getDirtyTime() const { return this->dirty_time; }

/**
 * @brief Set the block's dirty time
 * @param dirty_time: a date
 */
void Block::setDirtyTime(double dirty_time) { this->dirty_time = dirty_time; }
...
Block *Block::split(double remaining) {
  ...
  Block *new_blk =
      new Block(this->file_id, this->location, this->size - remaining,
                this->last_access, this->dirty, this->dirty_time);
  this->size = remaining;
  return new_blk;
}
