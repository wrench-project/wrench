...
MemoryManager::MemoryManager(simgrid::s4u::Disk *memory, double dirty_ratio,
                             int interval, int expired_time,
                             std::string hostname)
    : Service(hostname, "page_cache_manager_" + hostname), memory(memory),
      dirty_ratio(dirty_ratio), interval(interval), expired_time(expired_time) {
  // Get RAM disk size
 ...
 * @param expired_time: the expired time of dirty data to be flushed in
 * milliseconds
 * @param hostname: name of the host on which periodical flushing starts
 * @return
 */
std::shared_ptr<MemoryManager> MemoryManager::initAndStart(
    wrench::Simulation *simulation, simgrid::s4u::Disk *memory,
    double dirty_ratio, int interval, int expired_time, std::string hostname) {
  auto memory_manager = simulation->add(
 ...
 */
void MemoryManager::kill() { this->killActor(); }

/**
 * @brief Get the memory disk
 * @return the disk that acts as the memory cache
 */
simgrid::s4u::Disk *MemoryManager::getMemory() const { return memory; }
 ...
 */
double MemoryManager::getDirtyRatio() const { return dirty_ratio; }
 ...
 */
double MemoryManager::getFreeMemory() const { return free; }
 ...
 * @return a number of bytes
 */
double MemoryManager::getTotalCachedAmount() const { return cached; }

/**
 * @brief Get number of dirty bytes
 * @return a number of bytes
 */
double MemoryManager::getDirty() const { return dirty; }
 ...
 * @return a number of bytes
 */
double MemoryManager::getTotalMemory() { return this->total; }
...
double MemoryManager::flushLruList(std::vector<Block *> &list, double amount,
  ...
  for (const auto &blk : list) {
    if (!excluded_filename.empty() &&
        blk->getFileId().compare(excluded_filename) == 0) {
      continue;
    }
    ...
    if (blk->isDirty()) {
      if (flushed + blk->getSize() <= amount) {
        ...
        inactive_list.push_back(new Block(fn, blk->getLocation(), blk_flushed,
                                          blk->getLastAccess(), false,
                                          blk->getDirtyTime()));
      } else {
        // done flushing
        break;
...
double MemoryManager::flush(double amount,
  ...
  double flushed_inactive =
      flushLruList(inactive_list, amount, excluded_filename);

  double flushed_active = 0;
  if (flushed_inactive < amount) {
    flushed_active =
        flushLruList(active_list, amount - flushed_inactive, excluded_filename);
  }
 ...
 * Expired dirty data is the dirty data not accessed in a period longer than
 * expired_time
...
double MemoryManager::flushExpiredData(std::vector<Block *> &list) {
  ...
  while (true) {
    ...
    simgrid::s4u::Disk *disk =
        getDisk(block_to_deal_with->getLocation()->getPath(), this->hostname);
 ...
 */
simgrid::s4u::IoPtr
MemoryManager::readToCache(std::string filename,
  ...
  inactive_list.push_back(new Block(filename, location, amount,
                                    S4U_Simulation::getClock(), false, 0));
  balanceLruLists();
...
void MemoryManager::readChunkFromCache(std::string filename, double amount) {
    ...
    if (blk->getFileId().compare(filename) == 0) {
      ...
      if (read + blk->getSize() <= amount) {
        ...
        if (blk->isDirty()) {
          ...
          dirty_reaccessed += blk->getSize();
          std::rotate(active_list.begin() + i, active_list.begin() + i + 1,
                      active_list.end());
        } else {
  ...
  if (clean_reaccessed > 0) {
    Block *new_blk = new Block(filename, location, clean_reaccessed,
                               S4U_Simulation::getClock(), false, 0);
    active_list.push_back(new_blk);
  }
  //        if (dirty_reaccessed > 0) {
  //            Block *new_blk = new Block(filename, mnt_pt, dirty_reaccessed,
  //            S4U_Simulation::getClock(), true);
  //            active_list.push_back(new_blk);
  //        }
...
void MemoryManager::addToCache(std::string filename,
                               std::shared_ptr<FileLocation> location,
                               double amount, bool is_dirty) {
  auto *bl = new Block(filename, location, amount, S4U_Simulation::getClock(),
                       is_dirty, S4U_Simulation::getClock());
 ...
 * If the amount of data in the active list is more than doubled of the inactive
 * list, move blocks from the active list to the inactive list to make their
 * sizes equal.
 */
    ...
    for (unsigned int i = 0; i < active_list.size(); i++) {
      ...
      if (to_move_amt - (moved_amt + blk->getSize()) >= 0) {
        ...
        Block *new_blk = new Block(
            fn, blk->getLocation(), to_move_amt - moved_amt,
            blk->getLastAccess(), blk->isDirty(), blk->getDirtyTime());
...
simgrid::s4u::Disk *MemoryManager::getDisk(std::string mountpoint,
  ...
  if (not host) {
    throw std::invalid_argument("MemoryManager::getDisk: unknown host " +
                                hostname);
  }
...
void MemoryManager::export_log(std::string filename) {
  ...
  for (unsigned int i = 0; i < this->time_log.size(); i++) {
    fprintf(log_file, "%lf, %lf, %lf, %lf, %lf\n", this->time_log.at(i) - start,
            total / 1000000.0, this->dirty_log.at(i) / 1000000.0,
