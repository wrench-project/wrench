...
WRENCH_LOG_CATEGORY(wrench_core_network_proximity_service,
                    "Log category for Network Proximity Service");
 ...
 */
void NetworkProximityService::cleanup(bool has_returned_from_main,
                                      int return_value) {
 ...
 */
NetworkProximityService::NetworkProximityService(
    std::string hostname, std::vector<std::string> hosts_in_network,
  ...
  if (this->hosts_in_network.size() < 2) {
    throw std::invalid_argument(
        "NetworkProximityService requires at least 2 hosts to run");
  }
...
/**
 * @brief Look up the current (x,y) coordinates of a given host (only for a
 * Vivaldi network service type)
 * @param requested_host: the host whose coordinates are being requested
 * @return A pair:
 *      - a (x,y) coordinate pair
 *      - a timestamp (the oldest timestamp of measurements used to compute the
 * coordinate)
 *
...
NetworkProximityService::getHostCoordinate(std::string requested_host) {
  ...
  if (boost::iequals(
          ...
          "alltoall")) {
    throw std::runtime_error(
        "NetworkProximityService::getCoordinate() cannot be called with "
        "NETWORK_PROXIMITY_SERVICE_TYPE of ALLTOALL");
  }

  WRENCH_INFO("Obtaining current coordinates of network daemon on host %s",
              requested_host.c_str());

  auto answer_commport = S4U_Daemon::getRunningActorRecvCommPort();

  this->commport->putMessage(new CoordinateLookupRequestMessage(
 ...
 * @return A pair:
 *           - The proximity value between the pair of hosts (or DBL_MAX if
 * none)
 *           - The timestamp of the oldest measurement use to compute the
 * proximity value (or -1.0 if none)
 *
 * @throw ExecutionException
 * @throw std::runtime_error
 */
std::pair<double, double> NetworkProximityService::getHostPairDistance(
  ...
  if (boost::iequals(network_service_type, "alltoall")) {
    WRENCH_INFO("Obtaining the proximity value between %s and %s",
                hosts.first.c_str(), hosts.second.c_str());
  } else {
    WRENCH_INFO("Obtaining the approximate distance between %s and %s",
                hosts.first.c_str(), hosts.second.c_str());
  }

  auto answer_commport = S4U_Daemon::getRunningActorRecvCommPort();

  this->commport->putMessage(new NetworkProximityLookupRequestMessage(
  ...
  auto msg = answer_commport->getMessage<NetworkProximityLookupAnswerMessage>(
      this->network_timeout, "NetworkProximityService::query(): Received an");
 ...
 */
void NetworkProximityService::addEntryToDatabase(
    const std::pair<std::string, std::string> &pair_hosts,
    double proximity_value) {
  //        WRENCH_INFO("Received new measurement: %s-%s prox=%lf",
  //        pair_hosts.first.c_str(), pair_hosts.second.c_str(),
  //        proximity_value);

  if (this->entries.find(pair_hosts) == this->entries.end()) {
    std::pair<std::pair<std::string, std::string>, std::pair<double, double>>
        value = std::make_pair(
            pair_hosts, std::make_pair(proximity_value,
                                       Simulation::getCurrentSimulatedDate()));
    this->entries.insert(value);
  } else {
    this->entries[pair_hosts] =
...
int NetworkProximityService::main() {
  ...
  WRENCH_INFO("Network Proximity Service starting on host %s!",
  ...
  for (const auto &h : this->hosts_in_network) {
    ...
    auto np_receiver_daemon = std::make_shared<NetworkProximityReceiverDaemon>(
        this->simulation, h, this->messagepayload_list);
    this->network_receiver_daemons.push_back(np_receiver_daemon);

    // if this network service type is 'vivaldi', set up the coordinate lookup
    // table
    if (boost::iequals(
            this->getPropertyValueAsString(NetworkProximityServiceProperty::
                                               NETWORK_PROXIMITY_SERVICE_TYPE),
            "vivaldi")) {
      this->coordinate_lookup_table.insert(std::make_pair(
          h, std::make_pair((0.0), Simulation::getCurrentSimulatedDate())));
    }
  }

  // Start all network daemons
  for (auto &network_receiver_daemon : this->network_receiver_daemons) {
    network_receiver_daemon->start(network_receiver_daemon, true,
                                   true); // Daemonized, AUTO RESTART
  }
  for (auto &network_sender_daemon : this->network_sender_daemons) {
    network_sender_daemon->start(network_sender_daemon, true,
                                 true); // Daemonized, AUTO RESTART
  }
  ...
  WRENCH_INFO("Network Proximity Service on host %s cleanly terminating!",
              S4U_Simulation::getHostName().c_str());
  return 0;
}
  ...
  if (auto msg = std::dynamic_pointer_cast<ServiceStopDaemonMessage>(message)) {
    ...
    try {
      ...
      return false;
                 ...
                 NetworkProximityLookupRequestMessage>(message)) {
    ...
    if (msg->hosts.first == msg->hosts.second) {
      ...
      if (boost::iequals(network_service_type, "vivaldi")) {
        ...
        if (host1 != this->coordinate_lookup_table.end() &&
            host2 != this->coordinate_lookup_table.end()) {
          proximity_value =
              std::sqrt(norm(host2->second.first - host1->second.first));
    ...
    // Overhead
    S4U_Simulation::sleep(this->getPropertyValueAsTimeInSecond(
        NetworkProximityServiceProperty::LOOKUP_OVERHEAD));

    msg->answer_commport->dputMessage(new NetworkProximityLookupAnswerMessage(
    ...
    return true;
                 ...
                 NetworkProximityComputeAnswerMessage>(message)) {
    ...
    return true;
                 ...
                 std::dynamic_pointer_cast<NextContactDaemonRequestMessage>(
                     message)) {
    auto chosen_peer =
        NetworkProximityService::getCommunicationPeer(msg->daemon);

    msg->daemon->commport->dputMessage(new NextContactDaemonAnswerMessage(
        chosen_peer->getHostname(), chosen_peer, chosen_peer->commport,
        this->getMessagePayloadValue(
            NetworkProximityServiceMessagePayload::
    ...
    return true;
                 ...
                 std::dynamic_pointer_cast<CoordinateLookupRequestMessage>(
                     message)) {
    std::string requested_host = msg->requested_host;
    auto const coordinate_itr =
        this->coordinate_lookup_table.find(requested_host);
    ...
    if (coordinate_itr != this->coordinate_lookup_table.cend()) {
      msg_to_send_back = new CoordinateLookupAnswerMessage(
          requested_host, true,
          std::make_pair(coordinate_itr->second.first.real(),
      ...
      msg_to_send_back = new CoordinateLookupAnswerMessage(
          requested_host, false, std::make_pair(0, 0), 0,
          this->getMessagePayloadValue(
              NetworkProximityServiceMessagePayload::
                  NETWORK_DB_LOOKUP_ANSWER_MESSAGE_PAYLOAD));
    }
    ...
    throw std::runtime_error(
        "NetworkProximityService::processNextMessage(): Unexpected [" +
...
/**
 * @brief Internal method to choose a communication peer for the requesting
 * network proximity daemon
 * @param sender_daemon: the network daemon requesting a peer to communicate
 * with next
 * @return a shared_ptr to the network daemon that is the selected communication
 * peer
 */
std::shared_ptr<NetworkProximityReceiverDaemon>
NetworkProximityService::getCommunicationPeer(
    const std::shared_ptr<NetworkProximitySenderDaemon> sender_daemon) {
  // coverage will be (0 < coverage <= 1.0) if this is a 'vivaldi' network
  // service else if it is an 'alltoall' network service, coverage is set at 1.0
  ...
  // if the network_service type is 'alltoall', sender_daemon selects from a
  // pool of all other network daemons if the network_service type is 'vivaldi',
  // sender_daemon selects from a subset of the max_pool_size
  ...
  for (unsigned long index = 0; index < this->network_sender_daemons.size();
       ++index) {
    if (this->network_sender_daemons[index]->commport !=
        sender_daemon->commport) {
...
/**
 * @brief Internal method to compute and update coordinates based on Vivaldi
 * algorithm
 * @param proximity_value: one way elapsed time to send a message from the
 * sender to the receiving peer
 ...
 */
void NetworkProximityService::vivaldiUpdate(double proximity_value,
  ...
  auto search = this->coordinate_lookup_table.find(sender_hostname);
  ...
  double estimated_distance =
      std::sqrt(norm(peer_coordinates - sender_coordinates));
  ...
  if (estimated_distance == 0.0) {
    // TODO: maybe use time(0) from <ctime> as seed: This is just done once! Or
    // just use master_rng?
    static std::default_random_engine direction_rng(0);
    static std::uniform_real_distribution<double> dir_dist(-0.00000000001,
                                                           0.00000000001);
  ...
  // scaled_direction will get start to approach 0 when error_direction gets
  // small
  scaled_direction = error_direction * error;

  // compute the updated sender coordinates
  auto updated_sender_coordinates =
      sender_coordinates + (scaled_direction * sensitivity);
  ...
  WRENCH_DEBUG("Vivaldi updated coordinates of %s from (%f,%f) to (%f,%f)",
               sender_hostname.c_str(), sender_coordinates.real(),
               sender_coordinates.imag(), updated_sender_coordinates.real(),
...
void NetworkProximityService::validateProperties() {
  ...
  if (!boost::iequals(network_service_type, "alltoall") &&
      !boost::iequals(network_service_type, "vivaldi")) {
    throw std::invalid_argument(error_prefix +
                                "Invalid network proximity service type '" +
  ...
  if (boost::iequals(network_service_type, "alltoall")) {
    if (coverage != 1.0) {
      throw std::invalid_argument(
          error_prefix +
          "Invalid NETWORK_DAEMON_COMMUNICATION_COVERAGE value " +
          this->getPropertyValueAsString(
              NetworkProximityServiceProperty::
  ...
  if (this->getPropertyValueAsTimeInSecond(
          NetworkProximityServiceProperty::LOOKUP_OVERHEAD) < 0) {
    throw std::invalid_argument(
        error_prefix + "Invalid LOOKUP_OVERHEAD value " +
        this->getPropertyValueAsString(
            NetworkProximityServiceProperty::LOOKUP_OVERHEAD));
  }
  ...
  if (this->getPropertyValueAsDouble(
      ...
      0) {
    throw std::invalid_argument(
        error_prefix + "Invalid NETWORK_PROXIMITY_MESSAGE_SIZE value " +
        this->getPropertyValueAsString(
            NetworkProximityServiceProperty::NETWORK_PROXIMITY_MESSAGE_SIZE));
  }
  ...
  if (this->getPropertyValueAsTimeInSecond(
          NetworkProximityServiceProperty::
              NETWORK_PROXIMITY_MEASUREMENT_PERIOD) <= 0) {
    throw std::invalid_argument(
        error_prefix + "Invalid NETWORK_PROXIMITY_MEASUREMENT_PERIOD value " +
        this->getPropertyValueAsString(
            NetworkProximityServiceProperty::
                NETWORK_PROXIMITY_MEASUREMENT_PERIOD));
  }
  ...
  if (this->getPropertyValueAsDouble(
          NetworkProximityServiceProperty::
              NETWORK_PROXIMITY_MEASUREMENT_PERIOD_MAX_NOISE) < 0) {
    throw std::invalid_argument(
        error_prefix +
        "Invalid NETWORK_PROXIMITY_MEASUREMENT_PERIOD_MAX_NOISE value " +
        this->getPropertyValueAsString(
            NetworkProximityServiceProperty::
                NETWORK_PROXIMITY_MEASUREMENT_PERIOD_MAX_NOISE));
  }
...
/**
 * @brief Gets the list of hosts monitored by this service (does not involve
 * simulated network communications with the service)
