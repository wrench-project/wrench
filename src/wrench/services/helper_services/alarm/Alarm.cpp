...
WRENCH_LOG_CATEGORY(wrench_core_alarm_service,
                    "Log category for Alarm Service");
 ...
 * @brief Constructor
 *
 * @param date: the date at which the message should be sent. If date is in the
 * past message will be sent immediately.
...
Alarm::Alarm(double date, std::string hostname, S4U_CommPort *reply_commport,
    ...
    : Service(hostname, "alarm_service_" + suffix) {
  this->date = date;
...
int Alarm::main() {
  ...
  WRENCH_INFO("Alarm Service Sending a message to %s",
              this->reply_commport->get_cname());
  try {
 ...
 * @param date: the date at which the message should be sent (if date is in the
 * past then the message will be sent immediately)
 * @param hostname: the name of the host on which to start the alarm service
 * @param reply_commport: the commport to which the alarm service will send a
 * message
 * @param msg: the message to send
 * @param suffix: a (possibly empty) suffix to append to the daemon name (useful
 * in debug output)
...
Alarm::createAndStartAlarm(Simulation *simulation, double date,
                           std::string hostname, S4U_CommPort *reply_commport,
                           SimulationMessage *msg, std::string suffix) {
 ...
 */
void Alarm::kill() { this->killActor(); }
