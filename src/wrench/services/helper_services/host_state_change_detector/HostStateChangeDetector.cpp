...
WRENCH_LOG_CATEGORY(wrench_core_host_state_change_detector,
                    "Log category for HostStateChangeDetector");
 ...
 */
void wrench::HostStateChangeDetector::cleanup(bool has_returned_from_main,
                                              int return_value) {
  // Unregister the callback!
  simgrid::s4u::Host::on_onoff.disconnect(this->on_state_change_call_back_id);
  simgrid::s4u::Host::on_speed_change.disconnect(
      this->on_speed_change_call_back_id);
}
 ...
 * @param notify_when_turned_on: whether to send a notifications when hosts turn
 * on
 * @param notify_when_turned_off: whether to send a notifications when hosts
 * turn off
 * @param notify_when_speed_change: whether to send a notification when hosts
 * change speed
 * @param creator: the service that created this service (when its creator dies,
 * so does this service)
 * @param commport_to_notify: the commport to notify
 * @param property_list: a property list
 *
 */
wrench::HostStateChangeDetector::HostStateChangeDetector(
  ...
  // regarding Hosts
  this->on_state_change_call_back_id =
      simgrid::s4u::Host::on_onoff.connect([this](simgrid::s4u::Host const &h) {
        this->hostStateChangeCallback(&h);
      });

  // Connect my member method to the on_speed_change signal from SimGrid
  // regarding Hosts
  this->on_speed_change_call_back_id =
      simgrid::s4u::Host::on_speed_change.connect(
          [this](simgrid::s4u::Host const &h) {
            this->hostSpeedChangeCallback(&h);
          });
}

void wrench::HostStateChangeDetector::hostStateChangeCallback(
    const simgrid::s4u::Host *host) {
  if (std::find(this->hosts_to_monitor.begin(), this->hosts_to_monitor.end(),
                host) != this->hosts_to_monitor.end()) {
    this->hosts_that_have_recently_changed_state.emplace_back(host->get_name(),
                                                              host->is_on());
    this->s4u_actor->resume();
  }
}

void wrench::HostStateChangeDetector::hostSpeedChangeCallback(
    const simgrid::s4u::Host *host) {
  if (std::find(this->hosts_to_monitor.begin(), this->hosts_to_monitor.end(),
                host) != this->hosts_to_monitor.end()) {
    this->hosts_that_have_recently_changed_speed.emplace_back(
        host->get_name(), host->get_speed());
    this->s4u_actor->resume();
  }
}
...
int wrench::HostStateChangeDetector::main() {
  ...
  while (true) {
    ...
    while (not this->hosts_that_have_recently_changed_state.empty()) {
      ...
      this->hosts_that_have_recently_changed_state.erase(
          this->hosts_that_have_recently_changed_state.begin());
      ...
      WRENCH_INFO("Notifying commport '%s' that host '%s' has changed state",
                  this->commport_to_notify->get_cname(), hostname.c_str());
    ...
    while (not this->hosts_that_have_recently_changed_speed.empty()) {
      ...
      this->hosts_that_have_recently_changed_speed.erase(
          this->hosts_that_have_recently_changed_speed.begin());
      ...
      WRENCH_INFO("Notifying commport '%s' that host '%s' has changed speed",
                  this->commport_to_notify->get_cname(), hostname.c_str());
      this->commport_to_notify->dputMessage(msg);
    }
  }
 ...
 */
void wrench::HostStateChangeDetector::kill() { this->killActor(); }
