...
WRENCH_LOG_CATEGORY(wrench_core_action_scheduler,
                    "Log category for Action Scheduler");
 ...
 */
void ActionExecutionService::cleanup(bool has_returned_from_main,
                                     int return_value) {
...
/**
 * @brief Helper static method to parse resource specifications to the
 * <cores,ram> format
 * @param spec: specification string
 * @return a <host, core> tuple
 * @throw std::invalid_argument
 */
static std::tuple<simgrid::s4u::Host *, unsigned long>
parseResourceSpec(const std::string &spec) {
  ...
  case 1: // "num_cores" or "hostname"
  {
    ...
    if (sscanf(tokens[0].c_str(), "%lu", &num_threads) != 1) {
      return std::make_tuple(S4U_Simulation::get_host_or_vm_by_name(tokens[0]),
                             0);
    } else {
 ...
 */
void ActionExecutionService::submitAction(
    const std::shared_ptr<Action> &action) {
  assertServiceIsUp();

  if (action->getState() != Action::State::READY) {
    throw std::runtime_error(
        "Can only submit a ready action to the ActionExecutionService");
  }
  ...
  // At this point, there may still be insufficient resources to run the action,
  // but that will be handled later (and some ExecutionError with a "not enough
  // resources" FailureCause may be generated).

  auto answer_commport = S4U_Daemon::getRunningActorRecvCommPort();

  //  send a "run a standard job" message to the daemon's commport
  this->commport->putMessage(
      new ActionExecutionServiceSubmitActionRequestMessage(answer_commport,
                                                           action, 0.0));

  // Get the answer
  auto msg = answer_commport
                 ->getMessage<ActionExecutionServiceSubmitActionAnswerMessage>(
 ...
 * @param compute_resources: a map of <num_cores, memory> tuples, indexed by
 * host, which represents the compute resources available to this service.
 *          - use num_cores = ComputeService::ALL_CORES to use all cores
 * available on the host
 *          - use memory_manager_service = ComputeService::ALL_RAM to use all
 * RAM available on the host
 * @param parent_service: the parent compute service (nullptr if not known at
 * this time)
...
ActionExecutionService::ActionExecutionService(
    ...
    : Service(hostname, "action_execution_service") {
  ...
  // Check that there is at least one core per host and that hosts have enough
  // cores
  if (compute_resources.empty()) {
    throw std::invalid_argument(
        "ActionExecutionService::ActionExecutionService(): the resource list "
        "is empty");
  }
  for (auto host : compute_resources) {
    ...
    try {
      ...
      throw std::invalid_argument(
          "ActionExecutionService::ActionExecutionService(): Host '" +
          host.first->get_name() + "' does not exist");
    }
    ...
    if (requested_cores == 0) {
      throw std::invalid_argument(
          "ActionExecutionService::ActionExecutionService(): at least 1 core "
          "should be requested");
    }
    if (requested_cores > available_cores) {
      throw std::invalid_argument(
          "ActionExecutionService::ActionExecutionService(): " +
          host.first->get_name() + "only has " +
    ...
    if (requested_ram < 0) {
      throw std::invalid_argument(
          "ActionExecutionService::ActionExecutionService(): requested RAM "
    ...
    if (requested_ram > available_ram) {
      throw std::invalid_argument(
          "ActionExecutionService::ActionExecutionService(): host " +
          host.first->get_name() + "only has " + std::to_string(available_ram) +
          " bytes of RAM but " + std::to_string(requested_ram) +
          " are requested");
    }

    this->compute_resources[host.first] =
        std::make_tuple(requested_cores, requested_ram);
  }

  // Compute the total number of cores and set initial ram availabilities
  for (auto const &host : this->compute_resources) {
    this->ram_availabilities[host.first] =
        std::get<1>(this->compute_resources[host.first]);
...
int ActionExecutionService::main() {
  if (this->parent_service == nullptr) {
    throw std::runtime_error(
        "ActionExecutionService::main(): parent service not set - please call "
        "setParentService before starting this service");
  }
  ...
  if (Simulation::isEnergySimulationEnabled() or
    ...
    this->host_state_change_monitor =
        std::shared_ptr<HostStateChangeDetector>(new HostStateChangeDetector(
    ...
    this->host_state_change_monitor->start(
        this->host_state_change_monitor, true,
  ...
  if (Simulation::isEnergySimulationEnabled() or
    ...
    this->host_state_change_monitor =
        nullptr; // Which will release the pointer to this service!
  }

  WRENCH_INFO("ActionExecutionService on host %s terminating cleanly!",
 ...
 * @param required_host: the required host per service-specific arguments
 * (nullptr means: choose one)
 * @param required_num_cores: the required number of cores per service-specific
 * arguments (0 means: choose a number)
 * @param hosts_to_avoid: a list of hosts to not even consider
 * @return an allocation
 */
std::tuple<simgrid::s4u::Host *, unsigned long>
ActionExecutionService::pickAllocation(
    const std::shared_ptr<Action> &action, simgrid::s4u::Host *required_host,
  ...
  for (auto const &r : this->compute_resources) {
    ...
    unsigned long available_cores =
        std::get<0>(r.second) - this->running_thread_counts[r.first];
    ...
    if ((action->getMinRAMFootprint() > 0) and
        (hosts_to_avoid.find(r.first) != hosts_to_avoid.end())) {
      continue;
    }
    if (this->ram_availabilities[r.first] < action->getMinRAMFootprint()) {
      if (new_host_to_avoid == nullptr) {
        ...
        if (this->ram_availabilities[r.first] >
            new_host_to_avoid_ram_capacity) {
          // Make sure we "Avoid" the host with the most RAM (as it might become
          // usable sooner)
          new_host_to_avoid = r.first;
          new_host_to_avoid_ram_capacity = this->ram_availabilities[r.first];
        }
  ...
  for (auto const &h : possible_hosts) {
    ...
    if (required_num_cores == 0) {
      used_num_cores = std::min(
          num_cores, action->getMaxNumCores()); // as many cores as possible
    } else {
      used_num_cores = required_num_cores;
    }
    // A totally heuristic load estimate
    double load = ((((double)(num_running_threads + used_num_cores)) /
                    (double)num_cores)) /
...
void ActionExecutionService::dispatchReadyActions() {
  ...
  for (auto const &action : this->ready_actions) {
    ...
    std::tuple<simgrid::s4u::Host *, unsigned long> allocation =
        pickAllocation(action, std::get<0>(this->action_run_specs[action]),
    ...
    //            WRENCH_INFO("ALLOC %s: %s %ld %lf", action->getName().c_str(),
    //            target_host.c_str(), target_num_cores, required_ram);

    /** Dispatch it **/
    // Create an action executor on the target host
    auto action_executor = std::shared_ptr<ActionExecutor>(new ActionExecutor(
        target_host->get_name(), target_num_cores, required_ram,
        ...
        this->commport, action, this->getSharedPtr<ActionExecutionService>()));

    action_executor->setSimulation(this->simulation);
    try {
      action_executor->start(action_executor, true,
                             false); // Daemonized, no auto-restart
    } catch (ExecutionException &e) {
      // This is an error on the target host!!
      throw std::runtime_error(
          "ActionSchedule::dispatchReadyActions(): got a host error on the "
          "target host - this shouldn't happen");
    }

    // Start a failure detector for this action executor (which will send me a
    // message in case the action executor has died)
    auto failure_detector = std::shared_ptr<ServiceTerminationDetector>(
        new ServiceTerminationDetector(this->hostname, action_executor,
                                       this->commport, true, false));
  ...
  // Remove the Actions from the ready queue (this is inefficient, better data
  // structs would help)
  while (not dispatched_actions.empty()) {
    ...
    for (auto const &ready_action : this->ready_actions) {
      for (auto it = this->ready_actions.begin();
           it != this->ready_actions.end(); it++) {
  ...
  if (std::dynamic_pointer_cast<HostHasTurnedOnMessage>(message)) {
    ...
    if (this->getPropertyValueAsString(
            ActionExecutionServiceProperty::
                TERMINATE_WHENEVER_ALL_RESOURCES_ARE_DOWN) == "false") {
      return true;

    } else {
      // If not all resources are down or somebody is still running, nevermind
      // we may have gotten this "Host down" message before the "This Action
      // Executor has crashed" message. So we don't want to just quit right now.
      // We'll
      //  get an Action Executor Crash message, at which point we'll check
      //  whether all hosts are down again
      if (not this->areAllComputeResourcesDownWithNoActionExecutorRunning()) {
        WRENCH_INFO("Not terminating as there are still non-down resources "
                    "and/or WUE executors that "
                    "haven't reported back yet");
        return true;
      } else {
        this->terminate(false, ComputeService::TerminationCause::
                                   TERMINATION_COMPUTE_SERVICE_TERMINATED);
        this->exit_code = 1; // Exit code to signify that this is, in essence a
                             // crash (in case somebody cares)
        return false;
      }
    }
    ...
    this->terminate(msg->send_failure_notifications,
                    (ComputeService::TerminationCause)(msg->termination_cause));

    // This is Synchronous
    try {
      msg->ack_commport->putMessage(new ServiceDaemonStoppedMessage(0.0));
    } catch (ExecutionException &e) {
      return false;
    }
    return false;
    ...
    return true;
                 ...
                 message)) {
    processActionTerminationRequest(msg->action, msg->reply_commport,
                                    msg->termination_cause);
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<ActionExecutorDoneMessage>(
                 message)) {
    if (msg->action_executor->getAction()->getState() ==
    ...
    return true;
                 ...
                 std::dynamic_pointer_cast<ServiceHasCrashedMessage>(message)) {
    auto service = msg->service;
    auto action_executor = std::dynamic_pointer_cast<ActionExecutor>(service);
    if (not action_executor) {
      throw std::runtime_error(
          "Internal Error: Received a FailureDetectorServiceHasFailedMessage "
          "message, but that service is not "
          "an ActionExecutor!");
    }
    processActionExecutorCrash(action_executor);
    // If all hosts being off should not cause the service to terminate, then
    // nevermind
    if (this->getPropertyValueAsString(
            ActionExecutionServiceProperty::
                TERMINATE_WHENEVER_ALL_RESOURCES_ARE_DOWN) == "false") {
      ...
      // If not all resources are down or somebody is still running, nevermind
      // we may have gotten the "Host down" message before the "ActionExecutor
      // Has Crashed" message. So we don't want to just quit right now. We'll
      //  get an "ActionExecutor Has Crashed" message, at which point we'll
      //  check whether all hosts are down again
      if (not this->areAllComputeResourcesDownWithNoActionExecutorRunning()) {
        return true;
      }
...
void ActionExecutionService::killAction(
  ...
  bool killed_due_to_job_cancellation =
      (std::dynamic_pointer_cast<JobKilled>(cause) != nullptr);

  // If action is ready, remove it from ready list
  if (std::find(this->ready_actions.begin(), this->ready_actions.end(),
                action) != this->ready_actions.end()) {
    this->ready_actions.erase(std::find(this->ready_actions.begin(),
                                        this->ready_actions.end(), action));
  }
  ...
  if (this->action_executors.find(action) != this->action_executors.end()) {
    ...
    this->ram_availabilities[executor->getHost()] +=
        executor->getMemoryAllocated();
    this->running_thread_counts[executor->getHost()] -=
        executor->getNumCoresAllocated();
...
void ActionExecutionService::terminate(
  ...
  if (send_failure_notifications) {
    throw std::runtime_error("ActionExecutionService::terminate(): NEED TO "
 ...
 * @throw std::runtime_error
 */
void ActionExecutionService::terminateAction(
  ...
  auto msg =
      answer_commport
          ->getMessage<ActionExecutionServiceTerminateActionAnswerMessage>(
              this->network_timeout,
...
void ActionExecutionService::processActionExecutorCompletion(
  ...
  this->running_thread_counts[executor_host] -=
      executor->getNumCoresAllocated();
  ...
  this->parent_service->commport->dputMessage(
      new ActionExecutionServiceActionDoneMessage(action, 0.0));
}
 ...
 * @param executor: the action executor
 */
void ActionExecutionService::processActionExecutorFailure(
    const std::shared_ptr<ActionExecutor> &executor) {
  auto action = executor->getAction();
  ...
  WRENCH_INFO("Sending action failure notification to '%s'",
              parent_service->commport->get_cname());
  // NOTE: This is synchronous so that the process doesn't fall off the end
  try {
 ...
 */
void ActionExecutionService::processActionTerminationRequest(
  ...
  if (this->all_actions.find(action) == this->all_actions.end()) {
    ...
    std::string error_message =
        "Action cannot be terminated because it is not running";
    auto answer_message =
        new ActionExecutionServiceTerminateActionAnswerMessage(
            false,
            std::shared_ptr<FailureCause>(new NotAllowed(
                this->getSharedPtr<ActionExecutionService>(), error_message)),
            0.0);
  ...
  // reply
  auto answer_message =
      new ActionExecutionServiceTerminateActionAnswerMessage(true, nullptr, 0);
  answer_commport->dputMessage(answer_message);
}

/**
 * @brief Helper function that determines whether there is at least one host
 * with some number of cores (or more) and some ram capacity (or more)
 * @param num_cores: number of cores
 * @param ram: ram capacity
 * @return true is a host was found
 */
bool ActionExecutionService::isThereAtLeastOneHostWithResources(
    unsigned long num_cores, double ram) {
  for (auto const &r : this->compute_resources) {
    if ((std::get<0>(r.second) >= num_cores) and
        (std::get<1>(r.second) >= ram)) {
      return true;
    }
  }
  return false;
...
/**
 * @brief Helper method that determines whether a submitted action (with
 * service-specific arguments) can run given available  resources
 *
 * @param action: the action
 * @return true if the action can run
 */
bool ActionExecutionService::actionCanRun(
    const std::shared_ptr<Action> &action) {
  auto service_specific_arguments =
      action->getJob()->getServiceSpecificArguments();

  // No service-specific argument
  if ((service_specific_arguments.find(action->getName()) ==
       service_specific_arguments.end()) or
      (service_specific_arguments[action->getName()].empty())) {
    if (not isThereAtLeastOneHostWithResources(action->getMinNumCores(),
                                               action->getMinRAMFootprint())) {
      return false;
    }
  }

  // Parse the service-specific argument
  std::tuple<simgrid::s4u::Host *, unsigned long> parsed_spec =
      parseResourceSpec(service_specific_arguments[action->getName()]);
  ...
  if (desired_host == nullptr) {
    ...
    if (not isThereAtLeastOneHostWithResources(desired_num_cores,
                                               action->getMinRAMFootprint())) {
      return false;
    } else {
      return true;
  ...
  unsigned long num_cores_on_desired_host =
      std::get<0>(this->compute_resources[desired_host]);
  double ram_on_desired_host =
      std::get<1>(this->compute_resources[desired_host]);
...
void ActionExecutionService::processSubmitAction(
  ...
  // Construct the action run spec (i.e., keep track of service-specific
...
/**
 * @brief Determine whether there is at least one host with (currently)
 * available resources
 ...
 */
bool ActionExecutionService::IsThereAtLeastOneHostWithAvailableResources(
    unsigned long num_cores, double ram) {
  ...
  if (enough_ram) {
    for (auto const &r : this->running_thread_counts) {
      ...
      if (running_threads > cores) {
        throw std::runtime_error("ActionExecutionService::"
                                 "IsThereAtLeastOneHostWithAvailableResources()"
                                 ": The number of running threads exceeds "
                                 "the number of cores. This should not have "
...
ActionExecutionService::getResourceInformation(const std::string &key) {
  // Build a dictionary
  ...
  if (key == "num_hosts") {
    ...
    num_hosts.insert(
        std::make_pair(this->getName(), this->compute_resources.size()));
    return num_hosts;
    ...
    for (auto r : this->compute_resources) {
      num_cores.insert(
          std::make_pair(r.first->get_name(), (double)(std::get<0>(r.second))));
    }
    return num_cores;
    ...
    for (auto const &r : this->running_thread_counts) {
      unsigned long cores = std::get<0>(this->compute_resources[r.first]);
      unsigned long running_threads = r.second;
      num_idle_cores.insert(std::make_pair(
          r.first->get_name(),
          (double)(std::max<unsigned long>(cores - running_threads, 0))));
    }
    ...
    for (auto h : this->compute_resources) {
      flop_rates.insert(
          std::make_pair(h.first->get_name(), std::get<0>(h)->get_speed()));
    }
    return flop_rates;
    ...
    for (auto h : this->compute_resources) {
      ram_capacities.insert(std::make_pair(
          h.first->get_name(),
    ...
    for (auto const &r : this->ram_availabilities) {
      ram_availabilities_to_return.insert(
          std::make_pair(r.first->get_name(), r.second));
    }
    return ram_availabilities_to_return;

  } else {
    throw std::runtime_error(
        "ActionExecutionService::getResourceInformation(): unknown key");
  }
}
...
/**
 * @brief Process a crash of an ActionExecutor (although some work may has been
 * done, we'll just re-do the action from scratch)
 *
 * @param executor: the action executor that has crashed
 */
void ActionExecutionService::processActionExecutorCrash(
    const std::shared_ptr<ActionExecutor> &executor) {
  ...
  if (not this->getPropertyValueAsBoolean(
          ActionExecutionServiceProperty::
    ...
    WRENCH_INFO("Putting action %s back in the ready queue",
 ...
 * @return true or false
 */
bool ActionExecutionService::
    areAllComputeResourcesDownWithNoActionExecutorRunning() {
...
ActionExecutionService::getComputeResources() {
