...
WRENCH_LOG_CATEGORY(wrench_core_failure_detector,
                    "Log category for ServiceTerminationDetector");
 ...
 * @param notify_on_crash: whether to send a crash notification (in case of
 * non-clean termination)
 * @param notify_on_termination: whether to send a termination notification (in
 * case of clean termination)
 */
wrench::ServiceTerminationDetector::ServiceTerminationDetector(
    ...
    : Service(std::move(host_on_which_to_run),
              "service_termination_detector_for_" +
                  service_to_monitor->getName()) {
...
int wrench::ServiceTerminationDetector::main() {
  ...
  if (this->notify_on_crash and (not service_has_returned_from_main)) {
    // Failure detected!
    WRENCH_INFO("Detected crash of service %s (notifying commport %s)",
                this->service_to_monitor->getName().c_str(),
                this->commport_to_notify->get_cname());
    // No dput just before dying!
    this->commport_to_notify->putMessage(
        new ServiceHasCrashedMessage(this->service_to_monitor));
  }
  if (this->notify_on_termination and (service_has_returned_from_main)) {
    ...
    WRENCH_INFO("Detected termination of service %s (notifying commport %s)",
                ...
                this->commport_to_notify->get_cname());
    // No dput just before dying!
    this->commport_to_notify->dputMessage(new ServiceHasTerminatedMessage(
        this->service_to_monitor, return_value_from_main));
  }

  this->service_to_monitor =
      nullptr; // released, so that it can be freed in case ref-count = 0
  return 0;
}
