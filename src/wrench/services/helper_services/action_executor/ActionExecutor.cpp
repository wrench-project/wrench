...
WRENCH_LOG_CATEGORY(wrench_core_action_executor,
                    "Log category for  Action Executor");
 ...
 * @param simulate_computation_as_sleep: whether to simulate computation as
 * sleep
 * @param callback_commport: the callback commport to which a "action done" or
 * "action failed" message will be sent
...
ActionExecutor::ActionExecutor(
  ...
  if (action == nullptr) {
    throw std::invalid_argument(
        "ActionExecutor::ActionExecutor(): action cannot be nullptr");
  }
#endif
 ...
 * @return the action
 */
std::shared_ptr<Action> ActionExecutor::getAction() { return this->action; }
...
void ActionExecutor::cleanup(bool has_returned_from_main, int return_value) {
  WRENCH_DEBUG(
      "In on_exit.cleanup(): ActionExecutor: %s has_returned_from_main = %d "
      "(return_value = %d, killed_on_pupose = %d)",
  ...
  if (not has_returned_from_main and
      this->action->getState() == Action::State::STARTED) {
...
int ActionExecutor::main() {
  ...
  WRENCH_INFO("New Action Executor started to do action %s",
  ...
  auto msg_to_send_back =
      new ActionExecutorDoneMessage(this->getSharedPtr<ActionExecutor>());

  WRENCH_INFO(
      "Action executor for action %s terminating and action has %s",
      this->action->getName().c_str(),
      (this->action->getState() == Action::State::COMPLETED
           ? "succeeded"
           : ("failed (" + this->action->getFailureCause()->toString() + ")")
                 .c_str()));
  try {
    this->callback_commport->putMessage(msg_to_send_back);
  } catch (ExecutionException &e) {
    WRENCH_INFO(
        "Action executor can't report back due to network error.. oh well!");
  }
  return 0;
}
 ...
 *
 * @param job_termination: if the reason for being killed is that the job was
 * terminated by the submitter (as opposed to being terminated because the above
...
/**
 * @brief Get the action execution service that started this action executor (or
 * nullptr if stand-alone action executor)
 * @return an action execution service
 */
std::shared_ptr<ActionExecutionService>
ActionExecutor::getActionExecutionService() const {
