...
FileRegistryService::FileRegistryService(
    std::string hostname, WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list)
    : Service(std::move(hostname), "file_registry") {
...
FileRegistryService::lookupEntry(const std::shared_ptr<DataFile> &file) {
  if (file == nullptr) {
    throw std::invalid_argument(
        "FileRegistryService::lookupEntry(): Invalid argument");
  }
  ...
  this->commport->putMessage(new FileRegistryFileLookupRequestMessage(
      ...
      this->getMessagePayloadValue(FileRegistryServiceMessagePayload::
                                       FILE_LOOKUP_REQUEST_MESSAGE_PAYLOAD)));

  auto msg = answer_commport->getMessage<FileRegistryFileLookupAnswerMessage>(
      this->network_timeout, "FileRegistryService::lookupEntry(): Received in");
  return msg->locations;
}

/**
 * @brief Lookup entries for a file, including for each entry a network distance
 * from a reference host (as determined by a network proximity service)
 * @param file: the file to lookup
 * @param reference_host: reference host from which network proximity values are
 * to be measured
 * @param network_proximity_service: the network proximity service to use
 *
 * @return a map of <distance , file location> pairs
 */
std::map<double, std::shared_ptr<FileLocation>>
FileRegistryService::lookupEntry(
  ...
  if (file == nullptr) {
    throw std::invalid_argument("FileRegistryService::lookupEntryByProximity():"
  ...
  std::vector<std::string> monitored_hosts =
      network_proximity_service->getHostnameList();
  if (std::find(monitored_hosts.cbegin(), monitored_hosts.cend(),
                reference_host) == monitored_hosts.cend()) {
    throw std::invalid_argument("FileRegistryService::lookupEntryByProximity():"
                                " Invalid argument, host " +
                                reference_host + " does not exist");
  }
  ...
  this->commport->putMessage(
      new FileRegistryFileLookupByProximityRequestMessage(
          answer_commport, file, reference_host, network_proximity_service,
          this->getMessagePayloadValue(
              FileRegistryServiceMessagePayload::
                  FILE_LOOKUP_REQUEST_MESSAGE_PAYLOAD)));

  auto msg = answer_commport
                 ->getMessage<FileRegistryFileLookupByProximityAnswerMessage>(
 ...
 * @throw std::runtime_error
 */
void FileRegistryService::addEntry(
    const std::shared_ptr<FileLocation> &location) {
  if (location == nullptr) {
    throw std::invalid_argument(
        "FileRegistryService::addEntry(): Invalid nullptr argument");
  }
  assertServiceIsUp();

  auto answer_commport = S4U_Daemon::getRunningActorRecvCommPort();

  this->commport->putMessage(new FileRegistryAddEntryRequestMessage(
  ...
  auto msg = answer_commport->getMessage<FileRegistryAddEntryAnswerMessage>(
      this->network_timeout, "FileRegistryService::addEntry(): Received an");
}
...
void FileRegistryService::removeEntry(
  ...
  if (location == nullptr) {
    throw std::invalid_argument(
        " FileRegistryService::removeEntry(): Invalid nullptr argument");
  }
  ...
  this->commport->putMessage(new FileRegistryRemoveEntryRequestMessage(
      answer_commport, location,
      this->getMessagePayloadValue(FileRegistryServiceMessagePayload::
                                       REMOVE_ENTRY_REQUEST_MESSAGE_PAYLOAD)));

  auto msg = answer_commport->getMessage<FileRegistryRemoveEntryAnswerMessage>(
      this->network_timeout, "FileRegistryService::removeEntry(): Received an");
  if (!msg->success) {
    WRENCH_WARN("Attempted to remove non-existent (%s,%s) entry from file "
                "registry service (ignored)",
...
int FileRegistryService::main() {
  ...
  WRENCH_INFO("File Registry Service starting on host %s!",
  ...
  WRENCH_INFO("File Registry Service on host %s cleanly terminating!",
              S4U_Simulation::getHostName().c_str());
  return 0;
}
...
bool FileRegistryService::processNextMessage() {
  ...
  if (auto msg = std::dynamic_pointer_cast<ServiceStopDaemonMessage>(message)) {
    ...
    return false;
                 ...
                 FileRegistryFileLookupRequestMessage>(message)) {
    ...
    S4U_Simulation::compute(getPropertyValueAsDouble(
        FileRegistryServiceProperty::LOOKUP_COMPUTE_COST));

    msg->answer_commport->dputMessage(new FileRegistryFileLookupAnswerMessage(
        locations,
        this->getMessagePayloadValue(FileRegistryServiceMessagePayload::
    ...
    return true;
                 ...
                 FileRegistryFileLookupByProximityRequestMessage>(message)) {
    ...
    for (auto &location : all_file_locations) {
      proximity = std::get<0>(
          msg->network_proximity_service->getHostPairDistance(std::make_pair(
              reference_host, location->getStorageService()->getHostname())));
    ...
    msg->answer_commport->dputMessage(
        new FileRegistryFileLookupByProximityAnswerMessage(
            msg->file, msg->reference_host, map_to_return,
            this->getMessagePayloadValue(
                FileRegistryServiceMessagePayload::
                    FILE_LOOKUP_ANSWER_MESSAGE_PAYLOAD)));
    return true;
                 ...
                 std::dynamic_pointer_cast<FileRegistryAddEntryRequestMessage>(
                     message)) {
    addEntryToDatabase(msg->location);

    // Simulate an add overhead
    S4U_Simulation::compute(getPropertyValueAsDouble(
        FileRegistryServiceProperty::ADD_ENTRY_COMPUTE_COST));

    msg->answer_commport->dputMessage(new FileRegistryAddEntryAnswerMessage(
        this->getMessagePayloadValue(FileRegistryServiceMessagePayload::
                                         ADD_ENTRY_ANSWER_MESSAGE_PAYLOAD)));
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<
                 FileRegistryRemoveEntryRequestMessage>(message)) {
    bool success = removeEntryFromDatabase(msg->location);

    // Simulate a removal overhead
    S4U_Simulation::compute(getPropertyValueAsDouble(
        FileRegistryServiceProperty::REMOVE_ENTRY_COMPUTE_COST));

    msg->answer_commport->dputMessage(new FileRegistryRemoveEntryAnswerMessage(
        success,
        this->getMessagePayloadValue(FileRegistryServiceMessagePayload::
    ...
    throw std::runtime_error(
        "FileRegistryService::processNextMessage(): Unexpected [" +
 ...
 * @param location: a file location
 */
void FileRegistryService::addEntryToDatabase(
    const std::shared_ptr<FileLocation> &location) {
  auto file = location->getFile();
  WRENCH_INFO("Adding file (%s) at (%s) to the file registry",
              file->getID().c_str(),
 ...
 */
bool FileRegistryService::removeEntryFromDatabase(
    const std::shared_ptr<FileLocation> &location) {
  auto file = location->getFile();
  WRENCH_INFO("Removing file (%s) from the file registry",
              file->getID().c_str());
  if (this->entries.find(file) != this->entries.end()) {
