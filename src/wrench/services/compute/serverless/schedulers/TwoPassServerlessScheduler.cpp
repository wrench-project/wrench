...
WRENCH_LOG_CATEGORY(wrench_test_two_pass_serverless_scheduler, "Log category for TwoPassServerlessScheduler");
...
namespace wrench {
     ...
     */
    std::shared_ptr<ServerlessSchedulingDecisions> TwoPassServerlessScheduler::schedule(
     ...
     * @param state The current system state
     */
    void TwoPassServerlessScheduler::makeImageDecisions(const std::shared_ptr<ServerlessSchedulingDecisions>& decisions,
                            const std::vector<std::shared_ptr<Invocation>>& schedulable_invocations,
                            const ServerlessStateOfTheSystem* state) {
        ...
        auto sorted_schedulable_invocations = this->sortSchedulableInvocations(state, schedulable_invocations);

        // For each invocation, assign it to the first compute node with an available core
        for (const auto& invocation : sorted_schedulable_invocations) {
            auto image = invocation->getRegisteredFunction()->getImage();

            auto sorted_compute_nodes = this->sortComputeNodesForInvocation(state, invocation);

            for (const auto& compute_node : sorted_compute_nodes) {
        ...
        // Of course some of those may be infeasible
        {
            auto sorted_compute_nodes = this->sortComputeNodes(state);
            for (const auto& node : sorted_compute_nodes) {
     ...
     * @param state The current system state
     */
    void TwoPassServerlessScheduler::makeInvocationDecisions(const std::shared_ptr<ServerlessSchedulingDecisions>& decisions,
                                 const std::vector<std::shared_ptr<Invocation>>& schedulable_invocations,
                                 const ServerlessStateOfTheSystem* state) {
        ...
        for (const auto& inv : sorted_schedulable_invocations ) {
            ...
            auto sorted_compute_nodes = this->sortComputeNodesForInvocation(state, inv);
            for (const auto& node : sorted_compute_nodes) {
