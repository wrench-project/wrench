...
WRENCH_LOG_CATEGORY(wrench_core_htcondor_negotiator,
                    "Log category for HTCondorNegotiator");
 ...
 * @param non_grid_pre_overhead: a pre-job overhead for non-grid jobs, in
 * seconds
 * @param instant_resource_availabilities: true is instant resource
 * availabilities to used
 * @param fcfs: true if FCFS scheduling should be enforced
 * @param compute_services: a set of 'child' compute services available to and
 * via the HTCondor pool
 * @param running_jobs: a list of currently running jobs
 * @param pending_jobs: a list of pending jobs
 * @param reply_commport: the commport to which the "done/failed" message should
 * be sent
 */
HTCondorNegotiatorService::HTCondorNegotiatorService(
    std::string &hostname, double startup_overhead, double grid_pre_overhead,
    double non_grid_pre_overhead, bool instant_resource_availabilities,
    bool fcfs, std::set<std::shared_ptr<ComputeService>> &compute_services,
    std::map<std::shared_ptr<CompoundJob>, std::shared_ptr<ComputeService>>
        &running_jobs,
    std::vector<std::tuple<std::shared_ptr<CompoundJob>,
                           std::map<std::string, std::string>>> &pending_jobs,
    S4U_CommPort *reply_commport)
    : Service(hostname, "htcondor_negotiator"), reply_commport(reply_commport),
      compute_services(compute_services), running_jobs(running_jobs),
...
bool HTCondorNegotiatorService::JobPriorityComparator::operator()(
    std::tuple<std::shared_ptr<CompoundJob>, std::map<std::string, std::string>>
        &lhs,
    std::tuple<std::shared_ptr<CompoundJob>, std::map<std::string, std::string>>
        &rhs) {
...
int HTCondorNegotiatorService::main() {
  ...
  WRENCH_INFO("HTCondor Negotiator Service starting on host %s listening on "
              "commport %s",
  ...
  std::sort(this->pending_jobs.begin(), this->pending_jobs.end(),
  ...
  while (scheduled_a_job) {
    ...
    for (auto entry : this->pending_jobs) {
      ...
      bool is_standard_job =
          (std::dynamic_pointer_cast<CompoundJob>(job) != nullptr);

      auto target_compute_service =
          pickTargetComputeService(job, service_specific_arguments);
      ...
      if (target_compute_service) {
        ...
        target_compute_service->submitCompoundJob(job,
  ...
  try {
    this->reply_commport->putMessage(new NegotiatorCompletionMessage(
...
HTCondorNegotiatorService::pickTargetComputeService(
    std::shared_ptr<CompoundJob> job,
    const std::map<std::string, std::string> &service_specific_arguments) {

  if (HTCondorComputeService::isJobGridUniverse(job)) {
    return pickTargetComputeServiceGridUniverse(job,
                                                service_specific_arguments);
  } else {
    return pickTargetComputeServiceNonGridUniverse(job,
                                                   service_specific_arguments);
  }
}

/**
 * @brief Helper method to pick a target compute service for a job for a Grid
 * universe job
 ...
 */
std::shared_ptr<ComputeService>
HTCondorNegotiatorService::pickTargetComputeServiceGridUniverse(
    const std::shared_ptr<CompoundJob> &job,
    std::map<std::string, std::string> service_specific_arguments) {
  std::set<std::shared_ptr<BatchComputeService>>
      available_batch_compute_services;
  ...
  if (available_batch_compute_services.empty()) {
    throw std::invalid_argument(
        "HTCondorNegotiatorService::pickTargetComputeServiceGridUniverse(): A "
        "grid universe job was submitted, "
        "but no BatchComputeService is available to the "
  ...
  if ((service_specific_arguments.find("-N") ==
      ...
      (service_specific_arguments.find("-c") ==
       service_specific_arguments.end())) {
    throw std::invalid_argument(
        "HTCondorNegotiatorService::pickTargetComputeServiceGridUniverse(): A "
        "grid universe job must provide -N and -c service-specific arguments");
  }

  // -service service-specific arguments may be required and should point to an
  // existing servuce
  if (service_specific_arguments.find("-service") ==
      service_specific_arguments.end()) {
    if (available_batch_compute_services.size() == 1) {
      service_specific_arguments["-service"] =
          (*available_batch_compute_services.begin())->getName();
    } else {
      throw std::invalid_argument(
          "HTCondorNegotiatorService::pickTargetComputeServiceGridUniverse(): "
          "a grid universe job must provide a -service service-specific "
          "argument since "
          "the HTCondorComputeService has more than one available "
          "BatchComputeService");
    }
  }
  ...
  if (target_batch_cs == nullptr) {
    throw std::invalid_argument(
        "HTCondorNegotiatorService::pickTargetComputeServiceGridUniverse(): "
        "-service service-specific argument specifies a BatchComputeService "
        "compute service named '" +
        service_specific_arguments["-service"] +
...
/**
 * @brief Helper method to pick a target compute service for a job for a
 * Non-Grid universe job
 * @param job: job to run
 * @param service_specific_arguments: service-specific arguments (which may be
 * augmented by this method!)
 * @return A target compute service
 */
std::shared_ptr<ComputeService>
HTCondorNegotiatorService::pickTargetComputeServiceNonGridUniverse(
    const std::shared_ptr<CompoundJob> &job,
    const std::map<std::string, std::string> &service_specific_arguments) {
  ...
  for (auto const &cs : this->compute_services) {
    ...
    if (std::dynamic_pointer_cast<CompoundJob>(job) and
        (not cs->supportsCompoundJobs())) {
      continue;
    }

    // If service-specific arguments are provided, for now reject them
    if (not service_specific_arguments.empty()) {
      throw std::invalid_argument("HTCondorNegotiatorService::"
                                  "pickTargetComputeServiceNonGridUniverse(): "
                                  "service-specific arguments for Non-Grid "
                                  "universe jobs are currently not supported");
    }

    bool enough_idle_resources = false;
    if (not this->instant_resource_availabilities) {
      enough_idle_resources = bmcs->isThereAtLeastOneHostWithIdleResources(
          min_required_num_cores, min_required_memory);
    } else {
      // This is ABSOLUTELY horrible, but is necessary since now control
      // messages may be of size zero and take 0 time to transfer. So this call
      // could happen BEFORE the mbcs has had a chance to update its count of
      // idle cores due to the previous job submission, because that job
      // submission took zero time.
      wrench::Simulation::sleep(0.00000001);
      enough_idle_resources =
          bmcs->isThereAtLeastOneHostWithIdleResourcesInstant(
              min_required_num_cores, min_required_memory);
    }
