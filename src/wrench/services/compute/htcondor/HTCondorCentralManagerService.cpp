 ...
 * @param fast_bmcs_resource_availability: whether to obtain direct availabily
 * information from bare-metal services
 * @param fcfs: whether to enforce FCFS scheduling of jobs
 * @param compute_services: a set of 'child' compute resources available to and
 * via the HTCondor pool
...
HTCondorCentralManagerService::HTCondorCentralManagerService(
    const std::string &hostname, double negotiator_startup_overhead,
    double grid_pre_overhead, double grid_post_overhead,
    double non_grid_pre_overhead, double non_grid_post_overhead,
    bool fast_bmcs_resource_availability, bool fcfs,
 ...
 */
void HTCondorCentralManagerService::addComputeService(
    std::shared_ptr<ComputeService> compute_service) {
  this->compute_services.insert(compute_service);
  //  send a "wake up" message to the daemon's commport
  this->commport->putMessage(new CentralManagerWakeUpMessage(0));
}
...
void HTCondorCentralManagerService::submitCompoundJob(
  ...
  auto msg =
      answer_commport->getMessage<ComputeServiceSubmitCompoundJobAnswerMessage>(
          "HTCondorCentralManagerService::submitCompoundJob(): Received an");
  if (not msg->success) {
    ...
    if (not this->dispatching_jobs && not this->resources_unavailable) {
      ...
      if (not this->pending_jobs.empty()) {
        ...
        auto negotiator = std::make_shared<HTCondorNegotiatorService>(
            this->hostname, this->negotiator_startup_overhead,
            this->grid_pre_overhead, this->non_grid_pre_overhead,
            this->fast_bmcs_resource_availability, this->fcfs,
            this->compute_services, this->running_jobs, this->pending_jobs,
            this->commport);
...
HTCondorCentralManagerService::constructResourceInformation(
    const std::string &key) {
  throw std::runtime_error(
      "HTCondorComputeService::constructResourceInformation(): Not implemented "
      "in this service.");
}
...
bool HTCondorCentralManagerService::processNextMessage() {
  ...
  WRENCH_INFO("HTCondor Central Manager got a [%s] message",
  ...
  } else if (auto msg = std::dynamic_pointer_cast<NegotiatorCompletionMessage>(
                 message)) {
...
//            const std::string &answer_commport, std::shared_ptr <PilotJob>
//            job, std::map <std::string, std::string> &service_specific_args) {
//        this->pending_jobs.push_back(std::make_tuple(job,
//        service_specific_args)); this->resources_unavailable = false;
//
//        S4U_CommPort::dputMessage(
//                answer_commport,
//                new ComputeServiceSubmitPilotJobAnswerMessage(
//                        job,
//                        this->getSharedPtr<HTCondorCentralManagerService>(),
//                        true, nullptr, this->getMessagePayloadValue(
...
//     * @throw std::runtime_error
//     */
//    void HTCondorCentralManagerService::processPilotJobStarted(std::shared_ptr
//    <PilotJob> job) {
...
//     */
//    void
//    HTCondorCentralManagerService::processPilotJobCompletion(std::shared_ptr
//    <PilotJob> job) {
//        // Forward the notification
//        S4U_CommPort::dputMessage(
//                job->popCallbackCommPort(),
 ...
 * @throw std::runtime_error
 */
void HTCondorCentralManagerService::processCompoundJobCompletion(
  ...
  callback_commport->dputMessage(new ComputeServiceCompoundJobDoneMessage(
      job, this->getSharedPtr<HTCondorCentralManagerService>(),
      this->getMessagePayloadValue(HTCondorCentralManagerServiceMessagePayload::
 ...
 * @throw std::runtime_error
 */
void HTCondorCentralManagerService::processCompoundJobFailure(
    const std::shared_ptr<CompoundJob> &job) {
  WRENCH_INFO("A compound job has failed: %s", job->getName().c_str());
  auto callback_commport = job->popCallbackCommPort();

  // Send the callback to the originator
  callback_commport->dputMessage(new ComputeServiceCompoundJobFailedMessage(
      job, this->getSharedPtr<HTCondorCentralManagerService>(),
      this->getMessagePayloadValue(HTCondorCentralManagerServiceMessagePayload::
 ...
 *
 * @param scheduled_jobs: list of scheduled jobs upon negotiator cycle
 * completion
 */
void HTCondorCentralManagerService::processNegotiatorCompletion(
  ...
  for (const auto &sjob : scheduled_jobs) {
    for (auto it = this->pending_jobs.begin(); it != this->pending_jobs.end();
         ++it) {
...
//                (service_specific_arguments.find("-universe") !=
//                service_specific_arguments.end()) and
//                (service_specific_arguments["-universe"] == "grid");
//        bool is_standard_job = (std::dynamic_pointer_cast<StandardJob>(job) !=
//        nullptr);
//
//        bool found_one = false;
//        for (auto const &cs: this->compute_services) {
//            if (is_grid_universe and
//            (std::dynamic_pointer_cast<BatchComputeService>(cs) == nullptr)) {
//                continue;
//            }
//            if ((not is_grid_universe) and
//            (std::dynamic_pointer_cast<BareMetalComputeService>(cs) ==
//            nullptr)) {
//                continue;
//            }
...
/**
 * @brief Helper function to check whether a job can run on at least one child
 * compute service
 * @param job: the job
 * @param service_specific_arguments: the service-specific argument
 * @return true or false
...
std::shared_ptr<FailureCause> HTCondorCentralManagerService::jobCanRunSomewhere(
  ...
  bool is_grid_universe = (service_specific_arguments.find("-universe") !=
                           service_specific_arguments.end()) and
                          (service_specific_arguments["-universe"] == "grid");
  bool is_standard_job =
      (std::dynamic_pointer_cast<StandardJob>(job) != nullptr);
  ...
  for (auto const &cs : this->compute_services) {
    ...
    if ((not is_grid_universe) and
        (std::dynamic_pointer_cast<BareMetalComputeService>(cs) == nullptr)) {
      continue;
    }
    if (((is_grid_universe and
          (std::dynamic_pointer_cast<BatchComputeService>(cs))) or
         ((not is_grid_universe) and
          (std::dynamic_pointer_cast<BareMetalComputeService>(cs))))) {
    ...
    if (is_grid_universe) {
      if (service_specific_arguments.find("-N") ==
          service_specific_arguments.end()) {
        throw std::invalid_argument(
            "HTCondorCentralManagerService::jobCanRunSomewhere(): Grid "
            "universe job must have a '-N' service-specific argument");
      }
      if (service_specific_arguments.find("-c") ==
          service_specific_arguments.end()) {
        throw std::invalid_argument(
            "HTCondorCentralManagerService::jobCanRunSomewhere(): Grid "
            "universe job must have a '-c' service-specific argument");
      }
  ...
  if (not there_is_a_grid_appropriate_service) {
    ...
    return std::make_shared<NotAllowed>(this->getSharedPtr<Service>(),
                                        error_message);
  } else if (not found_one) {
    return std::make_shared<NotEnoughResources>(job,
 ...
 * @return true or false
 */
bool HTCondorCentralManagerService::supportsStandardJobs() { return true; }

/**
 * @brief Returns true if the service supports compound jobs
 * @return true or false
 */
bool HTCondorCentralManagerService::supportsCompoundJobs() { return true; }

/**
 * @brief Returns true if the service supports pilot jobs
 * @return true or false
 */
bool HTCondorCentralManagerService::supportsPilotJobs() { return false; }
