...
WRENCH_LOG_CATEGORY(wrench_core_HTCondor,
                    "Log category for HTCondorComputeService Scheduler");
 ...
 * @param compute_services: a set of 'child' compute services that have been
 * added to the simulation and that are available to and usable through the
 * HTCondor pool.
 *   - BatchComputeService instances will be used for Condor jobs in the "grid"
 * universe
 *   - BareMetalComputeService instances will be used for Condor jobs not in the
 * "grid" universe
...
HTCondorComputeService::HTCondorComputeService(
  ...
  //            throw
  //            std::invalid_argument("HTCondorComputeService::HTCondorComputeService():
  //            at least one 'child' compute service should be provided");
  //        }
  ...
  for (auto const &cs : compute_services) {
    if ((not std::dynamic_pointer_cast<BatchComputeService>(cs)) and
      ...
      throw std::invalid_argument(
          "HTCondorComputeService::HTCondorComputeService(): Only "
          "BatchComputeService or BareMetalComputeService instances can be "
          "used as 'child' services");
    }
  }
  ...
  this->central_manager = std::make_shared<HTCondorCentralManagerService>(
      ...
      this->getPropertyValueAsTimeInSecond(
          HTCondorComputeServiceProperty::NON_GRID_PRE_EXECUTION_DELAY),
      this->getPropertyValueAsTimeInSecond(
          HTCondorComputeServiceProperty::NON_GRID_POST_EXECUTION_DELAY),
      this->getPropertyValueAsBoolean(
          HTCondorComputeServiceProperty::INSTANT_RESOURCE_AVAILABILITIES),
 ...
 */
void HTCondorComputeService::addComputeService(
 ...
 * @throw std::runtime_error
 */
void HTCondorComputeService::submitCompoundJob(
...
HTCondorComputeService::getLocalStorageService() const {
 ...
 */
void HTCondorComputeService::setLocalStorageService(
    std::shared_ptr<wrench::StorageService> local_storage_service) {
 ...
 */
bool HTCondorComputeService::isJobGridUniverse(
    std::shared_ptr<CompoundJob> &job) {
  auto service_specific_arguments = job->getServiceSpecificArguments();
  return (service_specific_arguments.find("-universe") !=
...
int HTCondorComputeService::main() {
  ...
  WRENCH_INFO("HTCondor Service on host %s cleanly terminating!",
  ...
  if (auto msg = std::dynamic_pointer_cast<ServiceStopDaemonMessage>(message)) {
    ...
    try {
      msg->ack_commport->putMessage(new ServiceDaemonStoppedMessage(
          this->getMessagePayloadValue(HTCondorComputeServiceMessagePayload::
 ...
 * @throw std::runtime_error
 */
void HTCondorComputeService::processSubmitCompoundJob(
  ...
  auto failure_cause =
      this->central_manager->jobCanRunSomewhere(job, service_specific_args);
  if (failure_cause) {
  ...
  if (not service_specific_args.empty()) {
    if (service_specific_args.find("-universe") ==
        service_specific_args.end()) {
  ...
  if (not service_specific_args_valid) {
    std::string error_message = "Non-grid universe jobs submitted to HTCondor "
                                "cannot have service-specific arguments";
    answer_commport->dputMessage(
        new ComputeServiceSubmitCompoundJobAnswerMessage(
            job, this->getSharedPtr<HTCondorComputeService>(), false,
            std::shared_ptr<FailureCause>(new NotAllowed(
                this->getSharedPtr<HTCondorComputeService>(), error_message)),
            this->getMessagePayloadValue(
                HTCondorComputeServiceMessagePayload::
                    SUBMIT_COMPOUND_JOB_ANSWER_MESSAGE_PAYLOAD)));
    return;
  ...
  answer_commport->dputMessage(new ComputeServiceSubmitCompoundJobAnswerMessage(
      job, this->getSharedPtr<HTCondorComputeService>(), true, nullptr,
      this->getMessagePayloadValue(
          HTCondorComputeServiceMessagePayload::
              SUBMIT_COMPOUND_JOB_ANSWER_MESSAGE_PAYLOAD)));
}
 ...
 */
void HTCondorComputeService::processIsThereAtLeastOneHostWithAvailableResources(
    S4U_CommPort *answer_commport, unsigned long num_cores, double ram) {
  throw std::runtime_error(
      "HTCondorComputeService::"
      "processIsThereAtLeastOneHostWithAvailableResources(): A HTCondor "
...
void HTCondorComputeService::terminate() {
  ...
  this->central_manager->stop(
      true, ComputeService::TerminationCause::TERMINATION_JOB_KILLED);
 ...
 *
 * @param service_specific_args: the service-specific arguments (useful for some
 * services)
 */
void HTCondorComputeService::validateJobsUseOfScratch(
    std::map<std::string, std::string> &service_specific_args) {
  for (auto const &cs : this->central_manager->compute_services) {
    if (not cs->hasScratch()) {
      throw std::invalid_argument(
          "HTCondorComputeService::validateJobsUseOfScratch(): This HTCondor "
          "service cannot"
          " handle jobs that use scratch space because at least one of its "
          "subordinate compute "
 ...
 */
void HTCondorComputeService::validateServiceSpecificArguments(
  ...
  if (service_specific_args.find("-universe") != service_specific_args.end()) {
    if (service_specific_args.at("-universe") != "grid") {
      throw std::invalid_argument(
          "The value for the '-universe' key can only be 'grid'");
    }
    grid_universe = true;
  }
  ...
  if (not grid_universe) {
    if (not service_specific_args.empty()) {
      throw std::invalid_argument(
          "A non-grid-universe job cannot have any service-specific arguments");
    }
    return;
  }
  ...
  if (service_specific_args.find("-service") != service_specific_args.end()) {
    ...
    if (batch_cses.size() != 1) {
      throw std::invalid_argument(
          "'-service' service-specific argument required for grid-universe job "
          "as there are more than one BatchComputeService compute services "
          "available");
    } else {
  ...
  if (target_cs == nullptr) {
    throw std::invalid_argument(
        "Not able to determine target BatchComputeService compute service for "
        "grid-universe job");
  }

  // Now, invoke the BatchComputeService compute service with the arguments
  // (sort of) to validate them
  auto stripped_service_specific_args = service_specific_args;
  stripped_service_specific_args.erase("-universe");
  stripped_service_specific_args.erase(
      "-service"); // which may not be there, but whatever
  target_cs->validateServiceSpecificArguments(job,
                                              stripped_service_specific_args);
}
...
HTCondorComputeService::constructResourceInformation(const std::string &key) {
  throw std::runtime_error(
 ...
 */
bool HTCondorComputeService::supportsStandardJobs() { return true; }

/**
 * @brief Returns true if the service supports compound jobs
 * @return true or false
 */
bool HTCondorComputeService::supportsCompoundJobs() { return true; }

/**
 * @brief Returns true if the service supports pilot jobs
 * @return true or false
 */
bool HTCondorComputeService::supportsPilotJobs() { return true; }
