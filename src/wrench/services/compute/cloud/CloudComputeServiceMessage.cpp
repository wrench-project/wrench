 ...
 */
CloudComputeServiceMessage::CloudComputeServiceMessage(double payload)
...
CloudComputeServiceGetExecutionHostsRequestMessage::
    CloudComputeServiceGetExecutionHostsRequestMessage(
        S4U_CommPort *answer_commport, double payload)
    : CloudComputeServiceMessage(payload) {
...
CloudComputeServiceGetExecutionHostsAnswerMessage::
    CloudComputeServiceGetExecutionHostsAnswerMessage(
        std::vector<std::string> &execution_hosts, double payload)
    : CloudComputeServiceMessage(payload), execution_hosts(execution_hosts) {}

/**
 * @brief Constructor
 *
 * @param answer_commport: the commport to which to send the answer
 * @param num_cores: the number of cores the service can use (use
 * ComputeService::ALL_CORES to use all cores available on the host)
 * @param ram_memory: the VM's RAM memory_manager_service capacity (use
 * ComputeService::ALL_RAM to use all RAM available on the host, this can be
 * lead to an out of memory_manager_service issue)
 * @param physical_host: the physical execution host ("" means "pick a name for
 * me")
...
CloudComputeServiceCreateVMRequestMessage::
    CloudComputeServiceCreateVMRequestMessage(
        S4U_CommPort *answer_commport, unsigned long num_cores,
        double ram_memory, const std::string &physical_host,
  ...
  if ((answer_commport == nullptr) || (ram_memory < 0.0)) {
    //        std::cerr << answer_commport << " - " << pm_hostname << " - " <<
    //        vm_name << std::endl;
    throw std::invalid_argument(
        "CloudComputeServiceCreateVMRequestMessage::"
        "CloudComputeServiceCreateVMRequestMessage(): Invalid arguments");
  }
#endif
  this->answer_commport = answer_commport;
 ...
 * @param payload: the message size in bytes
 */
CloudComputeServiceCreateVMAnswerMessage::
    CloudComputeServiceCreateVMAnswerMessage(
        bool success, std::string &vm_name,
        std::shared_ptr<FailureCause> failure_cause, double payload)
    : CloudComputeServiceMessage(payload), success(success), vm_name(vm_name),
...
CloudComputeServiceShutdownVMRequestMessage::
  ...
  if ((answer_commport == nullptr) || vm_name.empty()) {
    throw std::invalid_argument(
        "CloudComputeServiceShutdownVMRequestMessage::"
        "CloudComputeServiceShutdownVMRequestMessage(): Invalid arguments");
  }
#endif
  this->answer_commport = answer_commport;
...
CloudComputeServiceShutdownVMAnswerMessage::
    CloudComputeServiceShutdownVMAnswerMessage(
        ...
        double payload)
    : CloudComputeServiceMessage(payload), success(success),
      failure_cause(std::move(failure_cause)) {}
...
CloudComputeServiceStartVMRequestMessage::
  ...
  if ((answer_commport == nullptr) || vm_name.empty()) {
    throw std::invalid_argument(
        "CloudComputeServiceStartVMRequestMessage::"
        "CloudComputeServiceStartVMRequestMessage(): Invalid arguments");
  }
#endif
  this->answer_commport = answer_commport;
  this->vm_name = vm_name;
 ...
 * @param cs: the BareMetalComputeService exposed by the started VM (or nullptr
 * if not success)
 * @param failure_cause: the cause of the failure (or nullptr if success)
 * @param payload: the message size in bytes
 */
CloudComputeServiceStartVMAnswerMessage::
    CloudComputeServiceStartVMAnswerMessage(
        bool success, std::shared_ptr<BareMetalComputeService> cs,
        std::shared_ptr<FailureCause> failure_cause, double payload)
    : CloudComputeServiceMessage(payload), success(success), cs(std::move(cs)),
      failure_cause(std::move(failure_cause)) {}
 ...
 * @throw std::invalid_argument
 */
CloudComputeServiceSuspendVMRequestMessage::
  ...
  if ((answer_commport == nullptr) || vm_name.empty()) {
    throw std::invalid_argument(
        "CloudComputeServiceSuspendVMRequestMessage::"
        "CloudComputeServiceSuspendVMRequestMessage(): Invalid arguments");
  }
#endif
  this->answer_commport = answer_commport;
  this->vm_name = vm_name;
 ...
 * @throw std::invalid_argument
 */
CloudComputeServiceResumeVMRequestMessage::
  ...
  if ((answer_commport == nullptr) || vm_name.empty()) {
    throw std::invalid_argument(
        "CloudComputeServiceResumeVMRequestMessage::"
        "CloudComputeServiceResumeVMRequestMessage(): Invalid arguments");
  }
#endif
  this->answer_commport = answer_commport;
  this->vm_name = vm_name;
 ...
 * @param payload: the message size in bytes
 */
CloudComputeServiceResumeVMAnswerMessage::
    CloudComputeServiceResumeVMAnswerMessage(
        bool success, std::shared_ptr<FailureCause> failure_cause,
        double payload)
    : CloudComputeServiceMessage(payload), success(success),
 ...
 * @throw std::invalid_argument
 */
CloudComputeServiceDestroyVMRequestMessage::
  ...
  if ((answer_commport == nullptr) || vm_name.empty()) {
    throw std::invalid_argument(
        "CloudComputeServiceDestroyVMRequestMessage::"
        "CloudComputeServiceDestroyVMRequestMessage(): Invalid arguments");
  }
#endif
  this->answer_commport = answer_commport;
  this->vm_name = vm_name;
 ...
 * @param payload: the message size in bytes
 */
CloudComputeServiceDestroyVMAnswerMessage::
    CloudComputeServiceDestroyVMAnswerMessage(
        bool success, std::shared_ptr<FailureCause> failure_cause,
        double payload)
    : CloudComputeServiceMessage(payload), success(success),
