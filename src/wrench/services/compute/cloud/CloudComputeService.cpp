...
WRENCH_LOG_CATEGORY(wrench_core_cloud_service,
                    "Log category for Cloud Service");
 ...
 * @param execution_hosts: the list of the names of the hosts available for
 * running virtual machines
 * @param scratch_space_mount_point: the mount point for the cloud service's
 * scratch space ("" means none)
...
CloudComputeService::CloudComputeService(
  ...
  if (execution_hosts.empty()) {
    throw std::invalid_argument(
        "CloudComputeService::CloudComputeService(): At least one execution "
 ...
 * @param property_list: a property list for the BareMetalComputeService that
 * will run on the VM ({} means "use all defaults")
 * @param messagepayload_list: a message payload list for the
 * BareMetalComputeService that will run on the VM ({} means "use all defaults")
 *
 * @return A VM name
 *
 ...
 * @throw std::runtime_error
 */
std::string CloudComputeService::createVM(
    unsigned long num_cores, double ram_memory,
    WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list) {
  if (num_cores == ComputeService::ALL_CORES) {
    throw std::invalid_argument(
        "CloudComputeService::createVM(): the VM's number of cores cannot be "
        "ComputeService::ALL_CORES");
  }
  if (ram_memory == ComputeService::ALL_RAM) {
    throw std::invalid_argument(
        "CloudComputeService::createVM(): the VM's memory_manager_service "
        "requirement cannot be ComputeService::ALL_RAM");
  }
  ...
  auto answer_message =
      sendRequestAndWaitForAnswer<CloudComputeServiceCreateVMAnswerMessage>(
          answer_commport,
          new CloudComputeServiceCreateVMRequestMessage(
              answer_commport, num_cores, ram_memory, "",
 ...
 * @throw std::invalid_argument
 */
void CloudComputeService::shutdownVM(
    const std::string &vm_name, bool send_failure_notifications,
    ComputeService::TerminationCause termination_cause) {
  if (this->vm_list.find(vm_name) == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::shutdownVM(): Unknown VM name '" + vm_name + "'");
  }
  ...
  auto answer_message =
      sendRequestAndWaitForAnswer<CloudComputeServiceShutdownVMAnswerMessage>(
          answer_commport, new CloudComputeServiceShutdownVMRequestMessage(
                               answer_commport, vm_name,
                               send_failure_notifications, termination_cause,
                               this->getMessagePayloadValue(
                                   CloudComputeServiceMessagePayload::
...
CloudComputeService::startVM(const std::string &vm_name) {
  if (this->vm_list.find(vm_name) == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::startVM(): Unknown VM name '" + vm_name + "'");
  }
  ...
  auto answer_message =
      sendRequestAndWaitForAnswer<CloudComputeServiceStartVMAnswerMessage>(
          answer_commport, new CloudComputeServiceStartVMRequestMessage(
                               answer_commport, vm_name,
                               this->getMessagePayloadValue(
                                   CloudComputeServiceMessagePayload::
 ...
 */
std::shared_ptr<BareMetalComputeService>
CloudComputeService::getVMComputeService(const std::string &vm_name) {
  if (this->vm_list.find(vm_name) == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::getVMComputeService(): Unknown VM name '" +
        vm_name + "'");
  }
 ...
 */
std::string
CloudComputeService::getVMPhysicalHostname(const std::string &vm_name) {
  if (this->vm_list.find(vm_name) == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::getVMPhysicalHostname(): Unknown VM name '" +
        vm_name + "'");
  }
...
void CloudComputeService::suspendVM(const std::string &vm_name) {
  if (this->vm_list.find(vm_name) == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::suspendVM(): Unknown VM name '" + vm_name + "'");
  }
  ...
  auto answer_message =
      sendRequestAndWaitForAnswer<CloudComputeServiceSuspendVMAnswerMessage>(
          answer_commport, new CloudComputeServiceSuspendVMRequestMessage(
                               answer_commport, vm_name,
                               this->getMessagePayloadValue(
                                   CloudComputeServiceMessagePayload::
...
void CloudComputeService::resumeVM(const std::string &vm_name) {
  if (this->vm_list.find(vm_name) == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::resumeVM(): Unknown VM name '" + vm_name + "'");
  }
  ...
  auto answer_message =
      sendRequestAndWaitForAnswer<CloudComputeServiceResumeVMAnswerMessage>(
          answer_commport, new CloudComputeServiceResumeVMRequestMessage(
                               answer_commport, vm_name,
                               this->getMessagePayloadValue(
                                   CloudComputeServiceMessagePayload::
...
void CloudComputeService::destroyVM(const std::string &vm_name) {
  if (this->vm_list.find(vm_name) == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::destroyVM(): Unknown VM name '" + vm_name + "'");
  }
  ...
  auto answer_message =
      sendRequestAndWaitForAnswer<CloudComputeServiceDestroyVMAnswerMessage>(
          answer_commport, new CloudComputeServiceDestroyVMRequestMessage(
                               answer_commport, vm_name,
                               this->getMessagePayloadValue(
                                   CloudComputeServiceMessagePayload::
...
bool CloudComputeService::isVMRunning(const std::string &vm_name) {
  ...
  if (vm_pair_it == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::isVMRunning(): Unknown VM name '" + vm_name +
        "'");
  }

  assertServiceIsUp();

  return (std::get<0>(vm_pair_it->second)->getState() ==
          S4U_VirtualMachine::State::RUNNING);
}
...
bool CloudComputeService::isVMDown(const std::string &vm_name) {
  ...
  if (vm_pair_it == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::isVMDown(): Unknown VM name '" + vm_name + "'");
  }

  assertServiceIsUp();

  return (std::get<0>(vm_pair_it->second)->getState() ==
          S4U_VirtualMachine::State::DOWN);
}
...
bool CloudComputeService::isVMSuspended(const std::string &vm_name) {
  ...
  if (vm_pair_it == this->vm_list.end()) {
    throw std::invalid_argument(
        "CloudComputeService::isVMSuspended(): Unknown VM name '" + vm_name +
        "'");
  }
  ...
  return (std::get<0>(vm_pair_it->second)->getState() ==
          S4U_VirtualMachine::State::SUSPENDED);
}
...
int CloudComputeService::main() {
  ...
  WRENCH_INFO("Cloud Service on host %s terminating cleanly!",
...
//     *
//     * @param answer_commport: the commport to which the answer message should
//     be sent
...
//    CloudComputeService::sendRequestAndWaitForAnswer(S4U_CommPort
//    *answer_commport,
...
//
//        // Wait for a reply
//        return S4U_CommPort::getMessage<TMessageType>(answer_commport,
...
bool CloudComputeService::processNextMessage() {
  ...
  if (auto msg = dynamic_cast<ServiceStopDaemonMessage *>(message.get())) {
    ...
    return true;
                 ...
                 dynamic_cast<CloudComputeServiceCreateVMRequestMessage *>(
                     message.get())) {
    ...
    return true;
                 ...
                 dynamic_cast<CloudComputeServiceShutdownVMRequestMessage *>(
                     message.get())) {
    processShutdownVM(msg->answer_commport, msg->vm_name,
                      msg->send_failure_notifications, msg->termination_cause);
    return true;

  } else if (auto msg =
                 dynamic_cast<CloudComputeServiceStartVMRequestMessage *>(
                     message.get())) {
    processStartVM(msg->answer_commport, msg->vm_name);
    return true;

  } else if (auto msg =
                 dynamic_cast<CloudComputeServiceSuspendVMRequestMessage *>(
                     message.get())) {
    processSuspendVM(msg->answer_commport, msg->vm_name);
    return true;

  } else if (auto msg =
                 dynamic_cast<CloudComputeServiceResumeVMRequestMessage *>(
                     message.get())) {
    processResumeVM(msg->answer_commport, msg->vm_name);
    return true;

  } else if (auto msg =
                 dynamic_cast<CloudComputeServiceDestroyVMRequestMessage *>(
                     message.get())) {
    ...
    return true;
                 ...
                 dynamic_cast<ServiceHasTerminatedMessage *>(message.get())) {
    if (auto bmcs =
            std::dynamic_pointer_cast<BareMetalComputeService>(msg->service)) {
      processBareMetalComputeServiceTermination(bmcs, msg->exit_code);
    } else {
      throw std::runtime_error("CloudComputeService::processNextMessage(): "
 ...
 * sent
 */
void CloudComputeService::processGetExecutionHosts(
 ...
 *
 */
void CloudComputeService::processCreateVM(
    S4U_CommPort *answer_commport, unsigned long requested_num_cores,
    double requested_ram, const std::string &physical_host,
    WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list) {
  WRENCH_INFO("Asked to create a VM with %s cores and %s RAM",
              (requested_num_cores == ComputeService::ALL_CORES
                   ? "max"
                   : std::to_string(requested_num_cores))
                  .c_str(),
              (requested_ram == ComputeService::ALL_RAM
                   ? "max"
                   : std::to_string(requested_ram))
                  .c_str());
  ...
  if (host.empty()) {
    ...
    msg_to_send_back = new CloudComputeServiceCreateVMAnswerMessage(
        ...
        std::shared_ptr<FailureCause>(new NotEnoughResources(
            nullptr, this->getSharedPtr<CloudComputeService>())),
        this->getMessagePayloadValue(CloudComputeServiceMessagePayload::
  ...
  // Pick a VM name (being paranoid about mistakenly picking an actual
  // hostname!)
  std::string vm_name;
  ...
  do {
    vm_name =
        this->getName() + "_vm" + std::to_string(CloudComputeService::VM_ID++);
  ...
  //                            new
  //                            NotAllowed(this->getSharedPtr<CloudComputeService>(),
  //                            error_msg)),
  //                    this->getMessagePayloadValue(
  ...
  // Create the VM
  auto vm = std::make_shared<S4U_VirtualMachine>(
      vm_name, requested_num_cores, requested_ram, std::move(property_list),
      std::move(messagepayload_list));
 ...
 * sent)
 */
void CloudComputeService::processShutdownVM(
  ...
  if (vm->getState() != S4U_VirtualMachine::State::RUNNING) {
    ...
    msg_to_send_back = new CloudComputeServiceShutdownVMAnswerMessage(
        ...
        std::shared_ptr<FailureCause>(new NotAllowed(
            this->getSharedPtr<CloudComputeService>(), error_message)),
        this->getMessagePayloadValue(CloudComputeServiceMessagePayload::
    ...
    // We do not shut down the VM. This will be done when the
    // CloudComputeService is notified of the BareMetalComputeService
    // completion.
    vm->shutdown();
...
/**
 * @brief A helper method that finds a host with a given number of available
 * cores and a given amount of available RAM using one of the several resource
...
std::string CloudComputeService::findHost(unsigned long desired_num_cores,
  ...
  for (auto const &host : this->execution_hosts) {
    ...
    auto num_available_cores =
        total_num_cores - this->used_cores_per_execution_host[host];
...
/**
 * @brief: Process a VM start request by starting a VM on a host (using best fit
 * for RAM first, and then for cores)
 *
 * @param answer_commport: the commport to which the answer message should be
 * sent
 * @param vm_name: the name of the VM
 */
void CloudComputeService::processStartVM(S4U_CommPort *answer_commport,
  ...
  if (not S4U_Simulation::isHostOn(host)) {
    ...
    msg_to_send_back = new CloudComputeServiceStartVMAnswerMessage(
        false, nullptr, std::shared_ptr<FailureCause>(new HostError(host)),
        this->getMessagePayloadValue(CloudComputeServiceMessagePayload::
                                         START_VM_ANSWER_MESSAGE_PAYLOAD));
    answer_commport->dputMessage(msg_to_send_back);
    return;
  ...
  Simulation::sleep(this->getPropertyValueAsTimeInSecond(
  ...
  if ((cs == nullptr) or (not cs->isUp())) {
    ...
    std::map<std::string, std::tuple<unsigned long, double>> compute_resources =
        {std::make_pair(vm_name,
                        std::make_tuple(vm->getNumCores(), vm->getMemory()))};

    // Create the BareMetal service, whose main daemon is on this (stable) host
    auto plist = vm->getPropertyList();
    plist.insert(std::make_pair(BareMetalComputeServiceProperty::
    ...
    cs = std::shared_ptr<BareMetalComputeService>(new BareMetalComputeService(
        this->hostname, compute_resources, plist, vm->getMessagePayloadList(),
        this->getScratch()));
  ...
  //        WRENCH_INFO("CREATING SERVICE TERMINATION DETECTOR THAT WILL REPORT
  //        TO COMMPORT %s", this->commport->get_cname());
  auto termination_detector = std::make_shared<ServiceTerminationDetector>(
      this->hostname, cs, this->commport, false, true);
...
CloudComputeService::constructResourceInformation(const std::string &key) {
  std::map<std::string, double> dict;

  if (key == "num_hosts") {
    // Num hosts
    dict.insert(
        std::make_pair(this->getName(), (double)(this->vm_list.size())));
    ...
    for (auto &host : this->execution_hosts) {
      ...
      dict.insert(std::make_pair(
          host, (double)(total_num_cores - num_cores_allocated_to_vms)));
    }
    ...
    throw std::runtime_error(
        "CloudComputeService::constructResourceInformation(): unknown key");
  }
 ...
 * needed)
 */
void CloudComputeService::processGetResourceInformation(
...
void CloudComputeService::processIsThereAtLeastOneHostWithAvailableResources(
  ...
  for (auto const &h : this->used_cores_per_execution_host) {
    ...
    auto available_ram = Simulation::getHostMemoryCapacity(h.first) -
                         this->used_ram_per_execution_host[h.first];
...
/**
 * @brief Process a termination by a previously started BareMetalComputeService
 * on a VM
...
void CloudComputeService::processBareMetalComputeServiceTermination(
  ...
  if (vm_name.empty()) {
    WRENCH_WARN(
        "CloudComputeService::processBareMetalComputeServiceTermination(): "
        "received a termination notification for an unknown "
        "BareMetalComputeService - Probably rapid-fire (zero-time) "
        "shutdown/restart shenanigans");
    return;
  }
...
void CloudComputeService::validateProperties() {
  ...
  try {
    vm_boot_overhead = this->getPropertyValueAsTimeInSecond(
        CloudComputeServiceProperty::VM_BOOT_OVERHEAD);
  ...
  if ((!success) or (vm_boot_overhead < 0)) {
    throw std::invalid_argument(
        "Invalid VM_BOOT_OVERHEAD property specification: " +
  ...
  if (vm_resource_allocation_algorithm != "first-fit" and
    ...
    throw std::invalid_argument(
        "Invalid VM_RESOURCE_ALLOCATION_ALGORITHM property specification: " +
 ...
 * @return true or false
 */
bool CloudComputeService::supportsStandardJobs() { return false; }

/**
 * @brief Returns true if the service supports compound jobs
 * @return true or false
 */
bool CloudComputeService::supportsCompoundJobs() { return false; }

/**
 * @brief Returns true if the service supports pilot jobs
 * @return true or false
 */
bool CloudComputeService::supportsPilotJobs() { return false; }
