...
WRENCH_LOG_CATEGORY(wrench_core_compute_service,
                    "Log category for Compute Service");
...
void ComputeService::stop(bool send_failure_notifications,
  ...
  this->shutting_down =
      true; // This is to avoid another process calling stop() and being stuck

  WRENCH_INFO("Telling the daemon listening on (%s) to terminate",
  ...
  try {
    this->commport->dputMessage(new ServiceStopDaemonMessage(
        ack_commport, send_failure_notifications, termination_cause,
  ...
  try {
    ack_commport->getMessage<ServiceDaemonStoppedMessage>(
        this->network_timeout, "ComputeService::stop(): Received an");
  } catch (...) {
 ...
 * @param job: the job
 * @param service_specific_args: arguments specific to compute services when
 * needed:
 *      - to a BareMetalComputeService: {}
 *          - If no entry is provided for an actionID, the service will pick on
 * which host and with how many cores to run the task
 *          - If a number of cores is provided (e.g., {"action1", "12"}), the
 * service will pick the host on which to run the task
 *          - If a hostname and a number of cores is provided (e.g., {"action1",
 * "host1:12"}, the service will run the action on that host with the specified
 * number of cores
 *      - to a BatchComputeService:
 * {"-t":"<int>","-N":"<int>","-c":"<int>"[,{"-u":"<string>"}],
 * [{actionID:[host[:num_cores]]}
...
void ComputeService::submitJob(
  ...
  if (job == nullptr) {
    throw std::invalid_argument(
        "ComputeService::submitJob(): invalid argument");
  }
...
void ComputeService::terminateJob(const std::shared_ptr<CompoundJob> &job) {
  if (job == nullptr) {
    throw std::invalid_argument(
        "ComputeService::terminateJob(): invalid argument");
  }
 ...
 * @param scratch_space_mount_point: the service's scratch space's mount point
 * ("" if none)
 */
ComputeService::ComputeService(const std::string &hostname,
  ...
  if ((not scratch_space_mount_point.empty()) and
      (not Simulation::hostHasMountPoint(hostname,
                                         scratch_space_mount_point))) {
    throw std::invalid_argument("ComputeService::ComputeService(): Host " +
                                hostname + " does not have a disk mounted at " +
                                scratch_space_mount_point);
  }
...
void ComputeService::startScratchStorageService() {
  if (this->scratch_space_storage_service)
    return; // Already started by somebody else
  if (this->scratch_space_mount_point.empty())
    return; // No mount point provided

  if (wrench::Simulation::isLinkShutdownSimulationEnabled() and
      (this->getPropertyValueAsDouble(
           ComputeServiceProperty::SCRATCH_SPACE_BUFFER_SIZE) == 0)) {
    throw std::runtime_error(
        "ComputeService::startScratchStorageService(): Compute service " +
        this->name +
        " cannot start a scratch service with (default) buffer size 0 because "
        "link shutdown " +
  ...
  auto ss = SimpleStorageService::createSimpleStorageService(
      hostname, {scratch_space_mount_point},
      {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE,
        this->getPropertyValueAsString(
            ComputeServiceProperty::SCRATCH_SPACE_BUFFER_SIZE)}},
      {});
  ...
  this->scratch_space_storage_service = this->simulation->startNewService(ss);
}
 ...
 * @param hostname: the name of the host on which the compute service runs
 * @param service_name: the name of the compute service
 * @param scratch_space: scratch storage space of the compute service (nullptr
 * if none)
 */
ComputeService::ComputeService(const std::string &hostname,
                               const std::string &service_name,
 ...
 * @param simulate_it: if true, simulate a message sent to the service to
 * retrieve the information, otherwise perform this operation in zero simulated
 * time.
...
ComputeService::getPerHostNumCores(bool simulate_it) {
  std::map<std::string, double> dict;
  dict = this->getServiceResourceInformation("num_cores", simulate_it);
 ...
 *        mean that asking for these cores right now will mean immediate
 * execution (since jobs may be pending and "ahead" in the queue, e.g., because
 * they depend on current actions that are not using all available resources).
 *
 * @throw ExecutionException
 * @throw std::runtime_error
 */
std::map<std::string, unsigned long>
ComputeService::getPerHostNumIdleCores(bool simulate_it) {
  std::map<std::string, double> dict;
...
ComputeService::getPerHostAvailableMemoryCapacity(bool simulate_it) {
  std::map<std::string, double> dict;
 ...
 * Note that this doesn't mean that asking for these cores right will mean
 * immediate execution (since jobs may be pending and "ahead" in the queue,
 * e.g., because they depend on current actions that are not using all available
 * resources).
 * @param simulate_it: if true, simulate a message sent to the service to
 * retrieve the information, otherwise perform this operation in zero simulated
 * time.
...
/**
 * @brief Method to find out if, right now, the compute service has at least one
 * host with some idle number of cores and some available RAM. Note that this
 * doesn't mean that asking for these resources right will mean immediate
 * execution (since jobs may be pending and "ahead" in the queue, e.g., because
 * they depend on current actions that are not using all available resources).
 ...
 */
bool ComputeService::isThereAtLeastOneHostWithIdleResources(
  ...
  this->commport->putMessage(
      new ComputeServiceIsThereAtLeastOneHostWithAvailableResourcesRequestMessage(
          answer_commport, num_cores, ram,
          this->getMessagePayloadValue(
              ComputeServiceMessagePayload::
                  IS_THERE_AT_LEAST_ONE_HOST_WITH_AVAILABLE_RESOURCES_REQUEST_MESSAGE_PAYLOAD)));

  // Get the reply
  auto msg = answer_commport->getMessage<
      ComputeServiceIsThereAtLeastOneHostWithAvailableResourcesAnswerMessage>(
      this->network_timeout,
      "BareMetalComputeService::isThereAtLeastOneHostWithIdleResources(): "
      "received an");
 ...
 */
std::map<std::string, double>
ComputeService::getCoreFlopRate(bool simulate_it) {
  std::map<std::string, double> dict;
 ...
 */
std::map<std::string, double>
ComputeService::getMemoryCapacity(bool simulate_it) {
  std::map<std::string, double> dict;
 ...
 */
std::map<std::string, double>
ComputeService::getServiceResourceInformation(const std::string &key,
                                              bool simulate_it) {
  assertServiceIsUp();
  ...
  if (simulate_it) {
    ...
    this->commport->putMessage(
        new ComputeServiceResourceInformationRequestMessage(
            answer_commport, key,
            this->getMessagePayloadValue(
                ComputeServiceMessagePayload::
    ...
    auto msg = answer_commport
                   ->getMessage<ComputeServiceResourceInformationAnswerMessage>(
                       this->network_timeout,
                       "BareMetalComputeService::getServiceResourceInformation("
                       "): received an");
    return msg->info;
  } else {
...
double ComputeService::getTotalScratchSpaceSize() {
  ...
  return this->scratch_space_storage_service
             ? this->scratch_space_storage_service->getTotalSpace()
             : 0.0;
}
...
//    /**
//    * @brief Get a shared pointer to the compute service's scratch storage
//    space
 ...
 * @return true if the compute service has some scratch storage space, false
 * otherwise
 */
bool ComputeService::hasScratch() const {
  return (not this->scratch_space_mount_point.empty()) or
 ...
 */
void ComputeService::validateServiceSpecificArguments(
    const std::shared_ptr<CompoundJob> &job,
    map<std::string, std::string> &service_specific_args) {
  throw std::runtime_error(
      "ComputeService::validateServiceSpecificArguments(): should be "
      "overridden in compute service implementation");
}

/**
 * @brief Method to validate that a job's use of the scratch space if ok. Throws
 * exception if not.
 * @param service_specific_args: the job;'s service-specific arguments (useful
 * for some services)
 */
void ComputeService::validateJobsUseOfScratch(
    std::map<std::string, std::string> &service_specific_args) {
  if (not this->hasScratch()) {
    throw std::invalid_argument("Compute service (" + this->getName() +
                                ") does not have scratch space");
  }
}
