      ...
      try {
        ...
        while (std::getline(infile, line)) {
          if (line[0] != ';') {
            ...
            if (tokens.size() < 10) {
              throw std::invalid_argument(
                      "TraceFileLoader::loadFromTraceFile(): Seeing less than 10 fields per line in batch workload trace file '" +
                      filename +
                      "'");
            }
              ...
              switch (itemnum) {
                ...
                case 1: // Submit time
                  if (sscanf(item.c_str(), "%lf", &sub_time) != 1) {
                    throw std::invalid_argument(
                            "TraceFileLoader::loadFromTraceFile(): Invalid submission time '" + item +
                ...
                case 3: // Run time
                  ...
                  if (sscanf(item.c_str(), "%lf", &time) != 1) {
                    throw std::invalid_argument(
                            "TraceFileLoader::loadFromTraceFile(): Invalid run time '" + item +
                            "' in batch workload trace file");
                  }
                  break;
                ...
                case 8: // Requested time
                  ...
                  if (sscanf(item.c_str(), "%lf", &requested_time) != 1) {
                    throw std::invalid_argument(
                            "TraceFileLoader::loadFromTraceFile(): Invalid requested time '" + item +
                            "' in batch workload trace file");
                  }
                  break;
                case 9: // Requested memory
                  ...
                  if (sscanf(item.c_str(), "%lf", &requested_ram) != 1) {
                    throw std::invalid_argument(
                            "TraceFileLoader::loadFromTraceFile(): Invalid requested memory '" + item +
                            "' in batch workload trace file");
                  }
                ...
                default:
                  throw std::runtime_error(
                          "TraceFileLoader::loadFromTraceFile(): Unknown batch workload trace file column, maybe there are more than 18 columns?"
                  );
              }
              itemnum++;
            }
            ...
            if ((requested_time < 0) or (time < 0)) {
              throw std::invalid_argument(
                      "TraceFileLoader::loadFromTraceFile(): invalid job with negative flops and negative requested flops in batch workload trace file");
            }
            if (requested_ram < 0) {
              requested_ram = 0;
            }
            if (sub_time < 0) {
              throw std::invalid_argument(
                      "TraceFileLoader::loadFromTraceFile(): invalid job with negative submission time in batch workload trace file");
            }
            ...
            if (requested_num_nodes < 0) {
              throw std::invalid_argument(
                      "TraceFileLoader::loadFromTraceFile(): invalid job with negative (requested) number of node in batch workload trace file");
            }
