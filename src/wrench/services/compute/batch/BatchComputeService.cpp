...
WRENCH_LOG_CATEGORY(wrench_core_batch_service,
                    "Log category for Batch Service");
 ...
 *                 - the hosts must be homogeneous (speed, number of cores, and
 * RAM size)
 *                 - all cores are usable by the BatchComputeService service on
 * each host
 *                 - all RAM is usable by the BatchComputeService service on
 * each host
 * @param scratch_space_mount_point: the mount point of the scratch storage
 * space for the service ("" means "no scratch space")
 * @param property_list: a property list that specifies
 * BatchComputeServiceProperty values ({} means "use all defaults")
 * @param messagepayload_list: a message payload list that specifies
 * BatchComputeServiceMessagePayload values ({} means "use all defaults")
 */
BatchComputeService::BatchComputeService(
    const std::string &hostname, std::vector<std::string> compute_hosts,
    std::string scratch_space_mount_point,
    WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list)
    : BatchComputeService(hostname, std::move(compute_hosts),
                          ComputeService::ALL_CORES, ComputeService::ALL_RAM,
                          scratch_space_mount_point, std::move(property_list),
 ...
 * @param scratch_space_mount_point: the mount point og the scratch storage
 * space for the service ("" means "no scratch space")
 * @param property_list: a property list ({} means "use all defaults")
 * @param messagepayload_list: a message payload list ({} means "use all
 * defaults")
...
BatchComputeService::BatchComputeService(
    const std::string &hostname, std::vector<std::string> compute_hosts,
    unsigned long cores_per_host, double ram_per_host,
    std::string scratch_space_mount_point,
    WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list,
    const std::string &suffix)
    : ComputeService(hostname, "BatchComputeService" + suffix,
  ...
  if (compute_hosts.empty()) {
    throw std::invalid_argument("BatchComputeService::BatchComputeService(): "
                                "at least one compute hosts must be provided");
  }
  ...
  for (auto const &h : this->compute_hosts) {
    ...
    if (std::abs(speed - h->get_speed()) > DBL_EPSILON) {
      throw std::invalid_argument(
          "BatchComputeService::BatchComputeService(): Compute hosts for a "
          "BatchComputeService service need "
          "to be homogeneous (different flop rates detected)");
    }
    // RAM
    if (std::abs(ram_available - S4U_Simulation::getHostMemoryCapacity(h)) >
        DBL_EPSILON) {
      throw std::invalid_argument(
          "BatchComputeService::BatchComputeService(): Compute hosts for a "
          "BatchComputeService service need "
    ...
    this->workload_trace = TraceFileLoader::loadFromTraceFile(
        ...
        this->getPropertyValueAsBoolean(
            BatchComputeServiceProperty::
                IGNORE_INVALID_JOBS_IN_WORKLOAD_TRACE_FILE),
        this->getPropertyValueAsDouble(
            BatchComputeServiceProperty::
                SUBMIT_TIME_OF_FIRST_JOB_IN_WORKLOAD_TRACE_FILE));
    // Fix value
  ...
  if (this->scheduling_algorithms.find(batch_scheduling_alg) ==
      this->scheduling_algorithms.end()) {
    throw std::invalid_argument(" BatchComputeService::BatchComputeService(): "
                                "unsupported scheduling algorithm " +
                                batch_scheduling_alg);
  }
  this->scheduler =
      std::unique_ptr<BatchScheduler>(new BatschedBatchScheduler(this));
#else
  auto batch_scheduling_alg = this->getPropertyValueAsString(
      BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM);
  if (this->scheduling_algorithms.find(batch_scheduling_alg) ==
  ...
  if (batch_scheduling_alg == "fcfs") {
    this->scheduler =
        std::unique_ptr<BatchScheduler>(new FCFSBatchScheduler(this));
  } else if (batch_scheduling_alg == "conservative_bf") {
    this->scheduler = std::unique_ptr<BatchScheduler>(
        new ConservativeBackfillingBatchScheduler(this));
  } else if (batch_scheduling_alg == "conservative_bf_core_level") {
    this->scheduler = std::unique_ptr<BatchScheduler>(
        new ConservativeBackfillingBatchSchedulerCoreLevel(this));
  }
#endif
 ...
 * Each configuration is a tuple as follows:
 ...
 *
 * @return start date predictions in seconds (as a map of ids). A prediction
 * that's negative means that the job configuration can not run on the service
 * (e.g., not enough hosts, not enough cores per host)
 */
std::map<std::string, double> BatchComputeService::getStartTimeEstimates(
  ...
  try {
    auto estimates =
        this->scheduler->getStartTimeEstimates(std::move(set_of_jobs));
    return estimates;
 ...
 */
std::vector<std::tuple<std::string, std::string, int, int, int, double, double>>
BatchComputeService::getQueue() {
  // Go through the currently running jobs
  std::vector<
      std::tuple<std::string, std::string, int, int, int, double, double>>
      queue_state;
  for (auto const &j : this->running_jobs) {
    auto batch_job = j.second;
    auto tuple = std::make_tuple(
        batch_job->getUsername(), batch_job->getCompoundJob()->getName(),
        batch_job->getRequestedNumNodes(),
        batch_job->getRequestedCoresPerNode(), batch_job->getRequestedTime(),
        batch_job->getArrivalTimestamp(), batch_job->getBeginTimestamp());
  ...
  for (auto const &j : this->waiting_jobs) {
    auto tuple = std::make_tuple(
        j->getUsername(), j->getCompoundJob()->getName(),
        j->getRequestedNumNodes(), j->getRequestedCoresPerNode(),
        j->getRequestedTime(), j->getArrivalTimestamp(), -1.0);
    queue_state.emplace_back(tuple);
  }
#endif
  ...
  std::sort(queue_state.begin(), queue_state.end(),
            [](const std::tuple<std::string, std::string, int, int, int, double,
                                double> &j1,
               const std::tuple<std::string, std::string, int, int, int, double,
                                double> &j2) -> bool {
...
unsigned long BatchComputeService::parseUnsignedLongServiceSpecificArgument(
  ...
  if (it != args.end()) {
    if (sscanf((*it).second.c_str(), "%lu", &value) != 1) {
      throw std::invalid_argument(
          "BatchComputeService::parseUnsignedLongServiceSpecificArgument(): "
          "Invalid " +
          key + " value '" + (*it).second + "'");
    }
    ...
    throw std::invalid_argument(
        "BatchComputeService::parseUnsignedLongServiceSpecificArgument(): "
        "Batch Service requires " +
        key + " argument to be specified for job submission");
  }
  return value;
}
 ...
 */
void BatchComputeService::submitCompoundJob(
  ...
  // Sanity check
  if ((num_hosts == 0) or (num_cores_per_host == 0) or
      (time_asked_for_in_seconds == 0)) {
    throw std::invalid_argument(
        "BatchComputeService::submitCompoundJob(): service-specific arguments "
        "should have non-zero values");
  }

  // Create a Batch Job
  unsigned long jobid = wrench::BatchComputeService::generateUniqueJobID();
  auto batch_job = std::make_shared<BatchJob>(
      job, jobid, time_asked_for_in_seconds, num_hosts, num_cores_per_host,
      username, -1, S4U_Simulation::getClock());
  ...
  this->commport->dputMessage(new BatchComputeServiceJobRequestMessage(
  ...
  // Get the answer
  auto msg =
      answer_commport->getMessage<ComputeServiceSubmitCompoundJobAnswerMessage>(
...
/**
 * @brief Helper function called by terminateStandardJob() and
 * terminatePilotJob() to process a job submission
 * @param job
 */
void BatchComputeService::terminateCompoundJob(
    std::shared_ptr<CompoundJob> job) {
  assertServiceIsUp();
  ...
  this->commport->putMessage(
      new ComputeServiceTerminateCompoundJobRequestMessage(
          ...
          this->getMessagePayloadValue(
              BatchComputeServiceMessagePayload::
                  TERMINATE_COMPOUND_JOB_REQUEST_MESSAGE_PAYLOAD)));

  // Get the answer
  auto msg =
      answer_commport
          ->getMessage<ComputeServiceTerminateCompoundJobAnswerMessage>(
              this->network_timeout,
 ...
 * @param job
 */
void BatchComputeService::sendPilotJobExpirationNotification(
  ...
  job->popCallbackCommPort()->dputMessage(
      new ComputeServicePilotJobExpiredMessage(
          ...
          this->getMessagePayloadValue(BatchComputeServiceMessagePayload::
 ...
 */
void BatchComputeService::sendCompoundJobFailureNotification(
    const std::shared_ptr<CompoundJob> &job, const std::string &job_id,
    const std::shared_ptr<FailureCause> &cause) {
  WRENCH_INFO("Sending compound job failure notification for job %s",
              job->getName().c_str());
  ...
  try {
    job->popCallbackCommPort()->putMessage(
        new ComputeServiceCompoundJobFailedMessage(
            ...
            this->getMessagePayloadValue(
                BatchComputeServiceMessagePayload::
                    COMPOUND_JOB_FAILED_MESSAGE_PAYLOAD)));
  } catch (ExecutionException &e) {
    return; // ignore
  }
}
 ...
 *              - bytes of RAM (double)
 */
void BatchComputeService::freeUpResources(
 ...
 * @param job
 */
void BatchComputeService::removeJobFromRunningList(
    const std::shared_ptr<BatchJob> &job) {
  if (this->running_jobs.find(job->getCompoundJob()) ==
      this->running_jobs.end()) {
    throw std::runtime_error(
        "BatchComputeService::removeJobFromRunningList(): Cannot find job!");
  }
 ...
 */
void BatchComputeService::removeBatchJobFromJobsList(
    const std::shared_ptr<BatchJob> &batch_job) {
 ...
 */
void BatchComputeService::processCompoundJobTimeout(
  ...
  if (this->running_bare_metal_one_shot_compute_services.find(compound_job) ==
    ...
    throw std::runtime_error("BatchComputeService::processCompoundJobTimeout():"
                             " Unknown compound job");
  }

  auto executor =
      this->running_bare_metal_one_shot_compute_services[compound_job];
 ...
 */
void BatchComputeService::terminateRunningCompoundJob(
    const std::shared_ptr<CompoundJob> &job,
    ComputeService::TerminationCause termination_cause) {
  if (this->running_bare_metal_one_shot_compute_services.find(job) ==
      this->running_bare_metal_one_shot_compute_services.end()) {
    throw std::runtime_error("BatchComputeService::terminateRunningCompoundJob("
                             "): Unknown compound job");
  }
  ...
  executor->stop(
      false,
      termination_cause); // failure notifications sent by me later, if needed
}

/**
 * @brief Declare all current jobs as failed (likely because the daemon is being
 * terminated or has timed out (because it's in fact a pilot job))
 *
 * @param send_failure_notifications: whether to send failure notifications or
 * not
 * @param termination_cause: the termination cause
 */
void BatchComputeService::terminate(
  ...
  while (not this->running_jobs.empty()) {
    ...
    // Popping, because I am terminating it, so the executor won't pop, and
    // right now if I am at the top of the stack!
    ...
    if (send_failure_notifications) {
      this->sendCompoundJobFailureNotification(
          ...
          std::shared_ptr<FailureCause>(new JobKilled(compound_job)));
    }
  ...
  while (not this->batch_queue.empty()) {
    ...
    auto compound_job = batch_job->getCompoundJob();
    //            WRENCH_INFO("SIMPLY REMOVING COMPOUND JOB %s FROM PENDING
    //            LIST", compound_job->getName().c_str());
    ...
    case ComputeService::TerminationCause::TERMINATION_JOB_KILLED:
      ...
      break;
    case ComputeService::TerminationCause::
        TERMINATION_COMPUTE_SERVICE_TERMINATED:
      failure_cause =
          std::make_shared<ServiceIsDown>(this->getSharedPtr<ComputeService>());
      break;
  ...
  while (not this->waiting_jobs.empty()) {
    ...
    auto compound_job = batch_job->getCompoundJob();
    //            WRENCH_INFO("SIMPLY REMOVING COMPOUND JOB %s FROM WAITING
    //            LIST", compound_job->getName().c_str());
 ...
 */
void BatchComputeService::cleanup(bool has_returned_from_main,
                                  int return_value) {
  this->scheduler->shutdown();

  // Do the default behavior (which will throw as this is not a fault-tolerant
  // service)
...
//        if
//        (getPropertyValueAsBoolean(BatchComputeServiceProperty::SUPPORTS_PILOT_JOBS))
//        {
//            WRENCH_INFO(
...
//                if (auto pjob =
//                std::dynamic_pointer_cast<PilotJob>(job->getCompoundJob())) {
//                    auto cs = pjob->getComputeService();
//                    if (cs == nullptr) {
//                        throw std::runtime_error(
//                                "BatchComputeService::terminate(): can't find
//                                compute service associated to pilot job");
//                    }
//                    try {
//                        cs->stop(false);
...
bool BatchComputeService::processNextMessage() {
  ...
  if (message == nullptr) {
    WRENCH_INFO(
        "Got a NULL message... Likely this means we're all done. Aborting!");
    return false;
  }
  ...
  if (auto msg = std::dynamic_pointer_cast<ServiceStopDaemonMessage>(message)) {
  ...
  } else if (auto msg = std::dynamic_pointer_cast<
                 ComputeServiceResourceInformationRequestMessage>(message)) {
    processGetResourceInformation(msg->answer_commport, msg->key);
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<
                 BatchComputeServiceJobRequestMessage>(message)) {
    processJobSubmission(msg->job, msg->answer_commport);
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<
                 ComputeServiceCompoundJobDoneMessage>(message)) {
    processCompoundJobCompletion(
        std::dynamic_pointer_cast<BareMetalComputeServiceOneShot>(
            msg->compute_service),
        msg->job);
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<
                 ComputeServiceCompoundJobFailedMessage>(message)) {
    processCompoundJobFailure(
        std::dynamic_pointer_cast<BareMetalComputeServiceOneShot>(
            msg->compute_service),
        msg->job, nullptr);
    return true;
    ...
    return true;
                 ...
                 std::dynamic_pointer_cast<AlarmJobTimeOutMessage>(message)) {
    processAlarmJobTimeout(msg->job);
    return true;

#ifdef ENABLE_BATSCHED
  } else if (auto msg =
                 std::dynamic_pointer_cast<BatchExecuteJobFromBatSchedMessage>(
                     message)) {
    ...
    return true;
      ...
      auto msg = std::dynamic_pointer_cast<
          ...
          message)) {
    processIsThereAtLeastOneHostWithAvailableResources(
        msg->answer_commport, msg->num_cores, msg->ram);
    return true;

  } else {
    throw std::runtime_error(
        "BatchComputeService::processNextMessage(): Unexpected [" +
 ...
 */
void BatchComputeService::processJobSubmission(
    const std::shared_ptr<BatchJob> &job, S4U_CommPort *answer_commport) {
  WRENCH_INFO("Asked to run a BatchComputeService job with id %ld",
              job->getJobID());
  ...
  if ((requested_hosts > this->available_nodes_to_cores.size()) or
      (requested_num_cores_per_host >
       (unsigned long)this->available_nodes_to_cores.begin()
           ->first->get_core_count()) or
           ...
           this->available_nodes_to_cores.begin()->first))) {
    {
      answer_commport->dputMessage(
          new ComputeServiceSubmitCompoundJobAnswerMessage(
              job->getCompoundJob(), this->getSharedPtr<BatchComputeService>(),
              false,
              std::shared_ptr<FailureCause>(new NotEnoughResources(
                  job->getCompoundJob(),
                  this->getSharedPtr<BatchComputeService>())),
              this->getMessagePayloadValue(
                  BatchComputeServiceMessagePayload::
                      SUBMIT_COMPOUND_JOB_ANSWER_MESSAGE_PAYLOAD)));
      return;
    }
  }

  // SUCCESS!
  answer_commport->dputMessage(new ComputeServiceSubmitCompoundJobAnswerMessage(
      job->getCompoundJob(), this->getSharedPtr<BatchComputeService>(), true,
      nullptr,
      this->getMessagePayloadValue(
          BatchComputeServiceMessagePayload::
...
void BatchComputeService::processCompoundJobCompletion(
  ...
  if (this->running_bare_metal_one_shot_compute_services.find(job) ==
      this->running_bare_metal_one_shot_compute_services.end()) {
    // warning
    WRENCH_WARN("BatchComputeService::processCompoundJobCompletion(): Received "
                "a compound job completion, but "
                "the executor is not in the executor list - Likely getting "
                "wires crossed due to concurrent "
  ...
  if (this->running_jobs.find(job) == this->running_jobs.end()) {
    throw std::runtime_error(
        "BatchComputeService::processCompoundJobCompletion(): Received a "
  ...
  WRENCH_INFO("A compound job executor has completed job %s",
              job->getName().c_str());
  ...
  job->popCallbackCommPort()->dputMessage(
      new ComputeServiceCompoundJobDoneMessage(
          job, this->getSharedPtr<BatchComputeService>(),
          this->getMessagePayloadValue(BatchComputeServiceMessagePayload::
                                           COMPOUND_JOB_DONE_MESSAGE_PAYLOAD)));

  // Free the job from the global (pending, running, waiting) job list, (doing
  // this at the end of this method to make sure
 ...
 */
void BatchComputeService::removeJobFromBatchQueue(
    const std::shared_ptr<BatchJob> &job) {
  for (auto it = this->batch_queue.begin(); it != this->batch_queue.end();
       it++) {
    if ((*it) == job) {
 ...
 */
void BatchComputeService::processCompoundJobFailure(
    const std::shared_ptr<BareMetalComputeServiceOneShot> &executor,
    const std::shared_ptr<CompoundJob> &job,
    const std::shared_ptr<FailureCause> &cause) {
  if (this->running_bare_metal_one_shot_compute_services.find(job) ==
      this->running_bare_metal_one_shot_compute_services.end()) {
    throw std::runtime_error("BatchComputeService::processCompoundJobFailure():"
                             " Received a compound job failure, "
  ...
  WRENCH_INFO("A compound job executor has failed to perform job %s",
              job->getName().c_str());
  ...
  this->sendCompoundJobFailureNotification(
      job, std::to_string((batch_job->getJobID())), cause);
  // Free the job from the global (pending, running, waiting) job list, (doing
  // this at the end of this method to make sure this job is not used anymore
  // anywhere)
  this->removeBatchJobFromJobsList(batch_job);
}
...
void BatchComputeService::startJob(
    ...
    const std::shared_ptr<CompoundJob> &compound_job,
    const std::shared_ptr<BatchJob> &batch_job,
    unsigned long num_nodes_allocated, unsigned long allocated_time,
    unsigned long cores_per_node_asked_for) {
  WRENCH_INFO("Creating a BareMetalComputeServiceOneShot for a compound job on "
              "%ld nodes with %ld cores per node",
  ...
  batch_job->setEndingTimestamp(S4U_Simulation::getClock() +
  ...
  std::shared_ptr<Alarm> alarm_ptr = Alarm::createAndStartAlarm(
      this->simulation, batch_job->getEndingTimestamp(), this->hostname,
      this->commport, msg, "batch_standard");
...
BatchComputeService::constructResourceInformation(const std::string &key) {
  ...
  if (key == "num_hosts") {
    // Num hosts
    dict.insert(std::make_pair(this->getName(),
                               (double)(this->nodes_to_cores_map.size())));
    ...
    for (const auto &h : this->nodes_to_cores_map) {
      dict.insert(std::make_pair(
          h.first->get_name(), S4U_Simulation::getHostMemoryCapacity(h.first)));
    }
    ...
    for (const auto &h : this->available_nodes_to_cores) {
      if ((double)h.second < S4U_Simulation::getHostMemoryCapacity(h.first)) {
        ...
        dict.insert(
            std::make_pair(h.first->get_name(),
                           S4U_Simulation::getHostMemoryCapacity(h.first)));
      }
    }
  } else {
    throw std::runtime_error(
        "BatchComputeService::processGetResourceInformation(): unknown key");
  }
  return dict;
}
 ...
 * @param answer_commport: the commport to which the description message should
 * be sent
 * @param key: the desired resource information (i.e., dictionary key) that's
 * needed)
 */
void BatchComputeService::processGetResourceInformation(
    S4U_CommPort *answer_commport, const std::string &key) {

  auto dict = this->constructResourceInformation(key);

  // Send the reply
  auto *answer_message = new ComputeServiceResourceInformationAnswerMessage(
      dict, this->getMessagePayloadValue(
...
void BatchComputeService::startBackgroundWorkloadProcess() {
  if (this->workload_trace.empty()) {
    throw std::runtime_error(
        "BatchComputeService::startBackgroundWorkloadProcess(): no workload "
  ...
  this->workload_trace_replayer = std::make_shared<WorkloadTraceFileReplayer>(
      ...
      S4U_Simulation::getHostName(), this->getSharedPtr<BatchComputeService>(),
 ...
 * sent
 */
void BatchComputeService::processCompoundJobTerminationRequest(
    const std::shared_ptr<CompoundJob> &job, S4U_CommPort *answer_commport) {
  ...
  if (not is_running) {
    if (batch_job == nullptr) {
      // Is it pending?
      for (auto it1 = this->batch_queue.begin(); it1 != this->batch_queue.end();
           it1++) {
  ...
  //        WRENCH_INFO("pending: %d   running: %d   waiting: %d", is_pending,
  //        is_running, is_waiting);
  if (!is_pending && !is_running && !is_waiting) {
    std::string msg = "Job cannot be terminated because it is neither pending, "
                      "not running, not waiting";
    ...
    auto answer_message = new ComputeServiceTerminateCompoundJobAnswerMessage(
        ...
        std::shared_ptr<FailureCause>(
            new NotAllowed(this->getSharedPtr<BatchComputeService>(), msg)),
        this->getMessagePayloadValue(
            BatchComputeServiceMessagePayload::
                TERMINATE_COMPOUND_JOB_ANSWER_MESSAGE_PAYLOAD));
    answer_commport->dputMessage(answer_message);
    return;
  }
  ...
  if (is_running) {
    ...
    terminateRunningCompoundJob(
        job, ComputeService::TerminationCause::TERMINATION_JOB_KILLED);
    this->freeUpResources(batch_job->getResourcesAllocated());
 ...
 * @param batch_job: the batch job
 */
void BatchComputeService::processAlarmJobTimeout(
    const std::shared_ptr<BatchJob> &batch_job) {
  ...
  if (not compound_job) {
    // time out
    WRENCH_INFO("BatchComputeService::processAlarmJobTimeout(): Received a "
                "time out message for an unknown "
 ...
 */
void BatchComputeService::processExecuteJobFromBatSched(
    const std::string &bat_sched_reply) {
  nlohmann::json execute_events = nlohmann::json::parse(bat_sched_reply);
  std::shared_ptr<CompoundJob> compound_job = nullptr;
  std::shared_ptr<BatchJob> batch_job = nullptr;
  for (auto it1 = this->waiting_jobs.begin(); it1 != this->waiting_jobs.end();
       it1++) {
  ...
  if (compound_job == nullptr) {
    WRENCH_WARN(
        "BatchComputeService::processExecuteJobFromBatSched(): Job received "
        "from batsched that does not "
        "belong to the list of known jobs... just telling Batsched that the "
        "job completed (Batsched seems "
  ...
  for (auto node : node_resources) {
    ...
    this->available_nodes_to_cores[this->host_id_to_names[node]] -=
        cores_per_node_asked_for;
    resources.insert(std::make_pair(
        this->host_id_to_names[node],
        std::make_tuple(cores_per_node_asked_for, ram_capacity)));
  }

  startJob(resources, compound_job, batch_job, num_nodes_allocated,
           time_in_seconds, cores_per_node_asked_for);
}
#endif
 ...
 */
void BatchComputeService::processIsThereAtLeastOneHostWithAvailableResources(
    S4U_CommPort *answer_commport, unsigned long num_cores, double ram) {
  throw std::runtime_error(
      "BatchComputeService::processIsThereAtLeastOneHostWithAvailableResources("
      "): A BatchComputeService compute service does not support this "
      "operation");
}
 ...
 */
void BatchComputeService::validateServiceSpecificArguments(
    const std::shared_ptr<CompoundJob> &job,
  ...
  for (auto const &arg : service_specific_args) {
    ...
    if (key == "-N") {
      ...
      if ((sscanf(value.c_str(), "%lu", &num_nodes) != 1) or (num_nodes == 0)) {
        throw std::invalid_argument("Invalid service-specific argument {\"" +
                                    key + "\",\"" + value + "\"}");
      }
      if (this->compute_hosts.size() < num_nodes) {
        throw ExecutionException(std::make_shared<NotEnoughResources>(
            job, this->getSharedPtr<ComputeService>()));
      }
    } else if (key == "-t") {
      found_dash_t = true;
      unsigned long requested_time;
      if ((sscanf(value.c_str(), "%lu", &requested_time) != 1) or
          (requested_time == 0)) {
        throw std::invalid_argument("Invalid service-specific argument {\"" +
                                    key + "\",\"" + value + "\"}");
      }
      ...
      if (not found_task) {
        throw std::invalid_argument(
            "Invalid service-specific argument {" + key + "," + value +
            "}: Job does not have any task with name " + key);
      }
    }
  }
  if (not found_dash_t) {
    throw std::invalid_argument(
        "Compute service requires a '-t' service-specific argument");
  }
  if (not found_dash_N) {
    throw std::invalid_argument(
        "Compute service requires a '-N' service-specific argument");
  }
  if (not found_dash_c) {
    throw std::invalid_argument(
        "Compute service requires a '-c' service-specific argument");
  }

  // Double check that memory requirements of all tasks can be met
  if (job->getMinimumRequiredMemory() >
      S4U_Simulation::getHostMemoryCapacity(
          this->available_nodes_to_cores.begin()->first)) {
    throw ExecutionException(std::make_shared<NotEnoughResources>(
        job, this->getSharedPtr<ComputeService>()));
 ...
 */
bool BatchComputeService::supportsStandardJobs() { return true; }

/**
 * @brief Returns true if the service supports compound jobs
 * @return true or false
 */
bool BatchComputeService::supportsCompoundJobs() { return true; }

/**
 * @brief Returns true if the service supports pilot jobs
 * @return true or false
 */
bool BatchComputeService::supportsPilotJobs() { return true; }
