...
WRENCH_LOG_CATEGORY(wrench_core_trace_file_loader,
                    "Log category for Trace File Loader");
...
std::string generateRandomUsername(unsigned long userid) {
  ...
  const char charset[] = "aabccdeeefghijklmnooopqrstttuuvwxyzz";
 ...
 * @param desired_submit_time_of_first_job: the desired submit of of the first
 * job (-1 means "use whatever time is in the trace file")
 *
 * @return  a vector of tuples, where each tuple is a job description with the
 * following fields:
 *              - job id
 *              - submission time
 *              - actual time
...
TraceFileLoader::loadFromTraceFile(std::string filename,
                                   bool ignore_invalid_jobs,
                                   double desired_submit_time_of_first_job) {
  ...
  if (tokens.size() < 2) {
    throw std::invalid_argument(
        "TraceFileLoader::loadFromTraceFile(): BatchComputeService workload "
        "trace file name must end with '.swf' or '.json'");
  }
  std::string extension = tokens[tokens.size() - 1];
  if (extension == "swf") {
    return loadFromTraceFileSWF(filename, ignore_invalid_jobs,
                                desired_submit_time_of_first_job);
  } else if (extension == "json") {
    return loadFromTraceFileJSON(filename, ignore_invalid_jobs,
                                 desired_submit_time_of_first_job);
  } else {
    throw std::invalid_argument(
        "TraceFileLoader::loadFromTraceFile(): BatchComputeService workload "
...
std::vector<std::tuple<std::string, double, double, double, double,
                       unsigned int, std::string>>
TraceFileLoader::loadFromTraceFileSWF(std::string filename,
                                      bool ignore_invalid_jobs,
                                      double desired_submit_time_of_first_job) {
  std::vector<std::tuple<std::string, double, double, double, double,
                         unsigned int, std::string>>
  ...
  if (not infile.is_open()) {
    throw std::invalid_argument(
        "TraceFileLoader::loadFromTraceFileSWF(): Cannot open "
        "BatchComputeService workload trace file " +
        filename);
  }
  ...
  while (std::getline(infile, line)) {
    if (line[0] != ';') {
      ...
      try {
        if (tokens.size() < 10) {
          throw std::invalid_argument(
              "TraceFileLoader::loadFromTraceFileSWF(): Seeing less than 10 "
              "fields per line in BatchComputeService workload trace file '" +
              filename + "'");
        }
        ...
        for (auto const &item : tokens) {
          ...
          case 1: // Submit time
            if (sscanf(item.c_str(), "%lf", &sub_time) != 1) {
              throw std::invalid_argument(
                  "TraceFileLoader::loadFromTraceFileSWF(): Invalid submission "
                  "time '" +
                  item + "' in BatchComputeService workload trace file");
            }
            if (original_submit_time_of_first_job < 0) {
              original_submit_time_of_first_job = sub_time;
            }
            if (desired_submit_time_of_first_job >= 0) {
              sub_time += (desired_submit_time_of_first_job -
                           original_submit_time_of_first_job);
            }
            break;
          case 2: // Wait time
            break;
          case 3: // Run time
            ...
            if (sscanf(item.c_str(), "%lf", &time) != 1) {
              throw std::invalid_argument(
                  "TraceFileLoader::loadFromTraceFileSWF(): Invalid run time "
                  "'" +
                  item + "' in BatchComputeService workload trace file");
            }
            break;
          case 4: // Number of Allocated Processors
            if (sscanf(item.c_str(), "%d", &num_nodes) != 1) {
              throw std::invalid_argument(
                  "TraceFileLoader::loadFromTraceFileSWF(): Invalid number of "
                  "processors '" +
                  item + "' in BatchComputeService workload trace file");
            }
            break;
          ...
          case 7: // Requested Number of Processors
            if (sscanf(item.c_str(), "%d", &requested_num_nodes) != 1) {
              throw std::invalid_argument(
                  "TraceFileLoader::loadFromTraceFileSWF(): Invalid requested "
          ...
          default:
            throw std::invalid_argument(
                "TraceFileLoader::loadFromTraceFileSWF(): Unknown "
                "BatchComputeService workload trace file column, maybe there "
                "are more than 18 columns?");
          }
          itemnum++;
        }
        ...
        if ((requested_time < 0) or (time < 0)) {
          throw std::invalid_argument(
              "TraceFileLoader::loadFromTraceFileSWF(): invalid job with "
              "negative flops (" +
              std::to_string(time) + ") and negative requested flops (" +
              std::to_string(requested_time) +
              ") in BatchComputeService workload trace file");
        }
        if (requested_time < time) {
          WRENCH_WARN(
              "TraceFileLoader::loadFromTraceFileSWF(): invalid job with "
              "requested time (%lf) smaller than actual time (%lf) in "
              "BatchComputeService workload trace file [fixing it]",
              requested_time, time);
          requested_time = time;
        }
        ...
        if (sub_time < 0) {
          throw std::invalid_argument(
              "TraceFileLoader::loadFromTraceFileSWF(): invalid job with "
              "negative submission time in BatchComputeService workload trace "
              "file");
        }
        ...
        if (requested_num_nodes <= 0) {
          throw std::invalid_argument(
              "TraceFileLoader::loadFromTraceFileSWF(): invalid job with "
              "negative (requested) number of node in BatchComputeService "
              "workload trace file");
        }
        if (userid == 0) {
          username = "user";
        ...
        if (ignore_invalid_jobs) {
          WRENCH_WARN("%s (in BatchComputeService workload file %s) IGNORING",
                      e.what(), filename.c_str());
          continue;
        } else {
          throw std::invalid_argument(
              "Error while reading BatchComputeService workload trace file " +
              filename + ": " + e.what());
        }
      }
      ...
      std::tuple<std::string, double, double, double, double, unsigned int,
                 std::string>
          job = std::tuple<std::string, double, double, double, double,
                           unsigned int, std::string>(
...
std::vector<std::tuple<std::string, double, double, double, double,
                       unsigned int, std::string>>
TraceFileLoader::loadFromTraceFileJSON(
    std::string filename, bool ignore_invalid_jobs,
    double desired_submit_time_of_first_job) {
  std::vector<std::tuple<std::string, double, double, double, double,
                         unsigned int, std::string>>
      trace_file_jobs = {};
  ...
  try {
    ...
    throw std::invalid_argument(
        "TraceFileLoader::loadFromTraceFileJSON(): Cannot open JSON "
        "BatchComputeService workload trace file " +
        filename);
  }
  ...
  try {
    ...
    throw std::invalid_argument(
        std::string(
            "TraceFileLoader::loadFromTraceFileJSON(): JSON parse error: ") +
        e.what());
  }
  ...
  try {
    ...
    throw std::invalid_argument(
        "TraceFileLoader::loadFromTraceFileJSON(): Could not find 'jobs' in "
        "JSON BatchComputeService workload trace file");
  }
  ...
  for (nlohmann::json::iterator it = jobs.begin(); it != jobs.end(); ++it) {
    ...
    try {
      if (not json_job.is_object()) {
        // Broken JSON
        throw std::invalid_argument(
            "TraceFileLoader::loadFromTraceFileJSON(): broken job "
            "specification in JSON BatchComputeService workload trace file");
      }
      ...
      try {
        ...
        throw std::invalid_argument(
            "TraceFileLoader::loadFromTraceFileJSON(): invalid job "
            "specification in JSON BatchComputeService workload trace file");
      }
      ...
      if (desired_submit_time_of_first_job >= 0) {
        subtime += (desired_submit_time_of_first_job -
                    original_submit_time_of_first_job);
      }
      ...
      if (ignore_invalid_jobs) {
        WRENCH_WARN("%s (in BatchComputeService workload file %s)", e.what(),
                    filename.c_str());
        continue;
      } else {
        throw std::invalid_argument(
    ...
    std::tuple<std::string, double, double, double, double, unsigned int,
        ...
        job =
            std::tuple<std::string, double, double, double, double,
                       unsigned int, std::string>(std::to_string(id), subtime,
                                                  walltime, requested_time, 0.0,
                                                  (unsigned int)res, "user");
    trace_file_jobs.push_back(job);
  }
