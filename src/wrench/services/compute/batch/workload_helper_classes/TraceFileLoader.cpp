...
namespace wrench {
    ...
    */
    ...
    TraceFileLoader::loadFromTraceFile(std::string filename, bool ignore_invalid_jobs, double desired_submit_time_of_first_job) {
        ...
        if (tokens.size() < 2) {
            throw std::invalid_argument(
                    "TraceFileLoader::loadFromTraceFile(): batch_standard_and_pilot_jobs workload trace file name must end with '.swf' or '.json'");
        }
    ...
    */
    ...
    TraceFileLoader::loadFromTraceFileSWF(std::string filename, bool ignore_invalid_jobs, double desired_submit_time_of_first_job) {
        ...
        if (not infile.is_open()) {
            throw std::invalid_argument(
                    "TraceFileLoader::loadFromTraceFileSWF(): Cannot open batch_standard_and_pilot_jobs workload trace file " + filename);
        }
        ...
        while (std::getline(infile, line)) {
            ...
            if (line[0] != ';') {
                ...
                try {
                    if (tokens.size() < 10) {
                        throw std::invalid_argument(
                                "TraceFileLoader::loadFromTraceFileSWF(): Seeing less than 10 fields per line in batch_standard_and_pilot_jobs workload trace file '" +
                                filename +
                                "'");
                    }
                    ...
                    for (auto const &item : tokens) {
                        switch (itemnum) {
                            ...
                            case 1: // Submit time
                                if (sscanf(item.c_str(), "%lf", &sub_time) != 1) {
                                    throw std::invalid_argument(
                                            ...
                                            item +
                                            "' in batch_standard_and_pilot_jobs workload trace file");
                                }
                            ...
                            default:
                                throw std::invalid_argument(
                                        "TraceFileLoader::loadFromTraceFileSWF(): Unknown batch_standard_and_pilot_jobs workload trace file column, maybe there are more than 18 columns?"
                                );
                        }
                        itemnum++;
                    }
                    ...
                    if ((requested_time < 0) or (time < 0)) {
                        throw std::invalid_argument(
                                ...
                                std::to_string(time) + ") and negative requested flops ("+ std::to_string(requested_time) + ") in batch_standard_and_pilot_jobs workload trace file");
                    }
                    if (requested_time < time) {
                        WRENCH_WARN(
                                "TraceFileLoader::loadFromTraceFileSWF(): invalid job with requested time (%lf) smaller than actual time (%lf) in batch_standard_and_pilot_jobs workload trace file [fixing it]", requested_time, time);
                        requested_time = time;
                    }
                    ...
                    if (sub_time < 0) {
                        throw std::invalid_argument(
                                "TraceFileLoader::loadFromTraceFileSWF(): invalid job with negative submission time in batch_standard_and_pilot_jobs workload trace file");
                    }
                    ...
                    if (requested_num_nodes <= 0) {
                        throw std::invalid_argument(
                                "TraceFileLoader::loadFromTraceFileSWF(): invalid job with negative (requested) number of node in batch_standard_and_pilot_jobs workload trace file");
                    }
                    if (userid == 0) {
                        username = "user";
                    } else {
                    ...
                    if (ignore_invalid_jobs) {
                        WRENCH_WARN("%s (in batch_standard_and_pilot_jobs workload file %s) IGNORING", e.what(), filename.c_str());
                        continue;
                    } else {
                        throw std::invalid_argument("Error while reading batch_standard_and_pilot_jobs workload trace file " + filename + ": " +  e.what());
                    }
                }
    ...
    */
    ...
    TraceFileLoader::loadFromTraceFileJSON(std::string filename, bool ignore_invalid_jobs, double desired_submit_time_of_first_job) {
        ...
        try {
            ...
            throw std::invalid_argument(
                    "TraceFileLoader::loadFromTraceFileJSON(): Cannot open JSON batch_standard_and_pilot_jobs workload trace file " + filename);
        }
        ...
        try {
            ...
            throw std::invalid_argument(
                    "TraceFileLoader::loadFromTraceFileJSON(): Could not find 'jobs' in JSON batch_standard_and_pilot_jobs workload trace file");
        }
        ...
        for (nlohmann::json::iterator it = jobs.begin(); it != jobs.end(); ++it) {
            ...
            try {
                if (not json_job.is_object()) {
                    // Broken JSON
                    throw std::invalid_argument(
                            "TraceFileLoader::loadFromTraceFileJSON(): broken job specification in JSON batch_standard_and_pilot_jobs workload trace file");
                }
                ...
                try {
                    ...
                    throw std::invalid_argument(
                            "TraceFileLoader::loadFromTraceFileJSON(): invalid job specification in JSON batch_standard_and_pilot_jobs workload trace file");
                }
                ...
                if (ignore_invalid_jobs) {
                    WRENCH_WARN("%s (in batch_standard_and_pilot_jobs workload file %s)", e.what(), filename.c_str());
                    continue;
                } else {
                    throw std::invalid_argument("Error while reading batch_standard_and_pilot_jobs workload trace file " + filename + ": " +  e.what());
                }
