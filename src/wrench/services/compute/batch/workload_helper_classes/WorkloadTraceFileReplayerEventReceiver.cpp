...
WRENCH_LOG_CATEGORY(wrench_core_one_job_wms,
                    "Log category for WorkloadTraceFileReplayerEventReceiver");
  ...
  while (true) {
    ...
    try {
      ...
      if (std::dynamic_pointer_cast<wrench::StandardJobCompletedEvent>(event)) {
        ...
        throw std::runtime_error("WorkloadTraceFileReplayerEventReceiver::main("
                                 "): Unexpected workflow execution event");
      }
    ...
    // DO NOT Remove tasks that correspond to the job, since people may still
    // care! And besides, timestamps have been generated that correspond to
    // these tasks!
