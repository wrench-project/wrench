 ...
 */
BatchJob::BatchJob(const std::shared_ptr<CompoundJob> &job,
                   unsigned long job_id, unsigned long time_in_seconds,
                   unsigned long num_nodes, unsigned long cores_per_node,
                   const std::string &username, double ending_time_stamp,
                   double arrival_time_stamp) {
#ifdef WRENCH_INTERNAL_EXCEPTIONS
  if (job == nullptr) {
    throw std::invalid_argument("BatchJob::BatchJob(): job cannot be null");
  }
  if (job_id <= 0 || num_nodes == 0 || cores_per_node == 0) {
    throw std::invalid_argument(
        "BatchJob::BatchJob(): either jobid (" + std::to_string(job_id) +
        "), num_nodes (" + std::to_string(num_nodes) + "), cores_per_node (" +
        std::to_string(cores_per_node) + ") is less than or equal to zero");
  }
#endif
 ...
 * @return a username
 */
std::string BatchJob::getUsername() { return this->username; }
 ...
 * @return a string id
 */
unsigned long BatchJob::getJobID() const { return this->job_id; }
 ...
 * @return a date
 */
double BatchJob::getBeginTimestamp() const { return this->begin_time_stamp; }

/**
 * @brief Get the BatchComputeService job's end timestamp
 * @return a date
 */
double BatchJob::getEndingTimestamp() const { return this->ending_time_stamp; }
...
void BatchJob::setEndingTimestamp(double time_stamp) {
  if (this->ending_time_stamp > 0) {
    throw std::runtime_error("BatchJob::setEndingTimestamp(): Cannot set time "
                             "stamp again for the same job");
  }
 ...
 * @return a list of resource, each as a <hostname, number of cores, bytes of
 * RAM> tuple
 */
std::map<simgrid::s4u::Host *, std::tuple<unsigned long, double>>
BatchJob::getResourcesAllocated() {
  return this->resources_allocated;
}

/**
 * @brief Set the resources allocated to this BatchComputeService job
 * @param resources: a list of resource, each as a <host, number of cores, bytes
 * of RAM> tuple
 */
void BatchJob::setAllocatedResources(
