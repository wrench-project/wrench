...
WRENCH_LOG_CATEGORY(wrench_core_batsched_batch_scheduler,
                    "Log category for BatschedBatchScheduler");
...
void BatschedBatchScheduler::init() {
  ...
  while (1) {
    ...
    this->batsched_port = 28000 + (getpid() % 1500) +
                          S4U_CommPort::generateUniqueSequenceNumber();
    //            std::cerr << "Thinking of starting Batsched on port " <<
    //            this->batsched_port << "...\n";
    this->pid = getpid();
    ...
    if (rc != 0) {
      //                std::cerr << "Couldn't bind socket to port (errno=" <<
      //                errno << " - EADDRINUSE = " << EADDRINUSE << ".
      //                Retrying...\n";
      continue;
    } else {
      //                std::cerr << "Was able to bind socket to port, unbinding
      //                and proceeding\n";
  ...
  if (top_pid == 0) { // Child process that will exec batsched
    std::string algorithm = this->cs->getPropertyValueAsString(
        BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM);
    bool is_supported = this->cs->scheduling_algorithms.find(algorithm) !=
                        this->cs->scheduling_algorithms.end();
    if (not is_supported) {
      exit(1);
    }
    ...
    bool is_queue_ordering_available =
        this->cs->queue_ordering_options.find(queue_ordering) !=
        this->cs->queue_ordering_options.end();
    if (not is_queue_ordering_available) {
      exit(2);
    }

    std::string rjms_delay = this->cs->getPropertyValueAsString(
        BatchComputeServiceProperty::BATCH_RJMS_PADDING_DELAY);
    std::string socket_endpoint =
        "tcp://*:" + std::to_string(this->batsched_port);
    ...
    num_args++;
    (args = (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
        strdup(algorithm.c_str());
    num_args++;
    (args = (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
        strdup("-o");
    num_args++;
    (args = (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
        strdup(queue_ordering.c_str());
    num_args++;
    (args = (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
        strdup("-s");
    num_args++;
    (args = (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
        strdup(socket_endpoint.c_str());
    num_args++;
    (args = (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
        strdup("--rjms_delay");
    num_args++;
    (args = (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
        strdup(rjms_delay.c_str());
    num_args++;

    if (this->cs->getPropertyValueAsBoolean(
            BatchComputeServiceProperty::BATSCHED_LOGGING_MUTED)) {
      (args =
           (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
          strdup("--verbosity=silent");
      num_args++;
    }
    if (this->cs->getPropertyValueAsBoolean(
            BatchComputeServiceProperty::BATSCHED_CONTIGUOUS_ALLOCATION)) {
      (args =
           (char **)realloc(args, (num_args + 1) * sizeof(char *)))[num_args] =
    ...
    switch (exit_code) {
    case 0: {
      // Establish a tether so that if the main process dies, then batsched is
      // brutally killed
      int tether[2]; // this is a local variable, only defined in this scope
      if (pipe(tether) != 0) { // the pipe however is opened during the whole
                               // duration of both processes
        kill(top_pid,
             SIGKILL); // kill the other child (that has fork-exec'd batsched)
        throw std::runtime_error(
            "startBatsched(): tether pipe creation failed!");
      }
      ...
      if (nested_pid > 0) {
        ...
        auto num_bytes_read =
            read(tether[0], &foo,
                 1); // blocking read which returns when the parent dies
        // check if the child that forked batsched is still running
        if (getpgid(top_pid)) {
          kill(top_pid,
               SIGKILL); // kill the other child that has fork-exec'd batsched
        }
        // my parent has died so, I will kill myself instead of exiting and
        // becoming a zombie
        kill(getpid(), SIGKILL);
    ...
    case 1:
      throw std::invalid_argument(
          ...
          this->cs->getPropertyValueAsString(
              BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM) +
    ...
    default:
      throw std::runtime_error("startBatsched(): Unknown fatal error");
    }

    // Create the output CSV file if needed
    std::string output_csv_file = this->cs->getPropertyValueAsString(
    ...
    if (not output_csv_file.empty()) {
      ...
      if (not file) {
        throw std::invalid_argument(
            "BatchComputeService(): Unable to create CSV output file " +
            output_csv_file +
            " (as specified by the "
            "BatchComputeServiceProperty::OUTPUT_CSV_JOB_LOG property)");
      }
    }

  } else {
    // fork failed
    throw std::runtime_error("Error while fork-exec of batsched");
  }
}
...
std::map<std::string, double> BatschedBatchScheduler::getStartTimeEstimates(
  ...
  std::set<std::string> supported_algorithms = {
      "conservative_bf", "fast_conservative_bf", "fcfs_fast"};
  if (supported_algorithms.find(this->cs->getPropertyValueAsString(
          BatchComputeServiceProperty::BATCH_SCHEDULING_ALGORITHM)) ==
      supported_algorithms.end()) {
    throw std::runtime_error("BatschedBatchScheduler::getStartTimeEstimates(): "
                             "Algorithm does not support start time estimates");
  }
  nlohmann::json batch_submission_data;
  batch_submission_data["now"] = S4U_Simulation::getClock();

  // IMPORTANT: THIS IGNORES THE NUMBER OF CORES (THIS IS A LIMITATION OF
  // batsched!)

  int idx = 0;
  batch_submission_data["events"] = nlohmann::json::array();
  for (auto job : set_of_jobs) {
    batch_submission_data["events"][idx]["timestamp"] =
        S4U_Simulation::getClock();
    batch_submission_data["events"][idx]["type"] = "QUERY";
    batch_submission_data["events"][idx]["data"]["requests"]
                         ["estimate_waiting_time"]["job_id"] = std::get<0>(job);
    batch_submission_data["events"][idx]["data"]["requests"]
                         ["estimate_waiting_time"]["job"]["id"] =
                             std::get<0>(job);
    batch_submission_data["events"][idx]["data"]["requests"]
                         ["estimate_waiting_time"]["job"]["res"] =
                             std::get<1>(job);
    batch_submission_data["events"][idx++]["data"]["requests"]
                         ["estimate_waiting_time"]["job"]["walltime"] =
                             std::get<3>(job);
  }

  std::string data = batch_submission_data.dump();

  auto batchsched_query_commport = S4U_CommPort::getTemporaryCommPort();

  std::shared_ptr<BatschedNetworkListener> network_listener =
      std::shared_ptr<BatschedNetworkListener>(new BatschedNetworkListener(
          this->cs->hostname, this->cs->getSharedPtr<BatchComputeService>(),
          batchsched_query_commport, std::to_string(this->batsched_port),
          data));
  ...
  for (auto job : set_of_jobs) {
    ...
    auto msg = batchsched_query_commport->getMessage<BatchQueryAnswerMessage>(
        "[This error likely means that the scheduling algorithm that Batsched "
...
void BatschedBatchScheduler::shutdown() {
  ...
  if (decision_events.size() > 0) {
    throw std::runtime_error(
        "BatschedBatchScheduler::shutdown(): Upon termination batsched "
        "returned a non-empty event set, which is unexpected");
  }
}
...
void BatschedBatchScheduler::processQueuedJobs() {
  ...
  for (i = 0, it = this->cs->batch_queue.begin();
       i < this->cs->batch_queue.size(); i++, it++) {
    auto batch_job = *it;
    ...
    batch_submission_data["events"][i]["timestamp"] =
        batch_job->getArrivalTimestamp();
    batch_submission_data["events"][i]["type"] = "JOB_SUBMITTED";
    batch_submission_data["events"][i]["data"]["job_id"] =
        std::to_string(batch_job->getJobID());
    batch_submission_data["events"][i]["data"]["job"]["id"] =
        std::to_string(batch_job->getJobID());
    batch_submission_data["events"][i]["data"]["job"]["res"] =
        num_nodes_asked_for;
    batch_submission_data["events"][i]["data"]["job"]["core"] =
        cores_per_node_asked_for;
    batch_submission_data["events"][i]["data"]["job"]["walltime"] =
        allocated_time + BATSCHED_JOB_EXTRA_TIME;
  ...
  std::shared_ptr<BatschedNetworkListener> network_listener =
      std::shared_ptr<BatschedNetworkListener>(new BatschedNetworkListener(
          this->cs->hostname, this->cs->getSharedPtr<BatchComputeService>(),
          this->cs->commport, std::to_string(this->batsched_port), data));
  network_listener->setSimulation(this->cs->getSimulation());
  network_listener->start(network_listener, true,
                          false); // Daemonized, no auto-restart
  network_listener = nullptr;     // detached mode
...
void BatschedBatchScheduler::processJobFailure(
    std::shared_ptr<BatchJob> batch_job) {
  this->notifyJobEventsToBatSched(std::to_string(batch_job->getJobID()),
                                  "TIMEOUT", "COMPLETED_FAILED", "",
                                  "JOB_COMPLETED");

  this->appendJobInfoToCSVOutputFile(batch_job.get(), "FAILED");
}

void BatschedBatchScheduler::processJobCompletion(
    std::shared_ptr<BatchJob> batch_job) {
  this->notifyJobEventsToBatSched(std::to_string(batch_job->getJobID()),
...
void BatschedBatchScheduler::processJobTermination(
    std::shared_ptr<BatchJob> batch_job) {
  // Fake it as a success
  this->notifyJobEventsToBatSched(std::to_string(batch_job->getJobID()),
                                  "SUCCESS", "COMPLETED_KILLED",
                                  "terminated by users", "JOB_COMPLETED");
...
void BatschedBatchScheduler::processJobSubmission(
    std::shared_ptr<BatchJob> batch_job) {
  // Do nothing
}
 ...
 */
void BatschedBatchScheduler::notifyJobEventsToBatSched(std::string job_id,
                                                       std::string status,
 ...
 */
void BatschedBatchScheduler::appendJobInfoToCSVOutputFile(BatchJob *batch_job,
                                                          std::string status) {
  std::string csv_file_path;
  static unsigned long job_id = 0;

  csv_file_path = this->cs->getPropertyValueAsString(
      BatchComputeServiceProperty::OUTPUT_CSV_JOB_LOG);
  ...
  if (not file) {
    throw std::runtime_error(
        "BatchComputeService::appendJobInfoToCSVOutputFile(): Unable to append "
        "to CSV output file " +
        csv_file_path +
        " (as specified by the BatchComputeServiceProperty::OUTPUT_CSV_JOB_LOG "
        "property)");
  }
  std::string csv_line = "";
  ...
  // Stretch
  double stretch =
      (batch_job->getEndingTimestamp() - batch_job->getArrivalTimestamp()) /
      (batch_job->getEndingTimestamp() - batch_job->getBeginTimestamp());
  ...
  // Waiting time
  double waiting_time =
      (batch_job->getBeginTimestamp() - batch_job->getArrivalTimestamp());
...
void BatschedBatchScheduler::startBatschedNetworkListener() {
  ...
  compute_resources_map["events"][0]["data"]["nb_resources"] =
      this->cs->nodes_to_cores_map.size();
  compute_resources_map["events"][0]["data"]["allow_time_sharing"] = false;
  //    This was the "old" batsched up until commit 39a30d83
  //      compute_resources_map["events"][0]["data"]["config"]["redis"]["enabled"]
  //      = false;
  compute_resources_map["events"][0]["data"]["config"]["redis-enabled"] = false;
  int count = 0;
  for (auto it = this->cs->nodes_to_cores_map.begin();
       it != this->cs->nodes_to_cores_map.end(); it++) {
    compute_resources_map["events"][0]["data"]["resources_data"][count]["id"] =
        std::to_string(count);
    compute_resources_map["events"][0]["data"]["resources_data"][count]
