...
WRENCH_LOG_CATEGORY(wrench_core_core_availability_time_line,
                    "Log category for CoreAvailabilityTimeLine");
 ...
 * @return
 */
bool operator==(const BatchJobSetCoreLevel &left,
 ...
 */
CoreAvailabilityTimeLine::CoreAvailabilityTimeLine(
    unsigned long max_num_nodes, unsigned long max_num_cores_per_node)
    : max_num_nodes(max_num_nodes) {
  std::set<BatchJob *> empty_set;
  this->availability_timeslots +=
      make_pair(boost::icl::interval<u_int32_t>::right_open(0, UINT32_MAX),
  ...
  while (true) {
    ...
    if (ts->first.lower() >= t) {
      ...
      this->availability_timeslots += make_pair(
          boost::icl::interval<u_int32_t>::right_open(new_left, new_right),
          job_set);
      return;
    }
  }
...
void CoreAvailabilityTimeLine::print() {
  ...
  for (auto &availability_timeslot : this->availability_timeslots) {
    ...
    for (auto const &j : availability_timeslot.second.jobs) {
      std::cerr << "j=" << j->getJobID() << "(" << j->getRequestedNumNodes()
 ...
 */
void CoreAvailabilityTimeLine::update(bool add, u_int32_t start, u_int32_t end,
  ...
  if (add) {
    this->availability_timeslots += make_pair(
        boost::icl::interval<u_int32_t>::right_open(start, end), *job_set);
  } else {
    this->availability_timeslots -= make_pair(
        boost::icl::interval<u_int32_t>::right_open(start, end), *job_set);
  }
...
CoreAvailabilityTimeLine::findEarliestStartTime(
    uint32_t duration, unsigned long num_nodes,
    unsigned long num_cores_per_node) {
  uint32_t start_time = UINT32_MAX;
  uint32_t remaining_duration = duration;

  //        WRENCH_INFO("IN FIND EARLIEST START TIME for job with %lu nodes and
  //        %lu cores per node", num_nodes, num_cores_per_node);
  // Assume all nodes are feasible
  std::set<int> possible_node_indices = this->integer_sequence;

  for (auto &availability_timeslot : this->availability_timeslots) {
    //            std::cerr << "LOOKING AT A TIME SLOT " <<
    //            availability_timeslot.first << "\n"; std::cerr << " RIGHT NOW
    //            POSSIBLE NODES: " << possible_node_indices.size() << "\n";
    // Remove infeasible hosts
    for (int i = 0; i < (int)this->max_num_nodes; i++) {
      //                std::cerr << "  - " <<
      //                availability_timeslot.second.core_utilization[i]  <<
      //                "\n";
      if (availability_timeslot.second.core_utilization[i] +
              num_cores_per_node >
          this->max_num_cores_per_node) {
        possible_node_indices.erase(i);
      }
    }

    //            std::cerr << " AFTER REMOVAL NOW POSSIBLE NODES: " <<
    //            possible_node_indices.size() << "\n";
    ...
    u_int32_t interval_length = availability_timeslot.first.upper() -
                                availability_timeslot.first.lower();
  ...
  std::vector<int> node_indices_to_return(possible_node_indices.begin(),
                                          possible_node_indices.end());
...
/**
 * @brief Get the BatchComputeService jobs in the first slot in the node
 * availability timeline
 * @return a set of BatchComputeService jobs
 */
std::set<std::shared_ptr<BatchJob>>
CoreAvailabilityTimeLine::getJobsInFirstSlot() {
