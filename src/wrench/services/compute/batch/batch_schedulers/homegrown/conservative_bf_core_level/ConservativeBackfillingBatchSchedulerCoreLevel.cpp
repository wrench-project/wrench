...
WRENCH_LOG_CATEGORY(
    wrench_core_conservative_bf_batch_scheduler_core_level,
    "Log category for ConservativeBackfillingBatchSchedulerCoreLevel");
...
ConservativeBackfillingBatchSchedulerCoreLevel::
  ...
  this->schedule = std::make_unique<CoreAvailabilityTimeLine>(
      cs->total_num_of_nodes, cs->num_cores_per_node);
}
 ...
 */
void ConservativeBackfillingBatchSchedulerCoreLevel::processJobSubmission(
    std::shared_ptr<BatchJob> batch_job) {
  WRENCH_INFO("Scheduling a new BatchComputeService job, %lu, that needs %lu "
              "nodes and %lu cores per node",
  ...
  this->schedule->setTimeOrigin(
      (u_int32_t)Simulation::getCurrentSimulatedDate());

  // Find its earliest possible start time
  auto ret_value = this->schedule->findEarliestStartTime(
      batch_job->getRequestedTime(), batch_job->getRequestedNumNodes(),
      batch_job->getRequestedCoresPerNode());
  ...
  batch_job->conservative_bf_expected_end_date =
      est + batch_job->getRequestedTime();
  WRENCH_INFO(
      "Scheduled BatchComputeService job %lu on %lu nodes from time %u to %u",
      batch_job->getJobID(), batch_job->getRequestedNumNodes(),
      batch_job->conservative_bf_start_date,
      batch_job->conservative_bf_expected_end_date);
...
void ConservativeBackfillingBatchSchedulerCoreLevel::processQueuedJobs() {
  ...
  std::set<std::shared_ptr<BatchJob>> next_jobs =
      this->schedule->getJobsInFirstSlot();
  if (next_jobs.empty()) {
  ...
  for (auto const &batch_job : next_jobs) {
    // If the job has already been allocated resources, it's already running
    // anyway
    ...
    //            WRENCH_INFO("LET'S DO IT: %lu hosts, %lu cores",
    //            num_nodes_asked_for, cores_per_node_asked_for);

    auto resources = this->scheduleOnHosts(
        num_nodes_asked_for, cores_per_node_asked_for, ComputeService::ALL_RAM);
    if (resources.empty()) {
      // Hmmm... we don't have the resources right now... we should get an
      // update soon....
      return;
      //                throw std::runtime_error("Can't run BatchComputeService
      //                job " + std::to_string(batch_job->getJobID()) +  " right
    ...
    // Start it!
    this->cs->startJob(resources, compound_job, batch_job, num_nodes_asked_for,
                       requested_time, cores_per_node_asked_for);
  }
}
...
void ConservativeBackfillingBatchSchedulerCoreLevel::compactSchedule() {
  ...
  for (auto const &batch_job : this->cs->batch_queue) {
    ...
    this->schedule->remove(batch_job->conservative_bf_start_date,
                           batch_job->conservative_bf_expected_end_date + 100,
                           batch_job);
    //            this->schedule->print();
 ...
 */
void ConservativeBackfillingBatchSchedulerCoreLevel::processJobCompletion(
    std::shared_ptr<BatchJob> batch_job) {
  WRENCH_INFO("Notified of completion of BatchComputeService job with id %lu",
              batch_job->getJobID());

  auto now = (u_int32_t)Simulation::getCurrentSimulatedDate();
  this->schedule->setTimeOrigin(now);
  this->schedule->remove(
      now, batch_job->conservative_bf_expected_end_date + 100, batch_job);
 ...
 */
void ConservativeBackfillingBatchSchedulerCoreLevel::processJobTermination(
    std::shared_ptr<BatchJob> batch_job) {
 ...
 */
void ConservativeBackfillingBatchSchedulerCoreLevel::processJobFailure(
    std::shared_ptr<BatchJob> batch_job) {
  // Just like a job Completion to me!
  this->processJobCompletion(batch_job);
}
...
/**
 * @brief Method to figure out on which actual resources a job could be
 * scheduled right now
...
ConservativeBackfillingBatchSchedulerCoreLevel::scheduleOnHosts(
    unsigned long num_nodes, unsigned long cores_per_node,
    double ram_per_node) {
  if (ram_per_node == ComputeService::ALL_RAM) {
    ram_per_node = S4U_Simulation::getHostMemoryCapacity(
        cs->available_nodes_to_cores.begin()->first);
  }
  if (cores_per_node == ComputeService::ALL_CORES) {
    cores_per_node =
        cs->available_nodes_to_cores.begin()->first->get_core_count();
  }

  if (ram_per_node > S4U_Simulation::getHostMemoryCapacity(
                         cs->available_nodes_to_cores.begin()->first)) {
    throw std::runtime_error("CONSERVATIVE_BFBatchScheduler::scheduleOnHosts():"
                             " Asking for too much RAM per host");
  }
  if (num_nodes > cs->available_nodes_to_cores.size()) {
    throw std::runtime_error("CONSERVATIVE_BFBatchScheduler::scheduleOnHosts():"
                             " Asking for too many hosts");
  }
  if (cores_per_node > (unsigned long)cs->available_nodes_to_cores.begin()
                           ->first->get_core_count()) {
    throw std::runtime_error(
        "CONSERVATIVE_BFBatchScheduler::scheduleOnHosts(): Asking for too many "
        "cores per host (asking  for " +
  ...
  //        std::map<simgrid::s4u::Host *, std::tuple<unsigned long, double>>
  //        resources = {}; std::vector<simgrid::s4u::Host *> hosts_assigned =
  //        {};
  ...
  if (host_selection_algorithm == "FIRSTFIT") {
    return HomegrownBatchScheduler::selectHostsFirstFit(
        cs, num_nodes, cores_per_node, ram_per_node);
  } else if (host_selection_algorithm == "BESTFIT") {
    return HomegrownBatchScheduler::selectHostsBestFit(
        cs, num_nodes, cores_per_node, ram_per_node);

  } else if (host_selection_algorithm == "ROUNDROBIN") {
    static unsigned long round_robin_host_selector_idx = -1;
    return HomegrownBatchScheduler::selectHostsRoundRobin(
        cs, &round_robin_host_selector_idx, num_nodes, cores_per_node,
        ram_per_node);
  } else {
    throw std::invalid_argument("ConservativeBackfillingBatchSchedulerCoreLevel"
                                "::scheduleOnHosts(): We don't support " +
...
ConservativeBackfillingBatchSchedulerCoreLevel::getStartTimeEstimates(
  ...
  for (auto const &j : set_of_jobs) {
    ...
    u_int64_t num_cores_per_host =
        this->cs->num_cores_per_node; // Ignore this one. Assume all  cores!
    if (std::get<3>(j) > UINT32_MAX) {
      throw std::runtime_error(
          "ConservativeBackfillingBatchSchedulerCoreLevel::"
          "getStartTimeEstimates(): job duration too large");
    }
    auto duration = (u_int32_t)(std::get<3>(j));

    auto ret_value = this->schedule->findEarliestStartTime(duration, num_nodes,
                                                           num_cores_per_host);
    auto est = ret_value.first;
    auto node_indices = ret_value.second;
    if (est < UINT32_MAX) {
