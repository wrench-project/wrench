...
WRENCH_LOG_CATEGORY(wrench_core_fcfs_batch_scheduler,
                    "Log category for FCFSBatchScheduler");
...
FCFSBatchScheduler::scheduleOnHosts(unsigned long num_nodes,
  ...
  if (ram_per_node > S4U_Simulation::getHostMemoryCapacity(
                         cs->available_nodes_to_cores.begin()->first)) {
    throw std::runtime_error("FCFSBatchScheduler::scheduleOnHosts(): Asking "
                             "for too much RAM per host");
  }
  if (num_nodes > cs->available_nodes_to_cores.size()) {
    throw std::runtime_error(
        "FCFSBatchScheduler::scheduleOnHosts(): Asking for too many hosts");
  }
  if (cores_per_node > (unsigned long)cs->available_nodes_to_cores.begin()
                           ->first->get_core_count()) {
    throw std::runtime_error("FCFSBatchScheduler::scheduleOnHosts(): Asking "
                             "for too many cores per host");
  }
  ...
  if (host_selection_algorithm == "FIRSTFIT") {
    ...
    throw std::invalid_argument(
        "FCFSBatchScheduler::scheduleOnHosts(): We don't support " +
        host_selection_algorithm + " as host selection algorithm");
  }
}
...
std::map<std::string, double> FCFSBatchScheduler::getStartTimeEstimates(
    std::set<std::tuple<std::string, unsigned long, unsigned long, double>>
        set_of_jobs) {
  if (cs->getPropertyValueAsString(
          BatchComputeServiceProperty::HOST_SELECTION_ALGORITHM) !=
      "FIRSTFIT") {
    throw std::runtime_error(
        "FCFSBatchScheduler::getStartTimeEstimates(): The fcfs scheduling "
        "algorithm can only provide start time estimates "
  ...
  for (auto const &job : cs->running_jobs) {
    ...
    double time_to_finish =
        std::max<double>(0, batch_job->getBeginTimestamp() +
    ...
    for (auto resource : batch_job->getResourcesAllocated()) {
      ...
      double new_available_time =
          *(core_available_times[host].begin() + num_cores - 1) +
          time_to_finish;
      for (unsigned int i = 0; i < num_cores; i++) {
        *(core_available_times[host].begin() + i) = new_available_time;
      }
      // Sort them!
      std::sort(core_available_times[host].begin(),
                core_available_times[host].end());
    }
  }
  ...
  for (auto const &job : this->cs->batch_queue) {
    ...
    for (auto h : core_available_times) {
      ...
      earliest_start_times.emplace_back(
          std::make_pair(h.first, earliest_start_time));
    }
    ...
    double earliest_job_start_time =
        (*(earliest_start_times.begin() + num_hosts - 1)).second;
    ...
    for (unsigned int i = 0; i < num_hosts; i++) {
      ...
      for (unsigned int j = 0; j < num_cores_per_host; j++) {
        *(core_available_times[host].begin() + j) =
            earliest_job_start_time + duration;
      }
      std::sort(core_available_times[host].begin(),
                core_available_times[host].end());
    }

    // Go through all hosts and make sure that no core is available before
    // earliest_job_start_time since this is a simple fcfs algorithm with no
    // "jumping ahead" of any kind
    for (auto h : cs->nodes_to_cores_map) {
      auto host = h.first;
      unsigned long num_cores = h.second;
      for (unsigned int i = 0; i < num_cores; i++) {
        if (*(core_available_times[host].begin() + i) <
  ...
  for (auto job : set_of_jobs) {
    ...
    if ((num_hosts > core_available_times.size()) ||
      ...
      for (auto h : core_available_times) {
        double earliest_start_time =
            *(h.second.begin() + num_cores_per_host - 1);
        earliest_start_times.emplace_back(
            std::make_pair(h.first, earliest_start_time));
      }
    ...
    // Note that below we translate predictions back to actual start dates given
    // the current time
    if (earliest_job_start_time > 0) {
      earliest_job_start_time = wrench::Simulation::getCurrentSimulatedDate() +
                                earliest_job_start_time;
    }
...
void FCFSBatchScheduler::processQueuedJobs() {
  while (true) {
    ...
    if (resources.empty()) {
      //                WRENCH_INFO("Can't run job %s right now",
      //                workflow_job->getName().c_str());
      break;
    }
 ...
 */
void FCFSBatchScheduler::processJobSubmission(
 ...
 * @param batch_job: a BatchComputeService job
 */
void FCFSBatchScheduler::processJobFailure(
    std::shared_ptr<BatchJob> batch_job) {
  // Do nothing
}
 ...
 * @param batch_job: a BatchComputeService job
 */
void FCFSBatchScheduler::processJobCompletion(
    std::shared_ptr<BatchJob> batch_job) {
  // Do nothing
}
 ...
 * @param batch_job: a BatchComputeService job
 */
void FCFSBatchScheduler::processJobTermination(
    std::shared_ptr<BatchJob> batch_job) {
  // Do nothing
}
