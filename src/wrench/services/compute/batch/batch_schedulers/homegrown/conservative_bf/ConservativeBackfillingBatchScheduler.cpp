...
WRENCH_LOG_CATEGORY(wrench_core_conservative_bf_batch_scheduler,
                    "Log category for ConservativeBackfillingBatchScheduler");
...
ConservativeBackfillingBatchScheduler::ConservativeBackfillingBatchScheduler(
  ...
  this->schedule =
      std::make_unique<NodeAvailabilityTimeLine>(cs->total_num_of_nodes);
}
 ...
 */
void ConservativeBackfillingBatchScheduler::processJobSubmission(
    std::shared_ptr<BatchJob> batch_job) {
  WRENCH_INFO(
      "Scheduling a new BatchComputeService job, %lu, that needs %lu nodes",
  ...
  auto est = this->schedule->findEarliestStartTime(
 ...
 */
void ConservativeBackfillingBatchScheduler::processJobCompletion(
    std::shared_ptr<BatchJob> batch_job) {
  WRENCH_INFO("Notified of completion of BatchComputeService job, %lu",
              batch_job->getJobID());
 ...
 */
void ConservativeBackfillingBatchScheduler::processJobTermination(
    std::shared_ptr<BatchJob> batch_job) {
 ...
 */
void ConservativeBackfillingBatchScheduler::processJobFailure(
    std::shared_ptr<BatchJob> batch_job) {
  // Just like a job Completion to me!
  this->processJobCompletion(batch_job);
}
...
ConservativeBackfillingBatchScheduler::scheduleOnHosts(
...
ConservativeBackfillingBatchScheduler::getStartTimeEstimates(
  ...
  for (auto const &j : set_of_jobs) {
    ...
    if (std::get<3>(j) > UINT32_MAX) {
      throw std::runtime_error(
          "ConservativeBackfillingBatchScheduler::getStartTimeEstimates(): job "
          "duration too large");
    }
