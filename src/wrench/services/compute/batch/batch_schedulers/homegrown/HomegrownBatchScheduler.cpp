...
HomegrownBatchScheduler::selectHostsFirstFit(BatchComputeService *cs,
  ...
  for (auto &available_nodes_to_core : cs->available_nodes_to_cores) {
    if (available_nodes_to_core.second >= cores_per_node) {
      ...
      resources[available_nodes_to_core.first] =
          std::make_tuple(cores_per_node, ram_per_node);
 ...
 */
std::map<simgrid::s4u::Host *, std::tuple<unsigned long, double>>
HomegrownBatchScheduler::selectHostsBestFit(BatchComputeService *cs,
                                            unsigned long num_nodes,
                                            unsigned long cores_per_node,
                                            double ram_per_node) {
  ...
  while (resources.size() < num_nodes) {
    ...
    for (auto h : cs->available_nodes_to_cores) {
      ...
      unsigned long tentative_target_num_cores =
          std::min(num_available_cores, cores_per_node);
    ...
    resources[target_host] =
        std::make_tuple(cores_per_node, ComputeService::ALL_RAM);
  }
 ...
 */
std::map<simgrid::s4u::Host *, std::tuple<unsigned long, double>>
HomegrownBatchScheduler::selectHostsRoundRobin(
    BatchComputeService *cs, unsigned long *round_robin_host_selector_idx,
  ...
  do {
    ...
    if (num_available_cores >= cores_per_node) {
      ...
      resources.insert(std::make_pair(
          cur_host, std::make_tuple(cores_per_node, ram_per_node)));
      if (++host_count >= num_nodes) {
        break;
      }
    }
