...
WRENCH_LOG_CATEGORY(wrench_core_batch_network_listener_service,
                    "Log category for Batch Network Listener Service");
 ...
 * @param batch_service_commport: the name of the commport of the
 * BatchComputeService
...
BatschedNetworkListener::BatschedNetworkListener(
    std::string hostname, std::shared_ptr<BatchComputeService> batch_service,
    ...
    : BatschedNetworkListener(hostname, batch_service, batch_service_commport,
...
int BatschedNetworkListener::main() {
  ...
  if (this->data_to_send.empty()) {
    throw std::runtime_error("BatschedNetworkListener::BatschedNetworkListener("
                             "):Network sending process has no data to send");
  }
  this->send_receive();

  WRENCH_INFO("Batch Network Listener Service on host %s terminating!",
 ...
 */
void BatschedNetworkListener::sendExecuteMessageToBatchComputeService(
    S4U_CommPort *answer_commport, std::string execute_job_reply_data) {
  this->batch_service_commport->S4U_CommPort::putMessage(
      new BatchExecuteJobFromBatSchedMessage(answer_commport,
                                             execute_job_reply_data, 0));
}
 ...
 */
void BatschedNetworkListener::sendQueryAnswerMessageToBatchComputeService(
...
void BatschedNetworkListener::send_receive() {
  ...
  memcpy(request.data(), this->data_to_send.c_str(),
         strlen(this->data_to_send.c_str()));
  ...
  if (trials == max_num_trials) {
    throw std::runtime_error(
        "Fatal Batsched Error (perhaps a failed assert in the scheduler or a "
        "time-out due to batsched taking so long... could be transient)!");
  }
  ...
  for (auto decisions : decision_events) {
    ...
    if (strcmp(decision_type.c_str(), "EXECUTE_JOB") == 0) {
      ...
      double estimated_waiting_time =
          execute_json_data["estimate_waiting_time"]["estimated_waiting_time"];
