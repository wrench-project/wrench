...
WRENCH_LOG_CATEGORY(wrench_core_bare_metal_compute_service,
                    "Log category for bare_metal_compute_service");
 ...
 */
void BareMetalComputeService::cleanup(bool has_returned_from_main,
                                      int return_value) {
  ...
  this->action_execution_service =
      nullptr; // to avoid leak due to circular refs
 ...
 */
std::tuple<std::string, unsigned long>
BareMetalComputeService::parseResourceSpec(const std::string &spec) {
...
/**
 * @brief Method the validates service-specific arguments (throws
 ...
 */
void BareMetalComputeService::validateServiceSpecificArguments(
    const std::shared_ptr<CompoundJob> &job,
    std::map<std::string, std::string> &service_specific_args) {

  auto cjob = std::dynamic_pointer_cast<CompoundJob>(job);
  auto compute_resources =
      this->action_execution_service->getComputeResources();
  // Check that each action can run w.r.t. the resource I have
  unsigned long max_cores = 0;
  double max_ram = 0;
  for (auto const &cr : compute_resources) {
    max_cores = (std::get<0>(cr.second) > max_cores ? std::get<0>(cr.second)
                                                    : max_cores);
    max_ram =
        (std::get<1>(cr.second) > max_ram ? std::get<1>(cr.second) : max_ram);
  }
  ...
  for (auto const &arg : service_specific_args) {
    if (not job->hasAction(arg.first)) {
      throw std::invalid_argument(
          "BareMetalComputeService::validateServiceSpecificArguments(): "
          "Invalid service-specific argument '{" +
  ...
  for (auto const &action : cjob->getActions()) {
    if ((service_specific_args.find(action->getName()) !=
         service_specific_args.end()) and
        (not service_specific_args.at(action->getName()).empty())) {
      std::tuple<std::string, unsigned long> parsed_spec;

      parsed_spec = BareMetalComputeService::parseResourceSpec(
          service_specific_args.at(action->getName()));

      auto target_host =
          S4U_Simulation::get_host_or_vm_by_name(std::get<0>(parsed_spec));
      ...
      if (target_host != nullptr) {
        if (compute_resources.find(target_host) == compute_resources.end()) {
          throw std::invalid_argument(
              "BareMetalComputeService::validateServiceSpecificArguments(): "
              "Invalid service-specific argument '" +
              service_specific_args.at(action->getName()) + "' for action '" +
              action->getName() + "': no such host");
        }

        if ((target_num_cores != ULONG_MAX) and
            (target_num_cores > std::get<0>(compute_resources[target_host]))) {
          throw ExecutionException(std::make_shared<NotEnoughResources>(
              job, this->getSharedPtr<BareMetalComputeService>()));
        }
      }
      ...
      if (target_num_cores != ULONG_MAX) {
        if (target_num_cores < action->getMinNumCores()) {
          throw std::invalid_argument(
              ...
              "Invalid service-specific argument '" +
              service_specific_args.at(action->getName()) + "' for action '" +
              action->getName() + "': the action requires more cores");
        }
        ...
        if (target_num_cores > action->getMaxNumCores()) {
          throw std::invalid_argument(
              ...
              "Invalid service-specific argument '" +
              service_specific_args.at(action->getName()) + "' for action '" +
              action->getName() + "': the action cannot use this many cores");
        }
 ...
 *
 *    These arguments are provided as a map of strings, indexed by action names.
 * These strings are formatted as "[hostname:][num_cores]" (e.g., "somehost:12",
 * "somehost","6", "").
 *
 *      - If a value is not provided for an action, then the service will choose
 * a host and use as many cores as possible on that host.
 *      - If a "" value is provided for an action, then the service will choose
 * a host and use as many cores as possible on that host.
 *      - If a "hostname" value is provided for an action, then the service will
 * run the action on that host, using as many of its cores as possible
 *      - If a "num_cores" value is provided for an action, then the service
 * will run that action with this many cores, but will choose the host on which
 * to run it.
 *      - If a "hostname:num_cores" value is provided for an action, then the
 * service will run that action with this many cores on that host.
 *
...
void BareMetalComputeService::submitCompoundJob(
  ...
  this->commport->putMessage(new ComputeServiceSubmitCompoundJobRequestMessage(
 ...
 * @param compute_resources: a map of <num_cores, memory_manager_service>
 * tuples, indexed by hostname, which represents the compute resources available
 * to this service.
 ...
 * @param scratch_space_mount_point: the compute service's scratch space's mount
 * point ("" means none)
...
BareMetalComputeService::BareMetalComputeService(
    ...
    : ComputeService(hostname, "bare_metal", scratch_space_mount_point) {
  initiateInstance(hostname, compute_resources, std::move(property_list),
                   std::move(messagepayload_list), nullptr);
}
 ...
 *
 * @param hostname: the name of the host on which the service should be started
 * @param compute_hosts: the names of the hosts available as compute resources
 * (the service will use all the cores and all the RAM of each host)
 * @param scratch_space_mount_point: the compute service's scratch space's mount
 * point ("" means none)
 * @param property_list: a property list ({} means "use all defaults")
 * @param messagepayload_list: a message payload list ({} means "use all
 ...
 */
BareMetalComputeService::BareMetalComputeService(
    const std::string &hostname, const std::vector<std::string> &compute_hosts,
    const std::string &scratch_space_mount_point,
    WRENCH_PROPERTY_COLLECTION_TYPE property_list,
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list)
    : ComputeService(hostname, "bare_metal", scratch_space_mount_point) {
  ...
  for (const auto &h : compute_hosts) {
    specified_compute_resources[h] =
        std::make_tuple(ComputeService::ALL_CORES, ComputeService::ALL_RAM);
  }

  initiateInstance(hostname, specified_compute_resources,
 ...
 *
 * @param hostname: the name of the host on which the service should be started
 * @param compute_resources: a list of <hostname, num_cores,
 * memory_manager_service> tuples, which represent the compute resources
...
BareMetalComputeService::BareMetalComputeService(
    ...
    : ComputeService(hostname, "bare_metal" + suffix,
                     std::move(scratch_space)) {
  initiateInstance(hostname, std::move(compute_resources),
                   std::move(property_list), std::move(messagepayload_list),
                   std::move(pj));
}
 ...
 * @brief Internal constructor
 *
 * @param hostname: the name of the host on which the job executor should be
 * started
 * @param compute_resources:: a list of <hostname, num_cores,
 * memory_manager_service> tuples, which represent the compute resources
 * available to this service
...
BareMetalComputeService::BareMetalComputeService(
    ...
    WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE messagepayload_list,
    std::shared_ptr<StorageService> scratch_space)
    : ComputeService(hostname, "bare_metal", std::move(scratch_space)) {
  initiateInstance(hostname, compute_resources, std::move(property_list),
                   std::move(messagepayload_list), nullptr);
}
 ...
 * @param compute_resources: compute_resources: a map of <num_cores,
 * memory_manager_service> pairs, indexed by hostname, which represent the
 * compute resources available to this service
 * @param property_list: a property list ({} means "use all defaults")
 * @param messagepayload_list: a message payload list ({} means "use all
 * defaults")
 * @param pj: a containing PilotJob  (nullptr if none)
...
void BareMetalComputeService::initiateInstance(
  ...
  for (const auto &h : compute_resources) {
    specified_compute_resources[S4U_Simulation::get_host_or_vm_by_name(
        h.first)] = h.second;
  }

  // Create an ActionExecutionService
  this->action_execution_service =
      std::shared_ptr<ActionExecutionService>(new ActionExecutionService(
          hostname, specified_compute_resources, nullptr,
          {
...
int BareMetalComputeService::main() {
  ...
  this->action_execution_service->setParentService(
      this->getSharedPtr<Service>());
  this->action_execution_service->setSimulation(this->simulation);
  this->action_execution_service->start(this->action_execution_service, true,
                                        false);

  if (this->getPropertyValueAsBoolean(
          BareMetalComputeServiceProperty::
              TERMINATE_WHENEVER_ALL_RESOURCES_ARE_DOWN)) {
    // Set up a service termination detector for the action execution service if
    // necessary
    auto termination_detector = std::make_shared<ServiceTerminationDetector>(
        this->hostname, this->action_execution_service, this->commport, false,
        true);
    termination_detector->setSimulation(this->simulation);
    termination_detector->start(termination_detector, true,
  ...
  if (auto msg = std::dynamic_pointer_cast<ServiceStopDaemonMessage>(message)) {
    ...
    try {
      msg->ack_commport->putMessage(new ServiceDaemonStoppedMessage(
          this->getMessagePayloadValue(BareMetalComputeServiceMessagePayload::
    ...
    return false;
                 ...
                 ComputeServiceSubmitCompoundJobRequestMessage>(message)) {
    processSubmitCompoundJob(msg->answer_commport, msg->job,
                             msg->service_specific_args);
    return true;
    ...
    return true;
                 ...
                 ActionExecutionServiceActionDoneMessage>(message)) {
    processActionDone(msg->action);
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<ServiceHasTerminatedMessage>(
                 message)) {
    if (std::dynamic_pointer_cast<ActionExecutionService>(msg->service)) {
      ...
      throw std::runtime_error("BareMetalComputeService::processNextMessage(): "
                               "Received a service termination message for "
...
/**
 * @brief Synchronously terminate a compound job previously submitted to the
 * compute service
 *
 * @param job: a compound job
 *
 ...
 * @throw std::runtime_error
 */
void BareMetalComputeService::terminateCompoundJob(
    std::shared_ptr<CompoundJob> job) {
  assertServiceIsUp();
...
void BareMetalComputeService::processSubmitCompoundJob(
  ...
  WRENCH_INFO("Asked to run compound job %s, which has %zu actions",
              job->getName().c_str(), job->getActions().size());
  for (auto const &action : job->getActions())
    WRENCH_INFO("  - ACTION %s (min #cores=%lu)", action->getName().c_str(),
                action->getMinNumCores());
  ...
  try {
    ...
    answer_commport->dputMessage(
        new ComputeServiceSubmitCompoundJobAnswerMessage(
            ...
            std::shared_ptr<FailureCause>(new ServiceIsDown(
 ...
 * @param termination_cause: termination cause (if failure notifications are
 * sent)
 */
 ...
 * sent
 */
void BareMetalComputeService::processCompoundJobTerminationRequest(
  ...
  if (this->current_jobs.find(job) == this->current_jobs.end()) {
    ...
    auto answer_message = new ComputeServiceTerminateCompoundJobAnswerMessage(
        ...
        std::shared_ptr<FailureCause>(
            new NotAllowed(this->getSharedPtr<BareMetalComputeService>(), msg)),
        this->getMessagePayloadValue(
            BareMetalComputeServiceMessagePayload::
 ...
 */
void BareMetalComputeService::
    processIsThereAtLeastOneHostWithAvailableResources(
        S4U_CommPort *answer_commport, unsigned long num_cores, double ram) {
  bool answer =
      this->action_execution_service
          ->IsThereAtLeastOneHostWithAvailableResources(num_cores, ram);
  answer_commport->dputMessage(
...
BareMetalComputeService::constructResourceInformation(const std::string &key) {
 ...
 * needed)
 */
void BareMetalComputeService::processGetResourceInformation(
...
/**
 * @brief Cleans up the scratch as I am a pilot job and I to need clean the
 * files stored by the standard jobs executed inside me
 */
void BareMetalComputeService::cleanUpScratch() {
  for (auto const &j : this->files_in_scratch) {
    for (auto const &f : j.second) {
      this->getScratch()->deleteFile(f, this->getScratch()->getBaseRootPath() +
                                            j.first->getName());
    }
  }
}
...
void BareMetalComputeService::validateProperties() {
  ...
  if ((!success) or (thread_startup_overhead < 0)) {
    throw std::invalid_argument(
        "Invalid THREAD_STARTUP_OVERHEAD property specification: " +
        this->getPropertyValueAsString(
            BareMetalComputeServiceProperty::THREAD_STARTUP_OVERHEAD));
  }
}
...
void BareMetalComputeService::dispatchReadyActions() {
  //        std::cerr << "DISPACHING READY ACTIONS: |" <<
  //        this->ready_actions.size() << " |\n";

  // Sort all the actions in the ready queue by (job.priority, action.priority,
  // action.job.submit_time, action.name)
  ...
  std::sort(this->ready_actions.begin(), this->ready_actions.end(),
            [](const std::shared_ptr<Action> &a,
               const std::shared_ptr<Action> &b) -> bool {
              if (a->getJob() != b->getJob()) {
                if (a->getJob()->getPriority() > b->getJob()->getPriority()) {
                  return true;
                } else if (a->getJob()->getPriority() <
                           b->getJob()->getPriority()) {
                  return false;
                } else if (a->getPriority() > b->getPriority()) {
                  return true;
                } else if (a->getPriority() < b->getPriority()) {
                  return false;
                } else if (a->getJob()->getSubmitDate() <
                           b->getJob()->getSubmitDate()) {
                  return true;
                  ...
                  return (unsigned long)(a->getJob().get()) >
                         (unsigned long)(b->getJob().get());
                }
              } else {
  ...
  for (auto const &action : this->ready_actions) {
    if (this->dispatched_actions.find(action) !=
        this->dispatched_actions.end()) {
      // The action has been dispatched (but its state it not set to RUNNING
      // yet, since there can be zero-size, instant communication!
      break;
    }
 ...
 * @param action
 */
void BareMetalComputeService::processActionDone(
  ...
  if (this->dispatched_actions.find(action) == this->dispatched_actions.end()) {
    WRENCH_INFO("Received a notification about action %s being done, but I "
                "don't know anything about this action - ignoring",
  ...
  try {
    if (job->hasSuccessfullyCompleted() and
        (this->num_dispatched_actions_for_cjob[job] == 0)) {
      this->current_jobs.erase(job);
 ...
 */
void BareMetalComputeService::terminateCurrentCompoundJob(
    const std::shared_ptr<CompoundJob> &job,
    ComputeService::TerminationCause termination_cause) {
  for (auto const &action : job->getActions()) {
    if (this->dispatched_actions.find(action) !=
      ...
      this->action_execution_service->terminateAction(action,
                                                      termination_cause);
    } else if (this->not_ready_actions.find(action) !=
               this->not_ready_actions.end()) {
      ...
      case ComputeService::TerminationCause::
        ...
        failure_cause =
            std::make_shared<ServiceIsDown>(job->getParentComputeService());
        break;
    ...
    } else if (std::find(this->ready_actions.begin(), this->ready_actions.end(),
 ...
 */
bool BareMetalComputeService::supportsStandardJobs() { return true; }

/**
 * @brief Returns true if the service supports compound jobs
 * @return true or false
 */
bool BareMetalComputeService::supportsCompoundJobs() { return true; }

/**
 * @brief Returns true if the service supports pilot jobs
 * @return true or false
 */
bool BareMetalComputeService::supportsPilotJobs() { return false; }

/**
 * @brief An "out of simulation time" (instant) method to check on resource
 * availability
 * @param num_cores: desired number of cores
 * @param ram: desire RAM footprint
 * @return true if there is at least one host with the available free resources,
 * false otherwise
 */
