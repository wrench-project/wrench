...
WRENCH_LOG_CATEGORY(wrench_core_bare_metal_compute_service_one_shot,
                    "Log category for bare_metal_compute_service_on_shot");
...
BareMetalComputeServiceOneShot::BareMetalComputeServiceOneShot(
    ...
    : BareMetalComputeService(hostname, std::move(compute_resources),
                              std::move(property_list),
                              std::move(messagepayload_list), std::move(pj),
                              suffix, std::move(scratch_space)),
      job(std::move(job)) {}
...
int BareMetalComputeServiceOneShot::main() {
  ...
  /** Note that the code below doesn't do any checks at all, the job had better
   * be well-formed **/
