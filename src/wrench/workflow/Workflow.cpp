...
void Workflow::clear() {
  ...
  //            //            std::cerr << "SIMULATION REMOVING FILE " <<
 ...
 * @param max_num_cores: the maximum number of cores that can be used by the
 * task (use INT_MAX for infinity)
...
std::shared_ptr<WorkflowTask> Workflow::addTask(const std::string &id,
  ...
  if (tasks.find(id) != tasks.end()) {
    throw std::invalid_argument("Workflow::addTask(): Task ID '" + id +
                                "' already exists");
  }

  // Create the WorkflowTask object
  auto task = std::shared_ptr<WorkflowTask>(new WorkflowTask(
      id, flops, min_num_cores, max_num_cores, memory_requirement));
...
void Workflow::removeFile(const std::shared_ptr<DataFile> &file) {
  if (this->task_output_files.find(file) != this->task_output_files.end()) {
    throw std::invalid_argument(
        "Workflow::removeFile(): File " + file->getID() +
        " cannot be removed because it is output of task " +
  ...
  if (this->task_input_files.find(file) != this->task_input_files.end() and
    ...
    throw std::invalid_argument(
        "Workflow::removeFile(): File " + file->getID() +
        " cannot be removed because it is input to " +
...
void Workflow::removeTask(const std::shared_ptr<WorkflowTask> &task) {
  ...
  if (tasks.find(task->id) == tasks.end()) {
    throw std::invalid_argument("Workflow::removeTask(): Task '" + task->id +
                                "' does not exist");
  }
  ...
  // Brute-force update of the top-level of all the children and the
  // bottom-level of the parents of the removed task
...
std::shared_ptr<WorkflowTask> Workflow::getTaskByID(const std::string &id) {
  if (tasks.find(id) == tasks.end()) {
    throw std::invalid_argument(
        "Workflow::getTaskByID(): Unknown WorkflowTask ID " + id);
  }
  return tasks[id];
}
 ...
 * @param redundant_dependencies: whether DAG redundant dependencies should be
 * kept in the graph
 *
 * @throw std::invalid_argument
 */
void Workflow::addControlDependency(const std::shared_ptr<WorkflowTask> &src,
                                    const std::shared_ptr<WorkflowTask> &dst,
                                    bool redundant_dependencies) {
  if ((src == nullptr) || (dst == nullptr)) {
    throw std::invalid_argument(
        "Workflow::addControlDependency(): Invalid arguments");
  }
  ...
  if (this->dag.doesPathExist(dst.get(), src.get())) {
    throw std::runtime_error(
        "Workflow::addControlDependency(): Adding dependency between task " +
        src->getID() + " and " + dst->getID() +
        " would create a cycle in the workflow graph");
  }

  if (redundant_dependencies ||
      not this->dag.doesPathExist(src.get(), dst.get())) {
    WRENCH_DEBUG("Adding control dependency %s-->%s", src->getID().c_str(),
                 dst->getID().c_str());
 ...
 */
void Workflow::removeControlDependency(
    const std::shared_ptr<WorkflowTask> &src,
    const std::shared_ptr<WorkflowTask> &dst) {
  if ((src == nullptr) || (dst == nullptr)) {
    throw std::invalid_argument(
        "Workflow::removeControlDependency(): Invalid arguments");
  }
  ...
  if (this->dag.doesEdgeExist(src.get(), dst.get())) {
    ...
    /* Update state */
    if ((dst->getState() == WorkflowTask::State::NOT_READY) and
        (dst->getInternalState() ==
         WorkflowTask::InternalState::TASK_NOT_READY)) {
      bool ready = true;
 ...
 */
unsigned long Workflow::getNumberOfTasks() { return this->tasks.size(); }
 ...
 */
bool Workflow::pathExists(const std::shared_ptr<WorkflowTask> &src,
                          const std::shared_ptr<WorkflowTask> &dst) {
 ...
 */
std::string Workflow::getName() const { return this->name; }
...
std::vector<std::shared_ptr<WorkflowTask>> Workflow::getReadyTasks() {
  std::vector<std::shared_ptr<WorkflowTask>> vector_of_ready_tasks(
      this->ready_tasks.begin(), this->ready_tasks.end());
...
Workflow::getTaskChildren(const std::shared_ptr<WorkflowTask> &task) {
  if (task == nullptr) {
    throw std::invalid_argument(
        "Workflow::getTaskChildren(): Invalid arguments");
  }
 ...
 */
long Workflow::getTaskNumberOfChildren(
    const std::shared_ptr<WorkflowTask> &task) {
  if (task == nullptr) {
    throw std::invalid_argument(
        "Workflow::getTaskNumberOfChildren(): Invalid arguments");
  }
 ...
 */
std::vector<std::shared_ptr<WorkflowTask>>
Workflow::getTaskParents(const std::shared_ptr<WorkflowTask> &task) {
  if (task == nullptr) {
    throw std::invalid_argument(
        "Workflow::getTaskParents(): Invalid arguments");
  }
 ...
 */
long Workflow::getTaskNumberOfParents(
    const std::shared_ptr<WorkflowTask> &task) {
  if (task == nullptr) {
    throw std::invalid_argument(
        "Workflow::getTaskNumberOfParents(): Invalid arguments");
  }
  return this->dag.getNumberOfParents(task.get());
}

/**
 * @brief Retrieve the list of the input files of the workflow (i.e., those
 * files that are input to some tasks but output from none)
 *
 * @return a map of files indexed by file ID
 */
std::map<std::string, std::shared_ptr<DataFile>>
Workflow::getInputFileMap() const {
  ...
  for (auto const &f : Simulation::getFileMap()) {
    ...
    if (this->task_output_files.find(f.second) !=
        this->task_output_files.end()) {
      continue;
    }
 ...
 */
std::map<std::string, std::shared_ptr<DataFile>>
Workflow::getOutputFileMap() const {
  ...
  for (auto const &f : Simulation::getFileMap()) {
    ...
    if (this->task_input_files.find(f.second) !=
            this->task_input_files.end() and
        (not this->task_input_files.at(f.second).empty())) {
      continue;
    }
 ...
 */
double
Workflow::getSumFlops(const std::vector<std::shared_ptr<WorkflowTask>> &tasks) {
 ...
 */
std::vector<std::shared_ptr<WorkflowTask>>
Workflow::getTasksInTopLevelRange(int min, int max) {
 ...
 */
std::vector<std::shared_ptr<WorkflowTask>>
Workflow::getTasksInBottomLevelRange(int min, int max) {
  std::vector<std::shared_ptr<WorkflowTask>> to_return;
  for (auto const &task : this->getTasks()) {
 ...
 */
std::map<std::string, std::shared_ptr<WorkflowTask>>
Workflow::getEntryTaskMap() const {
...
Workflow::getTaskThatOutputs(const std::shared_ptr<DataFile> &file) {
  if (this->task_output_files.find(file) == this->task_output_files.end()) {
    return nullptr;
  } else {
...
Workflow::getTasksThatInput(const std::shared_ptr<DataFile> &file) {
...
//     * @return a file
//     */
//    std::shared_ptr<DataFile> Workflow::addFile(const std::string &id, double
//    size) {
 ...
 *
 * @return a reference to the map of files in the workflow/simulation, indexed
 * by file ID
 */
