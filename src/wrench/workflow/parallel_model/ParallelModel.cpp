...
WRENCH_LOG_CATEGORY(wrench_core_parallel_model,
                    "Log category for ParallelModel");
 ...
 *         that the task is purely sequential, and setting it to 1 means that
 *the task is perfectly parallelizable.
 *
 ...
 * @param efficiency: the parallel efficiency (which does not depend on the
 *number of threads/cores).
 *
 * @return a model instance
 **/
std::shared_ptr<ParallelModel>
ParallelModel::CONSTANTEFFICIENCY(double efficiency) {
  return std::shared_ptr<ParallelModel>(
      new ConstantEfficiencyParallelModel(efficiency));
}
 ...
 *a number of threads, returns the amount of purely sequential work, in flops
 * @param lambda_per_thread: a function that, when given a total flop amount and
 *a number of threads, returns the amount of per-thread parallel work, in flops
 *
 * @return a model instance
 **/
std::shared_ptr<ParallelModel> ParallelModel::CUSTOM(
    const std::function<double(double, long)> &lambda_sequential,
    const std::function<double(double, long)> &lambda_per_thread) {
  return std::shared_ptr<ParallelModel>(
      new CustomParallelModel(lambda_sequential, lambda_per_thread));
}
