...
void wrench::DagOfTasks::removeVertex(wrench::WorkflowTask *task) {
  // Find the vertex
  if (this->task_map.find(task) == this->task_map.end() or
      this->task_map[task] >= this->task_list.size()) {
    throw std::invalid_argument("wrench::DagOfTasks::removeVertex(): Trying to "
  ...
  for (auto it = this->task_list.begin() + this->task_map[task];
       it != this->task_list.end(); ++it) {
    this->task_map[*it]--;
  }
...
void wrench::DagOfTasks::addEdge(wrench::WorkflowTask *src,
  ...
  if (this->task_map.find(src) == this->task_map.end() or
      this->task_map[src] >= this->task_list.size()) {
    throw std::invalid_argument("wrench::DagOfTasks::removeVertex(): Trying to "
                                "add an edge from a non-existing vertex");
  }
  if (this->task_map.find(dst) == this->task_map.end() or
      this->task_map[dst] >= this->task_list.size()) {
    throw std::invalid_argument("wrench::DagOfTasks::removeVertex(): Trying to "
                                "add an edge to a non-existing vertex");
  }
...
void wrench::DagOfTasks::removeEdge(wrench::WorkflowTask *src,
  ...
  if (this->task_map.find(src) == this->task_map.end() or
      this->task_map[src] >= this->task_list.size()) {
    throw std::invalid_argument("wrench::DagOfTasks::removeEdge(): Trying add "
                                "an edge from a non-existing vertex");
  }
  if (this->task_map.find(dst) == this->task_map.end() or
      this->task_map[dst] >= this->task_list.size()) {
    throw std::invalid_argument("wrench::DagOfTasks::removeEdge(): Trying add "
                                "an edge to a non-existing vertex");
  }
 ...
 */
bool wrench::DagOfTasks::doesPathExist(const wrench::WorkflowTask *src,
                                       const wrench::WorkflowTask *dst) {
  // Check that vertices exist
  if (this->task_map.find(src) == this->task_map.end() or
      this->task_map[src] >= this->task_list.size()) {
    throw std::invalid_argument("wrench::DagOfTasks::doesPathExist(): Trying "
                                "to find a path from a non-existing vertex");
  }
  if (this->task_map.find(dst) == this->task_map.end() or
      this->task_map[dst] >= this->task_list.size()) {
    throw std::invalid_argument("wrench::DagOfTasks::doesPathExist(): Trying "
                                "to find a path to a non-existing vertex");
  }
  // Find the vertices
 ...
 * @return true if there is a path between the tasks
 */
bool wrench::DagOfTasks::doesEdgeExist(const wrench::WorkflowTask *src,
                                       const wrench::WorkflowTask *dst) {
  // Check that vertices exist
  if (this->task_map.find(src) == this->task_map.end() or
      this->task_map[src] >= this->task_list.size()) {
...
long wrench::DagOfTasks::getNumberOfChildren(const WorkflowTask *task) {
  ...
  if (this->task_map.find(task) == this->task_map.end() or
    ...
    throw std::invalid_argument(
        "wrench::DagOfTasks::getNumberOfChildren(): Non-existing vertex");
  }
  auto vertex = this->task_map[task];
  boost::graph_traits<DAG>::out_edge_iterator eo, edge_end;
  long count = 0;
  for (boost::tie(eo, edge_end) = boost::out_edges(vertex, dag); eo != edge_end;
       ++eo) {
    count++;
  }
  return count;
 ...
 * @return the children
 */
std::vector<WorkflowTask *>
wrench::DagOfTasks::getChildren(const WorkflowTask *task) {
  // Find the vertex
  if (this->task_map.find(task) == this->task_map.end() or
      this->task_map[task] >= this->task_list.size()) {
    throw std::invalid_argument(
        "wrench::DagOfTasks::getChildren(): Non-existing vertex");
  }
  auto vertex = this->task_map[task];
  boost::graph_traits<DAG>::out_edge_iterator eo, edge_end;
...
long wrench::DagOfTasks::getNumberOfParents(const WorkflowTask *task) {
  ...
  if (this->task_map.find(task) == this->task_map.end() or
      this->task_map[task] >= this->task_list.size()) {
    throw std::invalid_argument(
        "wrench::DagOfTasks::getNumberOfParents(): Non-existing vertex");
  }
  auto vertex = this->task_map[task];
  boost::graph_traits<DAG>::in_edge_iterator ei, edge_end;
  long count = 0;
  for (boost::tie(ei, edge_end) = boost::in_edges(vertex, dag); ei != edge_end;
       ++ei) {
    count++;
  }
  return count;
 ...
 */
std::vector<WorkflowTask *>
wrench::DagOfTasks::getParents(const WorkflowTask *task) {
  // Find the vertex
  if (this->task_map.find(task) == this->task_map.end() or
      this->task_map[task] >= this->task_list.size()) {
    throw std::invalid_argument(
        "wrench::DagOfTasks::getParents(): Non-existing vertex");
  }
  auto vertex = this->task_map[task];
  boost::graph_traits<DAG>::in_edge_iterator ei, edge_end;
