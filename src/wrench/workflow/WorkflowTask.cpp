 ...
 * @param min_num_cores: the minimum number of cores required for running the
 * task
 * @param max_num_cores: the maximum number of cores that the task can use
 * (infinity: ULONG_MAX)
 * @param memory_requirement: memory_manager_service requirement in bytes
 */
WorkflowTask::WorkflowTask(std::string id, const double flops,
                           const unsigned long min_num_cores,
                           const unsigned long max_num_cores,
                           const double memory_requirement)
    : id(std::move(id)), color(""), flops(flops), min_num_cores(min_num_cores),
      max_num_cores(max_num_cores), memory_requirement(memory_requirement),
      execution_host(""), visible_state(WorkflowTask::State::READY),
...
void WorkflowTask::addInputFile(const std::shared_ptr<DataFile> &file) {
  ...
  if (this->workflow->task_output_files.find(file) !=
    ...
    workflow->addControlDependency(this->workflow->task_output_files[file],
                                   this->getSharedPtr());
  }
}
...
void WorkflowTask::addOutputFile(const std::shared_ptr<DataFile> &file) {
  WRENCH_DEBUG("Adding file '%s' as output t task %s", file->getID().c_str(),
               this->getID().c_str());

  // If the file is already input, complain
  if (this->input_files.find(file->getID()) != this->input_files.end()) {
    throw std::invalid_argument(
        "WorkflowTask::addOutputFile(): File ID '" + file->getID() +
        "' is already an input file of task '" + this->getID() + "'");
  }
 ...
 */
const std::string &WorkflowTask::getID() const { return this->id; }
 ...
 */
double WorkflowTask::getFlops() const { return this->flops; }
 ...
 */
void WorkflowTask::setFlops(double f) { this->flops = f; }
 ...
 * cases in which it is guaranteed that changing a task's minimum number of
 * cores after that task has been created is a valid thing to do)
 *
 ...
 *    (to be used only in very specific
 * cases in which it is guaranteed that changing a task's maximum number of
 * cores after that task has been created is a valid thing to do)
 *
 * @param num_cores: a number of cores
 */
...
/**
 * @brief Convert task state to a string (useful for output, debugging, logging,
 * etc.)
 ...
 */
std::string
WorkflowTask::stateToString(WorkflowTask::InternalState internal_state) {
 ...
 * @return a workflow
 */
Workflow *WorkflowTask::getWorkflow() const { return this->workflow; }
 ...
 * @param j: the j
 */
void WorkflowTask::setJob(Job *j) { this->job = j; }

/**
 * @brief Get the task's containing job
 * @return job: the job
 */
Job *WorkflowTask::getJob() const { return this->job; }

/**
 * @brief Get the cluster Id for the task
 * @return a cluster id, or an empty string
 */
std::string WorkflowTask::getClusterID() const { return this->cluster_id; }
 ...
 */
unsigned long WorkflowTask::getPriority() const { return this->priority; }

/**
 * @brief Set the task p
 * @param p: task p
 */
void WorkflowTask::setPriority(long p) { this->priority = p; }

/**
 * @brief Get the number of bytes read by the task
 * @return number of bytes read by the task
 */
unsigned long WorkflowTask::getBytesRead() const { return this->bytes_read; }
...
void WorkflowTask::updateStartDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::updateStartDate() cannot be called "
                             "before WorkflowTask::setStartDate()");
  }
  this->execution_history.top().task_start = date;
}
...
void WorkflowTask::setEndDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setEndDate() cannot be called "
                             "before WorkflowTask::setStartDate()");
  }
}
 ...
 *
 * @param date: the date when the computation portion of the WorkflowTask has
 * begun
 * @throws std::runtime_error
 */
void WorkflowTask::setComputationStartDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setComputationStartDate() cannot "
                             "be called before WorkflowTask::setStartDate()");
  }
}
...
void WorkflowTask::setComputationEndDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setComputationEndDate() cannot be "
                             "called before WorkflowTask::setStartDate()");
  }
}
...
void WorkflowTask::setReadInputStartDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setReadInputStartDate() cannot be "
                             "called before WorkflowTask::setStartDate()");
  }
}

/**
 * @brief Set the date when the read input portion of a WorkflowTask has
 * completed
 *
 * @param date: the date when the read input portion of a WorkflowTask has
 * completed
 * @throws std::runtime_error
 */
void WorkflowTask::setReadInputEndDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setReadInputEndDate() cannot be "
                             "called before WorkflowTask::setStartDate()");
  }
}
...
void WorkflowTask::setWriteOutputStartDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setWriteOutputStartDate() cannot "
                             "be called before WorkflowTask::setStartDate()");
  }
}

/**
 * @brief Set the date when the write output portion of a WorkflowTask has
 * completed
 *
...
void WorkflowTask::setWriteOutputEndDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setWriteOutputEndDate() cannot be "
                             "called before WorkflowTask::setStartDate()");
  }
}
...
void WorkflowTask::setFailureDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setFailureDate() cannot be called "
                             "before WorkflowTask::setStartDate()");
  }
}
...
void WorkflowTask::setTerminationDate(double date) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setTerminationDate() cannot be "
                             "called before WorkflowTask::setStartDate()");
  }
}
 ...
 *
 * @return a stack of WorkflowTaskExecution objects, one for each attempted
 * execution of the task
 */
std::stack<WorkflowTask::WorkflowTaskExecution>
WorkflowTask::getExecutionHistory() const {
 ...
 */
void WorkflowTask::incrementFailureCount() { this->failure_count++; }
...
double WorkflowTask::getStartDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().task_start
             : -1.0;
}

/**
 * @brief Get the task's most recent end date
 * @return a end date (-1 if task has not completed yet or if no execution
 * history exists for this task yet)
 */
double WorkflowTask::getEndDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().task_end
             : -1.0;
}

/**
 * @brief Get the task's most recent computation start date
 * @return the date when the computation portion of a task started (-1 if
 * computation has not started yet or if no execution history exists for this
 * task yet)
 */
double WorkflowTask::getComputationStartDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().computation_start
             : -1.0;
}

/**
 * @brief Get the task's most recent computation end date
 * @return the date when the computation portion of a task ended (-1 if
 * computation has not ended yet or if no execution history exists for this task
 * yet)
 */
double WorkflowTask::getComputationEndDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().computation_end
             : -1.0;
}

/**
 * @brief Get the task's most recent read input start date
 * @return the date when the read input portion of the task has begun (-1 if it
 * has not yet begun or if no execution history exists for this task yet)
 */
double WorkflowTask::getReadInputStartDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().read_input_start
             : -1.0;
}

/**
 * @brief Get the task's most recent read input end date
 * @return the date when the read input portion of the task has completed (-1 if
 * it has not begun or if no execution history exists for this task yet)
 */
double WorkflowTask::getReadInputEndDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().read_input_end
             : -1.0;
}

/**
 * @brief Get the task's most recent write output start date
 * @return the date when the write output portion of a task has begun (-1 if it
 * has not yet started or if no execution history exists for this task yet)
 */
double WorkflowTask::getWriteOutputStartDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().write_output_start
             : -1.0;
}

/**
 * @brief Get the task's most recent write output end date
 * @return the date when the write output portion of a task has completed (-1 if
 * it has not completed yet or if no execution history exists for this task yet)
 */
double WorkflowTask::getWriteOutputEndDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().write_output_end
             : -1.0;
}

/**
 * @brief Get the task's most recent failure date
 * @return the date when the task failed (-1 if it didn't fail or if no
 * execution history exists for this task yet)
 */
double WorkflowTask::getFailureDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().task_failed
             : -1.0;
}

/**
 * @brief Get the task's most recent termination date (when it was explicitly
 * requested to be terminated by the execution controller)
 * @return the date when the task was terminated (-1 if it wasn't terminated or
 * if not execution history exists for this task yet)
 */
double WorkflowTask::getTerminationDate() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().task_terminated
             : -1.0;
}

/**
 * @brief Update the task's top level (looking only at the parents, and updating
 * children)
 * @return the task's updated top level
 */
unsigned long WorkflowTask::updateTopLevel() {
  std::vector<std::shared_ptr<WorkflowTask>> parents =
      this->workflow->getTaskParents(this->getSharedPtr());
  if (parents.empty()) {
    this->toplevel = 0;
  } else {
    int max_toplevel = 0;
    for (const auto &parent : parents) {
      max_toplevel =
          (max_toplevel < parent->toplevel ? parent->toplevel : max_toplevel);
    }
    this->toplevel = 1 + max_toplevel;
  }
  std::vector<std::shared_ptr<WorkflowTask>> children =
      this->workflow->getTaskChildren(this->getSharedPtr());
...
/**
 * @brief Update the task's bottom level (looking only at the children, and
 * updating parents)
...
unsigned long WorkflowTask::updateBottomLevel() {
  ...
  if (children.empty()) {
    ...
    for (const auto &child : children) {
      max_bottomlevel =
          (max_bottomlevel < child->bottomlevel ? child->bottomlevel
                                                : max_bottomlevel);
    }
...
/**
 * @brief Update the task's top level recursively (to be used by the
 * updateAllTopBottomLevels method)
 */
void WorkflowTask::computeBottomLevel() {
  ...
  for (const auto &child : this->getChildren()) {
    ...
    max_child_bottom_level =
        std::max<int>(max_child_bottom_level, child->bottomlevel);
  }
  this->bottomlevel = 1 + max_child_bottom_level;
}

/**
 * @brief Update the task's bottom level recursively (to be used by the
 * updateAllTopBottomLevels method)
 */
void WorkflowTask::computeTopLevel() {
  ...
  for (const auto &parent : this->getParents()) {
    ...
    max_parent_top_level =
        std::max<int>(max_parent_top_level, parent->toplevel);
  }
  this->toplevel = 1 + max_parent_top_level;
}

/**
 * @brief Returns the task's top level (max number of hops on a reverse path up
 * to an entry task. Entry tasks have a top-level of 0)
 * @return
 */
int WorkflowTask::getTopLevel() const { return this->toplevel; }

/**
 * @brief Returns the task's bottom level (max number of hops on a path down to
 * an exit task. Exit tasks have a bottom-level of 0)
 * @return
 */
int WorkflowTask::getBottomLevel() const { return this->bottomlevel; }

/**
 * @brief Returns the name of the host on which the task has most recently been
 * executed, or "" if the task has never been executed yet. Could be a virtual
 * hostname.
 * @return hostname
 */
std::string WorkflowTask::getExecutionHost() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().execution_host
             : "";
}

/**
 * @brief Returns the name of the PHYSICAL host on which the task has most
 * recently been executed, or "" if the task has never been executed yet.
 * @return hostname
 */
std::string WorkflowTask::getPhysicalExecutionHost() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().physical_execution_host
             : "";
}

/**
 * @brief Returns the number of cores allocated for this task's most recent
 * execution or 0 if an execution attempt was never made
 * @return number of cores
 */
unsigned long WorkflowTask::getNumCoresAllocated() const {
  return (not this->execution_history.empty())
             ? this->execution_history.top().num_cores_allocated
             : 0;
}

/**
 * @brief Sets the host on which this task is running.If the hostname is a VM
 * name, then the corresponding physical host name will be set!
 * @param hostname: the host name
 */
void WorkflowTask::setExecutionHost(const std::string &hostname) {
  std::string physical_hostname;
  /** The conversion below has been removed as it makes more sense to keep the
   * virtual and the physical separate **/
  // Convert the virtual hostname to a physical hostname if needed
  if (S4U_VirtualMachine::vm_to_pm_map.find(hostname) !=
  ...
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setExecutionHost() cannot be "
                             "called before WorkflowTask::setStartDate()");
  }
}
...
void WorkflowTask::setNumCoresAllocated(unsigned long num_cores) {
  if (not this->execution_history.empty()) {
    ...
    throw std::runtime_error("WorkflowTask::setNumCoresAllocated() cannot be "
                             "called before WorkflowTask::setStartDate()");
  }
}
 ...
 */
std::string WorkflowTask::getColor() const { return this->color; }

/**
 * @brief Set the task's color
 * @param c: A color string in  "#rrggbb" format
 */
void WorkflowTask::setColor(const std::string &c) { this->color = c; }
