 ...
 * @param lambda_terminate: a lambda that implements the action's termination
 * (typically a no-op)
 */
CustomAction::CustomAction(
    ...
    : Action(name, "custom_"), ram(ram), num_cores(num_cores),
      lambda_execute(std::move(lambda_execute)),
      lambda_terminate(std::move(lambda_terminate)) {}
 ...
 */
unsigned long CustomAction::getMinNumCores() const { return this->num_cores; }

/**
 * @brief Returns the action's maximum number of required cores
 * @return a number of cores
 */
unsigned long CustomAction::getMaxNumCores() const { return this->num_cores; }

/**
 * @brief Returns the action's minimum required memory footprint
 * @return a number of bytes
 */
double CustomAction::getMinRAMFootprint() const { return this->ram; }
