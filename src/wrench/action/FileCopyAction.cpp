...
WRENCH_LOG_CATEGORY(wrench_core_file_copy_action,
                    "Log category for FileCopyAction");
 ...
 * @return the file
 */
std::shared_ptr<DataFile> FileCopyAction::getFile() const { return this->file; }
...
FileCopyAction::getDestinationFileLocation() const {
...
void FileCopyAction::execute(
  ...
  if (this->src_file_location->isScratch()) {
    ...
    this->src_file_location = FileLocation::LOCATION(
        cs->getScratch(),
        cs->getScratch()->getBaseRootPath() + this->getJob()->getName(),
        this->src_file_location->getFile());
  }
  if (this->dst_file_location->isScratch()) {
    auto cs = std::dynamic_pointer_cast<ComputeService>(
        action_executor->getActionExecutionService()->getParentService());
    this->dst_file_location = FileLocation::LOCATION(
        cs->getScratch(),
        cs->getScratch()->getBaseRootPath() + this->getJob()->getName(),
        this->src_file_location->getFile());
  }
...
bool FileCopyAction::usesScratch() const {
  return (this->src_file_location->isScratch() or
          this->dst_file_location->isScratch());
}
