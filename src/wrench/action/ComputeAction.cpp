 ...
 * @param parallel_model: the parallel model (to determine speedup vs. number of
 * cores)
 */
ComputeAction::ComputeAction(const std::string &name, double flops, double ram,
                             unsigned long min_num_cores,
                             unsigned long max_num_cores,
                             std::shared_ptr<ParallelModel> parallel_model)
    : Action(name, "compute_") {

  if ((flops < 0) || (ram < 0) || (min_num_cores < 1) ||
      (max_num_cores < min_num_cores)) {
    throw std::invalid_argument(
        "ComputeAction::ComputeAction(): invalid arguments");
  }
  this->flops = flops;
 ...
 */
double ComputeAction::getFlops() const { return this->flops; }
 ...
 */
double ComputeAction::getMinRAMFootprint() const { return this->ram; }
 ...
 */
void ComputeAction::execute(
    const std::shared_ptr<ActionExecutor> &action_executor) {
  auto num_threads = action_executor->getNumCoresAllocated();
  if ((num_threads < this->min_num_cores) ||
      (num_threads > this->max_num_cores) ||
      (action_executor->getMemoryAllocated() < this->ram)) {
    throw ExecutionException(std::shared_ptr<FailureCause>(
        new FatalFailure("Invalid resource specs for Action Executor")));
  }

  double sequential_work = this->getParallelModel()->getPurelySequentialWork(
      this->getFlops(), num_threads);
  double parallel_per_thread_work =
      this->getParallelModel()->getParallelPerThreadWork(this->getFlops(),
                                                         num_threads);
  if (action_executor->getSimulateComputationAsSleep()) {
    simulateComputationAsSleep(action_executor, num_threads, sequential_work,
                               parallel_per_thread_work);
  } else {
    simulateComputationAsComputation(action_executor, num_threads,
 ...
 */
void ComputeAction::simulateComputationAsSleep(
    const std::shared_ptr<ActionExecutor> &action_executor,
    unsigned long num_threads, double sequential_work,
    double parallel_per_thread_work) {
  // Thread startup_overhead
  S4U_Simulation::sleep(
      (double)(num_threads)*action_executor->getThreadCreationOverhead());
  // Then sleep for the computation duration
  double sleep_time =
      (sequential_work + parallel_per_thread_work) / Simulation::getFlopRate();
 ...
 */
void ComputeAction::simulateComputationAsComputation(
    const std::shared_ptr<ActionExecutor> &action_executor,
    unsigned long num_threads, double sequential_work,
    double parallel_per_thread_work) {
  try {
    S4U_Simulation::compute_multi_threaded(
        num_threads, action_executor->getThreadCreationOverhead(),
        sequential_work, parallel_per_thread_work);

  } catch (std::exception &e) {
    throw ExecutionException(
        std::shared_ptr<FailureCause>(new ComputationHasDied()));
  }
