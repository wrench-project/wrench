 ...
 * @brief Destructor
 */
Action::~Action() { UNTRACK_OBJECT("action"); }
 ...
 * @return the name
 */
const std::string &Action::getName() const { return this->name; }
 ...
 * @return the job
 */
std::shared_ptr<CompoundJob> Action::getJob() const { return this->job.lock(); }
...
void Action::setState(Action::State new_state) {
  ...
  this->job.lock()->updateStateActionMap(this->getSharedPtr(), old_state,
                                         new_state);
  //        std::cerr << "ACTION " << this->getName() << ": " <<
  //        Action::stateToString(old_state) << "-->" <<
  //        Action::stateToString(new_state) << "\n";
 ...
 */
void Action::setFailureCause(
    const std::shared_ptr<FailureCause> &failure_cause) {
  this->execution_history.top().failure_cause = failure_cause;
}

/**
 * @brief Sets the action's execution hosts (and the action's physical execution
 * host)
...
void Action::setExecutionHost(const std::string &host) {
  ...
  if (S4U_VirtualMachine::vm_to_pm_map.find(host) !=
...
void Action::updateState() {
  ...
  if (this->execution_history.top().state != Action::State::NOT_READY and
 ...
 */
unsigned long Action::getMinNumCores() const { return 0; }

/**
 * @brief Get the maximum number of cores that can be used to execute the action
 * @return a number of cores
 */
unsigned long Action::getMaxNumCores() const { return 0; }

/**
 * @brief Get the minimum required amount of RAM to execute the action
 * @return a number of bytes
 */
double Action::getMinRAMFootprint() const { return 0.0; }
 ...
 * @param p: a priority
 */
void Action::setPriority(double p) { this->priority = p; }

/**
 * @brief Get the action's priority
 * @return a priority
 */
double Action::getPriority() const { return this->priority; }
 ...
 */
std::string
Action::getActionTypeAsString(const std::shared_ptr<Action> &action) {
 ...
 */
bool Action::usesScratch() const { return false; }
