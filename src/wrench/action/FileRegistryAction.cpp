...
WRENCH_LOG_CATEGORY(wrench_core_file_registry_action,
                    "Log category for FileRegistryAction");
 ...
 */
FileRegistryAction::FileRegistryAction(
    FileRegistryAction::Type type, const std::string &name,
    std::shared_ptr<FileRegistryService> file_registry_service,
    std::shared_ptr<FileLocation> file_location)
    : Action(name, "file_registry"), type(type),
...
FileRegistryAction::getFileRegistryService() const {
 ...
 */
void FileRegistryAction::execute(
 ...
 * @param action_executor: the executor that executes this action
 */
void FileRegistryAction::terminate(
    const std::shared_ptr<ActionExecutor> &action_executor) {
  // Nothing to do
}
