...
WRENCH_LOG_CATEGORY(wrench_core_file_read_action,
                    "Log category for FileReadAction");
 ...
 * you)
 * @param file_locations: the locations to read from (will be tried in order
 * until one succeeds)
 * @param num_bytes_to_read: the number of bytes to read (if < 0: read the whole
 * file)
 */
FileReadAction::FileReadAction(
    ...
    : Action(name, "file_read_"), file_locations(file_locations) {

  this->file = file_locations.at(0)->getFile();
  for (auto const &l : file_locations) {
    if (l->getFile() != this->file) {
      throw std::invalid_argument("FileReadAction::FileReadAction(): All file "
  ...
  if (num_bytes_to_read < 0.0) {
    ...
    throw std::invalid_argument(
        "FileReadAction::FileReadAction(): cannot create a file read action "
        "that would read more bytes than the file size");
  }
}
 ...
 * @return the file
 */
std::shared_ptr<DataFile> FileReadAction::getFile() const { return this->file; }
...
FileReadAction::getFileLocations() const {
...
void FileReadAction::execute(
  ...
  for (unsigned long i = 0; i < this->file_locations.size(); i++) {
    if (this->file_locations[i]->isScratch()) {
      ...
      this->file_locations[i] = FileLocation::LOCATION(
          cs->getScratch(),
          cs->getScratch()->getBaseRootPath() + this->getJob()->getName(),
          this->file_locations[i]->getFile());
    }
  }
  // File read
  for (unsigned long i = 0; i < this->file_locations.size(); i++) {
    try {
      ...
      StorageService::readFileAtLocation(this->file_locations[i],
                                         this->num_bytes_to_read);
      continue;
...
bool FileReadAction::usesScratch() const {
  return std::any_of(this->file_locations.begin(), this->file_locations.end(),
