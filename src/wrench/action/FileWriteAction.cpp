...
WRENCH_LOG_CATEGORY(wrench_core_file_write_action,
                    "Log category for FileWriteAction");
...
FileWriteAction::FileWriteAction(const std::string &name,
    ...
    : Action(name, "file_write_"), file_location(std::move(file_location)) {}
 ...
 * @param action_executor: the executor that executes this action
 */
void FileWriteAction::terminate(
    const std::shared_ptr<ActionExecutor> &action_executor) {
  // Nothing to do
}
