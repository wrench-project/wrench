 ...
 */
MPIAction::MPIAction(
    const std::string &name, unsigned long num_processes,
    unsigned long num_cores_per_process,
    std::function<void(const std::shared_ptr<ExecutionController> &controller)>
        lambda_mpi)
    : Action(name, "mpi_"), num_processes(num_processes),
      num_cores_per_process(num_cores_per_process), lambda_mpi(lambda_mpi) {
 ...
 */
void MPIAction::execute(
    const std::shared_ptr<ActionExecutor> &action_executor) {

  // Make A COPY of the list of usable hosts
  auto resources_ref =
      action_executor->getActionExecutionService()->getComputeResources();
  std::map<simgrid::s4u::Host *, std::tuple<unsigned long, double>> resources(
      resources_ref.begin(), resources_ref.end());
  ...
  while (simgrid_hosts.size() != this->num_processes) {
    ...
    for (auto const &r : resources) {
      ...
      if (num_cores >= this->num_cores_per_process) {
        ...
        resources[r.first] = std::make_tuple(std::get<0>(resources[r.first]) -
                                                 this->num_cores_per_process,
                                             std::get<1>(resources[r.first]));
  ...
  // Start actors
  auto meta_lambda =
      [this](const std::shared_ptr<ActionExecutor> &action_executor) {
        // Get a commport
        auto commport = S4U_CommPort::getTemporaryCommPort();
        S4U_Daemon::map_actor_to_recv_commport
            [simgrid::s4u::this_actor::get_pid()] = commport;
        // Create and start my own Controller
        auto mpi_private_execution_controller =
            std::make_shared<MPIPrivateExecutionController>(
                action_executor->hostname, "mpi_private");
        mpi_private_execution_controller->setSimulation(
            action_executor->getSimulation());
        mpi_private_execution_controller->start(
            mpi_private_execution_controller, true,
  ...
  SMPI_app_instance_start(
      ("MPI_Action_" + std::to_string(simgrid::s4u::this_actor::get_pid()))
          .c_str(),
      MPIProcess(meta_lambda, barrier, action_executor), simgrid_hosts);
  barrier->wait();
}
 ...
 */
unsigned long MPIAction::getMinNumCores() const { return 0; }

/**
 * @brief Returns the action's maximum number of required cores
 * @return Always returns 0 for an MPIAction
 */
unsigned long MPIAction::getMaxNumCores() const { return 0; }

/**
 * @brief Returns the action's minimum required memory footprint
 * @return Always returns 0 for an MPIAction
 */
double MPIAction::getMinRAMFootprint() const { return 0.0; }

/**
 * @brief Returns the number of (MPI) processes to be executed
 * @return a number of processes
 */
unsigned long MPIAction::getNumProcesses() const { return this->num_processes; }
