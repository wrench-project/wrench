...
WRENCH_LOG_CATEGORY(wrench_core_file_delete_action,
                    "Log category for FileDeleteAction");
...
FileDeleteAction::FileDeleteAction(const std::string &name,
    ...
    : Action(name, "file_delete_"), file(std::move(file)),
 ...
 * @param action_executor: the executor that executes this action
 */
void FileDeleteAction::terminate(
    const std::shared_ptr<ActionExecutor> &action_executor) {
  // Nothing to do
}
