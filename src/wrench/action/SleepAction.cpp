 ...
 */
SleepAction::SleepAction(const std::string &name, double sleep_time)
    : Action(name, "sleep_"), sleep_time(sleep_time) {
  if (sleep_time < 0) {
    throw std::invalid_argument(
        "SleepAction::SleepAction(): Invalid (negative) sleep time");
  }
}
 ...
 */
double SleepAction::getSleepTime() const { return this->sleep_time; }
