...
//     * (Essentially Cut-And-Pasted from
//     simgrid/src/surf/xml/surfxml_sax_cb.cpp)
//     */
//    class unit_scale : public WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE {
//    public:
//        using std::unordered_map<std::string, double>::unordered_map;
//        // tuples are : <unit, value for unit, base (2 or 10), true if
//        abbreviated> explicit
//        unit_scale(std::initializer_list<std::tuple<const std::string, double,
//        int, bool>> generators);
//    };
...
UnitParser::unit_scale::unit_scale(
  ...
  for (const auto &gen : generators) {
    ...
    case 2:
      mult = 1024.0;
      prefixes = abbrev
                     ? std::vector<std::string>{"Ki", "Mi", "Gi", "Ti",
                                                "Pi", "Ei", "Zi", "Yi"}
                     : std::vector<std::string>{"kibi", "mebi", "gibi", "tebi",
                                                "pebi", "exbi", "zebi", "yobi"};
      break;
    case 10:
      mult = 1000.0;
      prefixes =
          abbrev
              ? std::vector<std::string>{"k", "M", "G", "T", "P", "E", "Z", "Y"}
              : std::vector<std::string>{"kilo", "mega", "giga", "tera",
                                         "peta", "exa",  "zeta", "yotta"};
      break;
    default:
      throw std::runtime_error(
          "UnitParser::unit_scale::unit_scale(): The impossible has happened");
    }
    emplace(unit, value);
...
double UnitParser::parseValueWithUnit(const std::string &string,
                                      const UnitParser::unit_scale &units,
  ...
  if (u == units.end())
    throw std::runtime_error("Unknown unit '" + std::string(ptr) +
                             " when parsing value " + string);
  return res * u->second;
}

/**
 * @brief Given a string size specification with units (e.g., "13Mb") return the
 * size in bytes
...
double UnitParser::parse_size(const std::string &string) {
  static const UnitParser::unit_scale units{
      std::make_tuple("b", 0.125, 2, true),
      std::make_tuple("b", 0.125, 10, true), std::make_tuple("B", 1.0, 2, true),
...
/**
 * @brief Given a string compute speed specification with units (e.g., "3f",
 * "10Gf") return the size in bytes
 * @param string: the size specification
 * @return the size in bytes
 *
 * @throws std::invalid_argument
...
/**
 * @brief Given a string bandwidth specification with units (e.g., "3Mbps",
 * "40MiBps") return the size in bytes
...
double UnitParser::parse_bandwidth(const std::string &string) {
  static const UnitParser::unit_scale units{
      std::make_tuple("bps", 0.125, 2, true),
...
/**
 * @brief Given a string time with units (e.g., "10us", "10ms") return the time
 * in seconds
