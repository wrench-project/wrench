...
WRENCH_LOG_CATEGORY(wrench_core_message_manager,
                    "Log category for MessageManager");

namespace wrench {

// TODO: At some point, we may want to make this with only unique pointers...

std::unordered_map<const S4U_CommPort *,
 ...
 */
void MessageManager::manageMessage(const S4U_CommPort *commport,
                                   SimulationMessage *msg) {
  if (msg == nullptr) {
    throw std::runtime_error("MessageManager::manageMessage()::Null Message "
                             "cannot be managed by MessageManager");
  }
  if (messages.find(commport) == messages.end()) {
    messages.insert({commport, {}});
  }
  messages[commport].insert(msg);
  //      WRENCH_INFO("MESSAGE_MANAGER: INSERTING [%s]:%s (%lu)",
  //      commport.c_str(), msg->getName().c_str(), (unsigned long)msg);
}

/**
 * @brief Clean up messages for a given commport (so as to free up
...
/**
 * @brief Clean up all the messages that MessageManager has stored (so as to
 * free up memory_manager_service)
 */
 ...
 * @param msg: the message
 */
void MessageManager::removeReceivedMessage(const S4U_CommPort *commport,
                                           SimulationMessage *msg) {
  //      if (messages.find(commport) != messages.end()) {
  //        if (messages[commport].find(msg) != messages[commport].end()) {
  //            WRENCH_INFO("MESSAGE_MANAGER: REMOVING [%s]:%s (%lu)",
  //            commport.c_str(), msg->getName().c_str(), (unsigned long)msg);
