...
void TerminalOutput::setThisProcessLoggingColor(Color color) {
  TerminalOutput::colormap[simgrid::s4u::Actor::self()] =
      TerminalOutput::color_codes[color];
}
 ...
 */
void TerminalOutput::disableColor() { TerminalOutput::color_enabled = false; }

/**
 * @brief Get the current output color ASCII code sequence for the current
 * process
...
std::string TerminalOutput::getThisProcessLoggingColor() {
  if (simgrid::s4u::this_actor::is_maestro() ||
      (TerminalOutput::colormap.find(simgrid::s4u::Actor::self()) ==
