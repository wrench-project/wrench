...
namespace wrench {
	...
	int MulticoreJobExecutor::runStandardJob(StandardJob *job) {
		...
		if (this->getProperty(ComputeService::SUPPORTS_STANDARD_JOBS) != "yes") {
			throw WRENCHException(
					"Implementation error: this compute service should have the SUPPORTS_STANDARD_JOBS property set to 'yes'");
		}
	 ...
	 *
	 * @param job is a pointer the pilot job
	 * @return 0 on success
	 */
	int MulticoreJobExecutor::runPilotJob(PilotJob *job) {
		...
		if (this->getProperty(ComputeService::SUPPORTS_PILOT_JOBS) != "yes") {
			throw WRENCHException(
					"Implementation error: this compute service should have the SUPPORTS_PILOT_JOBS property set to 'yes'");
		}
