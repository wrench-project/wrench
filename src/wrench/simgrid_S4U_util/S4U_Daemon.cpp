...
std::unordered_map<aid_t, S4U_CommPort *>
    S4U_Daemon::map_actor_to_recv_commport;
std::unordered_map<aid_t, std::set<simgrid::s4u::MutexPtr>>
    S4U_Daemon::map_actor_to_held_mutexes;
 ...
 * @param process_name_prefix: the prefix of the name of the simulated
 * process/actor
 */
S4U_Daemon::S4U_Daemon(const std::string &hostname,
                       const std::string &process_name_prefix) {
  if (not simgrid::s4u::Engine::is_initialized()) {
    throw std::runtime_error(
        "Simulation must be initialized before services can be created");
  }

  if (S4U_Simulation::get_host_or_vm_by_name_or_null(hostname) == nullptr) {
    throw std::invalid_argument(
        "S4U_Daemon::S4U_Daemon(): Unknown host or vm '" + hostname + "'");
  }
...
S4U_Daemon::~S4U_Daemon() {
  //        WRENCH_INFO("IN DAEMON DESTRUCTOR (%s, %p)'",
  //        this->getName().c_str(), this->s4u_actor.get());
...
//        if
//        (S4U_Daemon::map_actor_to_held_mutexes.find(simgrid::s4u::this_actor::get_pid())
//        != S4U_Daemon::map_actor_to_held_mutexes.end()) {
//            for (auto const &mutex :
//            S4U_Daemon::map_actor_to_held_mutexes[simgrid::s4u::this_actor::get_pid()])
//            {
//                mutex->unlock();
//            }
...
/**
 * @brief Cleanup function called when the daemon terminates (for whatever
 * reason). The default behavior is to throw an exception if the host is off.
 * This method should be overriden in a daemons implements some fault-tolerant
 * behavior, or is naturally tolerant.
 *
 * @param has_returned_from_main: whether the daemon returned from main() by
 * itself
 * @param return_value: the return value from main (if has_terminated_cleanly is
 * true)
 *
 * @throw std::runtime_error
 */
void S4U_Daemon::cleanup(bool has_returned_from_main, int return_value) {
  //        WRENCH_INFO("IN S4U_Daemon::cleanup() for %s",
  ...
  if ((not has_returned_from_main) and
      (not S4U_Simulation::isHostOn(hostname))) {
    throw std::runtime_error(
        "S4U_Daemon::cleanup(): This daemon has died due to a failure of its "
        "host, but does not override cleanup() "
        "(so that is can implement fault-tolerance or explicitly ignore "
        "fault) ");
  }
}
 ...
 * @return a state
 */
S4U_Daemon::State S4U_Daemon::getState() { return this->state; }

/**
 * @brief Delete the daemon's life-saver (use at your own risks, if your not the
 * Simulation class)
 */
 ...
 * @param auto_restart: whether the S4U actor should automatically restart after
 * a host reboot
 *
 * @throw std::runtime_error
...
void S4U_Daemon::startDaemon(bool daemonized, bool auto_restart) {
  ...
  if (not this->life_saver) {
    throw std::runtime_error("S4U_Daemon::startDaemon(): You must call "
                             "createLifeSaver() before calling startDaemon()");
  }

  // Check that the simulation pointer is set
  if (not this->simulation) {
    throw std::runtime_error("S4U_Daemon::startDaemon(): You must set the "
                             "simulation field before calling startDaemon() (" +
                             this->getName() + ")");
  }
  ...
  this->has_returned_from_main = false;
  //        this->commport = this->initial_commport + "_#" +
  //        std::to_string(this->num_starts);
  // Create the s4u_actor

  try {
    this->s4u_actor = simgrid::s4u::Actor::create(
        this->process_name.c_str(), this->host, S4U_DaemonActor(this));
  } catch (simgrid::Exception &e) {
    throw std::runtime_error("S4U_Daemon::startDaemon(): SimGrid actor "
                             "creation failed... shouldn't happen.");
  }

  //        WRENCH_INFO("STARTED ACTOR %p (%s)", this->s4u_actor.get(),
  //        this->process_name.c_str());

  // nullptr is returned if the host is off (not the current behavior in
  // SimGrid... just paranoid here)
  if (this->s4u_actor == nullptr) {
    throw ExecutionException(std::make_shared<HostError>(hostname));
  }

  // This test here is critical. It's possible that the created actor above
  // returns right away, in which case calling daemonize() on it cases the
  // calling actor to terminate immediately. This is a weird simgrid::s4u
  // behavior/bug, that may be fixed at some point, but this test saves us for
  // now.
...
void S4U_Daemon::setupOnExitFunction() {
  this->s4u_actor->on_exit([this](bool failed) {
    ...
    //          std::cerr << "*** NUM_NON_DAEMIONIZED_ACTORS_RUNNING = " <<
    //          S4U_Daemon::num_non_daemonized_actors_running << "\n";
    // Set state to down
    this->state = S4U_Daemon::State::DOWN;
    // Call cleanup
    this->cleanup(this->hasReturnedFromMain(), this->getReturnValue());
    // Free memory_manager_service for the object unless the service is set to
    // auto-restart
    if ((S4U_Daemon::num_non_daemonized_actors_running == 0) or
        (not this->isSetToAutoRestart())) {
 ...
 */
bool S4U_Daemon::isSetToAutoRestart() const { return this->auto_restart; }

/**
 * @brief Return the daemonized status of the daemon
 * @return true or false
 */
bool S4U_Daemon::isDaemonized() const { return this->daemonized; }

/**
 * @brief Method that run's the user-defined main method (that's called by the
 * S4U actor class)
 */
void S4U_Daemon::runMainMethod() {
  ...
  S4U_Daemon::map_actor_to_recv_commport.erase(
 ...
 *
 * @return true if actor was killed, false if not (e.g., daemon was already
 * dead)
...
bool S4U_Daemon::killActor() {
  ...
  if ((this->s4u_actor != nullptr) && (not this->has_returned_from_main)) {
    try {
      ...
      throw std::runtime_error(
          "simgrid::s4u::Actor::kill() failed... this shouldn't have happened");
    }
    // Really not sure why now we're setting this to true... but if we don't
    // some tests fail. Something to investigate at some point
    this->has_returned_from_main = true;
    return true;
  } else {
    return false;
 ...
 *
 * @return a pair <A,B> where A is boolean (true if the daemon terminated
 * cleanly (i.e., main() returned), or false otherwise) and B is the int
 * returned from main() (if main returned).
 */
std::pair<bool, int> S4U_Daemon::join() {
  if (this->s4u_actor != nullptr) {
    this->s4u_actor->join();
  } else {
    throw std::runtime_error("S4U_Daemon::join(): Fatal error: this->s4u_actor "
                             "shouldn't be nullptr");
  }
  //        WRENCH_INFO("JOIN ON '%s' HAS RETURNED: %d %d",
  //        this->getName().c_str(), this->hasReturnedFromMain(),
  //        this->getReturnValue());
  return std::make_pair(this->hasReturnedFromMain(), this->getReturnValue());
}

/**
 * @brief Returns true if the daemon has returned from main() (i.e., not
 * brutally killed)
...
/**
 * @brief Returns the value returned by main() (if the daemon has returned from
 * main)
 * @return The return value
 */
int S4U_Daemon::getReturnValue() const { return this->return_value; }

/**
 * @brief Retrieve the process name
 * @return the name
 */
std::string S4U_Daemon::getName() const { return this->process_name; }
...
void S4U_Daemon::createLifeSaver(std::shared_ptr<S4U_Daemon> reference) {
  if (this->life_saver) {
    throw std::runtime_error(
        "S4U_Daemon::createLifeSaver(): Lifesaver already created!");
  }
 ...
 * @return a simulation
 */
Simulation *S4U_Daemon::getSimulation() { return this->simulation; }
...
S4U_CommPort *S4U_Daemon::getRunningActorRecvCommPort() {
  ...
  return S4U_Daemon::map_actor_to_recv_commport
      [simgrid::s4u::this_actor::get_pid()];
}
