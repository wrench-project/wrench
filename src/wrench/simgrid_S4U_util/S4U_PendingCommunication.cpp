...
WRENCH_LOG_CATEGORY(wrench_core_pending_communication,
                    "Log category for Pending Communication");
...
S4U_PendingCommunication::wait(double timeout) {
  ...
  if (this->operation_type ==
      S4U_PendingCommunication::OperationType::RECEIVING) {

    //        if (log) WRENCH_DEBUG("Getting a message from commport '%s' with
    //        timeout %lf sec", this->comm_ptr->get_cname(), timeout);
    ...
    try {
      ...
      throw ExecutionException(std::make_shared<NetworkError>(
          NetworkError::RECEIVING, NetworkError::TIMEOUT,
          this->commport->get_name(), ""));
      ...
      if (failed_recv == comm_ptr) {
        ...
        throw ExecutionException(std::make_shared<NetworkError>(
            ...
            this->comm_ptr->get_name(), ""));
      } else {
        comm_ptr->cancel();
        throw ExecutionException(std::make_shared<wrench::FatalFailure>(
            "A communication on a MQ should never fail"));
      }
    }
    ...
    WRENCH_DEBUG("Received a '%s' message from commport '%s'",
                 this->simulation_message->getName().c_str(),
    ...
    if (this->comm_ptr) {
      try {
        ...
        if (this->operation_type ==
            S4U_PendingCommunication::OperationType::SENDING) {
          throw ExecutionException(std::make_shared<NetworkError>(
              NetworkError::OperationType::SENDING, NetworkError::FAILURE,
              this->commport->s4u_mb->get_name(), ""));
        } else {
          throw ExecutionException(std::make_shared<NetworkError>(
              NetworkError::OperationType::RECEIVING, NetworkError::FAILURE,
              this->commport->s4u_mb->get_name(), ""));
        }
      } catch (simgrid::TimeoutException &e) {
        if (this->operation_type ==
            S4U_PendingCommunication::OperationType::SENDING) {
          throw ExecutionException(std::make_shared<NetworkError>(
              NetworkError::OperationType::SENDING, NetworkError::TIMEOUT,
              this->commport->s4u_mb->get_name(), ""));
        } else {
          throw ExecutionException(std::make_shared<NetworkError>(
              NetworkError::OperationType::RECEIVING, NetworkError::TIMEOUT,
              this->commport->s4u_mb->get_name(), ""));
        }
      }
 ...
 *
 * @return the index of the comm to which something happened (success or
 * failure)
 *
...
unsigned long S4U_PendingCommunication::waitForSomethingToHappen(
  ...
  return S4U_PendingCommunication::waitForSomethingToHappen(raw_pointer_comms,
                                                            timeout);
}
 ...
 *
 * @return the index of the comm to which something happened (success or
 * failure), or ULONG_MAX if nothing happened before the timeout expired.
 *
 * @throw std::invalid_argument
 */
unsigned long S4U_PendingCommunication::waitForSomethingToHappen(
  ...
  if (pending_comms.empty()) {
    throw std::invalid_argument("S4U_PendingCommunication::"
                                "waitForSomethingToHappen(): invalid argument");
  }
  ...
  throw std::runtime_error("S4U_PendingCommunication::waitForSomethingToHappen("
