...
WRENCH_LOG_CATEGORY(wrench_core_s4u_virtual_machine,
                    "Log category for S4U_VirtualMachine");
...
S4U_VirtualMachine::S4U_VirtualMachine(
    const std::string &vm_name, unsigned long num_cores, double ram_memory,
...
std::string S4U_VirtualMachine::getPhysicalHostname() {
  ...
  //           throw
  //           std::runtime_error("S4U_VirtualMachine::getPhysicalHostname(): no
  //           physical host for VM '" + this->vm_name + "'");
  //        } else {
 ...
 */
unsigned long S4U_VirtualMachine::getNumCores() { return this->num_cores; }

/**
 * @brief Get the memory_manager_service consumption
 * @return the memory_manager_service consumption
 */
double S4U_VirtualMachine::getMemory() { return this->ram_memory; }
...
void S4U_VirtualMachine::start(std::string &pm_name) {
  if (this->state != State::DOWN) {
    throw std::runtime_error(
        "S4U_VirtualMachine::suspend(): Cannot suspend a VM that's in state " +
  ...
  if (physical_host == nullptr) {
    throw std::invalid_argument(
        "S4U_VirtualMachine::start(): unknown physical host '" + pm_name + "'");
  }

  this->vm = physical_host->create_vm(this->vm_name, (int)this->num_cores,
...
void S4U_VirtualMachine::resume() {
  if (this->state != State::SUSPENDED) {
    throw std::runtime_error(
        "S4U_VirtualMachine::resume(): Cannot resume a VM that's in state " +
        this->getStateAsString());
  }
  this->vm->resume();
  this->state = State::RUNNING;
...
void S4U_VirtualMachine::shutdown() {
  if (this->state == State::DOWN) {
    throw std::runtime_error("S4U_VirtualMachine::shutdown(): Cannot shutdown "
                             "a VM that's in state " +
                             this->getStateAsString());
  }
  this->state =
      State::DOWN; // Doing this first before a possible context switch
  this->vm->shutdown();
  this->vm->destroy();
  this->pm_name = "";
 ...
 * @return a state
 */
S4U_VirtualMachine::State S4U_VirtualMachine::getState() { return this->state; }
...
void S4U_VirtualMachine::migrate(const std::string &dest_pm_name) {
  ...
  S4U_VirtualMachine::vm_to_pm_map[this->vm_name] = this->pm_name;
  WRENCH_INFO("%s migrated to %s in %g s", src_pm_hostname.c_str(),
              dest_pm_name.c_str(), mig_end - mig_sta);
}

/**
 * @brief Get the property list for the bare-metal compute service that is to
...
/**
 * @brief Get the message payload list for the bare-metal compute service that
 * will run on the VM ({} means "use all defaults")
 * @return a message payload list
 */
WRENCH_MESSAGE_PAYLOADCOLLECTION_TYPE
S4U_VirtualMachine::getMessagePayloadList() {
