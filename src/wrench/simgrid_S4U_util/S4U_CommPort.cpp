...
/**
 * @brief Helper method that avoids calling WRENCH_DEBUG from a .h file and do
 * the logging for the templated getMessage() method. It also has the added
 * bonus of checking for inheritance
 *
 * @param type: a pointer to the message so we have its type
 * @param id: an integer id
 *
 */
void S4U_CommPort::templateWaitingLog(const std::string &type,
                                      unsigned long long id) {

  WRENCH_DEBUG(
      "Waiting for message of type <%s> from commport '%s'.  Request ID: %llu",
      type.c_str(), this->get_cname(), id);
}
 ...
 *
 */
void S4U_CommPort::templateWaitingLogUpdate(const std::string &type,
                                            unsigned long long id) {

  WRENCH_DEBUG(
      "Received a message of type <%s> from commport '%s'.  Request ID: %llu",
      type.c_str(), this->get_cname(), id);
}
 ...
 * @return the message, or nullptr (in which case it's likely a brutal
 * termination)
 *
 ...
 * @throw std::shared_ptr<NetworkError>
 */
std::unique_ptr<SimulationMessage> S4U_CommPort::getMessage(double timeout,
                                                            bool log) {
  if (this == S4U_CommPort::NULL_COMMPORT) {
    throw std::invalid_argument(
        "S4U_CommPort::getMessage(): Cannot be called with NULL_COMMPORT");
  }

  //        if (log) WRENCH_DEBUG("Getting a message from commport '%s' with
  //        timeout %lf sec", this->get_cname(), timeout);
  if (true)
    WRENCH_DEBUG("Getting a message from commport '%s' with timeout %lf sec",
                 this->get_cname(), timeout);

  simgrid::s4u::ActivitySet pending_receives;
  if (not this->mb_comm_posted) {
    WRENCH_DEBUG("POSTING GET ASYNC ON MB for %s: %p", this->get_cname(),
                 (this->mb_comm.get()));
    this->mb_comm = this->s4u_mb->get_async<SimulationMessage>(&this->msg_mb);
    this->mb_comm_posted = true;
  } else {
    WRENCH_DEBUG("GET ASYNC ON MB ALREADY POSTED FROM BEFORE FOR %s: %p",
                 this->get_cname(), this->mb_comm.get());
  }
  if (not this->mq_comm_posted) {
    WRENCH_DEBUG("POSTING GET ASYNC ON MQ for %s: %p", this->get_cname(),
                 (this->mq_comm.get()));
    this->mq_comm = this->s4u_mq->get_async<SimulationMessage>(&this->msg_mq);
    this->mq_comm_posted = true;
  } else {
    WRENCH_DEBUG("GET ASYNC ON MQ ALREADY POSTED FROM BEFORE FOR %s: %p",
                 this->get_cname(), this->mq_comm.get());
  }
  ...
  //                    this->mb_comm.get(),
  //                    this->mb_comm->get_mailbox()->get_cname(),
  //                    this->mq_comm.get(),
  //                    this->mq_comm->get_queue()->get_cname());
  ...
  try {
    ...
    throw ExecutionException(std::make_shared<NetworkError>(
        NetworkError::RECEIVING, NetworkError::TIMEOUT, this->name, ""));
  ...
  if (finished_recv == this->mb_comm) {
    ...
    throw std::runtime_error(
        "S4U_CommPort::getMessage(): unknown completed communication - this "
        "should never happen: " +
        std::to_string((unsigned long)(finished_recv.get())) + "  " +
        finished_recv->get_name());
  }
  ...
  WRENCH_DEBUG("Received a '%s' message from commport '%s' (%lf, %p bytes)",
               msg->getName().c_str(), this->get_cname(), msg->payload, msg);
...
void S4U_CommPort::putMessage(SimulationMessage *msg) {
  ...
  WRENCH_DEBUG("Putting a %s message (%.2lf bytes, %p) to commport '%s'",
               msg->getName().c_str(), msg->payload, msg, this->get_cname());
  ...
  if (msg->payload > 0) {
    try {
      ...
      throw ExecutionException(std::make_shared<NetworkError>(
          NetworkError::SENDING, NetworkError::FAILURE,
          this->s4u_mb->get_name(), msg->getName()));
    } catch (simgrid::TimeoutException &e) {
      // Can happen if the other side is doing a timeout.... I think
      throw ExecutionException(std::make_shared<NetworkError>(
          NetworkError::SENDING, NetworkError::TIMEOUT,
          this->s4u_mb->get_name(), msg->getName()));
    }
  } else {
    try {
      ...
      throw ExecutionException(std::make_shared<NetworkError>(
          NetworkError::SENDING, NetworkError::TIMEOUT,
          this->s4u_mq->get_name(), msg->getName()));
    } catch (simgrid::Exception &e) {
      throw ExecutionException(std::make_shared<NetworkError>(
          NetworkError::SENDING, NetworkError::FAILURE,
          this->s4u_mq->get_name(), msg->getName()));
...
/**
 * @brief Asynchronously send a message to a commport in a "fire and forget"
 * fashion
 *
 * @param msg: the SimulationMessage
 *
...
S4U_CommPort::iputMessage(SimulationMessage *msg) {
  ...
  WRENCH_DEBUG("Iputting a %s message (%.2lf bytes) to commport '%s'",
               msg->getName().c_str(), msg->payload, this->get_cname());
...
std::shared_ptr<S4U_PendingCommunication> S4U_CommPort::igetMessage() {
  ...
  if (this == S4U_CommPort::NULL_COMMPORT) {
    throw std::invalid_argument(
        "S4U_CommPort::igetMessage(): Cannot be called with NULL_COMMPORT");
  }

  WRENCH_DEBUG("Igetting a message from commport '%s'", this->get_cname());

  std::shared_ptr<S4U_PendingCommunication> pending_communication =
      std::make_shared<S4U_PendingCommunication>(
          this, S4U_PendingCommunication::OperationType::RECEIVING);

  try {
    auto comm_ptr = this->s4u_mb->get_async<void>(
        (void **)(&(pending_communication->simulation_message)));
    pending_communication->comm_ptr = comm_ptr;
  } catch (simgrid::NetworkFailureException &e) {
    throw ExecutionException(std::make_shared<NetworkError>(
        NetworkError::RECEIVING, NetworkError::FAILURE,
        this->s4u_mb->get_name(), ""));
  }

  simgrid::s4u::MessPtr mess_ptr = this->s4u_mq->get_async<void>(
      (void **)(&(pending_communication->simulation_message)));
  pending_communication->mess_ptr = mess_ptr;
  return pending_communication;
}
...
S4U_CommPort *S4U_CommPort::getTemporaryCommPort() {
  if (S4U_CommPort::free_commports.empty()) {
    throw std::runtime_error(
        "S4U_CommPort::getTemporaryCommPort(): Out of communication ports! "
        "(Increase the communication port pool size with the "
        "--wrench-commport-pool-size command-line argument (currently set "
        "at: " +
  ...
  if ((not commport->s4u_mb->empty()) or (not commport->s4u_mq->empty())) {
    ...
    //                auto to_drain = *(S4U_CommPort::commports_to_drain.end() -
    //                1); std::cerr << "############ UNWASTING MAILBOX " <<
    //                to_drain->get_name() << "\n";
  ...
  //        WRENCH_DEBUG("Gotten temporary commport %s (%p %p)",
  //        commport->name.c_str(), commport->mq_comm.get(),
  //        commport->mb_comm.get());
  return commport;
}
...
void S4U_CommPort::retireTemporaryCommPort(S4U_CommPort *commport) {
  //        WRENCH_DEBUG("Calling reset() on commport %s",
  //        commport->get_cname());
  if (commport->mb_comm) {
  ...
  commport->reset();
  if (S4U_CommPort::used_commports.find(commport) ==
      S4U_CommPort::used_commports.end()) {
    return;
  }
