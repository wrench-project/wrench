...
WRENCH_LOG_CATEGORY(wrench_core_s4u_simulation,
                    "Log category for S4U_Simulation");
...
/**
 * @brief Returns true if S4U_Simulation::initialize() has been called
 * successfully previously
 *
 * @return true or false
 */
bool S4U_Simulation::isInitialized() const { return this->initialized; }

/**
 * @brief Returns true if S4U_Simulation::setupPlatform() has been called
 * successfully previously
 * @return true or false
 */
bool S4U_Simulation::isPlatformSetup() const { return this->platform_setup; }
...
void S4U_Simulation::runSimulation() {
  ...
  if (this->initialized) {
    ...
    throw std::runtime_error(
        "S4U_Simulation::runSimulation(): Simulation has not been initialized");
  }
}
...
void S4U_Simulation::checkLinkBandwidths() {
  ...
  for (auto const &l : links) {
    if (l->get_bandwidth() <= 0) {
      throw std::invalid_argument("XML Platform error: link " + l->get_name() +
                                  " has zero bandwidth");
    }
  }
}
...
void S4U_Simulation::setupPlatform(const std::string &filename) {
  try {
    ...
    throw std::invalid_argument("XML Platform description file error: " +
                                std::string(e.what()));
 ...
 * @throw std::invalid_argument
 */
void S4U_Simulation::setupPlatform(
...
std::vector<std::string> S4U_Simulation::getAllHostnames() {
  std::vector<simgrid::s4u::Host *> host_list =
      simgrid::s4u::Engine::get_instance()->get_all_hosts();
...
std::vector<std::string> S4U_Simulation::getAllLinknames() {
  std::vector<simgrid::s4u::Link *> link_list =
      simgrid::s4u::Engine::get_instance()->get_all_links();
...
double S4U_Simulation::getLinkBandwidth(const std::string &name) {
  ...
  if (links.empty()) {
    throw std::invalid_argument(
        "S4U_Simulation::getLinkBandwidth(): unknown link " + name);
  }
 ...
 *
 */
void S4U_Simulation::setLinkBandwidth(const std::string &name,
                                      double bandwidth) {
  auto links = simgrid::s4u::Engine::get_instance()->get_filtered_links(
      [name](simgrid::s4u::Link *l) { return (l->get_name() == name); });
  if (links.empty()) {
    throw std::invalid_argument(
        "S4U_Simulation::setLinkBandwidth(): unknown link " + name);
  }
...
double S4U_Simulation::getLinkUsage(const std::string &name) {
  ...
  if (links.empty()) {
    throw std::invalid_argument(
        "S4U_Simulation::getLinkUsage(): unknown link " + name);
  }
  return links.at(0)->get_load();
}

/**
 * @brief Get the list of hostnames in each ClusterZone in the platform
 * (<cluster/> XML tag in the platform XML description)
...
S4U_Simulation::getAllHostnamesByCluster() {
  ...
  S4U_Simulation::traverseAllNetZonesRecursive(
      simgrid_engine->get_netzone_root(), result, false, false, false, true);
  return result;
}

/**
 * @brief Get the list of hostnames in each NetZone (<zone> and <cluster> tags
 * in the platform XML description). Note that this method does not recurse into
 * sub-zones, so it only returns the hosts that are declared directly under the
 * <zone> and or <cluster> tags.
 * @return a map of all zone ids and lists of hostnames
 */
std::map<std::string, std::vector<std::string>>
S4U_Simulation::getAllHostnamesByZone() {
  std::map<std::string, std::vector<std::string>> result;
  auto simgrid_engine = simgrid::s4u::Engine::get_instance();
  S4U_Simulation::traverseAllNetZonesRecursive(
      simgrid_engine->get_netzone_root(), result, false, false, true, false);
  return result;
}

/**
 * @brief Get the list of ids of all (sub-)zones in the platform within each
 * zone (<zone/> XML tag in the platform XML description)
 * @return a map of zone ids and the list of (sub-)zone ids
 */
std::map<std::string, std::vector<std::string>>
S4U_Simulation::getAllSubZoneIDsByZone() {
  std::map<std::string, std::vector<std::string>> result;
  auto simgrid_engine = simgrid::s4u::Engine::get_instance();
  S4U_Simulation::traverseAllNetZonesRecursive(
      simgrid_engine->get_netzone_root(), result, true, false, false, false);
  return result;
}

/**
 * @brief Get the list of ids of all ClusterZone in the platform within each
 * zone (<cluster/> XML tag in the platform XML description)
 * @return a map of zone ids and the list of cluster ids
 */
std::map<std::string, std::vector<std::string>>
S4U_Simulation::getAllClusterIDsByZone() {
  std::map<std::string, std::vector<std::string>> result;
  auto simgrid_engine = simgrid::s4u::Engine::get_instance();
  S4U_Simulation::traverseAllNetZonesRecursive(
      simgrid_engine->get_netzone_root(), result, false, true, false, false);
  return result;
}
 ...
 * @param result: a reference to a map that's being incrementally built
 * recursively
 ...
 * @param get_hosts_from_clusters: true if this should return hostnames from
 * clusters
 */
void S4U_Simulation::traverseAllNetZonesRecursive(
    simgrid::s4u::NetZone *nz,
    std::map<std::string, std::vector<std::string>> &result, bool get_subzones,
    bool get_clusters, bool get_hosts_from_zones,
    bool get_hosts_from_clusters) {
  //        std::cerr << "PROCESSING ROOT " << nz->get_name() << "\n";
  for (auto const &child : nz->get_children()) {
    //            std::cerr << "PROCESSING CHILD " << child->get_name() << "\n";
    bool is_cluster = dynamic_cast<simgrid::kernel::routing::ClusterZone *>(
        child->get_impl());
    ...
    } else if ((get_hosts_from_clusters and is_cluster) or
               (get_hosts_from_zones and is_zone)) {
    ...
    if (is_zone) {
      S4U_Simulation::traverseAllNetZonesRecursive(
          child, result, get_subzones, get_clusters, get_hosts_from_zones,
          get_hosts_from_clusters);
    } else {
      continue;
    }
  }
...
/**
 * @brief Determines whether a host exists for a given hostname (does not
 * include VMs)
...
double S4U_Simulation::getFlopRate() {
  return simgrid::s4u::Host::current()
      ->get_speed(); // changed it to speed of the current pstate
}
 ...
 */
double S4U_Simulation::getClock() { return simgrid::s4u::Engine::get_clock(); }
...
void S4U_Simulation::compute_multi_threaded(unsigned long num_threads,
  ...
  if (num_threads == 1) {
    simgrid::s4u::this_actor::execute(sequential_work +
                                      parallel_per_thread_work);
  } else {
    // Launch compute-heavy thread
    auto bottleneck_thread = simgrid::s4u::this_actor::exec_async(
        sequential_work + parallel_per_thread_work);
    // Launch all other threads
    simgrid::s4u::this_actor::thread_execute(
        simgrid::s4u::this_actor::get_host(), parallel_per_thread_work,
        (int)num_threads - 1);
 ...
 * @param hostname: name of host to which disk is attached (or empty if disk is
 * known)
 * @param mount_point: mount point (or empty if disk is known)
 * @param disk: a disk to write to, if known (if known, this method will run
 * much faster)
 */
void S4U_Simulation::writeToDisk(double num_bytes, const std::string &hostname,
                                 std::string mount_point,
                                 simgrid::s4u::Disk *disk) {
  mount_point = FileLocation::sanitizePath(mount_point);

  WRENCH_DEBUG("Writing %lf bytes to disk %s:%s", num_bytes, hostname.c_str(),
               mount_point.c_str());
  ...
  if (not disk) {
    ...
    if (host == nullptr) {
      throw std::invalid_argument(
          "S4U_Simulation::writeToDisk(): unknown host " + hostname);
    }
    ...
    if (not disk) {
      throw std::invalid_argument(
          "S4U_Simulation::writeToDisk(): unknown path " + mount_point +
          " at host " + hostname);
    }
  }
 ...
 */
void S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(
  ...
  WRENCH_DEBUG(
      "Reading %.2lf bytes from disk %s:%s and writing %lf bytes to disk %s:%s",
  ...
  if ((not src_disk) or (not dst_disk)) {
    ...
    auto sanitized_read_mount_point =
        FileLocation::sanitizePath(read_mount_point);
    auto sanitized_write_mount_point =
        FileLocation::sanitizePath(write_mount_point);
    auto host = S4U_Simulation::get_host_or_vm_by_name_or_null(hostname);
    if (host == nullptr) {
      throw std::invalid_argument(
          "S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(): unknown "
          "host " +
          hostname);
    }
    auto disk_list =
        S4U_Simulation::get_host_or_vm_by_name(hostname)->get_disks();
    for (auto const &disk : disk_list) {
      std::string disk_mountpoint = FileLocation::sanitizePath(
          std::string(std::string(disk->get_property("mount"))));
  ...
  if ((not src_disk) or (not dst_disk)) {
    throw std::invalid_argument(
        "S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(): Couldn't "
        "find src or dst disk!");
  }

  // Start asynchronous read
  auto read_activity = src_disk->io_init((sg_size_t)num_bytes_to_read,
                                         simgrid::s4u::Io::OpType::READ);
  read_activity->start();
 ...
 */
void S4U_Simulation::readFromDisk(double num_bytes, const std::string &hostname,
                                  std::string mount_point,
                                  simgrid::s4u::Disk *disk) {
  mount_point = FileLocation::sanitizePath(mount_point);

  WRENCH_DEBUG("Reading %.2lf bytes from disk %s:%s", num_bytes,
               hostname.c_str(), mount_point.c_str());

  if (not disk) {
    auto host = S4U_Simulation::get_host_or_vm_by_name_or_null(hostname);
    if (not host) {
      throw std::invalid_argument(
          "S4U_Simulation::readFromDisk(): unknown host " + hostname);
    }
    ...
    if (not disk) {
      throw std::invalid_argument(
          "S4U_Simulation::readFromDisk(): invalid mount point " + mount_point +
          " at host " + hostname);
    }
  }
 ...
 */
void S4U_Simulation::yield() { simgrid::s4u::this_actor::yield(); }
...
double S4U_Simulation::getHostMemoryCapacity(simgrid::s4u::Host *host) {
  ...
  std::set<std::string> tags = {
      "mem",    "Mem",   "MEM", "ram", "Ram", "RAM", "memory_manager_service",
      "Memory", "MEMORY"};
  ...
  for (auto const &tag : tags) {
    ...
    if (capacity_string) {
      if (capacity_value != S4U_Simulation::DEFAULT_RAM) {
        throw std::invalid_argument(
            "S4U_Simulation::getHostMemoryCapacity(): Host '" +
            std::string(host->get_cname()) +
      ...
      try {
        ...
        throw std::invalid_argument(
            "S4U_Simulation::getHostMemoryCapacity(): Host '" +
            std::string(host->get_cname()) +
            "'has invalid memory_manager_service capacity specification '" +
            tag + ":" + std::string(capacity_string) + "'");
      }
    }
  }
 ...
 */
std::string S4U_Simulation::getHostProperty(const std::string &hostname,
                                            const std::string &property_name) {
  auto host = simgrid::s4u::Host::by_name_or_null(hostname);
  if (host == nullptr) {
    throw std::invalid_argument(
        "S4U_Simulation::getHostProperty(): Unknown hostname " + hostname);
  }
  if (host->get_properties()->find(property_name) ==
      host->get_properties()->end()) {
    throw std::invalid_argument(
        "S4U_Simulation::getHostProperty(): Unknown property " + property_name);
  }
  return host->get_property(property_name);
}

/**
 * @brief Get the property associated to a cluster specified in the platform
 * file
 ...
 */
std::string
S4U_Simulation::getClusterProperty(const std::string &cluster_id,
                                   const std::string &property_name) {
  auto simgrid_engine = simgrid::s4u::Engine::get_instance();
  auto clusters =
      simgrid_engine
          ->get_filtered_netzones<simgrid::kernel::routing::ClusterZone>();
  for (auto c : clusters) {
    if (c->get_name() == cluster_id) {
      if (c->get_properties()->find(property_name) ==
          c->get_properties()->end()) {
        throw std::invalid_argument(
            "S4U_Simulation::getClusterProperty(): Unknown property " +
            property_name);
      }
  ...
  throw std::invalid_argument(
      "S4U_Simulation::getClusterProperty(): Unknown cluster " + cluster_id);
}

/**
 * @brief Set a property associated to a host specified in the platform file
 * (does not include VMs)
 * @param hostname: the host name
 * @param property_name: the property name
 * @param property_value: the property value
 */
void S4U_Simulation::setHostProperty(const std::string &hostname,
                                     const std::string &property_name,
...
double S4U_Simulation::getEnergyConsumedByHost(const std::string &hostname) {
  ...
  try {
    ...
    throw std::runtime_error(
        ...
        "energy consumed by the host. Make sure the energy plugin is enabled "
        "(--wrench-energy-simulation)");
  }
 ...
 *
 * @param hostname: the host name
 * @param pstate: the power state index (the power state index is specified in
 * the platform xml description file)
 * @throw std::invalid_argument
 * @throw std::runtime_error
 */
void S4U_Simulation::setPstate(const std::string &hostname,
                               unsigned long pstate) {
  auto host = simgrid::s4u::Host::by_name_or_null(hostname);
  if (host == nullptr) {
    throw std::invalid_argument(
        "S4U_Simulation::setPstate(): Unknown hostname " + hostname);
  }
  if (host->get_pstate_count() < pstate) {
    throw std::invalid_argument(
        "S4U_Simulation::setPstate(): Invalid pstate index (host " + hostname +
        " only has " + std::to_string(host->get_pstate_count()) + " pstates)");
  }
  try {
    host->set_pstate(pstate);
  } catch (std::exception &e) {
    throw std::runtime_error(
        "S4U_Simulation::setPstate(): Was not able to set the pstate of the "
        "host. "
        "Make sure the energy is plugin is enabled "
        "(--wrench-energy-simulation) and "
...
int S4U_Simulation::getNumberofPstates(const std::string &hostname) {
  ...
  try {
    ...
    throw std::runtime_error(
        "S4U_Simulation::getNumberofPstates():: Was not able to get the energy "
...
/**
 * @brief Get the minimum power consumption (i.e., idling) for a host at its
 * current pstate
 * @param hostname: the host name
 * @return The power consumption for this host if idle (as specified in the
 * platform xml description file)
 * @throw std::invalid_argument
 * @throw std::runtime_error
 */
double S4U_Simulation::getMinPowerConsumption(const std::string &hostname) {
  ...
  try {
    ...
    throw std::runtime_error(
        "S4U_Simulation::getMinPowerConsumption(): Was not able to get the min "
        "power available to the host. "
        "Make sure the energy plugin is enabled (--wrench-energy-simulation) ");
  }
}

/**
 * @brief Get the maximum power consumption (i.e., 100% load) for a host at its
 * current pstate
 * @param hostname: the host name
 * @return The power consumption for this host if 100% used (as specified in the
 * platform xml description file)
 * @throw std::invalid_argument
 * @throw std::runtime_error
 */
double S4U_Simulation::getMaxPowerConsumption(const std::string &hostname) {
  ...
  try {
    return sg_host_get_wattmax_at(
        simgrid::s4u::Host::by_name(hostname),
        (int)((simgrid::s4u::Host::by_name(hostname))->get_pstate()));
  } catch (std::exception &e) {
    throw std::runtime_error(
        "S4U_Simulation::getMaxPowerConsumption():: Was not able to get the "
        "max power possible for the host. "
        "Make sure the energy plugin is enabled (--wrench-energy-simulation)"
...
std::vector<int> S4U_Simulation::getListOfPstates(const std::string &hostname) {
  ...
  try {
    ...
    throw std::runtime_error(
        "S4U_Simulation::getListOfPstates(): Was not able to get the list of "
        "pstates for the host. "
        "Make sure the energy plugin is enabled (--wrench-energy-simulation) ");
  }
  return list;
}
...
/**
 * @brief Compute zero flop, which take zero time but will block if the host's
 * pstate has a zero flop/sec speed, until the host's pstate is changed to a
 * pstate with non-zero flop/sec speed.
 */
...
std::vector<std::string> S4U_Simulation::getDisks(const std::string &hostname) {
  ...
  try {
    ...
    throw std::invalid_argument("S4U_Simulation::getDisks(): Unknown host " +
                                hostname);
  }
 ...
 * @return a simgrid disk if the host has a disk attached to the specified mount
 * point, nullptr otherwise
 */
simgrid::s4u::Disk *
S4U_Simulation::hostHasMountPoint(const std::string &hostname,
  ...
  try {
    ...
    throw std::invalid_argument(
        "S4U_Simulation::hostHasMountPoint(): Unknown host " + hostname);
  }
  ...
  for (auto const &d : host->get_disks()) {
    ...
    if (FileLocation::sanitizePath(std::string(p)) ==
        FileLocation::sanitizePath(mount_point)) {
      return d;
    }
  }
 ...
 * @return a vector of link names
 */
std::vector<std::string> S4U_Simulation::getRoute(std::string &src_host,
                                                  std::string &dst_host) {
  simgrid::s4u::Host *src, *dst;
  try {
    src = S4U_Simulation::get_host_or_vm_by_name(src_host);
  } catch (std::exception &e) {
    throw std::invalid_argument("S4U_Simulation::getRoute(): Unknown host " +
                                src_host);
  }
  try {
...
/**
 * @brief Gets the capacity of a disk attached to some host for a given mount
 * point
 * @param hostname: the host's name
 ...
 * @throw std::invalid_argument
 */
double S4U_Simulation::getDiskCapacity(const std::string &hostname,
                                       std::string mount_point) {
  //        WRENCH_INFO("==== %s %s ==== ", hostname.c_str(),
  //        mount_point.c_str());
  simgrid::s4u::Host *host;
  try {
    host = S4U_Simulation::get_host_or_vm_by_name(hostname);
    ...
    throw std::invalid_argument(
        "S4U_Simulation::getDiskCapacity(): Unknown host " + hostname);
  }
  ...
  for (auto const &d : host->get_disks()) {
    ...
    if (capacity_str) {
      try {
        ...
        throw std::invalid_argument("S4U_Simulation::getDiskCapacity(): Disk " +
                                    d->get_name() + " at host " + hostname +
                                    " has invalid size");
      }
    } else {
  ...
  throw std::invalid_argument(
      "S4U_Simulation::getDiskCapacity(): Unknown mount point " + mount_point +
      " at host " + hostname);
}
 ...
 */
void S4U_Simulation::createNewDisk(const std::string &hostname,
  ...
  if (not host) {
    throw std::invalid_argument(
        "S4U_Simulation::createNewDisk(): unknown host " + hostname);
  }
  // Check that no similar disk exists
  for (auto const &d : host->get_disks()) {
    if (d->get_name() == disk_id) {
      throw std::invalid_argument(
          "S4U_Simulation::createNewDisk(): a disk with id " + disk_id +
          " already exists at host " + hostname);
    }
    if (d->get_property("mount") == mount_point) {
      throw std::invalid_argument(
          "S4U_Simulation::createNewDisk(): a disk with mount point " +
          mount_point + " already exists at host " + hostname);
    }
  }
  // Create the disk
  auto disk = host->create_disk(disk_id, read_bandwidth_in_bytes_per_sec,
                                write_bandwidth_in_bytes_per_sec);
...
/**
 * @brief Convenient s4u wrapper to retrieve a Host (which can be a VM) based on
 * a name
 * @param name: the host/vm name or null if none
 * @return a SimGrid host
 */
simgrid::s4u::Host *
S4U_Simulation::get_host_or_vm_by_name_or_null(const std::string &name) {
  auto host = simgrid::s4u::Host::by_name_or_null(name);
  if (!host) {
    // Perhaps it's a VM
    if (S4U_VirtualMachine::vm_to_pm_map.find(name) !=
        S4U_VirtualMachine::vm_to_pm_map.end()) {
      auto physical_host = simgrid::s4u::Host::by_name_or_null(
          S4U_VirtualMachine::vm_to_pm_map[name]);
 ...
 */
simgrid::s4u::Host *
S4U_Simulation::get_host_or_vm_by_name(const std::string &name) {
  if (name.empty())
    return nullptr;
  auto host = S4U_Simulation::get_host_or_vm_by_name_or_null(name);
  if (!host) {
    throw std::invalid_argument(
        "S4U_Simulation::get_host_or_vm_by_name(): Unknown host '" + name +
        "'");
  }
  return host;
}
