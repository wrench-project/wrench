...
typedef struct WorkflowTaskExecutionInstance {
  ...
  double getTaskEndTime() {
    return std::max({this->whole_task.second, this->failed, this->terminated});
  }
...
static std::vector<simgrid::s4u::Host *> get_all_physical_hosts() {
  ...
  for (auto const &h : hosts) {
    // Ignore VMs
    if (S4U_VirtualMachine::vm_to_pm_map.find(h->get_name()) !=
...
/**
 * @brief Function that generates a unified JSON file containing the information
 *specified by boolean arguments.
 *
 *<pre>
 * JSON Structure:
 * {
 ...
 *</pre>
 *
 * Any pieces not specified in the arguments are left out. For full structure
 *see documentation of specific sections.
 *
 ...
 * @param include_workflow_exec: boolean whether to include workflow execution
 *in JSON
 * @param include_workflow_graph: boolean whether to include workflow graph in
 *JSON
 * @param include_energy: boolean whether to include energy consumption in JSON
 * @param generate_host_utilization_layout: boolean specifying whether or not
 *you would like a possible host utilization layout to be generated
 * @param include_disk: boolean specifying whether to include disk operation in
 *JSON (disk timestamps must be enabled)
 * @param include_bandwidth: boolean specifying whether to include link
 *bandwidth measurements in JSON
 */
void SimulationOutput::dumpUnifiedJSON(
    const std::shared_ptr<Workflow> &workflow, std::string file_path,
    bool include_platform, bool include_workflow_exec,
    bool include_workflow_graph, bool include_energy,
    bool generate_host_utilization_layout, bool include_disk,
  ...
  if (include_workflow_exec) {
    dumpWorkflowExecutionJSON(workflow, file_path,
                              generate_host_utilization_layout, false);
 ...
 * @return bool
 */
bool isPointOnSegment(std::pair<unsigned long long, unsigned long long> segment,
                      unsigned long long point) {
  return (point <= std::max(segment.first, segment.second) and
          point >= std::min(segment.first, segment.second));
}

/**
 * @brief Determines if two line segments overlap along the x-axis and allows
 * for a slight overlap.
 * @param segment1: first segment
 * @param segment2: second segment
 * @return bool
 */
bool isSegmentOverlappingXAxis(
    std::pair<unsigned long long, unsigned long long> segment1,
    std::pair<unsigned long long, unsigned long long> segment2) {
  // Note: EPSILON looks big because we "blow up" the floats by 10^9 to do all
  // computations
  //       with unsigned long longs instead of doubles (thus avoiding float
  //       comparison weirdness) And the goal of this EPSILON is to capture the
  //       fact that this is for a visual display made up of pixels, thus we
  //       don't want to be too stringent in terms of overlaps
  ...
  } else if (isPointOnSegment(segment1, segment2.first) or
             isPointOnSegment(segment1, segment2.second) or
             isPointOnSegment(segment2, segment1.first) or
             isPointOnSegment(segment2, segment1.second)) {
    return true;
...
/**
 * @brief Determines if two line segments overlap along the y-axis using exact
 * values.
 * @param segment1: first segment
 * @param segment2: second segment
 * @return bool
 */
bool isSegmentOverlappingYAxis(
    std::pair<unsigned long long, unsigned long long> segment1,
    std::pair<unsigned long long, unsigned long long> segment2) {
...
/**
 * @brief Searches for a possible host utilization gantt chart layout and
 * updates the data to include what vertical position to plot each rectangle.
 *
 * Recursive backtracking search for a valid gantt chart layout. This algorithm
 * looks for a vertical position to place each task execution event such that it
 * doesn't overlap with any other task.
 *
 * @param data: JSON workflow execution data
 * @param index: the index of the workflow execution data up to where we would
 * like to check for a valid layout
 * @return bool
 */
bool searchForLayout(std::vector<WorkflowTaskExecutionInstance> &data,
                     std::size_t index) {
  ...
  unsigned long long num_cores_allocated =
      current_execution_instance.num_cores_allocated;
  unsigned long long execution_host_num_cores =
      current_execution_instance.host_num_cores;
  auto num_vertical_positions =
      execution_host_num_cores - num_cores_allocated + 1;
  ...
  /*
   * For each possible vertical position that an event can be in, we perform a
   * check to see that its vertical position doesn't make the event (a rectangle
   * on the graph) overlap with any of the other events. If it does not overlap,
   * then we can evaluate all events up to this one with the next event in the
   * list (recursive call). If it does overlap, then we try another vertical
   * position. If it doesn't overlap and this is the last item in the list, we
   * have found a valid layout.
   */
  for (std::size_t vertical_position = 0;
       vertical_position < num_vertical_positions; ++vertical_position) {
    // Set the vertical positions as we go so the entire graph layout is set
    // when the function returns
    current_execution_instance.vertical_position = vertical_position;

    auto current_rect_y_range =
        std::pair<unsigned long long, unsigned long long>(
            vertical_position, vertical_position + num_cores_allocated);

    //              std::cout << spaces + "  pos = " <<  vertical_position <<
    //              "\n";
    /*
     * We check this current event's position against all other events that were
     * added before it to make sure it doesn't overlap with any of those events.
     */
    bool has_overlap = false;
    for (std::size_t i = 0; i < index; ++i) {
      WorkflowTaskExecutionInstance other_execution_instance = data.at(i);

      // Evaluate the current event's position only against others that occurred
      // on the same host.
      if (current_execution_instance.hostname ==
          other_execution_instance.hostname) {
        auto other_rect_x_range =
            std::pair<unsigned long long, unsigned long long>(
                other_execution_instance.whole_task.first * PRECISION,
                other_execution_instance.getTaskEndTime() * PRECISION);

        auto other_rect_y_range =
            std::pair<unsigned long long, unsigned long long>(
                other_execution_instance.vertical_position,
                other_execution_instance.vertical_position +
                    other_execution_instance.num_cores_allocated);

        /*
         * Check overlap for the x_ranges first. If there is no overlap, we can
         * guarantee that the rectangles will not overlap. If the x_ranges do
         * overlap, then we need to evaluate the y_ranges for overlap.
         */
        if (isSegmentOverlappingXAxis(current_rect_x_range,
                                      other_rect_x_range)) {
          if (isSegmentOverlappingYAxis(current_rect_y_range,
                                        other_rect_y_range)) {
            has_overlap = true;
            break;
          }
    ...
    if (not has_overlap and index >= data.size() - 1) {
      host_utilization_layout[current_execution_instance.task_id] =
          vertical_position;
      return true;
...
/**
 * @brief Generates graph layout for host utilization and adds that information
 * to the JSON object.
 *
 * Searches for a possible gantt chart layout to represent host utilization. If
 * a layout is found (no tasks overlap), then information about where to plot
 * what is added to the JSON object. Note that this is a possible layout and
 * does not reflect what task ran on what core specifically. For example, we may
 * hav a task that was allocated 2-cores on a idle 4-core host. The task, when
 * plotted on the gantt chart may end up in 1 of 3 positions (using cores 0 and
 * 1, 1 and 2, or 2 and 3).
 * @param data: JSON workflow execution data
 *
 * @throws std::runtime_error
 */
void generateHostUtilizationGraphLayout(
    std::vector<WorkflowTaskExecutionInstance> &data) {
  if (not searchForLayout(data, 0)) {
    throw std::runtime_error(
        "SimulationOutput::generateHostUtilizationGraphLayout() could not find "
        "a valid layout.");
  }
}
...
/**
 * @brief Writes WorkflowTask execution history for each task to a file,
 ...
 * </pre>
 *
 *   If generate_host_utilization_layout is set to true, a recursive function
 * searches for a possible host utilization layout where tasks are assumed to
 * use contiguous numbers of cores on their execution hosts. Note that each
 * ComputeService does not enforce this, and such a layout may not exist for
 * some workflow executions. In this situation, the function will go through the
 * entire search space until all possible layouts are evaluated. For a large
 * Workflow, this may take a very long time.
 *
 *   If a host utilization layout is able to be generated, the
 * 'vertical_position' values will be set for each task run, and the task can be
 * plotted as a rectangle on a graph where the y-axis denotes the number of
 * cores - 1, and the x-axis denotes the workflow execution timeline. The
 * vertical_position specifies the bottom of the rectangle. num_cores_allocated
 ...
 * @param generate_host_utilization_layout: boolean specifying whether or not
 * you would like a possible host utilization layout to be generated
 * @param writing_file: whether or not the file is written, true by default but
 * will be false when utilized as part of dumpUnifiedJSON
 *
 * @throws std::invalid_argument
 */
void SimulationOutput::dumpWorkflowExecutionJSON(
    const std::shared_ptr<Workflow> &workflow, const std::string &file_path,
    bool generate_host_utilization_layout, bool writing_file) {
  if (workflow == nullptr || file_path.empty()) {
    throw std::invalid_argument("SimulationOutput::dumpWorkflowExecutionJSON() "
                                "requires a valid workflow and file_path");
  }

  auto tasks = workflow->getTasks();
  nlohmann::json task_json;

  auto read_start_timestamps =
      this->getTrace<SimulationTimestampFileReadStart>();
  auto read_completion_timestamps =
      this->getTrace<wrench::SimulationTimestampFileReadCompletion>();
  auto read_failure_timestamps =
      this->getTrace<wrench::SimulationTimestampFileReadFailure>();

  auto write_start_timestamps =
      this->getTrace<SimulationTimestampFileWriteStart>();
  auto write_completion_timestamps =
      this->getTrace<wrench::SimulationTimestampFileWriteCompletion>();
  auto write_failure_timestamps =
      this->getTrace<wrench::SimulationTimestampFileWriteFailure>();
    ...
    while (not execution_history.empty()) {
      ...
      for (auto const &r : current_execution_instance.reads) {
        nlohmann::json file_read =
            nlohmann::json::object({{"end", std::get<1>(r)},
                                    {"start", std::get<0>(r)},
                                    {"id", std::get<2>(r)}});
        file_reads.push_back(file_read);
      }
           ...
           {"execution_host",
            {{"hostname", current_task_execution.physical_execution_host},
             ...
             {"memory_manager_service",
              Simulation::getHostMemoryCapacity(
                  current_task_execution.physical_execution_host)},
  ...
  // For each attempted execution of a task, add a WorkflowTaskExecutionInstance
  // to the list.
  for (auto const &task : tasks) {
    auto execution_history = task->getExecutionHistory();
    ...
    while (not execution_history.empty()) {
      ...
      current_execution_instance.hostname =
          current_task_execution.physical_execution_host;
      current_execution_instance.host_flop_rate = Simulation::getHostFlopRate(
          current_task_execution.physical_execution_host);
      current_execution_instance.host_memory =
          Simulation::getHostMemoryCapacity(
              current_task_execution.physical_execution_host);
      ...
      current_execution_instance.num_cores_allocated =
          current_task_execution.num_cores_allocated;
      current_execution_instance.vertical_position = 0;

      current_execution_instance.whole_task = std::make_pair(
          current_task_execution.task_start, current_task_execution.task_end);
      current_execution_instance.compute =
          std::make_pair(current_task_execution.computation_start,
                         current_task_execution.computation_end);

      current_execution_instance.failed = current_task_execution.task_failed;
      current_execution_instance.terminated =
          current_task_execution.task_terminated;
  ...
  // Set the "vertical position" of each WorkflowExecutionInstance so we know
  // where to plot each rectangle
  ...
  workflow_execution_json_single["workflow_execution"] =
      workflow_execution_json;
...
/**
 * @brief Writes a JSON graph representation of the Workflow to a file, in the
 * WfFormat format which is defined at: https://wfcommons.org/format.
 *
 *        Note that in the output JSON, the machines::cpu::speed is give as a
 * GHz clock rate while in WRENCH core speeds are in Flop/sec. The GHz clock
 * rate is computed by dividing the flop rate by 10^9 (so it's not really a
 * clock rate).
 *
 * @param workflow: a pointer to the workflow
 * @param file_path: the path to write the file
 ...
 * @throws std::invalid_argument
 */
void SimulationOutput::dumpWorkflowGraphJSON(
    const std::shared_ptr<wrench::Workflow> &workflow,
    const std::string &file_path, bool writing_file) {
  if (workflow == nullptr || file_path.empty()) {
    throw std::invalid_argument("SimulationOutput::dumpWorkflowGraphJSON() "
                                "requires a valid workflow and file_path");
  }
  ...
  for (const auto &task : workflow->getTasks()) {
    ...
    for (const auto &f : task->getInputFiles()) {
      files.push_back(
          {{"link", "input"}, {"name", f->getID()}, {"size", f->getSize()}});
    }
    for (const auto &f : task->getOutputFiles()) {
      files.push_back(
          {{"link", "output"}, {"name", f->getID()}, {"size", f->getSize()}});
    }
...
/**
 * @brief Writes a JSON file containing host energy consumption information as a
 * JSON array.
 *
 * The JSON array has the following format:
 *
 ...
 * @throws std::runtime_error
 */
void SimulationOutput::dumpHostEnergyConsumptionJSON(
    const std::string &file_path, bool writing_file) {
  if (file_path.empty()) {
    throw std::invalid_argument(
        "SimulationOutput::dumpHostEnergyConsumptionJSON() requires a valid "
        "file_path");
  }
  ...
  try {
    ...
    for (const auto &host : hosts) {
      ...
      if (property_string == nullptr) {
        throw std::runtime_error(
            "Host " + std::string(host->get_name()) +
            " does not have a wattage_per_state property!");
      }
      std::string watts_per_state_property_string =
          std::string(property_string);
      std::vector<std::string> watts_per_state;
      boost::split(watts_per_state, watts_per_state_property_string,
                   boost::is_any_of(","));

      for (size_t pstate = 0; pstate < watts_per_state.size(); ++pstate) {
        std::vector<std::string> current_state_watts;
        boost::split(current_state_watts, watts_per_state.at(pstate),
                     boost::is_any_of(":"));
      ...
      for (const auto &pstate_timestamp :
           this->getTrace<SimulationTimestampPstateSet>()) {
      ...
      for (const auto &energy_consumption_timestamp :
           this->getTrace<SimulationTimestampEnergyConsumption>()) {
        if (host->get_name() ==
            energy_consumption_timestamp->getContent()->getHostname()) {
          datum["consumed_energy_trace"].push_back(
              {{"time", energy_consumption_timestamp->getDate()},
               {"joules",
                energy_consumption_timestamp->getContent()->getConsumption()}});
        }
      }
    ...
    energy_consumption["energy_consumption"] =
        hosts_energy_consumption_information;
...
/**
 * @brief Writes a JSON file containing all hosts, network links, and the routes
 * between each host.
 *
 * The JSON array has the following format:
 *
...
void SimulationOutput::dumpPlatformGraphJSON(const std::string &file_path,
  ...
  if (file_path.empty()) {
    throw std::invalid_argument(
        "SimulationOutput::dumpPlatformGraphJSON() requires a valid file_path");
  }

  nlohmann::json platform_graph_json;

  //        simgrid::s4u::Engine *simgrid_engine =
  ...
  std::map<std::string, std::vector<std::string>> cluster_to_hosts =
      S4U_Simulation::getAllHostnamesByCluster();
  ...
  for (const auto &host : hosts) {
    platform_graph_json["vertices"].push_back(
        {{"type", "host"},
         ...
         {"memory_manager_service",
          Simulation::getHostMemoryCapacity(host->get_name())},
  ...
  for (auto target = hosts.begin(); target != hosts.end(); ++target) {
    for (auto source = hosts.begin(); source != target; ++source) {
      ...
      // the route between target and source; the "route_backward" could be
      // different so we need to add it if it is in fact different
      ...
      if (route_forward.empty() and route_backward.empty()) {
        throw std::invalid_argument("Cannot generate platform graph because no "
                                    "route is found between hosts " +
                                    std::string((*source)->get_cname()) +
                                    " and " +
                                    std::string((*target)->get_cname()));
      }

      // check to see if the route from source to target is the same as from
      // target to source
  ...
  // maintain a unique list of edges where edges are represented using the
  // following string format:
  // <source_type>:<source_id>-<target_type>:<target_id> where type could be
  // 'host' or 'link'
  ...
  for (nlohmann::json::iterator route_itr =
           platform_graph_json["routes"].begin();
    ...
    source_id = (*route_itr)["route"]
                    .at(((*route_itr)["route"].size()) - 1)
                    .get<std::string>();
    source_string = LINK + ":" + source_id;
...
/**
 *
 * @brief Writes a JSON file containing disk operation information as a JSON
 * array.
 *
 * >>>>NOTE<<<< The timestamps the JSON is generated from are disabled by
 * default. Enable them with SimulationOutput::enableDiskTimestamps() to use.
 *
 * The JSON array has the following format:
 *
 * <pre>
 ...
 * @param writing_file - boolean, default true, to write the JSON to the
 * specified file path. Used for unified output.
 *
...
void SimulationOutput::dumpDiskOperationsJSON(const std::string &file_path,
  ...
  if (file_path.empty()) {
    throw std::invalid_argument(
        "SimulationOutput::dumpDiskOperationJSON() requires a valid file_path");
  }
  nlohmann::json disk_operations_json;

  auto read_start_timestamps =
      this->getTrace<SimulationTimestampDiskReadStart>();
  auto read_completion_timestamps =
      this->getTrace<wrench::SimulationTimestampDiskReadCompletion>();
  auto read_failure_timestamps =
      this->getTrace<wrench::SimulationTimestampDiskReadFailure>();

  auto write_start_timestamps =
      this->getTrace<SimulationTimestampDiskWriteStart>();
  auto write_completion_timestamps =
      this->getTrace<wrench::SimulationTimestampDiskWriteCompletion>();
  auto write_failure_timestamps =
      this->getTrace<wrench::SimulationTimestampDiskWriteFailure>();

  std::set<std::string> hostnames;

  // std::tuple<string, string, std::tuple<double, double, double>>
  // disk_operation;
  ...
  for (auto &host : hostnames) {
    ...
    for (auto &mount : mounts) {
      ...
      for (auto const &r : reads) {
        nlohmann::json disk_read =
            nlohmann::json::object({{"start", std::get<0>(r)},
                                    {"end", std::get<1>(r)},
                                    {"bytes", std::get<2>(r)},
                                    {"failed", "-1"}});
        ...
        for (auto &timestamp : read_failure_timestamps) {
          for (auto &disk_read : disk_reads) {
            if (timestamp->getContent()->getDate() == disk_read["end"] &&
                timestamp->getContent()->getEndpoint()->getDate() ==
                    disk_read["start"]) {
              disk_read["failed"] = "1";
            }
          }
      ...
      for (auto const &w : writes) {
        nlohmann::json disk_write =
            nlohmann::json::object({{"start", std::get<0>(w)},
                                    {"end", std::get<1>(w)},
                                    {"bytes", std::get<2>(w)},
                                    {"failed", "-1"}});
 ...
 *
 * This information will not be generated without using the bandwidth meter
 *service and providing it with linknames to monitor.
 *
 *<pre>
 * {
 *  "link_usage": {
...
void SimulationOutput::dumpLinkUsageJSON(const std::string &file_path,
  ...
  if (file_path.empty()) {
    throw std::invalid_argument(
        "SimulationOutput::dumpLinkUsageJSON() requires a valid file_path");
  }
  ...
  for (const auto &link : links) {
    ...
    for (const auto &link_usage_timestamp :
         this->getTrace<SimulationTimestampLinkUsage>()) {
      if (link->get_name() ==
          link_usage_timestamp->getContent()->getLinkname()) {
        datum["link_usage_trace"].push_back(
            {{"time", link_usage_timestamp->getDate()},
             {"bytes per second",
              link_usage_timestamp->getContent()->getUsage()}});
      }
    }
 ...
 */
void SimulationOutput::addTimestampTaskStart(
    double date, const std::shared_ptr<WorkflowTask> &task) {
  if (this->isEnabled<SimulationTimestampTaskStart>()) {
    this->addTimestamp<SimulationTimestampTaskStart>(
        new SimulationTimestampTaskStart(date, task));
  }
}
 ...
 * @param task: a workflow task
 */
void SimulationOutput::addTimestampTaskFailure(
    double date, const std::shared_ptr<WorkflowTask> &task) {
  if (this->isEnabled<SimulationTimestampTaskFailure>()) {
    this->addTimestamp<SimulationTimestampTaskFailure>(
        new SimulationTimestampTaskFailure(date, task));
  }
}
 ...
 * @param task: a workflow task
 */
void SimulationOutput::addTimestampTaskCompletion(
    double date, const std::shared_ptr<WorkflowTask> &task) {
  if (this->isEnabled<SimulationTimestampTaskCompletion>()) {
    this->addTimestamp<SimulationTimestampTaskCompletion>(
        new SimulationTimestampTaskCompletion(date, task));
  }
}
 ...
 * @param task: a workflow task
 */
void SimulationOutput::addTimestampTaskTermination(
    double date, const std::shared_ptr<WorkflowTask> &task) {
  if (this->isEnabled<SimulationTimestampTaskTermination>()) {
    this->addTimestamp<SimulationTimestampTaskTermination>(
        new SimulationTimestampTaskTermination(date, task));
  }
}
 ...
 */
void SimulationOutput::addTimestampFileReadStart(
    double date, const std::shared_ptr<DataFile> &file,
  ...
  if (this->isEnabled<SimulationTimestampFileReadStart>()) {
    this->addTimestamp<SimulationTimestampFileReadStart>(
        new SimulationTimestampFileReadStart(date, file, src, service,
                                             std::move(task)));
  }
}
 ...
 * @param task: the workflow task for which this read is done (or nullptr);
 */
void SimulationOutput::addTimestampFileReadFailure(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &src,
    const std::shared_ptr<StorageService> &service,
    std::shared_ptr<WorkflowTask> task) {
  if (this->isEnabled<SimulationTimestampFileReadFailure>()) {
    this->addTimestamp<SimulationTimestampFileReadFailure>(
        new SimulationTimestampFileReadFailure(date, file, src, service,
                                               std::move(task)));
  }
}
 ...
 * @param task: the workflow task for which this read is done (or nullptr);
 */
void SimulationOutput::addTimestampFileReadCompletion(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &src,
    const std::shared_ptr<StorageService> &service,
    std::shared_ptr<WorkflowTask> task) {
  if (this->isEnabled<SimulationTimestampFileReadCompletion>()) {
    this->addTimestamp<SimulationTimestampFileReadCompletion>(
        new SimulationTimestampFileReadCompletion(date, file, src, service,
                                                  std::move(task)));
  }
}
 ...
 */
void SimulationOutput::addTimestampFileWriteStart(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &src,
    const std::shared_ptr<StorageService> &service,
    std::shared_ptr<WorkflowTask> task) {
  if (this->isEnabled<SimulationTimestampFileWriteStart>()) {
    this->addTimestamp<SimulationTimestampFileWriteStart>(
        new SimulationTimestampFileWriteStart(date, file, src, service,
                                              std::move(task)));
  }
}
 ...
 * @param task: the workflow task for which this write is done (or nullptr);
 */
void SimulationOutput::addTimestampFileWriteFailure(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &src,
    const std::shared_ptr<StorageService> &service,
    std::shared_ptr<WorkflowTask> task) {
  if (this->isEnabled<SimulationTimestampFileWriteFailure>()) {
    this->addTimestamp<SimulationTimestampFileWriteFailure>(
        new SimulationTimestampFileWriteFailure(date, file, src, service,
                                                std::move(task)));
  }
}
 ...
 * @param task: the workflow task for which this write is done (or nullptr);
 */
void SimulationOutput::addTimestampFileWriteCompletion(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &src,
    const std::shared_ptr<StorageService> &service,
    std::shared_ptr<WorkflowTask> task) {
  if (this->isEnabled<SimulationTimestampFileWriteCompletion>()) {
    this->addTimestamp<SimulationTimestampFileWriteCompletion>(
        new SimulationTimestampFileWriteCompletion(date, file, src, service,
                                                   std::move(task)));
  }
}
 ...
 */
void SimulationOutput::addTimestampFileCopyStart(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &src,
 ...
 * @param dst: the target location
 */
void SimulationOutput::addTimestampFileCopyFailure(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &src,
    const std::shared_ptr<FileLocation> &dst) {
 ...
 * @param dst: the target location
 */
void SimulationOutput::addTimestampFileCopyCompletion(
    double date, std::shared_ptr<DataFile> file,
  ...
  if (this->isEnabled<SimulationTimestampFileCopyCompletion>()) {
    this->addTimestamp<SimulationTimestampFileCopyCompletion>(
        new SimulationTimestampFileCopyCompletion(
            date, std::move(file), std::move(src), std::move(dst)));
  }
}
...
void SimulationOutput::addTimestampDiskReadStart(double date,
  ...
  if (this->isEnabled<SimulationTimestampDiskReadStart>()) {
    this->addTimestamp<SimulationTimestampDiskReadStart>(
        new SimulationTimestampDiskReadStart(date, std::move(hostname),
                                             std::move(path), bytes,
 ...
 * @param unique_sequence_number: an integer id
 */
void SimulationOutput::addTimestampDiskReadFailure(double date,
  ...
  if (this->isEnabled<SimulationTimestampDiskReadFailure>()) {
    this->addTimestamp<SimulationTimestampDiskReadFailure>(
        new SimulationTimestampDiskReadFailure(date, hostname, path, bytes,
                                               unique_sequence_number));
  }
}
 ...
 * @param unique_sequence_number: an integer id
 */
void SimulationOutput::addTimestampDiskReadCompletion(
    double date, const std::string &hostname, const std::string &path,
    double bytes, int unique_sequence_number) {
  if (this->isEnabled<SimulationTimestampDiskReadCompletion>()) {
    this->addTimestamp<SimulationTimestampDiskReadCompletion>(
        new SimulationTimestampDiskReadCompletion(date, hostname, path, bytes,
                                                  unique_sequence_number));
  }
}
 ...
 * @param unique_sequence_number: an integer id
 */
void SimulationOutput::addTimestampDiskWriteStart(double date,
                                                  std::string hostname,
                                                  std::string path,
                                                  double bytes,
                                                  int unique_sequence_number) {
  if (this->isEnabled<SimulationTimestampDiskWriteStart>()) {
    this->addTimestamp<SimulationTimestampDiskWriteStart>(
        new SimulationTimestampDiskWriteStart(date, std::move(hostname),
                                              std::move(path), bytes,
                                              unique_sequence_number));
  }
}
 ...
 * @param unique_sequence_number: an integer id
 */
void SimulationOutput::addTimestampDiskWriteFailure(
    double date, const std::string &hostname, const std::string &path,
    double bytes, int unique_sequence_number) {
  if (this->isEnabled<SimulationTimestampDiskWriteFailure>()) {
    this->addTimestamp<SimulationTimestampDiskWriteFailure>(
        new SimulationTimestampDiskWriteFailure(date, hostname, path, bytes,
                                                unique_sequence_number));
  }
}
 ...
 * @param unique_sequence_number: an integer id
 */
void SimulationOutput::addTimestampDiskWriteCompletion(
    double date, const std::string &hostname, const std::string &path,
    double bytes, int unique_sequence_number) {
  if (this->isEnabled<SimulationTimestampDiskWriteCompletion>()) {
    this->addTimestamp<SimulationTimestampDiskWriteCompletion>(
        new SimulationTimestampDiskWriteCompletion(date, hostname, path, bytes,
                                                   unique_sequence_number));
  }
}
 ...
 */
void SimulationOutput::addTimestampPstateSet(double date,
                                             const std::string &hostname,
                                             int pstate) {
  if (this->isEnabled<SimulationTimestampPstateSet>()) {
    this->addTimestamp<SimulationTimestampPstateSet>(
        new SimulationTimestampPstateSet(date, hostname, pstate));
  }
}
 ...
 */
void SimulationOutput::addTimestampEnergyConsumption(
    double date, const std::string &hostname, double joules) {
  static std::unordered_map<std::string,
                            std::vector<SimulationTimestampEnergyConsumption *>>
      last_two_timestamps;
  ...
  auto new_timestamp =
      new SimulationTimestampEnergyConsumption(date, hostname, joules);
  ...
  bool can_merge =
      DBL_EQUAL(last_two_timestamps[hostname].at(0)->getConsumption(),
 ...
 */
void SimulationOutput::addTimestampLinkUsage(double date,
                                             const std::string &linkname,
                                             double bytes_per_second) {
  static std::unordered_map<std::string,
                            std::vector<SimulationTimestampLinkUsage *>>
      last_two_timestamps;
  ...
  auto new_timestamp =
      new SimulationTimestampLinkUsage(date, linkname, bytes_per_second);
  ...
  // Otherwise, check whether we can merge
  bool can_merge =
      DBL_EQUAL(last_two_timestamps[linkname].at(0)->getUsage(),
                last_two_timestamps[linkname].at(1)->getUsage()) and
      DBL_EQUAL(last_two_timestamps[linkname].at(1)->getUsage(),
                new_timestamp->getUsage());
