...
WRENCH_LOG_CATEGORY(wrench_core_simulation_timestamps,
                    "Log category for SimulationTimestamps");
 ...
 */
double SimulationTimestampType::getDate() const { return this->date; }

/**
 * @brief Set the date recorded for this timestamp
 * @param d: the date of this timestamp
 */
void SimulationTimestampType::setDate(double d) { this->date = d; }
 ...
 */
SimulationTimestampPair::SimulationTimestampPair(
    double date, SimulationTimestampPair *endpoint)
    : endpoint(endpoint) {
  this->date = date;
}

/**
 * @brief Retrieves the corresponding start/end SimulationTimestampType
 * associated with this timestamp
 * @return A pointer to a start SimulationTimestampType if this is a
 * failure/completion timestamp or vise versa
 */
 ...
 */
SimulationTimestampTask::SimulationTimestampTask(
  ...
  if (task == nullptr) {
    throw std::invalid_argument(
        "SimulationTimestampTask::SimulationTimestampTask() requires a pointer "
        "to a Workflowtask");
  }
#endif
}
...
/**
 * @brief A static map of SimulationTimestampTaskStart objects that have yet to
 * matched with SimulationTimestampTaskFailure,
 * SimulationTimestampTaskTerminated, SimulationTimestampTaskCompletion
 * timestamps
 */
std::map<std::string, SimulationTimestampTask *>
    SimulationTimestampTask::pending_task_timestamps;

/**
 * @brief Sets the endpoint of the calling object
 * (SimulationTimestampTaskFailure, SimulationTimestampTaskTerminated,
 * SimulationTimestampTaskStart) with a SimulationTimestampTaskStart object
 */
void SimulationTimestampTask::setEndpoints() {
  ...
  if (pending_tasks_itr != pending_task_timestamps.end()) {
    ...
    // the SimulationTimestampTaskStart is no longer waiting to be matched with
    // an end timestamp, remove it from the map
    pending_task_timestamps.erase(pending_tasks_itr);
  } else {
    throw std::runtime_error(
        "SimulationTimestampTask::setEndpoints() could not find a "
        "SimulationTimestampTaskStart object with taskID: " +
        this->task->getID());
  }
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampTaskStart::SimulationTimestampTaskStart(
    double date, const std::shared_ptr<WorkflowTask> &task)
    : SimulationTimestampTask(date, task) {
  WRENCH_DEBUG("Inserting a Taskstart timestamp for task '%s'",
               task->getID().c_str());

#ifdef WRENCH_INTERNAL_EXCEPTIONS
  if (task == nullptr) {
    throw std::invalid_argument(
        "SimulationTimestampTaskStart::SimulationTimestampTaskStart() requires "
        "a valid pointer to a WorkflowTask object");
  }
#endif

  /*
   * Upon creation, this object adds a pointer of itself to the
   * 'pending_task_timestamps' map so that it's endpoint can be set when a
   * SimulationTimestampTaskFailure, SimulationTimestampTaskTerminated, or
   * SimulationTimestampTaskCompletion is created
   */
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampTaskFailure::SimulationTimestampTaskFailure(
    double date, const std::shared_ptr<WorkflowTask> &task)
    : SimulationTimestampTask(date, task) {
  WRENCH_DEBUG("Inserting a TaskFailure timestamp for task '%s'",
               task->getID().c_str());
  ...
  if (task == nullptr) {
    throw std::invalid_argument(
        "SimulationTimestampTaskFailure::SimulationTimestampTaskFailure() "
        "requires a valid pointer to a WorkflowTask object");
  }
#endif
 ...
 * @param task: the WorkflowTask associated with this timestamp
 */
SimulationTimestampTaskCompletion::SimulationTimestampTaskCompletion(
    double date, const std::shared_ptr<WorkflowTask> &task)
    : SimulationTimestampTask(date, task) {
  WRENCH_DEBUG("Inserting a TaskCompletion timestamp for task '%s'",
               task->getID().c_str());
 ...
 * @param task: the WorkflowTask associated with this timestamp
 */
SimulationTimestampTaskTermination::SimulationTimestampTaskTermination(
    double date, const std::shared_ptr<WorkflowTask> &task)
    : SimulationTimestampTask(date, task) {
  WRENCH_DEBUG("Inserting a TaskTerminated timestamp for task '%s'",
               task->getID().c_str());

#ifdef WRENCH_INTERNAL_EXCEPTIONS
  if (task == nullptr) {
    throw std::invalid_argument(
        "SimulationTimestampTaskTerminated::SimulationTimestampTaskTerminated()"
        " requires a valid pointer to a WorkflowTask object");
  }
#endif
 ...
 */
SimulationTimestampFileCopy::SimulationTimestampFileCopy(
    double date, std::shared_ptr<DataFile> file,
    std::shared_ptr<FileLocation> src_location,
    std::shared_ptr<FileLocation> dst_location)
    : file(std::move(file)), source(std::move(src_location)),
      destination(std::move(dst_location)) {
  this->date = date;
}
...
/**
 * @brief A static unordered multimap of SimulationTimestampFileCopyStart
 * objects that have yet to be matched with Failure, Terminated or Completion
 * timestamps
 */
std::unordered_multimap<FileCopy, SimulationTimestampFileCopy *>
    SimulationTimestampFileCopy::pending_file_copies;

/**
 * @brief Sets the endpoint of the calling object
 * (SimulationTimestampFileCopyFailure, SimulationTimestampFileCopyCompletion,
 * SimulationTimestampFileCopyStart) with a SimulationTimestampFileCopyStart
 * object
 */
void SimulationTimestampFileCopy::setEndpoints() {
  auto pending_copies_itr = pending_file_copies.find(
      FileCopy(this->file, this->source, this->destination));
  if (pending_copies_itr != pending_file_copies.end()) {
    ...
    // the SimulationTimestampFileCopyStart is no longer waiting to be matched
    // with an end timestamp, remove it from the map
    pending_file_copies.erase(pending_copies_itr);
  } else {
    throw std::runtime_error(
        "SimulationTimestampFileCopy::setEndpoints() could not find a "
        "SimulationTimestampFileCopyStart object.");
  }
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampFileCopyStart::SimulationTimestampFileCopyStart(
    double date, std::shared_ptr<DataFile> file,
    std::shared_ptr<FileLocation> src, std::shared_ptr<FileLocation> dst)
    : SimulationTimestampFileCopy(date, std::move(file), std::move(src),
                                  std::move(dst)) {
  WRENCH_DEBUG("Inserting a FileCopyStart timestamp for file copy");

  // all information about a file copy should be passed
#ifdef WRENCH_INTERNAL_EXCEPTIONS
  if ((this->file == nullptr) || (this->source == nullptr) ||
      (this->destination == nullptr)) {
    throw std::invalid_argument(
        "SimulationTimestampFileCopyStart::SimulationTimestampFileCopyStart() "
        "cannot take nullptr arguments");
  }
#endif

  pending_file_copies.insert(std::make_pair(
      FileCopy(this->file, this->source, this->destination), this));
}
...
SimulationTimestampFileCopyFailure::SimulationTimestampFileCopyFailure(
  ...
  if ((this->file == nullptr) || (this->source == nullptr) ||
      (this->destination == nullptr)) {
    throw std::invalid_argument(
        "SimulationTimestampFileCopyFailure::"
        "SimulationTimestampFileCopyFailure() cannot take nullptr arguments");
  }
#endif
...
SimulationTimestampFileCopyCompletion::SimulationTimestampFileCopyCompletion(
  ...
  // Making this an internal exception causes problems, because sometimes the
  // exception is expected by the caller (for file copy operations)
  //#ifdef WRENCH_INTERNAL_EXCEPTIONS
  // all information about a file copy should be passed
  if ((this->file == nullptr) || (this->source == nullptr) ||
      (this->destination == nullptr)) {
    throw std::invalid_argument("SimulationTimestampFileCopyCompletion::"
 ...
 */
SimulationTimestampFileRead::SimulationTimestampFileRead(
    double date, std::shared_ptr<DataFile> file,
    std::shared_ptr<FileLocation> src_location,
    std::shared_ptr<StorageService> service, std::shared_ptr<WorkflowTask> task)
    : file(std::move(file)), source(std::move(src_location)),
      service(std::move(std::move(service))), task(std::move(task)) {
  this->date = date;
}
...
/**
 * @brief A static unordered multimap of SimulationTimestampFileReadStart
 * objects that have yet to be matched with Failure, Terminated or Completion
 * timestamps
 */
std::unordered_multimap<FileReadWrite, std::pair<SimulationTimestampFileRead *,
                                                 std::shared_ptr<WorkflowTask>>>
    SimulationTimestampFileRead::pending_file_reads;

/**
 * @brief Sets the endpoint of the calling object
 * (SimulationTimestampFileReadFailure, SimulationTimestampFileReadTerminated,
 * SimulationTimestampFileReadStart) with a SimulationTimestampFileReadStart
 * object
 */
void SimulationTimestampFileRead::setEndpoints() {
  // find the SimulationTimestampFileRead object containing the same task
  auto pending_reads_itr = pending_file_reads.find(
      FileReadWrite(this->file, this->source, this->service));
  if (pending_reads_itr != pending_file_reads.end()) {
    ...
    // the SimulationTimestampFileReadStart is no longer waiting to be matched
    // with an end timestamp, remove it from the map
    pending_file_reads.erase(pending_reads_itr);
  } else {
    throw std::runtime_error(
        "SimulationTimestampFileRead::setEndpoints() could not find a "
        "SimulationTimestampFileReadStart object.");
  }
}
...
SimulationTimestampFileReadStart::SimulationTimestampFileReadStart(
    double date, std::shared_ptr<DataFile> file,
    std::shared_ptr<FileLocation> src, std::shared_ptr<StorageService> service,
    std::shared_ptr<WorkflowTask> task)
    : SimulationTimestampFileRead(date, std::move(file), std::move(src),
                                  std::move(service), std::move(task)) {
  ...
  pending_file_reads.insert(
      std::make_pair(FileReadWrite(this->file, this->source, this->service),
                     std::make_pair(this, this->task)));
}
...
SimulationTimestampFileReadFailure::SimulationTimestampFileReadFailure(
    ...
    std::shared_ptr<WorkflowTask> task)
    : SimulationTimestampFileRead(date, file, src, service, std::move(task)) {
  ...
  if (file == nullptr || src == nullptr || (service == nullptr)) {
    throw std::invalid_argument(
        ...
        "SimulationTimestampFileReadFailure() requires a valid pointer to "
        "file, source and service objects");
  }
#endif
 ...
 * @param task: a task associated to  this file read (or nullptr)
 */
SimulationTimestampFileWrite::SimulationTimestampFileWrite(
    double date, std::shared_ptr<DataFile> file,
    std::shared_ptr<FileLocation> dst_location,
    std::shared_ptr<StorageService> service, std::shared_ptr<WorkflowTask> task)
    : file(std::move(file)), destination(std::move(dst_location)),
      service(std::move(service)), task(std::move(task)) {
  this->date = date;
}
...
/**
 * @brief A static unordered multimap of SimulationTimestampFileWriteStart
 * objects that have yet to be matched with Failure, Terminated or Completion
 * timestamps
 */
std::unordered_multimap<FileReadWrite, std::pair<SimulationTimestampFileWrite *,
                                                 std::shared_ptr<WorkflowTask>>>
    SimulationTimestampFileWrite::pending_file_writes;

/**
 * @brief Sets the endpoint of the calling object
 * (SimulationTimestampFileWriteFailure, SimulationTimestampFileWriteTerminated,
 * SimulationTimestampFileWriteStart) with a SimulationTimestampFileWriteStart
 * object
 */
void SimulationTimestampFileWrite::setEndpoints() {
  // find the SimulationTimestampFileWrite object containing the same task
  auto pending_writes_itr = pending_file_writes.find(
      FileReadWrite(this->file, this->destination, this->service));
  if (pending_writes_itr != pending_file_writes.end()) {
    ...
    // the SimulationTimestampFileWriteStart is no longer waiting to be matched
    // with an end timestamp, remove it from the map
    pending_file_writes.erase(pending_writes_itr);
  } else {
    throw std::runtime_error(
        "SimulationTimestampFileWrite::setEndpoints() could not find a "
        "SimulationTimestampFileWriteStart object.");
  }
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampFileWriteStart::SimulationTimestampFileWriteStart(
    double date, std::shared_ptr<DataFile> file,
    std::shared_ptr<FileLocation> dst, std::shared_ptr<StorageService> service,
    std::shared_ptr<WorkflowTask> task)
    : SimulationTimestampFileWrite(date, std::move(file), std::move(dst),
                                   std::move(service), std::move(task)) {
  WRENCH_DEBUG("Inserting a FileWriteStart timestamp for file write");

#ifdef WRENCH_INTERNAL_EXCEPTIONS
  // all information about a file write should be passed
  if ((this->file == nullptr) || (this->destination == nullptr) ||
      (this->service == nullptr)) {
    throw std::invalid_argument(
        "SimulationTimestampFileWriteStart::SimulationTimestampFileWriteStart()"
        " cannot take nullptr arguments");
  ...
  pending_file_writes.insert(std::make_pair(
      FileReadWrite(this->file, this->destination, this->service),
      std::make_pair(this, this->task)));
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampFileWriteFailure::SimulationTimestampFileWriteFailure(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &dst,
    const std::shared_ptr<StorageService> &service,
    std::shared_ptr<WorkflowTask> task)
    : SimulationTimestampFileWrite(date, file, dst, service, std::move(task)) {
  ...
  if (file == nullptr || dst == nullptr || service == nullptr) {
    throw std::invalid_argument(
        ...
        "SimulationTimestampFileWriteFailure() requires a valid pointer to "
        "file, destination and service objects");
  }
#endif
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampFileWriteCompletion::SimulationTimestampFileWriteCompletion(
    double date, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<FileLocation> &dst,
    const std::shared_ptr<StorageService> &service,
    std::shared_ptr<WorkflowTask> task)
...
SimulationTimestampDiskRead::SimulationTimestampDiskRead(double date,
                                                         ...
                                                         double bytes,
                                                         int counter)
    : hostname(std::move(hostname)), mount(std::move(mount)), bytes(bytes),
      counter(counter) {
  this->date = date;
}
 ...
 */
std::string SimulationTimestampDiskRead::getMount() { return this->mount; }

/**
 * @brief retrieves the amount of bytes being read
 * @return number of bytes as double
 */
double SimulationTimestampDiskRead::getBytes() const { return this->bytes; }

/**
 * @brief To get counter of disk operation
 * @return int of counter
 */
int SimulationTimestampDiskRead::getCounter() const { return this->counter; }
...
/**
 * @brief A static unordered multimap of SimulationTimestampDiskReadStart
 * objects that have yet to be matched with Failure, Terminated or Completion
 * timestamps
 */
std::unordered_multimap<DiskAccess, SimulationTimestampDiskRead *>
    SimulationTimestampDiskRead::pending_disk_reads;

/**
 * @brief Sets the endpoint of the calling object
 * (SimulationTimestampDiskReadFailure, SimulationTimestampDiskReadTerminated,
 * SimulationTimestampDiskReadStart) with a SimulationTimestampDiskReadStart
 * object
 */
void SimulationTimestampDiskRead::setEndpoints() {
  // find the SimulationTimestampDiskRead object containing the same task
  auto pending_disk_reads_itr = pending_disk_reads.find(
      DiskAccess(this->hostname, this->mount, this->counter));
  if (pending_disk_reads_itr != pending_disk_reads.end()) {
    ...
    // the SimulationTimestampDiskReadStart is no longer waiting to be matched
    // with an end timestamp, remove it from the map
    pending_disk_reads.erase(pending_disk_reads_itr);
  } else {
    throw std::runtime_error(
        "SimulationTimestampDiskRead::setEndpoints() could not find a "
        "SimulationTimestampDiskReadStart object.");
  }
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampDiskReadStart::SimulationTimestampDiskReadStart(
    double date, std::string hostname, std::string mount, double bytes,
    int counter)
    : SimulationTimestampDiskRead(date, std::move(hostname), std::move(mount),
                                  bytes, counter) {
  ...
  pending_disk_reads.insert(std::make_pair(
      DiskAccess(this->hostname, this->mount, this->counter), this));
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampDiskReadFailure::SimulationTimestampDiskReadFailure(
    double date, const std::string &hostname, const std::string &mount,
    double bytes, int counter)
  ...
  if (hostname.empty() || mount.empty()) {
    throw std::invalid_argument(
        "SimulationTimestampDiskReadFailure::"
        "SimulationTimestampDiskReadFailure() requires a valid pointer to "
        "file, destination and service objects");
  }
#endif
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampDiskReadCompletion::SimulationTimestampDiskReadCompletion(
    double date, const std::string &hostname, const std::string &mount,
    double bytes, int counter)
    : SimulationTimestampDiskRead(date, hostname, mount, bytes, counter) {
 ...
 * @param counter: an integer ID
 */
SimulationTimestampDiskWrite::SimulationTimestampDiskWrite(double date,
                                                           std::string hostname,
                                                           std::string mount,
                                                           double bytes,
                                                           int counter)
 ...
 * @return string of mount point
 */
std::string SimulationTimestampDiskWrite::getMount() { return this->mount; }

/**
 * @brief retrieves the amount of bytes being written
 * @return number of bytes as double
 */
double SimulationTimestampDiskWrite::getBytes() const { return this->bytes; }

/**
 * @brief retrieves the counter for this disk operation
 * @return int of counter
 */
int SimulationTimestampDiskWrite::getCounter() const { return this->counter; }
...
/**
 * @brief A static unordered multimap of SimulationTimestampDiskWriteStart
 * objects that have yet to be matched with Failure, Terminated or Completion
 * timestamps
 */
std::unordered_multimap<DiskAccess, SimulationTimestampDiskWrite *>
    SimulationTimestampDiskWrite::pending_disk_writes;

/**
 * @brief Sets the endpoint of the calling object
 * (SimulationTimestampDiskWriteFailure, SimulationTimestampDiskWriteTerminated,
 * SimulationTimestampDiskWriteStart) with a SimulationTimestampDiskWriteStart
 * object
 */
void SimulationTimestampDiskWrite::setEndpoints() {
  // find the SimulationTimestampDiskWrite object containing the same task
  auto pending_disk_writes_itr = pending_disk_writes.find(
      DiskAccess(this->hostname, this->mount, this->counter));
  if (pending_disk_writes_itr != pending_disk_writes.end()) {
    ...
    // the SimulationTimestampDiskWriteStart is no longer waiting to be matched
    // with an end timestamp, remove it from the map
    pending_disk_writes.erase(pending_disk_writes_itr);
  } else {
    throw std::runtime_error(
        "SimulationTimestampDiskWrite::setEndpoints() could not find a "
        "SimulationTimestampDiskWriteStart object.");
  }
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampDiskWriteStart::SimulationTimestampDiskWriteStart(
    double date, std::string hostname, std::string mount, double bytes,
    int counter)
    : SimulationTimestampDiskWrite(date, std::move(hostname), std::move(mount),
                                   bytes, counter) {
  ...
  pending_disk_writes.insert(std::make_pair(
      DiskAccess(this->hostname, this->mount, this->counter), this));
}
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampDiskWriteFailure::SimulationTimestampDiskWriteFailure(
    double date, const std::string &hostname, const std::string &mount,
    double bytes, int counter)
  ...
  if (hostname.empty() || mount.empty()) {
    throw std::invalid_argument(
        "SimulationTimestampDiskWriteFailure::"
        "SimulationTimestampDiskWriteFailure() requires a valid pointer to "
        "file, destination and service objects");
  }
#endif
 ...
 * @throw std::invalid_argument
 */
SimulationTimestampDiskWriteCompletion::SimulationTimestampDiskWriteCompletion(
    double date, const std::string &hostname, const std::string &mount,
    double bytes, int counter)
    : SimulationTimestampDiskWrite(date, hostname, mount, bytes, counter) {
 ...
 */
SimulationTimestampPstateSet::SimulationTimestampPstateSet(
    double date, const std::string &hostname, int pstate)
  ...
  if (hostname.empty()) {
    throw std::invalid_argument(
        "SimulationTimestampPstateSet::SimulationTimestampPstateSet() requires "
        "a valid hostname");
  }
#endif
}
 ...
 */
int SimulationTimestampPstateSet::getPstate() const { return this->pstate; }

/**
 * @brief Constructor
 * @param date: the date
 * @param hostname: the host on which this energy consumption timestamp applies
 * to
 * @param joules: the energy consumption in joules
 */
SimulationTimestampEnergyConsumption::SimulationTimestampEnergyConsumption(
  ...
  if (hostname.empty() || joules < 0.0) {
    throw std::invalid_argument(
        ...
        "SimulationTimestampEnergyConsumption() requires a valid hostname and "
        "an energy usage amount >= 0.0");
  }
#endif
}
 ...
 */
SimulationTimestampLinkUsage::SimulationTimestampLinkUsage(
    double date, const std::string &linkname, double bytes_per_second)
    : linkname(linkname), bytes_per_second(bytes_per_second) {
  this->date = date;
#ifdef WRENCH_INTERNAL_EXCEPTIONS
  if (linkname.empty() || bytes_per_second < 0.0) {
    throw std::invalid_argument(
        "SimulationTimestampLinkUsage::SimulationTimestampLinkUsage() requires "
        "a valid linkname and a link usage amount >= 0.0");
  }
#endif
}
