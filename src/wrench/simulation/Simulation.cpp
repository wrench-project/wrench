...
Simulation::Simulation() {
  ...
  if (previous_handler == SIG_ERR) {
    std::cerr << "SIGABRT handler setup failed... uncaught exceptions will "
                 "lead to unclean terminations\n";
  }
...
/**
 * @brief Initialize the simulation, which parses out WRENCH-specific and
 * SimGrid-specific command-line arguments, if any
 *
...
void Simulation::init(int *argc, char **argv) {
  if (Simulation::initialized) {
    throw std::runtime_error(
        "Simulation::init(): Simulation already initialized");
  }

  if (*argc < 1) {
    throw std::invalid_argument(
        "Simulation::init(): Invalid argc argument (must be >= 1)");
  }
  if ((argv == nullptr) || (*argv == nullptr)) {
    throw std::invalid_argument(
        "Simulation::init(): Invalid argument argv (nullptr)");
  }
  ...
  for (i = 0; i < *argc; i++) {
    if (!strncmp(argv[i],
                 "--cfg=surf/precision:", strlen("--cfg=surf/precision:"))) {
  ...
  for (i = 0; i < *argc; i++) {
    if (not strcmp(argv[i], "--help")) {
    ...
    } else if ((not strcmp(argv[i], "--wrench-no-color")) or
               (not strcmp(argv[i], "--wrench-no-colors"))) {
    ...
    } else if (not strncmp(argv[i], "--wrench-commport-pool-size",
                           strlen("--wrench-commport-pool-size"))) {
      char *equal_sign = strchr(argv[i], '=');
      if (!equal_sign) {
        throw std::invalid_argument(
            "Invalid --wrench-commport-pool-size argument value");
      }
    ...
    } else if (not strncmp(argv[i], "--wrench-default-control-message-size",
                           strlen("--wrench-default-control-message-size"))) {
      char *equal_sign = strchr(argv[i], '=');
      if (!equal_sign) {
        throw std::invalid_argument(
            "Invalid --wrench-default-control-message-size argument value");
      }
      // Check that the value is all digits
      char *ptr = equal_sign + 1;
      while (*ptr) {
      ...
      throw std::invalid_argument("The use of --wrench-pagecache-simulation "
                                  "has been deprecated for now");
  ...
  simgrid::s4u::Host::on_onoff.connect([](simgrid::s4u::Host const &h) {
    if (not Simulation::host_shutdown_enabled) {
      throw std::runtime_error(
          "It looks like you are simulating host failures/shutdowns during the "
          "simulated execution."
          " Please restart your simulation passing it the "
          "--wrench-host-shutdown-simulation command-line flag.");
    }
  });
  ...
  // that the --wrench-host-shutdown-simulation flag should have been passed
  // to the simulator if host shutdowns are to be simulated
  simgrid::s4u::Link::on_onoff_cb([](simgrid::s4u::Link const &l) {
    if (not Simulation::link_shutdown_enabled) {
      throw std::runtime_error(
          "It looks like you are simulating link failures/shutdowns during the "
          "simulated execution."
          " Please restart your simulation passing it the "
          "--wrench-link-shutdown-simulation command-line flag.");
    }
  });
  ...
  if (wrench_help_requested) {
    ...
    std::cout
        << "   --wrench-energy-simulation: activates SimGrid's energy plugin\n";
    std::cout << "     (requires host pstate definitions in XML platform "
                 "description file)\n";
    std::cout
        << "   --wrench-host-shutdown-simulation: activates WRENCH's "
           "capability to simulate host failures/shutdowns during execution\n";
    std::cout
        << "   --wrench-link-shutdown-simulation: activates WRENCH's "
           "capability to simulate link failures/shutdowns during execution\n";
    std::cout << "   --wrench-pagecache-simulation: Activate the in-memory "
                 "(Linux) page cache simulation (which ";
    std::cout << "                requires that all hosts in the platform have "
                 "a disk mounted at '/memory' that )\n";
    std::cout << "                acts as additional RAM that can only be used "
    ...
    std::cout
        << "     (use --log=xxx.threshold=info to enable log category xxxx)\n";
    std::cout << "   --help-logs for detailed help on (SimGrid's) logging "
                 "options/syntax)\n";
    std::cout << "   --help-simgrid: show full help on general Simgrid "
                 "command-line arguments\n";
    std::cout << "   --wrench-commport-pool-size=<integer>: set the number of "
                 "communication ports used by WRENCH (default: 50000).\n";
    std::cout << "      This value may need to be increased, especially for "
                 "simulations that simulate many\n";
    std::cout << "      failures, for which WRENCH has a hard time avoiding "
                 "all commport-related memory leaks\n";
    std::cout << "   --wrench-default-control-message-size=<double>: the "
                 "default size of control messages in bytes (default: 0). \n";
    std::cerr << "\n";
  }

  *argc = 0;
  for (const auto &a : cleanedup_args) {
    // free(argv[(*argc)]);//you cant free the base args, so no one is going to
    // try to free ours. This is just going to have to stay a memory leak
  ...
  if (version_requested) {
    std::cout << "This program was linked against WRENCH version "
              << Simulation::getWRENCHVersionString();
    ...
    std::cout << "\n\nTo cite WRENCH in a publication, please use:\n"
                 ...
                 "    author = {Casanova, Henri and Ferreira da Silva, Rafael "
                 "and Tanaka, Ryan and Pandey, Suraj and Jethwani, Gautam and "
  ...
  // If simulator help requested, put back in the "--help" argument that was
  // passed down
...
void Simulation::instantiatePlatform(const std::string &filename) {
  if (not this->s4u_simulation->isInitialized()) {
    throw std::runtime_error(
        "Simulation::instantiatePlatform(): Simulation is not initialized");
  }
  if (already_setup) {
    throw std::runtime_error(
        "Simulation::instantiatePlatform(): Platform already setup");
  }
 ...
 * @brief Instantiate a simulated platform
 *
 * @param platform: the string representation of a SimGrid XML platform
 * description
 *
 * @throw std::runtime_error
 */
void Simulation::instantiatePlatformFromString(const std::string &platform) {
  ...
  auto temp_path = temp_dir / ("tmp_wrench_platform_" + to_string(::getpid()) +
                               "_" + to_string(std::time(nullptr)) + ".xml");
  ofstream myfile;
 ...
 */
void Simulation::instantiatePlatform(
    const std::function<void()> &creation_function) {
  if (not this->s4u_simulation->isInitialized()) {
    throw std::runtime_error(
        "Simulation::instantiatePlatform(): Simulation is not initialized");
  }
 ...
 */
std::vector<std::string> Simulation::getRoute(std::string &src_host,
                                              std::string &dst_host) {
...
/**
 * @brief Obtains the current link bandwidth usage on a link and will add
 * SimulationTimestampLinkUsage to simulation output if record_as_time_stamp is
 * set to true
 *
 * @param link_name: the link's name
 * @param record_as_time_stamp: bool signaling whether or not to record a
 * SimulationTimestampLinkUsage object
...
double Simulation::getLinkUsage(const std::string &link_name,
  ...
  if (link_name.empty()) {
    throw std::invalid_argument(
        "Simulation::getLinkUsage() requires a valid link name");
  }
  ...
  if (record_as_time_stamp) {
    this->getOutput().addTimestampLinkUsage(
        Simulation::getCurrentSimulatedDate(), link_name, usage);
  }
...
///{ /        for (auto mem_mng: this->memory_managers) { /            if
///(strcmp(mem_mng.get()->getHostname().c_str(), hostname.c_str()) == 0) { /
///return mem_mng.get(); /            } /        } /        return nullptr; / }

/**
 * @brief Get the list of names of all the hosts in each cluster composing the
 * platform
 *
...
Simulation::getHostnameListByCluster() {
...
void Simulation::launch() {
  ...
  for (const auto &hostname : this->getHostnameList()) {
    this->getOutput().addTimestampPstateSet(
        Simulation::getCurrentSimulatedDate(), hostname,
        getCurrentPstate(hostname));
  }
...
void Simulation::checkSimulationSetup() {
  ...
  if (this->execution_controllers.empty()) {
    throw std::runtime_error("An execution controller should have been "
                             "instantiated and passed to Simulation.add()");
  }
}
...
void Simulation::startAllProcesses() {
  ...
  for (const auto &execution_controller : this->execution_controllers) {
    execution_controller->start(execution_controller,
                                execution_controller->daemonized,
                                false); // Not daemonized, no auto-restart
  }
  ...
  //                    compute_service->getScratch()->start(compute_service->getScratchSharedPtr(),
  //                    true,
  //                                                         false);//
  //                                                         Daemonized, no
  //                                                         auto-restart
  ...
  for (const auto &network_proximity_service :
       this->network_proximity_services) {
...
void Simulation::addService(const std::shared_ptr<ComputeService> &service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::addService(): invalid argument (nullptr service)");
  }
 ...
 * @throw std::invalid_argument
 */
void Simulation::addService(
    const std::shared_ptr<NetworkProximityService> &service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::addService(): invalid argument (nullptr service)");
  }
 ...
 */
void Simulation::addService(
    const std::shared_ptr<ExecutionController> &service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::addService(): invalid argument (nullptr service)");
  }
 ...
 */
void Simulation::addService(
    const std::shared_ptr<FileRegistryService> &service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::addService(): invalid argument (nullptr service)");
  }
 ...
 */
void Simulation::addService(
    const std::shared_ptr<EnergyMeterService> &service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::addService(): invalid argument (nullptr service)");
  }
 ...
 */
void Simulation::addService(
    const std::shared_ptr<BandwidthMeterService> &service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::addService(): invalid argument (nullptr service)");
  }
...
void Simulation::addService(
  ...
  if (memory_manager == nullptr) {
    throw std::invalid_argument(
        "Simulation::addService(): invalid argument (nullptr memory_manager)");
  }
...
/**
 * @brief Stage a copy of a file at a location (and add entries to all file
...
void Simulation::stageFile(const std::shared_ptr<FileLocation> &location) {
  if (location == nullptr) {
    throw std::invalid_argument(
        "Simulation::stageFile(): Invalid nullptr arguments");
  }

  if (std::dynamic_pointer_cast<CompoundStorageService>(
          location->getStorageService())) {
    throw std::invalid_argument(
        "Simulation::stageFile(): Can't stage on CompoundStorageService (it is "
        "only an abstraction layer)");
  }

  if (this->is_running) {
    throw std::runtime_error(" Simulation::stageFile(): Cannot stage a file "
  ...
  try {
    ...
    throw std::invalid_argument(
        "Simulation::stageFile(): Not enough space on disk");
  }
 ...
 * @param write_bandwidth_in_bytes_per_sec: the disk's write bandwidth in
 * byte/sec
 * @param capacity_in_bytes: the disk's capacity in bytes
 * @param mount_point: the disk's mount point (most people use "/")
 */
void Simulation::createNewDisk(const std::string &hostname,
  ...
  S4U_Simulation::createNewDisk(
      hostname, disk_id, read_bandwidth_in_bytes_per_sec,
      write_bandwidth_in_bytes_per_sec, capacity_in_bytes, mount_point);
}
 ...
 */
void Simulation::readFromDisk(double num_bytes, const std::string &hostname,
                              const std::string &mount_point,
                              simgrid::s4u::Disk *disk) {
  unique_disk_sequence_number += 1;
  int temp_unique_sequence_number = unique_disk_sequence_number;
  this->getOutput().addTimestampDiskReadStart(
      Simulation::getCurrentSimulatedDate(), hostname, mount_point, num_bytes,
      temp_unique_sequence_number);
  try {
    S4U_Simulation::readFromDisk(num_bytes, hostname, mount_point, disk);
  } catch (const std::invalid_argument &ia) {
    this->getOutput().addTimestampDiskReadFailure(
        Simulation::getCurrentSimulatedDate(), hostname, mount_point, num_bytes,
        temp_unique_sequence_number);
    throw;
  }
 ...
 * @throw invalid_argument
 */
void Simulation::readFromDiskAndWriteToDiskConcurrently(
    double num_bytes_to_read, double num_bytes_to_write,
    const std::string &hostname, const std::string &read_mount_point,
    const std::string &write_mount_point, simgrid::s4u::Disk *src_disk,
    simgrid::s4u::Disk *dst_disk) {
  unique_disk_sequence_number += 1;
  int temp_unique_sequence_number = unique_disk_sequence_number;
  this->getOutput().addTimestampDiskReadStart(
      Simulation::getCurrentSimulatedDate(), hostname, read_mount_point,
      num_bytes_to_read, temp_unique_sequence_number);
  this->getOutput().addTimestampDiskWriteStart(
      Simulation::getCurrentSimulatedDate(), hostname, write_mount_point,
      num_bytes_to_write, temp_unique_sequence_number);
  try {
    S4U_Simulation::readFromDiskAndWriteToDiskConcurrently(
        num_bytes_to_read, num_bytes_to_write, hostname, read_mount_point,
        write_mount_point, src_disk, dst_disk);
  } catch (const std::invalid_argument &ia) {
    this->getOutput().addTimestampDiskWriteFailure(
        Simulation::getCurrentSimulatedDate(), hostname, write_mount_point,
        num_bytes_to_write, temp_unique_sequence_number);
    this->getOutput().addTimestampDiskReadFailure(
        Simulation::getCurrentSimulatedDate(), hostname, read_mount_point,
  ...
  this->getOutput().addTimestampDiskWriteCompletion(
      Simulation::getCurrentSimulatedDate(), hostname, write_mount_point,
      num_bytes_to_write, temp_unique_sequence_number);
  this->getOutput().addTimestampDiskReadCompletion(
      Simulation::getCurrentSimulatedDate(), hostname, read_mount_point,
 ...
 * @throw invalid_argument
 */
void Simulation::writeToDisk(double num_bytes, const std::string &hostname,
                             const std::string &mount_point,
                             simgrid::s4u::Disk *disk) {
  unique_disk_sequence_number += 1;
  int temp_unique_sequence_number = unique_disk_sequence_number;
 ...
 */
void Simulation::readWithMemoryCache(
    const std::shared_ptr<DataFile> &file, double n_bytes,
  ...
  int temp_unique_sequence_number = unique_disk_sequence_number;
  this->getOutput().addTimestampDiskReadStart(
      Simulation::getCurrentSimulatedDate(), hostname, location->getPath(),
      n_bytes, temp_unique_sequence_number);
  ...
  mem_mng->flush(n_bytes + from_disk - mem_mng->getFreeMemory() -
...
/**
 * @brief Write a file locally with writeback strategy, only available if
 * writeback is activated.
 *
 * @param file: workflow file
 ...
 */
void Simulation::writebackWithMemoryCache(
    const std::shared_ptr<DataFile> &file, double n_bytes,
    const std::shared_ptr<FileLocation> &location, bool is_dirty) {
  std::string hostname = getHostName();
  ...
  this->getOutput().addTimestampDiskWriteStart(
      Simulation::getCurrentSimulatedDate(), hostname,
      location->getMountPoint(), n_bytes, temp_unique_sequence_number);

  MemoryManager *mem_mng = this->getMemoryManagerByHost(hostname);

  double remaining_dirty = 0;
  if (is_dirty) {
    remaining_dirty = mem_mng->getDirtyRatio() * mem_mng->getAvailableMemory() -
                      mem_mng->getDirty();
  } else {
  ...
  if (remaining_dirty > 0) {
    mem_mng->evict(std::min(n_bytes, remaining_dirty) -
                       mem_mng->getFreeMemory(),
                   file->getID());
  ...
  // if dirty_ratio is reached, dirty data needs to be flushed to disk to write
  // new data
  while (remaining > 0) {
...
/**
 * @brief Write-through a file locally, only available if writeback is
 * activated.
 *
 * @param file: workflow file
 * @param n_bytes: number of written bytes
 * @param location: file location
 */
void Simulation::writeThroughWithMemoryCache(
    const std::shared_ptr<DataFile> &file, double n_bytes,
    const std::shared_ptr<FileLocation> &location) {
  std::string hostname = getHostName();
  ...
  // Write to disk
  auto ss = std::dynamic_pointer_cast<SimpleStorageService>(
      location->getStorageService());
  if (!ss) {
    throw...
  }
 ...
 * @return true or false
 */
bool Simulation::hostHasMountPoint(const std::string &hostname,
                                   const std::string &mount_point) {
...
/**
 * @brief Get the memory_manager_service capacity of the host on which the
...
/**
 * @brief Get the number of cores of the host on which the calling process is
 * running
 * @return a number of cores
 */
...
/**
 * @brief Get the flop rate of one core of the host on which the calling process
 * is running
 * @return a flop rate
 */
double Simulation::getFlopRate() { return S4U_Simulation::getFlopRate(); }

/**
 * @brief Get the name of the host on which the calling process is running
 * @return a host name
 */
std::string Simulation::getHostName() { return S4U_Simulation::getHostName(); }

/**
 * @brief Make the calling process sleep for a number of (simulated) seconds
 * @param duration: a number of seconds
 */
void Simulation::sleep(double duration) { S4U_Simulation::sleep(duration); }

/**
 * @brief Make the calling process compute
 * @param flops: a number of floating point operations
 */
void Simulation::compute(double flops) { S4U_Simulation::compute(flops); }
...
void Simulation::computeMultiThreaded(unsigned long num_threads,
  ...
  S4U_Simulation::compute_multi_threaded(num_threads, thread_creation_overhead,
                                         sequential_work,
 ...
 */
SimulationOutput &Simulation::getOutput() { return this->output; }
...
/**
 * @brief Obtains the current energy consumption of a host and will add
 * SimulationTimestampEnergyConsumption to simulation output if can_record is
 * set to true
 * @param hostname: the host name
 * @param record_as_time_stamp: bool signaling whether or not to record a
 * SimulationTimestampEnergyConsumption object
 * @return current energy consumption in joules
 * @throws std::invalid_argument
 */
double Simulation::getEnergyConsumed(const std::string &hostname,
  ...
  if (hostname.empty()) {
    throw std::invalid_argument(
        "Simulation::getEnergyConsumed() requires a valid hostname");
  }
  ...
  if (record_as_time_stamp) {
    this->getOutput().addTimestampEnergyConsumption(
        Simulation::getCurrentSimulatedDate(), hostname, consumption);
  }
 ...
 * @return current energy consumption in joules for each host, as a map indexed
 * by hostnames
 * @throws std::invalid_argument
 */
std::map<std::string, double>
Simulation::getEnergyConsumed(const std::vector<std::string> &hostnames) {
 ...
 * set to true
 * @param hostnames: the list of hostnames
 * @param record_as_time_stamps: whether or not to record a
 * SimulationTimestampEnergyConsumption object for each host when this method is
 * called
 * @return current energy consumption in joules for each host, as a map indexed
 * by hostnames
 * @throws std::invalid_argument
...
Simulation::getEnergyConsumed(const std::vector<std::string> &hostnames,
  ...
  for (auto const &h : hostnames) {
    energy_consumptions[h] =
        Simulation::getEnergyConsumed(h, record_as_time_stamps);
  }
 ...
 * @param hostname: the host name
 * @param pstate: the power state index (as specified in the platform xml
 * description file)
 */
void Simulation::setPstate(const std::string &hostname, int pstate) {
  S4U_Simulation::setPstate(hostname, pstate);
  this->getOutput().addTimestampPstateSet(Simulation::getCurrentSimulatedDate(),
                                          hostname, pstate);
}
 ...
 * @param hostname: the host name
 * @return The number of power states available for the host (as specified in
 * the platform xml description file)
 */
 ...
 * @param hostname: the host name
 * @return The index of the current pstate of the host (as specified in the
 * platform xml description file)
 */
 ...
 * @param hostname: the host name
 * @return a list of power states available for the host (as specified in the
 * platform xml description file)
 */
...
/**
 * @brief Get the minimum power consumption for the host (i.e., idling) at its
 * current pstate
 * @param hostname: the host name
 * @return The "idling" power consumption (as specified in the platform xml
 * description file)
 */
...
/**
 * @brief Get the maximum power consumption for the host (i.e., 100%
 * utilization) at its current pstate
 * @param hostname: the host name
 * @return The "100% used" power consumption (as specified in the platform xml
 * description file)
 */
...
/**
 * @brief Starts a new compute service during execution (i.e., one that was not
 * passed to Simulation::add() before Simulation::launch() was called). The
 * simulation takes ownership of the reference and will call the destructor.
...
Simulation::startNewService(ComputeService *service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::startNewService(): invalid argument (nullptr service)");
  }

  if (not this->is_running) {
    throw std::runtime_error(
        "Simulation::startNewService(): simulation is not running yet");
  }

  service->simulation = this;
  std::shared_ptr<ComputeService> shared_ptr =
      std::shared_ptr<ComputeService>(service);
  ...
  //            service->getScratch()->start(service->getScratchSharedPtr(),
  //            true, false);// Daemonized, no auto-restart
  //        }
...
/**
 * @brief Starts a new storage service during  execution (i.e., one that was not
 * passed to Simulation::add() before Simulation::launch() was called). The
 * simulation takes ownership of the reference and will call the destructor.
 * @param service: An instance of a service
 * @return A pointer to the service instance
...
Simulation::startNewService(StorageService *service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::startNewService(): invalid argument (nullptr service)");
  }
  ...
  service->simulation = this;
  std::shared_ptr<StorageService> shared_ptr =
...
/**
 * @brief Starts a new network proximity service during execution (i.e., one
 * that was not passed to Simulation::add() before Simulation::launch() was
 * called). The simulation takes ownership of the reference and will call the
 * destructor.
 * @param service: An instance of a service
 * @return A pointer to the service instance
 * @throw std::invalid_argument
 * @throw std::runtime_error
 */
std::shared_ptr<NetworkProximityService>
Simulation::startNewService(NetworkProximityService *service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::startNewService(): invalid argument (nullptr service)");
  }
  ...
  service->simulation = this;
  std::shared_ptr<NetworkProximityService> shared_ptr =
      std::shared_ptr<NetworkProximityService>(service);
...
/**
 * @brief Starts a new file registry service during execution (i.e., one that
 * was not passed to Simulation::add() before Simulation::launch() was called).
 * The simulation takes ownership of the reference and will call the destructor.
 * @param service: An instance of a service
 * @return A pointer to the service instance
 * @throw std::invalid_argument
 * @throw std::runtime_error
...
Simulation::startNewService(FileRegistryService *service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::startNewService(): invalid argument (nullptr service)");
  }
  ...
  service->simulation = this;
  std::shared_ptr<FileRegistryService> shared_ptr =
      std::shared_ptr<FileRegistryService>(service);
...
#ifdef PAGE_CACHE_SIMULATION
/**
 * @brief Starts a new memory_manager_service manager service during execution
 * (i.e., one that was not passed to Simulation::add() before
...
Simulation::startNewService(MemoryManager *service) {
  if (service == nullptr) {
    throw std::invalid_argument(
        "Simulation::startNewService(): invalid argument (nullptr service)");
  }
  ...
  service->simulation = this;
  std::shared_ptr<MemoryManager> shared_ptr =
      std::shared_ptr<MemoryManager>(service);
...
void Simulation::platformSanityCheck() {
  ...
  for (auto const &h : hostnames) {
    ...
    for (unsigned int i = 0; i < disks.size(); i++) {
      for (unsigned int j = 0; j < disks.size(); j++) {
        ...
        if (disks[i] == disks[j]) {
          throw std::invalid_argument(
              "Simulation::platformSanityCheck(): Host " + h +
              " has two disks with the same mount point '" + disks[i] + "'");
        }
        if ((disks[j] != "/") and (disks[i] != "/") and
            (FileLocation::properPathPrefix(
                FileLocation::sanitizePath(disks[i]),
                FileLocation::sanitizePath(disks[j])))) {
          throw std::invalid_argument(
              "Simulation::platformSanityCheck(): Host " + h +
  ...
  //                    throw
  //                    std::invalid_argument("Simulation::platformSanityCheck():
  //                    For now, disks must have equal "
  //                                                "read and write bandwidth
  //                                                (offending disk: " + h + ":"
  //                                                + d->get_property("mount"));
  //                }
  //            }
  //        }
  ...
  if (this->isPageCachingEnabled()) {
    for (auto const &h : hostnames) {
      ...
      for (auto const &d :
           S4U_Simulation::get_host_or_vm_by_name(h)->get_disks()) {
      ...
      if (not has_memory_disk) {
        throw std::invalid_argument(
            "Simulation::platformSanityCheck(): Since --pagecache was passed, "
 ...
 *
 * @return the DataFile instance (or throws a std::invalid_argument if not
 * found)
 *
 * @throw std::invalid_argument
 */
std::shared_ptr<DataFile> Simulation::getFileByID(const std::string &id) {
  if (Simulation::data_files.find(id) == Simulation::data_files.end()) {
    throw std::invalid_argument(
        "Workflow::getFileByID(): Unknown DataFile ID " + id);
  } else {
...
/**
 * @brief Add a new file to the simulation (use at your own peril if you're
 * using the workflow API - use Workflow::addFile() instead)
 *
 ...
 * @throw std::invalid_argument
 */
std::shared_ptr<DataFile> Simulation::addFile(const std::string &id,
                                              double size) {
  if (size < 0) {
...
/**
 * @brief Remove a file from the simulation (use at your own peril if you're
 * using the workflow API - use Workflow::removeFile() instead)
 * @param file : file to remove
 */
void Simulation::removeFile(const std::shared_ptr<DataFile> &file) {
  if (Simulation::data_files.find(file->getID()) ==
      Simulation::data_files.end()) {
...
/**
 * @brief Remove all files from the simulation (use at your own peril if you're
 * using the workflow API)
 */
void Simulation::removeAllFiles() { Simulation::data_files.clear(); }

/**
 * @brief Determine if the simulation has been initialized
 * @return  true or false
 */
bool Simulation::isInitialized() { return Simulation::initialized; }
