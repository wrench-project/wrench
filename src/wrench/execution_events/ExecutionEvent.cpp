...
WRENCH_LOG_CATEGORY(wrench_core_workflow_execution_event,
                    "Log category for Workflow Execution Event");
...
ExecutionEvent::waitForNextExecutionEvent(S4U_CommPort *commport,
                                          double timeout) {
  ...
  if (auto m = std::dynamic_pointer_cast<JobManagerCompoundJobCompletedMessage>(
          message)) {
    return std::shared_ptr<CompoundJobCompletedEvent>(
        new CompoundJobCompletedEvent(m->job, m->compute_service));

  } else if (auto m =
                 std::dynamic_pointer_cast<JobManagerCompoundJobFailedMessage>(
                     message)) {
    return std::shared_ptr<CompoundJobFailedEvent>(
        new CompoundJobFailedEvent(m->job, m->compute_service, m->cause));

  } else if (auto m = std::dynamic_pointer_cast<
                 JobManagerStandardJobCompletedMessage>(message)) {
    std::set<std::shared_ptr<WorkflowTask>> failure_count_increments;
    m->job->applyTaskUpdates(m->necessary_state_changes,
    ...
    return std::shared_ptr<StandardJobCompletedEvent>(
        new StandardJobCompletedEvent(m->job, m->compute_service));
                 ...
                 std::dynamic_pointer_cast<JobManagerStandardJobFailedMessage>(
                     message)) {
    m->job->applyTaskUpdates(m->necessary_state_changes,
                             m->necessary_failure_count_increments);
    ...
    return std::shared_ptr<PilotJobStartedEvent>(
        new PilotJobStartedEvent(m->job, m->compute_service));

  } else if (auto m = std::dynamic_pointer_cast<
                 ComputeServicePilotJobExpiredMessage>(message)) {
    return std::shared_ptr<PilotJobExpiredEvent>(
        new PilotJobExpiredEvent(m->job, m->compute_service));

  } else if (auto m =
                 std::dynamic_pointer_cast<DataManagerFileCopyAnswerMessage>(
                     message)) {
    if (m->success) {
      return std::shared_ptr<FileCopyCompletedEvent>(
          new FileCopyCompletedEvent(m->src_location, m->dst_location));

    } else {
      return std::shared_ptr<FileCopyFailedEvent>(new FileCopyFailedEvent(
          m->src_location, m->dst_location, m->failure_cause));
    }
  } else if (auto m =
                 std::dynamic_pointer_cast<DataManagerFileReadAnswerMessage>(
                     message)) {
    if (m->success) {
      return std::shared_ptr<FileReadCompletedEvent>(
          new FileReadCompletedEvent(m->location, m->num_bytes));
      ...
      return std::shared_ptr<FileReadFailedEvent>(
          new FileReadFailedEvent(m->location, m->num_bytes, m->failure_cause));
                 ...
                 std::dynamic_pointer_cast<DataManagerFileWriteAnswerMessage>(
                     message)) {
    if (m->success) {
      return std::shared_ptr<FileWriteCompletedEvent>(
          new FileWriteCompletedEvent(m->location));
      ...
      return std::shared_ptr<FileWriteFailedEvent>(
          new FileWriteFailedEvent(m->location, m->failure_cause));
                 ...
                 ExecutionControllerAlarmTimerMessage>(message)) {
    return std::shared_ptr<TimerEvent>(new TimerEvent(m->message));
  } else {
    throw std::runtime_error(
        "ExecutionEvent::waitForNextExecutionEvent(): Non-handled message type "
        "when generating execution event (" +
        message->getName() + ")");
  }
}
