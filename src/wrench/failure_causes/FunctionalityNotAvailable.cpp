...
WRENCH_LOG_CATEGORY(wrench_core_functionality_not_available,
                    "Log category for FunctionalityNotAvailable");
 ...
 * @param functionality_name: a description of the functionality that's not
 * available
 */
FunctionalityNotAvailable::FunctionalityNotAvailable(
    std::shared_ptr<Service> service, std::string functionality_name) {
...
std::string FunctionalityNotAvailable::toString() {
  return "The request functionality (" + this->functionality_name +
         ") is not available on service " + this->service->getName();
}
