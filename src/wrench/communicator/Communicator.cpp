...
Communicator::createCommunicator(unsigned long size) {
  if (size <= 1) {
    throw std::invalid_argument(
        "Communicator::createCommunicator(): invalid argument");
  }
...
/**
 * @brief Join the communicator and obtain a rank, which will block until all
 * other communicator participants have joined (or just obtain the rank and
 * return immediately if already joined).
 * @return a rank
 */
...
/**
 * @brief Join the communicator with a particular rank, which will block until
 * all other communicator participants have joined (or return immediately if
 * already joined).
...
unsigned long Communicator::join(unsigned long desired_rank) {
  ...
  if (this->rank_to_commport.find(desired_rank) !=
      this->rank_to_commport.end()) {
    if (this->actor_to_rank.find(my_pid) != this->actor_to_rank.end()) {
      return this->actor_to_rank[my_pid];
    } else {
      throw std::invalid_argument(
          "Communicator::join(): rank already used by another participant");
    }
  }
...
/**
 * @brief Perform asynchronous sends and receives operations, using standard
 * WRENCH/SimGrid point to point communications.
 * @param sends: the specification of all outgoing communications as <rank,
 * volume in bytes> pairs
 * @param num_receives: the number of expected received (from any source)
 */
void Communicator::sendAndReceive(const std::map<unsigned long, double> &sends,
                                  int num_receives) {
 ...
 * using standard WRENCH/SimGrid point to point communications
 * @param sends: the specification of all outgoing communications as <rank,
 * volume in bytes> pairs
 * @param num_receives: the number of expected received (from any source)
 * @param flops: the number of floating point operations to compute
 */
void Communicator::sendReceiveAndCompute(
    const std::map<unsigned long, double> &sends, int num_receives,
    double flops) {
  auto my_pid = simgrid::s4u::this_actor::get_pid();

  if (this->actor_to_rank.find(my_pid) == this->actor_to_rank.end()) {
    throw std::invalid_argument("Communicator::communicate(): Calling process "
  ...
  for (auto const &send_operation : sends) {
    ...
    posted_sends.push_back(dst_commport->iputMessage(
        new wrench::SimulationMessage(send_operation.second)));
  }
...
/**
 * @brief Barrier method (all participants wait for each other), using standard
 * WRENCH/SimGrid mechanisms
 */
void Communicator::barrier() {
  auto my_pid = simgrid::s4u::this_actor::get_pid();
  static unsigned long count = 0;
  if (this->actor_to_rank.find(my_pid) == this->actor_to_rank.end()) {
    throw std::invalid_argument("Communicator::barrier(): Calling process is "
                                "not part of this communicator");
  }
...
/**
 * @brief Perform an MPI AllToAll collective, using SimGrid's SMPI
 * implementation
 *
...
void Communicator::MPI_Alltoall(double bytes, std::string config) {
  if (bytes < 1.0) {
    throw std::runtime_error(
        "Communicator::MPI_Alltoall(): invalid argument (should be >= 1.0)");
  }
  this->performSMPIOperation("Alltoall", this->participating_hosts, nullptr,
                             (int)bytes, "smpi/alltoall:" + std::move(config));
}
...
void Communicator::MPI_Bcast(int root_rank, double bytes, std::string config) {
  ...
  this->performSMPIOperation("Bcast", this->participating_hosts,
                             this->rank_to_host[root_rank], (int)bytes,
                             "smpi/bcast:" + std::move(config));
}
 ...
 * @param config: the SMPI config option
 */
void Communicator::performSMPIOperation(
    const std::string &op_name, std::vector<simgrid::s4u::Host *> &hosts,
    simgrid::s4u::Host *root_host, int data_size, const std::string &config) {
  ...
  static unsigned long count = 0;
  if (this->actor_to_rank.find(my_pid) == this->actor_to_rank.end()) {
    throw std::invalid_argument("Communicator::MPI_AllToAll(): Calling process "
                                "is not part of this communicator");
  }
  count++;
  if (count < this->size) {
    ...
    // I am the last arrived process and will be doing the entire operation with
    // temp actors Set the configuration
    ...
    if (op_name == "Alltoall") {
      ...
      throw std::runtime_error("Communicator::performSMPIOperation(): Internal "
