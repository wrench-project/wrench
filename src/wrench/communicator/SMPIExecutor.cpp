...
public:
  ...
  void operator()() {
    ...
    MPI_Alltoall(data, data_size, MPI_CHAR, data, data_size, MPI_CHAR,
                 MPI_COMM_WORLD);
...
void SMPIExecutor::performAlltoall(std::vector<simgrid::s4u::Host *> &hosts,
  ...
  std::string instance_id =
      "MPI_AllToAll_" + std::to_string(simgrid::s4u::this_actor::get_pid());
  SMPI_app_instance_start(instance_id.c_str(),
                          MPI_Alltoall_partipant(data_size), hosts);
...
void SMPIExecutor::performBarrier(std::vector<simgrid::s4u::Host *> &hosts) {
  std::string instance_id =
      "MPI_Barrier_" + std::to_string(simgrid::s4u::this_actor::get_pid());
  // Start actors to do an MPI_AllToAll
  SMPI_app_instance_start(instance_id.c_str(), MPI_Barrier_partipant(), hosts);
  SMPI_app_instance_join(instance_id.c_str());
}
...
void SMPIExecutor::performBcast(std::vector<simgrid::s4u::Host *> &hosts,
  ...
  // Make sure that the root_host is the first host in the list of hosts, so
  // that it's always rank 0
  auto it = std::find(hosts.begin(), hosts.end(), root_host);
  std::iter_swap(hosts.begin(), it);

  // Start actors to do an MPI_BCast
  std::string instance_id =
      "MPI_Bcast_" + std::to_string(simgrid::s4u::this_actor::get_pid());
  SMPI_app_instance_start(instance_id.c_str(), MPI_Bcast_partipant(data_size),
                          hosts);
  SMPI_app_instance_join(instance_id.c_str());
}
