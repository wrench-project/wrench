...
namespace wrench {
    ...
    EnergyMeter::EnergyMeter(WMS *wms, const std::map<std::string, double> &measurement_periods) :
        ...
        for (auto const &h : this->measurement_periods) {
            if (not this->simulation->hostExists(h.first)) {
                throw std::invalid_argument("EnergyMeter::EnergyMeter(): unknown host " + h.first);
            }
            if (h.second < 1) {
                throw std::invalid_argument("EnergyMeter::EnergyMeter(): measurement period must be at least 1 second (host " + h.first+")");
            }
    ...
    EnergyMeter::EnergyMeter(WMS *wms, const std::vector<std::string> &hostnames, double measurement_period) :
        ...
        for (auto const &h : hostnames) {
            if (not this->simulation->hostExists(h)) {
    ...
    int EnergyMeter::main() {
        ...
        while (life) {
            ...
            auto min_el = std::min_element(
                    ...
                    [] (decltype(this->time_to_next_measurement)::value_type &lhs, decltype(this->time_to_next_measurement)::value_type &rhs) {
