...
WRENCH_LOG_CATEGORY(wrench_core_data_movement_manager,
                    "Log category for Data Movement Manager");
 ...
 */
DataMovementManager::DataMovementManager(std::string hostname,
 ...
 */
void DataMovementManager::kill() { this->killActor(); }
 ...
 * @param file_registry_service: a file registry service to update once the file
 * copy has (successfully) completed (none if nullptr)
 *
 * @throw std::invalid_argument
 * @throw ExecutionException
 */
void DataMovementManager::initiateAsynchronousFileCopy(
  ...
  if ((src == nullptr) || (dst == nullptr)) {
    throw std::invalid_argument(
        "DataMovementManager::initiateAsynchronousFileCopy(): Invalid nullptr "
        "arguments");
  }
  if (src->getFile() != dst->getFile()) {
    throw std::invalid_argument(
        "DataMovementManager::initiateAsynchronousFileCopy(): src and dst "
        "locations should be for the same file");
  }

  DataMovementManager::CopyRequestSpecs request(src, dst,
 ...
 * @throw ExecutionException
 */
void DataMovementManager::initiateAsynchronousFileRead(
 ...
 */
void DataMovementManager::initiateAsynchronousFileRead(
    const std::shared_ptr<FileLocation> &location, const double num_bytes) {
  if ((location == nullptr)) {
    throw std::invalid_argument(
        "DataMovementManager::initiateAsynchronousFileRead(): Invalid nullptr "
        "arguments");
  }
  ...
  this->pending_file_reads.push_front(
      std::make_unique<ReadRequestSpecs>(location, num_bytes));
  // Initiate the read in a thread
  auto frt = std::make_shared<FileReaderThread>(this->hostname, this->commport,
                                                location, num_bytes);
 ...
 * @throw ExecutionException
 */
void DataMovementManager::initiateAsynchronousFileWrite(
    const std::shared_ptr<FileLocation> &location,
    const std::shared_ptr<FileRegistryService> &file_registry_service) {
  if ((location == nullptr)) {
    throw std::invalid_argument(
        "DataMovementManager::initiateAsynchronousFileWrite(): Invalid nullptr "
        "arguments");
  }

  DataMovementManager::WriteRequestSpecs request(location,
                                                 file_registry_service);
  ...
  this->pending_file_writes.push_front(
      std::make_unique<WriteRequestSpecs>(location, file_registry_service));

  // Initiate the write in a thread
  auto fwt = std::make_shared<FileWriterThread>(this->hostname, this->commport,
                                                location);
 ...
 * @throw ExecutionException
 */
void DataMovementManager::doSynchronousFileCopy(
    const std::shared_ptr<FileLocation> &src,
    const std::shared_ptr<FileLocation> &dst,
    const std::shared_ptr<FileRegistryService> &file_registry_service) {
  if ((src == nullptr) || (dst == nullptr)) {
    throw std::invalid_argument("DataMovementManager::doSynchronousFileCopy(): "
                                "Invalid nullptr arguments");
  }
  if (src->getFile() != dst->getFile()) {
    throw std::invalid_argument(
        "DataMovementManager::doSynchronousFileCopy(): src and dst locations "
        "should be for the same file");
  }
...
int DataMovementManager::main() {
  ...
  WRENCH_INFO("New Data Movement Manager starting (%s)",
...
bool DataMovementManager::processNextMessage() {
  ...
  WRENCH_INFO("Data Movement Manager got a %s message",
  ...
  if (std::dynamic_pointer_cast<ServiceStopDaemonMessage>(message)) {
    ...
    return false;
                 ...
                 std::dynamic_pointer_cast<StorageServiceFileCopyAnswerMessage>(
                     message)) {
    ...
    for (auto it = this->pending_file_copies.begin();
         it != this->pending_file_copies.end(); ++it) {
      request.src->getStorageService();
      request.dst->getStorageService();
    ...
    // Replay
    this->creator_commport->dputMessage(new DataManagerFileCopyAnswerMessage(
        msg->src, msg->dst, msg->success, std::move(msg->failure_cause)));
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<
                 DataMovementManagerFileReaderThreadMessage>(message)) {
    // Remove the record and find the File Registry Service, if any
    ReadRequestSpecs request(msg->location, msg->num_bytes);
    for (auto it = this->pending_file_reads.begin();
         it != this->pending_file_reads.end(); ++it) {
      if (*(*it) == request) {
    ...
    // Forward it back
    this->creator_commport->dputMessage(new DataManagerFileReadAnswerMessage(
        msg->location, msg->num_bytes, msg->success,
        std::move(msg->failure_cause)));
    return true;

  } else if (auto msg = std::dynamic_pointer_cast<
                 DataMovementManagerFileWriterThreadMessage>(message)) {

    // Remove the record and find the File Registry Service, if any
    DataMovementManager::WriteRequestSpecs request(msg->location, 0);
    for (auto it = this->pending_file_writes.begin();
         it != this->pending_file_writes.end(); ++it) {
      if (*(*it) == request) {
    ...
    // Forward it back
    this->creator_commport->dputMessage(new DataManagerFileWriteAnswerMessage(
        msg->location, msg->success, std::move(msg->failure_cause)));
    return true;

  } else {
    throw std::runtime_error(
        "DataMovementManager::waitForNextMessage(): Unexpected [" +
