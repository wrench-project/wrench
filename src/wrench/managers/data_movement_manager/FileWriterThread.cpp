...
WRENCH_LOG_CATEGORY(wrench_core_data_movement_manager_file_writer_thread,
                    "Log category for Data Movement Manager FileWriterThread");
...
FileWriterThread::FileWriterThread(std::string hostname,
    ...
    : Service(hostname, "file_writer_thread") {
...
int FileWriterThread::main() {
  ...
  try {
    ...
    msg = new DataMovementManagerFileWriterThreadMessage(this->location, true,
                                                         nullptr);
  } catch (ExecutionException &e) {
    msg = new DataMovementManagerFileWriterThreadMessage(this->location, false,
                                                         e.getCause());
  }
