...
WRENCH_LOG_CATEGORY(wrench_core_data_movement_manager_file_reader_thread,
                    "Log category for Data Movement Manager FileReaderThread");
...
FileReaderThread::FileReaderThread(std::string hostname,
                                   ...
                                   double num_bytes)
    : Service(hostname, "file_reader_thread") {
...
int FileReaderThread::main() {
  ...
  try {
    ...
    msg = new DataMovementManagerFileReaderThreadMessage(
        this->location, this->num_bytes, true, nullptr);
  } catch (ExecutionException &e) {
    msg = new DataMovementManagerFileReaderThreadMessage(
        this->location, this->num_bytes, false, e.getCause());
  }
