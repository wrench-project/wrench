 ...
 */
JobManager::JobManager(std::string hostname, S4U_CommPort *creator_commport)
...
void JobManager::stop() {
  this->commport->putMessage(new ServiceStopDaemonMessage(
      nullptr, false, ComputeService::TerminationCause::TERMINATION_NONE, 0.0));
}

/**
 * @brief Create a standard job
 *
 * @param tasks: a list of tasks (which must be either READY, or children of
 * COMPLETED tasks or of tasks also included in the standard job)
 * @param file_locations: a map that specifies locations where input/output
 * files, if any, should be read/written. When empty, it is assumed that the
 * ComputeService's scratch storage space will be used.
 * @param pre_file_copies: a vector of tuples that specify which file copy
 * operations should be completed before task executions begin.
 * @param post_file_copies: a vector of tuples that specify which file copy
 * operations should be completed after task executions end.
 * @param cleanup_file_deletions: a vector of file tuples that specify file
 * deletion operations that should be completed at the end of the job.
...
std::shared_ptr<StandardJob> JobManager::createStandardJob(
  ...
  return createStandardJob(
      tasks, file_locations_vector, std::move(pre_file_copies),
      std::move(post_file_copies), std::move(cleanup_file_deletions));
}
 ...
 * @param tasks: a list of tasks (which must be either READY, or children of
 * COMPLETED tasks or of tasks also included in the standard job)
 * @param file_locations: a map that specifies, for each file, a list of
 * locations, in preference order, where input/output files should be
 * read/written. When unspecified, it is assumed that the ComputeService's
 * scratch storage space will be used.
 * @param pre_file_copies: a vector of tuples that specify which file copy
 * operations should be completed before task executions begin.
 * @param post_file_copies: a vector of tuples that specify which file copy
 * operations should be completed after task executions end.
...
std::shared_ptr<StandardJob> JobManager::createStandardJob(
  ...
  for (const auto &t : tasks) {
    if (t == nullptr) {
      throw std::invalid_argument(
          "JobManager::createStandardJob(): nullptr task in the task vector");
    }
  }

  for (auto const &fl : file_locations) {
    if (fl.first == nullptr) {
      throw std::invalid_argument("JobManager::createStandardJob(): nullptr "
                                  "workflow file in the file_locations map");
    }
    if (fl.second.empty()) {
      throw std::invalid_argument("JobManager::createStandardJob(): empty "
                                  "location vector in the file_locations map");
    }
    for (auto const &fl_l : fl.second) {
      if (fl_l == nullptr) {
        throw std::invalid_argument("JobManager::createStandardJob(): nullptr "
                                    "file location in the file_locations map");
      }
    }
  }
  ...
  for (auto fc : pre_file_copies) {
    if (std::get<0>(fc) == nullptr) {
      throw std::invalid_argument("JobManager::createStandardJob(): nullptr "
                                  "src location in the pre_file_copies set");
    }
    if (std::get<1>(fc) == nullptr) {
      throw std::invalid_argument("JobManager::createStandardJob(): nullptr "
                                  "dst location in the pre_file_copies set");
    }
    if ((std::get<0>(fc)->isScratch()) and (std::get<1>(fc)->isScratch())) {
      throw std::invalid_argument(
          "JobManager::createStandardJob(): cannot have FileLocation::SCRATCH "
          "as both source and "
  ...
  for (auto fc : post_file_copies) {
    if (std::get<0>(fc) == nullptr) {
      throw std::invalid_argument("JobManager::createStandardJob(): nullptr "
                                  "src location in the post_file_copies set");
    }
    if (std::get<1>(fc) == nullptr) {
      throw std::invalid_argument("JobManager::createStandardJob(): nullptr "
                                  "dst location in the post_file_copies set");
    }
    if ((std::get<0>(fc)->isScratch()) and (std::get<1>(fc)->isScratch())) {
      throw std::invalid_argument(
          "JobManager::createStandardJob(): cannot have FileLocation::SCRATCH "
  ...
  for (auto const &fd : cleanup_file_deletions) {
    if (fd == nullptr) {
      throw std::invalid_argument(
          "JobManager::createStandardJob(): nullptr file location in the "
          "cleanup_file_deletions set");
    }
  }

  auto job = std::shared_ptr<StandardJob>(new StandardJob(
      this->getSharedPtr<JobManager>(), tasks, file_locations, pre_file_copies,
 ...
 *
 * @param tasks: a list of tasks  (which must be either READY, or children of
 * COMPLETED tasks or of tasks also included in the list)
 * @param file_locations: a map that specifies locations where files, if any,
 * should be read/written. When empty, it is assumed that the ComputeService's
 * scratch storage space will be used.
 *
 * @return the standard job
 *
...
std::shared_ptr<StandardJob> JobManager::createStandardJob(
    ...
    const std::map<std::shared_ptr<DataFile>, std::shared_ptr<FileLocation>>
        &file_locations) {
  if (tasks.empty()) {
    throw std::invalid_argument("JobManager::createStandardJob(): Invalid "
                                "arguments (empty tasks argument!)");
  }
 ...
 * @param file_locations: a map that specifies locations where input/output
 * files should be read/written. When unspecified, it is assumed that the
 * ComputeService's scratch storage space will be used.
 *
 * @return the standard job
 *
...
std::shared_ptr<StandardJob> JobManager::createStandardJob(
  ...
  if (task == nullptr) {
    throw std::invalid_argument(
        "JobManager::createStandardJob(): Invalid arguments");
  }
...
JobManager::createStandardJob(const std::shared_ptr<WorkflowTask> &task) {
  if (task == nullptr) {
    throw std::invalid_argument(
        "JobManager::createStandardJob(): Invalid arguments");
  }
  ...
  tasks.push_back(task);
  return this->createStandardJob(
      tasks, std::map<std::shared_ptr<DataFile>,
                      std::vector<std::shared_ptr<FileLocation>>>{});
}
...
std::shared_ptr<PilotJob> JobManager::createPilotJob() {
  auto job =
      std::shared_ptr<PilotJob>(new PilotJob(this->getSharedPtr<JobManager>()));
  return job;
}
 ...
 * ...}
 *           - If no value is provided for a task, then the service will choose
 * a host and use as many cores as possible on that host.
 *           - If a "" value is provided for a task, then the service will
 * choose a host and use as many cores as possible on that host.
 *           - If a "hostname" value is provided for a task, then the service
 * will run the task on that host, using as many of its cores as possible
 *           - If a "num_cores" value is provided for a task, then the service
 * will run that task with this many cores, but will choose the host on which to
 * run it.
 *           - If a "hostname:num_cores" value is provided for a task, then the
 * service will run that task with the specified number of cores on that host.
 *      - to a BatchComputeService: {{"-t":"<int>" (requested number of
 * seconds)},{"-N":"<int>" (number of requested hosts)},{"-c":"<int>" (number of
 * requested cores per host)}[,{"taskID":"[node_index:]num_cores"}]
 * [,{"-u":"<string>" (username)}]}
 *      - to a VirtualizedClusterComputeService: {} (jobs should not be
 * submitted directly to the service)}
 *      - to a CloudComputeService: {} (jobs should not be submitted directly to
 * the service)}
 *      - to a HTCondorComputeService:
 *           - For a "grid universe" job that will be submitted to a child
 * BatchComputeService: {{"-universe":"grid", {"-t":"<int>" (requested number of
 * seconds)},{"-N":"<int>" (number of requested hosts)},{"-c":"<int>" (number of
 * requested cores per host)}[,{"-service":"<string>" (BatchComputeService
 * service name)}] [, {"taskID":"[node_index:]num_cores"}] [, {"-u":"<string>"
 * (username)}]}
 *           - For a "non-grid universe" job that will be submitted to a child
...
void JobManager::submitJob(
  ...
  if (job->already_submitted_to_job_manager) {
    throw std::invalid_argument(
        "JobManager::submitJob(): Job was previously submitted");
  }
  ...
  if (not compute_service->supportsStandardJobs()) {
    throw std::invalid_argument(
        "JobManager::submitJob(): service does not support standard jobs");
  }
  ...
  for (const auto &t : job->tasks) {
    if ((t->getState() == WorkflowTask::State::COMPLETED) or
      ...
      throw std::invalid_argument("JobManager()::submitJob(): task " +
                                  t->getID() +
                                  " cannot be submitted as part of a standard "
                                  "job because its state is " +
  ...
  if (job->usesScratch()) {
    try {
      ...
      throw std::invalid_argument(
          "JobManager()::submitJob(): Job's use of scratch is invalid: " +
          std::string(e.what()));
    }
  }
  ...
  for (const auto &arg : service_specific_args) {
    ...
    if (arg.first.rfind("-", 0) == 0) {
      ...
      if (workflow == nullptr) {
        throw std::invalid_argument(
            "JobManager::submitJob():  invalid service-specific argument {" +
            arg.first + "," + arg.second + "} (unknown task ID " + arg.first +
            ")");
      }
  ...
  try {
    compute_service->validateServiceSpecificArguments(job->compound_job,
                                                      new_args);
  } catch (ExecutionException &e) {
    job->compound_job = nullptr;
    if (std::dynamic_pointer_cast<NotEnoughResources>(e.getCause())) {
      throw ExecutionException(
          std::make_shared<NotEnoughResources>(job, compute_service));
    } else {
      throw;
    }
  } catch (std::invalid_argument &e) {
 ...
 *      - to a BatchComputeService: {{"-t":"<int>" (requested number of
 * seconds)},{"-N":"<int>" (number of requested hosts)},{"-c":"<int>" (number of
 * requested cores per host)}[,{"actionID":"[node_index:]num_cores"}]
 * [,{"-u":"<string>" (username)}]}
 *      - to a VirtualizedClusterComputeService: {} (jobs should not be
 * submitted directly to the service)}
 *      - to a CloudComputeService: {} (jobs should not be submitted directly to
 ...
 * seconds)},{"-N":"<int>" (number of requested hosts)},{"-c":"<int>" (number of
 * requested cores per host)}[,{"-service":"<string>" (BatchComputeService
 * service name)}] [, {"actionID":"[node_index:]num_cores"}] [, {"-u":"<string>"
 * (username)}]}
 *           - For a "non-grid universe" job that will be submitted to a child
 * BareMetalComputeService: {}
 *
...
void JobManager::submitJob(
  ...
  if (job->actions.empty()) {
    throw std::invalid_argument("JobManager::submitJob(): Cannot submit a job "
                                "that doesn't have any actions");
  }
  ...
  if (not compute_service->supportsCompoundJobs()) {
    throw std::invalid_argument(
        "JobManager::submitJob(): service does not support compound jobs");
  }

  try {
    compute_service->validateServiceSpecificArguments(job,
 ...
 *      - to a BatchComputeService: {"-t":"<int>" (requested number of
 * seconds)},{"-N":"<int>" (number of requested hosts)},{"-c":"<int>" (number of
 * requested cores per host)}
 *      - to a BareMetalComputeService: {} (pilot jobs should not be submitted
 * directly to the service)}
 *      - to a VirtualizedClusterComputeService: {} (pilot jobs should not be
 * submitted directly to the service)}
 *      - to a CloudComputeService: {} (pilot jobs should not be submitted
 * directly to the service)}
 *      - to a HTCondorComputeService: {} (pilot jobs should be be submitted
 * directly to the service)
 *
 * @throw std::invalid_argument
 * @throw ExecutionException
...
void JobManager::submitJob(
  ...
  if (not compute_service->supportsPilotJobs()) {
    throw std::invalid_argument(
        "JobManager::submitJob(): service does not support pilot jobs");
  }

  auto callback_commport = this->commport;
  std::shared_ptr<CompoundJob> cjob =
      this->createCompoundJob("cjob_for_" + this->getName());
  cjob->addCustomAction(
      "pilot_job_", 0, 0,
      [callback_commport, job,
       compute_service](const std::shared_ptr<ActionExecutor> &executor) {
        ...
        auto bm_cs = std::shared_ptr<BareMetalComputeService>(
            new BareMetalComputeService(
                executor->hostname, specified_compute_resources, {}, {},
                nullptr, "_one_shot_bm",
                std::dynamic_pointer_cast<ComputeService>(
                    execution_service->getParentService())
                    ->getScratch()));
        ...
        // Send a call back
        callback_commport->dputMessage(new ComputeServicePilotJobStartedMessage(
            job, compute_service,
      ...
      [job](const std::shared_ptr<ActionExecutor> &executor) {
        job->compute_service->stop(
            true, ComputeService::TerminationCause::TERMINATION_JOB_TIMEOUT);
      });
...
void JobManager::terminateJob(const std::shared_ptr<StandardJob> &job) {
  ...
  if (job->getParentComputeService() == nullptr) {
    std::string err_msg = "Job cannot be terminated because it doesn't have a "
                          "parent compute service";
    throw ExecutionException(
        std::shared_ptr<FailureCause>(new NotAllowed(nullptr, err_msg)));
  }
  ...
  case StandardJob::State::NOT_SUBMITTED: {
    std::string err_msg =
        "job cannot be terminated because it's not pending/running";
    throw ExecutionException(
        std::shared_ptr<FailureCause>(new NotAllowed(nullptr, err_msg)));
  }
  default:
  ...
  this->acquireDaemonLock();
  auto it = std::find(this->jobs_to_dispatch.begin(),
                      this->jobs_to_dispatch.end(), job->compound_job);
  ...
  job->processCompoundJobOutcome(state_changes, failure_count_increments,
                                 job_failure_cause, this->simulation);
...
bool JobManager::processNextMessage() {
  ...
  if (std::dynamic_pointer_cast<JobManagerWakeupMessage>(message)) {
    ...
    } else if (this->cjob_to_pjob_map.find(msg->job) !=
               this->cjob_to_pjob_map.end()) {
      auto pjob = this->cjob_to_pjob_map[msg->job];
      auto pjob_action = *(msg->job->getActions().begin());
      if (std::dynamic_pointer_cast<JobTimeout>(
              pjob_action->getFailureCause())) {
        processPilotJobExpiration(pjob, msg->compute_service);
      } else {
        throw std::runtime_error("JobManager::processNextMessage(): Received "
                                 "unexpected pilot job failure cause " +
                                 pjob_action->getFailureCause()->toString());
        //                    processPilotJobFailure(pjob, msg->compute_service,
        //                    pjob_action->getFailureCause());
      }
    } else {
    ...
    return true;
                 ...
                 ComputeServicePilotJobStartedMessage>(message)) {
    processPilotJobStart(msg->job, msg->compute_service);
    return true;
  } else if (auto msg = std::dynamic_pointer_cast<
                 ComputeServicePilotJobExpiredMessage>(message)) {
    processPilotJobExpiration(msg->job, msg->compute_service);
    return true;
  } else {
    throw std::runtime_error("JobManager::main(): Unexpected [" +
 ...
 */
void JobManager::processStandardJobCompletion(
    const std::shared_ptr<StandardJob> &job,
  ...
  //        throw std::runtime_error("PROCESS STANDARD JOB COMPLETION NOT
  //        IMPLEMENTED");
}
 ...
 */
void JobManager::processStandardJobFailure(
    const std::shared_ptr<StandardJob> &job,
    std::shared_ptr<ComputeService> compute_service) {
  // update job state
  ...
  // Fix the failure cause in case it's a failure cause that refers to a job (in
  // which case it is right now referring to the compound job instead of the
  // standard job
  if (auto actual_cause =
          std::dynamic_pointer_cast<JobKilled>(job_failure_cause)) {
    job_failure_cause = std::make_shared<JobKilled>(job);
  } else if (auto actual_cause =
                 std::dynamic_pointer_cast<JobTimeout>(job_failure_cause)) {
    job_failure_cause = std::make_shared<JobTimeout>(job);
  } else if (auto actual_cause = std::dynamic_pointer_cast<NotEnoughResources>(
                 job_failure_cause)) {
    job_failure_cause =
        std::make_shared<NotEnoughResources>(job, actual_cause->getService());
  }
  ...
  // Forward the notification along the notification chain
  auto augmented_message = new JobManagerStandardJobFailedMessage(
      job, std::move(compute_service), state_changes, failure_count_increments,
...
void JobManager::processPilotJobStart(
  ...
  WRENCH_INFO("Forwarding to %s",
              job->getOriginCallbackCommPort()->get_cname());
  job->getOriginCallbackCommPort()->dputMessage(
      new ComputeServicePilotJobStartedMessage(job, std::move(compute_service),
                                               0.0));
}
 ...
 */
void JobManager::processPilotJobExpiration(
    const std::shared_ptr<PilotJob> &job,
    std::shared_ptr<ComputeService> compute_service) {
  // update job state
  ...
  WRENCH_INFO("Forwarding to %s",
              job->getOriginCallbackCommPort()->get_cname());
  job->getOriginCallbackCommPort()->dputMessage(
      new ComputeServicePilotJobExpiredMessage(job, std::move(compute_service),
                                               0.0));
}
...
//  */
//    void JobManager::processPilotJobFailure(const std::shared_ptr<PilotJob>
//    &job,
...
//        this->num_running_pilot_jobs--;
//
//        // Remove the job from the "dispatched" list and put it in the
//        completed list this->jobs_dispatched.erase(job->compound_job);
//
//        // Forward the notification to the source
//        WRENCH_INFO("Forwarding to %s",
//        job->getOriginCallbackCommPort()->get_cname());
//        S4U_CommPort::dputMessage(job->getOriginCallbackCommPort(),
//                                 new ComputeServicePilotJobFailedMessage(job,
//                                 std::move(compute_service), std::move(cause),
//                                 0.0));
//    }

/**
 * @brief Create a Compound job
 * @param name: the job's name (if empty, a unique job name will be picked for
 * you)
 * @return the job
 */
std::shared_ptr<CompoundJob> JobManager::createCompoundJob(std::string name) {
  auto job = std::shared_ptr<CompoundJob>(
      new CompoundJob(std::move(name), this->getSharedPtr<JobManager>()));
  return job;
}
    ...
    try {
      ...
      if (auto cjob = std::dynamic_pointer_cast<CompoundJob>(job)) {
        if (this->cjob_to_sjob_map.find(cjob) == this->cjob_to_sjob_map.end()) {
          ...
          try {
            auto message = new JobManagerCompoundJobFailedMessage(
                cjob, cjob->parent_compute_service, e.getCause());
          ...
          try {
            auto message = new JobManagerStandardJobFailedMessage(
                sjob, sjob->parent_compute_service, state_changes,
                failure_count_increments, e.getCause());
            job->popCallbackCommPort()->dputMessage(message);
          } catch (NetworkError &e) {
          }
        }
...
void JobManager::dispatchJob(const std::shared_ptr<CompoundJob> &job) {
  ...
  try {
    ...
    job->parent_compute_service->submitJob(job,
                                           job->getServiceSpecificArguments());
    if (this->cjob_to_pjob_map.find(job) != this->cjob_to_pjob_map.end()) {
      this->cjob_to_pjob_map[job]->state = PilotJob::State::PENDING;
    } else if (this->cjob_to_sjob_map.find(job) !=
               this->cjob_to_sjob_map.end()) {
 ...
 */
void JobManager::processCompoundJobCompletion(
    const std::shared_ptr<CompoundJob> &job,
    std::shared_ptr<ComputeService> compute_service) {
  ...
  // Forward the notification along the notification chain
  try {
    auto message = new JobManagerCompoundJobCompletedMessage(
        job, std::move(compute_service));
    job->popCallbackCommPort()->dputMessage(message);
  } catch (NetworkError &e) {
  }
}
 ...
 * @param compute_service: the compute service on which the job completed
 */
void JobManager::processCompoundJobFailure(
    const std::shared_ptr<CompoundJob> &job,
    std::shared_ptr<ComputeService> compute_service) {
  job->state = CompoundJob::State::DISCONTINUED;
  job->end_date = Simulation::getCurrentSimulatedDate();
