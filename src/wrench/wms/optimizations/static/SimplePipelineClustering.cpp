	...
	void SimplePipelineClustering::process(Workflow *workflow) {
		...
		for (auto task : workflow->getTasks()) {
			if (task->getNumberOfChildren() == 1 && task->getNumberOfParents() == 1 &&
				...
				std::set<WorkflowTask *> cluster_tasks;
				cluster_tasks.insert(task);
				int task_count = 1;
				...
				if (task_count > 1) {
					std::cout << "ADDING TASK: " << task_id << " - " << task_flops << " - " << task_num_procs << std::endl;
