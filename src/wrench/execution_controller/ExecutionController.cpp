...
WRENCH_LOG_CATEGORY(wrench_core_execution_controller,
                    "Log category for Execution Controller");
 ...
 * @param suffix: a string to append to the controller process name (which will
 * show up in logs)
 *
 * @throw std::invalid_argument
 */
ExecutionController::ExecutionController(const std::string &hostname,
...
std::shared_ptr<JobManager> ExecutionController::createJobManager() {
  ...
  job_manager->start(job_manager, true,
                     false); // Always daemonize, no auto-restart
 ...
 */
std::shared_ptr<DataMovementManager>
ExecutionController::createDataMovementManager() {
 ...
 */
std::shared_ptr<EnergyMeterService> ExecutionController::createEnergyMeter(
    const std::map<std::string, double> &measurement_periods) {
  auto energy_meter_raw_ptr =
      new EnergyMeterService(this->hostname, measurement_periods);
  std::shared_ptr<EnergyMeterService> energy_meter =
      std::shared_ptr<EnergyMeterService>(energy_meter_raw_ptr);
  energy_meter->setSimulation(this->simulation);
  energy_meter->start(energy_meter, true,
                      false); // Always daemonize, no auto-restart
  return energy_meter;
}
 ...
 */
std::shared_ptr<EnergyMeterService> ExecutionController::createEnergyMeter(
    const std::vector<std::string> &hostnames, double measurement_period) {
  auto energy_meter_raw_ptr =
      new EnergyMeterService(this->hostname, hostnames, measurement_period);
  std::shared_ptr<EnergyMeterService> energy_meter =
      std::shared_ptr<EnergyMeterService>(energy_meter_raw_ptr);
  energy_meter->setSimulation(this->simulation);
  energy_meter->start(energy_meter, true,
...
ExecutionController::createBandwidthMeter(
    const std::map<std::string, double> &measurement_periods) {
  auto bandwidth_meter_raw_ptr =
      new BandwidthMeterService(this->hostname, measurement_periods);
  std::shared_ptr<BandwidthMeterService> bandwidth_meter =
      std::shared_ptr<BandwidthMeterService>(bandwidth_meter_raw_ptr);
  bandwidth_meter->setSimulation(this->simulation);
  bandwidth_meter->start(bandwidth_meter, true,
                         false); // Always daemonize, no auto-restart
...
std::shared_ptr<BandwidthMeterService>
ExecutionController::createBandwidthMeter(
    const std::vector<std::string> &link_names, double measurement_period) {
  auto bandwidth_meter_raw_ptr =
      new BandwidthMeterService(this->hostname, link_names, measurement_period);
  std::shared_ptr<BandwidthMeterService> bandwidth_meter =
      std::shared_ptr<BandwidthMeterService>(bandwidth_meter_raw_ptr);
  bandwidth_meter->setSimulation(this->simulation);
  bandwidth_meter->start(bandwidth_meter, true,
...
void ExecutionController::setTimer(double date, std::string message) {
  Alarm::createAndStartAlarm(
      this->simulation, date, this->hostname, this->commport,
...
ExecutionController::waitForNextEvent(double timeout) {
  auto event =
      ExecutionEvent::waitForNextExecutionEvent(this->commport, timeout);
  return event;
}
...
/**
 * @brief Wait for an execution event and then call the associated function to
 * process that event
 */
 ...
 * @param timeout: a timeout value in seconds
 *
 * @return false if a timeout occurred (in which case no event was
 * received/processed)
...
bool ExecutionController::waitForAndProcessNextEvent(double timeout) {
  ...
  if (auto real_event =
          std::dynamic_pointer_cast<CompoundJobCompletedEvent>(event)) {
    processEventCompoundJobCompletion(real_event);
  } else if (auto real_event =
                 std::dynamic_pointer_cast<CompoundJobFailedEvent>(event)) {
    processEventCompoundJobFailure(real_event);
  } else if (auto real_event =
                 std::dynamic_pointer_cast<StandardJobCompletedEvent>(event)) {
    processEventStandardJobCompletion(real_event);
  } else if (auto real_event =
                 std::dynamic_pointer_cast<StandardJobFailedEvent>(event)) {
    processEventStandardJobFailure(real_event);
  } else if (auto real_event =
                 std::dynamic_pointer_cast<PilotJobStartedEvent>(event)) {
    processEventPilotJobStart(real_event);
  } else if (auto real_event =
                 std::dynamic_pointer_cast<PilotJobExpiredEvent>(event)) {
    processEventPilotJobExpiration(real_event);
  } else if (auto real_event =
                 std::dynamic_pointer_cast<FileCopyCompletedEvent>(event)) {
    processEventFileCopyCompletion(real_event);
  } else if (auto real_event =
                 std::dynamic_pointer_cast<FileCopyFailedEvent>(event)) {
    ...
    throw std::runtime_error("SimpleExecutionController::main(): Unknown "
 ...
 */
void ExecutionController::processEventStandardJobCompletion(
    std::shared_ptr<StandardJobCompletedEvent> event) {
  auto standard_job = event->standard_job;
  WRENCH_INFO(
      "In default event-handler: Notified that a %ld-task job has completed",
      standard_job->getNumTasks());
}
 ...
 */
void ExecutionController::processEventStandardJobFailure(
    std::shared_ptr<StandardJobFailedEvent> event) {
  WRENCH_INFO("In default event-handler: Notified that a standard job has "
              "failed (all its tasks are back in the ready state)");
}
 ...
 */
void ExecutionController::processEventPilotJobStart(
    std::shared_ptr<PilotJobStartedEvent> event) {
  WRENCH_INFO(
      "In default event-handler: Notified that a pilot job has started!");
}
 ...
 */
void ExecutionController::processEventPilotJobExpiration(
    std::shared_ptr<PilotJobExpiredEvent> event) {
  WRENCH_INFO(
      "In default event-handler: Notified that a pilot job has expired!");
}
 ...
 */
void ExecutionController::processEventFileCopyCompletion(
    std::shared_ptr<FileCopyCompletedEvent> event) {
  WRENCH_INFO(
      "In default event-handler: Notified that a file copy is completed!");
}
 ...
 */
void ExecutionController::processEventFileCopyFailure(
    std::shared_ptr<FileCopyFailedEvent> event) {
  WRENCH_INFO(
      "In default event-handler: Notified that a file copy has failed!");
}
 ...
 */
void ExecutionController::processEventCompoundJobCompletion(
    std::shared_ptr<CompoundJobCompletedEvent> event) {
  WRENCH_INFO(
      "In default event-handler: Notified that a %zu-action job has completed",
 ...
 */
void ExecutionController::processEventCompoundJobFailure(
    std::shared_ptr<CompoundJobFailedEvent> event) {
  WRENCH_INFO("In default event-handler: Notified that a standard job has "
              "failed (all its tasks are back in the ready state)");
}

/**
 * @brief Make the execution controller daemonized, meaning that the simulation
 * can terminate without
 *
 * @param daemonized: true if the controller should be daemonized, false
 * otherwise
 */
