 ...
 */
Job::Job(const std::string &name, std::shared_ptr<JobManager> job_manager) {
  if (name.empty()) {
 ...
 */
std::string Job::getName() { return this->name; }
 ...
 */
void Job::setParentComputeService(
...
/**
 * @brief Get the date at which the job was last submitted (<0 means "never
 * submitted")
 * @return the submit date
 */
double Job::getSubmitDate() const { return this->submit_date; }

/**
 * @brief Get the date at which the job ended (<0 means "never submitted")
 * @return the end date
 */
double Job::getEndDate() const { return this->end_date; }

/**
 * @brief Return the service-specific arguments that are used during job
 * submission
...
/**
 * @brief Sets the service-specific arguments that are used during job
 * submission
...
/**
 * @brief Get the job's priority (the higher the priority value, the higher the
 * priority)
 * @return the job's priority
 */
double Job::getPriority() const { return this->priority; }

/**
 * @brief Set the job's priority (the higher the priority value, the higher the
 * priority)
 * @param p: a priority
 */
void Job::setPriority(double p) { this->priority = p; }
} // namespace wrench
