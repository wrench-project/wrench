 ...
 *
 * @param tasks: the tasks in the job (which must be either READY, or children
 * of COMPLETED tasks or of tasks also included in the standard job)
 ...
 * @param cleanup_file_deletions: a vector of tuples that specify which file
 * copies should be removed from which locations. This will happen regardless of
...
StandardJob::StandardJob(
    ...
    : Job("", std::move(job_manager)), file_locations(file_locations),
      pre_file_copies(pre_file_copies), post_file_copies(post_file_copies),
...
/**
 * @brief Returns the minimum number of cores required to run the job (i.e., at
 * least one task in the job cannot run if fewer cores than this minimum are
 * available)
...
unsigned long StandardJob::getMinimumRequiredNumCores() const {
  ...
  for (const auto &t : tasks) {
    max_min_num_cores =
        std::max<unsigned long>(max_min_num_cores, t->getMinNumCores());
  }
  return max_min_num_cores;
}

/**
 * @brief Returns the minimum RAM capacity required to run the job (i.e., at
 * least one task in the job cannot run if less ram than this minimum is
 * available)
 * @return the number of cores
 */
 ...
 */
unsigned long StandardJob::getNumTasks() const { return this->tasks.size(); }
 ...
 */
std::map<std::shared_ptr<DataFile>, std::vector<std::shared_ptr<FileLocation>>>
StandardJob::getFileLocations() const {
 ...
 * @return the state
 */
StandardJob::State StandardJob::getState() { return this->state; }
 ...
 */
void StandardJob::createUnderlyingCompoundJob(
  ...
  auto cjob =
      this->job_manager->createCompoundJob("cjob_for_" + this->getName());

  // Create pre- and post-overhead work units
  if (this->getPreJobOverheadInSeconds() > 0.0) {
    pre_overhead_action =
        cjob->addSleepAction("", this->getPreJobOverheadInSeconds());
  }

  if (this->getPostJobOverheadInSeconds() > 0.0) {
    post_overhead_action =
        cjob->addSleepAction("", this->getPostJobOverheadInSeconds());
  }
  ...
  for (auto const &pfc : this->pre_file_copies) {
    ...
    //                src_location =
    //                FileLocation::LOCATION(compute_service->getScratch(),
    //                src_location->getFile());
    //            }
    //            if (dst_location->isScratch()) {
    //                dst_location =
    //                FileLocation::LOCATION(compute_service->getScratch(),
    //                dst_location->getFile());
    //            }

    pre_file_copy_actions.push_back(
        cjob->addFileCopyAction("", src_location, dst_location));
  }
  ...
  for (auto const &pfc : this->post_file_copies) {
    ...
    post_file_copy_actions.push_back(
        cjob->addFileCopyAction("", src_location, dst_location));
  }
  ...
  for (auto const &task : this->tasks) {
    ...
    auto compute_action = cjob->addComputeAction(
        "task_" + task->getID(), task->getFlops(), task->getMemoryRequirement(),
        task->getMinNumCores(), task->getMaxNumCores(),
        task->getParallelModel());
    ...
    for (auto const &f : task->getInputFiles()) {
      ...
      if (this->file_locations.find(f) != this->file_locations.end()) {
        ...
        //                    fixed_locations = this->file_locations[f]; for
        //                    (auto const &loc: this->file_locations[f]) {
    ...
    for (auto const &f : task->getOutputFiles()) {
      ...
      if (this->file_locations.find(f) != this->file_locations.end()) {
        std::vector<std::shared_ptr<FileLocation>> fixed_locations =
            this->file_locations[f];
        //                    for (auto const &loc: this->file_locations[f]) {
        //                        fixed_locations.push_back(loc);
        //                    }
        if (fixed_locations.size() > 1) {
          throw std::runtime_error(
              "StandardJob::createUnderlyingCompoundJob(): Internal WRENCH "
              "error - "
              "there should be a single location for a file write action");
        }
        fwrite_action =
            cjob->addFileWriteAction("", this->file_locations[f].at(0));
      } else {
  ...
  auto lambda_execute =
      [weak_cjob](
        ...
        if (scratch) {
          ...
          scratch->removeDirectory(scratch->getBaseRootPath() +
                                   cjob->getName());
        }
      };
  auto lambda_terminate =
      [](const std::shared_ptr<wrench::ActionExecutor> &action_executor) {};
  scratch_cleanup =
      cjob->addCustomAction("", 0, 0, lambda_execute, lambda_terminate);
  ...
  // Create dummy tasks
  std::shared_ptr<Action> pre_overhead_to_pre_file_copies =
      cjob->addSleepAction("", 0);
  std::shared_ptr<Action> pre_file_copies_to_tasks =
      cjob->addSleepAction("", 0);
  std::shared_ptr<Action> tasks_to_post_file_copies =
      cjob->addSleepAction("", 0);
  std::shared_ptr<Action> tasks_post_file_copies_to_cleanup =
      cjob->addSleepAction("", 0);
  std::shared_ptr<Action> cleanup_to_post_overhead =
      cjob->addSleepAction("", 0);
  cjob->addActionDependency(pre_overhead_to_pre_file_copies,
                            pre_file_copies_to_tasks);
  cjob->addActionDependency(pre_file_copies_to_tasks,
                            tasks_to_post_file_copies);
  cjob->addActionDependency(tasks_to_post_file_copies,
                            tasks_post_file_copies_to_cleanup);
  cjob->addActionDependency(tasks_post_file_copies_to_cleanup,
                            cleanup_to_post_overhead);

  // Add all dependencies, using the dummy tasks to help
  if (pre_overhead_action != nullptr) {
    cjob->addActionDependency(pre_overhead_action,
                              pre_overhead_to_pre_file_copies);
  }
  ...
  std::vector<std::shared_ptr<Action>> dummies = {
      pre_overhead_to_pre_file_copies, pre_file_copies_to_tasks,
      tasks_to_post_file_copies, tasks_post_file_copies_to_cleanup,
      cleanup_to_post_overhead};
...
void StandardJob::analyzeActions(
    const std::vector<std::shared_ptr<Action>> &actions,
    bool *at_least_one_failed, bool *at_least_one_killed,
    std::shared_ptr<FailureCause> *failure_cause, double *earliest_start_date,
    double *latest_end_date, double *earliest_failure_date) {
  ...
  *earliest_failure_date = -1.0;
  ...
  for (const auto &action : actions) {
    //            std::cerr << "   ANALYSING ACTION " << action->getName() << "
    //            END DATE " << action->getEndDate() << "\n";

    // Set the dates
    if ((*earliest_start_date == -1.0) or
        ((action->getStartDate() < *earliest_start_date) and
         (action->getStartDate() != -1.0))) {
      *earliest_start_date = action->getStartDate();
    }
    if ((*latest_end_date == -1.0) or
        ((action->getEndDate() > *latest_end_date) and
         (action->getEndDate() != -1.0))) {
      *latest_end_date = action->getEndDate();
    }

    if (action->getState() == Action::State::FAILED ||
        action->getState() == Action::State::KILLED) {
      *at_least_one_failed = true;
      if (not *failure_cause) {
        *failure_cause = action->getFailureCause();
      }
      if ((*earliest_failure_date == -1.0) or
          ((*earliest_failure_date > action->getEndDate()) and
           (action->getEndDate() != -1.0))) {
...
/**
 * @brief Compute all task updates based on the state of the underlying compound
 * job (also updates timing information and other task information)
 * @param state_changes: the set of task state changes to apply
 * @param failure_count_increments: the set ot task failure count increments to
 * apply
 * @param job_failure_cause: the job failure cause, if any
 * @param simulation: the simulation (to add timestamps!)
 */
void StandardJob::processCompoundJobOutcome(
  ...
  case StandardJob::State::RUNNING:
    throw std::runtime_error("StandardJob::processCompoundJobOutcome(): Cannot "
                             "be called on a RUNNING/PENDING job");
  default:
    break;
  }
#endif

  // At this point all tasks are pending, so no matter what we need to change
  // all states So we provisionally make them all NOT_READY right now, which we
  // may overwrite with COMPLETED, and then and the level above may turn some of
  // the NOT_READY into READY.
  for (auto const &t : this->tasks) {
  ...
  //            std::cerr << "ACTION " << a->getName() << ": STATE=" <<
  //            Action::stateToString(a->getState()) << "\n"; if
  //            (a->getFailureCause()) {
  //                std::cerr << "ACTION " << a->getName() << ": " <<
  //                a->getFailureCause()->toString() << "\n";
  //            } else {
  //                std::cerr << "ACTION " << a->getName() << ": NO FAILURE
  //                CAUSE\n";
  //            }
  //        }
  ...
  if (not this->pre_file_copy_actions.empty()) {
    ...
    this->analyzeActions(this->pre_file_copy_actions, &at_least_one_failed,
                         &at_least_one_killed, &failure_cause,
  ...
  for (auto &t : this->tasks) {
    ...
    std::string execution_host = this->task_compute_actions[t]
                                     ->getExecutionHistory()
                                     .top()
                                     .execution_host;
    ...
    for (auto const &a : this->task_file_read_actions[t]) {
      ...
      case Action::READY:
        break;
      case Action::STARTED:
        simulation->getOutput().addTimestampFileReadStart(
            fra->getStartDate(), fra_file, fra->getUsedFileLocation(),
            fra->getUsedFileLocation()->getStorageService(), t);
        break;
      case Action::COMPLETED:
        simulation->getOutput().addTimestampFileReadStart(
            fra->getStartDate(), fra_file, fra->getUsedFileLocation(),
            fra->getUsedFileLocation()->getStorageService(), t);
        simulation->getOutput().addTimestampFileReadCompletion(
            fra->getEndDate(), fra_file, fra->getUsedFileLocation(),
            fra->getUsedFileLocation()->getStorageService(), t);
        break;
      case Action::KILLED:
      case Action::FAILED:
        simulation->getOutput().addTimestampFileReadStart(
            fra->getStartDate(), fra_file, fra->getUsedFileLocation(),
            fra->getUsedFileLocation()->getStorageService(), t);
        simulation->getOutput().addTimestampFileReadFailure(
            fra->getEndDate(), fra_file, fra->getUsedFileLocation(),
            fra->getUsedFileLocation()->getStorageService(), t);
        break;
      }
    ...
    std::shared_ptr<FailureCause> failure_cause;
    double earliest_start_date, latest_end_date, earliest_failure_date;
    this->analyzeActions(this->task_file_read_actions[t], &at_least_one_failed,
                         &at_least_one_killed, &failure_cause,
                         &earliest_start_date, &latest_end_date,
                         &earliest_failure_date);
    ...
    if (at_least_one_failed or at_least_one_killed) {
      ...
      failure_count_increments.insert(t);
      state_changes[t] =
          WorkflowTask::State::READY; // This may be changed to NOT_READY later
                                      // based on other tasks
      simulation->getOutput().addTimestampTaskStart(earliest_start_date, t);
      if (at_least_one_killed) {
        job_failure_cause = std::make_shared<JobKilled>(this->getSharedPtr());
        t->setTerminationDate(earliest_failure_date);
        simulation->getOutput().addTimestampTaskTermination(
            earliest_failure_date, t);
      } else if (at_least_one_failed) {
        if (job_failure_cause == nullptr)
          job_failure_cause = failure_cause;

        simulation->getOutput().addTimestampTaskFailure(earliest_failure_date,
                                                        t);
      }
      continue;
    }

    t->updateStartDate(earliest_start_date); // could be -1.0 if there were no
                                             // input, but will be updated below
    t->setReadInputStartDate(earliest_start_date);
    ...
    simulation->getOutput().addTimestampTaskStart(
        compute_action->getStartDate(), t);

    t->setNumCoresAllocated(
        compute_action->getExecutionHistory().top().num_cores_allocated);
    ...
    if (compute_action->getState() != Action::State::COMPLETED) {
      if (not job_failure_cause)
        job_failure_cause = compute_action->getFailureCause();
      if (compute_action->getState() == Action::State::KILLED) {
        t->setTerminationDate(compute_action->getEndDate());
        simulation->getOutput().addTimestampTaskTermination(
            compute_action->getEndDate(), t);
      } else {
        simulation->getOutput().addTimestampTaskFailure(
            compute_action->getEndDate(), t);
    ...
    for (auto const &a : this->task_file_write_actions[t]) {
      ...
      case Action::READY:
        break;
      case Action::STARTED:
        simulation->getOutput().addTimestampFileWriteStart(
            fwa->getStartDate(), fwa_file, fwa->getFileLocation(),
            fwa->getFileLocation()->getStorageService(), t);
        break;
      case Action::COMPLETED:
        simulation->getOutput().addTimestampFileWriteStart(
            fwa->getStartDate(), fwa_file, fwa->getFileLocation(),
            fwa->getFileLocation()->getStorageService(), t);
        simulation->getOutput().addTimestampFileWriteCompletion(
            fwa->getEndDate(), fwa_file, fwa->getFileLocation(),
            fwa->getFileLocation()->getStorageService(), t);
        break;
      case Action::KILLED:
      case Action::FAILED:
        simulation->getOutput().addTimestampFileWriteStart(
            fwa->getStartDate(), fwa_file, fwa->getFileLocation(),
            fwa->getFileLocation()->getStorageService(), t);
        simulation->getOutput().addTimestampFileWriteFailure(
            fwa->getEndDate(), fwa_file, fwa->getFileLocation(),
            fwa->getFileLocation()->getStorageService(), t);
        break;
      }
    ...
    this->analyzeActions(this->task_file_write_actions[t], &at_least_one_failed,
                         &at_least_one_killed, &failure_cause,
                         &earliest_start_date, &latest_end_date,
                         &earliest_failure_date);
    ...
    if (earliest_start_date == -1)
      earliest_start_date = compute_action->getEndDate();
    if (latest_end_date == -1)
      latest_end_date = compute_action->getEndDate();
    t->setWriteOutputStartDate(earliest_start_date);
  ...
  if (not this->post_file_copy_actions.empty()) {
    ...
    std::shared_ptr<FailureCause> failure_cause;
    double earliest_start_date, latest_end_date, earliest_failure_date;
    this->analyzeActions(this->post_file_copy_actions, &at_least_one_failed,
                         &at_least_one_killed, &failure_cause,
                         &earliest_start_date, &latest_end_date,
                         &earliest_failure_date);
    if (at_least_one_killed) {
  ...
  if (not this->cleanup_actions.empty()) {
    ...
    std::shared_ptr<FailureCause> failure_cause;
    double earliest_start_date, latest_end_date, earliest_failure_date;
    this->analyzeActions(this->cleanup_actions, &at_least_one_failed,
                         &at_least_one_killed, &failure_cause,
                         &earliest_start_date, &latest_end_date,
                         &earliest_failure_date);
    if (at_least_one_killed) {
 ...
 * @param failure_count_increments: set of tasks whose failure counts should be
 * incremented by one
 */
void StandardJob::applyTaskUpdates(
    map<std::shared_ptr<WorkflowTask>, WorkflowTask::State> &state_changes,
