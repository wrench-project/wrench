 ...
 */
CompoundJob::CompoundJob(std::string name,
 ...
 * @return the state
 */
CompoundJob::State CompoundJob::getState() { return this->state; }
 ...
 * @param name: the action's name (if empty, a unique name will be picked for
 * you)
 * @param sleep_time: the time to sleep, in seconds
 * @return a sleep action
 */
std::shared_ptr<SleepAction>
CompoundJob::addSleepAction(const std::string &name, double sleep_time) {
  auto new_action =
      std::shared_ptr<SleepAction>(new SleepAction(name, sleep_time));
 ...
 */
std::shared_ptr<ComputeAction> CompoundJob::addComputeAction(
    const std::string &name, double flops, double ram,
    unsigned long min_num_cores, unsigned long max_num_cores,
    const std::shared_ptr<ParallelModel> &parallel_model) {
  auto new_action = std::shared_ptr<ComputeAction>(new ComputeAction(
      name, flops, ram, min_num_cores, max_num_cores, parallel_model));
  this->addAction(new_action);
  return new_action;
}
 ...
 */
std::shared_ptr<FileReadAction> CompoundJob::addFileReadAction(
    const std::string &name, const std::shared_ptr<DataFile> &file,
 ...
 */
std::shared_ptr<FileWriteAction> CompoundJob::addFileWriteAction(
    const std::string &name, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<StorageService> &storage_service) {
  return addFileWriteAction(name,
                            FileLocation::LOCATION(storage_service, file));
}
 ...
 */
std::shared_ptr<FileCopyAction> CompoundJob::addFileCopyAction(
    const std::string &name, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<StorageService> &src_storage_service,
    const std::shared_ptr<StorageService> &dst_storage_service) {
  return addFileCopyAction(name,
                           FileLocation::LOCATION(src_storage_service, file),
                           FileLocation::LOCATION(dst_storage_service, file));
}
 ...
 */
std::shared_ptr<FileDeleteAction> CompoundJob::addFileDeleteAction(
    const std::string &name, const std::shared_ptr<DataFile> &file,
    const std::shared_ptr<StorageService> &storage_service) {
  return addFileDeleteAction(name,
                             FileLocation::LOCATION(storage_service, file));
}
/**
 ...
 */
std::shared_ptr<FileReadAction> CompoundJob::addFileReadAction(
    const std::string &name, const std::shared_ptr<FileLocation> &file_location,
    const double num_bytes_to_read) {
  std::vector<std::shared_ptr<FileLocation>> v = {file_location};
 ...
 * @param name: the action's name (if empty, a unique name will be picked for
 * you)
 * @param file_locations: the locations to read the file from (will be tried in
 ...
 * @return
 */
std::shared_ptr<FileRegistryAddEntryAction>
CompoundJob::addFileRegistryAddEntryAction(
    const std::string &name,
 ...
 * @return
 */
std::shared_ptr<FileRegistryDeleteEntryAction>
CompoundJob::addFileRegistryDeleteEntryAction(
    const std::string &name,
    const std::shared_ptr<FileRegistryService> &file_registry,
    const std::shared_ptr<FileLocation> &file_location) {
 ...
 */
std::shared_ptr<CustomAction> CompoundJob::addCustomAction(
 ...
 */
std::shared_ptr<CustomAction>
CompoundJob::addCustomAction(std::shared_ptr<CustomAction> custom_action) {
 ...
 *         use-case for an MPI action is that never runs concurrently with other
 * actions within its job, and that that job is submitted to a
 * BatchComputeService, so that it has a set of resources dedicated to it. If
 * the job is submitted to a BareMetalComputeService, this action will use all
 * of that service's resources, regardless of other running actions/jobs on that
 * service.
 * @param name: the action's name (if empty, a unique name will be picked for
 * you)
 * @param mpi_code: a lambda/function that implements the MPI code that MPI
 * processes should execute
 * @param num_processes: the number of MPI processes that will be started.
 * @param num_cores_per_process: the number of core that each MPI process should
 * use. Note that this is not enforced by the runtime system. If the processes
 * compute with more cores, then they will cause time-sharing on cores.
 * @return an MPI action
 */
std::shared_ptr<MPIAction> CompoundJob::addMPIAction(
    const std::string &name,
    const std::function<
        void(const std::shared_ptr<ExecutionController> &controller)> &mpi_code,
    unsigned long num_processes, unsigned long num_cores_per_process) {
...
/**
 * @brief Add a dependency between two actions (does nothing if dependency
 * already exists)
 * @param parent: the parent action
 * @param child: the child action
 */
void CompoundJob::addActionDependency(const std::shared_ptr<Action> &parent,
                                      const std::shared_ptr<Action> &child) {
  assertJobNotSubmitted();
  if ((parent == nullptr) or (child == nullptr)) {
    throw std::invalid_argument(
        "CompoundJob::addDependency(): Arguments cannot be nullptr");
  }
  if (parent == child) {
    throw std::invalid_argument("CompoundJob::addDependency(): Cannot add a "
                                "dependency between a task and itself");
  }
  if (parent->getJob() != this->getSharedPtr() or
      child->getJob() != this->getSharedPtr()) {
    throw std::invalid_argument(
        "CompoundJob::addDependency(): Both actions muszt belong to this job");
  }
  if (pathExists(child, parent)) {
    throw std::invalid_argument("CompoundJob::addDependency(): Adding this "
...
/**
 * @brief Add a parent job to this job (be careful not to add circular
 * dependencies, which may lead to deadlocks)
...
void CompoundJob::addParentJob(const std::shared_ptr<CompoundJob> &parent) {
  ...
  if (parent == nullptr) {
    throw std::invalid_argument(
        "CompoundJob::addParentJob: Cannot add a nullptr parent");
  }
  if (pathExists(this->getSharedPtr(), parent)) {
    throw std::invalid_argument("CompoundJob::addChildJob(): Adding this "
                                "dependency would create a cycle");
  }
...
/**
 * @brief Add a child job to this job (be careful not to add circular
 * dependencies, which may lead to deadlocks)
...
void CompoundJob::addChildJob(const std::shared_ptr<CompoundJob> &child) {
  ...
  if (child == nullptr) {
    throw std::invalid_argument(
        "CompoundJob::addChildJob: Cannot add a nullptr child");
  }
 ...
 */
void CompoundJob::updateStateActionMap(const std::shared_ptr<Action> &action,
...
void CompoundJob::assertJobNotSubmitted() {
  if (this->state != CompoundJob::State::NOT_SUBMITTED) {
    throw std::runtime_error("CompoundJob::assertJobNotSubmitted(): Cannot "
                             "modify a CompoundJob once it has been submitted");
  }
}
...
void CompoundJob::assertActionNameDoesNotAlreadyExist(const std::string &name) {
  if (this->name_map.find(name) != this->name_map.end()) {
    throw std::runtime_error(
        "CompoundJob::assertActionNameDoesNotAlreadyExist(): Task with name " +
        name + " already exists");
  }
}
...
/**
 * @brief Returns an action in the job's given its name (or throws
...
std::shared_ptr<Action> CompoundJob::getActionByName(const std::string &name) {
  ...
  throw std::invalid_argument(
      "CompoundJob::getActionByName(): No action with name " + name);
}
...
void CompoundJob::printTaskMap() {
  ...
  std::cerr << "\n";
  std::vector<Action::State> states = {
      Action::State::NOT_READY, Action::State::READY,  Action::State::COMPLETED,
      Action::State::FAILED,    Action::State::KILLED, Action::State::STARTED};
  for (auto const &s : states) {
    std::cerr << "   " << Action::stateToString(s)
              << " (" + std::to_string(this->state_task_map[s].size()) + ") ";
...
/**
 * @brief Return whether the job has terminated and has done so with some tasks
 * having failed
 * @return true or false
 */
bool CompoundJob::hasFailed() {
  ...
  return (this->actions.size() == not_ready + completed + killed + failed) &&
         (this->actions.size() != completed);
}

/**
 * @brief Set all actions to FAILED for the same failure cause (e.g., a
 * job-level failure)
 * @param cause: a failure cause
 */
void CompoundJob::setAllActionsFailed(
 ...
 * @return
 */
bool CompoundJob::pathExists(const std::shared_ptr<Action> &a,
 ...
 * @return
 */
bool CompoundJob::pathExists(const std::shared_ptr<CompoundJob> &a,
...
//    /**
//     * @brief Returns a reference to the job's parent jobs, if any (use with
//     caution)
//     * @return a set of jobs (a reference)
//     */
...
void CompoundJob::printActionDependencies() {
  for (auto const &action : this->actions) {
    std::cerr << "* " << Action::getActionTypeAsString(action) << "-"
              << action->getName() << " (" << action->getStateAsString()
              << ")\n";
    if (not action->children.empty()) {
      std::cerr << "  CHILDREN:\n";
      for (auto const &child : action->children) {
        std::cerr << "    - "
                  << Action::getActionTypeAsString(child->getSharedPtr()) << "-"
                  << child->getName() << "\n";
      }
    }
    if (not action->parents.empty()) {
      ...
      for (auto const &parent : action->parents) {
        std::cerr << "    - "
                  << Action::getActionTypeAsString(parent->getSharedPtr())
                  << "-" << parent->getName() << "\n";
      }
    }
  }
...
unsigned long CompoundJob::getMinimumRequiredNumCores() {
  ...
  for (auto const &action : this->actions) {
    min_num_cores =
        (min_num_cores < action->getMinNumCores() ? action->getMinNumCores()
                                                  : min_num_cores);
  }
...
double CompoundJob::getMinimumRequiredMemory() {
  ...
  for (auto const &action : this->actions) {
    min_ram =
        (min_ram < action->getMinRAMFootprint() ? action->getMinRAMFootprint()
                                                : min_ram);
  }
  return min_ram;
}
